...
template <typename PointT, int nbins_> class PCL_EXPORTS CRHAlignment {
  private:
    /** \brief Sorts peaks */
    typedef struct {
        bool operator()(std::pair<float, int> const &a,
                        std::pair<float, int> const &b) {
     ...
     */
    void computeTransformToZAxes(Eigen::Vector3f &centroid,
     ...
     */
    void computeRollTransform(Eigen::Vector3f &centroidInput,
        ...
        Eigen::Affine3f transformRoll(
            Eigen::AngleAxisf(-static_cast<float>(roll_angle * M_PI / 180),
  ...
  public:
     ...
     */
    void align(pcl::PointCloud<pcl::Histogram<nbins_>> &input_ftt,
        ...
        // if the number of peaks is too big, we should try to reduce using
        // siluette matching

        for (size_t i = 0; i < peaks.size(); i++) {
            Eigen::Affine3f rollToRot;
            computeRollTransform(centroid_input_, centroid_target_, peaks[i],
                                 rollToRot);
     ...
     */
    void computeRollAngle(pcl::PointCloud<pcl::Histogram<nbins_>> &input_ftt,
                          pcl::PointCloud<pcl::Histogram<nbins_>> &target_ftt,
                          std::vector<float> &peaks) {
        ...
        for (int i = 2; i < (nbins_); i += 2)
            input_ftt_negate.points[0].histogram[i] =
                -input_ftt_negate.points[0].histogram[i];
        ...
        int k = 0;
        multAB[k].r = input_ftt_negate.points[0].histogram[0] *
                      target_ftt.points[0].histogram[0];
        k++;
        ...
        for (int i = 1; i < cutoff; i += 2, k++) {
            ...
            float tmp =
                sqrtf(multAB[k].r * multAB[k].r + multAB[k].i * multAB[k].i);
        ...
        multAB[nbins_ - 1].r =
            input_ftt_negate.points[0].histogram[nbins_ - 1] *
            target_ftt.points[0].histogram[nbins_ - 1];

        kiss_fft_cfg mycfg =
            kiss_fft_alloc(nr_bins_after_padding, 1, NULL, NULL);
        ...
        for (int i = 0; i < nr_bins_after_padding; i++)
            scored_peaks[i] = std::make_pair(float(invAB[i].r), i);
        ...
        for (int i = 0;
             ...
             !stop;
             i++) {
            if (scored_peaks[i].first >=
                scored_peaks[0].first * accept_threshold_) {
                bool insert = true;

                for (size_t j = 0; j < peaks_indices.size();
                     j++) { // check inserted peaks, first pick always inserted
                    if (std::abs(peaks_indices[j] - scored_peaks[i].second) <=
                            peak_distance ||
                        std::abs(peaks_indices[j] - (scored_peaks[i].second -
                                                     nr_bins_after_padding)) <=
                            peak_distance) {
                        insert = false;
                        break;
                    }
                ...
                if (insert) {
                    ...
                    peaks.push_back(
                        static_cast<float>(scored_peaks[i].second *
