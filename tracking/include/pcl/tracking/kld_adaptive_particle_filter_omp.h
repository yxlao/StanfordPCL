...
namespace pcl {
namespace tracking {
/** \brief @b KLDAdaptiveParticleFilterOMPTracker tracks the PointCloud which is
  ...
  sampling [D. Fox, NIPS-01], [D.Fox, IJRR03]. and the computation of the
  weights of the particles is parallelized using OpenMP.
  * \author Ryohei Ueda
  * \ingroup tracking
  */
...
class KLDAdaptiveParticleFilterOMPTracker
  ...
  public:
    ...
    using KLDAdaptiveParticleFilterTracker<PointInT,
                                           StateT>::change_detector_interval_;
    using KLDAdaptiveParticleFilterTracker<PointInT,
                                           StateT>::use_change_detector_;
    ...
    using KLDAdaptiveParticleFilterTracker<PointInT,
                                           StateT>::change_detector_filter_;
    using KLDAdaptiveParticleFilterTracker<PointInT,
                                           StateT>::transed_reference_vector_;
    ...
    using KLDAdaptiveParticleFilterTracker<PointInT,
                                           StateT>::normalizeParticleWeight;
