...
/** \brief @b ParticleFilterTracker tracks the PointCloud which is given by
    setReferenceCloud within the measured PointCloud using particle filter
  method.
  * \author Ryohei Ueda
  * \ingroup tracking
  */
...
class ParticleFilterTracker : public Tracker<PointInT, StateT> {
  ...
  public:
    ...
    ParticleFilterTracker()
        : iteration_num_(1), particle_num_(), min_indices_(1), ref_(),
          particles_(), coherence_(), step_noise_covariance_(),
          initial_noise_covariance_(), initial_noise_mean_(),
          resample_likelihood_thr_(0.0), occlusion_angle_thr_(M_PI / 2.0),
          alpha_(15.0), representative_state_(), trans_(), use_normal_(false),
          motion_(), motion_ratio_(0.25), pass_x_(), pass_y_(), pass_z_(),
          transed_reference_vector_(), change_detector_(), changed_(false),
          change_counter_(0), change_detector_filter_(10),
          change_detector_interval_(10), change_detector_resolution_(0.01),
    ...
    /** \brief set the covariance of the initial noise.
      ...
      * \param initial_noise_covariance the diagonal elements of covariance
      matrix of initial noise.
      */
    inline void setInitialNoiseCovariance(
        const std::vector<double> &initial_noise_covariance) {
     ...
     */
    inline void setResampleLikelihoodThr(const double resample_likelihood_thr) {
        resample_likelihood_thr_ = resample_likelihood_thr;
    }

    /** \brief set the threshold of angle to be considered occlusion (default:
      pi/2). ParticleFilterTracker does not take the occluded points into
      account according to the angle between the normal and the position.
      * \param occlusion_angle_thr threshold of angle to be considered
      occlusion.
      */
    inline void setOcclusionAngleThe(const double occlusion_angle_thr) {
    ...
    /** \brief set the minimum number of indices (default: 1).
      ...
      */
    inline void setMinIndices(const int min_indices) {
        min_indices_ = min_indices;
    }

    /** \brief set the transformation from the world coordinates to the frame of
     * the particles. \param trans Affine transformation from the
     * worldcoordinates to the frame of the particles.
     */
    ...
    /** \brief convert a state to affine transformation from the world
     * coordinates frame. \param particle an instance of StateT.
     */
     ...
     */
    inline double normalizeParticleWeight(double w, double w_min,
                                          double w_max) {
     ...
     */
    inline void setUseChangeDetector(bool use_change_detector) {
     ...
     */
    inline void setMotionRatio(double motion_ratio) {
     ...
     * become serialized in change detection \param change_detector_filter the
     * minimum amount of points required within leaf node
     */
    inline void
     ...
     */
    inline void setResolutionOfChangeDetection(double resolution) {
        change_detector_resolution_ = resolution;
    }

    /** \brief get the resolution of change detection. */
    inline double getResolutionOfChangeDetection() {
    ...
    inline double getFitRatio() const { return fit_ratio_; }
  ...
  protected:
    /** \brief compute the parameters for the bounding box of
      ...
      */
    void calcBoundingBox(double &x_min, double &x_max, double &y_min,
    ...
    /** \brief crop the pointcloud by the bounding box calculated
      ...
      */
    void cropInputPointCloud(const PointCloudInConstPtr &cloud,
                             PointCloudIn &output);
    ...
    /** \brief resampling phase of particle filter method.
        sampling the particles according to the weights calculated in weight
       method. in particular, "sample with replacement" is archieved by walker's
       alias method.
      */
    virtual void resample();

    /** \brief calculate the weighted mean of the particles and set it as the
     * result */
    virtual void update();
    ...
    /** \brief initialize the particles. initial_noise_covariance_ and
       initial_noise_mean_ are used for gausiaan sampling.
      */
    ...
    /** \brief implementation of "sample with replacement" using Walker's alias
       method. about Walker's alias method, you can check the paper below:
        @article{355749,
     ...
     */
    int sampleWithReplacement(const std::vector<int> &a,
                              const std::vector<double> &q);

    /** \brief generate the tables for walker's alias method */
    void genAliasTable(std::vector<int> &a, std::vector<double> &q,
    ...
    boost::shared_ptr<pcl::octree::OctreePointCloudChangeDetector<PointInT>>
        change_detector_;
