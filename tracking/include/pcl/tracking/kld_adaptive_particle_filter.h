...
/** \brief @b KLDAdaptiveParticleFilterTracker tracks the PointCloud which is
  given by setReferenceCloud within the measured PointCloud using particle
  filter method. The number of the particles changes adaptively based on KLD
  sampling [D. Fox, NIPS-01], [D.Fox, IJRR03].
  * \author Ryohei Ueda
  * \ingroup tracking
  */
...
class KLDAdaptiveParticleFilterTracker
  ...
  public:
    ...
    KLDAdaptiveParticleFilterTracker()
        : ParticleFilterTracker<PointInT, StateT>(), maximum_particle_number_(),
          epsilon_(0), delta_(0.99), bin_size_() {
     ...
     */
    inline void setMaximumParticleNum(unsigned int nr) {
        maximum_particle_number_ = nr;
    }

    /** \brief get the maximum number of the particles.*/
    inline unsigned int getMaximumParticleNum() const {
  ...
  protected:
    ...
    /** \brief calculate K-L boundary. K-L boundary follows 1/2e*chi(k-1,
     * 1-d)^2. \param[in] k the number of bins and the first parameter of chi
     * distribution.
     */
    virtual double calcKLBound(int k) {
        double z = normalQuantile(delta_);
        double chi =
            1.0 - 2.0 / (9.0 * (k - 1)) + sqrt(2.0 / (9.0 * (k - 1))) * z;
        return ((k - 1.0) / (2.0 * epsilon_) * chi * chi * chi);
    }

    /** \brief insert a bin into the set of the bins. if that bin is already
      registered, return false. if not, return true.
      * \param bin a bin to be inserted.
      * \param B a set of the bins
      */
    virtual bool insertIntoBins(std::vector<int> bin,
