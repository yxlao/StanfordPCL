...
bool pcl::tracking::KLDAdaptiveParticleFilterTracker<PointInT,
                                                     StateT>::initCompute() {
...
template <typename PointInT, typename StateT>
bool pcl::tracking::KLDAdaptiveParticleFilterTracker<
    PointInT, StateT>::insertIntoBins(std::vector<int> bin,
...
template <typename PointInT, typename StateT>
void pcl::tracking::KLDAdaptiveParticleFilterTracker<PointInT,
                                                     StateT>::resample() {
    unsigned int k = 0;
    unsigned int n = 0;
...
#define PCL_INSTANTIATE_KLDAdaptiveParticleFilterTracker(T, ST)                \
    template class PCL_EXPORTS                                                 \
        pcl::tracking::KLDAdaptiveParticleFilterTracker<T, ST>;
