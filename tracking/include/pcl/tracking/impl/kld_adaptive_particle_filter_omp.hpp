...
void pcl::tracking::KLDAdaptiveParticleFilterOMPTracker<PointInT,
                                                        StateT>::weight() {
    if (!use_normal_) {
#ifdef _OPENMP
#pragma omp parallel for num_threads(threads_)
        ...
        for (int i = 0; i < particle_num_; i++)
            this->computeTransformedPointCloudWithoutNormal(
        ...
        if (change_counter_ == 0) {
            ...
            if (!use_change_detector_ ||
                this->testChangeDetection(coherence_input)) {
                changed_ = true;
...
#pragma omp parallel for num_threads(threads_)
        ...
        for (int i = 0; i < particle_num_; i++) {
            this->computeTransformedPointCloudWithNormal(
                particles_->points[i], *indices_list[i],
...
#define PCL_INSTANTIATE_KLDAdaptiveParticleFilterOMPTracker(T, ST)             \
    template class PCL_EXPORTS                                                 \
        pcl::tracking::KLDAdaptiveParticleFilterOMPTracker<T, ST>;
