...
double PointCoherence<PointInT>::compute(PointInT &source, PointInT &target) {
    return computeCoherence(source, target);
}

template <typename PointInT>
double PointCloudCoherence<PointInT>::calcPointCoherence(PointInT &source,
                                                         PointInT &target) {
    double val = 0.0;
...
template <typename PointInT> bool PointCloudCoherence<PointInT>::initCompute() {
...
template <typename PointInT>
void PointCloudCoherence<PointInT>::compute(const PointCloudInConstPtr &cloud,
