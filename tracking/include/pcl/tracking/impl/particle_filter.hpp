...
bool pcl::tracking::ParticleFilterTracker<PointInT, StateT>::initCompute() {
    ...
    if (!change_detector_)
        change_detector_ = boost::shared_ptr<
            pcl::octree::OctreePointCloudChangeDetector<PointInT>>(
            new pcl::octree::OctreePointCloudChangeDetector<PointInT>(
...
template <typename PointInT, typename StateT>
int pcl::tracking::ParticleFilterTracker<
    PointInT, StateT>::sampleWithReplacement(const std::vector<int> &a,
...
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<PointInT, StateT>::genAliasTable(
...
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<PointInT, StateT>::initParticles(
    bool reset) {
    ...
    if (w_max != w_min) {
        for (size_t i = 0; i < particles_->points.size(); i++) {
            if (particles_->points[i].weight != 0.0) {
                particles_->points[i].weight = normalizeParticleWeight(
                    particles_->points[i].weight, w_min, w_max);
            }
        }
    } else {
        for (size_t i = 0; i < particles_->points.size(); i++)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<
    PointInT, StateT>::cropInputPointCloud(const PointCloudInConstPtr &,
                                           PointCloudIn &output) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<PointInT, StateT>::calcBoundingBox(
...
template <typename PointInT, typename StateT>
bool pcl::tracking::ParticleFilterTracker<
    PointInT, StateT>::testChangeDetection(const PointCloudInConstPtr &input) {
    ...
    if (!use_normal_) {
        for (size_t i = 0; i < particles_->points.size(); i++) {
            computeTransformedPointCloudWithoutNormal(
                particles_->points[i], *transed_reference_vector_[i]);
        }
...
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<
    PointInT, StateT>::computeTransformedPointCloud(const StateT &hypothesis,
...
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<PointInT, StateT>::
...
#endif
{
    ...
    for (size_t i = 0; i < cloud.points.size(); i++) {
        ...
        if (!pcl_isfinite(input_point.x) || !pcl_isfinite(input_point.y) ||
            !pcl_isfinite(input_point.z))
            continue;
...
template <typename PointInT, typename StateT>
void pcl::tracking::ParticleFilterTracker<PointInT,
                                          StateT>::resampleWithReplacement() {
    ...
    // with motion
    int motion_num = static_cast<int>(particles_->points.size()) *
