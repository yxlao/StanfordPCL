 ...
 * two points. \author Ryohei Ueda \ingroup tracking
 */
template <typename PointInT> class PointCoherence {
  public:
  ...
  public:
     ...
     */
    inline double compute(PointInT &source, PointInT &target);
  ...
  protected:
    ...
    inline const std::string &getClassName() const { return (coherence_name_); }
};

/** \brief @b PointCloudCoherence is a base class to compute coherence between
 * the two PointClouds. \author Ryohei Ueda \ingroup tracking
 */
template <typename PointInT> class PointCloudCoherence {
  public:
    ...
    /** \brief Constructor. */
    PointCloudCoherence()
        : coherence_name_(), target_input_(), point_coherences_() {}

    /** \brief Destructor. */
    virtual ~PointCloudCoherence() {}

    /** \brief compute coherence between two pointclouds. */
    inline void compute(const PointCloudInConstPtr &cloud,
                        const IndicesConstPtr &indices, float &w_i);

    /** \brief get a list of pcl::tracking::PointCoherence.*/
    inline std::vector<PointCoherencePtr> getPointCoherences() {
     ...
     */
    inline void setPointCoherences(std::vector<PointCoherencePtr> coherences) {
     ...
     */
    inline void addPointCoherence(PointCoherencePtr coherence) {
     ...
     * \param coherence a pointer to PointCoherence.
     */
    virtual inline void setTargetCloud(const PointCloudInConstPtr &cloud) {
        target_input_ = cloud;
    }

  protected:
    /** \brief Abstract method to compute coherence. */
    virtual void computeCoherence(const PointCloudInConstPtr &cloud,
