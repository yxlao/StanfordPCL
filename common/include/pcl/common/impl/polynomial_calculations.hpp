...
template <typename real>
inline void pcl::PolynomialCalculationsT<real>::Parameters::setZeroValue(
    real new_zero_value) {
...
template <typename real>
inline void pcl::PolynomialCalculationsT<real>::solveLinearEquation(
    real a, real b, std::vector<real> &roots) const {
...
template <typename real>
inline void pcl::PolynomialCalculationsT<real>::solveQuadraticEquation(
    real a, real b, real c, std::vector<real> &roots) const {
    // cout << "Trying to solve "<<a<<"x^2 + "<<b<<"x + "<<c<<" = 0\n";

    if (isNearlyZero(a)) {
        // cout << "Highest order element is 0 => Calling
        // solveLineaqrEquation.\n";
    ...
    if (isNearlyZero(c)) {
        roots.push_back(0.0);
        // cout << "Constant element is 0 => Adding root 0 and calling
        // solveLinearEquation.\n";
...
template <typename real>
inline void pcl::PolynomialCalculationsT<real>::solveCubicEquation(
    real a, real b, real c, real d, std::vector<real> &roots) const {
    // cout << "Trying to solve "<<a<<"x^3 + "<<b<<"x^2 + "<<c<<"x + "<<d<<" =
    // 0\n";
...
template <typename real>
inline void pcl::PolynomialCalculationsT<real>::solveQuarticEquation(
    real a, real b, real c, real d, real e, std::vector<real> &roots) const {
    // cout << "Trying to solve "<<a<<"x^4 + "<<b<<"x^3 + "<<c<<"x^2 + "<<d<<"x
    // + "<<e<<" = 0\n";
    ...
    double root1, root2, root3, root4,
        a2 = a * a, a3 = a2 * a, a4 = a2 * a2, b2 = b * b, b3 = b2 * b,
        b4 = b2 * b2, alpha = ((-3.0 * b2) / (8.0 * a2)) + (c / a),
    ...
    // cout << "Trying to solve y^4 + "<<alpha<<"y^2 + "<<beta<<"y +
    // "<<gamma<<"\n";
    ...
    if (isNearlyZero(beta)) { // y^4 + alpha*y^2 + gamma\n";
        ...
        // cout << "beta != 0\n";
        double alpha3 = alpha2 * alpha, beta2 = beta * beta,
               p = (-alpha2 / 12.0) - gamma,
...
pcl::PolynomialCalculationsT<real>::bivariatePolynomialApproximation(
    std::vector<Eigen::Matrix<real, 3, 1>> &samplePoints,
    unsigned int polynomial_degree, bool &error) const {
    pcl::BivariatePolynomialT<real> ret;
    error =
        bivariatePolynomialApproximation(samplePoints, polynomial_degree, ret);
    return ret;
}
...
pcl::PolynomialCalculationsT<real>::bivariatePolynomialApproximation(
    ...
    unsigned int parameters_size =
        BivariatePolynomialT<real>::getNoOfParametersFromDegree(
            polynomial_degree);
    // cout << PVARN (parameters_size);

    // cout << "Searching for the "<<parameters_size<<" parameters for the
    // bivariate polynom of degree "
    //     << polynomial_degree<<" using "<<samplePoints.size ()<<" points.\n";

    if (parameters_size > samplePoints.size()) // Too many parameters for this
                                               // number of equations (points)?
    {
        return false;
        // Reduce degree of polynomial
        // polynomial_degree = (unsigned int) (0.5f* (sqrtf (8*samplePoints.size
        // ()+1) - 3)); parameters_size =
        // BivariatePolynomialT<real>::getNoOfParametersFromDegree
        // (polynomial_degree); cout << "Not enough points, so degree of
        // polynomial was decreased to "<<polynomial_degree
    ...
    for (typename std::vector<Eigen::Matrix<real, 3, 1>>::const_iterator it =
             samplePoints.begin();
         it != samplePoints.end(); ++it) {
        currentX = (*it)[0];
        currentY = (*it)[1];
        currentZ = (*it)[2];
        // cout << "current point: "<<currentX<<","<<currentY<<" =>
        // "<<currentZ<<"\n"; unsigned int posInC = parameters_size-1;
        real *tmpCPtr = tmpCEndPtr;
        tmpX = 1.0;
        for (unsigned int xDegree = 0; xDegree <= polynomial_degree;
             ++xDegree) {
            tmpY = 1.0;
            for (unsigned int yDegree = 0;
                 yDegree <= polynomial_degree - xDegree; ++yDegree) {
                *(tmpCPtr--) = tmpX * tmpY;
                // cout << "x="<<currentX<<", y="<<currentY<<", Pos
                // "<<posInC--<<": "<<tmpX<<"*"<<tmpY<<"="<<tmpC[posInC]<<"\n";
                tmpY *= currentY;
            }
            tmpX *= currentX;
        }
    ...
    // cout << "Calculating matrix A and vector b (size "<<b.size ()<<") from
    // "<<samplePoints.size ()<<" points took "
    ...
    // for (typename std::vector<Eigen::Matrix<real, 3, 1> >::const_iterator
    ...
    ////cout << "x="<<currentX<<", y="<<currentY<<", Pos "<<posInC<<":
    ///"<<tmpX<<"*"<<tmpY<<"="<<tmpC[posInC]<<"\n";
    // tmpY *= currentY;
    // posInC--;
    //}
    // tmpX *= currentX;
    ...
    if (inversionCheckResult > 1e-5) {
        // cout << "Inversion result: "<< inversionCheckResult<<" for matrix
        // "<<A<<"\n";
        return false;
    }
