...
void pcl::apps::RenderViewsTesselatedSphere::generateViews() {
    ...
    for (size_t i = 0; i < cam_positions.size(); i++) {
        ...
        if (gen_organized_) {
            ...
            render_win->GetZbufferData(0, 0, resolution_ - 1, resolution_ - 1,
                                       &(depth[0]));
            ...
            for (int x = 0; x < resolution_; x++) {
                for (int y = 0; y < resolution_; y++) {
                    ...
                    cloud->points[count_valid_depth_pixels].getVector4fMap() =
                        backToRealScale_eigen *
                        cloud->points[count_valid_depth_pixels]
                            .getVector4fMap();
        ...
        if (compute_entropy_) {
            ...
            double visible_area = 0;
            for (int sel_id = 0; sel_id < (ids->GetNumberOfTuples());
                 sel_id++) {
        ...
        for (int x = 0; x < 4; x++)
            for (int y = 0; y < 4; y++)
                pose_view(x, y) =
                    float(transOCtoCC->GetMatrix()->GetElement(x, y));
