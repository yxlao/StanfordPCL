...
void subsampleAndCalculateNormals(
    ...
    cerr << "Before -> After subsampling: " << cloud->points.size() << " -> "
         << cloud_subsampled->points.size() << endl;
}
...
int main(int argc, char **argv) {
    ...
    PointCloud<Normal>::Ptr cloud_a_subsampled_normals,
    ...
    PointCloud<PPFSignature>::Ptr ppf_signature_a(
    ...
    for (size_t i = 0; i < 3; ++i)
        dim_range_input.push_back(
            pair<float, float>(float(-M_PI), float(M_PI)));
    dim_range_input.push_back(pair<float, float>(0.0f, 1.0f));
    for (size_t i = 0; i < 3; ++i)
        dim_range_target.push_back(
            pair<float, float>(float(-M_PI) * 10.0f, float(M_PI) * 10.0f));
