...
void displayEuclideanClusters(
    const pcl::PointCloud<PointT>::CloudVectorType &clusters,
    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer) {
    char name[1024];
    unsigned char red[6] = {255, 0, 0, 255, 255, 0};
    unsigned char grn[6] = {0, 255, 0, 255, 0, 255};
    ...
    for (size_t i = 0; i < clusters.size(); i++) {
        ...
        pcl::visualization::PointCloudColorHandlerCustom<PointT> color0(
            boost::make_shared<pcl::PointCloud<PointT>>(clusters[i]),
...
void displayCurvature(
    pcl::PointCloud<PointT> &cloud, pcl::PointCloud<pcl::Normal> &normals,
    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer) {
...
void displayDistanceMap(
    pcl::PointCloud<PointT> &cloud, float *distance_map,
    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer) {
...
bool compareClusterToRegion(pcl::PlanarRegion<PointT> &region,
    ...
    for (size_t i = 0; i < cluster.points.size(); i++) {
        double ptp_dist = fabs(model[0] * cluster.points[i].x +
                               ...
                               model[2] * cluster.points[i].z + model[3]);
...
bool comparePointToRegion(PointT &pt, pcl::ModelCoefficients &model,
                          pcl::PointCloud<PointT> &poly) {
    // bool dist_ok;

    double ptp_dist = fabs(model.values[0] * pt.x + model.values[1] * pt.y +
...
OrganizedSegmentationDemo::OrganizedSegmentationDemo(pcl::Grabber &grabber)
    ...
    plane_comparator_.reset(
        new pcl::PlaneCoefficientComparator<PointT, pcl::Normal>());
    euclidean_comparator_.reset(
        new pcl::EuclideanPlaneCoefficientComparator<PointT, pcl::Normal>());
    rgb_comparator_.reset(
        new pcl::RGBPlaneCoefficientComparator<PointT, pcl::Normal>());
...
void OrganizedSegmentationDemo::cloud_cb(const CloudConstPtr &cloud) {
    if (!capture_)
        return;
...
void OrganizedSegmentationDemo::timeoutSlot() {
    {
        ...
        if (capture_ && data_modified_) {
            removePreviousDataFromScreen(previous_data_size_,
                                         previous_clusters_size_, vis_);
            if (!vis_->updatePointCloud(
                    boost::make_shared<pcl::PointCloud<PointT>>(prev_cloud_),
                    "cloud")) {
                vis_->addPointCloud(
                    boost::make_shared<pcl::PointCloud<PointT>>(prev_cloud_),
                    "cloud");
...
void OrganizedSegmentationDemo::useEuclideanComparatorPressed() {
    QMutexLocker locker(&mtx_);
    PCL_INFO("Setting Comparator to Euclidean\n");
    mps.setComparator(euclidean_comparator_);
}

void OrganizedSegmentationDemo::useRGBComparatorPressed() {
    QMutexLocker locker(&mtx_);
    PCL_INFO("Setting Comparator to RGB\n");
    mps.setComparator(rgb_comparator_);
}

void OrganizedSegmentationDemo::usePlaneComparatorPressed() {
    QMutexLocker locker(&mtx_);
    PCL_INFO("Setting Comparator to Plane\n");
    mps.setComparator(plane_comparator_);
}

void OrganizedSegmentationDemo::useEdgeAwareComparatorPressed() {
    QMutexLocker locker(&mtx_);
    PCL_INFO("Setting Comparator to edge aware\n");
    mps.setComparator(edge_aware_comparator_);
}

void OrganizedSegmentationDemo::displayCurvaturePressed() {
    display_curvature_ = !display_curvature_;
}

void OrganizedSegmentationDemo::displayDistanceMapPressed() {
    display_distance_map_ = !display_distance_map_;
}

void OrganizedSegmentationDemo::displayNormalsPressed() {
...
int main(int argc, char **argv) {
    ...
    // itr("/u/atrevor/data/sushi_long_pcds_compressed"); itr != end_itr; ++itr)
    //{
