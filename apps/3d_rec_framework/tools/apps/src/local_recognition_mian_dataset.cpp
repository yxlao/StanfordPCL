...
void getScenesInDirectory(bf::path &dir, std::string &rel_path_so_far,
    ...
    for (bf::directory_iterator itr(dir); itr != end_itr; ++itr) {
        ...
        if (bf::is_directory(*itr)) {
            ...
            std::string so_far =
                rel_path_so_far + (itr->path().filename()).string() + "/";
#else
            std::string so_far =
                rel_path_so_far + (itr->path()).filename() + "/";
#endif
...
void recognizeAndVisualize(
    typename pcl::rec_3d_framework::LocalRecognitionPipeline<DistT, PointT,
                                                             FeatureT> &local,
    ...
    typename boost::shared_ptr<pcl::rec_3d_framework::Source<PointT>>
        model_source_ = local.getDataSource();
    ...
    if (!single_model) {
        ...
        for (size_t i = 0; i < files.size(); i++) {
            ...
            typename pcl::PointCloud<PointT>::Ptr scene(
            ...
            boost::shared_ptr<std::vector<
                ...
                transforms = local.getTransforms();
            ...
            for (size_t j = 0; j < models->size(); j++) {
                ...
                ConstPointInTPtr model_cloud =
                    models->at(j).getAssembled(0.0025f);
                ...
                pcl::visualization::PointCloudColorHandlerRandom<PointT>
                    random_handler(model_aligned);
                vis.addPointCloud<PointT>(model_aligned, random_handler,
                                          name.str());
            }
...
void getModelsInDirectory(bf::path &dir, std::string &rel_path_so_far,
                          std::vector<std::string> &relative_paths,
                          std::string &ext) {
    bf::directory_iterator end_itr;
    for (bf::directory_iterator itr(dir); itr != end_itr; ++itr) {
...
//./bin/pcl_local_or_mian -models_dir /home/aitor/data/Mians_dataset/models/
//-mians_scenes_dir /home/aitor/data/Mians_dataset/scenes/pcl_scenes/
//-training_dir trained_models2/ -icp_iterations 0 -descriptor_name shot_omp
//-gc_size 3 -use_cache 1 -scene -1
...
int main(int argc, char **argv) {
    ...
    if (mians_scenes.compare("") == 0) {
        PCL_ERROR("Set the directory containing mians scenes using the "
                  "-mians_scenes_dir [dir] option\n");
        return -1;
    }
    ...
    if (!bf::exists(models_dir_path)) {
        PCL_ERROR(
            "Models dir path %s does not exist, use -models_dir [dir] option\n",
            path.c_str());
        return -1;
    } else {
        std::vector<std::string> files;
    ...
    boost::shared_ptr<pcl::rec_3d_framework::MeshSource<pcl::PointXYZ>>
        mesh_source(new pcl::rec_3d_framework::MeshSource<pcl::PointXYZ>);
    ...
    cast_source = boost::static_pointer_cast<
        pcl::rec_3d_framework::MeshSource<pcl::PointXYZ>>(mesh_source);

    // configure normal estimator
    boost::shared_ptr<pcl::rec_3d_framework::PreProcessorAndNormalEstimator<
    ...
    boost::shared_ptr<
        pcl::rec_3d_framework::UniformSamplingExtractor<pcl::PointXYZ>>
    ...
    boost::shared_ptr<pcl::rec_3d_framework::KeypointExtractor<pcl::PointXYZ>>
        keypoint_extractor;
    keypoint_extractor = boost::static_pointer_cast<
        pcl::rec_3d_framework::KeypointExtractor<pcl::PointXYZ>>(
        uniform_keypoint_extractor);

    // configure cg algorithm (geometric consistency grouping)
    boost::shared_ptr<pcl::CorrespondenceGrouping<pcl::PointXYZ, pcl::PointXYZ>>
        cast_cg_alg;

    if (CG_ALG == 0) {
        boost::shared_ptr<
            pcl::GeometricConsistencyGrouping<pcl::PointXYZ, pcl::PointXYZ>>
            gcg_alg(new pcl::GeometricConsistencyGrouping<pcl::PointXYZ,
                                                          pcl::PointXYZ>);
        gcg_alg->setGCThreshold(CG_SIZE_);
        gcg_alg->setGCSize(CG_THRESHOLD_);
        cast_cg_alg = boost::static_pointer_cast<
            pcl::CorrespondenceGrouping<pcl::PointXYZ, pcl::PointXYZ>>(gcg_alg);
    } else if (CG_ALG == 1) {
        // configure cg algorithm (hough 3d)
        boost::shared_ptr<pcl::Hough3DGrouping<pcl::PointXYZ, pcl::PointXYZ>>
            hough_3d_alg(
                new pcl::Hough3DGrouping<pcl::PointXYZ, pcl::PointXYZ>);
        ...
        cast_cg_alg = boost::static_pointer_cast<
            pcl::CorrespondenceGrouping<pcl::PointXYZ, pcl::PointXYZ>>(
            hough_3d_alg);
    }

    // configure hypothesis verificator
    boost::shared_ptr<pcl::PapazovHV<pcl::PointXYZ, pcl::PointXYZ>> papazov(
    ...
    boost::shared_ptr<pcl::HypothesisVerification<pcl::PointXYZ, pcl::PointXYZ>>
        cast_hv_alg;
    cast_hv_alg = boost::static_pointer_cast<
        pcl::HypothesisVerification<pcl::PointXYZ, pcl::PointXYZ>>(papazov);

    /*boost::shared_ptr<pcl::GlobalHypothesesVerification<pcl::PointXYZ,
    pcl::PointXYZ> > go ( new pcl::GlobalHypothesesVerification<pcl::PointXYZ,
                                                                                                pcl::PointXYZ>);
    ...
    cast_hv_alg =
    boost::static_pointer_cast<pcl::HypothesisVerification<pcl::PointXYZ,
    pcl::PointXYZ> > (go);*/

    if (desc_name.compare("shot") == 0) {
        boost::shared_ptr<pcl::rec_3d_framework::SHOTLocalEstimation<
            pcl::PointXYZ, pcl::Histogram<352>>>
            estimator;
        estimator.reset(new pcl::rec_3d_framework::SHOTLocalEstimation<
                        pcl::PointXYZ, pcl::Histogram<352>>);
        ...
        boost::shared_ptr<pcl::rec_3d_framework::LocalEstimator<
            pcl::PointXYZ, pcl::Histogram<352>>>
            cast_estimator;
        cast_estimator =
            boost::dynamic_pointer_cast<pcl::rec_3d_framework::LocalEstimator<
                pcl::PointXYZ, pcl::Histogram<352>>>(estimator);

        pcl::rec_3d_framework::LocalRecognitionPipeline<
            flann::L1, pcl::PointXYZ, pcl::Histogram<352>>
            local;
    ...
    if (desc_name.compare("shot_omp") == 0) {
        ...
        boost::shared_ptr<pcl::rec_3d_framework::SHOTLocalEstimationOMP<
            pcl::PointXYZ, pcl::Histogram<352>>>
            estimator;
        estimator.reset(new pcl::rec_3d_framework::SHOTLocalEstimationOMP<
                        pcl::PointXYZ, pcl::Histogram<352>>);
        estimator->setNormalEstimator(normal_estimator);
        estimator->addKeypointExtractor(keypoint_extractor);
        estimator->setSupportRadius(0.04f);
    ...
    if (desc_name.compare("fpfh") == 0) {
        boost::shared_ptr<pcl::rec_3d_framework::FPFHLocalEstimation<
            pcl::PointXYZ, pcl::FPFHSignature33>>
            estimator;
        estimator.reset(new pcl::rec_3d_framework::FPFHLocalEstimation<
                        pcl::PointXYZ, pcl::FPFHSignature33>);
        estimator->setNormalEstimator(normal_estimator);
        estimator->addKeypointExtractor(keypoint_extractor);
        estimator->setSupportRadius(0.04f);
        ...
        cast_estimator =
            boost::dynamic_pointer_cast<pcl::rec_3d_framework::LocalEstimator<
                pcl::PointXYZ, pcl::FPFHSignature33>>(estimator);

        pcl::rec_3d_framework::LocalRecognitionPipeline<
            flann::L1, pcl::PointXYZ, pcl::FPFHSignature33>
            local;
        local.setDataSource(cast_source);
        local.setTrainingDir(training_dir);
        local.setDescriptorName(desc_name);
