...
class SHOTLocalEstimationOMP : public LocalEstimator<PointInT, FeatureT> {
  ...
  public:
    bool estimate(PointInTPtr &in, PointInTPtr &processed,
                  PointInTPtr &keypoints, FeatureTPtr &signatures) {
        if (!normal_estimator_) {
            PCL_ERROR("SHOTLocalEstimationOMP :: This feature needs normals... "
                      "please provide a normal estimator\n");
            return false;
        }

        if (keypoint_extractor_.size() == 0) {
            PCL_ERROR("SHOTLocalEstimationOMP :: This feature needs a keypoint "
                      "extractor... please provide one\n");
            return false;
        }
        ...
        for (size_t k = 0; k < shots->points.size(); k++) {
            ...
            if (NaNs == 0) {
                for (int i = 0; i < size_feat; i++) {
                    signatures->points[good].histogram[i] =
                        shots->points[k].descriptor[i];
                }
