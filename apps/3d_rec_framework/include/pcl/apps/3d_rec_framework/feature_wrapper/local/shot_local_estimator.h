  ...
  public:
    bool estimate(PointInTPtr &in, PointInTPtr &processed,
        ...
        if (keypoint_extractor_.size() == 0) {
            PCL_ERROR("SHOTLocalEstimation :: This feature needs a keypoint "
        ...
        // pcl::PointCloud<pcl::Normal>); normal_estimator_->estimate (in,
        // processed, normals);
        ...
        // compute signatures
        typedef
            typename pcl::SHOTEstimation<PointInT, pcl::Normal, pcl::SHOT352>
                SHOTEstimator;
        ...
        for (size_t k = 0; k < shots->points.size(); k++)
            for (int i = 0; i < size_feat; i++)
                signatures->points[k].histogram[i] =
