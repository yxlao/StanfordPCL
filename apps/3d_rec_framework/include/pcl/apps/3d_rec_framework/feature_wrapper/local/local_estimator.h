...
template <typename PointInT> class KeypointExtractor {
  protected:
  ...
  public:
    void setInputCloud(PointInTPtr &input) { input_ = input; }

    void setSupportRadius(float f) { radius_ = f; }

    virtual void compute(PointOutTPtr &keypoints) = 0;

    virtual void
    setNormals(const pcl::PointCloud<pcl::Normal>::Ptr & /*normals*/) {}

    virtual bool needNormals() { return false; }
};
...
class UniformSamplingExtractor : public KeypointExtractor<PointInT> {
  private:
    ...
    void filterPlanar(PointInTPtr &input,
        ...
        for (size_t i = 0; i < keypoints_cloud.points.size(); i++) {
            ...
            if (tree->radiusSearch(keypoints_cloud[i], radius_,
                ...
                if ((fabs(eigenValues[0] - eigenValues[1]) < 1.5e-4) ||
                    (eigsum != 0 && fabs(eigenValues[0] / eigsum) > 1.e-2)) {
  ...
  public:
    void setFilterPlanar(bool b) { filter_planar_ = b; }

    void setSamplingDensity(float f) { sampling_density_ = f; }

    void compute(PointInTPtr &keypoints) {
  ...
  public:
    void compute(PointInTPtr &keypoints) {
        ...
        for (size_t i = 0; i < input_->points.size(); i++) {
            input_cloud->points[i].getVector3fMap() =
...
class HarrisKeypointExtractor : public KeypointExtractor<PointInT> {
  ...
  public:
    ...
    void setResponseMethod(
        typename pcl::HarrisKeypoint3D<PointInT, pcl::PointXYZI>::ResponseMethod
            m) {
        m_ = m;
    }

    void setNonMaximaRadius(float r) { non_max_radius_ = r; }
...
template <typename PointInT, typename FeatureT> class LocalEstimator {
  protected:
    typedef typename pcl::PointCloud<PointInT>::Ptr PointInTPtr;
    typedef typename pcl::PointCloud<FeatureT>::Ptr FeatureTPtr;

    typename boost::shared_ptr<
        PreProcessorAndNormalEstimator<PointInT, pcl::Normal>>
        normal_estimator_;
    // typename boost::shared_ptr<UniformSampling<PointInT> >
    // keypoint_extractor_;
    std::vector<typename boost::shared_ptr<KeypointExtractor<PointInT>>>
    ...
    void computeKeypoints(PointInTPtr &cloud, PointInTPtr &keypoints,
  ...
  public:
    ...
    void setAdaptativeMLS(bool b) { adaptative_MLS_ = b; }

    virtual bool estimate(PointInTPtr &in, PointInTPtr &processed,
                          PointInTPtr &keypoints, FeatureTPtr &signatures) = 0;

    void setNormalEstimator(
        boost::shared_ptr<PreProcessorAndNormalEstimator<PointInT, pcl::Normal>>
            &ne) {
    ...
    void setKeypointExtractors(
        std::vector<typename boost::shared_ptr<KeypointExtractor<PointInT>>>
            &ke) {
        keypoint_extractor_ = ke;
    }

    void setSupportRadius(float r) { support_radius_ = r; }
    ...
    /*void
     ...
     if (tree->radiusSearch (keypoints_cloud[i], support_radius_,
     (*neighborhood_indices_)[good], (*neighborhood_dist_)[good]))
     {
     ...
     if ((fabs (eigenValues[0] - eigenValues[1]) < 1.5e-4) || (eigsum != 0 &&
     fabs (eigenValues[0] / eigsum) > 1.e-2))
     {
     //region is not planar, add to filtered keypoint
     keypoints_cloud.points[good] = keypoints_cloud.points[i];
     good++;
