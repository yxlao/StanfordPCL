...
class CRHEstimation : public GlobalEstimator<PointInT, FeatureT> {
    ...
    typename boost::shared_ptr<GlobalEstimator<PointInT, FeatureT>>
        feature_estimator_;
  ...
  public:
    ...
    estimate(PointInTPtr &in, PointInTPtr &processed,
        ...
        if (!feature_estimator_) {
            PCL_ERROR("CRHEstimation needs a global estimator... please "
                      "provide one\n");
            return;
        }
        ...
        typedef typename pcl::CRHEstimation<PointInT, pcl::Normal,
                                            pcl::Histogram<90>>
            CRHEstimation;
        CRHEstimation crh;
        ...
        for (size_t idx = 0; idx < signatures.size(); idx++) {
            Eigen::Vector4f centroid4f(centroids[idx][0], centroids[idx][1],
                                       centroids[idx][2], 0);
