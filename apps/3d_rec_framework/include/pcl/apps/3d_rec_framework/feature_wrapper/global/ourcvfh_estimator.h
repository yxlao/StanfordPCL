...
class OURCVFHEstimator : public GlobalEstimator<PointInT, FeatureT> {
  ...
  public:
    ...
    estimate(PointInTPtr &in, PointInTPtr &processed,
        ...
        typedef typename pcl::OURCVFHEstimation<PointInT, pcl::Normal,
                                                pcl::VFHSignature308>
            OURCVFHEstimation;
        ...
        if (normal_estimator_->compute_mesh_resolution_) {
            radius = normal_estimator_->mesh_resolution_ *
                     normal_estimator_->factor_normals_;
            ...
            if (normal_estimator_->do_voxel_grid_) {
                ...
                cluster_tolerance_radius *=
                    normal_estimator_->factor_voxel_grid_;
            }
        }
        ...
        // std::cout << "Res:" << normal_estimator_->mesh_resolution_ << "
        // Radius normals:" << radius << " Cluster tolerance:" <<
        // cluster_tolerance_radius << " " << eps_angle_threshold_ << " " <<
        // curvature_threshold_ << std::endl;
        ...
        for (size_t i = 0; i < cvfh_signatures.points.size(); i++) {
            ...
            for (int d = 0; d < 308; ++d)
                vfh_signature.points[0].histogram[d] =
                    cvfh_signatures.points[i].histogram[d];
    ...
    void setNormalizeBins(bool b) { normalize_bins_ = b; }
};
} // namespace rec_3d_framework
} // namespace pcl
