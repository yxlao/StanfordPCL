...
class PreProcessorAndNormalEstimator {
    ...
    float computeMeshResolution(PointInTPtr &input) {
        typedef typename pcl::KdTree<PointInT>::Ptr KdTreeInPtr;
        KdTreeInPtr tree =
            boost::make_shared<pcl::KdTreeFLANN<PointInT>>(false);
        ...
        float avg =
            avg_distances[static_cast<int>(avg_distances.size()) / 2 + 1];
        return avg;
    }
  ...
  public:
    ...
    void setFactorsForCMR(float f1, float f2) {
        factor_voxel_grid_ = f1;
        factor_normals_ = f2;
    }

    void setValuesForCMRFalse(float f1, float f2) {
        grid_resolution_ = f1;
        normal_radius_ = f2;
    }
    ...
    void setRemoveOutliers(bool b) { remove_outliers_ = b; }

    void setCMR(bool b) { compute_mesh_resolution_ = b; }

    void estimate(PointInTPtr &in, PointInTPtr &out,
                  pcl::PointCloud<pcl::Normal>::Ptr &normals) {
        if (compute_mesh_resolution_) {
            mesh_resolution_ = computeMeshResolution(in);
            std::cout << "compute mesh resolution:" << mesh_resolution_
                      << std::endl;
        }
        ...
        if (do_voxel_grid_) {
            ...
            grid_.setLeafSize(voxel_grid_size, voxel_grid_size,
                              voxel_grid_size);
        ...
        if (out->points.size() == 0) {
            PCL_WARN("NORMAL estimator: Cloud has no points after voxel grid, "
                     "wont be able to compute normals!\n");
            return;
        }
        ...
        if (remove_outliers_) {
            ...
            // thus creating a very sparse set of points that causes the normals
            // to be very noisy remove these points
        ...
        if (out->points.size() == 0) {
            PCL_WARN("NORMAL estimator: Cloud has no points after removing "
                     "outliers...!\n");
            return;
        }
        ...
        if (out->isOrganized()) {
            typedef typename pcl::IntegralImageNormalEstimation<PointInT,
                                                                pcl::Normal>
                NormalEstimator_;
            ...
            // check nans before computing normals
            {
                ...
                for (size_t i = 0; i < out->points.size(); ++i) {
                    if (!pcl_isfinite(out->points[i].x) ||
                        !pcl_isfinite(out->points[i].y) ||
                        !pcl_isfinite(out->points[i].z))
                        continue;
            ...
            typedef typename pcl::NormalEstimation<PointInT, pcl::Normal>
                NormalEstimator_;
            NormalEstimator_ n3d;
        ...
        /*for (size_t i = 0; i < out->points.size (); i++)
        {
          ...
          std::cout << "in normal estimator:" << r << " " << g << " " << b <<
        std::endl;
        }*/
    }
};
