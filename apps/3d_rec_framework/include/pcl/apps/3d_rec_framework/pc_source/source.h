...
template <typename PointT> class Model {
  ...
  public:
    ...
    PointTPtrConst getAssembled(float resolution) {
        if (resolution <= 0)
            return assembled_;
        ...
        if (it == voxelized_assembled_.end()) {
            ...
            PointTPtrConst voxelized_const(
                new pcl::PointCloud<PointT>(*voxelized));
...
/**
 * \brief Abstract data source class, manages filesystem, incremental training,
 * etc. \author Aitor Aldoma
 */

template <typename PointInT> class Source {
  ...
  protected:
    ...
    void getIdAndClassFromFilename(std::string &filename, std::string &id,
    ...
    void createClassAndModelDirectories(std::string &training_dir,
  ...
  public:
    ...
    float getScale() { return model_scale_; }

    void setModelScale(float s) { model_scale_ = s; }
    ...
    void voxelizeAllModels(float resolution) {
     ...
     */
    virtual void generate(std::string &training_dir) = 0;
    ...
    boost::shared_ptr<std::vector<ModelT>> getModels(std::string &model_id) {
    ...
    bool modelAlreadyTrained(ModelT m, std::string &base_dir,
                             std::string &descr_name) {
    ...
    std::string getModelDescriptorDir(ModelT m, std::string &base_dir,
                                      std::string &descr_name) {
        std::stringstream dir;
        dir << base_dir << "/" << m.class_ << "/" << m.id_ << "/" << descr_name;
        return dir.str();
    ...
    void removeDescDirectory(ModelT m, std::string &base_dir,
                             std::string &descr_name) {
    ...
    void setLoadViews(bool load) { load_views_ = load; }
};
} // namespace rec_3d_framework
} // namespace pcl
