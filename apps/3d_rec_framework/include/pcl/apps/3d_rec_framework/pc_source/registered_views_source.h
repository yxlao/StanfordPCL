 ...
 * containing a 4x4 matrix representing the transformation from camera
 * coordinates to a global object coordinates frame. \author Aitor Aldoma
 */
...
class RegisteredViewsSource : public Source<PointInT> {
  ...
  public:
    ...
    void setPrefix(std::string &pre) { view_prefix_ = pre; }

    void setPoseRowOrder(int o) { pose_files_order_ = o; }

    void getViewsFilenames(bf::path &path_with_views,
                           std::vector<std::string> &view_filenames) {
        int number_of_views = 0;
        bf::directory_iterator end_itr;
        for (bf::directory_iterator itr(path_with_views); itr != end_itr;
             ++itr) {
            if (!(bf::is_directory(*itr))) {
                ...
                if (extension == "pcd" &&
                    (strs_[0].compare(view_prefix_) == 0)) {
#if BOOST_FILESYSTEM_VERSION == 3
    ...
    void assembleModelFromViewsAndPoses(ModelT &model,
        ...
        for (size_t i = 0; i < model.views_->size(); i++) {
            ...
            pcl::transformPointCloud(*(model.views_->at(i)), *global_cloud,
                                     inv);
            *(model.assembled_) += *global_cloud;
        }
    }

    void loadOrGenerate(std::string &dir, std::string &model_path,
                        ModelT &model) {
        ...
        model.views_.reset(
            new std::vector<typename pcl::PointCloud<PointInT>::Ptr>);
        model.poses_.reset(
        ...
        if (bf::exists(trained_dir)) {
            ...
            int number_of_views = 0;
            bf::directory_iterator end_itr;
            for (bf::directory_iterator itr(trained_dir); itr != end_itr;
                 ++itr) {
                // check if its a directory, then get models in it
                if (!(bf::is_directory(*itr))) {
                    // check that it is a ply file and then add, otherwise
                    // ignore..
                    std::vector<std::string> strs;
                    std::vector<std::string> strs_;
            ...
            random_handler (model.assembled_, 255, 0, 0);
            vis.addPointCloud<PointInT> (model.assembled_, random_handler,
            "points");

            Eigen::Matrix4f view_transformation = model.poses_->at(0).inverse();
            typename pcl::PointCloud<PointInT>::Ptr view_trans(new
            pcl::PointCloud<PointInT>);
            pcl::transformPointCloud(*(model.views_->at(0)), *view_trans,
            view_transformation);

            pcl::visualization::PointCloudColorHandlerCustom<PointInT>
            random_handler2 (view_trans, 0, 255, 0); vis.addPointCloud<PointInT>
            (view_trans, random_handler2, "view");
    ...
    /**
     * \brief Creates the model representation of the training set, generating
     * views if needed
     */
    void generate(std::string &training_dir) {
        ...
        for (size_t i = 0; i < files.size(); i++) {
            ...
            // check which of them have been trained using training_dir and the
            // model_id_ load views, poses and self-occlusions for those that
            // exist generate otherwise
