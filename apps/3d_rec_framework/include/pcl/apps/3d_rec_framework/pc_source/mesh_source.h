...
template <typename PointInT> class MeshSource : public Source<PointInT> {
  ...
  public:
    ...
    void setTesselationLevel(int lev) { tes_level_ = lev; }
    ...
    void setRadiusSphere(float r) { radius_sphere_ = r; }

    void setViewAngle(float a) { view_angle_ = a; }
    ...
    void loadOrGenerate(std::string &dir, std::string &model_path,
        ...
        if (bf::exists(trained_dir)) {
            ...
            for (size_t i = 0; i < view_filenames.size(); i++) {
                ...
                float entropy = 0;
                PersistenceUtils::readFloatFromFile(entropy_file.str(),
                                                    entropy);
            ...
            model.views_.reset(
                new std::vector<typename PointCloud<PointInT>::Ptr>());
            model.poses_.reset(
                new std::vector<Eigen::Matrix4f,
                                Eigen::aligned_allocator<Eigen::Matrix4f>>());
            model.self_occlusions_.reset(new std::vector<float>());
            ...
            for (size_t i = 0; i < model.views_->size(); i++) {
                ...
                pcl::io::savePCDFileBinary(path_view.str(),
                                           *(model.views_->at(i)));
                ...
                pcl::rec_3d_framework::PersistenceUtils::writeMatrixToFile(
                    path_pose.str(), model.poses_->at(i));
