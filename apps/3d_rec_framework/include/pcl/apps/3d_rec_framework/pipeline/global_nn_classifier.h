...
template <typename PointInT> class PCL_EXPORTS GlobalClassifier {
  public:
    typedef typename pcl::PointCloud<PointInT>::Ptr PointInTPtr;

    virtual void setNN(int nn) = 0;

    virtual void getCategory(std::vector<std::string> &categories) = 0;

    virtual void getConfidence(std::vector<float> &conf) = 0;

    virtual void classify() = 0;

    virtual void setIndices(std::vector<int> &indices) = 0;

    virtual void setInputCloud(const PointInTPtr &cloud) = 0;
};
 ...
 * list of classes. Available features: ESF, VFH, CVFH See
 * apps/3d_rec_framework/tools/apps/global_classification.cpp for usage \author
 * Aitor Aldoma
 */
...
class PCL_EXPORTS GlobalNNPipeline
  ...
  public:
    ...
    void getCategory(std::vector<std::string> &categories) {
        categories = categories_;
    }

    void getConfidence(std::vector<float> &conf) { conf = confidences_; }
    ...
    void setFeatureEstimator(
        typename boost::shared_ptr<GlobalEstimator<PointInT, FeatureT>> &feat) {
        estimator_ = feat;
    }
