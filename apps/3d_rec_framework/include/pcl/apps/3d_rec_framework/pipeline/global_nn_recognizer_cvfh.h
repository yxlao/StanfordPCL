 ...
 * features. Available features: CVFH \author Aitor Aldoma
 */
...
class PCL_EXPORTS GlobalNNCVFHRecognizer {
  ...
  protected:
    ...
    typename boost::shared_ptr<OURCVFHEstimator<PointInT, FeatureT>>
        micvfh_estimator_;
    ...
    class flann_model {
      public:
        ...
        bool operator<(const flann_model &other) const {
        ...
        bool operator==(const flann_model &other) const {
            return (model.id_ == other.model.id_) &&
    ...
    inline void convertToFLANN(const std::vector<flann_model> &models,
        ...
        flann::Matrix<float> flann_data(
            ...
            indices->size(), models[0].descr.size());

        for (size_t i = 0; i < data.rows; ++i)
            for (size_t j = 0; j < data.cols; ++j) {
                flann_data.ptr()[i * data.cols + j] =
                    models[indices->at(i)].descr[j];
            }
    ...
    bool getRollPose(ModelT &model, int view_id, int d_id,
                     Eigen::Matrix4f &pose_matrix);

    void getCentroid(ModelT &model, int view_id, int d_id,
                     Eigen::Vector3f &centroid);

    void getView(ModelT &model, int view_id, PointInTPtr &view);
  ...
  public:
    ...
    void getDescriptorDistances(std::vector<float> &ds) {
        ds = descriptor_distances_;
    }

    void setComputeScale(bool d) { compute_scale_ = d; }
    ...
    void setUseSingleCategories(bool b) { use_single_categories_ = b; }
