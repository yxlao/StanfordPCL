...
inline bool correspSorter(const pcl::Correspondence &i,
                          const pcl::Correspondence &j) {
 ...
 * point-to-point correspondence grouping, pose refinement and hypotheses
 * verification Available features: SHOT, FPFH See
 * apps/3d_rec_framework/tools/apps for usage \author Aitor Aldoma, Federico
 * Tombari
 */
...
class PCL_EXPORTS LocalRecognitionPipeline {
    ...
    typename boost::shared_ptr<CorrespondenceGrouping<PointInT, PointInT>>
        cg_algorithm_;

    /** \brief Hypotheses verification algorithm */
    typename boost::shared_ptr<HypothesisVerification<PointInT, PointInT>>
        hv_algorithm_;
    ...
    std::map<std::pair<std::string, int>, Eigen::Matrix4f,
             ...
             Eigen::aligned_allocator<
                 std::pair<std::pair<std::string, int>, Eigen::Matrix4f>>>
        poses_cache_;
    ...
    inline void convertToFLANN(const std::vector<flann_model> &models,
    ...
    void nearestKSearch(flann::Index<DistT> *index, const flann_model &model,
    ...
    class ObjectHypothesis {
      public:
        ...
        pcl::CorrespondencesPtr
            correspondences_to_inputcloud; // indices between
                                           // correspondences_pointcloud and
                                           // scene cloud
    };

    void getPose(ModelT &model, int view_id, Eigen::Matrix4f &pose_matrix);

    void getKeypoints(ModelT &model, int view_id,
                      typename pcl::PointCloud<PointInT>::Ptr &keypoints_cloud);

    void drawCorrespondences(PointInTPtr &cloud, ObjectHypothesis &oh,
        ...
        vis.addPointCloud<PointInT>(cloud_sampled, random_handler_sampled,
                                    "sampled");

        for (size_t kk = 0; kk < correspondences.size(); kk++) {
            pcl::PointXYZ p;
            p.getVector4fMap() = oh.correspondences_pointcloud
                                     ->points[correspondences[kk].index_query]
                                     .getVector4fMap();
            ...
            vis.addLine<pcl::PointXYZ, pcl::PointXYZ>(p_scene, p,
                                                      line_name.str());
        }
  ...
  public:
    ...
    void setISPK(typename pcl::PointCloud<FeatureT>::Ptr &signatures,
                 PointInTPtr &p, PointInTPtr &keypoints) {
        keypoints_input_ = keypoints;
        signatures_ = signatures;
        processed_ = p;
    }

    void setVoxelSizeICP(float s) { VOXEL_SIZE_ICP_ = s; }
    void setSearchModel(std::string &id) { search_model_ = id; }
    ...
    void setKdtreeSplits(int n) { kdtree_splits_ = n; }

    void setIndices(std::vector<int> &indices) { indices_ = indices; }

    void setICPIterations(int it) { ICP_iterations_ = it; }

    void setUseCache(bool u) { use_cache_ = u; }
     ...
     */
    void setDataSource(typename boost::shared_ptr<Source<PointInT>> &source) {
        source_ = source;
    }

    typename boost::shared_ptr<Source<PointInT>> getDataSource() {
        return source_;
    }
     ...
     */
    void setFeatureEstimator(
        typename boost::shared_ptr<LocalEstimator<PointInT, FeatureT>> &feat) {
        estimator_ = feat;
    }
     ...
     */
    void setInputCloud(const PointInTPtr &cloud) { input_ = cloud; }

    /**
     * \brief Sets the descriptor name
     */
    void setDescriptorName(std::string &name) { descr_name_ = name; }

    /**
     * \brief Filesystem dir where to keep the generated training data
     */
    void setTrainingDir(std::string &dir) { training_dir_ = dir; }

    void setComputeTablePlane(bool b) { compute_table_plane_ = b; }

    void getProcessed(PointInTPtr &cloud) { cloud = processed_; }
    ...
    void initialize(bool force_retrain = false);
