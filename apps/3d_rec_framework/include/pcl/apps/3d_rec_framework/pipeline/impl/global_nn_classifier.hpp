...
void pcl::rec_3d_framework::GlobalNNPipeline<
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNPipeline<Distance, PointInT, FeatureT>::
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNPipeline<Distance, PointInT,
                                             FeatureT>::classify() {
    ...
    if (signatures.size() > 0) {
        ...
        std::sort(indices_scores.begin(), indices_scores.end(),
                  sortIndexScoresOp);
        first_nn_category_ =
            flann_models_[indices_scores[0].idx_models_].first.class_;
        ...
        for (int i = 0; i < num_n; ++i) {
            std::string cat =
                flann_models_[indices_scores[i].idx_models_].first.class_;
        ...
        for (it = category_map.begin(); it != category_map.end(); it++) {
            float prob =
                static_cast<float>(it->second) / static_cast<float>(num_n);
    ...
    for (size_t i = 0; i < models->size(); i++) {
        if (!source_->modelAlreadyTrained(models->at(i), training_dir_,
            ...
            for (size_t v = 0; v < models->at(i).views_->size(); v++) {
                ...
                // source_->makeModelPersistent (models->at (i), training_dir_,
                // descr_name_, static_cast<int> (v));
                std::string path = source_->getModelDescriptorDir(
                    models->at(i), training_dir_, descr_name_);
