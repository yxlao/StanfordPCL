...
void pcl::rec_3d_framework::LocalRecognitionPipeline<
        ...
        for (bf::directory_iterator itr_in(inside); itr_in != end_itr;
            ...
            if (strs[0] == "descriptor") {
                ...
                if (use_cache_) {
                    ...
                    std::string path = source_->getModelDescriptorDir(
                        models->at(i), training_dir_, descr_name_);
                    dir_keypoints << path << "/keypoint_indices_"
                                  << descr_model.view_id << ".pcd";
    ...
    flann_index_ =
        new flann::Index<DistT>(flann_data_, flann::KDTreeIndexParams(4));
    flann_index_->buildIndex();
}

template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::
    LocalRecognitionPipeline<Distance, PointInT, FeatureT>::nearestKSearch(
    ...
    index->knnSearch(p, indices, distances, k,
                     flann::SearchParams(kdtree_splits_));
    delete[] p.ptr();
}
...
void pcl::rec_3d_framework::LocalRecognitionPipeline<
    ...
    for (size_t i = 0; i < models->size(); i++) {
        std::cout << models->at(i).class_ << " " << models->at(i).id_
                  << std::endl;
        ...
        if (!source_->modelAlreadyTrained(models->at(i), training_dir_,
            ...
            for (size_t v = 0; v < models->at(i).views_->size(); v++) {
                ...
                bool success =
                    estimator_->estimate(models->at(i).views_->at(v), processed,
                ...
                if (success) {
                    ...
                    keypoints_sstr << path << "/keypoint_indices_" << v
                                   << ".pcd";

                    /*boost::shared_ptr < std::vector<int> > indices (new
                    std::vector<int> ()); indices->resize (keypoints.points.size
                    ()); for (size_t kk = 0; kk < indices->size (); kk++)
                      (*indices)[kk] = keypoints.points[kk];
                    typename pcl::PointCloud<PointInT> keypoints_pointcloud;
                    pcl::copyPointCloud (*processed, *indices,
                    keypoints_pointcloud);*/
                    pcl::io::savePCDFileBinary(keypoints_sstr.str(),
                                               *keypoints_pointcloud);
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::LocalRecognitionPipeline<Distance, PointInT,
                                                     FeatureT>::recognize() {
    ...
    if (signatures_ != 0 && processed_ != 0 &&
        ...
        if (indices_.size() > 0) {
            ...
            estimator_->estimate(sub_input, processed, keypoints_pointcloud,
                                 signatures);
        } else {
            estimator_->estimate(input_, processed, keypoints_pointcloud,
                                 signatures);
        }
    ...
    typename pcl::PointCloud<PointInT>::Ptr keypoints_pointcloud (new
    pcl::PointCloud<PointInT> ()); pcl::copyPointCloud (*processed, *indices,
    *keypoints_pointcloud);*/
    ...
    /*{
      ...
      pcl::visualization::PointCloudColorHandlerCustom<PointInT> handler_votes
    (keypoints_pointcloud, 255, 0, 0); vis.addPointCloud<PointInT>
    (keypoints_pointcloud, handler_votes, "votes_cloud");
    ...
    std::map<std::string, ObjectHypothesis> object_hypotheses;
    {
        for (size_t idx = 0; idx < signatures->points.size(); idx++) {
            ...
            // read view pose and keypoint coordinates, transform keypoint
            ...
            if ((it_map = object_hypotheses.find(
                     flann_models_.at(indices[0][0]).model.id_)) !=
                ...
                oh.correspondences_to_inputcloud->push_back(pcl::Correspondence(
                    static_cast<int>(
                        oh.correspondences_pointcloud->points.size() - 1),
                ...
                typename pcl::PointCloud<PointInT>::Ptr
                    correspondences_pointcloud(new pcl::PointCloud<PointInT>());
                ...
                // last keypoint for this model is a correspondence the current
                // scene keypoint

                pcl::CorrespondencesPtr corr(new pcl::Correspondences());
                oh.correspondences_to_inputcloud = corr;
                oh.correspondences_to_inputcloud->push_back(pcl::Correspondence(
                    0, static_cast<int>(idx), distances[0][0]));

                boost::shared_ptr<std::vector<float>> feat_dist(
    ...
    {
        // pcl::ScopeTime t("Geometric verification, RANSAC and transform
        // estimation");
        for (it_map = object_hypotheses.begin();
             it_map != object_hypotheses.end(); it_map++) {
            std::vector<pcl::Correspondences> corresp_clusters;
            cg_algorithm_->setSceneCloud(keypoints_pointcloud);
            cg_algorithm_->setInputCloud(
                (*it_map).second.correspondences_pointcloud);
            cg_algorithm_->setModelSceneCorrespondences(
                (*it_map).second.correspondences_to_inputcloud);
            cg_algorithm_->cluster(corresp_clusters);

            std::cout << "Instances:" << corresp_clusters.size()
                      << " Total correspondences:"
                      << (*it_map).second.correspondences_to_inputcloud->size()
                      << " " << it_map->first << std::endl;
            std::vector<bool> good_indices_for_hypothesis(
                corresp_clusters.size(), true);

            if (threshold_accept_model_hypothesis_ < 1.f) {
                // sort the hypotheses for each model according to their
                // correspondences and take those that are
                // threshold_accept_model_hypothesis_ over the max cardinality
                ...
                for (size_t i = 0; i < corresp_clusters.size(); i++) {
                    ...
                    // << std::endl;
                    if (max_cardinality <
                        static_cast<int>(corresp_clusters[i].size())) {
                        max_cardinality =
                            static_cast<int>(corresp_clusters[i].size());
                    }
                }
            ...
            for (size_t i = 0; i < corresp_clusters.size(); i++) {
                ...
                // drawCorrespondences (processed, it_map->second,
                // keypoints_pointcloud, corresp_clusters[i]);

                Eigen::Matrix4f best_trans;
                typename pcl::registration::TransformationEstimationSVD<
                    PointInT, PointInT>
                    t_est;
                t_est.estimateRigidTransformation(
                    *(*it_map).second.correspondences_pointcloud,
                    *keypoints_pointcloud, corresp_clusters[i], best_trans);
...
#pragma omp parallel for schedule(dynamic, 4) num_threads(omp_get_num_procs())
        for (int i = 0; i < static_cast<int>(models_->size()); i++) {
            ...
            ConstPointInTPtr model_cloud =
                models_->at(i).getAssembled(VOXEL_SIZE_ICP_);
        ...
        (0.0025f); PointInTPtr model_aligned (new pcl::PointCloud<PointInT>);
           pcl::transformPointCloud (*model_cloud, *model_aligned,
        transforms_->at (i));
           pcl::visualization::PointCloudColorHandlerCustom<PointInT>
        random_handler (model_aligned, 0, 255, 0); vis.addPointCloud<PointInT>
        (model_aligned, random_handler, "cloud_model"); std::cout << "Adding
        point cloud..." << std::endl; vis.spin();
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::
    LocalRecognitionPipeline<Distance, PointInT, FeatureT>::getKeypoints(
        ModelT &model, int view_id,
        typename pcl::PointCloud<PointInT>::Ptr &keypoints_cloud) {

    if (use_cache_) {
        std::pair<std::string, int> pair_model_view =
            std::make_pair(model.id_, view_id);
        typename std::map<std::pair<std::string, int>, PointInTPtr>::iterator
            it = keypoints_cache_.find(pair_model_view);
