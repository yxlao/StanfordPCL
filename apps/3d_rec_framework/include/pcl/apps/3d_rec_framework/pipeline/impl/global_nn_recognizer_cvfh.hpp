...
void pcl::rec_3d_framework::GlobalNNCVFHRecognizer<
    Distance, PointInT, FeatureT>::getPose(ModelT &model, int view_id,
    ...
    if (use_cache_) {
        ...
        std::map<mv_pair, Eigen::Matrix4f, std::less<mv_pair>,
                 Eigen::aligned_allocator<
                     std::pair<mv_pair, Eigen::Matrix4f>>>::iterator it =
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
bool pcl::rec_3d_framework::GlobalNNCVFHRecognizer<
    Distance, PointInT, FeatureT>::getRollPose(ModelT &model, int view_id,
                                               int d_id,
                                               Eigen::Matrix4f &pose_matrix) {
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNCVFHRecognizer<
    Distance, PointInT, FeatureT>::getCentroid(ModelT &model, int view_id,
                                               int d_id,
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNCVFHRecognizer<
    Distance, PointInT, FeatureT>::getView(ModelT &model, int view_id,
                                           PointInTPtr &view) {
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNCVFHRecognizer<
    Distance, PointInT, FeatureT>::loadFeaturesAndCreateFLANN() {
    ...
    if (use_single_categories_) {
        for (size_t i = 0; i < models->size(); i++) {
            std::map<std::string, boost::shared_ptr<std::vector<int>>>::iterator
                it;
    ...
    for (size_t i = 0; i < models->size(); i++) {
        ...
        for (bf::directory_iterator itr_in(inside); itr_in != end_itr;
             ++itr_in) {
            ...
            if (strs[0] == "descriptor") {
                ...
                int size_feat =
                    sizeof(signature->points[0].histogram) / sizeof(float);
                ...
                if (use_single_categories_) {
                    std::map<std::string,
                             boost::shared_ptr<std::vector<int>>>::iterator it;
                    std::string cat_model = models->at(i).class_;
                    it = single_categories.find(cat_model);
                    if (it == single_categories.end()) {
                        ...
                        it->second->push_back(
                            static_cast<int>(flann_models_.size()));
                    }
                }
                ...
                if (use_cache_) {
                    ...
                    dir_pose << path << "/pose_" << descr_model.view_id
                             << ".txt";

                    Eigen::Matrix4f pose_matrix;
                    PersistenceUtils::readMatrixFromFile(dir_pose.str(),
                                                         pose_matrix);
    ...
    flann_index_ =
        new flann::Index<DistT>(flann_data_, flann::LinearIndexParams());
    ...
    if (use_single_categories_) {
        ...
        int kk = 0;
        for (it = single_categories.begin(); it != single_categories.end();
             it++) {
            // create index and flann data
            convertToFLANN(flann_models_, it->second,
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::
    GlobalNNCVFHRecognizer<Distance, PointInT, FeatureT>::nearestKSearch(
        flann::Index<DistT> *index, const flann_model &model, int k,
        flann::Matrix<int> &indices, flann::Matrix<float> &distances) {
    flann::Matrix<float> p = flann::Matrix<float>(new float[model.descr.size()],
                                                  1, model.descr.size());
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNCVFHRecognizer<Distance, PointInT,
                                                   FeatureT>::recognize() {
    ...
    if (signatures.size() > 0) {

        {
            ...
            if (use_single_categories_ &&
                (categories_to_be_searched_.size() > 0)) {

                // perform search of the different signatures in the
                // categories_to_be_searched_
                for (size_t c = 0; c < categories_to_be_searched_.size(); c++) {
                    std::cout
                        << "Using category:" << categories_to_be_searched_[c]
                        << std::endl;
                    for (size_t idx = 0; idx < signatures.size(); idx++) {
                        /*float* hist = signatures[idx].points[0].histogram;
                         std::vector<float> std_hist (hist, hist +
                         getHistogramLength (dummy)); flann_model histogram ("",
                         std_hist); flann::Matrix<int> indices;
                         flann::Matrix<float> distances;

                         std::map<std::string, int>::iterator it;
                         it = category_to_vectors_indices_.find
                         (categories_to_be_searched_[c]);

                         assert (it != category_to_vectors_indices_.end ());
                         nearestKSearch (single_categories_index_[it->second],
                         histogram, nmodels_, indices, distances);*/

                        float *hist = signatures[idx].points[0].histogram;
                        int size_feat =
                            sizeof(signatures[idx].points[0].histogram) /
                            sizeof(float);
                        ...
                        nearestKSearch(single_categories_index_[it->second],
                                       histogram, NN_, indices, distances);
                        ...
                        for (size_t i = 0; i < NN_; ++i) {
                            ...
                            index_score is;
                            is.idx_models_ =
                                single_categories_pointers_to_models_
                                    [it->second]
                                        ->at(indices[0][i]);
                    ...
                    // we cannot add more than nmodels per category, so sort
                    // here and remove offending ones...
                    std::sort(indices_scores.begin(), indices_scores.end(),
                              sortIndexScoresOp);
                ...
                for (size_t idx = 0; idx < signatures.size(); idx++) {
                    ...
                    flann::Matrix<int> indices;
                    flann::Matrix<float> distances;
                    nearestKSearch(flann_index_, histogram, NN_, indices,
                                   distances);
            ...
            /*
             * There might be duplicated candidates, in those cases it makes
             * sense to take the closer one in descriptor space
             */
            ...
            /*
             * Filter some hypothesis regarding to their distance to the first
             * neighbour
             */
            ...
            /*std::vector<index_score> indices_scores_filtered;
             ...
             {
             //std::cout << best_score << indices_scores[i].score_ <<
             (best_score / indices_scores[i].score_) << std::endl; if
             ((best_score / indices_scores[i].score_) > 0.65)
             {
             ...
             //std::cout << indices_scores_filtered.size () << " § " << num_n <<
             std::endl;

             indices_scores = indices_scores_filtered;
             num_n = indices_scores.size ();*/
            std::cout << "Number of object hypotheses... " << num_n
                      << std::endl;
            ...
            for (int i = 0; i < num_n; ++i) {
                ...
                int view_id =
                    flann_models_[indices_scores[i].idx_models_].view_id;
                int desc_id =
                    flann_models_[indices_scores[i].idx_models_].descriptor_id;

                int idx_input = indices_scores[i].idx_input_;

                std::cout << m.class_ << "/" << m.id_ << " ==> "
                          << indices_scores[i].score_ << std::endl;

                Eigen::Matrix4f roll_view_pose;
                bool roll_pose_found =
                    getRollPose(m, view_id, desc_id, roll_view_pose);
                ...
                if (roll_pose_found && valid_trans[idx_input]) {
                    ...
                    if (compute_scale_) {
                        ...
                        PointInTPtr view_transformed(
                            new pcl::PointCloud<PointInT>);
                        Eigen::Matrix4f hom_from_OVC_to_CC;
                        hom_from_OVC_to_CC =
                            transformations[idx_input].inverse() * transposed;
                        pcl::transformPointCloud(*view, *view_transformed,
                                                 hom_from_OVC_to_CC);

                        Eigen::Vector3f input_centroid =
                            centroids[indices_scores[i].idx_input_];
                        Eigen::Vector3f view_centroid;
                        getCentroid(m, view_id, desc_id, view_centroid);

                        Eigen::Vector4f cmatch4f(view_centroid[0],
                                                 view_centroid[1],
                                                 view_centroid[2], 0);
                        Eigen::Vector4f cinput4f(input_centroid[0],
                                                 input_centroid[1],
                                                 input_centroid[2], 0);
                        ...
                        std::cout << max_dist_view << " " << max_dist_input
                                  << std::endl;

                        float scale_factor_view =
                            max_dist_input / max_dist_view;
                        std::cout << "Scale factor:" << scale_factor_view
                                  << std::endl;
                    ...
                    hom_from_OC_to_CC = scale_mat *
                                        transformations[idx_input].inverse() *
                    ...
                    PCL_WARN("The roll pose was not found, should use CRH "
                             "here... \n");
                }
            }
        }

        std::cout << "Number of object hypotheses:" << models_->size()
                  << std::endl;
...
#pragma omp parallel for num_threads(omp_get_num_procs())
            for (int i = 0; i < static_cast<int>(models_->size()); i++) {
                ...
                if (compute_scale_) {
                    ...
                    pcl::transformPointCloud(*model_cloud, *model_aligned,
                                             transforms_->at(i));
                }
        ...
        if (hv_algorithm_) {
            ...
            std::vector<typename pcl::PointCloud<PointInT>::ConstPtr>
                aligned_models;
            ...
            for (size_t i = 0; i < models_->size(); i++) {
                ...
                // ConstPointInTPtr model_cloud = models_->at (i).getAssembled
                // (0.005f); PointInTPtr model_aligned (new
                // pcl::PointCloud<PointInT>); pcl::transformPointCloud
                // (*model_cloud, *model_aligned, transforms_->at (i));
            ...
            boost::shared_ptr<std::vector<
                Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>
                transforms_temp;
...
void pcl::rec_3d_framework::GlobalNNCVFHRecognizer<
    ...
    // use the source to know what has to be trained and what not, checking if
    // the descr_name directory exists unless force_retrain is true, then train
    // everything
    boost::shared_ptr<std::vector<ModelT>> models = source_->getModels();
    ...
    if (force_retrain) {
        for (size_t i = 0; i < models->size(); i++) {
            source_->removeDescDirectory(models->at(i), training_dir_,
                                         descr_name_);
        }
    }

    for (size_t i = 0; i < models->size(); i++) {
        if (!source_->modelAlreadyTrained(models->at(i), training_dir_,
                                          descr_name_)) {
            for (size_t v = 0; v < models->at(i).views_->size(); v++) {
                ...
                PersistenceUtils::writeMatrixToFile(
                    path_pose.str(), models->at(i).poses_->at(v));

                std::stringstream path_entropy;
                path_entropy << path << "/entropy_" << v << ".txt";
                PersistenceUtils::writeFloatToFile(
                    path_entropy.str(), models->at(i).self_occlusions_->at(v));

                // save signatures and centroids to disk
                for (size_t j = 0; j < signatures.size(); j++) {
                    if (valid_trans[j]) {
                        std::stringstream path_centroid;
                        path_centroid << path << "/centroid_" << v << "_" << j
                                      << ".txt";
                        Eigen::Vector3f centroid(
                            centroids[j][0], centroids[j][1], centroids[j][2]);
                        PersistenceUtils::writeCentroidToFile(
                            path_centroid.str(), centroid);

                        std::stringstream path_descriptor;
                        path_descriptor << path << "/descriptor_" << v << "_"
                                        << j << ".pcd";
                        pcl::io::savePCDFileBinary(path_descriptor.str(),
                                                   signatures[j]);

                        // save roll transform
                        std::stringstream path_pose;
                        path_pose << path << "/roll_trans_" << v << "_" << j
                                  << ".txt";
            ...
            // there is no need to keep the views in memory once the model has
            // been trained
