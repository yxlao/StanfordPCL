...
void pcl::rec_3d_framework::GlobalNNCRHRecognizer<
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNCRHRecognizer<
    Distance, PointInT, FeatureT>::getCRH(ModelT &model, int view_id, int d_id,
...
void pcl::rec_3d_framework::
    GlobalNNCRHRecognizer<Distance, PointInT, FeatureT>::nearestKSearch(
...
template <template <class> class Distance, typename PointInT, typename FeatureT>
void pcl::rec_3d_framework::GlobalNNCRHRecognizer<Distance, PointInT,
                                                  FeatureT>::recognize() {
    ...
    if (signatures.size() > 0) {

        {
            ...
            for (size_t idx = 0; idx < signatures.size(); idx++) {
                ...
                int size_feat =
                    sizeof(signatures[idx].points[0].histogram) / sizeof(float);
            ...
            if (do_CRH_) {
                /*
                 * Once we have the models, we need to find a 6DOF pose using
                 * the roll histogram pass to pcl_recognition::CRHAlignment both
                 * views, centroids and CRH
                 */
                ...
                for (int i = 0; i < num_n; ++i) {
                    ModelT m =
                        flann_models_[indices_scores[i].idx_models_].model;
                    int view_id =
                        flann_models_[indices_scores[i].idx_models_].view_id;
                    int desc_id = flann_models_[indices_scores[i].idx_models_]
                                      .descriptor_id;

                    std::cout << m.id_ << " " << view_id << " " << desc_id
                              << std::endl;

                    // get crhs
                    CRHPointCloud::Ptr input_crh =
                        crh_histograms[indices_scores[i].idx_input_];
                    ...
                    crha.setInputAndTargetCentroids(view_centroid,
                                                    input_centroid);
                    ...
                    for (size_t k = 0; k < roll_transforms.size(); k++) {
                        Eigen::Matrix4f final_roll_trans(roll_transforms[k] *
                                                         model_view_pose);
                        models_->push_back(m);
...
#pragma omp parallel for num_threads(omp_get_num_procs())
        ...
        if (hv_algorithm_) {
            ...
            for (size_t i = 0; i < models_->size(); i++) {
                ConstPointInTPtr model_cloud =
                    models_->at(i).getAssembled(0.005f);
                PointInTPtr model_aligned(new pcl::PointCloud<PointInT>);
                pcl::transformPointCloud(*model_cloud, *model_aligned,
                                         transforms_->at(i));
        ...
        if (!source_->modelAlreadyTrained(models->at(i), training_dir_,
            ...
            for (size_t v = 0; v < models->at(i).views_->size(); v++) {
                ...
                std::vector<pcl::PointCloud<FeatureT>,
                    ...
                    signatures;
                std::vector<Eigen::Vector3f> centroids;
                crh_estimator_->estimate(view, processed, signatures,
                                         centroids);
                ...
                for (size_t j = 0; j < signatures.size(); j++) {
                    ...
                    Eigen::Vector3f centroid(centroids[j][0], centroids[j][1],
                                             centroids[j][2]);
                    PersistenceUtils::writeCentroidToFile(path_centroid.str(),
                                                          centroid);

                    std::stringstream path_descriptor;
                    path_descriptor << path << "/descriptor_" << v << "_" << j
                                    << ".pcd";
                    ...
                    pcl::io::savePCDFileBinary(path_roll.str(),
                                               *crh_histograms[j]);
                }
            }
