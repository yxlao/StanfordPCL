...
template <class T> struct HistIntersectionUnionDistance {
    typedef T ElementType;
    typedef typename Accumulator<T>::Type ResultType;

    /**
     *  Compute a distance between two vectors using (1 - (1 +
     * sum(min(a_i,b_i))) / (1 + sum(max(a_i, b_i))) )
     *
     *  This distance is not a valid kdtree distance, it's not dimensionwise
     * additive and ignores worst_dist parameter.
     */
