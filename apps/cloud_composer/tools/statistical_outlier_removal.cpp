...
Q_EXPORT_PLUGIN2(cloud_composer_statistical_outlier_removal_tool,
                 pcl::cloud_composer::StatisticalOutlierRemovalToolFactory)

pcl::cloud_composer::StatisticalOutlierRemovalTool::
    StatisticalOutlierRemovalTool(PropertiesModel *parameter_model,
                                  QObject *parent)
...
pcl::cloud_composer::StatisticalOutlierRemovalTool::performAction(
    ...
    if (input_data.size() == 0) {
        ...
        qWarning() << "Input vector has more than one item in "
                      "StatisticalOutlierRemovalTool";
    }
    ...
    if (input_item->type() == CloudComposerItem::CLOUD_ITEM) {
        ...
        double std_dev_thresh =
            parameter_model_->getProperty("Std Dev Thresh").toDouble();
        ...
        CloudItem *cloud_item =
            new CloudItem(input_item->text() + tr(" sor filtered"),
                          cloud_filtered, source_origin, source_orientation);

        output.append(cloud_item);
    } else {
        qDebug()
            << "Input item in StatisticalOutlierRemovalTool is not a cloud!!!";
    }
...
pcl::cloud_composer::StatisticalOutlierRemovalToolFactory::
    ...
    parameter_model->addProperty("Mean K", 50,
