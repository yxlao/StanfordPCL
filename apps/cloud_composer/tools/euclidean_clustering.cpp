...
Q_EXPORT_PLUGIN2(cloud_composer_euclidean_clustering_tool,
...
pcl::cloud_composer::EuclideanClusteringTool::performAction(
    ...
    if (input_data.size() == 0) {
        ...
        qWarning()
            << "Input vector has more than one item in Euclidean Clustering!";
    }
    input_item = input_data.value(0);

    if (input_item->type() == CloudComposerItem::CLOUD_ITEM) {
        const CloudItem *cloud_item =
            dynamic_cast<const CloudItem *>(input_item);
        if (cloud_item->isSanitized()) {
            ...
            int max_cluster_size =
                parameter_model_->getProperty("Max Cluster Size").toInt();

            sensor_msgs::PointCloud2::ConstPtr input_cloud =
                input_item->data(ItemDataRole::CLOUD_BLOB)
                    .value<sensor_msgs::PointCloud2::ConstPtr>();
            ...
            Eigen::Quaternionf source_orientation =
                input_item->data(ItemDataRole::ORIENTATION)
            ...
            for (std::vector<pcl::PointIndices>::const_iterator it =
                ...
                // It's annoying that I have to do this, but Euclidean returns a
                // PointIndices struct
                ...
                qDebug() << "Cluster has " << cloud_filtered->width
                         << " data points.";
                CloudItem *cloud_item = new CloudItem(
                    input_item->text() + tr("-Clstr %1").arg(cluster_count),
                    cloud_filtered, source_origin, source_orientation);
                output.append(cloud_item);
                ++cluster_count;
            }
            // We copy input cloud over for special case that no clusters found,
            // since ExtractIndices doesn't work for 0 length vectors
            sensor_msgs::PointCloud2::Ptr remainder_cloud(
                new sensor_msgs::PointCloud2(*input_cloud));
            ...
            CloudItem *cloud_item = new CloudItem(
                input_item->text() + " unclustered", remainder_cloud,
