...
pcl::cloud_composer::CloudView::CloudView(const CloudView &to_copy)
    : QWidget(), vis_(to_copy.vis_), model_(to_copy.model_),
      qvtk_(to_copy.qvtk_) {}
...
void pcl::cloud_composer::CloudView::connectSignalsAndSlots() {
...
void pcl::cloud_composer::CloudView::refresh() { qvtk_->update(); }

void pcl::cloud_composer::CloudView::itemChanged(QStandardItem *changed_item) {
...
void pcl::cloud_composer::CloudView::rowsInserted(const QModelIndex &parent,
    ...
    // If the parent is invisibleRootItem this will be true (parent is not
    // valid)
    if (!parent.isValid())
    ...
    for (int row = start; row <= end; ++row) {
        ...
        //  qDebug () << "Making recursive call, start =0,
        //  end="<<new_item->rowCount ()-1;
...
void pcl::cloud_composer::CloudView::rowsAboutToBeRemoved(
    ...
    for (int row = start; row <= end; ++row) {
        ...
        CloudComposerItem *item =
            dynamic_cast<CloudComposerItem *>(item_to_remove);
        if (item)
            item->removeFromView(vis_);

        // Recursive call, need to remove all children as well
        //  qDebug () << "Making recursive call, start =0,
        //  end="<<item_to_remove->rowCount ()-1;
...
void pcl::cloud_composer::CloudView::paintEvent(QPaintEvent *) {
    qvtk_->update();
}

void pcl::cloud_composer::CloudView::resizeEvent(QResizeEvent *) {
    qvtk_->update();
}

void pcl::cloud_composer::CloudView::selectedItemChanged(
        ...
        if (previous.isValid()) {
            ...
            if (previous_item->type() == CloudComposerItem::CLOUD_ITEM ||
                ...
                vis_->setPointCloudSelected(
                    false, previous_item->data(ItemDataRole::ITEM_ID)
                               .toString()
                               .toStdString());
            }
        }
    ...
    foreach (QModelIndex current, current_indices) {
        if (current.isValid()) {
            ...
            if (current_item->type() == CloudComposerItem::CLOUD_ITEM ||
                ...
                vis_->setPointCloudSelected(
                    true, current_item->data(ItemDataRole::ITEM_ID)
                              .toString()
                              .toStdString());
            }
        }
...
void pcl::cloud_composer::CloudView::dataChanged(
...
void pcl::cloud_composer::CloudView::addOrientationMarkerWidgetAxes() {
    if (!axes_widget_) {
...
void pcl::cloud_composer::CloudView::removeOrientationMarkerWidgetAxes() {
    if (axes_widget_) {
...
/////////////////////////////////////////////////////////////////////////////
void pcl::cloud_composer::CloudView::initializeInteractorSwitch() {
    ...
    connections_->Connect(
        style_switch_->getInteractorStyle(
            interactor_styles::RECTANGULAR_FRUSTUM),
    ...
    connections_->Connect(
        ...
        SLOT(
            manipulationCompleted(vtkObject *, unsigned long, void *, void *)));
}

void pcl::cloud_composer::CloudView::setInteractorStyle(
...
void pcl::cloud_composer::CloudView::selectionCompleted(vtkObject *caller,
    ...
    boost::shared_ptr<SelectionEvent> selected(
        static_cast<SelectionEvent *>(call_data));

    if (selected) {
        qDebug() << "Selection Complete! - Num points="
                 << selected->getNumPoints();
        model_->setPointSelection(selected);
        style_switch_->setCurrentInteractorStyle(
            interactor_styles::PCL_VISUALIZER);
    }
}

void pcl::cloud_composer::CloudView::manipulationCompleted(
    vtkObject *caller, unsigned long event_id, void *client_data,
    void *call_data) {
