...
void pcl::cloud_composer::SignalMultiplexer::connect(const Connection &conn) {
    if (!object)
        return;
    ...
    if (conn.sender)
        QObject::connect((QObject *)conn.sender, conn.signal, (QObject *)object,
                         conn.slot);
    else
...
void pcl::cloud_composer::SignalMultiplexer::disconnect(
...
void pcl::cloud_composer::SignalMultiplexer::setCurrentObject(
    QObject *newObject) {
    if (newObject == object)
        return;
