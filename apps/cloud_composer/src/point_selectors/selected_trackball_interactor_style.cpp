...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::
    setSelectedActors() {
    QList<QString> selected_cloud_ids;
    QModelIndexList selected_indexes =
        model_->getSelectionModel()->selectedIndexes();
    ...
    for (it = actors_->begin(); it != actors_->end(); ++it) {
        ...
        if (selected_cloud_ids.contains(id)) {
            ...
            vtkSmartPointer<vtkMatrix4x4> start_matrix =
...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::OnLeftButtonDown() {
...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::OnLeftButtonUp() {
    ...
    foreach (QString id, selected_actors_map_.keys()) {
        ...
        vtkSmartPointer<vtkMatrix4x4> end_matrix =
            vtkSmartPointer<vtkMatrix4x4>::New();
        actor->GetMatrix(end_matrix);
        manip_event->addManipulation(id, start_matrix_map_.value(id),
                                     end_matrix);
        this->InvokeEvent(this->manipulation_complete_event_, manip_event);
    }
}

void pcl::cloud_composer::SelectedTrackballStyleInteractor::OnRightButtonUp() {
...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::Rotate() {
    ...
    double radius =
        sqrt(vtkMath::Distance2BetweenPoints(disp_obj_center, outsidept));
...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::Spin() {
    if (this->CurrentRenderer == NULL || this->InteractionProp == NULL) {
        return;
    }
...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::Pan() {
    if (this->CurrentRenderer == NULL || this->InteractionProp == NULL) {
        return;
    }
    ...
    this->ComputeDisplayToWorld(rwi->GetEventPosition()[0],
                                rwi->GetEventPosition()[1], disp_obj_center[2],
                                new_pick_point);
    ...
    foreach (QString id, selected_actors_map_.keys()) {
        ...
        if (actor->GetUserMatrix() != NULL) {
            ...
            actor->AddPosition(motion_vector[0], motion_vector[1],
                               motion_vector[2]);
        }
    }
...
void pcl::cloud_composer::SelectedTrackballStyleInteractor::UniformScale() {
    if (this->CurrentRenderer == NULL || this->InteractionProp == NULL) {
        return;
    }
