...
pcl::cloud_composer::InteractorStyleSwitch::InteractorStyleSwitch() {
    pcl_vis_style_ = vtkSmartPointer<
        pcl::visualization::PCLVisualizerInteractorStyle>::New();
    name_to_style_map_.insert(interactor_styles::PCL_VISUALIZER,
                              pcl_vis_style_);

    rectangular_frustum_selector_ =
        vtkSmartPointer<RectangularFrustumSelector>::New();
...
void pcl::cloud_composer::InteractorStyleSwitch::initializeInteractorStyles(
    ...
    selected_trackball_interactor_style_->setCloudActorMap(
        vis_->getCloudActorMap());
    selected_trackball_interactor_style_->setProjectModel(project_model_);

    click_trackball_interactor_style_->setCloudActorMap(
        vis_->getCloudActorMap());
    click_trackball_interactor_style_->setProjectModel(project_model_);
}

void pcl::cloud_composer::InteractorStyleSwitch::setCurrentInteractorStyle(
    ...
    if (current_style_) {
        ...
        if (interactor_style == interactor_styles::RECTANGULAR_FRUSTUM) {
            ...
            if (rubber_band_style) {
                vis_->getRenderWindow()->GetInteractor()->SetPicker(
                    area_picker_);
...
//----------------------------------------------------------------------------
void pcl::cloud_composer::InteractorStyleSwitch::SetInteractor(
...
//----------------------------------------------------------------------------
void pcl::cloud_composer::InteractorStyleSwitch::SetDefaultRenderer(
...
//----------------------------------------------------------------------------
void pcl::cloud_composer::InteractorStyleSwitch::SetCurrentRenderer(
    vtkRenderer *renderer) {
...
void pcl::cloud_composer::InteractorStyleSwitch::OnLeave() {
    qDebug() << "ON LEAVE";
}
