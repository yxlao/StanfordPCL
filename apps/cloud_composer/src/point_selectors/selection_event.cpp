...
void pcl::cloud_composer::SelectionEvent::findIndicesInItem(
    CloudItem *cloud_item, pcl::PointIndices::Ptr indices) {
    // WE DONT NEED TO DO THIS SEARCH BECAUSE WE HAVE A 1-1 CORRESPONDENCE VTK
    // TO PCL THIS IS ONLY THE CASE FOR CLOUDS WITH NO NANs
    // Go through every point in the selected data set and find the matching
    // index in this cloud pcl::search::KdTree<pcl::PointXYZ>::Ptr search =
    // cloud_item->data (KD_TREE_SEARCH).value
    // <pcl::search::Search<pcl::PointXYZ>::Ptr> ();
    // double dblpts[3];
    // PointXYZ pt;
    ...
    /*for (int i = 0; i < points->GetNumberOfPoints (); ++i)
    {
      ...
      else if (indices.size () > 1)
      {
        ...
        if (idx < indices.size ())
        {
          ...
          qCritical () << "More found points in selection than are possible from
    cloud items!";
      }

    }*/
    if (id_selected_data_map_.contains(cloud_item->getId())) {
        vtkPolyData *points_in_item =
            id_selected_data_map_.value(cloud_item->getId());
        ...
        // qDebug () << points_in_item->GetNumberOfPoints () << " selected
        // points in "<<cloud_item->getId ();
    }
}
