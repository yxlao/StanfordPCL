...
void pcl::cloud_composer::ClickTrackballStyleInteractor::OnLeftButtonDown() {
...
void pcl::cloud_composer::ClickTrackballStyleInteractor::OnRightButtonDown() {
...
void pcl::cloud_composer::ClickTrackballStyleInteractor::OnLeftButtonUp() {
    ...
    if (selected_actor) {
        ...
        for (pcl::visualization::CloudActorMap::const_iterator itr =
                 actors_->begin();
             itr != end; ++itr) {
        ...
        if (!manipulated_id.isEmpty()) {
            manip_event->addManipulation(manipulated_id, start_matrix_,
                                         end_matrix_);
            this->InvokeEvent(this->manipulation_complete_event_, manip_event);
        } else {
            qWarning()
                << "Could not find actor which matches manipulated actor in "
                   "ClickTrackballStyleInteractor::OnLeftButtonUp!!!";
        }
    }
}

void pcl::cloud_composer::ClickTrackballStyleInteractor::OnRightButtonUp() {
