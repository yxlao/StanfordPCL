...
pcl::cloud_composer::MergeSelection::performAction(
    ...
    foreach (const CloudComposerItem *input_item, input_data) {
        if (input_item->type() != CloudComposerItem::CLOUD_ITEM) {
            qCritical() << "Input in MergeSelection not valid, contained "
                           "non-CloudItem input";
            return output;
        }
    }
    ...
    foreach (const CloudItem *input_cloud_item,
        ...
        if (!input_data.contains(input_cloud_item)) {
            ...
            sensor_msgs::PointCloud2::Ptr original_minus_indices =
                boost::make_shared<sensor_msgs::PointCloud2>();
            filter.setNegative(true);
            filter.filter(*original_minus_indices);
            filter.setNegative(false);
            sensor_msgs::PointCloud2::Ptr selected_points(
            ...
            Eigen::Vector4f source_origin =
                input_cloud_item->data(ItemDataRole::ORIGIN)
                    .value<Eigen::Vector4f>();
            Eigen::Quaternionf source_orientation =
                input_cloud_item->data(ItemDataRole::ORIENTATION)
            ...
            sensor_msgs::PointCloud2::Ptr temp_cloud =
                boost::make_shared<sensor_msgs::PointCloud2>();
