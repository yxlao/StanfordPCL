...
pcl::cloud_composer::NormalsItem::NormalsItem(
    QString name, pcl::PointCloud<pcl::Normal>::Ptr normals_ptr, double radius)
    : CloudComposerItem(name), normals_ptr_(normals_ptr)

{
    pcl::PointCloud<pcl::Normal>::ConstPtr normals_const = normals_ptr;
    this->setData(QVariant::fromValue(normals_const),
...
void pcl::cloud_composer::NormalsItem::paintView(
    ...
    if (parent()->type() == CLOUD_ITEM) {
        ...
        sensor_msgs::PointCloud2::ConstPtr cloud_blob =
            cloud_ptr.value<sensor_msgs::PointCloud2::ConstPtr>();
        ...
        qWarning() << "Normal item inserted, but parent not a cloud. Don't "
                      "know how to draw that!";
}

void pcl::cloud_composer::NormalsItem::removeFromView(
    boost::shared_ptr<pcl::visualization::PCLVisualizer> vis) const {
