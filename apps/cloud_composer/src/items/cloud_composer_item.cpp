...
pcl::cloud_composer::CloudComposerItem::getCloudPtr () const
{
  ...
  // Get Extract the pointer from the cloud contained in this item, if the type
can't be converted, default-constructed value is returned CloudPtrT ptr; if
(cloud_variant.canConvert <CloudPtrT> ()) ptr =  cloud_variant.value <CloudPtrT>
(); else qCritical () << "Requested Cloud of incorrect type from "<<this->text
()<<" correct type is "<<cloud_variant.typeName();
