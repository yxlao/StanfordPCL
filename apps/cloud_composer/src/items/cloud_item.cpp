...
pcl::cloud_composer::CloudItem::CloudItem(
    QString name, sensor_msgs::PointCloud2::Ptr cloud_ptr,
    const Eigen::Vector4f &origin, const Eigen::Quaternionf &orientation,
    bool make_templated_cloud)
    : CloudComposerItem(name), origin_(origin), orientation_(orientation),
      template_cloud_set_(false), point_type_(PointTypeFlags::NONE),
      is_sanitized_(false) {
    ...
    this->setData(QVariant::fromValue(const_cloud_ptr),
                  ItemDataRole::CLOUD_BLOB);
    ...
    color_handler_.reset(new pcl::visualization::PointCloudColorHandlerRGBField<
                         sensor_msgs::PointCloud2>(cloud_ptr));
    this->setData(QVariant::fromValue(color_handler_),
                  ItemDataRole::COLOR_HANDLER);
    ...
    properties_->addProperty("Width", QVariant(cloud_blob_ptr_->width),
                             Qt::NoItemFlags, "Core Properties");
...
pcl::cloud_composer::CloudItem *pcl::cloud_composer::CloudItem::clone() const {
    sensor_msgs::PointCloud2::Ptr cloud_copy(
        new sensor_msgs::PointCloud2(*cloud_blob_ptr_));
    // Vector4f and Quaternionf do deep copies using constructor
    CloudItem *new_item =
        new CloudItem(this->text(), cloud_copy, origin_, orientation_);
...
void pcl::cloud_composer::CloudItem::paintView(
    boost::shared_ptr<pcl::visualization::PCLVisualizer> vis) const {
    vis->addPointCloud(cloud_blob_ptr_, geometry_handler_, color_handler_,
                       origin_, orientation_, getId().toStdString());
...
void pcl::cloud_composer::CloudItem::removeFromView(
    boost::shared_ptr<pcl::visualization::PCLVisualizer> vis) const {
    vis->removePointCloud(getId().toStdString());
}

QVariant pcl::cloud_composer::CloudItem::data(int role) const {
    // Check if we're trying to get something which is template dependant, if
    // so, create the template if it hasn't been set
    if ((role == ItemDataRole::CLOUD_TEMPLATED ||
         role == ItemDataRole::KD_TREE_SEARCH) &&
        !template_cloud_set_) {
        qCritical()
            << "Attempted to access templated types which are not set!!";
    }

    return CloudComposerItem::data(role);
}

void pcl::cloud_composer::CloudItem::setData(const QVariant &value, int role) {
    if (role == ItemDataRole::CLOUD_TEMPLATED)
...
void pcl::cloud_composer::CloudItem::setTemplateCloudFromBlob() {
    if (!template_cloud_set_) {
        int num_fields = cloud_blob_ptr_->fields.size();
        std::vector<sensor_msgs::PointField>::iterator end =
            cloud_blob_ptr_->fields.end();
        std::vector<sensor_msgs::PointField>::iterator itr =
            cloud_blob_ptr_->fields.begin();
        ...
        if (field_names.contains("normal_x") &&
            field_names.contains("normal_y") &&
        ...
        case (PointTypeFlags::XYZ): {
            pcl::PointCloud<PointXYZ>::Ptr cloud_ptr =
                boost::make_shared<pcl::PointCloud<PointXYZ>>();
            ...
            pcl::search::KdTree<PointXYZ>::Ptr kd_search =
                boost::make_shared<search::KdTree<PointXYZ>>();
            kd_search->setInputCloud(cloud_ptr);
            kd_tree_variant = QVariant::fromValue(kd_search);
            break;
        }
        case (PointTypeFlags::XYZ | PointTypeFlags::RGB): {
            pcl::PointCloud<PointXYZRGB>::Ptr cloud_ptr =
                boost::make_shared<pcl::PointCloud<PointXYZRGB>>();
            pcl::fromROSMsg(*cloud_blob_ptr_, *cloud_ptr);
            cloud_pointer_variant = QVariant::fromValue(cloud_ptr);
            pcl::search::KdTree<PointXYZRGB>::Ptr kd_search =
                boost::make_shared<search::KdTree<PointXYZRGB>>();
            kd_search->setInputCloud(cloud_ptr);
            kd_tree_variant = QVariant::fromValue(kd_search);
            break;
        }
        case (PointTypeFlags::XYZ | PointTypeFlags::RGBA): {
            pcl::PointCloud<PointXYZRGBA>::Ptr cloud_ptr =
                boost::make_shared<pcl::PointCloud<PointXYZRGBA>>();
            pcl::fromROSMsg(*cloud_blob_ptr_, *cloud_ptr);
            cloud_pointer_variant = QVariant::fromValue(cloud_ptr);
            pcl::search::KdTree<PointXYZRGBA>::Ptr kd_search =
                boost::make_shared<search::KdTree<PointXYZRGBA>>();
            kd_search->setInputCloud(cloud_ptr);
            kd_tree_variant = QVariant::fromValue(kd_search);
            break;
        }
        ...
        case (PointTypeFlags::NONE):
            QMessageBox::warning(
                0, "Unknown blob type!",
                "Could not find appropriate template type for this cloud blob! "
                "Only blob functionality enabled!");
        }
        ...
        qDebug()
            << "Trying to set cloud from blob, but template cloud already set!";
}

bool pcl::cloud_composer::CloudItem::checkIfFinite() {
    if (!cloud_blob_ptr_)
        return false;
