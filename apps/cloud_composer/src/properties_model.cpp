...
pcl::cloud_composer::PropertiesModel::PropertiesModel(
    CloudComposerItem *parent_item, QObject *parent)
...
void pcl::cloud_composer::PropertiesModel::addProperty(const QString prop_name,
    ...
    if (category.size() > 0) {
        ...
        if (items.size() == 0)
            ...
            qCritical() << "Multiple categories with same name found!! This is "
                           "not good...";
        else
...
void pcl::cloud_composer::PropertiesModel::addCategory(
    const QString category_name) {
    QStandardItem *new_category = new QStandardItem(category_name);
    appendRow(new_category);
}

QVariant pcl::cloud_composer::PropertiesModel::getProperty(
    const QString prop_name) const {
    ...
    if (items.size() == 0) {
        ...
        qWarning() << "Multiple properties found with name " << prop_name
                   << " in " << parent_item_->text();
    }
...
void pcl::cloud_composer::PropertiesModel::copyProperties(
    const PropertiesModel *to_copy) {
    for (int i = 0; i < to_copy->rowCount(); ++i) {
        QList<QStandardItem *> new_row;
        QStandardItem *parent = to_copy->item(i, 0);
        QModelIndex parent_index = to_copy->index(i, 0);
        qDebug() << "Copying " << parent->text()
                 << " cols =" << to_copy->columnCount();
        new_row.append(parent->clone());
...
void pcl::cloud_composer::PropertiesModel::propertyChanged(
    QStandardItem *property_item) {
