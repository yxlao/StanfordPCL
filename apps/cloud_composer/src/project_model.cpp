...
void pcl::cloud_composer::ProjectModel::setPointSelection(
    boost::shared_ptr<SelectionEvent> selected_event) {
    ...
    foreach (CloudItem *cloud_item, project_clouds) {
        pcl::PointIndices::Ptr found_indices =
        ...
        if (found_indices->indices.size() > 0) {
            qDebug() << "Found " << found_indices->indices.size()
                     << " points in " << cloud_item->text();
...
void pcl::cloud_composer::ProjectModel::manipulateClouds(
    ...
    QMap<QString, vtkSmartPointer<vtkMatrix4x4>> transform_map =
        manip_event->getEndMap();
...
void pcl::cloud_composer::ProjectModel::insertNewCloudFromFile() {
    qDebug() << "Inserting cloud from file...";
    QString filename = QFileDialog::getOpenFileName(
        0, tr("Select cloud to open"), last_directory_.absolutePath(),
        tr("PointCloud(*.pcd)"));
    if (filename.isNull()) {
    ...
    pcl::PCDReader pcd;
    if (pcd.read(filename.toStdString(), *cloud_blob, origin, orientation,
                 version) < 0) {
...
void pcl::cloud_composer::ProjectModel::enqueueToolAction(AbstractTool *tool) {
    qDebug() << "Enqueuing tool action " << tool->getToolName()
             << " in project model " << this->getName();
    // Get the currently selected item(s), put them in a list, and create the
    // command
    ConstItemList input_data;
    ...
    if (selected_indexes.size() == 0) {
        QMessageBox::warning(qobject_cast<QWidget *>(this->parent()),
                             "No Items Selected",
                             "Cannot use tool, no item is selected in the "
...
void pcl::cloud_composer::ProjectModel::commandCompleted(
    CloudCommand *command) {
    // We set the project model here - this wasn't done earlier so model is
...
void pcl::cloud_composer::ProjectModel::insertNewCloudComposerItem(
...
///////////////////////////////////////////////////////////////
void pcl::cloud_composer::ProjectModel::clearSelection() {
...
void pcl::cloud_composer::ProjectModel::deleteSelectedItems() {

    QModelIndexList selected_indexes = selection_model_->selectedIndexes();
    if (selected_indexes.size() == 0) {
        QMessageBox::warning(qobject_cast<QWidget *>(this->parent()),
                             "No Items Selected",
                             "Cannot execute delete command, no item is "
                             "selected in the browser or cloud view");
        return;
    }
    ...
    // qDebug () << "Input for command is "<<input_data.size () << "
    // element(s)";
...
void pcl::cloud_composer::ProjectModel::setAxisVisibility(bool visible) {
    // qDebug () << "Setting axis visibility to "<<visible;
    axis_visible_ = visible;
    cloud_view_->setAxisVisibility(axis_visible_);
}

void pcl::cloud_composer::ProjectModel::mouseStyleChanged(
    QAction *new_style_action) {
...
void pcl::cloud_composer::ProjectModel::createNewCloudFromSelection() {
    // We need only clouds to be selected
    if (!onlyCloudItemsSelected()) {
        qCritical() << "Only Clouds Selected = False -- Cannot create a new "
                       "cloud from non-cloud selection";
        return;
    }
    ...
    // We don't call the enqueueToolAction function since that would abort if we
    // only have a green selection Move the tool object to the work queue thread
    merge_tool->moveToThread(work_thread_);
    // Emit signal which tells work queue to enqueue this new action
    emit enqueueNewAction(merge_tool, input_data);
}

void pcl::cloud_composer::ProjectModel::selectAllItems(QStandardItem *item) {
...
////////////////////////////////////////////////////////
void pcl::cloud_composer::ProjectModel::emitAllStateSignals() {
    ...
    QMap<interactor_styles::INTERACTOR_STYLES, bool>::iterator itr =
        selected_style_map_.begin();
    foreach (interactor_styles::INTERACTOR_STYLES style,
             selected_style_map_.keys()) {
...
void pcl::cloud_composer::ProjectModel::itemSelectionChanged(
    const QItemSelection &selected, const QItemSelection &deselected) {
    // qDebug () << "Item selection changed!";
    // Set all point selected cloud items back to green text, since if they are
    // selected they get changed to white
    foreach (CloudItem *selected_item, selected_item_index_map_.keys()) {
...
bool pcl::cloud_composer::ProjectModel::onlyCloudItemsSelected() {
    QModelIndexList selected_indexes = selection_model_->selectedIndexes();
    foreach (QModelIndex model_index, selected_indexes) {
        if (this->itemFromIndex(model_index)->type() !=
            CloudComposerItem::CLOUD_ITEM) {
            return false;
        }
    }
    return true;
...
void pcl::cloud_composer::ProjectModel::setSelectedStyle(
