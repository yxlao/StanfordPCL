...
void pcl::cloud_composer::ComposerMainWindow::connectFileActions() {}

void pcl::cloud_composer::ComposerMainWindow::connectEditActions() {
    // Replace the actions in the menu with undo actions created using the undo
    // group
    ...
    // multiplexer_->connect (action_manipulate_selected_, SIGNAL (triggered
    // ()), SLOT (selectedTrackballInteractorStyle ()));
...
void pcl::cloud_composer::ComposerMainWindow::setMouseStyleAction(
...
void pcl::cloud_composer::ComposerMainWindow::connectViewActions() {
...
void pcl::cloud_composer::ComposerMainWindow::initializeCloudBrowser() {
...
void pcl::cloud_composer::ComposerMainWindow::initializeCloudViewer() {
    // Signal emitted when user selects new tab (ie different project) in the
    // viewer
    connect(cloud_viewer_, SIGNAL(newModelSelected(ProjectModel *)), this,
            SLOT(setCurrentModel(ProjectModel *)));
}

void pcl::cloud_composer::ComposerMainWindow::initializeItemInspector() {}

void pcl::cloud_composer::ComposerMainWindow::initializeToolBox() {
    tool_box_model_ =
        new ToolBoxModel(tool_box_view_, tool_parameter_view_, this);
    ...
    connect(action_run_tool_, SIGNAL(clicked()), tool_box_model_,
            SLOT(toolAction()));
    // tool_box_view_->setStyleSheet("branch:has-siblings:!adjoins-item:image
    // none");
    // tool_box_view_->setStyleSheet("branch:!has-children:!has-siblings:adjoins-item:image:
    // none");
}

void pcl::cloud_composer::ComposerMainWindow::initializePlugins() {
    ...
    foreach (QString filename, plugin_dir.entryList(QDir::Files)) {
        ...
        // This is automatically deleted when the library is unloaded (on app
        // exit)
        ...
        if (tool_factory) {
            ...
            qDebug() << "Could not load "
                     << plugin_dir.relativeFilePath(filename);
            qDebug() << loader.errorString();
        }
    }
}

void pcl::cloud_composer::ComposerMainWindow::setCurrentModel(
    ProjectModel *model) {
...
void pcl::cloud_composer::ComposerMainWindow::enqueueToolAction(
    AbstractTool *tool) {
    if (current_model_)
...
void pcl::cloud_composer::ComposerMainWindow::
...
void pcl::cloud_composer::ComposerMainWindow::on_action_exit__triggered() {
    qDebug() << "Exiting...";
}
