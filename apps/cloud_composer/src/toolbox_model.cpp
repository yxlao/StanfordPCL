...
pcl::cloud_composer::ToolBoxModel::ToolBoxModel(QTreeView *tool_view,
                                                ...
                                                QObject *parent)
    : QStandardItemModel(parent), tool_view_(tool_view),
      parameter_view_(parameter_view_), project_model_(0) {}
...
void pcl::cloud_composer::ToolBoxModel::addTool(ToolFactory *tool_factory) {
    ...
    PropertiesModel *new_tool_parameters =
        tool_factory->createToolParameterModel(this);
    new_tool_item->setData(QVariant::fromValue(new_tool_parameters),
                           PARAMETER_MODEL);
...
void pcl::cloud_composer::ToolBoxModel::setSelectionModel(
...
void pcl::cloud_composer::ToolBoxModel::activeProjectChanged(
    ProjectModel *new_model, ProjectModel *previous_model) {
    ...
    if (project_model_) {
        ...
        disconnect(project_model_, SIGNAL(modelChanged()), this,
                   SLOT(modelChanged()));
    }
    ...
    if (project_model_) {
        ...
        connect(project_model_->getSelectionModel(),
                ...
                SLOT(selectedItemChanged(QItemSelection, QItemSelection)));
        connect(project_model_, SIGNAL(modelChanged()), this,
                SLOT(modelChanged()));
    }
}

void pcl::cloud_composer::ToolBoxModel::selectedToolChanged(
...
void pcl::cloud_composer::ToolBoxModel::toolAction() {
    ...
    ToolFactory *tool_factory =
        (current_index.data(FACTORY)).value<ToolFactory *>();
...
void pcl::cloud_composer::ToolBoxModel::selectedItemChanged(
    const QItemSelection &selected, const QItemSelection &deselected) {
    updateEnabledTools(selected);
}

void pcl::cloud_composer::ToolBoxModel::enableAllTools() {
    foreach (QStandardItem *tool, tool_items) { tool->setEnabled(true); }
}

void pcl::cloud_composer::ToolBoxModel::modelChanged() {
    updateEnabledTools(project_model_->getSelectionModel()->selection());
}
void pcl::cloud_composer::ToolBoxModel::updateEnabledTools(
    ...
    foreach (QModelIndex current, current_indices) {
        if (current.isValid()) {
            QStandardItem *current_item =
                project_model_->itemFromIndex(current);
    ...
    while (enabled_itr.hasNext()) {
        ...
        ToolFactory *tool_factory =
            (tool_item->data(FACTORY)).value<ToolFactory *>();
        CloudComposerItem::ItemType input_type =
            tool_factory->getInputItemType();
        ...
        // Check if selection includes at least one item with correct input type
            ...
            disabled_tools.insert(
                tool_item, tr("Tool Requires item type %1 selected")
        ...
        // Check if any of selected items have required children
            ...
            QList<CloudComposerItem::ItemType> missing_children =
            ...
            foreach (QStandardItem *item, matching_selected_items) {
                ...
                for (int i = 0; i < item->rowCount(); ++i)
                    found_children_types.append(
                        static_cast<CloudComposerItem::ItemType>(
                            item->child(i)->type()));
                // Make temporary copy, remove type from it if is present as
                // child
                QList<CloudComposerItem::ItemType> req_children_temp =
                    required_children_types;
            ...
            if (!found_valid_items) {
                ...
                foreach (CloudComposerItem::ItemType type, missing_children)
                    missing_children_string.append(
                        " " + ITEM_TYPES_STRINGS.value(
                                  type - QStandardItem::UserType));
                disabled_tools.insert(
                    tool_item, tr("Tool Requires child item of type(s) %1")
    ...
    foreach (QStandardItem *tool, tool_items) {
        if (enabled_tools.contains(tool)) {
            ...
            // qDebug () << tool->text() << " disabled: "<<disabled_tools.value
            // (tool);
