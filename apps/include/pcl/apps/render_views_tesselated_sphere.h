...
namespace pcl {
namespace apps {
/** \brief @b Class to render synthetic views of a 3D mesh using a tesselated
 * sphere NOTE: This class should replace renderViewTesselatedSphere from
 * pcl::visualization. Some extensions are planned in the near future to this
 * class like removal of duplicated views for symmetrical objects, generation of
 * RGB synthetic clouds when RGB available on mesh, etc. \author Aitor Aldoma
 * \ingroup apps
 */
class PCL_EXPORTS RenderViewsTesselatedSphere {
  private:
    ...
    struct camPosConstraintsAllTrue {
        bool operator()(const Eigen::Vector3f & /*pos*/) const { return true; };
    };
  ...
  public:
     ...
     */
    void setGenOrganized(bool b) { gen_organized_ = b; }
    ...
    /* \brief Wether to use the vertices or triangle centers of the tesselated
     * sphere \param use true indicates to use vertices, false triangle centers
     */

    void setUseVertices(bool use) { use_vertices_ = use; }
     ...
     */
    void setRadiusSphere(float radius) { radius_sphere_ = radius; }

    /* \brief Wether to compute the entropies (level of occlusions for each
     * view) \param compute true to compute entropies, false otherwise
     */
    void setComputeEntropies(bool compute) { compute_entropy_ = compute; }

    /* \brief How many times the icosahedron should be tesselated. Results in
     * more or less camera positions and generated views. \param level amount of
     * tesselation
     */
    void setTesselationLevel(int level) { tesselation_level_ = level; }

    /* \brief Sets the view angle of the virtual camera
     * \param angle view angle in degrees
     */
    void setViewAngle(float angle) { view_angle_ = angle; }
     ...
     * \param poses 4x4 matrices representing the pose of the cloud relative to
     ...
     */
    void getEntropies(std::vector<float> &entropies) { entropies = entropies_; }
};
