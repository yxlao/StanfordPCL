...
void PointCloud2Vector3d(pcl::PointCloud<Point>::Ptr cloud,
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        printf("\nUsage: pcl_example_nurbs_fitting_surface "
               "pcd<PointXYZ>-file\n\n");
        exit(0);
    }
    ...
    pcl::visualization::PointCloudColorHandlerCustom<Point> handler(cloud, 0,
                                                                    255, 0);
    ...
    for (unsigned i = 0; i < iterations; i++) {
        ...
        viewer.updatePolygonMesh<pcl::PointXYZ>(mesh_cloud, mesh_vertices,
                                                mesh_id);
    }
    ...
    ON_NurbsCurve curve_nurbs = pcl::on_nurbs::FittingCurve2d::initNurbsCurve2D(
        order, curve_data.interior);
    ...
    //  (curve_fit.m_nurbs, fit.m_nurbs, curve_cloud, 8); viewer.addPointCloud
    //  (curve_cloud); for (std::size_t i = 0; i < curve_cloud->size () - 1;
    //  i++)
    //  {
