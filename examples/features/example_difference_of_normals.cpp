...
int main(int argc, char *argv[]) {
    ...
    /// segment scene into clusters with given distance tolerance using
    ...
    if (approx) {
        ...
        small_cloud_downsampled =
            PointCloud<PointT>::Ptr(new pcl::PointCloud<PointT>);
        ...
        cout << "Using leaf size of " << smalldownsample << " for small scale, "
             << small_cloud_downsampled->size() << " points" << endl;

        // Create downsampled point cloud for DoN NN search with large scale
        large_cloud_downsampled =
            PointCloud<PointT>::Ptr(new pcl::PointCloud<PointT>);
        ...
        cout << "Using leaf size of " << largedownsample << " for large scale, "
             << large_cloud_downsampled->size() << " points" << endl;
    }
    ...
    pcl::PointCloud<PointNT>::Ptr normals_small_scale(
        new pcl::PointCloud<PointNT>);
    ...
    pcl::PointCloud<PointNT>::Ptr normals_large_scale(
        new pcl::PointCloud<PointNT>);
    ...
    if (!don.initCompute()) {
        std::cerr << "Error: Could not intialize DoN feature operator"
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    // build the condition
    pcl::ConditionOr<PointOutT>::Ptr range_cond(
        new pcl::ConditionOr<PointOutT>());
    range_cond->addComparison(pcl::FieldComparison<PointOutT>::ConstPtr(
        new pcl::FieldComparison<PointOutT>("curvature", pcl::ComparisonOps::GT,
                                            threshold)));
    // build the filter
    pcl::ConditionalRemoval<PointOutT> condrem(range_cond);
    condrem.setInputCloud(doncloud);

    pcl::PointCloud<PointOutT>::Ptr doncloud_filtered(
    ...
    // Filter by magnitude
    cout << "Clustering using EuclideanClusterExtraction with tolerance <= "
         << segradius << "..." << endl;

    pcl::search::KdTree<PointOutT>::Ptr segtree(
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it =
        ...
        pcl::PointCloud<PointOutT>::Ptr cloud_cluster_don(
            new pcl::PointCloud<PointOutT>);
        ...
        std::cout << "PointCloud representing the Cluster: "
                  << cloud_cluster_don->points.size() << " data points."
                  << std::endl;
        std::stringstream ss;
        ss << outfile.substr(0, outfile.length() - 4) << "_threshold_"
           << threshold << "_cluster_" << j << ".pcd";
