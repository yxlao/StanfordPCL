 ...
 * \note The idea is to offer a simple visualizer that stores and display the
 * last X values as a curve. \note The class is based on PCLHistogramVisualizer
 * and pcl::VFHSignature308 for display. \note Therefore, the number of values
 * is limited to [2-308]. \author Raphael Favier \ingroup visualization
 */
class PCL_EXPORTS PCLSimpleBufferVisualizer {
  public:
    ...
    PCLSimpleBufferVisualizer(const int nb_values) {
        ...
        if (nb_values_ > 308) {
            PCL_WARN("Maximum number of values can only be 308 (%d given). "
                     "Setting back to 308. \n");
            nb_values_ = 308;
        }

        if (nb_values_ <= 1) {
            PCL_WARN("Number of values must be at least 2 (%d given). Setting "
                     "it to default (308). \n");
            nb_values_ = 308;
        }
     ...
     * \param[in] time - How long (in ms) should the visualization loop be
     * allowed to run
     */
     ...
     * allowed to run
     */
    void spinOnce(const int time = 1) { histo_->spinOnce(time); }

    /** \brief spin method. */
    void spin() { histo_->spin(); }

    /** \brief set background color handling mode.
     * \note The point here is to change the background to orange when the
     * latest value is under a threshold. \param[in] value if true, automatic
     * mode is enabled. Else, background will be white \param[in] threshold
     * value that triggers the background to turn orange if the latest value is
     * lower \note This functionality does not work yet at time of commit (see
     * http://dev.pointclouds.org/issues/829)
     */
    void setAutomaticBackgroundColorControl(const bool value = true,
                                            const float threshold = 0.0f) {
        control_background_color_ = value;

        // if the user sets it back to false, we make sure to reset the bckgrd
        // color to white
        if (value == false)
            histo_->setBackgroundColor(255.0, 255.0, 255.0);
     ...
     * \note If set to false, the Y scale is automatically adjusted to the
     * current values (default). \param[in] value boolean that enable or disable
     * this policy
     */
  ...
  private:
     ...
     * \note The size is set by private member nb_values_ which is in the range
     * [2-308].
     */
    ...
    /** \brief pushes the values contained inside the buffer to the cloud used
     * for visualization. */
    ...
    /** \brief boolean used to know if we need to change the backgroud color in
     * case of low values. */
    ...
    /** \brief boolean used to know if we need to change the backgroud color in
     * case of low values. True means we do it ourselves. */
