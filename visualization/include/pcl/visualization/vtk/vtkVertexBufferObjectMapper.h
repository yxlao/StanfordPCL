...
// .SECTION Description
// vtkVertexBufferObjectMapper is a class that maps polygonal data (i.e.,
// vtkPolyData) to graphics primitives. vtkVertexBufferObjectMapper serves as a
// superclass for device-specific poly data mappers, that actually do the
// mapping to the rendering/graphics hardware/software.
...
class PCL_EXPORTS vtkVertexBufferObjectMapper : public vtkMapper {
  public:
    ...
    // componentno indicates which component from the data array must be passed
    // as the attribute. If -1, then all components are passed.
    ...
    //  virtual void RemoveVertexAttributeMapping(const char*
    //  vertexAttributeName);
    //
    //  // Description:
  ...
  protected:
    ...
    // to be updated depending on whether this->Static is set or not. This
    // method simply obtains the bounds from the data-object and returns it.
  ...
  private:
    vtkVertexBufferObjectMapper(
        const vtkVertexBufferObjectMapper &);            // Not implemented.
