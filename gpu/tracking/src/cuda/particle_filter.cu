...
__global__ void ParticleInitializerKernel(ParticleInitializer pi) { pi(); }
...
void pcl::device::computeTracking(
    const DeviceArray2D<PointType> &ref,
    const DeviceArray2D<PixelRGB> &ref_color,
    const DeviceArray2D<PointType> &input,
    const DeviceArray2D<PixelRGB> &input_color, PtrSz<curandState> rng_states,
