...
struct Edges {
    ...
    __device__ __host__ __forceinline__ Edges() : data(0){};
};
...
namespace pcl {
namespace device {
__global__ void fillInvalidEdges(PtrStepSz<Edges> output) {
...
void pcl::device::ConnectedComponents::initEdges(
    int rows, int cols, DeviceArray2D<unsigned char> &edges) {
...
__device__ __forceinline__ float3 computePoint(unsigned short depth, int x,
                                               int y, const Intr &intr) {
...
__global__ void computeEdgesKernel(const PtrStepSz<unsigned char> labels,
                                   const PtrStep<unsigned short> depth,
                                   const Intr intr, const int num_parts,
            ...
            if (d) {
                ...
                if (x > 0)
                    if (labels.ptr(y)[x - 1] == label) {
                        float3 pl =
                            computePoint(depth.ptr(y)[x - 1], x - 1, y, intr);
                ...
                if (y > 0)
                    if (labels.ptr(y - 1)[x] == label) {
                        float3 pu =
                            computePoint(depth.ptr(y - 1)[x], x, y - 1, intr);
...
void pcl::device::ConnectedComponents::computeEdges(
    const Labels &labels, const Depth &depth, int num_parts, float sq_radius,
    ...
    computeEdgesKernel<<<grid, block>>>(labels, depth, intr, num_parts,
                                        sq_radius, edges);
    cudaSafeCall(cudaGetLastError());
    cudaSafeCall(cudaDeviceSynchronize());
}
...
namespace pcl {
namespace device {
__global__ void smemTilesKernel(const PtrStepSz<unsigned char> edges,
                                PtrStep<int> comps) {
...
//            "=r"(reinterpret_cast<int&>(val)) : _ASM_PTR_(ptr));
//          #else
//            val = *ptr;
//          #endif
//        }
...
__device__ __forceinline__ int findRoot(const PtrStepSz<int> &comps,
                                        int label) {
    for (;;) {
...
struct TilesMerge {
    ...
    __device__ __forceinline__ void unionF(int l1, int l2,
                                           bool &changed) const {
...
namespace pcl {
namespace device {
__global__ void flattenTreesKernel(PtrStepSz<int> comps,
                                   const PtrStep<Edges> edges) {
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void pcl::device::ConnectedComponents::labelComponents(
