...
__global__ void colorKernel(const PtrStepSz<unsigned char> labels,
...
__global__ void mixedColorKernel(const PtrStepSz<unsigned char> labels,
...
void pcl::device::colorLMap(const Labels &labels,
                            const DeviceArray<uchar4> &map, Image &rgba) {
...
#define nppSafeCall(expr)                                                      \
    pcl::gpu::___nppSafeCall(expr, __FILE__, __LINE__, __func__)
...
void ___nppSafeCall(int err_code, const char *file, const int line,
                    const char *func = "") {
    if (err_code < 0) {
        char buf[4096];
...
void pcl::device::Dilatation::prepareRect5x5Kernel(
...
void pcl::device::Dilatation::invoke(const Mask &src, const Kernel &kernel,
                                     Mask &dst) {
    ...
    nppSafeCall(nppiDilate_8u_C1R(src.ptr(ANCH_Y) + ANCH_X, (int)src.step(),
                                  dst.ptr(ANCH_Y) + ANCH_X, (int)dst.step(), sz,
                                  kernel, ksz, anchor));
}
...
namespace pcl {
namespace device {
__global__ void fgDepthKernel(const PtrStepSz<unsigned short> depth1,
    ...
    if (x < depth1.cols && y < depth1.rows) {
        ...
        depth2.ptr(y)[x] =
            inv_mask.ptr(y)[x] ? d : numeric_limits<unsigned short>::max();
    }
}
} // namespace device
} // namespace pcl
...
void pcl::device::prepareForeGroundDepth(const Depth &depth1,
                                         Mask &inverse_mask, Depth &depth2) {
...
float pcl::device::computeHue(int rgba) { return computeHueFunc(rgba); }

namespace pcl {
namespace device {
__global__ void computeHueKernel(const PtrStepSz<int> rgba,
...
void pcl::device::computeHueWithNans(const Image &rgba, const Depth &depth,
                                     HueImage &hue) {
...
namespace pcl {
namespace device {
__global__ void reprojectDepthKenrel(const PtrStepSz<unsigned short> depth,
                                     const Intr intr, PtrStep<float4> cloud) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void pcl::device::computeCloud(const Depth &depth, const Intr &intr,
                               Cloud &cloud) {
