...
struct HistogramPair {
  public:
    ...
    inline void accumTrue(const Label label) { m_h_true[label]++; }
    inline void accumFalse(const Label label) { m_h_false[label]++; }
...
/**
 * This is a little weird.. it will just compute the entropy of the merged
 * histograms
 */
...
static void writeLabeledFeatureVec(std::ostream &os,
...
inline void writeInfoFile(const std::string &filename, int attribId,
                          Attrib threshold, double gain,
                          const HistogramPair &HP) {
...
inline void readInfoFile(const std::string &filename, int &attribId,
                         Attrib &threshold, double &gain, HistogramPair &HP) {
    ...
    if (fin.fail())
        throw std::runtime_error(std::string("(E) malformed splitInfo file ") +
                                 filename);
}
