 ...
 */
template <typename _Scalar, int _Dim> class Transform {
  public:
  ...
  public:
     ...
     * sizes. */
    template <typename OtherDerived>
    friend inline const typename ProductReturnType<OtherDerived,
                                                   MatrixType>::Type
...
Transform<Scalar, Dim>::operator=(const QMatrix &other) {
...
template <typename Scalar, int Dim>
Transform<Scalar, Dim> &
Transform<Scalar, Dim>::operator=(const QTransform &other) {
    EIGEN_STATIC_ASSERT(Dim == 2, YOU_MADE_A_PROGRAMMING_MISTAKE)
    m_matrix << other.m11(), other.m21(), other.dx(), other.m12(), other.m22(),
...
Transform<Scalar, Dim>::operator=(const TranslationType &t) {
    linear().setIdentity();
...
template <typename Scalar, int Dim>
inline Transform<Scalar, Dim>
Transform<Scalar, Dim>::operator*(const TranslationType &t) const {
    Transform res = *this;
...
template <typename Scalar, int Dim>
inline Transform<Scalar, Dim> &
Transform<Scalar, Dim>::operator=(const ScalingType &s) {
    m_matrix.setZero();
...
template <typename Scalar, int Dim>
inline Transform<Scalar, Dim>
Transform<Scalar, Dim>::operator*(const ScalingType &s) const {
    Transform res = *this;
    res.scale(s.coeffs());
    return res;
}
