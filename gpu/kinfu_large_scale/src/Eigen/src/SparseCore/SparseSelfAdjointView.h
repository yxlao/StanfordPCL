...
class SparseSelfAdjointView
  ...
  public:
    ...
    operator*(const MatrixBase<OtherDerived> &rhs) const {
        return SparseSelfAdjointTimeDenseProduct<MatrixType, OtherDerived,
                                                 UpLo>(m_matrix, rhs.derived());
    }
    ...
    operator*(const MatrixBase<OtherDerived> &lhs,
        ...
        return DenseTimeSparseSelfAdjointProduct<OtherDerived,
    ...
    void evalTo(DynamicSparseMatrix<DestScalar, ColMajor, Index> &_dest) const {
        ...
        SparseMatrix<DestScalar, ColMajor, Index> tmp(_dest.rows(),
                                                      _dest.cols());
    ...
    twistedBy(const PermutationMatrix<Dynamic, Dynamic, Index> &perm) const {
        return SparseSymmetricPermutationProduct<_MatrixTypeNested, UpLo>(
            m_matrix, perm);
    }

    template <typename SrcMatrixType, int SrcUpLo>
    SparseSelfAdjointView &
    operator=(const SparseSymmetricPermutationProduct<SrcMatrixType, SrcUpLo>
                  &permutedMatrix) {
    ...
    SparseSelfAdjointView &
    operator=(const SparseSelfAdjointView<SrcMatrixType, SrcUpLo> &src) {
        PermutationMatrix<Dynamic> pnull;
        return *this = src.twistedBy(pnull);
    }
...
class SparseSelfAdjointTimeDenseProduct
  ...
  private:
    ...
    operator=(const SparseSelfAdjointTimeDenseProduct &);
};
...
class DenseTimeSparseSelfAdjointProduct
  ...
  private:
    ...
    operator=(const DenseTimeSparseSelfAdjointProduct &);
};
...
void permute_symm_to_symm(const MatrixType &mat,
    ...
    enum {
        ...
        DstUpLo = DstOrder == RowMajor ? (_DstUpLo == Upper ? Lower : Upper)
                                       : _DstUpLo,
        SrcUpLo = SrcOrder == RowMajor ? (_SrcUpLo == Upper ? Lower : Upper)
                                       : _SrcUpLo
    };
    ...
    for (Index j = 0; j < size; ++j) {
        ...
        for (typename MatrixType::InnerIterator it(mat, j); it; ++it) {
            ...
            Index k = count[int(DstUpLo) == int(Lower) ? (std::min)(ip, jp)
                                                       : (std::max)(ip, jp)]++;
            dest.innerIndexPtr()[k] = int(DstUpLo) == int(Lower)
                                          ? (std::max)(ip, jp)
                                          : (std::min)(ip, jp);
