 ...
 */
template <typename _Scalar, typename _Index> class AmbiVector {
  public:
    ...
    AmbiVector(Index size)
        : m_buffer(0), m_zero(0), m_size(0), m_allocatedSize(0),
  ...
  protected:
    ...
    void reallocateSparse() {
        ...
        m_allocatedElements =
            (std::min)(Index(m_allocatedElements * 1.5), m_size);
        Index allocSize = m_allocatedElements * sizeof(ListEl);
        allocSize = allocSize / sizeof(Scalar) +
                    (allocSize % sizeof(Scalar) > 0 ? 1 : 0);
...
class AmbiVector<_Scalar, _Index>::Iterator {
  public:
     ...
     * are skipped.
     */
    Iterator(const AmbiVector &vec, RealScalar epsilon = 0) : m_vector(vec) {
        m_epsilon = epsilon;
        ...
        if (m_isDense) {
            do {
                ++m_cachedIndex;
                     ...
                     internal::abs(m_vector.m_buffer[m_cachedIndex]) <
                         m_epsilon);
