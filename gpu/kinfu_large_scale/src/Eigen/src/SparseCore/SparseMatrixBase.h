...
template <typename Derived> class SparseMatrixBase : public EigenBase<Derived> {
  public:
    ...
    enum {
        ...
        /**< This is a rough measure of how expensive it is to read one
         * coefficient from this expression.
         */
...
#undef EIGEN_CURRENT_STORAGE_BASE_CLASS
  ...
  protected:
    ...
    inline Derived &assign(const OtherDerived &other) {
        ...
        const Index outerSize = (int(OtherDerived::Flags) & RowMajorBit)
                                    ? other.rows()
                                    : other.cols();
        if ((!transpose) && other.isRValue()) {
            ...
            for (Index j = 0; j < outerSize; ++j) {
                derived().startVec(j);
                for (typename OtherDerived::InnerIterator it(other, j); it;
                     ++it) {
                    Scalar v = it.value();
    ...
    inline void assignGeneric(const OtherDerived &other) {
        ...
        for (Index j = 0; j < outerSize; ++j) {
            temp.startVec(j);
            for (typename OtherDerived::InnerIterator it(other.derived(), j);
                 it; ++it) {
                Scalar v = it.value();
    ...
    friend std::ostream &operator<<(std::ostream &s,
        ...
        if (Flags & RowMajorBit) {
            ...
            for (Index row = 0; row < nm.outerSize(); ++row) {
                Index col = 0;
                for (typename NestedCleaned::InnerIterator it(nm.derived(),
                                                              row);
                     it; ++it) {
            ...
            if (m.cols() == 1) {
                Index row = 0;
                for (typename NestedCleaned::InnerIterator it(nm.derived(), 0);
                     it; ++it) {
                ...
                s << static_cast<const SparseMatrixBase<
                    SparseMatrix<Scalar, RowMajorBit>> &>(trans);
            }
        }
        return s;
    }
...
#define EIGEN_SPARSE_CWISE_PRODUCT_RETURN_TYPE                                 \
    ...
    // diagonal * sparse
    template <typename OtherDerived>
    friend const SparseDiagonalProduct<OtherDerived, Derived>
    ...
    operator*(const MatrixBase<OtherDerived> &lhs, const Derived &rhs) {
        return
            typename DenseSparseProductReturnType<OtherDerived, Derived>::Type(
                lhs.derived(), rhs);
    }
    ...
    template <typename OtherDerived>
    bool
    isApprox(const MatrixBase<OtherDerived> &other,
             RealScalar prec = NumTraits<Scalar>::dummy_precision()) const {
