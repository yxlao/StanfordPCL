...
class SparseTriangularView<MatrixType, Mode>::InnerIterator
  ...
  public:
    EIGEN_STRONG_INLINE InnerIterator(const SparseTriangularView &view,
        ...
        if (SkipFirst) {
            while ((*this) && (HasUnitDiag ? this->index() <= outer
                                           : this->index() < outer))
                Base::operator++();
            if (HasUnitDiag)
                m_returnOne = true;
    ...
    EIGEN_STRONG_INLINE operator bool() const {
        if (HasUnitDiag && m_returnOne)
            return true;
        return (SkipFirst ? Base::operator bool()
                          : (Base::operator bool() &&
  ...
  public:
    EIGEN_STRONG_INLINE ReverseInnerIterator(const SparseTriangularView &view,
        ...
        eigen_assert((!HasUnitDiag) &&
                     "ReverseInnerIterator does not support yet triangular "
                     "views with a unit diagonal");
        if (SkipLast)
