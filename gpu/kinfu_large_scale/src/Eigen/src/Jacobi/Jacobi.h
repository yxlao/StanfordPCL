...
bool JacobiRotation<Scalar>::makeJacobi(RealScalar x, Scalar y, RealScalar z) {
    ...
    if (y == Scalar(0)) {
        ...
        RealScalar n =
            RealScalar(1) / internal::sqrt(internal::abs2(t) + RealScalar(1));
        m_s = -sign_t * (internal::conj(y) / internal::abs(y)) *
              internal::abs(t) * n;
        m_c = n;
        return true;
    }
...
apply_rotation_in_the_plane(VectorX &_x, VectorY &_y,
                            const JacobiRotation<OtherScalar> &j) {
        ...
        if (internal::first_aligned(x, size) == alignedStart) {
            ...
            for (Index i = alignedStart; i < peelingEnd;
                ...
                pstoreu(px + PacketSize,
                        padd(pmul(pc, xi1), pcj.pmul(ps, yi1)));
                pstore(py, psub(pcj.pmul(pc, yi), pmul(ps, xi)));
