...
typedef struct cholmod_common_struct {
    ...
    double dbound; /* Smallest absolute value of diagonal entries of D
                    ...
                    * Entries in the range -dbound to 0 are replaced with
                    * -dbound.  No
                    * changes are made to the diagonal if dbound <= 0.  Default:
                    * zero */

    double grow0; /* For a simplicial factorization, L->i and L->x can
                   * grow if necessary.  grow0 is the factor by which
                   * it grows.  For the initial space, L is of size MAX
                   * (1,grow0) times the required space.  If L runs out of
                   * space, the new size of L is MAX(1.2,grow0) times the new
                   * required space.   If you do not plan on modifying the LDL'
                   * factorization in the Modify module, set grow0 to zero (or
                   * set grow2 to 0, see below).  Default: 1.2 */
    ...
    size_t grow2; /* For a simplicial factorization, each column j of L
                   ...
                   * modifying the factorization in the Modify module, set
                   * grow2 to zero.  Default: grow1 = 1.2, grow2 = 5. */

    size_t
        maxrank; /* rank of maximum update/downdate.  Valid values:
                  ...
                  * (Xwork, below) of size nrow-by-maxrank double's is allocated
                  * for the update/downdate. If an update/downdate of rank-k is
                  * requested, with k > maxrank, it is done in steps of maxrank.
                  * Default: 8, which is fastest. Memory usage can be reduced by
                  * setting maxrank to 2 or 4.
                  */
    ...
    int final_resymbol; /* if cholmod_factorize performed a supernodal
                         ...
                         * due to exact numeric cancellation, since doing so
                         * would break the update/downdate rowadd/rowdel
    ...
    int prefer_zomplex; /* X = cholmod_solve (sys, L, B, Common) computes
                         ...
                         * both real, then X is real.  Otherwise, X is returned
                         * as CHOLMOD_COMPLEX if Common->prefer_zomplex is
                         * FALSE, or CHOLMOD_ZOMPLEX if Common->prefer_zomplex
                         * is TRUE.  This parameter is needed because there is
                         * no supernodal zomplex L.  Suppose the caller wants
                         * all complex matrices to be stored in zomplex form
                         * (MATLAB, for example).  A supernodal L is returned in
                         * complex form if A is zomplex.  B can be real, and
                         * thus X = cholmod_solve (L,B) should return X as
                         * zomplex.  This cannot be inferred from the input
                         * arguments L and B.  Default: FALSE, since all data
                         * types are supported in CHOLMOD_COMPLEX form and since
                         * this is the native type of LAPACK and the BLAS.  Note
                         * that the MATLAB/cholmod.c mexFunction sets this
                         * parameter to TRUE, since MATLAB matrices are in
    ...
    int prefer_upper; /* cholmod_analyze and cholmod_factorize work
                       ...
                       * When the matrix is ordered as-is, they work fastest
                       * when a symmetric matrix is in lower triangular form. In
                       * MATLAB, R=chol(A) does the opposite.  This parameter
                       * affects only how cholmod_read returns a symmetric
                       * matrix. If TRUE (the default case), a symmetric matrix
    ...
    int nmethods; /* The number of ordering methods to try.  Default: 0.
                   ...
                   * ordering.  Let anz be the number of nonzeros in the upper
                   * or lower triangular part of the symmetric matrix A.  If
                   * fl/lnz < 500 or lnz/anz < 5, then this is a good ordering,
                   * and METIS is not attempted.  Otherwise, METIS is tried. The
                   * best ordering found is used.  If nmethods > 0, the methods
                   * used are given in the method[ ] array, below.  The first
                   * three methods in the default suite of orderings is (1) use
                   * the given permutation (if provided), (2) use AMD, and (3)
                   * use METIS.  Maximum allowed value is CHOLMOD_MAXMETHODS. */
    ...
    struct cholmod_method_struct {
        ...
        double
            fl; /* flop count for a "pure", real simplicial LL'
                 * factorization, with no extra work due to
                 * amalgamation.  Subtract n to get the LDL' flop count.
                 * Multiply by about 4 if the matrix is complex or zomplex. */

        /* ordering method parameters */
        double
            prune_dense; /* dense row/col control for AMD, SYMAMD, CSYMAMD,
                          ...
                          *
                          * Removing dense rows and columns improves the
                          * run-time of the ordering methods.  It has some
                          * impact on ordering quality (usually minimal,
                          * sometimes good, sometimes bad).
                          *
                          * Default: 10. */

        double
            prune_dense2; /* dense row control for COLAMD and CCOLAMD.
                           ...
                           *
                           * If prune_dense2 < 0, only completely dense
                           * rows/cols are removed.
                           *
                           * Default: -1.  Note that this is not the default for
                           * COLAMD and CCOLAMD.  -1 is best for Cholesky.  10
                           * is best for LU.  */

        double nd_oksep; /* in NESDIS, when a node separator is computed, it
                          * discarded if nsep >= nd_oksep*n, where nsep is
                          * the number of nodes in the separator, and n is the
                          * size of the graph being cut.  Valid range is 0 to 1.
                          * If 1 or greater, the separator is discarded if it
                          * consists of the entire graph. Default: 1 */
        ...
        int order_for_lu; /* CCOLAMD can be optimized to produce an ordering
                           ...
                           * performs a Cholesky factorization.  However, you
                           * may wish to use CHOLMOD as an interface for CCOLAMD
                           * but use it for your own LU factorization.  In this
                           * case, order_for_lu should be set to FALSE. When
                           * factorizing in CHOLMOD itself, you should *** NEVER
                           * *** set this parameter FALSE.  Default: TRUE. */
        ...
        int nd_camd; /* If 1, follow the nested dissection ordering
                      ...
                      * respects the partitioning just found (using CAMD).  If
                      * 2, use CSYMAMD instead.  If you set nd_small very small,
                      * you may not need this ordering, and can save time by
                      * setting it to zero (no constrained minimum degree
                      * ordering).  Default: 1. */

        int nd_components; /* The nested dissection ordering finds a node
                            * separator that splits the graph into two parts,
                            * which may be unconnected.  If nd_components is
                            * TRUE, each of these connected components is split
                            * independently.  If FALSE, each part is split as a
                            * whole, even if it consists of more than one
    ...
    int postorder; /* If TRUE, cholmod_analyze follows the ordering with a
                    ...
                    * supernode amalgamation.  Does not affect fundamental
                    * nnz(L) and flop count.  Default: TRUE. */
    ...
    double
        metis_memory; /* This is a parameter for CHOLMOD's interface to
                       ...
                       * memory.  To mitigate this problem, the CHOLMOD
                       * interface can allocate a single block of memory equal
                       * in size to an empirical upper bound of METIS's memory
                       * usage times the Common->metis_memory parameter, and
                       * then immediately free it.  It then calls METIS.  If
                       * this pre-allocation fails, it is possible that METIS
                       * will fail as well, and so CHOLMOD returns with an
                       * out-of-memory condition without calling METIS.
                       *
                       * METIS_NodeND (used in the CHOLMOD_METIS ordering
                       * option) with its default parameter settings typically
                       * uses about (4*nz+40n+4096) times sizeof(int) memory,
                       * where nz is equal to the number of entries in A for the
                       * symmetric case or AA' if an unsymmetric matrix is being
                       * ordered (where nz includes both the upper and lower
                       * parts of A or AA').  The observed "upper bound" (with 2
                       * exceptions), measured in an instrumented copy of
                       * METIS 4.0.1 on thousands of matrices, is
                       * (10*nz+50*n+4096) * sizeof(int).  Two large matrices
                       * exceeded this bound, one by almost a factor of 2
                       * (Gupta/gupta2).
                       *
    ...
    size_t metis_nswitch; /* fault with one matrix of order n = 3005 and
                           ...
                           * matrices of dimension greater than
                           * Common->metis_nswitch (default 3000) or more and
                           * with density of Common->metis_dswitch (default
                           * 0.66) or more. cholmod_nested_dissection has no
                           * problems with the same matrix, even though it uses
                           * METIS_NodeComputeSeparator on this matrix.  If this
                           * seg fault does not affect you, set metis_nswitch to
                           * zero or less, and CHOLMOD will not switch to AMD
                           * based just on the density of the matrix (it will
                           * still switch to AMD if the metis_memory parameter
                           * causes the switch).
                           */
    ...
    void *Xwork; /* a double array.  Its size varies.  It is nrow for
                  ...
                  * cholmod_aat, cholmod_norm, cholmod_ssmult) for the real
                  * case, twice that when the input matrices are complex or
                  * zomplex.  It is of size 2*nrow for cholmod_rowadd and
                  * cholmod_rowdel.  For cholmod_updown, its size is
                  * maxrank*nrow where maxrank is 2, 4, or 8.  Kept cleared
    ...
    int no_workspace_reallocate; /* this is an internal flag, used as a
                                  * precaution by cholmod_analyze.  It is
                                  * normally false.  If true,
                                  * cholmod_allocate_work is not allowed to
                                  * reallocate any workspace; they must use the
                                  * existing workspace in Common (Iwork, Flag,
                                  * Head, and Xwork).  Added for CHOLMOD v1.1 */
    ...
    int prefer_binary; /* cholmod_read_triplet converts a symmetric
                        ...
                        * prefer_binary is FALSE, the diagonal entries are set
                        * to 1 + the degree of the row/column, and off-diagonal
                        * entries are set to -1 (resulting in a positive
                        * definite matrix if the diagonal is zero-free).  Most
                        * symmetric patterns are the pattern a positive definite
                        * matrix.  If this parameter is TRUE, then the matrix is
                        * returned with a 1 in each entry, instead.  Default:
                        * FALSE.  Added in v1.3. */
...
typedef struct cholmod_sparse_struct {
    ...
    void *nz; /* nz [0..ncol-1], the # of nonzeros in each col.  In
               ...
               * A->i [A->p [j] ... A->p [j+1]-1].  In unpacked form, column j
               * is in A->i [A->p [j] ... A->p [j]+A->nz[j]-1] instead.  In both
               * cases, the numerical values (if present) are in the
               * corresponding locations in the array x (or z if A->xtype is
               * CHOLMOD_ZOMPLEX). */
    ...
    int stype; /* Describes what parts of the matrix are considered:
                ...
                * triangular parts (the matrix may actually be symmetric in
                * pattern and value, but both parts are explicitly stored and
                * used).  May be square or rectangular. >0: matrix is square and
                * symmetric, use upper triangular part. Entries in the lower
                * triangular part are ignored. <0: matrix is square and
                * symmetric, use lower triangular part. Entries in the upper
                * triangular part are ignored.
                *
                * Note that stype>0 and stype<0 are different for cholmod_sparse
                * and cholmod_triplet.  See the cholmod_triplet data structure
                * for more details.
                */
...
typedef struct cholmod_triplet_struct {
    ...
    int stype; /* Describes what parts of the matrix are considered:
                ...
                *pattern and value, but both parts are explicitly stored and
                *used).  May be square or rectangular. >0: matrix is square and
                *symmetric.  Entries in the lower triangular part are transposed
                *and added to the upper triangular part when the matrix is
                *converted to cholmod_sparse form. <0: matrix is square and
                *symmetric.  Entries in the upper triangular part are transposed
                *and added to the lower triangular part when the matrix is
                *converted to cholmod_sparse form.
                *
                * Note that stype>0 and stype<0 are different for cholmod_sparse
                ...
                *is an array of size n. Suppose a symmetric matrix A is
                *represent by a triplet matrix T, with entries only in the upper
                *triangular part.  Then the following code:
                *
                *	Ti = T->i ;
                *	Tj = T->j ;
                ...
                * contains just the upper triangular entries (T->stype = 1),
                *after permutation it has entries in both the upper and lower
                ...
                *
                * Since the triplet matrix T is so simple to generate, it's
                *quite easy to remove entries that you do not want, prior to
                *converting T to the cholmod_sparse form.  So if you include
                *these entries in T, CHOLMOD assumes that there must be a reason
                *(such as the one above).  Thus, no entry in a triplet matrix is
                *ever ignored.
                */
