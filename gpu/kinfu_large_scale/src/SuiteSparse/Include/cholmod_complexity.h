...
#define C_MULT(x, z, p, ax, az, q, bx, bz, r)                                  \
    x[2 * (p)] =                                                               \
        ax[2 * (q)] * bx[2 * (r)] - ax[2 * (q) + 1] * bx[2 * (r) + 1];         \
    x[2 * (p) + 1] =                                                           \
        ax[2 * (q) + 1] * bx[2 * (r)] + ax[2 * (q)] * bx[2 * (r) + 1]
...
#define C_LDLDOT(x, p, ax, az, q, bx, r)                                       \
    x[2 * (p)] -=                                                              \
        (ax[2 * (q)] * ax[2 * (q)] + ax[2 * (q) + 1] * ax[2 * (q) + 1]) /      \
        bx[r]
