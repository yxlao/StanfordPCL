...
template <> EIGEN_DONT_INLINE Packet4f pasin(Packet4f x) {
...

    {
        ...
        Packet4f temp = pmul(s4, z1); // not really a madd but a mul by z so
                                      // that the next term can be a madd
        z1 = pmadd(temp, x1, x1);
        z1 = padd(z1, z1);
...

    {
        ...
        Packet4f temp = pmul(s4, z2); // not really a madd but a mul by z so
                                      // that the next term can be a madd
        z2 = pmadd(temp, x2, x2);
    }
