...
class MatrixFunction {
  public:
    /** \brief Constructor.
     *
     * \param[in]  A       argument of matrix function, should be a square
     * matrix. \param[in]  atomic  class for computing matrix function of atomic
     * blocks.
     *
...
class MatrixFunction<MatrixType, AtomicType, 1> {
  ...
  private:
    ...
    ListOfClusters m_clusters; /**< \brief Partition of eigenvalues into
                                  clusters of ei'vals "close" to each other */
    DynamicIntVectorType
        m_eivalToCluster; /**< \brief m_eivalToCluster[i] = j means i-th ei'val
                             is in j-th cluster */
    DynamicIntVectorType
        m_clusterSize; /**< \brief Number of eigenvalues in each clusters  */
    DynamicIntVectorType
        m_blockStart; /**< \brief Row index at which block corresponding to i-th
                         cluster starts */
...
void MatrixFunction<MatrixType, AtomicType, 1>::partitionEigenvalues() {
    ...
    for (Index i = 0; i < rows; ++i) {
        ...
        for (Index j = i + 1; j < rows; ++j) {
            if (internal::abs(diag(j) - diag(i)) <= separation() &&
...
template <typename MatrixType, typename AtomicType>
Block<MatrixType>
MatrixFunction<MatrixType, AtomicType, 1>::block(MatrixType &A, Index i,
                                                 Index j) {
...
void MatrixFunction<MatrixType, AtomicType, 1>::computeOffDiagonal() {
    for (Index diagIndex = 1; diagIndex < m_clusterSize.rows(); diagIndex++) {
        for (Index blockIndex = 0;
             blockIndex < m_clusterSize.rows() - diagIndex; blockIndex++) {
            ...
            for (Index k = blockIndex + 1; k < blockIndex + diagIndex; k++) {
                C += block(m_fT, blockIndex, k) *
                     block(m_T, k, blockIndex + diagIndex);
                C -= block(m_T, blockIndex, k) *
...
class MatrixFunctionReturnValue
  ...
  public:
    ...
    inline void evalTo(ResultType &result) const {
        ...
        typedef Matrix<ComplexScalar, Dynamic, Dynamic, Options,
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::sin() const {
    eigen_assert(rows() == cols());
    typedef
        typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
