...
template <typename MatrixType> class MatrixLogarithmAtomic {
  public:
     ...
     * \param[in]  A  argument of matrix logarithm, should be upper triangular
     * and atomic \returns  The logarithm of \p A.
     */
            ...
            std::numeric_limits<RealScalar>::digits <= 53 ? 7
                ...
                std::numeric_limits<RealScalar>::digits <= 64
                    ...
                    std::numeric_limits<RealScalar>::digits <= 106
                        ? 10
                        : 11; // double-double or quadruple precision
...
MatrixLogarithmAtomic<MatrixType>::atanh(typename MatrixType::Scalar x) {
    using std::abs;
    using std::sqrt;
...
void MatrixLogarithmAtomic<MatrixType>::compute2x2(const MatrixType &A,
    ...
    if (A(0, 0) == A(1, 1)) {
        ...
        result(0, 1) =
            A(0, 1) *
            (Scalar(2) * atanh(z) + Scalar(0, 2 * M_PI * unwindingNumber)) /
            (A(1, 1) - A(0, 0));
    }
}
...
int MatrixLogarithmAtomic<MatrixType>::getPadeDegree(double normTminusI) {
    const double maxNormForPade[] = {
        1.6206284795015624e-2 /* degree = 3 */, 5.3873532631381171e-2,
...
void MatrixLogarithmAtomic<MatrixType>::computePade3(MatrixType &result,
    ...
    for (int k = 0; k < degree; ++k)
        result += weights[k] * (MatrixType::Identity(T.rows(), T.rows()) +
                                nodes[k] * TminusI)
