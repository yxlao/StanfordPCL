 ...
 */
template <typename MatrixType> class MatrixSquareRootQuasiTriangular {
  public:
    /** \brief Constructor.
     *
...
void MatrixSquareRootQuasiTriangular<MatrixType>::compute2x2diagonalBlock(
    ...
    //       in EigenSolver. If we expose it, we could call it directly from
    //       here.
...
void MatrixSquareRootQuasiTriangular<MatrixType>::compute1x2offDiagonalBlock(
    ...
    Matrix<Scalar, 2, 2> A =
        sqrtT.coeff(i, i) * Matrix<Scalar, 2, 2>::Identity();
...
void MatrixSquareRootQuasiTriangular<MatrixType>::compute2x1offDiagonalBlock(
    ...
    Matrix<Scalar, 2, 2> A =
        sqrtT.coeff(j, j) * Matrix<Scalar, 2, 2>::Identity();
 ...
 */
template <typename MatrixType> class MatrixSquareRootTriangular {
  public:
...
void MatrixSquareRootTriangular<MatrixType>::compute(ResultType &result) {
    ...
    for (Index j = 1; j < m_A.cols(); j++) {
        for (Index i = j - 1; i >= 0; i--) {
            ...
            result.coeffRef(i, j) = (T.coeff(i, j) - tmp) /
                                    (result.coeff(i, i) + result.coeff(j, j));
        }
    }
...
template <typename MatrixType> class MatrixSquareRoot<MatrixType, 0> {
  public:
...
template <typename MatrixType> class MatrixSquareRoot<MatrixType, 1> {
  public:
    MatrixSquareRoot(const MatrixType &A) : m_A(A) {
        eigen_assert(A.rows() == A.cols());
    }
