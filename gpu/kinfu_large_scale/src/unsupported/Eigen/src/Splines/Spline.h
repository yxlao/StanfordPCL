...
template <typename _Scalar, int _Dim, int _Degree> class Spline {
  public:
     ...
     *evaluated. \param order The order up to which the derivatives are
     *computed.
     **/
    ...
    /**
     * \brief Computes the non-zero spline basis function derivatives up to
     *given order.
     *
    ...
    basisFunctionDerivatives(Scalar u,
...
Spline<_Scalar, _Dim, _Degree>::BasisFunctions(
    ...
    typedef typename Spline<_Scalar, _Dim, _Degree>::BasisVectorType
        BasisVectorType;
    ...
    VectorBlock<BasisVectorType, Degree>(left, 1, p) =
        u -
        VectorBlock<const KnotVectorType, Degree>(U, i + 1 - p, p).reverse();
