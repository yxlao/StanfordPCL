...
class LevenbergMarquardt {
  public:
    ...
    LevenbergMarquardtSpace::Status
    lmstr1(FVectorType &x,
           const Scalar tol = internal::sqrt(NumTraits<Scalar>::epsilon()));

    LevenbergMarquardtSpace::Status minimizeOptimumStorage(FVectorType &x);
    LevenbergMarquardtSpace::Status minimizeOptimumStorageInit(FVectorType &x);
    LevenbergMarquardtSpace::Status
    minimizeOptimumStorageOneStep(FVectorType &x);
...
LevenbergMarquardt<FunctorType, Scalar>::minimizeInit(FVectorType &x) {
    ...
    assert((!useExternalScaling || diag.size() == n) ||
           "When useExternalScaling is set, the caller must provide a valid "
           "'diag'");
    qtf.resize(n);
    ...
    if (n <= 0 || m < n || parameters.ftol < 0. || parameters.xtol < 0. ||
        parameters.gtol < 0. || parameters.maxfev <= 0 ||
        parameters.factor <= 0.)
        return LevenbergMarquardtSpace::ImproperInputParameters;
...
LevenbergMarquardt<FunctorType, Scalar>::minimizeOneStep(FVectorType &x) {
    ...
    if (fnorm != 0.)
        for (Index j = 0; j < n; ++j)
            if (wa2[permutation.indices()[j]] != 0.)
                gnorm = (std::max)(
                    gnorm, internal::abs(fjac.col(j).head(j + 1).dot(
                                             qtf.head(j + 1) / fnorm) /
    ...
    do {
        ...
        if (internal::abs(actred) <= parameters.ftol &&
            prered <= parameters.ftol && Scalar(.5) * ratio <= 1. &&
        ...
        if (internal::abs(actred) <= NumTraits<Scalar>::epsilon() &&
...
LevenbergMarquardt<FunctorType, Scalar>::minimizeOptimumStorageOneStep(
    ...
    do {
        ...
        /* determine the levenberg-marquardt parameter. */
        internal::lmpar<Scalar>(fjac, permutation.indices(), diag, qtf, delta,
                                par, wa1);
