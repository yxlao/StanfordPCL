...
template <typename _Scalar, int _Dim, typename _Object> class KdBVH {
  public:
    ...
    void init(OIter begin, OIter end, BIter boxBegin, BIter boxEnd) {
        ...
        build(objCenters, 0, n, objBoxes,
              0); // the recursive part of the algorithm
     ...
     * and \a outOBegin and \a outOEnd range over the object children of the
     * node */
    ...
    getChildren(Index index, VolumeIterator &outVBegin, VolumeIterator &outVEnd,
        ...
        if (index < 0) {
            ...
            if (!objects.empty())
                outOBegin = &(objects[0]);
            outOEnd =
                outOBegin + objects.size(); // output all objects--necessary
