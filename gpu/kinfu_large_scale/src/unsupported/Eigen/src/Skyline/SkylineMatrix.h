...
class SkylineMatrix
  ...
  protected:
    typedef SkylineMatrix<Scalar, (Flags & ~RowMajorBit) |
        ...
        if (IsRowMajor) {
            ...
            if (outer > inner) // upper matrix
            {
                ...
                if (outer <= maxOuterIndex)
                    return this->m_data.upper(m_colStartIndex[inner] +
                                              (outer - inner));
                else
                    return Scalar(0);
            }
            if (outer < inner) // lower matrix
            {
                ...
                if (inner <= maxInnerIndex)
                    return this->m_data.lower(m_rowStartIndex[outer] +
                                              (inner - outer));
                else
                    return Scalar(0);
            }
        ...
        if (IsRowMajor) {
            if (col > row) // upper matrix
            {
                ...
                eigen_assert(outer >= minOuterIndex &&
                             "you try to acces a coeff that do not exist in "
                             "the storage");
  ...
  public:
        ...
        if (IsRowMajor) {
            if (outer < inner) // upper matrix
            {
                ...
                if (outer < minOuterIndex) // The value does not yet exist
                {
                    ...
                    for (Index innerIdx = stop; innerIdx >= start; innerIdx--) {
                        m_data.upper(innerIdx + bandIncrement) =
                            m_data.upper(innerIdx);
                    }

                    for (Index innerIdx = cols(); innerIdx > inner;
                         innerIdx--) {
            ...
            if (outer > inner) // lower matrix
            {
                ...
                if (inner < minInnerIndex) // The value does not yet exist
                {
                    ...
                    for (Index innerIdx = stop; innerIdx >= start; innerIdx--) {
                        m_data.lower(innerIdx + bandIncrement) =
                            m_data.lower(innerIdx);
                    }

                    for (Index innerIdx = rows(); innerIdx > outer;
                         innerIdx--) {
            ...
            if (outer > inner) // upper matrix
            {
                ...
                if (outer > maxOuterIndex) // The value does not yet exist
                {
                    ...
                    memset(this->_upperPtr() + m_rowStartIndex[inner] +
                               previousProfile + 1,
            ...
            if (outer < inner) // lower matrix
            {
                ...
                if (inner > maxInnerIndex) // The value does not yet exist
                {
                    ...
                    memset(this->_lowerPtr() + m_colStartIndex[outer] +
                               previousProfile + 1,
                           0, (bandIncrement - 1) * sizeof(Scalar));
    ...
    inline void finalize() {
        if (IsRowMajor) {
            if (rows() > cols())
                m_data.resize(cols(), cols(), rows(),
                              m_colStartIndex[cols()] + 1,
                              m_rowStartIndex[rows()] + 1);
            else
                m_data.resize(rows(), cols(), rows(),
                              m_colStartIndex[cols()] + 1,
                              m_rowStartIndex[rows()] + 1);

            //            eigen_assert(rows() == cols() && "memory
            //            reorganisatrion only works with suare matrix");
            //
            //            Scalar* newArray = new Scalar[m_colStartIndex[cols()]
            //            + 1 + m_rowStartIndex[rows()] + 1]; Index dataIdx = 0;
            //            for (Index row = 0; row < rows(); row++) {
            //
            //                const Index nbLowerElts = m_rowStartIndex[row + 1]
            //                - m_rowStartIndex[row];
            //                //                std::cout << "nbLowerElts" <<
            //                nbLowerElts << std::endl; memcpy(newArray +
            //                dataIdx, m_data.m_lower + m_rowStartIndex[row],
            //                nbLowerElts * sizeof (Scalar));
            ...
            //                * sizeof (Scalar)); m_colStartIndex[row] =
            //                dataIdx; dataIdx += nbUpperElts;
            //
            //
            //            }
            ...
            //            m_rowStartIndex[rows()-1] +
            //            m_data.lowerProfile(rows()-1); m_colStartIndex[cols()]
            //            = m_colStartIndex[cols()-1] +
            ...
            if (rows() > cols())
                m_data.resize(cols(), rows(), cols(),
                              m_rowStartIndex[cols()] + 1,
                              m_colStartIndex[cols()] + 1);
            else
                m_data.resize(rows(), rows(), cols(),
                              m_rowStartIndex[rows()] + 1,
    ...
    inline SkylineMatrix(size_t rows, size_t cols)
        : m_outerSize(0), m_innerSize(0), m_colStartIndex(0),
          m_rowStartIndex(0) {
        resize(rows, cols);
    }
