...
template <typename MatrixType> class SkylineInplaceLU {
  ...
  public:
    /** Creates a LU object and compute the respective factorization of \a
     * matrix using flags \a flags. */
    SkylineInplaceLU(MatrixType &matrix, int flags = 0)
        : /*m_matrix(matrix.rows(), matrix.cols()),*/ m_flags(flags),
          m_status(0), m_lu(matrix) {
     ...
     * imcomplete factorization with fewer non zero coefficients. Such
     * approximate factors are especially useful to initialize an iterative
     * solver.
     *
...
void SkylineInplaceLU<MatrixType>::compute() {
    ...
    for (Index row = 0; row < rows; row++) {
        ...
        const Index &col = row;
        for (typename MatrixType::InnerLowerIterator lIt(m_lu, col); lIt;
             ++lIt) {
            lIt.valueRef() /= pivot;
        }
        ...
        for (Index rrow = row + 1; rrow < m_lu.rows(); rrow++) {
            ...
            for (Index i = 0; i < rrow - row - 1; i++) {
                m_lu.coeffRefLower(rrow, row + i + 1) -=
                    uItPivot.value() * coef;
                ++uItPivot;
            }
            ++lIt3;
...
#ifdef VECTORIZE
                Map<VectorXd> rowVal(
                    lIt.valuePtr() + (offset > 0 ? 0 : -offset), stop);
                Map<VectorXd> colVal(uIt.valuePtr() + (offset > 0 ? offset : 0),
                                     stop);
...
#ifdef VECTORIZE
            Map<VectorXd> rowVal(lIt.valuePtr() + (offset > 0 ? 0 : -offset),
                                 stop);
            Map<VectorXd> colVal(uIt.valuePtr() + (offset > 0 ? offset : 0),
                                 stop);
