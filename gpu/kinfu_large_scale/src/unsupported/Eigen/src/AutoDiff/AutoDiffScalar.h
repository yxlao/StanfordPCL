...
template <typename A, typename B> void make_coherent(const A &a, const B &b) {
    make_coherent_impl<A, B>::run(a.const_cast_derived(),
 ...
 *  - internal::abs, internal::sqrt, internal::pow, internal::exp,
 * internal::log, internal::sin, internal::cos,
...
class AutoDiffScalar
  ...
  public:
    ...
    template <typename OtherDerType>
    inline const AutoDiffScalar<CwiseUnaryOp<
...
struct make_coherent_impl<
    ...
    static void run(A &a, B &b) {
        if ((A_Rows == Dynamic || A_Cols == Dynamic) && (a.size() == 0)) {
            a.resize(b.size());
            a.setZero();
        } else if ((B_Rows == Dynamic || B_Cols == Dynamic) &&
                   (b.size() == 0)) {
            b.resize(a.size());
            b.setZero();
        }
...
EIGEN_AUTODIFF_DECLARE_GLOBAL_UNARY(
    ...
    return ReturnType(tan(x.value()),
                      x.derivatives() *
                          (Scalar(1) / internal::abs2(cos(x.value()))));)

EIGEN_AUTODIFF_DECLARE_GLOBAL_UNARY(
    asin, using std::sqrt; using std::asin;
    return ReturnType(asin(x.value()),
                      x.derivatives() *
                          (Scalar(1) / sqrt(1 - internal::abs2(x.value()))));)
