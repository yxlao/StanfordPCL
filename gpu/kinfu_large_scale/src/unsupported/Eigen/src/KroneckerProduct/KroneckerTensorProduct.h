...
void kroneckerProduct_full(const Derived_A &A, const Derived_B &B,
                           Derived_AB &AB) {
    const unsigned int Ar = A.rows(), Ac = A.cols(), Br = B.rows(),
                       Bc = B.cols();
...
/*!
 * Kronecker tensor product helper function for matrices, where at least one is
 * sparse
 *
 * \param A   Matrix A
 * \param B   Matrix B
 ...
 */
template <typename Derived_A, typename Derived_B, typename Derived_AB>
void kroneckerProduct_sparse(const Derived_A &A, const Derived_B &B,
                             Derived_AB &AB) {
    const unsigned int Ar = A.rows(), Ac = A.cols(), Br = B.rows(),
                       Bc = B.cols();
            ...
            for (typename Derived_A::InnerIterator itA(A, kA); itA; ++itA) {
                for (typename Derived_B::InnerIterator itB(B, kB); itB; ++itB) {
                    const unsigned int iA = itA.row(), jA = itA.col(),
                                       iB = itB.row(), jB = itB.col(),
                                       i = iA * Br + iB, j = jA * Bc + jB;
 ...
 */
template <typename A, typename B, typename CScalar, int CRows, int CCols,
          int COptions, int CMaxRows, int CMaxCols>
void kroneckerProduct(
    const MatrixBase<A> &a, const MatrixBase<B> &b,
...
void kroneckerProduct(const MatrixBase<A> &a, const MatrixBase<B> &b,
 ...
 */
template <typename A, typename B, typename C>
void kroneckerProduct(const MatrixBase<A> &a, const SparseMatrixBase<B> &b,
 ...
 */
template <typename A, typename B, typename C>
void kroneckerProduct(const SparseMatrixBase<A> &a, const MatrixBase<B> &b,
                      SparseMatrixBase<C> &c) {
    internal::kroneckerProduct_sparse(a.derived(), b.derived(), c.derived());
}
 ...
 */
template <typename A, typename B, typename C>
void kroneckerProduct(const SparseMatrixBase<A> &a,
