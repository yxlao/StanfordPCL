...
bool test_random_setter(DynamicSparseMatrix<T> &sm, const DenseType &ref,
    ...
    while (!remaining.empty()) {
        int i =
            internal::random<int>(0, static_cast<int>(remaining.size()) - 1);
...
void test_sparse_extra() {
    for (int i = 0; i < g_repeat; i++) {
        ...
        //    )); CALL_SUBTEST_3((
        //    sparse_basic(DynamicSparseMatrix<double,ColMajor,long int>(s, s))
        //    ));

        CALL_SUBTEST_3(
            (sparse_product<DynamicSparseMatrix<float, ColMajor>>()));
        CALL_SUBTEST_3(
            (sparse_product<DynamicSparseMatrix<float, RowMajor>>()));
    }
}
