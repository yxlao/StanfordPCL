...
int main(int argc, char **argv) {
    ...
    std::cout << "INFO: Saved " << cloud.points.size()
    ...
    for (size_t i = 0; i < sizes.size(); ++i) {
        ...
        if (sizes[i] != 0) {
            ...
            for (size_t j = 0; j < sizes[i]; ++j) {
                cloud_result.points.push_back(
                    cloud.points[data[j + i * max_answers]]);
                std::cout << "INFO: data : " << j << " " << j + i * max_answers
                          << " data " << data[j + i * max_answers] << std::endl;
            }
            std::stringstream ss;
            ss << "cloud_cluster_" << i << ".pcd";
            cloud_result.width = cloud_result.points.size();
            pcl::io::savePCDFileASCII(ss.str(), cloud_result);
            std::cout << "INFO: Saved " << cloud_result.points.size()
                      << " data points to " << ss.str() << std::endl;
        }
    }
    return 0;
}
