...
pcl::PPFRGBEstimation<PointInT, PointNT, PointOutT>::PPFRGBEstimation()
    ...
    // Slight hack in order to pass the check for the presence of a search
    // method in Feature::initCompute ()
    Feature<PointInT, PointOutT>::tree_.reset(
...
//////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename PointNT, typename PointOutT>
void pcl::PPFRGBEstimation<PointInT, PointNT, PointOutT>::computeFeature(
    PointCloudOut &output) {
    // Initialize output container - overwrite the sizes done by
    // Feature::initCompute ()
        ...
        for (size_t j = 0; j < input_->points.size(); ++j) {
            ...
            if (i != j) {
                if (pcl::computeRGBPairFeatures(
                        ...
                        input_->points[j].getRGBVector4i(), p.f1, p.f2, p.f3,
                        p.f4, p.r_ratio, p.g_ratio, p.b_ratio)) {
                    // Calculate alpha_m angle
                    Eigen::Vector3f
                        model_reference_point =
                    ...
                    Eigen::AngleAxisf rotation_mg(
                        acosf(model_reference_normal.dot(
                            Eigen::Vector3f::UnitX())),
                        model_reference_normal.cross(Eigen::Vector3f::UnitX())
                            .normalized());
                    Eigen::Affine3f transform_mg =
                        Eigen::Translation3f(rotation_mg *
                                             ((-1) * model_reference_point)) *
                        rotation_mg;

                    Eigen::Vector3f model_point_transformed =
                        transform_mg * model_point;
                    float angle = atan2f(-model_point_transformed(2),
                                         model_point_transformed(1));
                    if (sin(angle) * model_point_transformed(2) < 0.0f)
                        angle *= (-1);
                    p.alpha_m = -angle;
                } else {
                    PCL_ERROR(
                        "[pcl::%s::computeFeature] Computing pair feature "
                        "vector between points %zu and %zu went wrong.\n",
                        getClassName().c_str(), i, j);
                    p.f1 = p.f2 = p.f3 = p.f4 = p.alpha_m = p.r_ratio =
                        p.g_ratio = p.b_ratio = 0.f;
                }
            }
            // Do not calculate the feature for identity pairs (i, i) as they
            // are not used in the following computations
            else
                p.f1 = p.f2 = p.f3 = p.f4 = p.alpha_m = p.r_ratio = p.g_ratio =
                    p.b_ratio = 0.f;
...
//////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename PointNT, typename PointOutT>
pcl::PPFRGBRegionEstimation<PointInT, PointNT,
                            PointOutT>::PPFRGBRegionEstimation()
    : FeatureFromNormals<PointInT, PointNT, PointOutT>() {
    feature_name_ = "PPFRGBEstimation";
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename PointNT, typename PointOutT>
void pcl::PPFRGBRegionEstimation<PointInT, PointNT, PointOutT>::computeFeature(
    PointCloudOut &output) {
    PCL_INFO("before computing output size: %u\n", output.size());
    output.resize(indices_->size());
    for (int index_i = 0; index_i < static_cast<int>(indices_->size());
         ++index_i) {
        ...
        for (std::vector<int>::iterator nn_it = nn_indices.begin();
            ...
            if (i != j) {
                ...
                if (pcl::computeRGBPairFeatures(
                        ...
                        input_->points[j].getVector4fMap(),
                        normals_->points[j].getNormalVector4fMap(),
                        input_->points[j].getRGBVector4i(), f1, f2, f3, f4,
