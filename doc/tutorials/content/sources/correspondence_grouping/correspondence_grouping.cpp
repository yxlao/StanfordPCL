...
void showHelp(char *filename) {
    ...
    std::cout << "     -k:                     Show used keypoints."
              << std::endl;
...
void parseCommandLine(int argc, char *argv[]) {
    ...
    if (pcl::console::parse_argument(argc, argv, "--algorithm",
                                     used_algorithm) != -1) {
...
int main(int argc, char *argv[]) {
        ...
        if (found_neighs == 1 &&
            neigh_sqr_dists[0] <
                0.25f) //  add match only if the squared descriptor distance is
                       //  less than 0.25 (SHOT descriptor distances are between
                       //  0 and 1 by design)
        {
    ...
    if (show_correspondences_ || show_keypoints_) {
        //  We are translating the model so that it doesn't end in the middle of
    ...
    for (size_t i = 0; i < rototranslations.size(); ++i) {
        ...
        if (show_correspondences_) {
            for (size_t j = 0; j < clustered_corrs[i].size(); ++j) {
                ...
                PointType &model_point = off_scene_model_keypoints->at(
                    clustered_corrs[i][j].index_query);
                PointType &scene_point =
                    scene_keypoints->at(clustered_corrs[i][j].index_match);

                //  We are drawing a line for each pair of clustered
                //  correspondences found between the model and the scene
                viewer.addLine<PointType, PointType>(model_point, scene_point,
                                                     0, 255, 0, ss_line.str());
            }
        }
    }
