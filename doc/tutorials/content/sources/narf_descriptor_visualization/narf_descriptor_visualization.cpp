...
void printUsage(const char *progName) {
    std::cout
        ...
        << "-o <0/1>     switch rotational invariant version of the feature "
           "on/off"
...
int main(int argc, char **argv) {
    ...
    for (unsigned int y = 0; y < range_image.height; ++y) {
        for (unsigned int x = 0; x < range_image.width; ++x) {
            ...
            pcl::Narf::extractFromRangeImageAndAddToList(
                range_image, x, y, descriptor_size, support_size,
                rotation_invariant != 0, narfs[index]);
            // std::cout << "Extracted "<<narfs[index].size ()<<" features for
            // pixel "<<x<<","<<y<<".\n";
        }
    }
    std::cout << "100%\n";
    std::cout << "Done.\n\n Now you can click on points in the image to "
                 "visualize how the descriptor is "
              << "extracted and see the descriptor distances to every other "
                 "point..\n";
    ...
    while (true) {
        ...
        // range_image.real2DToInt2D (clicked_pixel_x_f, clicked_pixel_y_f,
        // clicked_pixel_x, clicked_pixel_y);
        if (!range_image.isValid(clicked_pixel_x, clicked_pixel_y))
            continue;
        // Vector3f clicked_3d_point;
        // range_image.getPoint (clicked_pixel_x, clicked_pixel_y,
        // clicked_3d_point);
        ...
        int selected_index =
            clicked_pixel_y * range_image.width + clicked_pixel_x;
        pcl::Narf narf;
        if (!narf.extractFromRangeImage(range_image, clicked_pixel_x,
                                        clicked_pixel_y, descriptor_size,
                                        support_size)) {
            continue;
        }
        ...
        surface_patch_widget.showFloatImage(
            narf.getSurfacePatch(), surface_patch_pixel_size,
            surface_patch_pixel_size, -0.5f * surface_patch_world_size,
        ...
        float cell_size =
                  surface_patch_world_size / float(surface_patch_pixel_size),
        ...
        for (int descriptor_value_idx = 0;
             descriptor_value_idx < narf.getDescriptorSize();
             ++descriptor_value_idx) {
            float angle =
                descriptor_value_idx * angle_step_size + surface_patch_rotation;
        ...
        for (unsigned int i = 0; i < rotations.size(); ++i) {
            // surface_patch_widget.markLine (radius-0.5, radius-0.5,
            // radius-0.5f + 2.0f*radius*sinf (rotations[i]), radius-0.5f
            // - 2.0f*radius*cosf (rotations[i]), pcl::visualization::Vector3ub
            // (255,0,0));
        }

        descriptor_widget.showFloatImage(narf.getDescriptor(),
                                         narf.getDescriptorSize(), 1, -0.1f,
                                         0.3f, true);

        //===================================
        //=====Compare with all features=====
        //===================================
        const std::vector<pcl::Narf *> &narfs_of_selected_point =
            narfs[selected_index];
        ...
        descriptor_distances_widget.showFloatImage(
            descriptor_distance_image, range_image.width, range_image.height,
