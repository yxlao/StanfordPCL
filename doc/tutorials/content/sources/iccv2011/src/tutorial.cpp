...
template <typename FeatureType> class ICCVTutorial {
  public:
    ICCVTutorial(
        boost::shared_ptr<pcl::Keypoint<pcl::PointXYZRGB, pcl::PointXYZI>>
            keypoint_detector,
        typename pcl::Feature<pcl::PointXYZRGB, FeatureType>::Ptr
            feature_extractor,
  ...
  protected:
     ...
     * @param segmented the resulting segmented point cloud containing only
     ...
     * (press, release) etc.
     * @param cookie user defined data passed during registration of the
     * callback
     */
...
ICCVTutorial<FeatureType>::ICCVTutorial(
    ...
    : source_keypoints_(new pcl::PointCloud<pcl::PointXYZI>()),
      ...
      surface_reconstructor_(surface_reconstructor), source_(source),
      target_(target), source_segmented_(new pcl::PointCloud<pcl::PointXYZRGB>),
    ...
    visualizer_.registerKeyboardCallback(&ICCVTutorial::keyboard_callback,
                                         *this, 0);
...
template <typename FeatureType>
void ICCVTutorial<FeatureType>::segmentation(
    typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr source,
    typename pcl::PointCloud<pcl::PointXYZRGB>::Ptr segmented) const {
...
template <typename FeatureType>
void ICCVTutorial<FeatureType>::detectKeypoints(
    typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr input,
    pcl::PointCloud<pcl::PointXYZI>::Ptr keypoints) const {
...
template <typename FeatureType>
void ICCVTutorial<FeatureType>::extractDescriptors(
    typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr input,
    typename pcl::PointCloud<pcl::PointXYZI>::Ptr keypoints,
    typename pcl::PointCloud<FeatureType>::Ptr features) {
    ...
    typename pcl::FeatureFromNormals<pcl::PointXYZRGB, pcl::Normal,
                                     FeatureType>::Ptr feature_from_normals =
        boost::dynamic_pointer_cast<pcl::FeatureFromNormals<
            pcl::PointXYZRGB, pcl::Normal, FeatureType>>(feature_extractor_);
...
template <typename FeatureType>
void ICCVTutorial<FeatureType>::findCorrespondences(
...
void ICCVTutorial<FeatureType>::reconstructSurface() {
    ...
    *merged += *target_segmented_;
    ...
    pcl::NormalEstimation<pcl::PointXYZRGB, pcl::PointXYZRGBNormal>
        normal_estimation;
    normal_estimation.setSearchMethod(
        pcl::search::Search<pcl::PointXYZRGB>::Ptr(
            new pcl::search::KdTree<pcl::PointXYZRGB>));
...
template <typename FeatureType> void ICCVTutorial<FeatureType>::run() {
    visualizer_.spin();
}

template <typename FeatureType>
void ICCVTutorial<FeatureType>::keyboard_callback(
    ...
    if (event.keyUp()) {
        ...
        case '3':
            if (!visualizer_.removePointCloud("source_segmented")) {
                visualizer_.addPointCloud(source_segmented_,
                                          "source_segmented");
            }
            break;
        ...
        case '4':
            if (!visualizer_.removePointCloud("target_segmented")) {
                visualizer_.addPointCloud(target_segmented_,
                                          "target_segmented");
            }
            break;
        ...
        case '7':
            if (!show_source2target_)
                visualizer_.addCorrespondences<pcl::PointXYZI>(
                    source_keypoints_, target_keypoints_, source2target_,
                    "source2target");
            else
        ...
        case '8':
            if (!show_target2source_)
                visualizer_.addCorrespondences<pcl::PointXYZI>(
                    target_keypoints_, source_keypoints_, target2source_,
                    "target2source");
            else
...
int main(int argc, char **argv) {
    if (argc < 6) {
        pcl::console::print_info(
            "Syntax is: %s <source-pcd-file> <target-pcd-file> "
            "<keypoint-method> <descriptor-type> "
    ...
    pcl::console::print_info(
        "t - show/hide triangulation (surface reconstruction)\n");
    ...
    if (keypoint_type == 1) {
        ...
        pcl::HarrisKeypoint3D<pcl::PointXYZRGB, pcl::PointXYZI> *harris3D =
            new pcl::HarrisKeypoint3D<pcl::PointXYZRGB, pcl::PointXYZI>(
                pcl::HarrisKeypoint3D<pcl::PointXYZRGB,
                                      pcl::PointXYZI>::HARRIS);
        ...
        case 2:
            harris3D->setMethod(pcl::HarrisKeypoint3D<pcl::PointXYZRGB,
                                                      pcl::PointXYZI>::HARRIS);
            break;

        case 3:
            harris3D->setMethod(pcl::HarrisKeypoint3D<pcl::PointXYZRGB,
                                                      pcl::PointXYZI>::TOMASI);
            break;
    ...
    case 1: {
        pcl::Feature<pcl::PointXYZRGB, pcl::FPFHSignature33>::Ptr
            feature_extractor(
        ...
        ICCVTutorial<pcl::FPFHSignature33> tutorial(
            keypoint_detector, feature_extractor, surface_reconstruction,
            source, target);
        tutorial.run();
    } break;

    case 2: {
        pcl::SHOTColorEstimationOMP<pcl::PointXYZRGB, pcl::Normal,
                                    pcl::SHOT1344> *shot =
            new pcl::SHOTColorEstimationOMP<pcl::PointXYZRGB, pcl::Normal,
                                            pcl::SHOT1344>;
        shot->setRadiusSearch(0.04);
        pcl::Feature<pcl::PointXYZRGB, pcl::SHOT1344>::Ptr feature_extractor(
            shot);
    ...
    case 3: {
        pcl::Feature<pcl::PointXYZRGB, pcl::PFHSignature125>::Ptr
            feature_extractor(
                new pcl::PFHEstimation<pcl::PointXYZRGB, pcl::Normal,
                                       pcl::PFHSignature125>);
    ...
    case 4: {
        pcl::Feature<pcl::PointXYZRGB, pcl::PFHRGBSignature250>::Ptr
            feature_extractor(
