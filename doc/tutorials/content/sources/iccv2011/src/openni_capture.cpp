...
OpenNICapture::OpenNICapture(const std::string &device_id)
    : grabber_(device_id), most_recent_frame_(), frame_counter_(0),
      use_trigger_(false), trigger_(false) {
    // Register a callback function to our OpenNI grabber...
    boost::function<void(const PointCloudConstPtr &)> frame_cb =
        boost::bind(&OpenNICapture::onNewFrame, this, _1);
...
const PointCloudPtr OpenNICapture::snap() {
    if (use_trigger_) {
        if (!preview_) {
            ...
            preview_ = boost::shared_ptr<pcl::visualization::PCLVisualizer>(
...
void OpenNICapture::onKeyboardEvent(
...
/** \brief Display a preview window and wait for the user to trigger a frame
 * capture */
    ...
    while (!trigger_) {
        ...
        if (frame_counter_ > last_frame) {
            ...
            if (most_recent_frame_) {
                mutex_.lock();
                if (!preview_->updatePointCloud(most_recent_frame_,
                                                "preview")) {
