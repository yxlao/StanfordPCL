...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        pcl::console::print_info("    -p dist_threshold,max_iters  ..... "
                                 "Subtract the dominant plane\n");
        pcl::console::print_info(
            "    -c tolerance,min_size,max_size ... Cluster points\n");
        pcl::console::print_info("    -s output.pcd .................... Save "
                                 "the largest cluster\n");
        return (1);
    }
    ...
    if (subtract_plane) {
        ...
        pcl::console::print_info(
            "Subtracted %zu points along the detected plane\n",
            n - cloud->size());
    }
    ...
    bool cluster_points =
        pcl::console::parse_3x_arguments(argc, argv, "-c", tolerance, min_size,
                                         max_size) > 0;
    if (cluster_points) {
        clusterObjects(cloud, tolerance, (int)min_size, (int)max_size,
                       cluster_indices);
    ...
    if (save_cloud) {
        // If clustering was performed, save only the first (i.e., largest)
        // cluster
        if (cluster_points) {
        ...
        if (cluster_points) {
            for (size_t i = 0; i < cluster_indices.size(); ++i) {
                ...
                pcl::visualization::PointCloudColorHandlerRandom<pcl::PointXYZ>
                    random_color(cluster_i);

                // Create a unique identifier
                std::stringstream cluster_id("cluster");
                cluster_id << i;

                // Add the i_th cluster to the visualizer with a random color
                // and a unique identifier
                vis.addPointCloud<pcl::PointXYZ>(cluster_i, random_color,
                                                 cluster_id.str());
            }
        } else {
