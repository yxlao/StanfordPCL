...
/* Use a PassThrough filter to remove points with depth values that are too
 * large or too small */
PointCloudPtr thresholdDepth(const PointCloudPtr &input, float min_depth,
                             float max_depth) {
 ...
 * neighbors */
PointCloudPtr removeOutliers(const PointCloudPtr &input, float radius,
                             int min_neighbors) {
 ...
 */
PointCloudPtr applyFilters(const PointCloudPtr &input, float min_depth,
                           float max_depth, float leaf_size, float radius,
                           float min_neighbors) {
