 ...
 * Return: A pointer to the ModelCoefficients (i.e., the 4 coefficients of the
 * plane, represented in c0*x + c1*y + c2*z + c3 = 0 form)
 */
...
/* Use SACSegmentation and an ExtractIndices filter to find the dominant plane
 * and subtract it Inputs: input The input point cloud max_iterations The
 * maximum number of RANSAC iterations to run distance_threshold The
 * inlier/outlier threshold.  Points within this distance from the hypothesized
 * plane are scored as inliers. Return: A pointer to a new point cloud which
 * contains only the non-plane points
 */
 ...
 * Return (by reference): a vector of PointIndices containing the points indices
 * in each cluster
 */
void clusterObjects(const PointCloudPtr &input, float cluster_tolerance,
