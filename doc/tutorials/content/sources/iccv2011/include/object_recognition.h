...
class ObjectRecognition {
  public:
    ...
    void populateDatabase(const std::vector<std::string> &filenames) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (filename.compare(filename.size() - 4, 4, ".pcd") == 0) {
                // If filename ends pcd extension, load the points and process
                // them
                ...
                // If the filename has no extension, load the pre-computed
                // models
                models_[i].points =
                    loadPointCloud<PointT>(filename, "_points.pcd");
                models_[i].keypoints =
                    loadPointCloud<PointT>(filename, "_keypoints.pcd");
                models_[i].local_descriptors = loadPointCloud<LocalDescriptorT>(
                    filename, "_localdesc.pcd");
                models_[i].global_descriptor =
                    loadPointCloud<GlobalDescriptorT>(filename,
                                                      "_globaldesc.pcd");
            }
    ...
    const ObjectModel &recognizeObject(const PointCloudPtr &query_cloud) {
        ...
        const GlobalDescriptorT &query_descriptor =
            query_object.global_descriptor->points[0];
    ...
    /* Construct an object model by filtering, segmenting, and estimating
     * feature descriptors */
  ...
  protected:
    /* Apply a series of filters (threshold depth, downsample, and remove
     * outliers) */
    PointCloudPtr
    applyFiltersAndSegment(const PointCloudPtr &input,
        ...
        cloud = removeOutliers(cloud, params.outlier_rejection_radius,
                               params.outlier_rejection_min_neighbors);

        cloud =
            findAndSubtractPlane(cloud, params.plane_inlier_distance_threshold,
    ...
    void estimateFeatures(const PointCloudPtr &points,
        ...
        normals_out =
            estimateSurfaceNormals(points, params.surface_normal_radius);

        keypoints_out = detectKeypoints(
            points, normals_out, params.keypoints_min_scale,
            params.keypoints_nr_octaves, params.keypoints_nr_scales_per_octave,
    ...
    alignModelPoints(const ObjectModel &source, const ObjectModel &target,
        ...
        tform = computeInitialAlignment(
            source.keypoints, source.local_descriptors, target.keypoints,
