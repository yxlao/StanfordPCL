...
int main(int argc, char **argv) {
    if (argc < 4) {
        std::cout
            << "Specify the input cloud, ground truth and parameter files:\n";
        std::cout << "   " << argv[0]
                  << " input_cloud.pcd ground_truth.txt parameters.txt\n";
        exit(0);
    }
    ...
    // If "" is passed for the target-cloud file, source-cloud will be
    // transformed by ground_truth to get the target cloud
    test_features.setInputClouds(argv[1], "", argv[1]);
    test_features.setThreshold(0.1f, 1.0f, 0.1f);

    // The independent variable need not be set separately, its values will be
    // read from the file std::string parameters = "searchradius=0.05";
