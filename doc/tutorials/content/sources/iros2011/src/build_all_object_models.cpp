...
inline void getModelsInDirectory(bf::path &dir, std::string &rel_path_so_far,
...
int main(int argc, char **argv) {
    if (argc < 3) {
        ...
        pcl::console::print_info("    --filter parameters.txt ............ "
                                 "Threshold, downsample, and denoise\n");
        pcl::console::print_info("    --segment parameters.txt  .......... "
                                 "Remove dominant plane and cluster\n");
        pcl::console::print_info(
            "    --feature parameters.txt ........... Compute normals, "
            "keypoints, and descriptors\n");
        pcl::console::print_info("    --registration parameters.txt ...... "
                                 "Align best fitting model to query\n");
        pcl::console::print_info(
            "  and where the parameters files must contain the following "
            "values (one per line):\n");
        ...
        pcl::console::print_info("Note: The output's base filename must be "
                                 "specified without the .pcd extension\n");
        pcl::console::print_info("      Four output files will be created with "
    ...
    pcl::console::parse_argument(argc, argv, "--filter",
                                 filter_parameters_file) > 0;
    ...
    if (params_stream.is_open()) {
        ...
        pcl::console::print_info(
            "Failed to open the filter parameters file (%s)\n",
    ...
    pcl::console::parse_argument(argc, argv, "--segment",
                                 segmentation_parameters_file) > 0;
    ...
    if (params_stream.is_open()) {
        ...
        pcl::console::print_info(
            "Failed to open the segmentation parameters file (%s)\n",
    ...
    pcl::console::parse_argument(argc, argv, "--feature",
                                 feature_estimation_parameters_file) > 0;
    ...
    pcl::console::parse_argument(argc, argv, "--registration",
                                 registration_parameters_file) > 0;
    ...
    if (params_stream.is_open()) {
        ...
        pcl::console::print_info(
            "Failed to open the registration parameters file (%s)\n",
    ...
    for (size_t i = 0; i < files.size(); i++) {
        ...
        base_filestream << directory << "/" << id << "/" << id
                        << strs[1].substr(0, 1);
