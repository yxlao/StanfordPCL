...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        pcl::console::print_info("    --surfel radius,order............... "
        ...
        pcl::console::print_info(
            "    --greedy radius,max_nn,mu,surf_angle,min_angle,max_angle ... "
            "Compute greedy triangulation\n");
        pcl::console::print_info(
    ...
    bool compute_surface_elements =
        pcl::console::parse_2x_arguments(argc, argv, "--surfel", mls_radius,
                                         polynomial_order) > 0;
    ...
    MeshPtr convex_hull;
    bool compute_convex_hull =
        pcl::console::find_argument(argc, argv, "--convex") > 0;
    ...
    if (perform_greedy_triangulation) {
        ...
        greedy_mesh =
            greedyTriangulation(surfels, radius, max_nearest_neighbors, mu,
    ...
    bool perform_marching_cubes =
        pcl::console::parse_2x_arguments(argc, argv, "--mc", leaf_size,
                                         iso_level) > 0;
    if (perform_marching_cubes) {
        assert(surfels);

        marching_cubes_mesh =
            marchingCubesTriangulation(surfels, leaf_size, iso_level);

        pcl::console::print_info(
            "Performed marching cubes surface triangulation\n");
    }
    ...
    // Or visualize the result
        ...
        pcl::console::print_info(
            "Starting visualizer... Close window to exit.\n");
        pcl::visualization::PCLVisualizer vis;
        vis.addPointCloud(cloud);
        vis.resetCamera();

        if (compute_convex_hull) {
            vis.addPolygonMesh<PointT>(convex_hull->points, convex_hull->faces,
                                       "convex_hull");
        }
        if (compute_concave_hull) {
            vis.addPolygonMesh<PointT>(concave_hull->points,
                                       concave_hull->faces, "concave_hull");
        }
        if (perform_greedy_triangulation) {
