...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        pcl::console::print_info(
            ...
            "surface normals\n");
        pcl::console::print_info(
            "    -k min_scale,nr_octaves,nr_scales,min_contrast... Detect "
            "keypoints\n");
        pcl::console::print_info(
            "    -l radius ....................................... Compute "
            "local descriptors\n");
        pcl::console::print_info("    -s output_name (without .pcd "
                                 "extension).......... Save outputs\n");
        pcl::console::print_info("Note: \n");
        pcl::console::print_info("  Each of the first four options depends on "
                                 "the options above it.\n");
        pcl::console::print_info("  Saving (-s) will output individual files "
                                 "for each option used (-n,-k,-f,-g).\n");
        return (1);
    }
    ...
    if (detect_keypoints) {
        ...
        keypoints = detectKeypoints(cloud, normals, min_scale, nr_octaves,
                                    nr_scales, min_contrast);
    ...
    bool save_cloud =
        pcl::console::parse_argument(argc, argv, "-s", base_filename) > 0;
    if (save_cloud) {
        if (normals) {
    ...
    // Or visualize the result
        ...
        if (normals) {
            vis.addPointCloudNormals<PointT, NormalT>(cloud, normals, 4, 0.02,
                                                      "normals");
        }
        if (keypoints) {
        ...
        if (global_descriptor) {
            hist_vis.addFeatureHistogram(*global_descriptor, 308,
                                         "Global descriptor");
        }
        vis.resetCamera();
        vis.spin();
