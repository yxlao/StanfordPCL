...
void printUsage(const char *progName) {
    cout << "\n\nUsage: " << progName
         << " [options] [scene.pcd] <model.pcl> [model_2.pcl] ... "
            "[model_n.pcl]\n\n"
         << "Options:\n"
         << "-------------------------------------------\n"
...
int main(int argc, char **argv) {
    ...
    boost::shared_ptr<pcl::RangeImagePlanar> range_image_planar_ptr(
        new pcl::RangeImagePlanar);
    ...
    narf_keypoint_detector.getParameters().max_no_of_threads =
        max_no_of_threads;
    ...
    // false; narf_keypoint_detector.getParameloadters
    ...
    while (!viewer.wasStopped()) {
        ...
        if (received_new_depth_data && depth_image_mutex.try_lock()) {
            ...
            const unsigned short *depth_map =
                depth_image_ptr->getDepthMetaData().Data();
            int width = depth_image_ptr->getWidth(),
                height = depth_image_ptr->getHeight();
            ...
            float original_angular_resolution =
                asinf(0.5f * float(width) / float(focal_length_x)) /
                (0.5f * float(width));
            float desired_angular_resolution = angular_resolution;
            range_image_planar.setDepthImage(
                depth_map, width, height, center_x, center_y, focal_length_x,
                focal_length_y, desired_angular_resolution);
            depth_image_mutex.unlock();
        ...
        double keypoint_extraction_time =
            pcl::getTime() - keypoint_extraction_start_time;
        std::cout << "Found " << keypoint_indices.points.size()
                  << " key points. "
                  << "This took " << 1000.0 * keypoint_extraction_time
                  << "ms.\n";
        ...
        for (size_t i = 0; i < keypoint_indices.points.size(); ++i)
            keypoints_cloud.points[i].getVector3fMap() =
                range_image_planar.points[keypoint_indices.points[i]]
                    .getVector3fMap();
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>
            color_handler_keypoints(keypoints_cloud_ptr, 0, 255, 0);
        if (!viewer.updatePointCloud(keypoints_cloud_ptr,
                                     color_handler_keypoints, "keypoints"))
            viewer.addPointCloud(keypoints_cloud_ptr, color_handler_keypoints,
                                 "keypoints");
