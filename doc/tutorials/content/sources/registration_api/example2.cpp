...
void estimateKeypoints(const PointCloud<PointXYZ>::Ptr &src,
    ...
    // For debugging purposes only: uncomment the lines below and use pcd_viewer
    // to view the results, i.e.: pcd_viewer source_pcd keypoints_src.pcd -ps 1
    // -ps 10
...
void estimateNormals(const PointCloud<PointXYZ>::Ptr &src,
    ...
    // For debugging purposes only: uncomment the lines below and use pcd_viewer
    // to view the results, i.e.: pcd_viewer normals_src.pcd
...
void estimateFPFH(const PointCloud<PointXYZ>::Ptr &src,
    ...
    // For debugging purposes only: uncomment the lines below and use pcd_viewer
    // to view the results, i.e.: pcd_viewer fpfhs_src.pcd
    PointCloud2 s, t, out;
    toROSMsg(*keypoints_src, s);
    toROSMsg(fpfhs_src, t);
    concatenateFields(s, t, out);
    savePCDFile("fpfhs_src.pcd", out);
    toROSMsg(*keypoints_tgt, s);
    toROSMsg(fpfhs_tgt, t);
    concatenateFields(s, t, out);
...
void computeTransformation(const PointCloud<PointXYZ>::Ptr &src,
    ...
    print_info(
        "Found %zu and %zu keypoints for the source and target datasets.\n",
    ...
    print_info(
        "Estimated %zu and %zu normals for the source and target datasets.\n",
    ...
    // Obtain the best transformation between the two sets of keypoints given
