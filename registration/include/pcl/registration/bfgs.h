...
template <typename _Scalar, int NX = Eigen::Dynamic> struct BFGSDummyFunctor {
 ...
 * It reproduces Fletcher's original algorithm in Practical Methods of
 * Optimization algorithms : 2.6.2 and 2.6.4 and uses the same politics in GSL
 * with cubic interpolation whenever it is possible else falls to quadratic
 * interpolation for alpha parameter.
 */
template <typename FunctorType> class BFGS {
  public:
    ...
    struct Parameters {
        Parameters()
            : max_iters(400), bracket_iters(100), section_iters(100), rho(0.01),
              sigma(0.01), tau1(9), tau2(0.05), tau3(0.5), step_size(1),
              order(3) {}
  ...
  private:
    ...
    BFGSSpace::Status lineSearch(Scalar rho, Scalar sigma, Scalar tau1,
                                 Scalar tau2, Scalar tau3, int order,
                                 Scalar alpha1, Scalar &alpha_new);
    Scalar interpolate(Scalar a, Scalar fa, Scalar fpa, Scalar b, Scalar fb,
                       Scalar fpb, Scalar xmin, Scalar xmax, int order);
    void checkExtremum(const Eigen::Matrix<Scalar, 4, 1> &coefficients,
    ...
    void updatePosition(Scalar alpha, FVectorType &x, Scalar &f,
                        FVectorType &g);
...
void BFGS<FunctorType>::checkExtremum(
    const Eigen::Matrix<Scalar, 4, 1> &coefficients, Scalar x, Scalar &xmin,
    Scalar &fmin) {
...
template <typename FunctorType> void BFGS<FunctorType>::moveTo(Scalar alpha) {
    x_alpha = x0 + alpha * p;
    x_cache_key = alpha;
}

template <typename FunctorType>
typename BFGS<FunctorType>::Scalar BFGS<FunctorType>::slope() {
    return (g_alpha.dot(p));
}

template <typename FunctorType>
typename BFGS<FunctorType>::Scalar BFGS<FunctorType>::applyF(Scalar alpha) {
...
template <typename FunctorType>
typename BFGS<FunctorType>::Scalar BFGS<FunctorType>::applyDF(Scalar alpha) {
...
template <typename FunctorType>
void BFGS<FunctorType>::applyFDF(Scalar alpha, Scalar &f, Scalar &df) {
...
template <typename FunctorType>
void BFGS<FunctorType>::updatePosition(Scalar alpha, FVectorType &x, Scalar &f,
                                       FVectorType &g) {
    {
...
template <typename FunctorType> void BFGS<FunctorType>::changeDirection() {
    x_alpha = x0;
    x_cache_key = 0.0;
    f_cache_key = 0.0;
    g_alpha = g0;
...
BFGSSpace::Status BFGS<FunctorType>::minimizeOneStep(FVectorType &x) {
    ...
    if (delta_f < 0) {
        Scalar del = std::max(
            -delta_f, 10 * std::numeric_limits<Scalar>::epsilon() * fabs(f0));
        alpha1 = std::min(1.0, 2.0 * del / (-fp0));
    } else
        alpha1 = fabs(parameters.step_size);

    BFGSSpace::Status status = lineSearch(
        parameters.rho, parameters.sigma, parameters.tau1, parameters.tau2,
...
template <typename FunctorType>
typename BFGSSpace::Status BFGS<FunctorType>::testGradient(Scalar epsilon) {
    if (epsilon < 0)
...
BFGS<FunctorType>::interpolate(Scalar a, Scalar fa, Scalar fpa, Scalar b,
                               Scalar fb, Scalar fpb, Scalar xmin, Scalar xmax,
                               int order) {
    /* Map [a,b] to [0,1] */
    ...
    if (order > 2 && !(fpb != fpb) &&
        fpb != std::numeric_limits<Scalar>::infinity()) {
        fpa = fpa * (b - a);
        fpb = fpb * (b - a);
        ...
        checkExtremum(coefficients, ymax, y, fmin);
        {
            ...
            bool real_roots;
            Eigen::PolynomialSolver<Scalar, 2> solver(coefficients2,
                                                      real_roots);
            if (real_roots) {
...
template <typename FunctorType>
BFGSSpace::Status
BFGS<FunctorType>::lineSearch(Scalar rho, Scalar sigma, Scalar tau1,
                              Scalar tau2, Scalar tau3, int order,
                              Scalar alpha1, Scalar &alpha_new) {
    Scalar f0, fp0, falpha, falpha_prev, fpalpha, fpalpha_prev, delta,
        alpha_next;
    ...
    while (i++ < parameters.bracket_iters) {
...

        {
            ...
            alpha_next =
                interpolate(alpha_prev, falpha_prev, fpalpha_prev, alpha,
                            falpha, fpalpha, lower, upper, order);
        }
