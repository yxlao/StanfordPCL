...
class ON_CLASS ON_MeshParameters {
  ...
  public:
    ...
    */
    ...
    // C++ default works fine // ON_MeshParameters& operator=(const
    // ON_MeshParameters&);
    ...
    */
    ...
    bool m_bDoublePrecision; // false - (default) the mesh vertices will be
                             //         float precision values in the m_V[]
                             //         array.
                             // true -  The mesh vertices will be double
                             // precision
                             //         values in the DoublePrecisionVertices()
                             //         array.  Float precision values will also
                             //         be returned in the m_V[] array.
    bool
        m_bCustomSettingsEnabled; // false - if these settings should be ignored
  ...
  public:
    ...
    double
        m_grid_aspect_ratio; // desired aspect ratio of quads in grid
    ...
    double m_grid_amplification; // The parameters above generate a grid.
                                 // If you want fewer quads, set
                                 // m_grid_amplification to a value < 1.  If you
                                 // want more quads, set m_grid_amplification to
                                 // a value > 1. default = 1 and values <= 0 are
                                 // treated as 1.
...
class ON_CLASS ON_MeshCurvatureStats {
  public:
    ...
    bool Set(ON::curvature_style,
             ...
             const ON_3fVector *, // N[] surface normals needed for normal
...
struct ON_MeshTopologyEdge {
    ...
    // m_topf_count = number of topological faces tat share this topological
    // edge
    int m_topf_count;
...
struct ON_MeshFaceSide {
    ...
    unsigned char side;   // edge connects mesh m_V[m_F[fi].vi[side]] and
                          // m_V[m_F[fi].vi[(side+1)%4]]
    unsigned char dir;    // 0 = counterclockwise, 1 = clockwise (reversed)
    unsigned short value; // Set to zero by ON_Mesh::GetFaceSideList(). Can be
                          // used as needed.
};
...
class ON_CLASS ON_MeshTopology {
  ...
  private:
    ...
    volatile int m_b32IsValid; // sizeof(m_bIsValid) must be 4 - it is used in
                               // sleep locks.
                               //    0: Not Valid
                               //    1: Valid
  private:
...
class ON_CLASS ON_MappingTag {
  public:
    ...
    int Compare(const ON_MappingTag &other, bool bCompareId = true,
                bool bCompareCRC = true, bool bCompareXform = true) const;
...
class ON_CLASS ON_Mesh : public ON_Geometry {
  ...
  public:
    ...
    */
    ...
    ON_BOOL32 SwapCoordinates(int, int // indices of coords to swap
    );
    ...
    */
    ...
    void InvalidateVertexNormalBoundingBox(); // Call if defining geometry is
                                              // changed by directly
                                              // manipulating the m_N[] array.
    void
    InvalidateTextureCoordinateBoundingBox(); // Call if defining geometry is
                                              // changed by directly
                                              // manipulating the m_T[] array.
    void InvalidateCurvatureStats(); // Call if defining geometry is changed by
                                     // directly manipulating the m_T[] array.
    void InvalidateBoundingBoxes();  // Invalidates all cached bounding box
                                     // information.
    ...
    void FlipFaceOrientation(); // reverses face orientation (does nothing to
                                // normals)
    ...
    */
    bool ConvertTrianglesToQuads(double angle_tol_radians,
    ...
    */
    int GetClashingFacePairs(int max_pair_count,
    ...
    */
    bool SetTextureCoordinates(const class ON_TextureMapping &mapping,
    ...
    // finds all coincident vertices and merges them if break angle is small
    // enough
    ...
    */
    bool CombineIdenticalVertices(bool bIgnoreVertexNormals = false,
    ...
    */
    bool IsManifold(bool bTopologicalTest, bool *pbIsOriented = NULL,
    ...
    */
    ...
    See Also:
      ON_Mesh::IsSolid()
    */
    bool IsPointInside(ON_3dPoint test_point, double tolerance,
    ...
    */
    int GetVertexEdges(int vcount, const int *vertex_index, bool bNoDuplicates,
    ...
    */
    ...
    Remarks:
      ...
      Degenerate faces are processed, but degenerate sides (equal vertex
    indices) are not added to the list.
    */
    int GetMeshFaceSideList(const int *Vid,
    ...
    Parameters:
      ...
      ci_meshtop_edge_map - [out]
        If you call the verson of GetMeshEdgeList() with the
    ci_meshtop_edge_map[], parameter, then the edge in edge_list[i] cooresponds
    to the edge in ON_MeshTopology.m_tope[ci_meshtop_edge_map[i]]. The value
      ...
      ci_meshtop_vertex_map - [out]
        If you call the verson of GetMeshEdgeList() with the
    ci_meshtop_vertex_map[], parameter, then the vertex m_V[i] cooresponds to
    the vertex in ON_MeshTopology.m_topv[ci_meshtop_vertex_map[i]]. The value
    ...
    Remarks:
      ...
      clear m_is_closed.
    */
    int GetMeshEdgeList(ON_SimpleArray<ON_2dex> &edge_list,
    ...
    */
    ...
    ON_SimpleArray<ON_SurfaceCurvature>
        m_K; // OPTIONAL surface curvatures
    ...
    ON_SimpleArray<ON_Color> m_C; // OPTIONAL vertex color
                                  // Either m_C[] has zero count or it has the
                                  // same count as m_V[], in which case m_C[j]
                                  // reports the color assigned to m_V[j].

    /////////////////////////////////////////////////////////////////
    // Implementation - runtime vertex visibility - not saved in 3dm files.
    ON_SimpleArray<bool>
        m_H;            // OPTIONAL vertex visibility.
    ...
    const ON_Object
        *m_parent; // runtime parent geometry (use ...::Cast() to get it)
  ...
  private:
    char m_mesh_is_closed; // 0 = unset, 1 = all edges have 2 or more faces, 2 =
                           // at least one boundary edge
    char m_mesh_is_manifold; // 0 = unset, 1 = all edges have 1 or 2 faces, 2 =
                             // not manifold
    char m_mesh_is_oriented; // 0 = unset, 1 = faces normals agree across all
                             // edges that have 2 faces, 2 = not oriented
    char m_mesh_is_solid; // 0 = unset, 1 = solid with outward face normals, 2 =
                          // solid with inward face normals, 3 = not solid
  ...
  protected:
    ...
    ON_MeshCurvatureStats
        *m_kstat[4]; // gaussian,mean,min,max,sectionx,sectiony,sectionz
...
class ON_CLASS ON_MeshVertexRef : public ON_Geometry {
  ...
  public:
    ...
    */
    ...
    int Dimension() const;
    ON_BOOL32 GetBBox(double *boxmin, double *boxmax,
...
*/
ON_DECL
ON_Mesh *ON_MeshSurface(const ON_Surface &surface, int mesh_density = 0,
                        ON_Mesh *mesh = 0);
...
ON_Mesh *ON_MeshSurface(const ON_Surface &surface, int u_count, const double *u,
                        int v_count, const double *v, ON_Mesh *mesh = 0);
...
*/
ON_DECL
bool ON_ClosestPointToTriangleFast(const ON_3dPoint &A, const ON_3dPoint &B,
                                   const ON_3dPoint &C, ON_3dPoint P, double *a,
                                   double *b, double *c);
...
Parameters:
  ...
  q - [out] barycentric coordinates for Quad
        ...
        Tri and  (q[0])*Quad[0] + (q[1])*Quad[1] + (q[2])*Quad[2] +
(q[3])*Quad[3] is the point on Quad.  It is always the case that t[0]+t[1]+t[2]
= 1, t[0]>=0, t[1]>0, t[2]>0, q[0]+q[1]+q[2]+q[3] = 1, q[0]>=0, q[1]>0, q[2]>0,
q[3]. The surface of Quad is defined to be the two triangle surfaces, <Quad[0],
Quad[1], Quad[2]> and <Quad[2], Quad[3], Quad[0]>.  It will always be the case
that either q[1]=0 or q[3]=0. Returns: True if successful.
*/
ON_DECL
...
Parameters:
  ...
  b - [out] barycentric coordinates for quad B
        ...
        always the case that a[0]+a[1]+a[2]+a[3] = 1, a[0]>=0, a[1]>0, a[2]>0,
a[3]>0. b[0]+b[1]+b[2]+b[3] = 1, b[0]>=0, b[1]>0, b[2]>0, b[3]>0 The surface of
Quad is defined to be the two triangle surfaces, <Quad[0], Quad[1], Quad[2]> and
<Quad[2], Quad[3], Quad[0]>.  It will always be the case that either q[1]=0 or
q[3]=0. Returns: True if successful.
*/
ON_DECL
...
ON_Mesh *ON_ControlPolygonMesh(const ON_NurbsSurface &nurbs_surface,
                               bool bCleanMesh, ON_Mesh *input_mesh = NULL);
...
Returns:
  ...
  corner of the i-th (0<= i < point_count-2) triangle are
    (triangle[i*tri_stride], triangle[i*tri_stride+1],
triangle[i*tri_stride+2]). Remarks: Do NOT duplicate the start/end point; i.e.,
a triangle will have a point count of 3 and P will specify 3 distinct
non-collinear points.
*/
ON_DECL
bool ON_Mesh2dPolygon(int point_count, int point_stride, const double *P,
...
Parameters:
  ...
  point_stride - [in] i-th point = (point[j],point[j+1]), where
                      j = i*point_stride.
  point - [in] 2d point locations.  It is ok to include points that are inside
the region but not at the ednd of an edge.  Duplicate points are not permitted.
  ...
  edge_stride - [in] i-th edge connects points (edge[j],edge[j+1]) where
                     j = i*edge_stride.
  edge - [in] indices of edge ends.  Edges should intersect only at shared end
points. edge_side - [in] if NULL, the triangles are built on the left side of
the edges.  If not NULL, then edge[i] determines which side(s) of the edge need
                   triangles.  1 = left side only, 2 = right side only, 3 = both
sides triangles - [out]  triangles are appended to this list.  The (i,j,k) are
                     vertex indices.
  new_points - [out]
...
*/
ON_DECL
int ON_Mesh2dRegion(int point_count, int point_stride, const double *point,
                    int edge_count, int edge_stride, const int *edge,
                    const int *edge_side, ON_SimpleArray<ON_3dex> &triangles);
...
*/
ON_DECL
int ON_Mesh2dRegion(unsigned int point_count, unsigned int point_stride,
                    const double *point, unsigned int edge_count,
                    unsigned int edge_stride, const unsigned int *edge,
