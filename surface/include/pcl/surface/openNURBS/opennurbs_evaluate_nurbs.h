...
ON_DECL
bool ON_IncreaseBezierDegree(int,       // dimension
...
ON_DECL
bool ON_EvaluateBezier(int,            // dimension
                       ON_BOOL32,      // true if Bezier is rational
                       int,            // order (>=2)
...
*/
bool ON_EvaluateNurbsSpan(int dim, int is_rat, int order, const double *knot,
                          int cv_stride, const double *cv, int der_count,
...
Parameters:
  ...
  order1 - [in] >= 2
  knot0 - [in]
    NURBS knot vector with 2*(order0-1) knots, knot0[order0-2] !=
knot0[order0-1] knot1 - [in] NURBS knot vector with 2*(order1-1) knots,
knot1[order1-2] != knot1[order1-1] cv_stride0 - [in] cv_stride1 - [in] cv - [in]
...
*/
ON_DECL
bool ON_EvaluateNurbsSurfaceSpan(int dim, int is_rat, int order0, int order1,
                                 const double *knot0, const double *knot1,
                                 int cv_stride0, int cv_stride1,
                                 const double *cv, int der_count, double s,
                                 double t, int v_stride, double *v);
...
Parameters:
  ...
  knot0 - [in]
    NURBS knot vector with 2*(order0-1) knots, knot0[order0-2] !=
knot0[order0-1] knot1 - [in] NURBS knot vector with 2*(order1-1) knots,
knot1[order1-2] != knot1[order1-1] knot2 - [in] NURBS knot vector with
2*(order1-1) knots, knot2[order2-2] != knot2[order2-1] cv_stride0 - [in]
  cv_stride1 - [in]
  cv_stride2 - [in]
  cv - [in]
...
v_stride*(der_count+1)*(der_count+2)*(der_count+3)/6. The evaluation results are
...
bool ON_EvaluateNurbsCageSpan(int dim, int is_rat, int order0, int order1,
                              ...
                              const double *cv, int der_count, double t0,
...
bool ON_EvaluateNurbsDeBoor( // for expert users only - no support available
    ...
    double *,       // cv array - values changed to result of applying De Boor's
                    // algorithm
