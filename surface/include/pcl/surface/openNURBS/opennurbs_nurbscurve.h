...
class ON_CLASS ON_NurbsCurve : public ON_Curve {
  ...
  public:
    ...
    */
    ...
    static ON_NurbsCurve *New(int dimension, ON_BOOL32 bIsRational, int order,
                              int cv_count);
    ...
    ON_NurbsCurve(int dimension, ON_BOOL32 bIsRational, int order,
                  int cv_count);
    ...
    */
    bool IsDuplicate(const ON_NurbsCurve &other, bool bIgnoreParameterization,
    ...
    //   dimension - [in] 1, 2 or 3
    //   order - [in] (>=2) order=degree+1
    //   point_count - [in] (>=max(3,order-1)) number of distinct control
    //   vertices point - [in] array of distinct control vertex locations.
    //   knot_delta - [in] (>0.0) knot spacing
    // Returns:
    //   true if successful
    bool CreatePeriodicUniformNurbs(int dimension, int order, int point_count,
                                    const ON_3dPoint *point,
                                    double knot_delta = 1.0);
    ...
    */
    ...
    ON_BOOL32 IsArc(const ON_Plane *plane = NULL, ON_Arc *arc = NULL,
                    double tolerance = ON_ZERO_TOLERANCE) const;
    ...
    ON_BOOL32 IsInPlane(const ON_Plane &test_plane,
                        double tolerance = ON_ZERO_TOLERANCE) const;
    ...
    */
    bool SpanIsLinear(int span_index, double min_length,
    ...
    */
    ...
    ON_BOOL32 Split(double split_param, // t = curve parameter to split curve at
                    ON_Curve *&left_result, // left portion returned here (must
                                            // be an ON_NurbsCurve)
                    ON_Curve *&right_result // right portion returned here (must
                                            // be an ON_NurbsCurve)
    ) const;
  ...
  public:
    ...
    */
    ...
    double SuperfluousKnot(int // 0 = start, 1 = end
    ) const;

    double GrevilleAbcissa(int // index (0 <= index < CVCount(dir)
    ) const;
    ...
    //   knot_multiplicity - [in] 1 to degree - includes multiplicity of
    //   existing knots.
    // Remarks:
    ...
    bool ReserveCVCapacity(int // number of doubles to reserve
    );
    bool ReserveKnotCapacity(int // number of doubles to reserve
    );
