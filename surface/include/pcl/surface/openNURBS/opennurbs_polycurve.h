...
/*
 Description:
        ...
        is mapped to 	m_t[i] and m_segment[i].Domain()[1] is mapped to
 m_t[i+1].
*/
class ON_PolyCurve;
class ON_CLASS ON_PolyCurve : public ON_Curve {
  ...
  public:
    ...
    */
    int IsPolyline(ON_SimpleArray<ON_3dPoint> *pline_points = NULL,
    ...
    */
    ...
    ON_BOOL32 Split(double t, // t = curve parameter to split curve at
    ...
    ON_BOOL32 GetCurveParameterFromNurbFormParameter(double,  // nurbs_t
                                                     double * // curve_t
    ) const;

    // virtual ON_Curve::GetNurbFormParameterFromCurveParameter override
    ON_BOOL32 GetNurbFormParameterFromCurveParameter(double,  // curve_t
                                                     double * // nurbs_t
    ) const;
    ...
    */
    double PolyCurveParameter(int segment_index,
    ...
    */
    int SegmentIndex(ON_Interval sub_domain, int *segment_index0,
    ...
    */
    ...
    // AppendANdMatch(),and Insert() are deleted by ~ON_PolyCurve(). Use
    // ON_CurveProxy( ON_Curve*) if you want the original curve segment to
    // survive ~ON_PolyCurve().
    ON_BOOL32 Prepend(ON_Curve *); // Prepend curve.
    ON_BOOL32 Append(ON_Curve *);  // Append curve.
    ON_BOOL32 Insert(int,          // segment_index,
                     ON_Curve *);

    // PrependAndMatch() and AppendAndMatch() return false if this->IsCLosed()
    // or this->Count() > 0 and curve is closed
    ON_BOOL32 PrependAndMatch(
        ON_Curve *); // Prepend and match end of curve to start of polycurve
    ON_BOOL32 AppendAndMatch(
        ON_Curve *); // Append and match start of curve to end of polycurve
...
*/
    ...
    */
    ...
    Returns:
      ...
      Put another way, returns true if SegmentDomain(i) =
    SegmentCurve(i).Domain() for every segment index.
    */
    ...
    */
    ...
    /*
            Description:
                    Lookup a parameter in the m_t array, optionally using a
       built in snap tolerance to snap a parameter value to an element of m_t.
            Parameters:
                    t    - [in]	  	parameter
                    index -[out]	index into m_t such that if the function
       returns true then t is equal to, or is within tolerance of m_t[index]. if
       function returns false then the value of index is

                                                                             @table
                                                                                                    condition									value of index
                                                                            t<m_t[0]
       or m_t is empty				-1 m_t[i] < t < m_t[i+1]
       i for 0<=i<=m_t.Count()-2 t>m_t[ m_t.Count()-1]
       m_t.Count()-1

                    bEnableSnap -[in]  if true use tolerance when comparing to
       m_t values Returns true if the t is exactly equal to, or within tolerance
       of (only if bEnableSnap==true) m_t[index].
    */
