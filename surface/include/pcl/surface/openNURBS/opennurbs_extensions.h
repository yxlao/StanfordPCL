...
class ON_CLASS ONX_Model_UserData {
  ...
  private:
    void Destroy();
    unsigned int
        *m_ref_count; // reference counts used to avoid expensive object copying
...
/*
Description:
  Used to store render light table light definition and attributes in an
ONX_Model.
*/
  ...
  public:
    ...
    */
    ...
    See Also:
      ...
      ONX_Model::Read
    */
    bool Write(ON_BinaryArchive &archive, int version = 5,
               const char *sStartSectionComment = NULL,
               ON_TextLog *error_log = NULL);

    bool Write(const char *filename, int version = 5,
               const char *sStartSectionComment = NULL,
               ON_TextLog *error_log = NULL);

    bool Write(const wchar_t *filename, int version = 5,
               const char *sStartSectionComment = NULL,
               ON_TextLog *error_log = NULL);
      ...
      warnings - [out]
          If problems were found, warning ids are appended to this list.
            @untitled table
             ...
             3      some m_object_table[].m_attributes.m_uuid was nil or not
    unique. 4      some m_object_table[].IsValid() is false 5      some
    m_idef_table[] has an invalid or duplicate name 6      warning some
    m_idef_table[].m_object_uuid[] is not valid 7      warning some
    m_object_table[].m_object is null 8      warning some
    m_object_table[].m_object->IsValid() is false 9      warning some
    m_object_table[].m_attributes is not valid 10      linetype table is not
    perfect. 11      lineset table is not perfect. 12      some
    m_idef_table[].m_uuid was nil or not unique. 13      some
    m_texture_mapping_table[i].m_mapping_id was nil or not unique. 14      some
    m_material_table[i].m_material_id was nil or not unique. 15      some
    m_light_table[i].m_light_id was nil or not unique. Returns: True if model is
    valid and false if the model has serious
      @untitled table
      <0      model has serious errors
      =0      model is ok
      >0      number of problems that were found.
    */
    virtual int Audit(bool bAttemptRepair, int *repair_count,
    ...
    */
    void GetRenderMaterial(const ON_3dmObjectAttributes &attributes,
    ...
    */
    void GetLinetype(const ON_3dmObjectAttributes &attributes,
    ...
    */
    bool GetDocumentUserString(const wchar_t *key,
    ...
    */
    ...
    static bool GetRDKDocumentInformation(const ONX_Model_UserData &docud,
                                          ON_wString &rdk_xml_document_data);

    static bool IsRDKObjectInformation(const ON_UserData &objectud);
    static bool GetRDKObjectInformation(const ON_Object &object,
