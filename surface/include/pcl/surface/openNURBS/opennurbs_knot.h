...
ON_DECL
double ON_DomainTolerance(double, // start of domain
                          double  // end of domain
);

ON_DECL
double ON_KnotTolerance(int,            // order (>=2)
                        int,            // cv count
...
ON_DECL
double ON_SpanTolerance(int,            // order (>=2)
                        int,            // cv count
                        const double *, // knot[] array
                        int             // span index
);
...
ON_DECL
int ON_KnotMultiplicity(int,            // order (>=2)
                        int,            // cv_count (>=order)
                        const double *, // knot[]
                        int             // knot_index
);
...
ON_DECL
int ON_KnotVectorSpanCount(int,           // order (>=2)
                           int,           // cv count
                           const double * // knot[] array
);

ON_DECL
bool ON_GetKnotVectorSpanVector(int,            // order (>=2)
                                int,            // cv count
                                const double *, // knot[] array
                                double *        // s[] array
);
...
*/
ON_DECL
int ON_NurbsSpanIndex(int order, int cv_count, const double *knot, double t,
                      int side, int hint);
...
ON_DECL
int ON_GetSpanIndices( // returns span count, which is one less than length of
                       // span_indices[]
    int order, int cv_count, const double *knot,
...
ON_DECL
bool ON_GetKnotVectorDomain(int,            // order (>=2)
                            int,            // cv count
                            const double *, // knot[] array
                            double *, double *);

ON_DECL
bool ON_ReverseKnotVector(int,     // order (>=2)
                          int,     // cv count
                          double * // knot[] array
);
...
int ON_InsertKnot(double,   // knot_value,
                  int,      // knot_multiplicity, (1 to order-1 including
                            // multiplicity of any existing knots)
                  int,      // cv_dim ( = dim+1 for rational cvs )
                  int,      // order (>=2)
                  int,      // cv_count,
                  int,      // cv_stride (>=cv_dim)
                  double *, // cv[]  NULL or cv array with room for at least
                            // knot_multiplicity new cvs
                  double *, // knot[] knot array with room for at least
                            // knot_multiplicity new knots
                  int * // hint, optional hint about where to search for span to
                        // add knots to pass NULL if no hint is available
);
