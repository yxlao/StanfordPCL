...
class ON_CLASS ON_DimStyle : public ON_Object {
  ...
  public:
    ...
    */
    ...
    // source dimstyle for all of the fields that are marked overridden in the
    // source and to match the parent for all of the fields not marked
    // overriden. Returns true if any overrides were set.
    bool OverrideFields(const ON_DimStyle &source, const ON_DimStyle &parent);

    //
    // Change the fields in this dimstyle to match the fields of the
    // parent dimstyle for all of the fields that are not marked overridden in
    // the target dimstyle. This is the complement of OverrideFields()
  ...
  public:
    ...
    double m_arrowsize;    // length of an arrow - may mean different things to
                           // different arrows
    double m_centermark;   // size of the + at circle centers
    double m_textgap;      // gap around the text for clipping dim line
    double m_textheight; // model unit height of dimension text before applying
                         // dimscale
  ...
  private:
    unsigned int
        m_valid; // Obsolete deprecated field to be removed - Do not use
  public:
    // field added version 1.4, Dec 28, 05
    double m_dimextension; // (dimdle) dimension line extension past the "tip"
                           // location

    // fields added version 1.5 Mar 23 06
    double
        m_leaderarrowsize; // Like dimension arrow size but applies to leaders
