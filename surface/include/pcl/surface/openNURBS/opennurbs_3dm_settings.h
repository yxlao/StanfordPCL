...
class ON_CLASS ON_3dmUnitsAndTolerances {
  public:
    ...
    //
    // double m_custom_unit_scale;      // 1 meter = m_custom_unit_scale custom
    // units ON_wString m_custom_unit_name;   // name of custom units
};
...
class ON_CLASS ON_3dmConstructionPlaneGridDefaults {
  public:
    ...
    ~ON_3dmConstructionPlaneGridDefaults();
    ON_3dmConstructionPlaneGridDefaults(
        const ON_3dmConstructionPlaneGridDefaults &);
    ON_3dmConstructionPlaneGridDefaults &
...
class ON_CLASS ON_3dmViewTraceImage {
  public:
    ...
    bool m_bFiltered;  // true if image should be filtered (bilinear) before
                       // displayed.
};
...
class ON_CLASS ON_3dmRenderSettings {
  ...
  public:
    ...
    int m_background_style;      // 0 = solid color, 1 = "wallpaper" image, 2 =
                                 // Gradient, 3 = Environment
...
class ON_CLASS ON_EarthAnchorPoint {
  public:
    ...
    static int Compare(const ON_EarthAnchorPoint *,
                       const ON_EarthAnchorPoint *);

    static int CompareEarthLocation(const ON_EarthAnchorPoint *,
                                    const ON_EarthAnchorPoint *);

    static int CompareModelDirection(const ON_EarthAnchorPoint *,
                                     const ON_EarthAnchorPoint *);

    static int CompareIdentification(const ON_EarthAnchorPoint *,
                                     const ON_EarthAnchorPoint *);
    ...
    */
    bool GetModelToEarthXform(const ON_UnitSystem &model_unit_system,
...
class ON_CLASS ON_3dmIOSettings {
  public:
    ...
    int m_idef_link_update; // 0 = use ON_InstanceDefinition setting
                            //       The
                            //       ON_InstanceDefinition::m_idef_update_type
                            //       field controls when and with how much
                            //       prompting embedded or linked idefs get
                            //       updated.
                            // 1 = prompt
...
class ON_CLASS ON_3dmSettings {
  public:
    ...
    //   1       boundary + "knot" wires + 1 interior wire if no interior
    //   "knots" N>=2    boundry + "knot" wires + (N-1) interior wires
