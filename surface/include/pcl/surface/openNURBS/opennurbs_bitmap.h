...
class ON_CLASS ON_Bitmap : public ON_Object {
  ...
  public:
    ...
    virtual int Height() const = 0; // >0 means it's a bottom-up bitmap with
                                    // origin at lower right <0 means it's a
                                    // top-down bitmap with origin at upper left
    virtual int BitsPerPixel() const = 0; // bits per pixel
    virtual int SizeofScan() const = 0;   // number of bytes per scan line
    virtual int SizeofImage() const = 0;  // size of current map in bytes

    virtual unsigned char *Bits(int // index of scan line
                                ) = 0;
    virtual const unsigned char *Bits(int // index of scan line
    ) const = 0;
...
class ON_CLASS ON_EmbeddedFile : public ON_Object {
  ...
  public:
    ...
    */
    bool Create(const void *source_buffer, ON__UINT64 sizeof_source_buffer,
                bool bCompress);
    ...
    */
    ...
    ON_wString
        m_relative_file_name; // relative path when the archive was last saved.
  ...
  public:
    ON__UINT32 m_buffer_crc; // will be different from m_file_crc if the buffer
                             // is compressed.
    ON_Buffer m_buffer;
...
// For details searh for "BITMAPINFOHEADER" at
...
struct ON_WindowsBITMAPINFOHEADER {
    ...
    unsigned short
        biBitCount; // WORD  = bits per pixel (0,1,4,8,16,24,32 are valid)
                    ...
                    //           http://msdn.microsoft.com/default.asp
    unsigned int
        biCompression; // DWORD   Currently, Windows defines the following
                       //         types of compression.
                       //         =0  BI_RGB (no compression)
                       //         =1  BI_RLE8 (run length encoded used for 8
                       //         bpp) =2  BI_RLE4 (run length encoded used for
                       //         4 bpp) =3  BI_BITFIELDS  Specifies that the
                       //         bitmap is
    ...
    unsigned int
        biClrUsed; // DWORD = 0 or true length of bmiColors[] array.  If 0,
...
struct ON_WindowsBITMAPINFO {
    ...
    // For details searh for "BITMAPINFO" at
    // http://msdn.microsoft.com/default.asp
    ON_WindowsBITMAPINFOHEADER bmiHeader;
    ON_WindowsRGBQUAD
        bmiColors[1]; // The "[1]" is for the compiler.  In
...
class ON_CLASS ON_WindowsBitmap : public ON_Bitmap {
  ...
  public:
    ...
    */
    ...
    int Width() const;
    int Height()
        const; // >0 means it's a bottom-up bitmap with origin at lower right
    ...
    unsigned char *Bits(int // index of scan line
    );
    const unsigned char *Bits(int // index of scan line
    ) const;

    // int PaletteIndex( ON_Color ) const; // for 8bpp bitmaps

    ON_Color Pixel(int, // 0 <= i < width
                   int  // 0 <= j < height
    ) const;
    ON_Color Pixel(int,                  // 0 <= i < width
    ...
    */
    ...
    Example:
            ...
            BITMAPINFO* bmi = (LPBITMAPINFO)calloc( 1, sizeof(*bmi) +
    sizeof_palette );
            ...
            HBITMAP hbm = ::CreateDIBSection( NULL, bmi, ..., (LPVOID*)&bits,
    NULL, 0);
    ...
    */
    bool Create(const BITMAPINFO *bmi, const unsigned char *bits, bool bCopy);
  ...
  private:
    int m_bFreeBMI; // 0 m_bmi and m_bits are not freed by
                    // ON_WindowsBitmap::Destroy 1 m_bmi  memory is freed by
                    // ON_WindowsBitmap::Destroy 2 m_bits memory is freed by
                    // ON_WindowsBitmap::Destroy 3 m_bmi and m_bits memory is
                    // freed by ON_WindowsBitmap::Destroy
};
