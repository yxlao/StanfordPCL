...
class ON_CLASS ON_Ellipse {
  public:
    ...
    ON_BOOL32 Create(const ON_Plane &, // point on the plane
    ...
    double Radius(int // 0 = x axis radius, 1 = y axis radius
    ) const;
    ...
    */
    ...
    // t -> plane.origin + cos(t)*radius[0]*plane.xaxis +
    // sin(t)*radius[1]*plane.yaxis evaluate parameters and return point
    ON_3dPoint PointAt(double) const;
    ON_3dVector DerivativeAt(int,   // desired derivative ( >= 0 )
                             double // parameter
    ) const;
    ...
    ON_BOOL32 Rotate(double,             // sin(angle)
                     double,             // cos(angle)
                     const ON_3dVector & // axis of rotation
    );
    ON_BOOL32 Rotate(double,             // angle in radians
                     const ON_3dVector & // axis of rotation
    );
