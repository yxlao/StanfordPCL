  ...
  public:
    ...
    */
    ...
    bool IsEmptyInterval()
        const;            // returns true if m_t[0] == m_t[1] == ON_UNSET_VALUE
...
class ON_CLASS ON_2dPoint {
  public:
    ...
    ON_2dPoint &operator/=(double);
    ON_2dPoint &
    operator+=(const ON_2dPoint &); // Adding this was a mistake - cannot remove
                                    // without breaking SDK
    ON_2dPoint &operator+=(const ON_2dVector &);
    ON_2dPoint &
    operator+=(const ON_3dVector &); // Adding this was a mistake - cannot
                                     // remove without breaking SDK
    ON_2dPoint &
    operator-=(const ON_2dPoint &); // Adding this was a mistake - cannot remove
                                    // without breaking SDK
    ON_2dPoint &operator-=(const ON_2dVector &);
    ...
    double operator*(
        const ON_2dPoint &) const; // dot product for points acting as vectors
    double operator*(
        const ON_2dVector &) const; // dot product for points acting as vectors
...
class ON_CLASS ON_3dPoint {
  public:
    ...
    static const ON_3dPoint
        UnsetPoint; // (ON_UNSET_VALUE,ON_UNSET_VALUE,ON_UNSET_VALUE)
    ...
    ON_3dPoint &operator/=(double);
    ON_3dPoint &
    operator+=(const ON_3dPoint &); // Adding this was a mistake - cannot remove
                                    // without breaking SDK
    ON_3dPoint &operator+=(const ON_3dVector &);
    ON_3dPoint &
    operator-=(const ON_3dPoint &); // Adding this was a mistake - cannot remove
    ...
    double operator*(
        const ON_3dPoint &) const; // dot product for points acting as vectors
    double operator*(
        const ON_3dVector &) const; // dot product for points acting as vectors
    double operator*(const ON_4dPoint &) const;
    ...
    */
    ...
    double Fuzz(double tolerance = ON_ZERO_TOLERANCE)
        const; // tolerance to use when comparing 3d points
    ...
    void Rotate(double sin_angle,        // sin(angle)
                double cos_angle,        // cos(angle)
                const ON_3dVector &axis, // axis of rotation
                const ON_3dPoint &center // center of rotation
    );
...
class ON_CLASS ON_2dVector {
  public:
    ...
    static const ON_2dVector &UnitVector(int // index
    );
    ...
    double operator*(const ON_2dPoint &)
        const; // inner (dot) product (point acting as vector)
    ...
    */
    ...
    //
    bool IsTiny(double tiny_tol = ON_ZERO_TOLERANCE // tiny_tol
    ) const;
    ...
    void Transform(const ON_Xform & // can use ON_Xform here
    );
...
ON_DECL
double ON_DotProduct(const ON_2dVector &, const ON_2dVector &);

ON_DECL
ON_3dVector ON_CrossProduct(const ON_2dVector &, const ON_2dVector &);

ON_DECL
double ON_WedgeProduct(   // signed area of the parallelagram.  Volume element.
    const ON_2dVector &A, // returns A.x * B.y - A.y * B.x
    const ON_2dVector &B);

ON_DECL
bool ON_IsOrthogonalFrame( // true if X, Y are nonzero and mutually
                           // perpendicular
    const ON_2dVector &,   // X
    const ON_2dVector &    // Y
);
...
ON_DECL
bool ON_IsOrthonormalFrame( // true if X, Y are orthogonal and unit length
    const ON_2dVector &,    // X
    const ON_2dVector &     // Y
);

ON_DECL
bool ON_IsRightHandFrame( // true if X, Y are orthonormal and right handed
    const ON_2dVector &,  // X
    const ON_2dVector &   // Y
);
...
class ON_CLASS ON_3dVector {
  public:
    ...
    static const ON_3dVector &UnitVector(int // index
    );
    ...
    */
    ...
    double Fuzz(double tolerance = ON_ZERO_TOLERANCE)
        const; // tolerance to use when comparing 3d vectors
...
class ON_CLASS ON_PlaneEquation : public ON_3dVector {
  public:
    ...
    */
    double *ValueAt(int Pcount, const ON_3fPoint *P, double *value,
                    double value_range[2]) const;

    double *ValueAt(int Pcount, const ON_3dPoint *P, double *value,
                    double value_range[2]) const;
    ...
    */
    double MaximumValueAt(bool bRational, int point_count, int point_stride,
    ...
    Returns:
      ...
      If the input is not valid, then ON_UNSET_VALUE is returned.
    */
    double MinimumValueAt(bool bRational, int point_count, int point_stride,
                          const double *points, double stop_value) const;
    ...
    Returns:
      ...
      If the input is not valid, then ON_UNSET_VALUE is returned.
    */
    double MaximumAbsoluteValueAt(bool bRational, int point_count,
    ...
    */
    bool IsNearerThan(const class ON_BezierCurve &bezcrv, double s0, double s1,
...
ON_DECL
double ON_DotProduct(const ON_3dVector &, const ON_3dVector &);

ON_DECL
ON_3dVector ON_CrossProduct(const ON_3dVector &, const ON_3dVector &);

ON_DECL
ON_3dVector ON_CrossProduct( // 3d cross product for old fashioned arrays
    const double *,          // array of 3d doubles
    const double *           // array of 3d doubles
);

ON_DECL
double ON_TripleProduct(const ON_3dVector &, const ON_3dVector &,
                        const ON_3dVector &);

ON_DECL
double ON_TripleProduct( // 3d triple product for old fashioned arrays
    const double *,      // array of 3d doubles
    const double *,      // array of 3d doubles
    const double *       // array of 3d doubles
);
...
ON_DECL
bool ON_IsOrthogonalFrame( // true if X, Y, Z are nonzero and mutually
                           // perpendicular
    const ON_3dVector &,   // X
    const ON_3dVector &,   // Y
    const ON_3dVector &    // Z
...
ON_DECL
bool ON_IsOrthonormalFrame( // true if X, Y, Z are orthogonal and unit length
    const ON_3dVector &,    // X
    const ON_3dVector &,    // Y
    const ON_3dVector &     // Z
);
...
ON_DECL
bool ON_IsRightHandFrame( // true if X, Y, Z are orthonormal and right handed
    const ON_3dVector &,  // X
    const ON_3dVector &,  // Y
    const ON_3dVector &   // Z
);
