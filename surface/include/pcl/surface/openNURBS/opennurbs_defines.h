...
// OPENNURBS_EXPORTS is Microsoft's prefered defined for building an opennurbs
// DLL.
...
// OPENNURBS_EXPORTS is Microsoft's prefered defined for linking with an
// opennurbs DLL.
...
/* compiling OpenNurbs as a Windows DLL - export classes, functions, templates,
 * and globals */
...
/* using OpenNurbs as a Windows DLL - import classes, functions, templates, and
 * globals */
...
class ON_CLASS ON {
  public:
    ...
    */
    ...
    static int CloseAllFiles(); // like _fcloseall() - needed when OpenNURBS is
                                // used as a DLL
    ...
    Parameters:
      ...
      lastmodify_time - [out] (can be NULL if you do not want last modification
    time) Returns: True if file exists, can be opened for read, and fstat
    worked.
    */
    static bool GetFileStats(const wchar_t *filename, size_t *filesize,
                             time_t *create_time, time_t *lastmodify_time);

    static bool GetFileStats(FILE *fp, size_t *filesize, time_t *create_time,
    ...
    Remarks:
      ...
      parameters.
    */
    static double UnitScale(ON::unit_system us_from, ON::unit_system us_to);
    static double UnitScale(const class ON_UnitSystem &us_from,
                            const class ON_UnitSystem &us_to);
    static double UnitScale(ON::unit_system us_from,
                            const class ON_UnitSystem &us_to);
    static double UnitScale(const class ON_UnitSystem &us_from,
                            ON::unit_system us_to);
    static double UnitScale(const class ON_3dmUnitsAndTolerances &us_from,
    ...
    */
    ...
    enum surface_style {
        ...
        ruled_surface =
            9, // portion of a ruled surface this is none of the above
    ...
    //////////////////////////////////////////////////////////////////
    enum endian {
        little_endian = 0, // least significant byte first or reverse byte order
                           // - Intel x86, ...
        big_endian =
            1 // most significant byte first - Motorola, Sparc, MIPS, ...
    };
    ...
    */
    ...
    enum exception_type {
        unknown_exception = 0,
        out_of_memory,
        corrupt_object, // invalid object encountered - continuing would crash
                        // or result in corrupt object being saved in archive.
        unable_to_write_archive, // write operation failed - out of file
                                 // space/read only mode/...?
        unable_to_read_archive,  // read operation failed - truncated
                                 // archive/locked file/... ?
        unable_to_seek_archive,  // seek operation failed - locked file/size out
                                 // of bounds/... ?
    ...
    enum layer_mode {
        normal_layer =
            0, // visible, objects on layer can be selected and changed
        hidden_layer =
            1, // not visible, objects on layer cannot be selected or changed
        locked_layer =
            2, // visible, objects on layer cannot be selected or changed
        layer_mode_count = 3
    };
    ...
    enum object_color_source {
        ...
        color_from_parent =
            3 // for objects with parents (like objects in instance references,
              // use parent linetype) if no parent, treat as color_from_layer
    };
    ...
    enum plot_color_source {
        ...
        plot_color_from_parent =
            3 // for objects with parents (like objects in instance references,
              // use parent plot color) if no parent, treat as
    ...
    enum object_linetype_source {
        ...
        linetype_from_parent = 3  // for objects with parents (like objects in
                                 // instance references, use parent linetype) if
                                 // not parent, treat as linetype_from_layer.
    };
    ...
    enum light_style {
        ...
        // view_directional_light   = 1, // light location and direction in clip
        // coordinates view_point_light         = 2, view_spot_light          =
        // 3,
        camera_directional_light =
            4, // light location and direction in camera coordinates
        camera_point_light =
            5, //   +x points to right, +y points up, +z points towards camera
        camera_spot_light = 6,
        world_directional_light =
            7, // light location and direction in world coordinates
        world_point_light = 8,
        world_spot_light = 9,
    ...
    enum object_type {
        ...
        curve_object =
            4, // some type of ON_Curve like ON_LineCurve, ON_NurbsCurve, etc.
        surface_object = 8,     // some type of ON_Surface like ON_PlaneSurface,
                                // ON_NurbsSurface, etc.
        ...
        polysrf_filter =
            0x200000, // selection filter value - not a real object type
        edge_filter =
            0x400000, // selection filter value - not a real object type
        polyedge_filter =
            0x800000, // selection filter value - not a real object type
  ...
  public:
    ...
    */
    ...
    Returns:
      ...
      > 0: a > b
    */
    static int Compare(const ON_COMPONENT_INDEX *a,
    ...
    */
    ...
    The interpretation of m_index depends on the m_type value.
      ...
      no_type            used when context makes it clear what array is being
    index brep_vertex        ON_Brep.m_V[] array index brep_edge ON_Brep.m_E[]
    array index brep_face          ON_Brep.m_F[] array index brep_trim
    ON_Brep.m_T[] array index brep_loop          ON_Brep.m_L[] array index
      ...
      extrusion_bottom_profile  Use ON_Extrusion::Profile3d() to get 3d profile
    curve extrusion_top_profile     Use ON_Extrusion::Profile3d() to get 3d
    profile curve extrusion_wall_edge       Use ON_Extrusion::WallEdge() to get
    3d line curve extrusion_wall_surface    Use ON_Extrusion::WallSurface() to
    get 3d wall surface extrusion_cap_surface      0 = bottom cap, 1 = top cap
      extrusion_path            -1 = entire path, 0 = start of path, 1 = end of
    path
