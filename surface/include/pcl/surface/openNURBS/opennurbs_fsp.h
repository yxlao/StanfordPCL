...
class ON_CLASS ON_FixedSizePool {
  public:
    ...
    Parameters:
      sizeof_element - [in]
        number of bytes in each element. This parameter must be greater than
    zero. In general, use sizeof(element type).  If you pass a "raw" number as
    ...
    */
    bool Create(size_t sizeof_element, size_t element_count_estimate,
    ...
    */
    ...
    /*
    Returns:
      Number of active elements. (Elements that have been returned are not
    active.)
    */
    ...
    /*
    Returns:
      Total number of elements = number of active elements + number of returned
    elements.
    */
    ...
    Example:
      ...
      using ReturnElement().
            ...
            for ( void* p = FirstElement(); 0 != p; p = NextElement() )
            {
              ...
              // must check some value in p located after the first
    sizeof(void*)
  ...
  private:
    ...
    size_t
        m_total_element_count; // total number of elements (active + returned)
