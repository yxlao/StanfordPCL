...
class ON_CLASS ON_ClassId {
  public:
    ...
    //   create - [in] function to create a new object(like
    //   CreateNewON_Geometry()) copy - [in] function to copy sUUID - [in] UUID
    //   in registry format from Windows guidgen.exe
    ON_ClassId(const char *sClassName, const char *sBaseClassName,
               ON_Object *(*create)(), const char *sUUID);

    ON_ClassId(const char *sClassName, const char *sBaseClassName,
               ON_Object *(*create)(),
               bool (*copy)(const ON_Object *, ON_Object *), const char *sUUID);
    ...
    // Example:
    //   ON_UUID brep_uuid =
    //   ON_UuidFromString("60B5DBC5-E660-11d3-BFE4-0010830122F0"); const
    //   ON_ClassId* brep_id = ON_CLassId::ClassId(brep_uuid);
  ...
  private:
    ...
    void ConstructorHelper(const char *sClassName, const char *sBaseClassName,
                           const char *sUUID);
...
#define ON_VIRTUAL_OBJECT_IMPLEMENT(cls, basecls, uuid)                        \
    ...
    const cls *cls::Cast(const ON_Object *p) {                                 \
        return (p && p->IsKindOf(&cls::m_##cls##_class_id)) ? (const cls *)p   \
                                                            : 0;               \
    }                                                                          \
    const ON_ClassId *cls::ClassId() const {                                   \
    ...
    cls *cls::Duplicate() const {                                              \
        return static_cast<cls *>(DuplicateObject());                          \
    }
...
class ON_CLASS ON_Object {
  ...
  public:
    ...
    */
    int SetUserStrings(int count, const ON_UserString *user_strings,
                       bool bReplace);
