...
class ON_CLASS ON_Cylinder {
  public:
    ...
    bool Create(const ON_Circle & // point on the bottom plane
    );
    ...
    ON_Circle CircleAt(double // linear parameter
    ) const;
    ON_Line LineAt(double // angular parameter
    ) const;
    ...
    bool Rotate(double,             // sin(angle)
                double,             // cos(angle)
                const ON_3dVector & // axis of rotation
    );
    bool Rotate(double,             // angle in radians
                const ON_3dVector & // axis of rotation
    );
    ...
    // parameterization of NURBS surface does not match cylinder's
