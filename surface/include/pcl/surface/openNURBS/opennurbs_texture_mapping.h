...
class ON_CLASS ON_TextureMapping : public ON_Object {
  public:
    ...
    /*
    Determines whether the mapping, as currently set up, requires vertex normals
    to be present on the mesh in order to evaluate the mapping correctly.
            */
    ...
    */
    bool SetPlaneMapping(const ON_Plane &plane, const ON_Interval &dx,
    ...
    */
    bool SetBoxMapping(const ON_Plane &plane, ON_Interval dx, ON_Interval dy,
                       ON_Interval dz, bool bIsCapped);
...
NOTE WELL:
  ...
  location of the plane will be the same.
    */
    bool GetMappingPlane(ON_Plane &plane, ON_Interval &dx, ON_Interval &dy,
...
location of the box will be the same.
    */
    bool GetMappingBox(ON_Plane &plane, ON_Interval &dx, ON_Interval &dy,
                       ON_Interval &dz) const;
    ...
    */
    virtual int Evaluate(const ON_3dPoint &P, const ON_3dVector &N,
                         ON_3dPoint *T) const;
    ...
    int EvaluatePlaneMapping(const ON_3dPoint &P, const ON_3dVector &N,
                             ON_3dPoint *T) const;

    int EvaluateSphereMapping(const ON_3dPoint &P, const ON_3dVector &N,
                              ON_3dPoint *T) const;

    int EvaluateCylinderMapping(const ON_3dPoint &P, const ON_3dVector &N,
                                ON_3dPoint *T) const;

    int EvaluateBoxMapping(const ON_3dPoint &P, const ON_3dVector &N,
                           ON_3dPoint *T) const;

    int EvaluateMeshMapping(const ON_3dPoint &P, const ON_3dVector &N,
                            const ON_Mesh *mesh, ON_3dPoint *T) const;

    int EvaluateSurfaceMapping(const ON_3dPoint &P, const ON_3dVector &N,
                               const ON_Surface *srf, ON_3dPoint *T) const;

    int EvaluateBrepMapping(const ON_3dPoint &P, const ON_3dVector &N,
                            const ON_Brep *brep, ON_3dPoint *T) const;

    static void
        SetAdvancedBrepMappingToolFunctions(TEXMAP_INTERSECT_LINE_SURFACE,
    ...
    Returns:
      ...
      mapping.
    */
    bool HasMatchingTextureCoordinates(const ON_Mesh &mesh,
                                       const ON_Xform *object_xform = 0) const;
    bool HasMatchingTextureCoordinates(const class ON_MappingTag &tag,
                                       const ON_Xform *object_xform = 0) const;
    ...
    Returns:
      True if successful.
    */
    bool GetTextureCoordinates(const ON_Mesh &mesh,
