...
class ON_CLASS ON_Surface : public ON_Geometry {
  ...
  public:
    ...
    Remarks:
      ...
      the returned surfaces's parameterization and locus match the proxy
    surface's.
    */
    ...
    */
    ...
    bool SetDomain(int dir, // 0 sets first parameter's domain, 1 gets second
                            // parameter's domain
                   ON_Interval domain);

    virtual ON_BOOL32 SetDomain(int dir, // 0 sets first parameter's domain, 1
                                         // gets second parameter's domain
                                double t0, double t1);

    virtual ON_Interval Domain(int dir // 0 gets first parameter's domain, 1
                                       // gets second parameter's domain
    ) const = 0;

    virtual int SpanCount(int dir // 0 gets first parameter's domain, 1 gets
                                  // second parameter's domain
    ) const = 0; // number of smooth nonempty spans in the parameter direction

    virtual ON_BOOL32 GetSpanVector( // span "knots"
    ...
    */
    virtual ISO IsIsoparametric(const ON_Curve &curve,
    ...
    */
    bool IsSphere(ON_Sphere *sphere = NULL,
                  double tolerance = ON_ZERO_TOLERANCE) const;
    ...
    */
    bool IsCylinder(ON_Cylinder *cylinder = NULL,
                    double tolerance = ON_ZERO_TOLERANCE) const;
    ...
    */
    bool IsTorus(ON_Torus *torus = NULL,
                 double tolerance = ON_ZERO_TOLERANCE) const;

    virtual ON_BOOL32 IsClosed( // true if surface is closed in direction
        int                     // dir  0 = "s", 1 = "t"
    ) const;
    ...
    */
    ...
    virtual ON_BOOL32 Evaluate( // returns false if unable to evaluate
        double u, double v,     // evaluation parameters
        ...
        int quadrant =
            0,        // optional - determines which quadrant to evaluate from
                      //         0 = default
                      //         1 from NE quadrant
                      //         2 from NW quadrant
                      //         3 from SW quadrant
    ...
    */
    virtual ON_BOOL32 Split(int dir, double c, ON_Surface *&west_or_south_side,
    ...
    */
    ...
    // See Also:
    //   ON_Surface::GetNurbForm
    ON_NurbsSurface *NurbsSurface(ON_NurbsSurface *pNurbsSurface = NULL,
...
class ON_CLASS ON_SurfaceArray : public ON_SimpleArray<ON_Surface *> {
  public:
    ...
    ON_BOOL32 Duplicate(
        ON_SurfaceArray &) const; // operator= copies the pointer values
                                  // duplicate copies the surfaces themselves
};

ON_DECL
double ON_ClosestPointAngle(const ON_Line &, const ON_Curve &, ON_Interval,
                            const ON_3dPoint &, ON_3dPoint &, double *,
                            double *);

ON_DECL
double ON_GetFittingTolerance(const class ON_SurfaceTreeNode *snodeA,
