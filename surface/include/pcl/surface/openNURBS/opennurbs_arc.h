...
Details:
        The curve is parameterized by	an angle expressed in radians. For an
IsValid() arc the total subtended angle 	AngleRadians() = Domain()(1) -
Domain()(0) must satisfy 0< AngleRadians() <2*Pi .

        The parameterization of the ON_Arc is inherited from the ON_Circle it is
derived from. In particular t -> center + cos(t)*radius*xaxis +
sin(t)*radius*yaxis where xaxis and yaxis, (part of ON_Circle::m_plane) form an
othonormal frame of the plane containing the circle.
*/
class ON_CLASS ON_Arc : public ON_Circle {
  public:
    ...
    */
    ...
    ON_Arc(const ON_Plane &, // circle is in this plane with center at m_origin
           double,           // radius
           double            // angle in radians
    );
    ...
    bool Create(const ON_Circle &, // [IN]
    ...
    */
    ...
    bool Create(const ON_Plane
                    &,  // [IN] circle is in this plane with center at m_origin
                double, // [IN] radius
                double  // [IN] angle in radians
    );
    ...
    bool Create(const ON_3dPoint &, // [IN] center
                double,             // [IN] radius
                double              // [IN] angle in radians
    );
    ...
    bool Create(const ON_Plane &, // [IN] circle will be parallel to this plane
    ...
    //////////
    // Create an arc from a 2d start point, 2d start direction, and 2d end
    // point.
    bool Create(const ON_2dPoint &,  // [IN] point P
                const ON_2dVector &, // [IN] arc direction at P
                const ON_2dPoint &   // [IN] point R
    );

    //////////
    // Create an arc from a 3d start point, 3d start direction, and 3d end
    // point.
    bool Create(const ON_3dPoint &,  // [IN] point P
    ...
    */
    ...
    // Parameters:
    //   domain_radian - [in]   0 < domain_radian[1] - domain_radian[0] <= 2.0 *
    //   ON*PI
    //
    ...
    */
    bool GetRadianFromNurbFormParameter(double nurbs_parameter,
    ...
    See Also:
      ON_Arc::GetNurbFormParameterFromRadian
    */
    bool GetNurbFormParameterFromRadian(double arc_radians_parameter,
