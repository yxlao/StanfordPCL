...
class ON_CLASS ON_FileStream {
  public:
    ...
    */
    static bool GetFileInformation(FILE *fp, ON__UINT64 *file_size,
...
class ON_CLASS ON_FileIterator {
  public:
    ...
    ); 0 != filename; filename = fit.NextFile()
                )
            {
    ...
    */
    const wchar_t *FirstFile(const wchar_t *directory_name,
  ...
  private:
    ...
    char m_dirent_name_buffer[NAME_MAX + 1]; // < this field provide storage for
                                             // m_dirent.d_name[]
...
class ON_CLASS ON_Buffer {
  public:
    ...
    */
    ...
    Example:
          // silly example that compresses and then uncompresses a buffer in
    place
...
struct ON_3DM_BIG_CHUNK {
    ...
    ON__UINT8 m_bLongChunk; // true if chunk is a long chunk and m_big_value is
                            // a length.
...
// Used int ON_3dmProperties::Read() to set
// ON_BinaryArchive.m_3dm_opennurbs_version Do not call directly.
...
class ON_CLASS ON_BinaryArchive // use for generic serialization of binary data
{
  public:
    ...
    virtual size_t CurrentPosition( // current offset (in bytes) into archive (
                                    // like ftell() )
    ) const = 0;
    virtual bool SeekFromCurrentPosition( // seek from current position ( like
                                          // fseek( ,SEEK_CUR) )
        int // byte offset ( >= -CurrentPostion() )
        ) = 0;
    virtual bool
        SeekFromStart( // seek from current position ( like fseek( ,SEEK_SET) )
            size_t     // byte offset ( >= 0 )
            ) = 0;
    virtual bool AtEnd() const = 0; // true if at end of file
    ...
    */
    ...
    static char *ON_TypecodeParse(unsigned int tcode, char *typecode_name,
                                  size_t max_length);
    ...
    */
    ...
    bool WriteByte(size_t,
                   const void *); // must fail if mode is not write or readwrite
    ...
    */
    bool ReadCompressedBuffer(size_t sizeof__outbuffer, void *outbuffer,
                              int *bFailedCRC);
    ...
    */
    ...
    bool
        EnableSave3dmAnalysisMeshes(ON_BOOL32 = true); // returns previous state
    ...
    */
    ...
    bool Read3dmStartSection(int *, // returns version (1,2,3,4,5,50,...)
                             ON_String &);
    ...
    //
    int Read3dmGroup(ON_Group ** // ppGroup
    );
    ...
    //
    int Read3dmFont(ON_Font ** // ppFont
    );
    ...
    //
    int Read3dmDimStyle(ON_DimStyle ** // ppDimStyle
    );
    ...
    int Read3dmLight( // returns 0 at end of light table
        ...
        ON_3dmObjectAttributes
            * // optional - if NOT NULL, object attributes are
              //            returned here
    );
    ...
    ///////////////////////////////////////////////////////////////////
    // Step 14: REQUIRED - Write/Read instance definition table (it can be
    // empty)
    //
    ...
    /*
     ...
     Example:
       ...
       to BeginRead3dmInstanceDefinitionTable() /
    ...
    */
    ...
    ///////////////////////////////////////////////////////////////////
    // Step 15: REQUIRED - Write/Read geometry and annotation table (it can be
    // empty)
    //
    bool BeginWrite3dmObjectTable();
    bool Write3dmObject(const ON_Object &,
                        const ON_3dmObjectAttributes * // optional
    );
    ...
    int Read3dmObject( // returns 0 at end of object table
                       //         1 if object is read
                       //         2 if object is skipped because it does not
                       //         match filter
                       //        -1 if file is corrupt
    ...
    */
    bool BeginWrite3dmUserTable(const ON_UUID &plugin_id, bool bSavingGoo,
    ...
    */
    bool Write3dmAnonymousUserTableRecord(const ON_UUID &plugin_id,
                                          int goo_3dm_version,
    ...
    */
    bool BeginRead3dmUserTable(ON_UUID &plugin_id, bool *bLastSavedAsGoo,
    ...
    Description:
      ...
      If you do not need to resave the information, then simply call
    EndRead3dmUserTable() to skip over this table.
    */
    bool Read3dmAnonymousUserTable(int archive_3dm_version,
                                   int archive_opennurbs_version,
                                   ON_3dmGoo &goo);
    ...
    bool Read3dmEndMark(size_t * // sizeof_archive
    );
    ...
    // Returns:
    //   true if write was successful.
    bool BeginWrite3dmChunk(unsigned int, // typecode
                            int           // value
    );
    ...
    Parameters:
      tcode - [in] chunk's typecode from opennurbs_3dm.h.  This cannot be a
    short tcode. major_version - [in] ( >= 1) minor_version - [in] ( >= 0 )
    Returns:
    ...
    */
    bool BeginWrite3dmChunk(unsigned int tcode, int major_version,
                            int minor_version);
    ...
    bool BeginRead3dmBigChunk(unsigned int *, // typecode from opennurbs_3dm.h
                              ON__INT64 *     // value
    );
    /*
    Description:
    ...
    */
    bool BeginRead3dmChunk(unsigned int expected_tcode, int *major_version,
                           int *minor_version);

    /*
    Description:
      Calling this will skip rest of stuff in chunk if it was only partially
    read. Parameters: bSupressPartiallyReadChunkWarning - [in] Generally, a call
    to ON_WARNING is made when a chunk is partially read.  If
    bSupressPartiallyReadChunkWarning is true, then no warning is issued for
    ...
    */
    ...
    Remarks:
      ...
      or you may finish writing the dictionay by calling
        EndWriteDictionary();
    */
    bool BeginWriteDictionary(ON_UUID dictionary_id, unsigned int version,
    ...
    */
    ...
    bool BeginReadDictionary(ON_UUID *dictionary_id, unsigned int *version,
    ...
    */
    ...
    // in some other way.
    bool Write3dmChunkVersion(int, // major // 0 to 15
                              int  // minor // 0 to 16
    );
    bool Read3dmChunkVersion(int *, // major // 0 to 15
                             int *  // minor // 0 to 16
    );
    ...
    */
    bool FindTableInDamagedArchive(unsigned int tcode_table,
  ...
  private:
    ...
    bool BeginWrite3dmTable(unsigned int // tcode
    );
    bool EndWrite3dmTable(unsigned int // tcode
    );
    bool BeginRead3dmTable(unsigned int // tcode
    );
    bool EndRead3dmTable(unsigned int // tcode
    );
    ...
    bool Read3dmV1AttributesOrMaterial(ON_3dmObjectAttributes *, // attributes,
  ...
  public:
    ...
    bool ReadV1_TCODE_RHINOIO_OBJECT_NURBS_CURVE(ON_Object **,
                                                 ON_3dmObjectAttributes *);
    bool ReadV1_TCODE_RHINOIO_OBJECT_NURBS_SURFACE(ON_Object **,
                                                   ON_3dmObjectAttributes *);
    bool ReadV1_TCODE_RHINOIO_OBJECT_BREP(ON_Object **,
                                          ON_3dmObjectAttributes *);
    bool ReadV1_TCODE_ANNOTATION(unsigned int, ON_Object **,
                                 ON_3dmObjectAttributes *);
  ...
  private:
    // When a 3DM archive is read, m_3dm_opennurbs_version records the version
    // of OpenNURBS used to create the archive.  Otherwise,
    // m_3dm_opennurbs_version is zero.
    //
    ...
    bool FindMisplacedTable(ON__UINT64 filelength, const ON__UINT32 table_tocde,
  ...
  private:
    ...
    size_t WriteDeflate(size_t,      // sizeof uncompressed input data
                        const void * // uncompressed input data
    );
    bool ReadInflate(size_t, // sizeof uncompressed input data
  ...
  private:
    ...
    bool m_bSavePreviewImage; // true to save 200x200 preview bitmap (increases
                              // file size)
    bool m_bEmbedTextureBitmaps; // true to embed texture, bump, trace, and
                                 // wallpaper bitmaps (increases file size)
    bool m_bSaveRenderMeshes;    // true to save meshes used to render B-rep
                                 // objects (increases file size)
    bool m_bSaveAnalysisMeshes;  // true to save meshes used in surface analysis
                                 // (increases file size)
...
class ON_CLASS ON_BinaryFile : public ON_BinaryArchive {
  public:
    ...
    */
    ...
    // of 10 to 100.
    void EnableMemoryBuffer(int = 16384 // capacity of memory buffer
    );
...
class ON_CLASS ON_Read3dmBufferArchive : public ON_BinaryArchive {
  public:
    /*
    Description:
      Construct an ON_BinaryArchive for reading information from a memory
    buffer. Parameters: sizeof_buffer - [in] size of buffer in bytes (>0) buffer
    - [in] memory buffer containing binary archive bCopyBuffer - [in] true -
    copy the input buffer. Useful when the buffer may be destroyed while this
    class is still in use. false - Do not copy the input buffer. In this case
    you are responsible for making certain the input buffer is valid while this
    class is in use. archive_3dm_version  - [in] (1,2,3,4 or 5)
      archive_opennurbs_version - [in] YYYYMMDDn
    */
    ON_Read3dmBufferArchive(size_t sizeof_buffer, const void *buffer,
                            bool bCopyBuffer, int archive_3dm_version,
                            int archive_opennurbs_version);
  ...
  protected:
    // ON_BinaryArchive overrides
    size_t Read(size_t,
                void *); // return actual number of bytes read (like fread())
    size_t Write(size_t, const void *);
    bool Flush();
