...
class ON_CLASS ON_Light : public ON_Geometry {
  ...
  public:
    ...
    */
    ...
    Returns:
      true if successful.
    */
    ON_BOOL32 GetLightXform(const ON_Viewport &vp,
    ...
    double Attenuation(
        double) const; // computes 1/(a[0] + d*a[1] + d^2*a[2]) where d =
  ...
  public:
    ...
    ON_3dVector m_length;    // only for linear and rectangular lights
                             // ends of linear lights are m_location and
                             // m_location+m_length
    ...
    double m_intensity; // Linear dimming/brightening factor: 0.0 = off, 1.0 =
                        ...
                        // are not consistently interpreted by various
                        // renderers. Renderers should clamp the range to
                        // [0.0, 1.0] if their lighting model does not support
                        // more exotic interpretations of m_intensity.
    ...
    // shawdow casting
    double m_shadow_intensity; // 0.0 = no shadow casting, 1.0 = full shadow
                               // casting
};
