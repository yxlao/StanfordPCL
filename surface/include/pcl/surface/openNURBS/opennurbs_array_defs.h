...
template <class T> T *ON_SimpleArray<T>::Realloc(T *ptr, int capacity) {
...
template <class T> ON_SimpleArray<T>::~ON_SimpleArray() { SetCapacity(0); }
...
template <class T> void ON_SimpleArray<T>::EmergencyDestroy(void) {
    m_count = 0;
    m_capacity = 0;
    m_a = 0;
}
...
template <class T> int ON_SimpleArray<T>::Count() const { return m_count; }

template <class T> unsigned int ON_SimpleArray<T>::UnsignedCount() const {
    return ((unsigned int)m_count);
}

template <class T> int ON_SimpleArray<T>::Capacity() const {
    return m_capacity;
}

template <class T> unsigned int ON_SimpleArray<T>::SizeOfArray() const {
    return ((unsigned int)(m_capacity * sizeof(T)));
}

template <class T> unsigned int ON_SimpleArray<T>::SizeOfElement() const {
...
template <class T> T &ON_SimpleArray<T>::operator[](int i) {
#if defined(ON_DEBUG)
    if (i < 0 || i > m_capacity) {
...
template <class T> T &ON_SimpleArray<T>::operator[](unsigned int i) {
#if defined(ON_DEBUG)
...
template <class T> T &ON_SimpleArray<T>::operator[](ON__INT64 i) {
#if defined(ON_DEBUG)
...
template <class T> T &ON_SimpleArray<T>::operator[](ON__UINT64 i) {
#if defined(ON_DEBUG)
...
template <class T> const T &ON_SimpleArray<T>::operator[](int i) const {
#if defined(ON_DEBUG)
    if (i < 0 || i > m_capacity) {
        ON_ERROR("ON_SimpleArray[i]: i out of range.");
    }
...
template <class T> const T &ON_SimpleArray<T>::operator[](ON__INT64 i) const {
#if defined(ON_DEBUG)
    if (i < 0 || i > ((ON__INT64)m_capacity)) {
        ON_ERROR("ON_SimpleArray[i]: i out of range.");
    }
...
template <class T> const T &ON_SimpleArray<T>::operator[](ON__UINT64 i) const {
#if defined(ON_DEBUG)
    if (i > (ON__UINT64)m_capacity) {
        ON_ERROR("ON_SimpleArray[i]: i out of range.");
    }
...
template <class T> ON_SimpleArray<T>::operator const T *() const {
    return (m_count > 0) ? m_a : 0;
}

template <class T> T *ON_SimpleArray<T>::Array() { return m_a; }

template <class T> const T *ON_SimpleArray<T>::Array() const { return m_a; }

template <class T> T *ON_SimpleArray<T>::KeepArray() {
    T *p = m_a;
    m_a = 0;
    m_count = 0;
    m_capacity = 0;
...
template <class T> void ON_SimpleArray<T>::SetArray(T *p) {
    if (m_a && m_a != p)
        onfree(m_a);
    m_a = p;
}
...
template <class T> T *ON_SimpleArray<T>::First() {
    return (m_count > 0) ? m_a : 0;
}

template <class T> const T *ON_SimpleArray<T>::First() const {
    return (m_count > 0) ? m_a : 0;
}

template <class T> T *ON_SimpleArray<T>::At(int i) {
    return (i >= 0 && i < m_count) ? m_a + i : 0;
}

template <class T> T *ON_SimpleArray<T>::At(unsigned int i) {
    return (i < (unsigned int)m_count) ? m_a + i : 0;
}

template <class T> const T *ON_SimpleArray<T>::At(int i) const {
    return (i >= 0 && i < m_count) ? m_a + i : 0;
}

template <class T> const T *ON_SimpleArray<T>::At(unsigned int i) const {
    return (i < (unsigned int)m_count) ? m_a + i : 0;
}

template <class T> T *ON_SimpleArray<T>::At(ON__INT64 i) {
    return (i >= 0 && i < (ON__INT64)m_count) ? m_a + i : 0;
}

template <class T> T *ON_SimpleArray<T>::At(ON__UINT64 i) {
    return (i < (ON__UINT64)m_count) ? m_a + i : 0;
}

template <class T> const T *ON_SimpleArray<T>::At(ON__INT64 i) const {
    return (i >= 0 && i < (ON__INT64)m_count) ? m_a + i : 0;
}

template <class T> const T *ON_SimpleArray<T>::At(ON__UINT64 i) const {
    return (i < (ON__UINT64)m_count) ? m_a + i : 0;
}

template <class T> T *ON_SimpleArray<T>::Last() {
    return (m_count > 0) ? m_a + (m_count - 1) : 0;
}

template <class T> const T *ON_SimpleArray<T>::Last() const {
    return (m_count > 0) ? m_a + (m_count - 1) : 0;
}
...
template <class T> T &ON_SimpleArray<T>::AppendNew() {
...
template <class T> void ON_SimpleArray<T>::Append(const T &x) {
    if (m_count == m_capacity) {
        ...
        if (m_a) {
            ...
            if (s >= 0 && s < m_capacity) {
                ...
                T temp;   // ON_*Array<> templates do not require robust copy
                          // constructor.
...
template <class T> void ON_SimpleArray<T>::Append(int count, const T *p) {
    if (count > 0 && p) {
...
template <class T> void ON_SimpleArray<T>::Insert(int i, const T &x) {
    if (i >= 0 && i <= m_count) {
        if (m_count == m_capacity) {
            int newcapacity = NewCapacity();
            Reserve(newcapacity);
...
template <class T> void ON_SimpleArray<T>::Remove() { Remove(m_count - 1); }

template <class T> void ON_SimpleArray<T>::Remove(int i) {
    if (i >= 0 && i < m_count) {
...
template <class T> void ON_SimpleArray<T>::Empty() {
    if (m_a)
        memset(m_a, 0, m_capacity * sizeof(T));
    m_count = 0;
}

template <class T> void ON_SimpleArray<T>::Reverse() {
    // NOTE:
...
template <class T> void ON_SimpleArray<T>::Swap(int i, int j) {
    if (i != j) {
        const T t(m_a[i]);
        m_a[i] = m_a[j];
        m_a[j] = t;
...
template <class T> int ON_SimpleArray<T>::Search(const T &key) const {
    const T *p = &key;
...
template <class T>
bool ON_SimpleArray<T>::Sort(ON::sort_algorithm sa, int *index,
                             int (*compar)(const T *, const T *)) const {
    bool rc = false;
...
template <class T> bool ON_SimpleArray<T>::Permute(const int *index) {
    bool rc = false;
...
template <class T> void ON_SimpleArray<T>::Zero() {
    if (m_a && m_capacity > 0) {
        memset(m_a, 0, m_capacity * sizeof(T));
    }
}

template <class T> void ON_SimpleArray<T>::MemSet(unsigned char value) {
    if (m_a && m_capacity > 0) {
...
template <class T> void ON_SimpleArray<T>::Reserve(int newcap) {
    if (m_capacity < newcap)
        SetCapacity(newcap);
}

template <class T> void ON_SimpleArray<T>::Shrink() { SetCapacity(m_count); }

template <class T> void ON_SimpleArray<T>::Destroy() { SetCapacity(0); }

// low level memory managment //////////////////////////////////////////

template <class T> void ON_SimpleArray<T>::SetCount(int count) {
    if (count >= 0 && count <= m_capacity)
        m_count = count;
}

template <class T> void ON_SimpleArray<T>::SetCapacity(int capacity) {
    ...
    if (capacity != m_capacity) {
        if (capacity > 0) {
            ...
            if (m_a) {
                if (capacity > m_capacity) {
                    // zero new memory
                    memset(m_a + m_capacity, 0,
                           (capacity - m_capacity) * sizeof(T));
                }
                m_capacity = capacity;
            } else {
                // out of memory
...
template <class T> int ON_SimpleArray<T>::NewCapacity() const {
    // Note:
...
template <class T> int ON_ClassArray<T>::NewCapacity() const {
    // Note:
    //   This code appears in ON_SimpleArray<T>::NewCapacity()
    //   and ON_ClassArray<T>::NewCapacity().  Changes made to
    //   either function should be made to both functions.
...
template <class T> ON_ObjectArray<T>::ON_ObjectArray() {}
...
template <class T> T *ON_ObjectArray<T>::Realloc(T *ptr, int capacity) {
...
template <class T> T *ON_ClassArray<T>::Realloc(T *ptr, int capacity) {
    return (T *)onrealloc(ptr, capacity * sizeof(T));
}
...
template <class T> ON_ClassArray<T>::~ON_ClassArray() { SetCapacity(0); }
...
template <class T> void ON_ClassArray<T>::EmergencyDestroy(void) {
    m_count = 0;
    m_capacity = 0;
    m_a = 0;
}
...
template <class T> int ON_ClassArray<T>::Count() const { return m_count; }

template <class T> unsigned int ON_ClassArray<T>::UnsignedCount() const {
    return ((unsigned int)m_count);
}

template <class T> int ON_ClassArray<T>::Capacity() const { return m_capacity; }

template <class T> unsigned int ON_ClassArray<T>::SizeOfArray() const {
    return ((unsigned int)(m_capacity * sizeof(T)));
}

template <class T> unsigned int ON_ClassArray<T>::SizeOfElement() const {
    return ((unsigned int)(sizeof(T)));
}

template <class T> T &ON_ClassArray<T>::operator[](int i) {
#if defined(ON_DEBUG)
    if (i < 0 || i > m_capacity) {
...
template <class T> T &ON_ClassArray<T>::operator[](ON__INT64 i) {
#if defined(ON_DEBUG)
    if (i < 0 || i > (ON__INT64)m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> T &ON_ClassArray<T>::operator[](unsigned int i) {
#if defined(ON_DEBUG)
    if (i > (unsigned int)m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> T &ON_ClassArray<T>::operator[](ON__UINT64 i) {
#if defined(ON_DEBUG)
    if (i > (ON__UINT64)m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> const T &ON_ClassArray<T>::operator[](int i) const {
#if defined(ON_DEBUG)
    if (i < 0 || i > m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> const T &ON_ClassArray<T>::operator[](ON__INT64 i) const {
#if defined(ON_DEBUG)
    if (i < 0 || i > (ON__INT64)m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> const T &ON_ClassArray<T>::operator[](unsigned int i) const {
#if defined(ON_DEBUG)
    if (i > (unsigned int)m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> const T &ON_ClassArray<T>::operator[](ON__UINT64 i) const {
#if defined(ON_DEBUG)
    if (i > (ON__UINT64)m_capacity) {
        ON_ERROR("ON_ClassArray[i]: i out of range.");
    }
...
template <class T> ON_ClassArray<T>::operator T *() {
    return (m_count > 0) ? m_a : 0;
}

template <class T> ON_ClassArray<T>::operator const T *() const {
    return (m_count > 0) ? m_a : 0;
}

template <class T> T *ON_ClassArray<T>::Array() { return m_a; }

template <class T> const T *ON_ClassArray<T>::Array() const { return m_a; }

template <class T> T *ON_ClassArray<T>::KeepArray() {
    T *p = m_a;
    m_a = 0;
    m_count = 0;
    m_capacity = 0;
...
template <class T> void ON_ClassArray<T>::SetArray(T *p) {
    if (m_a && m_a != p)
        Destroy();
    m_a = p;
}
...
template <class T> T *ON_ClassArray<T>::First() {
    return (m_count > 0) ? m_a : 0;
}

template <class T> const T *ON_ClassArray<T>::First() const {
    return (m_count > 0) ? m_a : 0;
}

template <class T> T *ON_ClassArray<T>::At(int i) {
    return (i >= 0 && i < m_count) ? m_a + i : 0;
}

template <class T> T *ON_ClassArray<T>::At(unsigned int i) {
    return (i < (unsigned int)m_count) ? m_a + i : 0;
}

template <class T> const T *ON_ClassArray<T>::At(int i) const {
    return (i >= 0 && i < m_count) ? m_a + i : 0;
}

template <class T> const T *ON_ClassArray<T>::At(unsigned int i) const {
    return (i < (unsigned int)m_count) ? m_a + i : 0;
}

template <class T> T *ON_ClassArray<T>::At(ON__INT64 i) {
    return (i >= 0 && i < (ON__INT64)m_count) ? m_a + i : 0;
}

template <class T> T *ON_ClassArray<T>::At(ON__UINT64 i) {
    return (i < (ON__UINT64)m_count) ? m_a + i : 0;
}

template <class T> const T *ON_ClassArray<T>::At(ON__INT64 i) const {
    return (i >= 0 && i < (ON__INT64)m_count) ? m_a + i : 0;
}

template <class T> const T *ON_ClassArray<T>::At(ON__UINT64 i) const {
    return (i < (ON__UINT64)m_count) ? m_a + i : 0;
}

template <class T> T *ON_ClassArray<T>::Last() {
    return (m_count > 0) ? m_a + (m_count - 1) : 0;
}

template <class T> const T *ON_ClassArray<T>::Last() const {
    return (m_count > 0) ? m_a + (m_count - 1) : 0;
}
...
template <class T> void ON_ClassArray<T>::ConstructDefaultElement(T *p) {
    // use placement ( new(size_t,void*) ) to construct
    // T in supplied memory
    new (p) T;
}

template <class T> void ON_ClassArray<T>::DestroyElement(T &x) { x.~T(); }

template <class T> T &ON_ClassArray<T>::AppendNew() {
    if (m_count == m_capacity) {
        int newcapacity = NewCapacity();
        Reserve(newcapacity);
    } else {
...
template <class T> void ON_ClassArray<T>::Append(const T &x) {
    if (m_count == m_capacity) {
        const int newcapacity = NewCapacity();
        if (m_a) {
            const int s = (int)(&x - m_a); // (int) cast is for 64 bit pointers
...
template <class T> void ON_ClassArray<T>::Append(int count, const T *p) {
    int i;
    if (count > 0 && p) {
        if (count + m_count > m_capacity) {
            int newcapacity = NewCapacity();
...
template <class T> void ON_ClassArray<T>::Insert(int i, const T &x) {
    if (i >= 0 && i <= m_count) {
        if (m_count == m_capacity) {
            int newcapacity = NewCapacity();
            Reserve(newcapacity);
...
template <class T> void ON_ClassArray<T>::Remove() { Remove(m_count - 1); }

template <class T> void ON_ClassArray<T>::Remove(int i) {
    if (i >= 0 && i < m_count) {
...
template <class T> void ON_ClassArray<T>::Empty() {
    int i;
...
template <class T> void ON_ClassArray<T>::Reverse() {
    // NOTE:
    // If anything in "T" depends on the value of this's address,
    // then don't call Reverse().
    char t[sizeof(T)];
...
template <class T> void ON_ClassArray<T>::Swap(int i, int j) {
...
int ON_ClassArray<T>::BinarySearch(const T *key,
    ...
    return found
               ? ((int)((((ON__UINT64)found) - ((ON__UINT64)m_a)) / sizeof(T)))
               : -1;
#endif
}
...
bool ON_ObjectArray<T>::HeapSort(int (*compar)(const T *, const T *)) {
    ...
    if (this->m_a && this->m_count > 0 && compar) {
        if (this->m_count > 1) {
            ...
            // The MemoryRelocate step is required to synch userdata back
            // pointers so the user data destructor will work correctly.
            int i;
            for (i = 0; i < this->m_count; i++) {
...
template <class T>
bool ON_ClassArray<T>::Sort(ON::sort_algorithm sa, int *index,
                            int (*compar)(const T *, const T *)) const {
    bool rc = false;
    if (m_a && m_count > 0 && compar && index) {
        if (m_count > 1)
...
template <class T> bool ON_ClassArray<T>::Permute(const int *index) {
    bool rc = false;
    if (m_a && m_count > 0 && index) {
        int i;
        T *buffer = (T *)onmalloc(m_count * sizeof(buffer[0]));
...
template <class T> void ON_ClassArray<T>::Zero() {
    int i;
    if (m_a && m_capacity > 0) {
...
template <class T> void ON_ClassArray<T>::Reserve(int newcap) {
    if (m_capacity < newcap)
        SetCapacity(newcap);
}

template <class T> void ON_ClassArray<T>::Shrink() { SetCapacity(m_count); }

template <class T> void ON_ClassArray<T>::Destroy() { SetCapacity(0); }

// low level memory managment //////////////////////////////////////////

template <class T> void ON_ClassArray<T>::SetCount(int count) {
    if (count >= 0 && count <= m_capacity)
        m_count = count;
}

template <class T> void ON_ClassArray<T>::SetCapacity(int capacity) {
...
template <class T> static int ON_CompareIncreasing(const T *a, const T *b) {
    if (*a < *b)
        return -1;
    if (*b < *a)
        return 1;
...
template <class T> static int ON_CompareDecreasing(const T *a, const T *b) {
    if (*b < *a)
        return -1;
    if (*a < *b)
        return 1;
