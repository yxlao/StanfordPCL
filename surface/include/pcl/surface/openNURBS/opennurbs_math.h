...
class ON_CLASS ON_Evaluator {
  public:
    ...
    */
    ON_Evaluator(int parameter_count, int value_count,
    ...
    */
    virtual int Evaluate(const double *parameters, double *values,
                         double **jacobian) = 0;
    ...
    Returns:
     -1 = Hessian evaluation not available.
      ...
      2 = found answer, terminate search
    */
    virtual int EvaluateHessian(const double *parameters, double *value,
  ...
  private:
    ...
    ON_Evaluator &operator=(
        const ON_Evaluator &); // prohibit operator= (can't copy const members)
};
...
// TODO - ADD FAST ugly bit check#define ON_IS_VALID_FLOAT(x)  (x !=
// ON_UNSET_FLOAT && 0x7FF0 != (*((unsigned short*)(&x) + 3) & 0x7FF0))
...
ON_DECL
void ON_ArrayScale(int,           // size of arrays (can be zero)
                   float,         // a
                   const float *, // A[]
                   float *        // returns a*A[]
);
...
ON_DECL
void ON_Array_aA_plus_B(int,           // size of arrays (can be zero)
                        float,         // a
                        const float *, // A[]
                        const float *, // B[]
                        float *        // returns a*A[] + B[]
...
*/
ON_DECL
int ON_IsPointListPlanar(bool bRational, int count, int stride,
                         const double *points, const double *boxMin,
                         const double *boxMax, double tolerance,
...
ON_DECL
ON_BOOL32 ON_SwapPointGridCoordinates(int, int, // point_count0, point_count1,
                                      int, int, // point_stride0, point_stride1,
                                      double *, int, int // coordinates to swap
);
...
// is returned
ON_DECL
int ON_Solve2x2(double, double, // a00 a01 = first row of 2x2 matrix
...
//         = (d0,d1,d2).
ON_DECL
int ON_Solve3x2(const double[3], // col0
                const double[3], // col1
                double,          // d0
                double,          // d1
                double,          // d2
...
*/
ON_DECL
ON_BOOL32 ON_EvNormalPartials(const ON_3dVector &ds, const ON_3dVector &dt,
                              const ON_3dVector &dss, const ON_3dVector &dst,
                              const ON_3dVector &dtt, ON_3dVector &ns,
                              ON_3dVector &nt);
...
ON_DECL
bool ON_EvTangent(const ON_3dVector &, // first derivative
...
ON_DECL
ON_BOOL32 ON_EvCurvature(const ON_3dVector &, // first derivative
                         const ON_3dVector &, // second derivative
                         ON_3dVector &,       // Unit tangent returned here
...
ON_BOOL32 ON_EvPrincipalCurvatures(
    ...
    double
        *, // kappa1, // = largest principal curvature value (may be negative)
    double
        *, // kappa2, // = smallest principal curvature value (may be negative)
...
ON_DECL
ON_BOOL32
ON_IsContinuous(ON::continuity c, ON_3dPoint Pa, ON_3dVector D1a,
                ON_3dVector D2a, ON_3dPoint Pb, ON_3dVector D1b,
                ON_3dVector D2b, double point_tolerance = ON_ZERO_TOLERANCE,
...
*/
ON_DECL
void ON_Sort(ON::sort_algorithm method, int *index, const void *base,
...
*/
ON_DECL
void ON_hsort(void *base, size_t count, size_t sizeof_element,
              int (*compare)(const void *, const void *));

ON_DECL
void ON_qsort(void *base, size_t count, size_t sizeof_element,
              int (*compare)(const void *, const void *));
...
*/
ON_DECL
void ON_SortDoubleArray(ON::sort_algorithm sort_algorithm, double *a,
                        size_t nel);
...
*/
ON_DECL
void ON_SortUnsignedIntArray(ON::sort_algorithm sort_algorithm, unsigned int *a,
                             size_t nel);
...
*/
ON_DECL
void ON_SortStringArray(ON::sort_algorithm sort_algorithm, char **a,
                        size_t nel);
...
ON_DECL
const ON_2dex *ON_BinarySearch2dexArray(int key_i, const ON_2dex *base,
                                        size_t nel);
...
ON_DECL
bool ON_Intersect(const ON_Line &, const ON_Line &,
...
ON_DECL
bool ON_Intersect(const ON_Line &, const ON_Plane &,
                  double * // parameter on line
);

ON_DECL
bool ON_Intersect(const ON_Plane &, const ON_Plane &,
                  ON_Line & // intersection line is returned here
);

ON_DECL
bool ON_Intersect(const ON_Plane &, const ON_Plane &, const ON_Plane &,
...
ON_DECL
int ON_Intersect(const ON_Plane &, const ON_Sphere &, ON_Circle &);
...
// the sphere.
ON_DECL
int ON_Intersect(const ON_Line &, const ON_Sphere &, ON_3dPoint &,
...
ON_DECL
int ON_Intersect(const ON_Line &,     // [in]
...
ON_DECL
int ON_Intersect(const ON_Line &line, const ON_Circle &circle, double *line_t0,
                 ON_3dPoint &circle_point0, double *line_t1,
...
//         and line.PointAt(*line_t1).
ON_DECL
int ON_Intersect(const ON_Line &line, const ON_Arc &arc, double *line_t0,
                 ON_3dPoint &arc_point0, double *line_t1,
...
ON_DECL
int ON_Intersect(const ON_Plane &plane, const ON_Circle &circle,
                 ON_3dPoint &point0, ON_3dPoint &point1);
...
//	 3		 Arc lies on plane
ON_DECL
int ON_Intersect(const ON_Plane &plane, const ON_Arc &arc, ON_3dPoint &point0,
                 ON_3dPoint &point1);
...
int ON_ArePointsOnLine(
    ...
    const ON_BoundingBox
        &, // if needed, use ON_GetBoundingBox(dim,is_rat,count,stride,point)
    const ON_Line &,
...
bool ON_GetClosestPointInPointList(int point_count,
                                   const ON_3dPoint *point_list, ON_3dPoint P,
...
*/
ON_DECL bool
ON_GetConicEquationThrough6Points(int stride, const double *points2d,
...
Returns:
  ...
  are not changed.
*/
ON_DECL bool ON_GetEllipseConicEquation(double a, double b, double x0,
                                        double y0, double alpha,
                                        double conic[6]);
