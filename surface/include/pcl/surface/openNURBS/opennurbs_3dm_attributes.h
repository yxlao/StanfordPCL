...
class ON_CLASS ON_3dmObjectAttributes : public ON_Object {
  ...
  public:
    ...
    */
    ...
    // The Linetype used to display an OpenNURBS object is specified in one of
    // two ways. If LinetypeSource() is ON::linetype_from_layer, then the
    // object's layer ON_Layer::Linetype() is used. If LinetypeSource() is
    // ON::linetype_from_object, then value of m_linetype is used.
    ON::object_linetype_source LinetypeSource() const;
    void SetLinetypeSource(ON::object_linetype_source); // See LinetypeSource().

    // The color used to display an OpenNURBS object is specified in one of
    // three ways. If ColorSource() is ON::color_from_layer, then the object's
    // layer ON_Layer::Color() is used. If ColorSource() is
    // ON::color_from_object, then value of m_color is used. If ColorSource() is
    // ON::color_from_material, then the diffuse color of the object's render
    // material is used.  See ON_3dmObjectAttributes::MaterialSource() to
    ...
    // If PlotColorSource() is ON::plot_color_from_layer, then the object's
    // layer ON_Layer::PlotColor() is used. If PlotColorSource() is
    // ON::plot_color_from_object, then value of PlotColor() is used.
    ...
    */
    ...
    // Linetype definitions in an OpenNURBS model are stored in a linetype
    // table. The linetype table is conceptually an array of ON_Linetype
    // classes.  Every OpenNURBS object in a model references some linetype. The
    // object's linetype is specified by zero based indicies into the
    // ON_Linetype array. index 0 is reserved for continuous linetype (no
    // pattern)
    ...
    */
    ...
    // If ON::plot_color_from_object == PlotColorSource(), then m_color is the
    // object's display color.
    ON_Color m_plot_color;

    // Display order used to force objects to be drawn on top or behind each
    // other 0  = draw object in standard depth buffered order <0 = draw object
    // behind "normal" draw order objects >0 = draw object on top of "noraml"
    // draw order objects Larger number draws on top of smaller number.
    int m_display_order;

    // Plot weight in millimeters.
    //   =0.0 means use the default width
    //   <0.0 means don't plot (visible for screen display, but does not show on
    //   plot)
    double m_plot_weight_mm;

    // Used to indicate an object has a decoration (like an arrowhead on a
    // curve)
  ...
  public:
    ...
    ON_BOOL32 IsInGroup(int // zero based group index
    ) const;

    // Returns true if the object is in any of the groups in the list
    ON_BOOL32 IsInGroups(int,        // group_list_count
                         const int * // group_list[] array
    ) const;

    // Returns true if object is in any of the groups in the list
    ON_BOOL32 IsInGroups(const ON_SimpleArray<int> & // group_list[] array
    ) const;
    ...
    */
    bool FindDisplayMaterialId(const ON_UUID &viewport_id,
    ...
    */
    ...
    See Also:
      ...
      ON_3dmObjectAttributes::AddDisplayMaterialRef
    */
    bool RemoveDisplayMaterialRef(ON_UUID viewport_id,
                                  ON_UUID display_material_id = ON_nil_uuid);
