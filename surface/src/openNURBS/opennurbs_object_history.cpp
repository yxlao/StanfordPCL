...
int ON_Value::GetPolyEdgePointers(ON_ClassArray<ON_PolyEdgeHistory> &) const {
    return 0;
}
...
// virtual
int ON_BoolValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_IntValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_DoubleValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_PointValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_VectorValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_XformValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_ColorValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_UuidValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_StringValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_ObjRefValue::Count() const { return m_value.Count(); }
...
///////////////////////////////////////////////////////////////////////
//
// ON_GeometryValue saves geometry values in the ON_HistoryRecord::m_value[]
// array
//
...
// virtual
int ON_GeometryValue::Count() const { return m_value.Count(); }
...
///////////////////////////////////////////////////////////////////////
//
// ON_PolyEdgeHistoryValue saves geometry values in the
// ON_HistoryRecord::m_value[] array
//
...
// ON_PolyEdgeHistoryValue::ON_PolyEdgeHistoryValue(const
// ON_PolyEdgeHistoryValue& src) : ON_Value(src)
//{
//  *this = src;
//}
//
...
// virtual
int ON_PolyEdgeHistoryValue::Count() const { return m_value.Count(); }
...
// virtual
int ON_PolyEdgeHistoryValue::GetPolyEdgePointers(
...
void ON_HistoryRecord::CopyHelper(const ON_HistoryRecord &src) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (src_v) {
            ...
            if (v) {
                m_value.Append(v);
                if (m_bValuesSorted && prev_v &&
                    prev_v->m_value_id > v->m_value_id)
...
ON_BOOL32 ON_HistoryRecord::IsValid(ON_TextLog *text_log) const { return true; }
...
ON_Value *ON_HistoryRecord::FindValueHelper(int value_id, int value_type,
...
bool ON_HistoryRecord::SetIntValues(int value_id, int count, const int *i) {
    ON_IntValue *v = static_cast<ON_IntValue *>(
        FindValueHelper(value_id, ON_Value::int_value, true));
    if (v) {
        v->m_value.SetCount(0);
        v->m_value.SetCapacity(count);
...
bool ON_HistoryRecord::SetDoubleValues(int value_id, int count,
                                       const double *x) {
...
bool ON_HistoryRecord::SetPointValues(int value_id, int count,
                                      const ON_3dPoint *P) {
...
bool ON_HistoryRecord::SetVectorValues(int value_id, int count,
                                       const ON_3dVector *V) {
...
bool ON_HistoryRecord::SetXformValues(int value_id, int count,
...
bool ON_HistoryRecord::SetColorValues(int value_id, int count,
                                      const ON_Color *c) {
...
bool ON_HistoryRecord::SetUuidValues(int value_id, int count,
                                     const ON_UUID *u) {
...
bool ON_HistoryRecord::SetStringValues(int value_id, int count,
...
bool ON_HistoryRecord::SetObjRefValues(int value_id, int count,
                                       const ON_ObjRef *oref) {
...
bool ON_HistoryRecord::SetGeometryValues(
    int value_id, const ON_SimpleArray<ON_Geometry *> a) {
...
bool ON_HistoryRecord::SetPolyEdgeValues(int value_id, int count,
...
bool ON_HistoryRecord::GetBoolValue(int value_id, bool *b) const {
    bool rc = false;
    const ON_BoolValue *v = static_cast<ON_BoolValue *>(
        FindValueHelper(value_id, ON_Value::bool_value, 0));
...
bool ON_HistoryRecord::GetIntValue(int value_id, int *i) const {
    bool rc = false;
    const ON_IntValue *v = static_cast<ON_IntValue *>(
        FindValueHelper(value_id, ON_Value::int_value, 0));
    if (v && 1 == v->m_value.Count()) {
        *i = v->m_value[0];
        rc = true;
    }
...
bool ON_HistoryRecord::GetPolyEdgeValue(
    int value_id, const ON_PolyEdgeHistory *&polyedge) const {
    bool rc = false;
    polyedge = 0;
...
bool ON_HistoryRecord::GetUuidValue(int value_id, ON_UUID *uuid) const {
    bool rc = false;
    const ON_UuidValue *v = static_cast<ON_UuidValue *>(
        FindValueHelper(value_id, ON_Value::uuid_value, 0));
    if (v && 1 == v->m_value.Count()) {
        *uuid = v->m_value[0];
        rc = true;
    }
...
int ON_HistoryRecord::GetStringValues(int value_id,
                                      ON_ClassArray<ON_wString> &a) const {
    a.SetCount(0);
...
int ON_HistoryRecord::GetObjRefValues(int value_id,
                                      ON_ClassArray<ON_ObjRef> &a) const {
    a.SetCount(0);
    const ON_ObjRefValue *v = static_cast<ON_ObjRefValue *>(
        FindValueHelper(value_id, ON_Value::objref_value, 0));
    if (v) {
...
int ON_HistoryRecord::GetGeometryValues(
    int value_id, ON_SimpleArray<const ON_Geometry *> &a) const {
    a.SetCount(0);
    const ON_GeometryValue *v = static_cast<ON_GeometryValue *>(
        FindValueHelper(value_id, ON_Value::geometry_value, 0));
    if (v) {
...
int ON_HistoryRecord::GetPolyEdgeValues(
    int value_id, ON_SimpleArray<const ON_PolyEdgeHistory *> &a) const {
    a.SetCount(0);
    const ON_PolyEdgeHistoryValue *v = static_cast<ON_PolyEdgeHistoryValue *>(
        FindValueHelper(value_id, ON_Value::polyedge_value, 0));
    if (v) {
...
ON_BOOL32 ON_HistoryRecord::Read(ON_BinaryArchive &archive) {
    ...
    for (;;) {
        ...
        if (rc) {
            ...
            for (i = 0; i < count && rc; i++) {
                int mjv = 0, mnv = 0;
                rc = archive.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &mjv,
                                               &mnv);
                if (!rc)
                    break;
                for (;;) {
        ...
        if (rc) {
            ...
            for (i = 0; i < count && rc; i++) {
                ...
                for (;;) {
                    ...
                    rc = archive.WriteInt(value ? value->m_value_type
...
ON_UUID ON_HistoryRecord::ModelObjectId() const { return m_record_id; }

void ON_HistoryRecord::RemapObjectIds(
    const ON_SimpleArray<ON_UuidPair> &id_remap) {
    if (id_remap.Count() > 0) {
        int i, j;
...
bool ON_CurveProxyHistory::Read(ON_BinaryArchive &file) {
    ...
    if (!file.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &version_major,
                                &version_minor))
        return false;
