...
void ON_qsort(void *base, size_t nel, size_t width,
...
void ON_hsort(void *base, size_t nel, size_t width,
              int (*compar)(const void *, const void *)) {
    size_t i_end, k;
    ...
    e_tmp =
        (width > work_size) ? (unsigned char *)onmalloc(width) : work_memory;
    for (;;) {
        if (k) {
            --k;
            memcpy(e_tmp, ((unsigned char *)base) + k * width,
                   width); /* e_tmp = e[k]; */
        } else {
        ...
        }
        {
            ...
            while (j <= i_end) {
                ...
                if (j < i_end &&
                    compar(e_j, e_j + width) < 0 /*e[j] < e[j + 1] */) {
                    j++;
                    e_j += width;
                }
...
void ON_hsort(void *base, size_t nel, size_t width,
    ...
    for (;;) {
        ...
        }
        {
            ...
            while (j <= i_end) {
                e_j = ((unsigned char *)base) + j * width;
                if (j < i_end && compar(context, e_j, e_j + width) <
                                     0 /*e[j] < e[j + 1] */) {
                    j++;
                    e_j += width;
                }
...
void ON_SortFloatArray(ON::sort_algorithm sort_algorithm, float *a,
                       size_t nel) {
    if (ON::heap_sort == sort_algorithm)
