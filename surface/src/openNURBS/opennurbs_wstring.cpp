...
static int w2c_size(int,
                    const wchar_t *); // gets minimum "c_count" arg for w2c().
...
static int w2c_size(int w_count, const wchar_t *w) {
    ...
    if (w) {
        ...
        rc = ON_ConvertWideCharToUTF8(false, w, w_count, 0, 0, &error_status, 0,
                                      0, 0);
        if (error_status) {
...
static int w2c(int w_count, const wchar_t *w, int c_count,
    ...
    if (c_count > 0 && c) {
        ...
        if (w) {
            ...
            const wchar_t *p1 = 0;
            rc = ON_ConvertWideCharToUTF8(false, w, w_count, c, c_count,
                                          &error_status, error_mask,
                                          error_code_point, &p1);
            if (error_status) {
                ON_ERROR("Error converting UTF-16 encoded wchar_t string to "
                         "UTF-8 encoded char string.");
            }
            if (rc > 0 && rc <= c_count)
                c[rc] = 0;
            else {
...
static int c2w(int c_count, const char *c, int w_count,
    ...
    if (w_count > 0 && w && c_count > 0 && c && c[0]) {
        ...
        if (c) {
            ...
            ON__UINT32 error_code_point = 0xFFFD;
            const char *p1 = 0;
            rc = ON_ConvertUTF8ToWideChar(c, c_count, w, w_count, &error_status,
            ...
            if (0 != error_status) {
                ON_ERROR("Error converting UTF-8 encoded char string to UTF-16 "
                         "encoded wchar_t string.");
            }
        }
    }
    return rc;
...
void ON_wString::Create() { m_s = (wchar_t *)pEmptywString; }
...
void ON_wString::CreateArray(int capacity) {
    Destroy();
    if (capacity > 0) {
        ON_wStringHeader *p = (ON_wStringHeader *)onmalloc(
            sizeof(ON_wStringHeader) + (capacity + 1) * sizeof(*m_s));
        p->ref_count = 1;
        p->string_length = 0;
...
void ON_wString::EmergencyDestroy() { Create(); }

void ON_wString::EnableReferenceCounting(bool bEnable) {
    // TODO fill this in;
}

bool ON_wString::IsReferenceCounted() const { return true; }
...
void ON_wString::ReserveArray(size_t array_capacity) {
    ...
    if (p == pEmptyStringHeader) {
        ...
        const int size =
            (capacity < p->string_length) ? capacity : p->string_length;
        if (size > 0) {
...
int ON_wString::Length(const char *s) {
    ...
    int n = ((0 < slen && slen <= 2147483645)
                 ? ((int)slen)
                 : 0); // the (int) cast is for 64 bit size_t conversion
    return n;
}
...
ON_wString::ON_wString(const char *s) {
    Create();
    if (s && s[0]) {
        CopyToArray((int)strlen(s),
                    s); // the (int) is for 64 bit size_t conversion
    }
}
...
ON_wString::ON_wString(const unsigned char *s) {
    ...
    if (s && s[0]) {
        CopyToArray(
            ...
            (const char *)s); // the (int) is for 64 bit size_t conversion
    }
}
...
int ON_wString::Length() const { return Header()->string_length; }
...
wchar_t ON_wString::operator[](int i) const { return m_s[i]; }
        ...
        for ( k = 1; k*len1 <= len+1; k++ )
        {
          ...
          for ( i0 = 0; i0 < 2*len1 + gap; i0++ )
          {
            ...
            if ( repcount != count || replen != len + count*(len2-len1) )
            {
              ...
              RhinoApp().Print(L"%s (%d tokens, %d
chars)\n",str.Array(),repcount,replen); if ( bRepeat )
              {
                bRepeat = false;
              }
              else
...
int ON_wString::Find(unsigned char c) const { return Find((char)c); }
...
int ON_wString::Find(const char *s) const {
    ...
    if (s && s[0] && !IsEmpty()) {
        const int s_count =
            (int)strlen(s); // the (int) is for 64 bit size_t conversion
...
wchar_t ON_wString::GetAt(int i) const { return m_s[i]; }
...
void ON_wString::SetAt(int i, unsigned char c) { SetAt(i, (char)c); }
...
ON_wString ON_wString::Mid(int i) const { return Mid(i, Length() - i); }
