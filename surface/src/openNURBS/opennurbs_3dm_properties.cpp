...
ON_3dmNotes::ON_3dmNotes()
    : m_bVisible(0), m_bHTML(0), m_window_left(0), m_window_top(0),
      m_window_right(0), m_window_bottom(0) {}
...
void ON_SetBinaryArchiveOpenNURBSVersion(ON_BinaryArchive &file, int value) {
    if (value >= 200012210) {
        ...
        ON_ERROR(
            "ON_SetBinaryArchiveOpenNURBSVersion - invalid opennurbs version");
    ...
    for (;;) {
        ...
        case TCODE_PROPERTIES_OPENNURBS_VERSION: {
            ...
            if (value > 299912319 || (value != 0 && value < 200101010)) {
                ON_ERROR("ON_3dmProperties::Read - "
                         "TCODE_PROPERTIES_OPENNURBS_VERSION corrupt value");
                rc = false;
            } else {
        ...
        case TCODE_PROPERTIES_REVISIONHISTORY: // file creation/revision
                                               // information
        ...
        case TCODE_PROPERTIES_COMPRESSED_PREVIEWIMAGE: // compressed preview
                                                       // image
...
ON_BOOL32 ON_3dmProperties::Write(ON_BinaryArchive &file) const {
    ...
    // This short chunk identifies the version of OpenNURBS that was used to
    // write this file.
    rc = file.BeginWrite3dmChunk(TCODE_PROPERTIES_OPENNURBS_VERSION,
                                 ON::Version());
    if (rc)
    ...
    if (rc && m_PreviewImage.IsValid()) {
        rc = file.BeginWrite3dmChunk(TCODE_PROPERTIES_COMPRESSED_PREVIEWIMAGE,
                                     0);
        if (rc) {
