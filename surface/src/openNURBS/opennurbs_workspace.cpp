...
int ON_Workspace::KeepMemory(void *p) {
    ...
    if (p) {
        ...
        while (pBlk) {
            if (pBlk->pMem == p) {
                // Remove pBlk from list so ~ON_Workspace() won't onfree() its
                // memory and any future GrowMemory...() or KeepMemory() calls
                // won't have to search past it.
                pBlk->pMem = NULL;
                if (pPrevBlk) {
...
int **ON_Workspace::GetIntMemory(size_t row_count, size_t col_count) {
    ...
    if (row_count > 0 && col_count > 0) {
        p = (int **)GetMemory(row_count *
                              (sizeof(*p) + col_count * sizeof(**p)));
        if (p) {
            p[0] = (int *)(p + row_count);
...
double **ON_Workspace::GetDoubleMemory(size_t row_count, size_t col_count) {
    ...
    if (row_count > 0 && col_count > 0) {
        ...
        i = (sizeof(*p) < sizeof(**p)) ? (row_count + (row_count % 2))
                                       : row_count;
        p = (double **)GetMemory(i * sizeof(*p) +
                                 row_count * col_count * sizeof(**p));
        if (p) {
            p[0] = (double *)(p + i);
            for (i = 1; i < row_count; i++) {
                p[i] = p[i - 1] + col_count;
