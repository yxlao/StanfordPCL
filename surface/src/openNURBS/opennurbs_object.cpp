...
const ON_UUID ON_max_uuid = {0xFFFFFFFF,
                             0xFFFF,
                             0xFFFF,
...
const ON_UUID ON_rhino3_id = {0xA7BBFF3C,
                              0xFF19,
                              0x4883,
                              {0x85, 0x8D, 0xB1, 0xE7, 0xDB, 0x4F, 0x1A, 0x7E}};

// {E2143A46-BB01-4b0c-AC4D-C34B5652FAE0}
const ON_UUID ON_rhino4_id = {0xe2143a46,
                              0xbb01,
                              0x4b0c,
                              {0xac, 0x4d, 0xc3, 0x4b, 0x56, 0x52, 0xfa, 0xe0}};

// {60515F84-8F7F-41da-801D-1C87E32F88F5}
const ON_UUID ON_rhino5_id = {0x60515f84,
                              0x8f7f,
                              0x41da,
                              {0x80, 0x1d, 0x1c, 0x87, 0xe3, 0x2f, 0x88, 0xf5}};
...
const ON_UUID ON_v3_userdata_id = {
    ...
    0x6a9d,
    0x478e,
    {0xb0, 0xa2, 0x7c, 0x57, 0x79, 0x97, 0xc6, 0x63}};
...
const ON_UUID ON_v4_userdata_id = {
    ...
    0xa244,
    0x44c2,
    {0xb7, 0xc2, 0x7e, 0x27, 0x39, 0xd, 0x11, 0x96}};
...
const ON_UUID ON_opennurbs4_id = {
    ...
    0x17ba,
    0x4e45,
    {0x9e, 0x67, 0xa2, 0xb8, 0xd9, 0xbe, 0x52, 0xd}};
...
const ON_UUID ON_opennurbs5_id = {
    ...
    0xd957,
    0x4625,
    {0xa4, 0xb3, 0xa0, 0xb5, 0x10, 0xfc, 0x30, 0xd4}};
...
static double ON__dblinithelper(int i) {
    ...
    if (0x40 == u.b[7] && 0 == u.b[0] && 0 == u.b[1] && 0 == u.b[2] &&
        0 == u.b[3] && 0 == u.b[4] && 0 == u.b[5] && 0 == u.b[6]) {
        // little endian doubles
        i7 = 7;
        i6 = 6;
    } else if (0x40 == u.b[0] && 0 == u.b[7] && 0 == u.b[1] && 0 == u.b[2] &&
               0 == u.b[3] && 0 == u.b[4] && 0 == u.b[5] && 0 == u.b[6]) {
        // big endian doubles
        i7 = 0;
        i6 = 1;
...
int ON_ClassId::CurrentMark() { return m_mark0; }
...
int ON_ClassId::Purge(int mark_value) {
    // Fundamental openNURBS class ids have a mark value of 0 and cannot be
    // purged.
    int purge_count = 0;
    if (mark_value > 0) {
        ON_ClassId *prev = 0;
...
const ON_ClassId *ON_ClassId::LastClassId() { return m_p1; }
...
ON_ClassId::ON_ClassId(const char *sClassName, const char *sBaseClassName,
    ...
    : m_pNext(0), m_pBaseClassId(0), m_create(create), m_mark(m_mark0),
      m_class_id_version(1), m_copy(copy), m_f2(0), m_f3(0), m_f4(0), m_f5(0),
      m_f6(0), m_f7(0), m_f8(0) {
...
void ON_ClassId::ConstructorHelper(
    const char *sClassName, const char *sBaseClassName,
    const char *sUUID // UUID in registry format from guidgen
) {
    ...
    if (0 != duplicate_class && m_mark0 > 2) {
        char s[7];
        int ver;
        ON_WARNING("ON_ClassId::ON_ClassId() - class name already in use.  "
                   "Will append number to make it unique.");
    ...
    if (m_sClassName[0] != 'O' || m_sClassName[1] != 'N' ||
        m_sClassName[2] != '_' || m_sClassName[3] != 'O' ||
        m_sClassName[4] != 'b' || m_sClassName[5] != 'j' ||
        m_sClassName[6] != 'e' || m_sClassName[7] != 'c' ||
        m_sClassName[8] != 't' || m_sClassName[9] != 0) {
...
const ON_ClassId *ON_ClassId::ClassId(ON_UUID uuid) {
    ...
    if (!p && !g_bDisableDemotion) {
        // enable OpenNURBS toolkit to read files that contain old uuids even
        // when old class definitions are not loaded.

        // 5EAF1119-0B51-11d4-BFFE-0010830122F0 = TL_NurbsCurve
        ON_UUID nc0 = {0x5EAF1119,
                       0x0B51,
                       0x11d4,
                       {0xBF, 0xFE, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // 76A709D5-1550-11d4-8000-0010830122F0 = old nurbs curve
        ON_UUID nc1 = {0x76A709D5,
                       0x1550,
                       0x11d4,
                       {0x80, 0x00, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // 4760C817-0BE3-11d4-BFFE-0010830122F0 = TL_NurbsSurface
        ON_UUID ns0 = {0x4760C817,
                       0x0BE3,
                       0x11d4,
                       {0xBF, 0xFE, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // FA4FD4B5-1613-11d4-8000-0010830122F0 = old nurbs surface
        ON_UUID ns1 = {0xFA4FD4B5,
                       0x1613,
                       0x11d4,
                       {0x80, 0x00, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // EF638317-154B-11d4-8000-0010830122F0 = old poly curve
        ON_UUID pc0 = {0xEF638317,
                       0x154B,
                       0x11d4,
                       {0x80, 0x00, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // 0705FDEF-3E2A-11d4-800E-0010830122F0 = old trimmed surface
        ON_UUID br0 = {0x0705FDEF,
                       0x3E2A,
                       0x11d4,
                       {0x80, 0x0E, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // 2D4CFEDB-3E2A-11d4-800E-0010830122F0 = old b-rep
        ON_UUID br1 = {0x2D4CFEDB,
                       0x3E2A,
                       0x11d4,
                       {0x80, 0x0E, 0x00, 0x10, 0x83, 0x01, 0x22, 0xF0}};

        // F06FC243-A32A-4608-9DD8-A7D2C4CE2A36 = TL_Brep
        ON_UUID br2 = {0xF06FC243,
                       0xA32A,
                       0x4608,
                       {0x9D, 0xD8, 0xA7, 0xD2, 0xC4, 0xCE, 0x2A, 0x36}};

        // 0A8401B6-4D34-4b99-8615-1B4E723DC4E5 = TL_RevSurface
        ON_UUID revsrf = {0xa8401b6,
                          0x4d34,
                          0x4b99,
                          {0x86, 0x15, 0x1b, 0x4e, 0x72, 0x3d, 0xc4, 0xe5}};

        // 665F6331-2A66-4cce-81D0-B5EEBD9B5417 = TL_SumSurface
        ON_UUID sumsrf = {0x665f6331,
                          0x2a66,
                          0x4cce,
                          {0x81, 0xd0, 0xb5, 0xee, 0xbd, 0x9b, 0x54, 0x17}};
        ...
        if (!ON_UuidCompare(&uuid, &nc0) || !ON_UuidCompare(&uuid, &nc1))
            ...
            p = 0; // <- does nothing but it's a good place for debugger
                   // breakpoint
    }
    return p;
}
...
int ON__ClassIdDumpNode::CompareClassUuid(
    const class ON__ClassIdDumpNode &other) const {
    int rc = 0;
...
int ON__ClassIdDumpNode::CompareClassName(
    const class ON__ClassIdDumpNode &other) const {
    int rc = 0;
    const ON_ClassId *a = m_class_id;
    const ON_ClassId *b = other.m_class_id;
...
const char *ON_ClassId::ClassName() const { return m_sClassName; }

const char *ON_ClassId::BaseClassName() const { return m_sBaseClassName; }

ON_UUID ON_ClassId::Uuid() const { return m_uuid; }

int ON_ClassId::Mark() const { return (m_mark & 0x7FFFFFFF); }

unsigned int ON_ClassId::ClassIdVersion() const {
    return (0 != (m_mark & 0x80000000)) ? m_class_id_version : 0;
}

const ON_ClassId *ON_ClassId::BaseClass() const { return m_pBaseClassId; }
...
ON_VIRTUAL_OBJECT_IMPLEMENT(ON_Object, 0,
...
bool ON_Object::CopyFrom(const ON_Object *src) {
    ...
    return (cid && cid->ClassIdVersion() >= 1 && cid->m_copy)
               ? cid->m_copy(src, this)
               : false;
}
...
void ON_Object::EmergencyDestroy() { m_userdata_list = 0; }
...
ON_BOOL32 ON_Object::AttachUserData(ON_UserData *p) {
    ...
    if (p && NULL == p->m_userdata_owner &&
        ...
        if (p->IsUnknownUserData()) {
            // make sure we have valid user data - the first beta release of
            // Rhino 2.0 created empty user data.
            ON_UnknownUserData *uud = ON_UnknownUserData::Cast(p);
            if (uud)
                rc = uud->IsValid();
            if (!rc) {
                ON_ERROR("ON_Object::AttachUserData() - attempt to attach "
                         "invalid UnknownUserData.");
            }
        } else
            rc = true;
        if (rc) {
...
ON_UserData *ON_Object::GetUserData(const ON_UUID &userdata_uuid) const {
    ...
    for (p = m_userdata_list; p; prev = p, p = p->m_userdata_next) {
        if (!ON_UuidCompare(&p->m_userdata_uuid, &userdata_uuid)) {
            if (p->IsUnknownUserData()) {
                // See if we can convert this unknown user data into something
                // useful. Unknown user data is created when a 3dm archive is
                // read and the definition of the specific user data class is
                // not loaded. If something is getting around to asking for a
                // specific kind of user data, the class definition has probably
                ...
                if (uud) {
                    ...
                    if (realp) {
                        ...
                        if (prev)
                            ...
                            // user data.
                            ON_Object *pNotConst =
                                const_cast<ON_Object *>(this);
                            pNotConst->m_userdata_list = realp;
                            realp->m_userdata_owner =
                                pNotConst; // Dale Lear added 22 Jan 2004 to fix
                                           // I/O bug
                        }
...
ON_UserData *ON_Object::FirstUserData() const { return m_userdata_list; }
...
ON_UUID ON_Object::ModelObjectId() const { return ON_nil_uuid; }

ON_UUID ON_Material::ModelObjectId() const { return m_material_id; }

ON_UUID ON_Layer::ModelObjectId() const { return m_layer_id; }

ON_UUID ON_Font::ModelObjectId() const { return m_font_id; }

ON_UUID ON_DimStyle::ModelObjectId() const { return m_dimstyle_id; }

ON_UUID ON_HatchPattern::ModelObjectId() const { return m_hatchpattern_id; }

ON_UUID ON_Linetype::ModelObjectId() const { return m_linetype_id; }

ON_UUID ON_Bitmap::ModelObjectId() const { return m_bitmap_id; }

ON_UUID ON_Light::ModelObjectId() const { return m_light_id; }

ON_UUID ON_TextureMapping::ModelObjectId() const { return m_mapping_id; }

ON_UUID ON_InstanceDefinition::ModelObjectId() const { return m_uuid; }
...
ON_BOOL32 ON_Object::Write(ON_BinaryArchive &) const {
...
ON_BOOL32 ON_Object::Read(ON_BinaryArchive &) {
