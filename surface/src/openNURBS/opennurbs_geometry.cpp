...
bool ON_Geometry::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
...
ON_BOOL32 ON_Geometry::SwapCoordinates(int i, int j // indices of coords to swap
) {
    ON_BOOL32 rc = false;
    const int dim = Dimension();
...
ON_BOOL32 ON_Geometry::Rotate(double sin_angle,        // sin(angle)
...
ON_BOOL32 ON_Geometry::Rotate(double angle,            // angle in radians
                              const ON_3dVector &axis, // axis of rotation
                              const ON_3dPoint &center // center of rotation
) {
    if (angle == 0.0)
...
bool ON_Geometry::IsDeformable() const { return false; }

bool ON_Geometry::MakeDeformable() { return false; }
