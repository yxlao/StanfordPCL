...
ON_Surface *ON_Surface::DuplicateSurface() const { return Duplicate(); }

ON::object_type ON_Surface::ObjectType() const { return ON::surface_object; }
...
ON_BOOL32 ON_Surface::GetParameterTolerance( // returns tminus < tplus:
...
ON_Surface::ISO
ON_Surface::IsIsoparametric(const ON_Curve &curve,
    ...
    if (bbox.m_min.z == bbox.m_max.z) {
        ...
        if (s0 < s1 && t0 < t1 && (ds <= stol || dt <= ttol)) {
            if (ds * (t1 - t0) <= dt * (s1 - s0)) {
                ...
                if (iso == not_iso &&
                    (s0 < bbox.m_max.x || bbox.m_min.x < s1)) {
                    // check for interior "u = constant" iso
                    GetParameterTolerance(
                        0, 0.5 * (bbox.m_min.x + bbox.m_max.x), &a, &b);
                    if (a <= bbox.m_min.x && bbox.m_max.x <= b)
                        iso = x_iso;
                }
            } else {
                ...
                if (iso == not_iso &&
                    (t0 < bbox.m_max.x || bbox.m_min.x < t1)) {
                    // check for interior "t = constant" iso
                    GetParameterTolerance(
                        1, 0.5 * (bbox.m_min.y + bbox.m_max.y), &a, &b);
                    if (a < bbox.m_min.y && bbox.m_max.y <= b)
                        iso = y_iso;
                }
            }
...
ON_BOOL32 ON_Surface::IsPeriodic(int dir) const { return false; }

bool ON_Surface::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
        ...
        if (bTestC0) {
            ...
            if ((t0 < domain[1] && t1 >= domain[1]) ||
                ...
                if (IsClosed(dir)) {
                    ...
                    for (span_index = 0; span_index < span_count && 1 != *dtype;
                         span_index++) {
                        span_domain.Set(span_vector[span_index],
                                        span_vector[span_index + 1]);
                        for (j = (span_index ? 1 : 0); j <= 2 && 1 != *dtype;
                             j++) {
                            st0[1 - dir] = span_domain.ParameterAt(0.5 * j);
                            st1[1 - dir] = st0[1 - dir];
                            if (bTestD1 || bTestT) {
                                // need to check locus continuity at start/end
                                // of closed surface.
                                if (Evaluate(st0.x, st0.y, 2, 3, &Da[0].x, 1,
                                             hinta) &&
                                    Evaluate(st1.x, st1.y, 2, 3, &Db[0].x, 2,
                                             hintb)) {
                                    if (bTestD1) {
                                        if (!(D1a - D1b).IsTiny(
...
static bool PrincipalCurvaturesAreContinuous(
    bool bSmoothTest, double k1a, double k2a, // side "a" principal curvatures
    ...
    bool rc =
        (bSmoothTest)
            ? ON_IsGsmoothCurvatureContinuous(K[0], K[1], 0.0,
                                              curvature_tolerance)
    ...
    if (rc) {
        ...
        K[0].x = k2a;
        K[1].x = k2b;
        rc = (bSmoothTest) ? ON_IsGsmoothCurvatureContinuous(
                                 K[0], K[1], 0.0, curvature_tolerance)
                           : ON_IsG2CurvatureContinuous(K[0], K[1], 0.0,
                                                        curvature_tolerance);
    }
    return rc;
}
...
bool ON_Surface::IsContinuous(
    ...
    case ON::C2_continuous:
        for (qi = 0; qi < 4; qi++) {
            if (!Ev2Der(sq[qi], tq[qi], P[qi], Ds[qi], Dt[qi], Dss[qi], Dst[qi],
                        Dtt[qi], qi + 1, hint))
                return false;
            if (qi) {
                if (!(P[qi] - P[qi - 1]).IsTiny(point_tolerance))
    ...
    case ON::Gsmooth_continuous: {
        ...
        for (qi = 0; qi < 4; qi++) {
            if (!Ev2Der(sq[qi], tq[qi], P[qi], Ds[qi], Dt[qi], Dss[qi], Dst[qi],
                        Dtt[qi], qi + 1, hint))
                return false;
            ON_EvPrincipalCurvatures(Ds[qi], Dt[qi], Dss[qi], Dst[qi], Dtt[qi],
                                     N[qi], &gauss[qi], &mean[qi], &kappa1[qi],
                                     &kappa2[qi], K1[qi], K2[qi]);
            if (qi) {
                if (!(P[qi] - P[qi - 1]).IsTiny(point_tolerance))
                    return false;
                if (N[qi] * N[qi - 1] < cos_angle_tolerance)
                    return false;
                if (!PrincipalCurvaturesAreContinuous(
                        bSmoothCon, kappa1[qi], kappa2[qi], kappa1[qi - 1],
                        kappa2[qi - 1], curvature_tolerance))
                    return false;
            }
            if (!(P[3] - P[0]).IsTiny(point_tolerance))
                return false;
            if (N[3] * N[0] < cos_angle_tolerance)
                return false;
            if (!PrincipalCurvaturesAreContinuous(
                    bSmoothCon, kappa1[3], kappa2[3], kappa1[0], kappa2[0],
                    curvature_tolerance))
                return false;
        }
    } break;
...
ON_BOOL32
ON_Surface::IsSingular(int side) const { return false; }
...
bool ON_Surface::IsAtSingularity(double s, double t,
...
ON_3dPoint ON_Surface::PointAt(double s, double t) const {
    ON_3dPoint p(0.0, 0.0, 0.0);
    EvPoint(s, t, p);
    return p;
}

ON_3dVector ON_Surface::NormalAt(double s, double t) const {
...
ON_Surface::EvNormal(    // returns false if unable to evaluate
    ...
    if (rc) {
        ...
        // below.
        if (len_ds > ON_SQRT_EPSILON * len_dt &&
            len_dt > ON_SQRT_EPSILON * len_ds) {
            ...
            if (!bOnSide) {
                ...
                if (len_ds > ON_EPSILON * len_dt &&
                    len_dt > ON_EPSILON * len_ds) {
                    ON_3dVector a = ds / len_ds;
                    ON_3dVector b = dt / len_dt;
                    normal = ON_CrossProduct(a, b);
                    rc = normal.Unitize();
                ...
                if (rc) {
                    rc = ON_EvNormal(normal_side, v[1], v[2], v[3], v[4], v[5],
                                     normal);
                }
            }
        }
...
// virtual
ON_BOOL32 ON_Surface::Trim(int dir, const ON_Interval &domain) { return false; }

// virtual
bool ON_Surface::Extend(int dir, const ON_Interval &domain) { return false; }

// virtual
ON_BOOL32 ON_Surface::Split(int dir, double c, ON_Surface *&west_or_south_side,
...
// virtual
int ON_Surface::HasNurbForm() const { return 0; }
...
ON_BOOL32 ON_Surface::HasBrepForm() const { return true; }
