...
ON_Quaternion ON_CrossProduct(const ON_Quaternion &p, const ON_Quaternion &q) {
    return ON_Quaternion(0.0, p.c * q.d - p.d * q.c, p.d * q.b - p.b * q.d,
                         p.b * q.c - p.c * q.d);
}

ON_Quaternion ON_QuaternionProduct(const ON_Quaternion &p,
...
ON_Quaternion ON_Quaternion::Slerp(ON_Quaternion q0, ON_Quaternion q1,
                                   double t) {
    // Added 8 Jan 2010 - not tested yet
    ON_Quaternion q;
    if (t <= 0.5) {
...
void ON_Quaternion::SetRotation(const ON_Plane &plane0,
    ...
    m[0][0] = plane1.xaxis.x * plane0.xaxis.x +
              plane1.yaxis.x * plane0.yaxis.x + plane1.zaxis.x * plane0.zaxis.x;
    m[0][1] = plane1.xaxis.x * plane0.xaxis.y +
              plane1.yaxis.x * plane0.yaxis.y + plane1.zaxis.x * plane0.zaxis.y;
    m[0][2] = plane1.xaxis.x * plane0.xaxis.z +
              plane1.yaxis.x * plane0.yaxis.z + plane1.zaxis.x * plane0.zaxis.z;
    m[1][0] = plane1.xaxis.y * plane0.xaxis.x +
              plane1.yaxis.y * plane0.yaxis.x + plane1.zaxis.y * plane0.zaxis.x;
    m[1][1] = plane1.xaxis.y * plane0.xaxis.y +
              plane1.yaxis.y * plane0.yaxis.y + plane1.zaxis.y * plane0.zaxis.y;
    m[1][2] = plane1.xaxis.y * plane0.xaxis.z +
              plane1.yaxis.y * plane0.yaxis.z + plane1.zaxis.y * plane0.zaxis.z;
    m[2][0] = plane1.xaxis.z * plane0.xaxis.x +
              plane1.yaxis.z * plane0.yaxis.x + plane1.zaxis.z * plane0.zaxis.x;
    m[2][1] = plane1.xaxis.z * plane0.xaxis.y +
              plane1.yaxis.z * plane0.yaxis.y + plane1.zaxis.z * plane0.zaxis.y;
    m[2][2] = plane1.xaxis.z * plane0.xaxis.z +
              plane1.yaxis.z * plane0.yaxis.z + plane1.zaxis.z * plane0.zaxis.z;
    ...
    //     x^2(1-cos(t)) + cos(t), y^2(1-cos(t)) + cos(t), z^2(1-cos(t)) +
    //     cos(t),
...
double ON_Quaternion::Scalar() const { return a; }
...
bool ON_Quaternion::IsNotZero() const {
    return ((0.0 != a || 0.0 != b || 0.0 != c || 0.0 != d) && ON_IsValid(a) &&
            ON_IsValid(b) && ON_IsValid(c) && ON_IsValid(d));
}
...
bool ON_Quaternion::IsValid() const {
    return (
        (ON_IS_VALID(a) && ON_IS_VALID(b) && ON_IS_VALID(c) && ON_IS_VALID(d))
            ? true
            : false);
}
...
ON_3dVector ON_Quaternion::Rotate(ON_3dVector v) const {
    ...
    const ON_Quaternion qv(
        -b * v.x - c * v.y - d * v.z, a * v.x + c * v.z - d * v.y,
        a * v.y - b * v.z + d * v.x, a * v.z + b * v.y - c * v.x);
...
ON_Quaternion::ON_Quaternion(double qa, double qb, double qc, double qd)
    : a(qa), b(qb), c(qc), d(qd) {}

ON_Quaternion::ON_Quaternion(const ON_3dVector &v)
    : a(0.0), b(v.x), c(v.y), d(v.z) {}
