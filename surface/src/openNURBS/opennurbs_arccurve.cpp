...
ON_OBJECT_IMPLEMENT(ON_ArcCurve, ON_Curve,
...
int ON_ArcCurve::Dimension() const { return m_dim; }
...
ON_BOOL32 ON_ArcCurve::IsValid(ON_TextLog *text_log) const {
    if (!m_t.IsIncreasing()) {
        if (0 != text_log)
            text_log->Print("ON_ArcCurve - m_t=(%g,%g) - it should be an "
                            "increasing interval.\n",
                            m_t[0], m_t[1]);
        return false;
    }
...
ON_BOOL32 ON_ArcCurve::Write(ON_BinaryArchive &file // open binary file
) const {
...
ON_BOOL32 ON_ArcCurve::Read(ON_BinaryArchive &file // open binary file
) {
    int major_version = 0;
    int minor_version = 0;
...
ON_Interval ON_ArcCurve::Domain() const { return m_t; }

ON_BOOL32 ON_ArcCurve::ChangeClosedCurveSeam(double t) {
    bool rc = false;
    if (IsCircle()) {
...
int ON_ArcCurve::SpanCount() const { return 1; }
...
int ON_ArcCurve::Degree() const { return 2; }
...
ON_BOOL32 ON_ArcCurve::SetStartPoint(ON_3dPoint start_point) {
    ...
    if (m_dim == 3 || start_point.z == 0.0) {
        ...
        if (rc) {
            ...
            if (end_point.DistanceTo(start_point) <
                ON_ZERO_TOLERANCE * m_arc.Radius()) {
                // make arc into circle
                m_arc.plane.xaxis = end_point - m_arc.Center();
                m_arc.plane.xaxis.Unitize();
                m_arc.plane.yaxis =
                    ON_CrossProduct(m_arc.Normal(), m_arc.plane.xaxis);
...
ON_BOOL32 ON_ArcCurve::Evaluate( // returns false if unable to evaluate
    ...
    int,  // side - formal parameter intentionally ignored in this virtual
          // function
    int * // hint - formal parameter intentionally ignored in this virtual
          // function
) const {
    ON_3dVector d;
    ON_BOOL32 rc = false;
    if (m_t[0] < m_t[1]) {
        ...
        double scale = 1.0;
        double a =
            m_arc.DomainRadians().ParameterAt(m_t.NormalizedParameterAt(t));
        double c = cos(a) * m_arc.radius;
        double s = sin(a) * m_arc.radius;
        ON_3dPoint p =
            m_arc.plane.origin + c * m_arc.plane.xaxis + s * m_arc.plane.yaxis;
        v[0] = p.x;
        v[1] = p.y;
        if (m_dim == 3)
            v[2] = p.z;
...
ON_BOOL32 ON_ArcCurve::Split(double t, ON_Curve *&left_side,
...
static ON_BOOL32 NurbsCurveArc(const ON_Arc &arc, int dim,
                               ON_NurbsCurve &nurb) {
    if (!arc.IsValid())
        return false;
...
bool ON_Arc::GetRadianFromNurbFormParameter(double NurbParameter,
                                            double *RadianParameter) const {
    //  TRR#53994.
    // 16-Sept-09  Replaced this code so we dont use LocalClosestPoint.
    // In addition to being slower than neccessary the old method suffered from
    // getting the wrong answer at the seam of a full circle,  This probably
    // only happened with large coordinates where many digits of precision get
    // lost.
    ...
    if (fabs(NurbParameter - dom[0]) <= 2.0 * ON_EPSILON * fabs(dom[0])) {
        *RadianParameter = dom[0];
        return true;
    } else if (fabs(NurbParameter - dom[1]) <=
               2.0 * ON_EPSILON * fabs(dom[1])) {
    ...
    if (theta < dom[0] || theta > dom[1]) {
        ...
        // are that the radius is way to small relative to the coordinates of
        // the center. In this case the circle is just numerical noise around
        // the center anyway.)
...
bool ON_Arc::GetNurbFormParameterFromRadian(double RadianParameter,
...
ON_ArcCurve::GetCurveParameterFromNurbFormParameter(double nurbs_t,
    ...
    *curve_t =
        m_t.ParameterAt(m_arc.DomainRadians().NormalizedParameterAt(radians));
...
ON_ArcCurve::GetNurbFormParameterFromCurveParameter(double curve_t,
    ...
    if (rc) // Oct 29, 2009 - Dale Lear added condition to set *nurbs_t =
            // curve_t
        *nurbs_t = m_t.ParameterAt(
            m_arc.DomainRadians().NormalizedParameterAt(arcnurb_t));
    else
        *nurbs_t = curve_t;
    return rc;
}

bool ON_ArcCurve::IsCircle() const { return m_arc.IsCircle() ? true : false; }

double ON_ArcCurve::Radius() const { return m_arc.Radius(); }

double ON_ArcCurve::AngleRadians() const { return m_arc.AngleRadians(); }

double ON_ArcCurve::AngleDegrees() const { return m_arc.AngleDegrees(); }
