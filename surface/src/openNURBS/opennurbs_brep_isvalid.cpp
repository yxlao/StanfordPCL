...
bool ON_Brep::IsValidVertexTopology(int vertex_index,
    ...
    for (vei = 0; vei < vertex_edge_count; vei++) {
        ...
        if (edge.m_vi[0] != vertex_index && edge.m_vi[1] != vertex_index) {
            if (text_log) {
                text_log->Print(
                    "brep.m_V[%d] vertex or brep.m_E[%d] edge is not valid.\n",
                    vertex_index, ei);
                text_log->PushIndent();
                text_log->Print(
                    "vertex.m_ei[%d] = %d but brep.m_E[%d].m_vi[] = [%d,%d]. "
...
bool ON_Brep::IsValidEdgeTopology(int edge_index, ON_TextLog *text_log) const {
    ...
    // This checks to make sure that m_C3[] and the edge's proxy information is
    // valid. This isn't exactly "topology", but if this stuff isn't set right,
    // then the "geometry" checks might crash.
    ...
    if (m_C3[c3i] != edge.ProxyCurve()) {
        if (text_log) {
            ...
            text_log->PushIndent();
            text_log->Print(
                "m_E[%d].m_c3i=%d, m_C3[%d] != m_E[%d].ProxyCurve()\n",
    ...
    if (!c3_dom.Includes(proxy_sub_dom)) {
        if (text_log) {
            ...
            text_log->PushIndent();
            text_log->Print(
                "m_C3[%d].Domain() = (%g,%g) does not inlclude "
                "m_E[%d].ProxyCurveDomain() = (%g,%g) is not increasing\n",
                c3i, c3_dom[0], c3_dom[1], edge_index, proxy_sub_dom[0],
                proxy_sub_dom[1]);
            text_log->PopIndent();
        }
        return false;
...
bool ON_Brep::IsValidLoopTopology(int loop_index, ON_TextLog *text_log) const {
    ...
    if (loop.m_ti.Count() < 1) {
        if (text_log)
            text_log->Print(
                "ON_Brep.m_L[%d].m_ti.Count() = %d  (should be > 0 )\n",
        ...
        if (0 == lti) {
            ...
            if (text_log) {
                text_log->Print("brep loop m_L[%d] is not valid.\n",
                                loop_index);
                text_log->PushIndent();
                text_log->Print("m_T[loop.m_ti[%d]=%d].m_vi[1] = %d != "
                                "m_T[loop.m_ti[%d]=%d].m_vi[0]=%d.\n",
                                lti - 1, prev_trim_ti, prev_trim_vi1, lti,
    ...
    if (first_trim_ti >= 0 && first_trim_vi0 != prev_trim_vi1) {
        ...
        if (text_log) {
            ...
            text_log->PushIndent();
            text_log->Print("m_T[loop.m_ti[%d]=%d].m_vi[1] = %d != "
                            "m_T[loop.m_ti[0]=%d].m_vi[0]=%d.\n",
                            loop.m_ti.Count() - 1, prev_trim_ti, prev_trim_vi1,
...
bool ON_Brep::IsValidTrimTopology(int trim_index, ON_TextLog *text_log) const {
    ...
    if (0 == trim.ProxyCurve()) {
        if (text_log)
            text_log->Print(
                "brep.m_T[%d].m_c2i = %d, but trim.ProxyCurve() is NULL.\n",
                trim_index, trim.m_c2i);
        return false;
    }
    ...
    if (m_C2[trim.m_c2i] != trim.ProxyCurve()) {
        if (text_log) {
            ...
            text_log->PushIndent();
            text_log->Print(
                "m_T[%d].m_c2i=%d, m_C2[%d] != m_T[%d].ProxyCurve()\n",
    ...
    if (!c2_dom.Includes(proxy_sub_dom)) {
        if (text_log) {
            ...
            text_log->PushIndent();
            text_log->Print(
                "m_C2[%d].Domain() = (%g,%g) does not include "
                "m_T[%d].ProxyCurveDomain() = (%g,%g) is not increasing\n",
                trim.m_c2i, c2_dom[0], c2_dom[1], trim_index, proxy_sub_dom[0],
                proxy_sub_dom[1]);
            text_log->PopIndent();
        }
        return false;
    ...
    if (!bFoundTrim) {
        if (text_log) {
            text_log->Print(
                "brep.m_T[%d] trim or brep.m_L[%d] loop is not valid.\n",
                trim_index, trim.m_li);
            text_log->PushIndent();
            text_log->Print("trim.m_li = %d but loop.m_ti[] does not contain "
                            "%d (should appear once in).\n",
    ...
    if (trim.m_type == ON_BrepTrim::singular) {
        ...
        if (trim.m_ei != -1) {
            if (text_log)
                text_log->Print("ON_Brep.m_T[%d].m_type = singular, but m_ei = "
                                "%d (should be -1).\n",
        ...
        if (trim.m_bRev3d) {
            if (text_log)
                text_log->Print("ON_Brep.m_T[%d].m_type = singular, but "
                                "m_bRev3d = %d (should be 0).\n",
                                trim_index, trim.m_bRev3d);
            return false;
        }
        if (trim.m_vi[0] != trim.m_vi[1]) {
            if (text_log)
                text_log->Print("ON_Brep.m_T[%d].m_type = singular, but m_vi = "
                                "(%d,%d) (should be same vertex index).\n",
...
bool ON_Brep::IsValidTopology(ON_TextLog *text_log) const {
...
bool ON_Brep::IsValidGeometry(ON_TextLog *text_log) const {
    const int curve2d_count = m_C2.Count();
    const int curve3d_count = m_C3.Count();
    const int surface_count = m_S.Count();
    const int vertex_count = m_V.Count();
...
bool ON_Brep::IsValidTolerancesAndFlags(ON_TextLog *text_log) const {
    const int vertex_count = m_V.Count();
    const int edge_count = m_E.Count();
    const int trim_count = m_T.Count();
    const int loop_count = m_L.Count();
