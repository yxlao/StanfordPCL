...
ON_OBJECT_IMPLEMENT(ON_NurbsSurface, ON_Surface,
...
ON_NurbsSurface *ON_NurbsSurface::New(const ON_NurbsSurface &nurbs_surface) {
    // static function replaces new ON_NurbsSurface(const ON_NurbsSurface&
    // nurbs_surface);
...
ON_NurbsSurface *ON_NurbsSurface::New(int dimension, ON_BOOL32 bIsRational,
                                      int order0, int order1, int cv_count0,
                                      int cv_count1) {
    // static function replaces new ON_NurbsSurface(dim, is_rat, order0, ...,
    // cv_count1 );
    return new ON_NurbsSurface(dimension, bIsRational, order0, order1,
                               cv_count0, cv_count1);
}
...
ON__UINT32 ON_NurbsSurface::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder = ON_CRC32(
        current_remainder, KnotCount(0) * sizeof(m_knot[0][0]), m_knot[0]);
    current_remainder = ON_CRC32(
        current_remainder, KnotCount(1) * sizeof(m_knot[1][0]), m_knot[1]);

    return current_remainder;
}

ON_BOOL32
ON_NurbsSurface::SetDomain(int dir, // 0 sets first parameter's domain, 1 gets
                                    // second parameter's domain
                           double t0, double t1) {
    bool rc = false;
    if (m_order[dir] >= 2 && m_cv_count[dir] >= m_order[dir] && m_knot &&
        t0 < t1) {
...
int ON_NurbsSurface::Dimension() const { return m_dim; }

bool ON_NurbsSurface::IsRational() const { return m_is_rat ? true : false; }

int ON_NurbsSurface::CVSize() const { return (m_is_rat) ? m_dim + 1 : m_dim; }

int ON_NurbsSurface::Order(int dir) const { return m_order[dir ? 1 : 0]; }

int ON_NurbsSurface::Degree(int dir) const {
    return (m_order[dir ? 1 : 0] >= 2) ? m_order[dir ? 1 : 0] - 1 : 0;
}

int ON_NurbsSurface::CVCount(int dir) const { return m_cv_count[dir ? 1 : 0]; }
...
double ON_NurbsSurface::Weight(int i, int j) const {
    return (m_cv && m_is_rat)
               ? m_cv[i * m_cv_stride[0] + j * m_cv_stride[1] + m_dim]
               : 1.0;
}
...
bool ON_NurbsSurface::MakeClampedUniformKnotVector(int dir, double delta) {
    if (dir < 0 || dir > 1)
        return false;
    ReserveKnotCapacity(dir, ON_KnotCount(m_order[dir], m_cv_count[dir]));
    return ON_MakeClampedUniformKnotVector(m_order[dir], m_cv_count[dir],
                                           m_knot[dir], delta);
}

bool ON_NurbsSurface::MakePeriodicUniformKnotVector(int dir, double delta) {
    if (dir < 0 || dir > 1)
        return false;
    ReserveKnotCapacity(dir, ON_KnotCount(m_order[dir], m_cv_count[dir]));
    return ON_MakePeriodicUniformKnotVector(m_order[dir], m_cv_count[dir],
                                            m_knot[dir], delta);
}
...
void ON_NurbsSurface::EmergencyDestroy() { Initialize(); }
    ...
    if (m_knot_capacity[dir] < capacity) {
        if (m_knot[dir]) {
            if (m_knot_capacity[dir]) {
                m_knot[dir] = (double *)onrealloc(
                    m_knot[dir], capacity * sizeof(*m_knot[dir]));
...
static void ON_NurbsSurfaceCopyHelper(const ON_NurbsSurface &src,
    ...
    if (src.m_knot[0]) {
        ...
        memcpy(dest.m_knot[0], src.m_knot[0],
               dest.KnotCount(0) * sizeof(*dest.m_knot[0]));
    }
    if (src.m_knot[1]) {
        // copy knot array
        dest.ReserveKnotCapacity(1, dest.KnotCount(1));
        memcpy(dest.m_knot[1], src.m_knot[1],
               dest.KnotCount(1) * sizeof(*dest.m_knot[1]));
    }
    if (src.m_cv) {
        // copy cv array
        ...
        if (src_stride[0] == dest.m_cv_stride[0] &&
            src_stride[1] == dest.m_cv_stride[1]) {
            memcpy(dest.m_cv, src.m_cv,
                   dest.m_cv_count[0] * dest.m_cv_count[1] *
                       dest.m_cv_stride[1] * sizeof(*dest.m_cv));
        } else {
            const double *src_cv;
...
void ON_NurbsSurface::Dump(ON_TextLog &dump) const {
    dump.Print("ON_NurbsSurface dim = %d is_rat = %d\n"
               ...
               m_dim, m_is_rat, m_order[0], m_order[1], m_cv_count[0],
               m_cv_count[1]);
    int dir;
    for (dir = 0; dir < 2; dir++) {
    ...
    if (!m_cv) {
        ...
        for (i = 0; i < m_cv_count[0]; i++) {
            ...
            dump.PrintPointList(m_dim, m_is_rat, m_cv_count[1], m_cv_stride[1],
                                CV(i, 0), sPreamble);
        }
    }
}
    ...
    if (m_dim <= 0) {
        ...
        for (i = 0; i < 2 && rc; i++) {
            ...
            if (m_order[i] < 2) {
                ...
                if (text_log) {
                    text_log->Print("ON_NurbsSurface.m_cv_count[%d] = %d "
                                    "(should be >= m_order[%d]=%d).\n",
            ...
            } else if (!ON_IsValidKnotVector(m_order[i], m_cv_count[i],
                                             m_knot[i], text_log)) {
                if (text_log) {
                    text_log->Print("ON_NurbsSurface.m_knot[%d] is not a valid "
                                    "knot vector.\n",
                                    i);
                }
            } else if (m_cv_stride[i] < CVSize()) {
                if (text_log) {
                    text_log->Print("ON_NurbsSurface.m_cv_stride[%d]=%d is too "
                                    "small (should be >= %d).\n",
        ...
        if (rc) {
            ...
            if (m_cv_stride[0] < a0 || m_cv_stride[1] < a1) {
                if (m_cv_stride[0] < b0 || m_cv_stride[1] < b1) {
                    if (text_log) {
                        text_log->Print("ON_NurbsSurface.m_cv_stride[] = "
                                        "{%d,%d} is not valid.\n",
...
ON_BOOL32 ON_NurbsSurface::GetBBox( // returns true if successful
    ...
    return ON_GetPointGridBoundingBox(
        m_dim, m_is_rat, m_cv_count[0], m_cv_count[1], m_cv_stride[0],
        m_cv_stride[1], m_cv, boxmin, boxmax, bGrowBox ? true : false);
}
...
ON_BOOL32 ON_NurbsSurface::Transform(const ON_Xform &xform) {
    ...
    return ON_TransformPointGrid(m_dim, m_is_rat, m_cv_count[0], m_cv_count[1],
                                 m_cv_stride[0], m_cv_stride[1], m_cv, xform);
}

bool ON_NurbsSurface::IsDeformable() const { return true; }

bool ON_NurbsSurface::MakeDeformable() { return true; }

ON_BOOL32 ON_NurbsSurface::Write(ON_BinaryArchive &file // open binary file
) const {
    ...
    if (rc) {
        ...
        if (rc) {
            ON_BoundingBox
                bbox; // write invalid bounding box - may be used in future
        ...
        count =
            (m_cv && cv_size > 0 && m_cv_count[0] > 0 && m_cv_count[1] > 0 &&
...
ON_BOOL32 ON_NurbsSurface::Read(ON_BinaryArchive &file // open binary file
) {
    // NOTE - check legacy I/O code if changed
    int major_version = 0;
    int minor_version = 0;
    ...
    if (rc && major_version == 1) {
        ...
        int dim = 0, is_rat = 0, order0 = 0, order1 = 0, cv_count0 = 0,
            cv_count1 = 0;
...
ON_Interval ON_NurbsSurface::Domain(int dir) const {
    ...
    if (dir)
        dir = 1;
    ON_GetKnotVectorDomain(m_order[dir], m_cv_count[dir], m_knot[dir],
                           &d.m_t[0], &d.m_t[1]);
    return d;
}
...
double ON_NurbsSurface::ControlPolygonLength(int dir) const {
    ...
    if (dir >= 0 && dir <= 1 && m_cv_count[0] >= 2 && m_cv_count[1] >= 2 &&
        ...
        for (i = 0; i < m_cv_count[1 - dir]; i++) {
            length = 0.0;
            p = (dir) ? CV(i, 0) : CV(0, i);
            ON_GetPolylineLength(m_dim, m_is_rat, m_cv_count[dir],
                                 m_cv_stride[dir], p, &length);
...
ON_BOOL32 ON_NurbsSurface::GetSpanVector(int dir, double *s) const {
    if (dir)
        dir = 1;
    return ON_GetKnotVectorSpanVector(m_order[dir], m_cv_count[dir],
                                      m_knot[dir], s);
}

ON_BOOL32
ON_NurbsSurface::GetParameterTolerance( // returns tminus < tplus: parameters
                                        // tminus <= s <= tplus
    int dir,
...
ON_NurbsSurface::Evaluate( // returns false if unable to evaluate
    ...
    ON_BOOL32 rc = false;
    int span_index[2];
    span_index[0] = ON_NurbsSpanIndex(m_order[0], m_cv_count[0], m_knot[0], s,
                                      (side == 2 || side == 3) ? -1 : 1,
                                      (hint) ? hint[0] : 0);
    span_index[1] = ON_NurbsSpanIndex(m_order[1], m_cv_count[1], m_knot[1], t,
                                      (side == 3 || side == 4) ? -1 : 1,
                                      (hint) ? hint[1] : 0);
    rc = ON_EvaluateNurbsSurfaceSpan(
        m_dim, m_is_rat, m_order[0], m_order[1], m_knot[0] + span_index[0],
        m_knot[1] + span_index[1], m_cv_stride[0], m_cv_stride[1],
        m_cv +
            (span_index[0] * m_cv_stride[0] + span_index[1] * m_cv_stride[1]),
        der_count, s, t, v_stride, v);
    if (hint) {
        hint[0] = span_index[0];
        hint[1] = span_index[1];
    }
...
ON_Curve *ON_NurbsSurface::IsoCurve(
    ...
    if (dir == 0 || dir == 1 && IsValid()) {
        ...
        memcpy(nurbscrv->m_knot, m_knot[dir],
               nurbscrv->KnotCount() * sizeof(*nurbscrv->m_knot));
        span_index = ON_NurbsSpanIndex(m_order[1 - dir], m_cv_count[1 - dir],
                                       m_knot[1 - dir], c, 1, 0);
        if (span_index < 0)
            span_index = 0;
        else if (span_index > m_cv_count[1 - dir] - m_order[1 - dir])
            span_index = m_cv_count[1 - dir] - m_order[1 - dir];
        ON_NurbsCurve N(Scvsize * nurbscrv->CVCount(), 0, m_order[1 - dir],
                        m_order[1 - dir]);
        memcpy(N.m_knot, m_knot[1 - dir] + span_index,
               N.KnotCount() * sizeof(*N.m_knot));
...
static ON_NurbsCurve *ToCurve(const ON_NurbsSurface &srf, int dir,
    ...
    if (crv->m_cv == srf.m_cv) {
        tmp_cv = (double *)onmalloc(crv->m_dim * crv->m_cv_stride *
                                    sizeof(tmp_cv[0]));
        crv->m_cv = tmp_cv;
    }
    double *pdst;
    ...
    if (tmp_cv) {
        crv->m_cv = srf.m_cv;
        memcpy(crv->m_cv, tmp_cv,
               crv->m_dim * crv->m_cv_stride * sizeof(tmp_cv[0]));
        onfree(tmp_cv);
    }
    if (crv->m_knot != srf.m_knot[dir])
        memcpy(crv->m_knot, srf.m_knot[dir],
               crv->KnotCount() * sizeof(crv->m_knot[0]));
    return crv;
}
...
ON_BOOL32 ON_NurbsSurface::Trim(int dir, const ON_Interval &domain) {
    ON_BOOL32 rc = false;
    if (dir < 0 || dir > 1)
        return false;
    ON_Interval current_domain = Domain(dir);
    if (current_domain[0] == ON_UNSET_VALUE &&
        current_domain[1] == ON_UNSET_VALUE)
...
ON_BOOL32 ON_NurbsSurface::Split(int dir, double c,
    ...
    if (left_srf != this) {
        ...
        memcpy(left_srf->m_knot[1 - dir], m_knot[1 - dir],
    ...
    if (right_srf != this) {
        ...
        memcpy(right_srf->m_knot[1 - dir], m_knot[1 - dir],
               KnotCount(1 - dir) * sizeof(m_knot[1 - dir][0]));
    }
...
ON_BOOL32 ON_NurbsSurface::IsPlanar(ON_Plane *plane, double tolerance) const {
    ON_Plane pln;
    double d;
    ...
    ON_BOOL32 rc = EvNormal(udom.ParameterAt(0.5), vdom.ParameterAt(0.5),
                            center, du, dv, normal);
...
ON_NurbsSurface::IsClosed(int dir) const {
    ...
    if (dir >= 0 && dir <= 1 && m_dim > 0) {
        if (ON_IsKnotVectorClamped(m_order[dir], m_cv_count[dir],
                                   m_knot[dir])) {
            if (ON_IsPointGridClosed(m_dim, m_is_rat, m_cv_count[0],
                                     m_cv_count[1], m_cv_stride[0],
                                     m_cv_stride[1], m_cv, dir))
                bIsClosed = true;
...
ON_NurbsSurface::IsPeriodic(int dir) const {
    ...
    if (dir >= 0 && dir <= 1) {
        int k;
        bIsPeriodic =
            ON_IsKnotVectorPeriodic(m_order[dir], m_cv_count[dir], m_knot[dir]);
        if (bIsPeriodic) {
...
bool ON_NurbsSurface::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
    ...
    ki = ON_NurbsSpanIndex(m_order[dir], m_cv_count[dir], m_knot[dir], t0, 1,
                           *hint);
    ...
    if (t0 < t1) {
        ...
        if (t0 < m_knot[dir][ii + 1] && t1 > m_knot[dir][ii + 1] &&
            ...
            t0 = m_knot[dir][ii + 1];
            ki = ON_NurbsSpanIndex(m_order[dir], m_cv_count[dir], m_knot[dir],
                                   t0, 1, *hint);
        }
        *hint = ki;
        ki += m_order[dir] - 2;
        ...
        if (ki >= m_cv_count[dir] - 1) {
            if (input_c != c && t0 < m_knot[dir][m_cv_count[dir] - 1] &&
                ...
                return ON_Surface::GetNextDiscontinuity(
                    dir, input_c, t0, t1, t, hint, dtype, cos_angle_tolerance,
                    curvature_tolerance);
            }
            return false;
        }
    ...
    while (m_knot[dir][ki] < t1) {
        if (delta_ki > 0) {
            // t0 < t1 case
            while (ki < m_cv_count[dir] - 1 &&
                   m_knot[dir][ki] == m_knot[dir][ki + 1])
                ki++;
            if (ki >= m_cv_count[dir] - 1)
                break;
        } else {
        ...
        if (m_knot[dir][ki] == m_knot[dir][ki + delta]) {
            ...
            int j, j0 = 0, otherki;
            for (otherki = m_order[1 - dir] - 2;
                 otherki < m_cv_count[1 - dir] - 1; otherki++) {
                span_domain.Set(m_knot[1 - dir][otherki],
                                m_knot[1 - dir][otherki + 1]);
                for (j = j0; j <= 2; j++) {
                    st[1 - dir] = span_domain.ParameterAt(0.5 * j);

                    Evaluate(st.x, st.y, bEv2ndDer ? 2 : 1, 3, &Vm[0].x, 3,
                             hint);
                    Evaluate(st.x, st.y, bEv2ndDer ? 2 : 1, 3, &Vp[0].x, 1,
                             hint);
    ...
    if (input_c != c) {
        ...
        rc = ON_Surface::GetNextDiscontinuity(dir, input_c, t0, t1, t, hint,
...
ON_BOOL32
ON_NurbsSurface::SetCV(int i, int j, ON::point_style style,
                       const double *Point) {
    ON_BOOL32 rc = true;
    int k;
    double w;
    ...
    case ON::euclidean_rational: // input Point is euclidean rational
        if (IsRational()) {
            // NURBS surface is rational - convert euclean point to homogeneous
            // form
            w = Point[m_dim];
            for (k = 0; k < m_dim; k++)
...
ON_BOOL32
ON_NurbsSurface::GetCV(int i, int j, ON::point_style style,
                       double *Point) const {
...
ON_NurbsSurface::Reverse(int dir) {
    if (dir < 0 || dir > 1)
        return false;
    ON_BOOL32 rc0 =
        ON_ReverseKnotVector(m_order[dir], m_cv_count[dir], m_knot[dir]);
    ON_BOOL32 rc1 =
        ON_ReversePointGrid(3, m_is_rat, m_cv_count[0], m_cv_count[1],
                            m_cv_stride[0], m_cv_stride[1], m_cv, dir);
    return rc0 && rc1;
}
...
ON_NurbsSurface::SwapCoordinates(int i, int j) {
    ...
    if (m_cv_count[0] <= m_cv_count[1]) {
        for (k = 0; k < m_cv_count[0]; k++) {
            if (!ON_SwapPointListCoordinates(m_cv_count[1], m_cv_stride[1],
                                             CV(k, 0), i, j))
                rc = false;
        }
    } else {
        for (k = 0; k < m_cv_count[1]; k++) {
            if (!ON_SwapPointListCoordinates(m_cv_count[0], m_cv_stride[0],
                                             CV(0, k), i, j))
                rc = false;
        }
    }
...
ON_BOOL32 ON_NurbsSurface::SetCVRow(int row_index, const ON_3dPoint &point) {
    int i;
...
ON_BOOL32 ON_NurbsSurface::SetCVColumn(int col_index, const ON_3dPoint &point) {
    int j;
...
bool ON_NurbsSurface::GetGrevilleAbcissae( // see ON_GetGrevilleAbcissae() for
                                           // details
    int dir,                               // dir
    double *g                              // g[cv1-cv0]
) const {
    if (dir)
        dir = 1;
    return ON_GetGrevilleAbcissae(m_order[dir], m_cv_count[dir], m_knot[dir],
                                  false, g);
}
...
bool ON_NurbsSurface::SetClampedGrevilleKnotVector(
    ...
    return ON_GetGrevilleKnotVector(g_stride, g, false, Order(dir),
                                    CVCount(dir), m_knot[dir]);
}
...
bool ON_NurbsSurface::SetPeriodicGrevilleKnotVector(
    ...
    if (!m_knot[dir] && m_order[dir] >= 2 && m_cv_count[dir] >= m_order[dir])
        ReserveKnotCapacity(dir, KnotCount(dir));
    return ON_GetGrevilleKnotVector(g_stride, g, true, Order(dir), CVCount(dir),
                                    m_knot[dir]);
}
...
bool ON_NurbsSurface::IsClamped( // determine if knot vector is clamped
    ...
    bool rc = false;
    if (dir == 0 || dir == 1)
        rc = ON_IsKnotVectorClamped(m_order[dir], m_cv_count[dir], m_knot[dir],
                                    end);
    return rc;
}

static void ConvertToCurve(const ON_NurbsSurface &srf, int dir,
                           ON_NurbsCurve &crv) {
    // DO NOT MAKE THIS FUNCTION PUBLIC - IT IS DELICATE AND DEDICATED TO USE IN
    // THIS FILE
...
static void ConvertFromCurve(ON_NurbsCurve &crv, int dir,
                             ON_NurbsSurface &srf) {
    // DO NOT MAKE THIS FUNCTION PUBLIC - IT IS DELICATE AND DEDICATED TO USE IN
    // THIS FILE
    ...
    if (crv.m_cv) {
        ...
        if (srf.m_cv) {
            ...
            memcpy(srf.m_cv, crv.m_cv,
                   crv.m_cv_stride * crv.m_cv_count * sizeof(*srf.m_cv));
        } else {
...
bool ON_NurbsSurface::InsertKnot(int dir, // dir 0 = "s", 1 = "t"
                                 double knot_value,
    ...
    if ((dir == 0 || dir == 1) && IsValid() && knot_multiplicity > 0 &&
        ...
        if (knot_value < domain.Min() || knot_value > domain.Max()) {
            ON_ERROR(
                "ON_NurbsSurface::InsertKnot() knot_value not inside domain.");
        } else {
            ON_NurbsCurve crv;
            crv.m_knot = m_knot[dir];
...
bool ON_NurbsSurface::ChangeDimension(
    ...
    if (desired_dimension < m_dim) {
        ...
        if (cv_size > old_stride0 && cv_size > old_stride1) {
            new_stride0 = (old_stride0 <= old_stride1)
                              ? cv_size
                              : (cv_size * m_cv_count[1]);
            new_stride1 = (old_stride0 <= old_stride1)
                              ? (cv_size * m_cv_count[0])
                              : cv_size;
...
bool ON_NurbsSurface::IncreaseDegree(int dir,           // dir 0 = "s", 1 = "t"
...
ON_BOOL32 ON_NurbsSurface::TensorProduct(const ON_NurbsCurve &nurbscurveA,
    ...
    double *cvC;
    int i, j, k, cv_countA, cv_countB, dimA, dimB, dimC, is_ratA, is_ratB,
        is_ratC;
    ...
    if (tensor.DimensionA() > dimA) {
        ON_ERROR(
            "ON_NurbsSurface::TensorProduct() - tensor.DimensionA() > dimA");
        return false;
    }
    if (tensor.DimensionB() > dimB) {
        ON_ERROR(
            "ON_NurbsSurface::TensorProduct() - tensor.DimensionB() > dimB");
        return false;
    }
    ...
    Create(dimC, is_ratC, nurbscurveA.Order(), nurbscurveB.Order(), cv_countA,
           cv_countB);

    if (m_knot[0] != nurbscurveA.m_knot)
        memcpy(m_knot[0], nurbscurveA.m_knot,
               KnotCount(0) * sizeof(*m_knot[0]));
    if (m_knot[1] != nurbscurveB.m_knot)
        memcpy(m_knot[1], nurbscurveB.m_knot,
               KnotCount(1) * sizeof(*m_knot[1]));
...
static bool ON_MakeKnotVectorsCompatible(ON_NurbsCurve &nurbs_curveA,
    ...
    ki = order - 1;
    while ((ki < nurbs_curveA.m_cv_count - 1 ||
        ...
        if (b == nurbs_curveB.m_knot[ki - 1])
            return false;
        multA = ON_KnotMultiplicity(order, nurbs_curveA.m_cv_count,
                                    nurbs_curveA.m_knot, ki);
        multB = ON_KnotMultiplicity(order, nurbs_curveB.m_cv_count,
                                    nurbs_curveB.m_knot, ki);

        if (a < b) {
            // insert a in nurbs_curveB
            span.Set(nurbs_curveB.m_knot[ki - 1], nurbs_curveB.m_knot[ki]);
            ktol = ON_SQRT_EPSILON *
                   (span.Length() + fabs(nurbs_curveB.m_knot[ki - 1]) +
            ...
            ktol = ON_SQRT_EPSILON *
                   (span.Length() + fabs(nurbs_curveA.m_knot[ki - 1]) +
...
bool ON_NurbsSurface::CollapseSide(int side, ON_3dPoint point) {
...
ON_BOOL32
ON_NurbsSurface::ConvertSpanToBezier(int span_index0, int span_index1,
        ...
        return false;
    {
        ...
        bispan.Create(m_dim, m_is_rat, m_order[0], m_order[1], m_order[0],
                      m_order[1]);
        const int sizeof_cv = CVSize() * sizeof(*bispan.m_cv);
        for (i = 0; i < m_order[0]; i++)
            for (j = 0; j < m_order[1]; j++) {
                memcpy(bispan.CV(i, j), CV(span_index0 + i, span_index1 + j),
                       sizeof_cv);
            }
        i = span_index0 + m_order[0] - 2;
        j = span_index1 + m_order[1] - 2;
        ...
        if (m_knot[1][span_index1 + m_order[1] - 1] !=
            ...
            bClamp = true;
        if (bClamp) {
            memcpy(bispan.m_knot[0], m_knot[0] + span_index0,
                   bispan.KnotCount(0) * sizeof(bispan.m_knot[0][0]));
            memcpy(bispan.m_knot[1], m_knot[1] + span_index1,
