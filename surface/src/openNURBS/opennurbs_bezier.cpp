...
ON_PolynomialSurface::ON_PolynomialSurface(int dim, ON_BOOL32 is_rat,
...
ON_BOOL32 ON_PolynomialSurface::Create(int dim, int is_rat, int order0,
                                       int order1) {
    ON_BOOL32 rc = true;
    if (dim > 0)
        m_dim = dim;
...
ON_BezierCurve::ON_BezierCurve()
    : m_dim(0), m_is_rat(0), m_order(0), m_cv_stride(0), m_cv(0),
      m_cv_capacity(0) {
...
ON_BezierCurve &ON_BezierCurve::operator=(const ON_PolynomialCurve &src) {
    ...
    if (src.m_dim > 0 && src.m_cv.Count() == src.m_order && src.m_order >= 2) {
        ...
        Loft(src.m_is_rat ? src.m_dim + 1 : src.m_dim, src.m_order, 4, &pt[0].x,
             0, NULL);
...
bool ON_BezierCurve::Loft(int pt_dim, int pt_count, int pt_stride,
                          const double *pt, int t_stride, const double *t) {
    bool rc = false;
    if (pt_dim >= 1 && pt_count >= 2 && pt_stride >= pt_dim && pt != NULL &&
        ...
        M.BackSolve(ON_EPSILON, m_dim, m_order, m_cv_stride, m_cv, m_cv_stride,
                    m_cv);
        if (rank == m_order)
            rc = true;
    }
...
void ON_BezierCurve::Dump(ON_TextLog &dump) const {
    ...
    dump.Print("Control Points  %d %s points\n"
               "  index               value\n",
               m_order, (m_is_rat) ? "rational" : "non-rational");
    if (!m_cv) {
        dump.Print("  NULL cv array\n");
    } else {
        dump.PrintPointList(m_dim, m_is_rat, m_order, m_cv_stride, m_cv,
                            "  CV");
    }
}

int ON_BezierCurve::Dimension() const { return m_dim; }
...
void ON_BezierCurve::EmergencyDestroy() { m_cv = 0; }

bool ON_BezierCurve::GetBBox( // returns true if successful
    double *boxmin,           // minimum
    double *boxmax,           // maximum
    int bGrowBox) const {
    return ON_GetPointListBoundingBox(m_dim, m_is_rat, m_order, m_cv_stride,
                                      m_cv, boxmin, boxmax, bGrowBox);
}
...
bool ON_BezierCurve::GetTightBoundingBox(ON_BoundingBox &tight_bbox,
    ...
    return ON_GetPointListBoundingBox(m_dim, m_is_rat, m_order, m_cv_stride,
                                      m_cv, tight_bbox, bGrowBox, xform);
}
...
bool ON_Line::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                  const ON_Xform *xform) const {
...
bool ON_Arc::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                 const ON_Xform *xform) const {
...
bool ON_Circle::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                    const ON_Xform *xform) const {
...
bool ON_ArcCurve::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                      const ON_Xform *xform) const {
    return m_arc.GetTightBoundingBox(tight_bbox, bGrowBox, xform);
}

bool ON_LineCurve::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                       const ON_Xform *xform) const {
...
bool ON_PolyCurve::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                       const ON_Xform *xform) const {
...
bool ON_Mesh::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                  const ON_Xform *xform) const {
    if (bGrowBox && !tight_bbox.IsValid()) {
        bGrowBox = false;
    }
...
bool ON_BezierCurve::Transform(const ON_Xform &xform) {
    ...
    return ON_TransformPointList(m_dim, m_is_rat, m_order, m_cv_stride, m_cv,
                                 xform);
}

bool ON_BezierCurve::Rotate(double sin_angle,        // sin(angle)
...
bool ON_BezierCurve::Rotate(double angle,            // angle in radians
                            const ON_3dVector &axis, // axis of rotation
                            const ON_3dPoint &center // center of rotation
) {
...
bool ON_BezierCurve::Reverse() {
    bool rc = ON_ReversePointList(m_dim, m_is_rat, m_order, m_cv_stride, m_cv)
                  ? true
                  : false;
    // if ( rc )
    //  m_domain.Reverse();
...
bool ON_BezierCurve::Evaluate( // returns false if unable to evaluate
    ...
    return ON_EvaluateBezier(m_dim, m_is_rat, m_order, m_cv_stride, m_cv, 0.0,
...
bool ON_BezierCurve::GetNurbForm(ON_NurbsCurve &n) const {
    ...
    if (n.Create(m_dim, m_is_rat, m_order, m_order)) {
        ...
        rc = ON_ClampKnotVector(n.m_order, n.m_cv_count, n.m_knot, 2) ? true
                                                                      : false;
    }
    return rc;
}

bool ON_BezierCurve::IsRational() const { return m_is_rat ? true : false; }

int ON_BezierCurve::CVSize() const {
    return (m_dim > 0 && m_is_rat) ? m_dim + 1 : m_dim;
}

int ON_BezierCurve::Order() const { return m_order; }

int ON_BezierCurve::Degree() const { return m_order >= 2 ? m_order - 1 : 0; }
...
int ON_BezierCurve::CVCount() const { return Order(); }
    ...
    if (desired_degree > 0) {
        if (desired_degree == m_order - 1)
            ...
            while (m_order <= desired_degree) {
                rc = ON_IncreaseBezierDegree(m_dim, m_is_rat, m_order,
                                             m_cv_stride, m_cv)
                         ? true
                         : false;
                if (!rc)
...
bool ON_BezierCurve::Trim(const ON_Interval &n) {
    ...
    if (rc) {
        ...
        double t1 = n.Max();
        int cvdim =
            CVSize(); // 9/9/03 fix rat trim bug - use cvdim instead of m_dim
        if (t0 != 1.0) {
            ...
            if (t0 != 0.0) {
                ...
                double s0 = t1 / t0;
                ON_EvaluatedeCasteljau(cvdim, m_order, +1, m_cv_stride, m_cv,
                                       s0);
            }
        }
    }
...
bool ON_BezierCurve::Split(
    ...
    if (rc) {
        ...
        if (this != &left_bez) {
            if (!left_bez.m_cv || (0 < left_bez.m_cv_capacity &&
                                   left_bez.m_cv_capacity < cvdim * m_order)) {
                left_bez.Create(m_dim, m_is_rat, m_order);
            } else if (left_bez.m_dim != m_dim &&
        ...
        if (this != &right_bez) {
            if (!right_bez.m_cv ||
                (0 < right_bez.m_cv_capacity &&
                 right_bez.m_cv_capacity < cvdim * m_order)) {
                right_bez.Create(m_dim, m_is_rat, m_order);
            } else if (right_bez.m_dim != m_dim &&
                           right_bez.m_is_rat != m_is_rat &&
...
ON_BezierSurface::ON_BezierSurface(int dim, ON_BOOL32 is_rat, int order0,
                                   int order1)
    : m_dim(0), m_is_rat(0), m_cv(0), m_cv_capacity(0) {
    m_order[0] = 0;
    m_order[1] = 0;
...
bool ON_BezierSurface::IsValid() const {
    ...
    if (m_cv_stride[1] < m_dim + m_is_rat)
        return false;
    if (m_cv_capacity > 0 &&
        m_cv_capacity < (m_dim + m_is_rat) * m_order[0] * m_order[1])
        return false;
...
void ON_BezierSurface::Dump(ON_TextLog &dump) const {
    ...
    if (!m_cv) {
        ...
        for (i = 0; i < m_order[0]; i++) {
            ...
            dump.PrintPointList(m_dim, m_is_rat, m_order[1], m_cv_stride[1],
                                CV(i, 0), sPreamble);
        }
    }
}

int ON_BezierSurface::Dimension() const { return m_dim; }

bool ON_BezierSurface::Create(int dim, ON_BOOL32 is_rat, int order0,
                              int order1) {
    if (m_cv_capacity < 1)
        m_cv = 0;
    m_dim = (dim > 0) ? dim : 0;
...
void ON_BezierSurface::EmergencyDestroy() { m_cv = 0; }
...
bool ON_BezierSurface::Loft(int curve_count,
                            const ON_BezierCurve *const *curve_list) {
    // 06-06-06 Dale Fugier - tested
    bool rc = false;
    if (curve_count >= 2 && 0 != curve_list && 0 != curve_list[0]) {
        // determine order, dimension, is_rat, and cv_stride of compatible shape
        // curves
        ...
        const int shape_cv_stride =
            (shape_is_rat) ? (shape_dim + 1) : shape_dim;
        ON_SimpleArray<double> meta_point(curve_count * shape_cv_stride *
                                          shape_order);
        ...
        if (meta_point.Count() == curve_count * shape_cv_stride * shape_order) {
            ...
            rc = bez.Loft(shape_cv_stride * shape_dim, curve_count,
                          shape_cv_stride * shape_dim, meta_point.Array(), 1,
                          t.Array())
                     ? true
                     : false;
            if (rc) {
...
bool ON_BezierSurface::GetBBox( // returns true if successful
    ...
    for (i = 0; rc && i < m_order[0]; i++) {
        rc = ON_GetPointListBoundingBox(m_dim, m_is_rat, m_order[1],
                                        m_cv_stride[1], CV(i, 0), boxmin,
                                        boxmax, bGrowBox);
        bGrowBox = true;
    }
    return rc;
...
bool ON_BezierSurface::Transform(const ON_Xform &xform) {
    ...
    if (rc) {
        ...
        for (i = 0; rc && i < m_order[0]; i++) {
            rc = ON_TransformPointList(m_dim, m_is_rat, m_order[1],
                                       m_cv_stride[1], CV(i, 0), xform);
        }
    }
    return rc;
}

bool ON_BezierSurface::Rotate(double sin_angle,        // sin(angle)
                              double cos_angle,        // cos(angle)
                              const ON_3dVector &axis, // axis of rotation
                              const ON_3dPoint &center // center of rotation
) {
...
bool ON_BezierSurface::Rotate(double angle,            // angle in radians
                              const ON_3dVector &axis, // axis of rotation
                              const ON_3dPoint &center // center of rotation
) {
    return Rotate(sin(angle), cos(angle), axis, center);
...
bool ON_BezierSurface::Reverse(int dir) {
    ...
    if (dir > 0) {
        for (i = 0; rc && i < m_order[0]; i++) {
            rc = ON_ReversePointList(m_dim, m_is_rat, m_order[1],
                                     m_cv_stride[1], CV(i, 0));
        }
        // m_domain[1].Reverse();
    } else {
        for (i = 0; rc && i < m_order[1]; i++) {
            rc = ON_ReversePointList(m_dim, m_is_rat, m_order[0],
                                     m_cv_stride[0], CV(0, i));
        }
        // m_domain[0].Reverse();
    }
    return rc;
...
bool ON_BezierSurface::Evaluate( // returns false if unable to evaluate
    ...
    // TODO: When time permits write a faster special case bezier surface
    // evaluator. For now, cook up some fake knot vectors and use NURBS surface
    // span evaluator.
...
ON_BezierCurve *ON_BezierSurface::IsoCurve(int dir, double t,
                                           ON_BezierCurve *pCrv) const {
    if (pCrv == NULL) {
    ...
    if (m_cv_stride[1 - dir] > m_cv_stride[dir]) {
        ...
        workspace = (double *)onmalloc((bigdim * m_order[1 - dir]) *
                                       sizeof(*workspace));
        ...
        for (i = 0; i < m_order[1 - dir]; i++) {
            ...
            for (j = 0; j < m_order[dir];
                 j++, dst += cvsize, src += m_cv_stride[dir])
                memcpy(dst, src, cvsize_bytes);
        }
    }

    ON_EvaluateBezier(bigdim, 0, m_order[1 - dir], stride, cv, 0.0, 1.0, 0, t,
                      bigdim, pCrv->m_cv);
...
bool ON_BezierSurface::GetNurbForm(ON_NurbsSurface &n) const {
    bool rc = false;
    if (n.Create(m_dim, m_is_rat, m_order[0], m_order[1], m_order[0],
                 m_order[1])) {
        if (n.m_cv == m_cv) {
...
bool ON_BezierSurface::IsRational() const { return m_is_rat ? true : false; }
...
bool ON_BezierSurface::SetCV(int i, int j, ON::point_style style,
                             const double *Point) {
    bool rc = true;
    int k;
    double w;
...
bool ON_BezierSurface::GetCV(int i, int j, ON::point_style style,
                             double *Point) const {
...
bool ON_BezierSurface::Trim(int dir, const ON_Interval &domain) {
    bool rc = false;
    ON_BezierCurve crv;
    double *cv;
...
bool ON_BezierSurface::Split(
    ...
    double t, // t = splitting parameter must 0 < t < 1
    ON_BezierSurface
        &left_bez, // west/south side returned here (can pass *this)
    ON_BezierSurface
        &right_bez // east/north side returned here (can pass *this)
) const {
    bool rc = false;
    if (0.0 < t && t < 1.0) {
        double *crvcv;
        ...
        if (rc) {
            ...
            right_bez.m_cv_stride[1 - dir] = left_bez.m_cv_stride[1 - dir] =
                hdim;
            left_bez.m_cv_stride[dir] =
                leftcrv.m_cv_stride; // 3 March 2005 - Dale Lear changed crvdim
                                     // to m_cv_stride
...
bool ON_BezierSurface::IsSingular( // true if surface side is collapsed to a
                                   // point
...
bool ON_ReparameterizeRationalBezierCurve(double c, int dim, int order,
...
bool ON_BezierCurve::Reparametrize(double c) { return Reparameterize(c); }
...
bool ON_BezierCurve::Reparameterize(double c) {
    ...
    return ON_ReparameterizeRationalBezierCurve(c, m_dim, m_order, m_cv_stride,
                                                m_cv);
...
bool ON_ChangeRationalBezierCurveWeights(int dim, int order, int cvstride,
                                         double *cv, int i0, double w0, int i1,
                                         double w1) {
    // Reference - Farauki
    double r, s, v0, v1;
    int i, j;
...
bool ON_BezierCurve::ChangeWeights(int i0, double w0, int i1, double w1) {
    ...
    MakeRational();
    return ON_ChangeRationalBezierCurveWeights(m_dim, m_order, m_cv_stride,
                                               m_cv, i0, w0, i1, w1);
...
bool ON_BezierCurve::EvTangent(double t, ON_3dPoint &point,
        ...
        if (Ev2Der(t, point, D1, D2)) {
            ...
            if (rc) {
                ...
                if (domain.IsIncreasing() &&
                    ON_GetParameterTolerance(domain[0], domain[1], t, &tminus,
                                             &tplus)) {
                    ON_3dPoint p;
                    ON_3dVector d1, d2;
                    double eps = 0.0;
                    ...
                    if (negative_count > 0 &&
                        test_count == negative_count + zero_count) {
...
bool ON_BezierCurve::EvCurvature(double t, ON_3dPoint &point,
                                 ON_3dVector &tangent,
...
bool ON_BezierCurve::Ev1Der( // returns false if unable to evaluate
    double t,                // evaluation parameter
    ON_3dPoint &point, ON_3dVector &derivative) const {
    bool rc = false;
    const int dim = Dimension();
    double ws[2 * 64];
    double *v;
...
bool ON_BezierCurve::Ev2Der( // returns false if unable to evaluate
    double t,                // evaluation parameter
    ON_3dPoint &point, ON_3dVector &firstDervative,
