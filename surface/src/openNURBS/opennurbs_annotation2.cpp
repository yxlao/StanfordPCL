...
const ON_TextExtra *ON_TextExtra::TextExtension(const ON_TextEntity2 *pText,
                                                bool bCreate) {
...
ON_BOOL32 ON_TextExtra::Read(ON_BinaryArchive &archive) {
    int major_version = 1;
    int minor_version = 0;
    bool rc = archive.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &major_version,
                                        &minor_version);
    if (!rc)
        return false;
    if (major_version != 1)
...
ON_UUID ON_TextExtra::ParentUUID() const { return m_parent_uuid; }

void ON_TextExtra::SetParentUUID(ON_UUID parent_uuid) {
    m_parent_uuid = parent_uuid;
}

bool ON_TextExtra::DrawTextMask() const { return m_bDrawMask; }

void ON_TextExtra::SetDrawTextMask(bool bDraw) { m_bDrawMask = bDraw; }

int ON_TextExtra::MaskColorSource() const { return m_color_source; }
...
ON_Color ON_TextExtra::MaskColor() const { return m_mask_color; }

void ON_TextExtra::SetMaskColor(ON_Color color) { m_mask_color = color; }

double ON_TextExtra::MaskOffsetFactor() const { return m_border_offset; }
...
static ON_DimensionExtra *AnnotationExtension(ON_Annotation2 *pDim,
    ...
    if (pDim) {
        pExtra = ON_DimensionExtra::Cast(pDim->GetUserData(
            ON_DimensionExtra::m_ON_DimensionExtra_class_id.Uuid()));
        if (pExtra == 0 && bCreate) {
...
ON_DimensionExtra::DimensionExtension(const ON_LinearDimension2 *pDim,
                                      bool bCreate) {
...
const ON_DimensionExtra *
ON_DimensionExtra::DimensionExtension(const ON_RadialDimension2 *pDim,
                                      bool bCreate) {
    return DimensionExtension((ON_RadialDimension2 *)pDim, bCreate);
}

ON_DimensionExtra *
ON_DimensionExtra::DimensionExtension(ON_OrdinateDimension2 *pDim,
                                      bool bCreate) {
    return AnnotationExtension((ON_Annotation2 *)pDim, bCreate);
}

const ON_DimensionExtra *
ON_DimensionExtra::DimensionExtension(const ON_OrdinateDimension2 *pDim,
                                      bool bCreate) {
...
ON_BOOL32 ON_DimensionExtra::Write(ON_BinaryArchive &archive) const {
    int major_version = 1;
    int minor_version = 1;
    bool rc = archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK, major_version,
                                         minor_version);
...
ON_UUID ON_DimensionExtra::ParentUUID() const { return m_partent_uuid; }

void ON_DimensionExtra::SetParentUUID(ON_UUID partent_uuid) {
    m_partent_uuid = partent_uuid;
}

int ON_DimensionExtra::ArrowPosition() const { return m_arrow_position; }
...
double ON_DimensionExtra::DistanceScale() const { return m_distance_scale; }

void ON_DimensionExtra::SetDistanceScale(double s) { m_distance_scale = s; }
...
void ON_DimensionExtra::SetAlternateToleranceUpperString( const wchar_t*
upper_string)
{
  m_alt_upper_string = upper_string;
}

void ON_DimensionExtra::SetAlternateToleranceUpperString( ON_wString&
upper_string)
{
  m_alt_upper_string = upper_string;
}
...
void ON_DimensionExtra::SetAlternateToleranceLowerString( const wchar_t*
lower_string)
{
  m_alt_lower_string = lower_string;
}

void ON_DimensionExtra::SetAlternateToleranceLowerString( ON_wString&
lower_string)
{
  m_alt_lower_string = lower_string;
}
...
ON_OBJECT_IMPLEMENT(ON_TextDot, ON_Geometry,
...
int ON_Annotation2::Index() const { return m_index; }

void ON_Annotation2::SetIndex(int index) { m_index = index; }
...
bool ON_Annotation2::EvaluatePoint(const ON_ObjRef &objref,
                                   ON_3dPoint &P) const {
    bool rc = false;
...
ON_BOOL32 ON_Annotation2::IsValid(ON_TextLog *text_log) const {
    ...
    default:
        if (text_log) {
            text_log->Print(
                "ON_Annotation2 - m_type = %d is not a valid enum value\n",
                m_type);
        }
        return false;
        break;
...
ON_BOOL32 ON_Annotation2::Write(ON_BinaryArchive &file) const {
    ...
    if (bInChunk) {
        ...
        //   I incremented chunk version to 1.2 and wrote the TextFormaula()
        //   string.
    ...
    while (rc) {
        ...
        if (file.Archive3dmVersion() <= 4 && m_type == ON::dtTextBlock) {
            ...
            if (m_justification & tjBottom) {
                if (lines > 1) {
                    ON_3dPoint p =
                        plane.PointAt(0.0, height * (lines - 1) * linefeed);
                    plane.SetOrigin(p);
                }
        ...
        // 28 Aug 2010 - Lowell - Added flag for whether text gets scaled in
        // modelspace
...
ON_BOOL32 ON_Annotation2::Read(ON_BinaryArchive &file) {
    ...
    bool bInChunk = (file.Archive3dmVersion() >= 5 &&
                     file.ArchiveOpenNURBSVersion() >= 200710180);
...
int ON_Annotation2::Dimension() const { return 3; }
...
ON_BOOL32 ON_AngularDimension2::Transform(const ON_Xform &xform) {
    ...
    if (!rc) {
        ...
        if (dim_pt_count == m_points.Count() && plane.Transform(xform)) {
            ...
            a[1].Set(m_radius * cos(0.5 * (a0 + a1)),
                     m_radius * sin(0.5 * (a0 + a1)));
                ...
                if (bUpdatePoints) {
                    ...
                    if (rc) {
                        ...
                        if (m_angle > 0.0 && m_points[arc_pt_index].IsValid()) {
                            arc_pt_angle = atan2(m_points[arc_pt_index].y,
                                                 m_points[arc_pt_index].x);
                            if (arc_pt_angle < 0.0)
...
bool ON_Annotation2::IsText() const { return (ON::dtTextBlock == m_type); }

bool ON_Annotation2::IsLeader() const { return (ON::dtLeader == m_type); }

bool ON_Annotation2::IsDimension() const {
    return (ON::dtTextBlock != m_type && ON::dtLeader != m_type);
}

double ON_Annotation2::NumericValue() const { return ON_UNSET_VALUE; }
...
double ON_Annotation2::Height() const { return m_textheight; }
...
ON::eAnnotationType ON_Annotation2::Type() const { return m_type; }

void ON_Annotation2::SetPlane(const ON_Plane &plane) { m_plane = plane; }

const ON_Plane &ON_Annotation2::Plane() const { return m_plane; }
...
int ON_Annotation2::PointCount() const { return m_points.Count(); }

void ON_Annotation2::SetPoints(const ON_2dPointArray &points) {
    m_points = points;
}

const ON_2dPointArray &ON_Annotation2::Points() const { return m_points; }
...
ON_2dPoint ON_Annotation2::Point(int idx) const {
    return (idx >= 0 && idx < m_points.Count()) ? m_points[idx]
                                                : ON_2dPoint(0.0, 0.0);
}
...
bool ON_Annotation2::UserPositionedText() const { return m_userpositionedtext; }
...
ON_BOOL32 ON_LinearDimension2::IsValid(ON_TextLog *text_log) const {
    if (m_type != ON::dtDimLinear && m_type != ON::dtDimAligned) {
        if (text_log) {
            text_log->Print("ON_LinearDimension2 - m_type !=  ON::dtDimLinear "
                            "or ON::dtDimAligned.\n");
        }
        return false;
    }
    ...
    if (m_points[1].x != m_points[0].x) {
        if (text_log) {
            text_log->Print("ON_LinearDimension2 - m_points[1].x = %g != %g = "
                            "m_points[0].x (should be equal)\n",
    ...
    if (m_points[3].x != m_points[2].x) {
        if (text_log) {
            text_log->Print("ON_LinearDimension2 - m_points[3].x = %g != %g = "
                            "m_points[2].x\n",
    ...
    if (m_points[3].y != m_points[1].y) {
        if (text_log) {
            text_log->Print("ON_LinearDimension2 - m_points[3].y = %g != %g = "
                            "m_points[1].y\n",
...
ON_BOOL32 ON_LinearDimension2::GetBBox(double *boxmax, double *boxmin,
...
int ON_LinearDimension2::Repair() {
    ...
    if (m_points.Count() >= dim_pt_count && m_points[ext0_pt_index].IsValid() &&
        ...
        if (ON::dtDimAligned == m_type && (m_points[ext1_pt_index].x < 0.0 ||
                                           m_points[ext1_pt_index].y != 0.0)) {
            rc = 2;
            // Aligned dims must have m_points[ext1_pt_index].x =
            // m_points[ext0_pt_index].x
            if (m_points[ext1_pt_index].x > 100.0 * ON_SQRT_EPSILON &&
                ...
                for (i = 0; i < dim_pt_count; i++) {
                    ...
                    m_points[i].Set(v.x * p.x - v.y * p.y,
                                    v.y * p.x + v.x * p.y);
                }
                m_points[ext0_pt_index].Set(0.0, 0.0);
        ...
        if (!ON_IsValid(m_points[arrow0_pt_index].y)) {
            ...
            if (!ON_IsValid(m_points[arrow1_pt_index].y))
                m_points[arrow1_pt_index].y = 0.5 * (m_points[ext0_pt_index].y +
                                                     m_points[ext1_pt_index].y);
        ...
        if (!m_userpositionedtext) {
            if (m_points[userpositionedtext_pt_index].y !=
                    m_points[arrow0_pt_index].y ||
                m_points[userpositionedtext_pt_index].x !=
                    0.5 * (m_points[arrow0_pt_index].x +
                           m_points[arrow1_pt_index].x)) {
                rc = 2;
                m_points[userpositionedtext_pt_index].y =
                    m_points[arrow0_pt_index].y;
                m_points[userpositionedtext_pt_index].x =
                    0.5 *
                    (m_points[arrow0_pt_index].x + m_points[arrow1_pt_index].x);
            }
        }
...
int ON_LinearDimension2::StyleIndex() const { return ON_Annotation2::Index(); }

void ON_LinearDimension2::SetStyleIndex(int i) { ON_Annotation2::SetIndex(i); }
...
ON_2dPoint ON_LinearDimension2::Dim2dPoint(int point_index) const {
    ...
    if (m_points.Count() < dim_pt_count) {
        ...
        if (text_pivot_pt == point_index) {
            point_index =
                m_userpositionedtext ? userpositionedtext_pt_index : dim_mid_pt;
        }
...
ON_3dPoint ON_LinearDimension2::Dim3dPoint(int point_index) const {
    ...
    return (ON_UNSET_VALUE == p2.x) ? ON_UNSET_POINT
                                    : m_plane.PointAt(p2.x, p2.y);
}
...
bool ON_LinearDimension2::CreateFromV2(const ON_Annotation &v2_ann,
    ...
    if (ON::dtDimLinear == v2_ann.m_type || ON::dtDimAligned == v2_ann.m_type) {
        if (v2_ann.m_points.Count() >= ON_LinearDimension2::dim_pt_count) {
            m_points.Reserve(ON_LinearDimension2::dim_pt_count);
            m_points.SetCount(0);
            m_points.Append(ON_LinearDimension2::dim_pt_count,
                            v2_ann.Points().Array());
...
int ON_LinearDimension2::GetDimensionLineSegments(
    ON_RECT gdi_text_rect, int gdi_height_of_I, ON_Xform gdi_to_world,
    ...
    const double x1 =
        m_points[(ext0_pt_index == i) ? ext1_pt_index : ext0_pt_index].x;
    ...
    ON::eTextDisplayMode textdisplay =
    ...
    int ForceArrows = 0;
    const ON_DimensionExtra *pDE =
        ON_DimensionExtra::DimensionExtension(this, false);
    if (pDE)
    ...
    if (ForceArrows == 1) // force inside
    {
        rc = 1;
               ...
               dimwidth < mindimwidth) // arrowheads have to be "outside" - they
                                       // won't fit inside even without text
    {
    ...
    // Horizontal text or Userpositioned text
        ...
        R.Set(gdi_text_rect.left - gdi_gap, gdi_text_rect.bottom + gdi_gap,
              0.0);
        ...
        R.Set(gdi_text_rect.right + gdi_gap, gdi_text_rect.bottom + gdi_gap,
              0.0);
        ray.from = gdi_to_world * R;
        ray.to = ray.from + vp_zaxis;
        P = (ON_Intersect(ray, m_plane, &t)) ? ray.PointAt(t) : ray.from;
        ...
        for (i = 0; i < 4; i++) {
            y0 = corners[i].y -
                 m_points[1].y; // vertical dist from corner to dimension line
            y1 = corners[(i + 1) % 4].y -
                 m_points[1]
                     .y; // vertical dist from next corner to dimension line
            // if they're both above or both below, no intersection of this
            // segment
        ...
        // Nov 4 2009 - Lowell - Added test for no intersection of text with
        // dimension line rr33003 Important for user positioned text that's
        // moved out of the way of the dimension line
...
ON_BOOL32 ON_RadialDimension2::IsValid(ON_TextLog *text_log) const {
    if (m_type != ON::dtDimRadius && m_type != ON::dtDimDiameter) {
        if (text_log) {
            text_log->Print("ON_RadialDimension2 - m_type !=  ON::dtDimRadius "
                            "or ON::dtDimDiameter\n");
        }
        return false;
    }
    ...
    if (4 != m_points.Count()) {
        if (text_log) {
            text_log->Print("ON_RadialDimension2 - m_points.Count() = %d "
                            "(should be 4 or 5)\n",
                            m_points.Count());
        }
        return false;
...
ON_BOOL32 ON_RadialDimension2::GetBBox(double *boxmax, double *boxmin,
                                       ON_BOOL32 bGrowBox) const {
    ON_BoundingBox bbox;
    if (bGrowBox) {
        bbox.m_min.x = boxmin[0];
...
bool ON_RadialDimension2::CreateFromPoints(ON_3dPoint center,
    ...
    bool rc = false;
    if (center.IsValid() && arrowtip.IsValid() && normal.IsValid() &&
        !normal.IsZero() && xaxis.IsValid() && !xaxis.IsZero()) {
        ...
        if (c != 0.0 || s != 0.0) {
            ...
            if (m_plane.ClosestPointTo(arrowtip, &tip.x, &tip.y)) {
                ...
                m_points[knee_pt_index] =
                    m_points[arrow_pt_index] + offset_distance * tip;
...
int ON_RadialDimension2::StyleIndex() const { return ON_Annotation2::Index(); }

void ON_RadialDimension2::SetStyleIndex(int i) { ON_Annotation2::SetIndex(i); }
...
bool ON_RadialDimension2::CreateFromV2(const ON_Annotation &v2_ann,
                                       ...
                                       int dimstyle_index) {
    bool rc = false;
    if (ON::dtDimRadius == v2_ann.m_type ||
        ON::dtDimDiameter == v2_ann.m_type) {
        ...
        if (points.Count() >= 4) {
            ...
            m_type = v2_ann.Type();
            m_textdisplaymode =
                (2 == settings.m_textalign) ? ON::dtHorizontal : ON::dtInLine;
            m_index = dimstyle_index;
            ON_2dVector v = m_points[0];
            if (!v.IsZero()) {
                m_plane.origin = m_plane.PointAt(v.x, v.y);
...
ON_AngularDimension2Extra::AngularDimensionExtra(ON_AngularDimension2 *pDim) {
    ...
    if (pDim) {
        pExtra = ON_AngularDimension2Extra::Cast(pDim->GetUserData(
            ON_AngularDimension2Extra::m_ON_AngularDimension2Extra_class_id
                .Uuid()));
        if (pExtra == 0) {
...
ON_AngularDimension2Extra::AngularDimensionExtra(
    const ON_AngularDimension2 *pDim) {
...
ON_BOOL32 ON_AngularDimension2::IsValid(ON_TextLog *text_log) const {
    ...
    if (0.0 == m_points[1].x && 0.0 == m_points[1].y) {
        if (text_log) {
            text_log->Print("ON_AngularDimension2 - angle dim m_points[1] = "
                            "center (should be on start ray).\n");
        }
        return false;
    }
    ...
    if (0.0 == m_points[2].x && 0.0 == m_points[2].y) {
        if (text_log) {
            text_log->Print("ON_AngularDimension2 - angle dim m_points[2] = "
                            "center (should be on end ray).\n");
        }
        return false;
    }
    ...
    if (0.0 == m_points[3].x && 0.0 == m_points[3].y) {
        if (text_log) {
            text_log->Print("ON_AngularDimension2 - angle dim m_points[3] = "
                            "center (should be on interior of arc).\n");
        }
        return false;
    }
    ...
    while (a2 <= a1)
        a2 += 2.0 * ON_PI;
    while (a3 < a1) // Oct 23 2009 LW changed from a3 <= a1 to allow point at
                    // end of arc rr53634
        a3 += 2.0 * ON_PI;

    if (fabs(m_angle - (a2 - a1)) >
        ON_ZERO_TOLERANCE + m_angle * ON_SQRT_EPSILON) {
        if (text_log) {
            text_log->Print("ON_AngularDimension2 - m_angle = %g != %g = (end "
                            "angle - start angle)\n",
                            m_angle, a2 - a1);
        }
        return false;
    ...
    if (a3 > a2) // Oct 23 2009 LW changed from a3 >= a2 to allow point at end
                 // of arc rr53634
    {
        if (text_log) {
            text_log->Print("ON_AngularDimension2 - angle dim m_points[3] = "
...
ON_BOOL32 ON_AngularDimension2::GetBBox(double *boxmax, double *boxmin,
                                        ON_BOOL32 bGrowBox) const {
    ON_BoundingBox bbox;
    if (bGrowBox) {
        bbox.m_min.x = boxmin[0];
...
ON_2dPoint ON_AngularDimension2::Dim2dPoint(int point_index) const {
    ...
    if (m_points.Count() < dim_pt_count || point_index < 0) {
        ...
        if (text_pivot_pt == point_index) {
            point_index =
                m_userpositionedtext ? userpositionedtext_pt_index : arcmid_pt;
        }
...
bool ON_AngularDimension2::CreateFromV2(
    const ON_Annotation &v2_ann, const ON_3dmAnnotationSettings &settings,
    int dimstyle_index) {
...
bool ON_AngularDimension2::GetArc(ON_Arc &arc) const {
    ...
    if (ON_IsValid(m_radius) && m_radius > ON_SQRT_EPSILON &&
        ...
        m_plane.origin.IsValid() && m_plane.xaxis.IsValid() &&
        m_plane.yaxis.IsValid() && m_plane.zaxis.IsValid() &&
        fabs(m_plane.zaxis.Length() - 1.0) <= ON_SQRT_EPSILON &&
        4 == m_points.Count()) {
        ON_3dVector X = m_plane.PointAt(m_points[start_pt_index].x,
                                        m_points[start_pt_index].y) -
                        m_plane.origin;
...
bool ON_AngularDimension2::GetExtensionLines(ON_Line extensions[2]) const {
    ...
    if (ON_IsValid(m_radius) && m_radius > ON_SQRT_EPSILON &&
        ...
        if (pDE != 0) {
            ...
            e00 = m_plane.PointAt(m_points[start_pt_index].x,
                                  m_points[start_pt_index].y);
            e10 = m_plane.PointAt(m_points[end_pt_index].x,
                                  m_points[end_pt_index].y);
...
bool ON_AngularDimension2::CreateFromPoints(const ON_3dPoint &pc,
...
void ON_AngularDimension2::SetAngle(double angle) { m_angle = angle; }

double ON_AngularDimension2::Angle() const { return m_angle; }

void ON_AngularDimension2::SetRadius(double radius) { m_radius = radius; }

double ON_AngularDimension2::Radius() const { return m_radius; }
...
int ON_AngularDimension2::StyleIndex() const { return ON_Annotation2::Index(); }

void ON_AngularDimension2::SetStyleIndex(int i) { ON_Annotation2::SetIndex(i); }
...
void ON_AngularDimension2::ConvertBack(
    ON_AngularDimension2 & // target - formal parameter intentionally ignored in
...
int ON_AngularDimension2::GetDimensionArcSegments(
    ...
    // June 7, 2010 - Lowell - Added some special handling for very small
    // dimensions
    ...
    if (m_radius <= arrowwidth + tailwidth ||
        ...
        // arc is tiny with respect to arrowhead size - arrowheads have to be
        // "outside" 2 arc segments, arrowheads outside
        a[0] = a0;
        a[1] = a0 - arrow_angle;
        a[2] = a1 + arrow_angle;
        a[3] = a1;
    ...
    if (ON::dtInLine == textdisplay &&
        m_radius <= 0.5 * (textwidth + 2.0 * textgap)) {
    ...
    const double textangle =
        (ON::dtInLine == textdisplay && !m_userpositionedtext)
            ? 2.0 * asin(sin_angle)
            : 0.0;
    ...
    if ((ON::dtHorizontal == textdisplay && vp) || m_userpositionedtext) {
        ...
        if (ON_UNSET_VALUE != aa0 && ON_UNSET_VALUE != aa1 && a0 <= aa0 &&
            aa0 < aa1 && aa1 <= a1) {
            t = arrow_angle;
...
ON_2dPoint ON_OrdinateDimension2::Dim2dPoint(int point_index,
    ...
    int dir = m_direction;
    if (dir == -1 &&
        (point_index == offset_pt_0 || point_index == offset_pt_1)) {
        if (fabs(m_points[definition_pt_index].y -
                 m_points[leader_end_pt_index].y) >
            fabs(m_points[definition_pt_index].x -
                 m_points[leader_end_pt_index].x))
            dir = 0;
        else
            dir = 1;
    }
    ...
    if (point_index >= 0 && point_index < dim_pt_count &&
        ...
        if (dir == x) {
            ...
            if (m_points[leader_end_pt_index].y >
                m_points[definition_pt_index].y)
                p2.y = m_points[leader_end_pt_index].y - offset0 - offset1;
            else
                p2.y = m_points[leader_end_pt_index].y + offset0 + offset1;
        } else if (dir == y) {
            p2.y = m_points[definition_pt_index].y;
            if (m_points[leader_end_pt_index].x >
                m_points[definition_pt_index].x)
...
ON_BOOL32 ON_OrdinateDimension2::IsValid(ON_TextLog *text_log) const {
    ...
    if (m_points.Count() != 2) {
        if (text_log) {
            text_log->Print("ON_OrdinateDimension2 - m_points.Count() = %d "
                            "(should be 2).\n",
                            m_points.Count());
        }
        return false;
...
ON_BOOL32 ON_OrdinateDimension2::Read(ON_BinaryArchive &file) {
    ...
    if (rc) {
        if (1 != major_version) {
            ...
            rc = file.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK,
                                        &submajor_version, &subminor_version);
            if (rc) {
...
ON_BOOL32 ON_OrdinateDimension2::GetBBox(double *boxmin, double *boxmax,
                                         ON_BOOL32 bGrowBox) const {
    ON_BoundingBox bbox;
    if (bGrowBox) {
        bbox.m_min.x = boxmin[0];
...
int ON_OrdinateDimension2::Direction() const { return m_direction; }
...
ON_BOOL32 ON_TextEntity2::IsValid(ON_TextLog *text_log) const {
    ...
    // character with a value greater than 32 (the value of space) is
    // "printable."
    ...
    if (!bValidText) {
        if (text_log) {
            text_log->Print("ON_TextEntity2 - m_usertext does not contain "
    ...
    if (0 != m_points.Count()) {
        if (text_log) {
            text_log->Print(
                "ON_TextEntity2 - m_points.Count() = %d (should be 0)\n",
                m_points.Count());
        }
        return false;
    }
...
ON_BOOL32 ON_TextEntity2::GetBBox(double *boxmax, double *boxmin,
                                  ON_BOOL32 bGrowBox) const {
    ON_BoundingBox bbox;
    if (bGrowBox) {
        bbox.m_min.x = boxmin[0];
...
int ON_TextEntity2::FontIndex() const { return m_index; }

void ON_TextEntity2::SetFontIndex(int i) { m_index = i; }
...
unsigned int ON_TextEntity2::Justification() { return m_justification; }
...
bool ON_TextEntity2::AnnotativeScaling() const { return m_annotative_scale; }

void ON_TextEntity2::SetAnnotativeScaling(bool b) { m_annotative_scale = b; }
...
ON_BOOL32 ON_Leader2::GetBBox(double *boxmax, double *boxmin,
                              ON_BOOL32 bGrowBox) const {
    ON_BoundingBox bbox;
    if (bGrowBox) {
        bbox.m_min.x = boxmin[0];
...
bool ON_Leader2::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
                                     const ON_Xform *xform) const {
    const int point_count = m_points.Count();
    if (point_count >= 2) {
        ON_3dPointArray P(point_count);
...
void ON_Leader2::AddPoint(const ON_2dPoint &point) { m_points.Append(point); }
...
// April 22, 2010 Lowell - Added to support right justified text on left
// pointing leader tails rr64292
...
bool ON_RadialDimension2::GetArrowHeadDirection(
    ON_2dVector &arrowhead_dir) const {
    bool rc = false;
    if (m_points.Count() < 4) {
        arrowhead_dir.Set(-1.0, 0.0);
    } else {
...
ON_TextDot::ON_TextDot()
    : m_point(ON_origin), m_height(14), m_text(L'1'), m_fontface(L"Arial bold"),
      m_display(0) {}
...
ON_BOOL32 ON_TextDot::IsValid(ON_TextLog *text_log) const {
...
ON::object_type ON_TextDot::ObjectType() const { return ON::text_dot; }

int ON_TextDot::Dimension() const { return 3; }

ON_BOOL32 ON_TextDot::GetBBox(double *box_min, double *box_max,
...
bool ON_TextDot::IsDeformable() const { return true; }

bool ON_TextDot::MakeDeformable() { return true; }

const ON_3dPoint &ON_TextDot::Point() const { return m_point; }

void ON_TextDot::SetPoint(const ON_3dPoint &point) { m_point = point; }

int ON_TextDot::Height() const { return m_height; }
...
bool ON_TextDot::AlwaysOnTop() const { return (m_display & 1) == 1; }
...
bool ON_TextDot::Transparent() const { return (m_display & 2) == 2; }
...
bool ON_TextDot::Bold() const { return (m_display & 4) == 4; }
...
bool ON_TextDot::Italic() const { return (m_display & 8) == 8; }
...
bool ON_Annotation2::GetTextXform(ON_RECT gdi_text_rect, const ON_Font &font,
                                  const ON_DimStyle &dimstyle, double dimscale,
                                  const ON_Viewport *vp,
                                  ON_Xform &xform) const {
    ON_ERROR("This function should not be used. Use the version that takes a "
             "model transform argument.");
    return false;
...
bool ON_Annotation2::GetTextXform(ON_RECT gdi_text_rect, const ON_Font &font,
    ...
    const ON::eTextDisplayMode textalignment =
        dimstyle ? ON::TextDisplayMode(dimstyle->TextAlignment())
                 : ON::dtNormal;
    const ON_3dVector cameraX = (vp) ? vp->CameraX() : m_plane.xaxis;
    const ON_3dVector cameraY = (vp) ? vp->CameraY() : m_plane.yaxis;
    if (dimstyle) {
        ...
        // multi-line tolerance text since GetTextXform doesn't do that.
        if ((dimstyle->ToleranceStyle() == 2 ||
             dimstyle->ToleranceStyle() == 3) &&
            (Type() == ON::dtDimLinear || Type() == ON::dtDimAligned))
            textgap += textheight * 0.5;
    }
    return GetTextXform(gdi_text_rect, gdi_height_of_I, textheight, textgap,
                        textalignment, dimscale, cameraX, cameraY, model_xform,
                        xform);
}

static bool GetLeaderEndAndDirection(const ON_Annotation2 *pAnn, ON_2dPoint &E,
                                     ON_2dVector &R) {
    bool rc = false;
...
// SDKBREAK Oct 30, 07 - LW
// This function should not be used any longer
bool ON_Annotation2::GetTextXform(ON_RECT gdi_text_rect, int gdi_height_of_I,
                                  double dimstyle_textheight,
                                  double dimstyle_textgap,
                                  ON::eTextDisplayMode dimstyle_textalignment,
                                  double dimscale, ON_3dVector cameraX,
                                  ON_3dVector cameraY, ON_Xform &xform) const {
    ON_ERROR("This function should not be used. Use the version that takes a "
             "model transform argument.");
    return false;
    ...
    //    const ON_AngularDimension2* angular_dim =
    //    ON_AngularDimension2::Cast(ann); if ( 0 != angular_dim )
    //    {
    ...
    //      if ( y < 0.0 )
    //      {
    //        text_centering_rotation.Reverse(); // add another 180 degrees of
    //        rotation
    //      }
    ...
    //    ON_2dVector R(1.0,0.0); // unit vector from penultimate point to end
    //    point GetLeaderEndAndDirection( this, E, R );

    //    text_centering_rotation = R;

    //    text_centering_translation = (dimstyle_textgap +
    //    0.5*text_line_width)*text_centering_rotation;

    //    ON_3dVector V = text_centering_rotation.x*m_plane.xaxis +
    //    text_centering_rotation.y*m_plane.yaxis; x = V*cameraX; y = V*cameraY;
    ...
    // ON_2dVector text_offset_translation(0.0,0.0); // CRhinoText::Offset() =
    // text->Offset() switch( ann_type )
    //{
    // case ON::dtDimLinear:
    // case ON::dtDimAligned:
    //  if ( m_points.Count() >= ON_LinearDimension2::dim_pt_count )
    //  {
    //    const ON_LinearDimension2* linear_dim =
    //    ON_LinearDimension2::Cast(ann); if ( linear_dim )
    //    {
    ...
    //  {
    //    const ON_AngularDimension2* angular_dim =
    //    ON_AngularDimension2::Cast(ann); if ( angular_dim )
    //    {
    //      text_offset_translation =
    ...
    //    text_offset_translation =
    //    m_points[ON_RadialDimension2::tail_pt_index];
    //  }
    //  break;
    ...
    //    ON_2dPoint E(0.0,0.0); // end point
    //    ON_2dVector R(1.0,0.0); // unit vector from penultimate point to end
    //    point GetLeaderEndAndDirection( this, E, R ); ON_3dVector V =
    //    R.x*m_plane.xaxis + R.y*m_plane.yaxis; x = V*cameraX; y = ( x >
    //    -ON_SQRT_EPSILON )
    //      ? dimstyle_textgap
...
// To use model xform to draw annotation in blocks correctly
bool ON_Annotation2::GetTextXform(
    ON_RECT gdi_text_rect, int gdi_height_of_I, double dimstyle_textheight,
    ...
    double textheight = (ON::dtTextBlock == ann_type) ? m_textheight * dimscale
                                                      : dimstyle_textheight;
    if (0.0 == textheight)
        textheight = 1.0;
    ...
    if (ON::dtHorizontal != dimstyle_textalignment) {
        if (((cameraZ * m_plane.zaxis) < -ON_SQRT_EPSILON)) {
            ...
            case 2: // ON::dtDimDiameter, ON::dtDimRadius, ON::dtLeader
                    // flip.m_xform[1][1] = -1.0;
                    // flip.m_xform[1][3] = -(gdi_text_rect.top +
                    // gdi_text_rect.bottom);
                break;
            }
            gdi_to_plane = gdi_to_plane * flip;
        }
    ...
    if (ON::dtHorizontal != dimstyle_textalignment) {
        if (ON::dtDimLinear == ann_type || ON::dtDimAligned == ann_type) {
            if (ON::dtAboveLine == dimstyle_textalignment) {
                text_centering_translation.y =
                    0.5 * textheight + dimstyle_textgap;
            }
            y = ann->m_plane.yaxis * cameraY;
            x = -ann->m_plane.yaxis * cameraX;
            if (fabs(y) <= ON_SQRT_EPSILON && fabs(x) > ON_SQRT_EPSILON) {
            ...
            if (0 != angular_dim) {
                ...
                if (y < 0.0) {
                    text_centering_rotation
                        .Reverse(); // add another 180 degrees of rotation
                }

                if (ON::dtAboveLine == dimstyle_textalignment) {
                    y = 0.5 * textheight + dimstyle_textgap;
                    text_centering_translation.x =
                        -y * text_centering_rotation.y;
                    text_centering_translation.y =
                        y * text_centering_rotation.x;
                }
            }
        } else if (ON::dtDimDiameter == ann_type ||
                   ON::dtDimRadius == ann_type || ON::dtLeader == ann_type ||
                   ON::dtDimOrdinate == ann_type) {
            ON_2dPoint E(0.0, 0.0); // end point
            ON_2dVector R(
                1.0, 0.0); // unit vector from penultimate point to end point
            GetLeaderEndAndDirection(this, E, R);

            text_centering_rotation = R;

            text_centering_translation =
                (dimstyle_textgap + 0.5 * text_line_width) *
                text_centering_rotation;
        ...
        if ((m_justification & tjRight) == tjRight)
            text_centering_translation.Set(
                -(dimstyle_textgap + 0.5 * text_line_width), 0.0);
    ...
    ON_2dVector text_offset_translation(
        0.0, 0.0); // CRhinoText::Offset() = text->Offset()
    switch (ann_type) {
    case ON::dtDimLinear:
    case ON::dtDimAligned:
        if (m_points.Count() >= ON_LinearDimension2::dim_pt_count) {
            const ON_LinearDimension2 *linear_dim =
                ON_LinearDimension2::Cast(ann);
            if (linear_dim) {
                text_offset_translation =
                    linear_dim->Dim2dPoint(ON_LinearDimension2::text_pivot_pt);
            }
    ...
    case ON::dtDimAngular:
        if (m_points.Count() >= ON_AngularDimension2::dim_pt_count) {
            const ON_AngularDimension2 *angular_dim =
                ON_AngularDimension2::Cast(ann);
            if (angular_dim) {
                text_offset_translation = angular_dim->Dim2dPoint(
                    ON_AngularDimension2::text_pivot_pt);
            }
        }
        break;
    ...
    ON_Xform horizonal_xform(1.0);
    if (ON::dtHorizontal == dimstyle_textalignment) {
        ON_3dPoint fixed_point = ann->m_plane.PointAt(
            text_offset_translation.x, text_offset_translation.y);
        horizonal_xform.Rotation(fixed_point, ann->m_plane.xaxis,
                                 ann->m_plane.yaxis, ann->m_plane.zaxis,
                                 fixed_point, cameraX, cameraY, cameraZ);
...
bool ON_Annotation2::GetTextPoint(ON_2dPoint &text_2d_point) const {
    ...
    case ON::dtDimAngular: {
        const ON_AngularDimension2 *angular_dim =
            ON_AngularDimension2::Cast(this);
        if (angular_dim) {
            if (m_userpositionedtext) {
                if (m_points.Count() >= 0) {
                ...
                text_2d_point.x =
                    angular_dim->m_radius * cos(angular_dim->m_angle);
                text_2d_point.y =
                    angular_dim->m_radius * sin(angular_dim->m_angle);
                rc = true;
            }
        }
    } break;
...
#error Never copy this class definition or put this definition in a header file!
  ...
  public:
    ...
    ~ON_AnnotationTextFormula();
    // NO! - do not add IO support to this userdata! // ON_BOOL32
    // Write(ON_BinaryArchive&) const; NO! - do not add IO support to this
    // userdata! // ON_BOOL32 Read(ON_BinaryArchive&);
    ON_BOOL32 GetDescription(ON_wString &);
    // NO! - do not add IO support to this userdata! // ON_BOOL32 Archive()
    // const;
...
ON_AnnotationTextFormula::Get(const ON_Annotation2 *p) {
    return (0 != p) ? ON_AnnotationTextFormula::Cast(p->GetUserData(
                          ON_AnnotationTextFormula::
                              m_ON_AnnotationTextFormula_class_id.Uuid()))
                    : 0;
}
