...
ON_OBJECT_IMPLEMENT(ON_Light, ON_Geometry,
                    "85A08513-F383-11d3-BFE7-0010830122F0");
...
ON::object_type ON_Light::ObjectType() const { return ON::light_object; }

int ON_Light::Dimension() const { return 3; }
...
ON_BOOL32 ON_Light::GetBBox( // returns true if successful
    ...
    case ON::world_rectangular_light:
        ...
        points.Append(m_location + m_width + m_length);
        {
            ...
            marker.Unitize();
            marker *= (m_width + m_length).Length() /
                      12.0; // from GetRectangularLightSegments
...
ON_BOOL32 ON_Light::IsEnabled() const { return m_bOn; }

ON::light_style ON_Light::Style() const { return m_style; }
...
void ON_Light::SetStyle(ON::light_style s) { m_style = s; }
...
const ON_wString &ON_Light::LightName() const { return m_light_name; }
...
void ON_Light::SetAttenuation(const ON_3dVector &att) { m_attenuation = att; }

ON_3dVector ON_Light::Attenuation() const { return m_attenuation; }
...
double ON_Light::SpotAngleDegrees() const { return m_spot_angle; }
...
void ON_Light::SetSpotExponent(double e) {
    ...
    if (e < 0.0 || !ON_IsValid(e))
        ...
        m_spot_exponent = e;
    m_hotspot = ON_UNSET_VALUE; // indicates hotspot should be computed from
                                // m_spot_exponent
}
...
double ON_Light::HotSpot() const {
    ...
    if (h < 0.0 || h > 1.0) {
        ...
        if (m_spot_exponent >= 65536.0)
            h = 0.0;
        else if (m_spot_exponent <= 0.0 || m_spot_angle <= 0.0 ||
                 m_spot_angle > 90.0)
            h = 1.0;
        else {
...
void ON_Light::SetLength(const ON_3dVector &v) { m_length = v; }

ON_3dVector ON_Light::Length() const { return m_length; }

void ON_Light::SetWidth(const ON_3dVector &v) { m_width = v; }

ON_3dVector ON_Light::Width() const { return m_width; }
...
double ON_Light::ShadowIntensity() const { return m_shadow_intensity; }

void ON_Light::SetLightIndex(int i) { m_light_index = i; }

int ON_Light::LightIndex() const { return m_light_index; }

void ON_Light::SetAmbient(ON_Color c) { m_ambient = c; }

void ON_Light::SetDiffuse(ON_Color c) { m_diffuse = c; }
...
ON_Color ON_Light::Ambient() const { return m_ambient; }

ON_Color ON_Light::Diffuse() const { return m_diffuse; }

ON_Color ON_Light::Specular() const { return m_specular; }
...
void ON_Light::SetLocation(const ON_3dPoint &loc) { m_location = loc; }

void ON_Light::SetDirection(const ON_3dVector &dir) { m_direction = dir; }

ON_3dPoint ON_Light::Location() const { return m_location; }

ON_3dVector ON_Light::Direction() const { return m_direction; }
...
ON_3dVector ON_Light::PerpindicularDirection() const {
    ...
    if (IsLinearLight() || IsRectangularLight()) {
        xdir = m_length;
        if (xdir.IsValid() && xdir.Unitize() &&
            fabs(xdir * dir) <= ON_SQRT_EPSILON)
            return xdir;
    }
...
double ON_Light::PowerWatts() const { return m_watts; }

double ON_Light::PowerLumens() const { return m_watts / 683.0; }

double ON_Light::PowerCandela() const { return m_watts / 683.0; }
