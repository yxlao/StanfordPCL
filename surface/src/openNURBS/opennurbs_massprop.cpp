    ...
    if (0 == sMass) {
        ...
        if (m_bValidCentroid)
            dump.Print("Centroid = (%g,%g,%g) (+/- %g,%g,%g)\n", m_x0, m_y0,
                       m_z0, m_x0_err, m_y0_err, m_z0_err);
...
/*
Description:
        QL Algorithm with implict shifts, to determine the eigenvalues and
eigenvectors of a symmetric, tridiagonal matrix.

Parametrers:
        d - [in/out]	On input d[0] to d[n-1] are the diagonal entries of the
matrix. As output d[0] to d[n-1] are the eigenvalues. e - [in/out]  On Input
e[0] to e[n-1] are the off-diagonal entries of the matrix. with e[n-1] not used,
but must be allocated. on output e is unpredictable. n - [in]      matrix is n
by n pV - [out]		If not NULL the it should be an n by n matix. The kth
column will be a normalized eigenvector of d[k]
*/
...
Returns:
  True if successful.
*/
bool ON_SymTriDiag3x3EigenSolver(double A, double B, double C, double D,
                                 double E, double *e1, ON_3dVector &E1,
                                 double *e2, ON_3dVector &E2, double *e3,
                                 ON_3dVector &E3) {
...
Returns:
  True if successful.
*/
bool ON_Sym3x3EigenSolver(double A, double B, double C, double D, double E,
                          double F, double *e1, ON_3dVector &E1, double *e2,
                          ON_3dVector &E2, double *e3, ON_3dVector &E3) {
    ...
    if (F != 0.0) {
        ...
        FF = (cos_phi * cos_phi - sin_phi * sin_phi) * F +
             sin_phi * cos_phi * (A - C);
    }

    double ee1, ee2, ee3;
    ON_3dVector EE1, EE2, EE3;
    bool rc = ON_SymTriDiag3x3EigenSolver(AA, BB, CC, DD, EE, &ee1, EE1, &ee2,
                                          EE2, &ee3, EE3);

    E1.Set(cos_phi * EE1.x - sin_phi * EE1.z, EE1.y,
           sin_phi * EE1.x + cos_phi * EE1.z);
    E2.Set(cos_phi * EE2.x - sin_phi * EE2.z, EE2.y,
           sin_phi * EE2.x + cos_phi * EE2.z);
    E3.Set(cos_phi * EE3.x - sin_phi * EE3.z, EE3.y,
           sin_phi * EE3.x + cos_phi * EE3.z);
    if (e1)
        *e1 = ee1;
    if (e2)
        *e2 = ee2;
...
ON_MassProperties::ON_MassProperties() { Create(); }

ON_MassProperties::~ON_MassProperties() {}

void ON_MassProperties::Create() { memset(this, 0, sizeof(*this)); }
...
ON_Matrix *
ON_MassProperties::WorldCoordIntertiaMatrix(ON_Matrix *matrix) const {
...
bool ON_MassProperties::WorldCoordPrincipalMoments(double *pxx, ON_3dVector &Ax,
    ...
    if (m_bValidSecondMoments && m_bValidProductMoments) {
        rc = ON_Sym3x3EigenSolver(m_world_xx, m_world_yy, m_world_zz,
                                  m_world_xy, m_world_yz, m_world_zx, pxx, Ax,
                                  pyy, Ay, pzz, Az);
    }
    return rc;
}
...
ON_Matrix *
ON_MassProperties::CentroidCoordIntertiaMatrix(ON_Matrix *matrix) const {
    if (m_bValidSecondMoments && m_bValidProductMoments) {
        if (0 != matrix) {
            if (3 != matrix->RowCount() || 3 != matrix->ColCount())
                matrix->Create(3, 3);
...
bool ON_MassProperties::CentroidCoordPrincipalMoments(
    double *pxx, ON_3dVector &Ax, double *pyy, ON_3dVector &Ay, double *pzz,
    ON_3dVector &Az) const {
    bool rc = false;
    if (m_bValidSecondMoments && m_bValidProductMoments) {
        rc =
            ON_Sym3x3EigenSolver(m_ccs_xx, m_ccs_yy, m_ccs_zz, m_ccs_xy,
                                 m_ccs_yz, m_ccs_zx, pxx, Ax, pyy, Ay, pzz, Az);
    }
    return rc;
}

bool ON_MassProperties::Sum(int count, const ON_MassProperties *summands,
                            bool bAddTo) {
    if (0 == m_mass_type)
        bAddTo = false;
    ...
    //  Cleaned up error handling.  ON_MassProperties::m_mass_type==0 is treated
    //  as an empty record not an error. If m_bValidMass==false for any of the
    //  summands then m_bValidMass==false for the result ( and similary for
    ...
    for (i = 0; i < count; i++) {
        ...
        if (summands[i].m_bValidMass && summands[i].m_bValidFirstMoments) {
    ...
    if (m_bValidFirstMoments) {
        ...
        m_x0_err =
            (m_world_x_err + fabs(m_world_x) * m_mass_err / m_mass) / m_mass;
        m_y0 = m_world_y / m_mass;
        m_y0_err =
            (m_world_y_err + fabs(m_world_y) * m_mass_err / m_mass) / m_mass;
        m_z0 = m_world_z / m_mass;
        m_z0_err =
            (m_world_z_err + fabs(m_world_z) * m_mass_err / m_mass) / m_mass;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (summands[i].m_bValidMass && summands[i].m_bValidCentroid &&
            ...
            ex.Plus(summands[i].m_ccs_xx_err + 2.0 * dx_err * fabs(dx) * m +
                    dx * dx * m_err);
            y.Plus(summands[i].m_ccs_yy + dy * dy * m);
            ey.Plus(summands[i].m_ccs_yy_err + 2.0 * dy_err * fabs(dy) * m +
                    dy * dy * m_err);
            z.Plus(summands[i].m_ccs_zz + dz * dz * m);
            ez.Plus(summands[i].m_ccs_zz_err + 2.0 * dz_err * fabs(dz) * m +
                    dz * dz * m_err);
        } else
    ...
    if (x.SummandCount() > 0) {
        ...
        if (m_bValidSecondMoments) {
            ...
            m_world_xx_err = m_ccs_xx_err +
                             2.0 * m_x0_err * fabs(m_x0) * m_mass +
                             m_x0 * m_x0 * m_mass_err;
            m_world_yy = m_ccs_yy + m_y0 * m_y0 * m_mass;
            m_world_yy_err = m_ccs_yy_err +
                             2.0 * m_y0_err * fabs(m_y0) * m_mass +
                             m_y0 * m_y0 * m_mass_err;
            m_world_zz = m_ccs_zz + m_z0 * m_z0 * m_mass;
            m_world_zz_err = m_ccs_zz_err +
                             2.0 * m_z0_err * fabs(m_z0) * m_mass +
                             m_z0 * m_z0 * m_mass_err;
        }
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (summands[i].m_bValidMass && summands[i].m_bValidCentroid &&
            ...
            ex.Plus(summands[i].m_ccs_xy_err + fabs(dx_err * dy * m) +
                    fabs(dy_err * dx * m) + fabs(dx * dy * m_err));
            y.Plus(summands[i].m_ccs_yz + dy * dz * m);
            ey.Plus(summands[i].m_ccs_yz_err + fabs(dy_err * dz * m) +
                    fabs(dz_err * dy * m) + fabs(dy * dz * m_err));
            z.Plus(summands[i].m_ccs_zx + dz * dx * m);
            ez.Plus(summands[i].m_ccs_zx_err + fabs(dz_err * dx * m) +
                    fabs(dx_err * dz * m) + fabs(dz * dx * m_err));
        } else
    ...
    if (x.SummandCount() > 0) {
        ...
        if (m_bValidProductMoments) {
            ...
            m_world_xy_err = m_ccs_xy_err + fabs(m_x0_err * m_y0 * m_mass) +
                             fabs(m_y0_err * m_x0 * m_mass) +
                             fabs(m_x0 * m_y0 * m_mass_err);
            m_world_yz = m_ccs_yz + m_y0 * m_z0 * m_mass;
            m_world_yz_err = m_ccs_yz_err + fabs(m_y0_err * m_z0 * m_mass) +
                             fabs(m_z0_err * m_y0 * m_mass) +
                             fabs(m_y0 * m_z0 * m_mass_err);
            m_world_zx = m_ccs_zx + m_z0 * m_x0 * m_mass;
            m_world_zx_err = m_ccs_zx_err + fabs(m_z0_err * m_x0 * m_mass) +
                             fabs(m_x0_err * m_z0 * m_mass) +
                             fabs(m_z0 * m_x0 * m_mass_err);
        }
    }
