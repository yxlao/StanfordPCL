...
void ON_DetailView::MemoryRelocate() { m_boundary.MemoryRelocate(); }
...
ON::object_type ON_DetailView::ObjectType() const { return ON::detail_object; }

int ON_DetailView::Dimension() const { return m_boundary.Dimension(); }

ON_BOOL32 ON_DetailView::GetBBox(double *boxmin, double *boxmax,
                                 int bGrowBox) const {
    return m_boundary.GetBBox(boxmin, boxmax, bGrowBox);
}

bool ON_DetailView::GetTightBoundingBox(ON_BoundingBox &tight_bbox,
                                        int bGrowBox,
