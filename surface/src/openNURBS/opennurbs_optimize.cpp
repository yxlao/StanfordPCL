...
int ON_FindLocalMinimum(int (*f)(void *, double, double *, double *),
                        void *farg, double ax, double bx, double cx,
                        double rel_stepsize_tol, double abs_stepsize_tol,
                        int max_it, double *t_addr)
/* Use Brent's algorithm (with derivative) to Find a (local) minimum of a
 * function
 *
 * INPUT:
 ...
 * abs_stepsize_tol) rel_stepsize_tol is a fractional tolerance and
 * abs_stepsize_tol is an absolute tolerance that determine the minimum step
 * size for a given iteration. minimum delta t = rel_stepsize_tol*|t| +
 * abs_stepsize_tol. When in doubt, use rel_stepsize_tol = ON_EPSILON
 ...
 */
{
    // See Numerical Recipes in C's dbrent() for a description of the basic
    // algorithm
    int rc, ok1, ok2;
    double a, b, d, d1, d2, du, dv, dw, dx, e, fu, fv, fw, fx, olde, tol1, tol2,
        u, u1, u2, v, w, x, xm;
    ...
    *t_addr = bx;
    ...
    if (rc) {
        ...
        *t_addr = x;
        return rc > 0 ? 1 : 0; // return 1 means f() said result is good enough,
                               // return = 0 means f() failed
    }
    fw = fv = fx;
    dw = dv = dx;
    while (max_it--) {
        ...
        if (fabs(d) >= tol1) {
            ...
            rc = f(farg, u, &fu, &du);
            if (rc >= 0 && fu > fx) {
                // tweaking x any more increases function value - x is a
                // numerical minimum
                *t_addr = x;
                return 1;
            }
...
ON_LocalZero1::ON_LocalZero1()
    : m_t0(ON_UNSET_VALUE), m_t1(ON_UNSET_VALUE), m_f_tolerance(0.0),
      m_t_tolerance(0.0), m_k(NULL), m_k_count(0) {}

ON_LocalZero1::~ON_LocalZero1() {}

ON_BOOL32
ON_LocalZero1::BracketZero(double s0, double f0, double s1, double f1,
                           int level) {
    double s, f, d;
...
ON_LocalZero1::BracketSpan(double s0, double f0, double s1, double f1) {
    ...
    if (m_k && m_k_count >= 3) {
        ...
        if (i0 <= i1) {
            ...
            Evaluate(m_k[i0], &fm, NULL,
                     -1); // gaurd against C0 discontinuities
            ...
            if ((f0 <= 0.0 && fm >= 0.0) || (f0 >= 0.0 && fm <= 0.0)) {
                ...
                if (i0 < i1) {
                    ...
                    if ((f1 <= 0.0 && fp >= 0.0) || (f1 >= 0.0 && fp <= 0.0)) {
                        m_s0 = m_k[i1];
                        m_f0 = fp;
                    } else if ((f0 <= 0.0 && fm >= 0.0) ||
                               (f0 >= 0.0 && fm <= 0.0)) {
                        m_s1 = m_k[i1];
                        m_f1 = fm;
                        // we have s0 = m_k[i0] < m_k[i1] = s1 and a bonafide
                        // sign change
                        while (i0 + 1 < i1) {
                            // bisect m_k[i0],...,m_k[i1] until we get a sign
                            // change between m_k[i],m_k[i+1].  We need to do
                            // this in order to make sure we are passing a C2
                            // function to the repeated zero finders.
                            i = (i0 + i1) >> 1;
                            ...
                            if ((f0 <= 0.0 && fm >= 0.0) ||
                                ...
                                while (i1 > 0 && m_k[i1 - 1] == m_k[i1])
                                    i1--;
                            } else if ((f1 <= 0.0 && fp >= 0.0) ||
                                       (f1 >= 0.0 && fp <= 0.0)) {
                                m_s0 = m_k[i];
                                m_f0 = fp;
                                i0 = i;
                                while (i0 < m_k_count - 2 &&
                                       m_k[i0] == m_k[i0 + 1])
                                    i0++;
                            } else {
...
ON_BOOL32 ON_LocalZero1::FindZero(double *t) {
    ...
    ON_BOOL32 rc =
        (m_t0 == ON_UNSET_VALUE || m_t1 == ON_UNSET_VALUE) ? true : false;
...
ON_BOOL32 ON_LocalZero1::NewtonRaphson(double s0, double f0, double s1,
                                       double f1, int maxit, double *t) {
    ...
    while (maxit--) {
        if ((f + (s0 - s) * d) * (f + (s1 - s) * d) >
                0.0 // true if NR's line segment doesn't cross zero inside
                    // interval
            || fabs(2.0 * f) >
                   fabs(prevds * d) // true if expected decrease in function
