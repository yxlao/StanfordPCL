...
ON_OBJECT_IMPLEMENT(ON_NurbsCage, ON_Geometry,
...
ON_BOOL32 ON_NurbsCage::Read(ON_BinaryArchive &archive) {
    ...
    if (rc) {
        while (rc) {
            if (major_version != 1) {
                ON_ERROR("ON_NurbsCage::Read - old code unable to read new "
                         "version of chunk");
                rc = false;
                break;
            }
            ...
            rc = Create(dim, is_rat == 1, order0, order1, order2, cv_count0,
                        cv_count1, cv_count2);
            if (!rc)
                break;
...
ON_NurbsCage::ON_NurbsCage(int dim, bool is_rat, int order0, int order1,
                           int order2, int cv_count0, int cv_count1,
                           int cv_count2)
    : m_dim(0), m_is_rat(0), m_cv_capacity(0), m_cv(0) {
    m_order[0] = 0;
    m_order[1] = 0;
    ...
    m_cv_stride[1] = 0;
    m_cv_stride[2] = 0;
    Create(dim, is_rat, order0, order1, order2, cv_count0, cv_count1,
           cv_count2);
}
...
ON_NurbsCage::ON_NurbsCage(const ON_3dPoint *box_corners, int order0,
                           int order1, int order2, int cv_count0, int cv_count1,
                           int cv_count2)
    : m_dim(0), m_is_rat(0), m_cv_capacity(0), m_cv(0) {
    m_order[0] = 0;
    m_order[1] = 0;
    ...
    m_cv_stride[1] = 0;
    m_cv_stride[2] = 0;
    Create(box_corners, order0, order1, order2, cv_count0, cv_count1,
           cv_count2);
}
...
int ON_NurbsCage::KnotCount(int dir) const {
    return (dir >= 0 && dir <= 2) ? ON_KnotCount(m_order[dir], m_cv_count[dir])
                                  : 0;
}
...
bool ON_NurbsCage::MakeDeformable() { return true; }

bool ON_NurbsCage::IsDeformable() const { return true; }

bool ON_NurbsCage::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
...
ON__UINT32 ON_NurbsCage::DataCRC(ON__UINT32 current_remainder) const {
        ...
        for (i = 0; i < m_cv_count[0]; i++) {
            for (j = 0; j < m_cv_count[1]; j++) {
                ...
                for (k = 0; i < m_cv_count[2]; k++) {
                    current_remainder =
                        ON_CRC32(current_remainder, sizeof_cv, cv);
                    cv += m_cv_stride[2];
                }
            }
        }
    ...
    current_remainder = ON_CRC32(
        current_remainder, KnotCount(2) * sizeof(m_knot[2][0]), m_knot[2]);
...
ON_NurbsCage &ON_NurbsCage::operator=(const ON_NurbsCage &src) {
    if (this != &src) {
        ...
        if (Create(src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1],
                   src.m_order[2], src.m_cv_count[0], src.m_cv_count[1],
                   src.m_cv_count[2])) {
            if (m_order[0] >= 2 && m_cv_count[0] >= m_order[0] && m_knot[0] &&
                src.m_knot[0])
                memcpy(m_knot[0], src.m_knot[0],
                       KnotCount(0) * sizeof(m_knot[0][0]));
            if (m_order[1] >= 2 && m_cv_count[1] >= m_order[1] && m_knot[1] &&
                src.m_knot[1])
                memcpy(m_knot[1], src.m_knot[1],
                       KnotCount(1) * sizeof(m_knot[1][0]));
            if (m_order[2] >= 2 && m_cv_count[2] >= m_order[2] && m_knot[2] &&
                src.m_knot[2])
                memcpy(m_knot[2], src.m_knot[2],
                       KnotCount(2) * sizeof(m_knot[2][0]));
...
ON_BOOL32 ON_NurbsCage::IsValid(ON_TextLog * // text_log
) const {
    if (0 == m_cv)
        return false;
...
void ON_NurbsCage::Dump(ON_TextLog &dump) const {
    dump.Print("ON_NurbsCage dim = %d is_rat = %d\n"
               "        order = (%d, %d, %d) \n",
               "        cv_count = (%d, %d, %d) \n", m_dim, m_is_rat,
               m_order[0], m_order[1], m_order[2], m_cv_count[0], m_cv_count[1],
               m_cv_count[2]);
    ...
    if (!m_cv) {
        ...
        for (i = 0; i < m_order[0]; i++) {
            for (j = 0; j < m_order[1]; j++) {
                ...
                dump.PrintPointList(m_dim, m_is_rat, m_cv_count[2],
                                    m_cv_stride[2], CV(i, j, 0), sPreamble);
            }
            if (i < m_order[0] - 1)
                dump.Print("\n");
        }
...
int ON_NurbsCage::Dimension() const { return m_dim; }

bool ON_NurbsCage::Create(const ON_BoundingBox &bbox, int order0, int order1,
                          int order2, int cv_count0, int cv_count1,
                          int cv_count2) {
    /*
              7______________6
    ...
    */
    ...
    return Create(box_corners, order0, order1, order2, cv_count0, cv_count1,
                  cv_count2);
}

bool ON_NurbsCage::Create(int dim, bool is_rat, int order0, int order1,
                          int order2, int cv_count0, int cv_count1,
                          int cv_count2) {
    Destroy();
    if (order0 < 2 || order1 < 2 || order2 < 2) {
        if (0 == dim && 0 == is_rat && 0 == order0 && 0 == order1 &&
            0 == order2 && 0 == cv_count0 && 0 == cv_count1 && 0 == cv_count2) {
            return true;
        }
...
bool ON_NurbsCage::Create(const ON_3dPoint *box_corners, int order0, int order1,
                          int order2, int cv_count0, int cv_count1,
                          int cv_count2) {
    int i, j, k;
    double r, s, t;
    ...
    if (2 == cv_count0 && 2 == cv_count1 && 2 == cv_count2) {
        ...
        if (!Create(3, 0, order0, order1, order2, cv_count0, cv_count1,
                    cv_count2))
            return false;

        double *g0 = (double *)onmalloc(m_cv_count[0] * m_cv_count[1] *
                                        m_cv_count[2] * sizeof(*g0));
...
ON_BOOL32 ON_NurbsCage::GetBBox( // returns true if successful
    ...
    int i, j;
    bool rc = (0 != m_cv && m_cv_count[0] >= 2 && m_cv_count[1] >= 2 &&
               m_cv_count[2] >= 2 && m_cv_stride[0] > 0 && m_cv_stride[1] > 0 &&
               m_cv_stride[2] > 0)
                  ? true
                  : false;
    if (!rc) {
        ...
        for (i = 0; rc && i < m_cv_count[0]; i++)
            for (j = 0; rc && j < m_cv_count[1]; j++) {
                rc = ON_GetPointListBoundingBox(
                    m_dim, m_is_rat, m_cv_count[2], m_cv_stride[2], CV(i, j, 0),
...
ON_BOOL32 ON_NurbsCage::Transform(const ON_Xform &xform) {
    int i, j;
    bool rc = (m_cv_count[0] > 0 && m_cv_count[1] > 0 && m_cv_count[2]) ? true
                                                                        : false;
    if (rc || !xform.IsIdentity()) {
        ...
        for (i = 0; rc && i < m_cv_count[0]; i++) {
            for (j = 0; rc && j < m_cv_count[1]; j++) {
                rc = ON_TransformPointList(m_dim, m_is_rat, m_cv_count[2],
                                           m_cv_stride[2], CV(i, j, 0), xform);
            }
        }
    }
    return rc;
...
bool ON_EvaluateNurbsCageSpan(int dim, int is_rat, int order0, int order1,
                              int order2, const double *knot0,
                              const double *knot1, const double *knot2,
                              int cv_stride0, int cv_stride1, int cv_stride2,
                              const double *cv0, int der_count, double t0,
                              double t1, double t2, int v_stride, double *v) {
    double c;
                    ...
                    for (j1 = 0; j1 < order1; j1++) {
                        ...
                        for (j2 = 0; j2 < order2; j2++) {
                            ...
                            for (d0 = d; d0 >= 0; d0--) {
                                for (d1 = d - d0; d1 >= 0; d1--) {
                                    d2 = d - d0 - d1;
                                    if (d0 > der_count0 || d1 > der_count1 ||
                                        d2 > der_count2) {
...
bool ON_NurbsCage::Evaluate(      // returns false if unable to evaluate
    ...
    int *hint) const {
    int side0 =
        (side && (side == 2 || side == 3 || side == 6 || side == 7)) ? -1 : 1;
    int side1 =
        (side && (side == 3 || side == 4 || side == 7 || side == 8)) ? -1 : 1;
    int side2 = (side >= 5 && side <= 8) ? -1 : 1;

    int hint0 = (hint) ? hint[0] : 0;
    int hint1 = (hint) ? hint[1] : 0;
    int hint2 = (hint) ? hint[2] : 0;
    const int span_index0 = ON_NurbsSpanIndex(m_order[0], m_cv_count[0],
                                              m_knot[0], r, side0, hint0);
    const int span_index1 = ON_NurbsSpanIndex(m_order[1], m_cv_count[1],
                                              m_knot[1], s, side1, hint1);
    const int span_index2 = ON_NurbsSpanIndex(m_order[2], m_cv_count[2],
                                              m_knot[2], t, side2, hint2);

    bool rc = ON_EvaluateNurbsCageSpan(
        m_dim, m_is_rat, m_order[0], m_order[1], m_order[2],
        m_knot[0] + span_index0, m_knot[1] + span_index1,
        m_knot[2] + span_index2, m_cv_stride[0], m_cv_stride[1], m_cv_stride[2],
...
ON_3dPoint ON_NurbsCage::PointAt(double r, double s, double t) const {
    ON_3dPoint pt;
    if (m_dim <= 3) {
        pt.x = 0.0;
        pt.y = 0.0;
...
ON_NurbsSurface *ON_NurbsCage::IsoSurface(int dir, double c,
                                          ON_NurbsSurface *srf) const {
    ...
    if (m_order[dir] < 2 || m_cv_count[dir] < m_order[dir] ||
        0 == m_knot[dir]) {
    ...
    nurbs_curve.m_dim = cage_cvdim * m_cv_count[0] * m_cv_count[1] *
                        m_cv_count[2] / m_cv_count[dir];
    ...
    */
    ...
    for (i = span_index; i < i1; i++) {
        ...
        for (j = 0; j < j1; j++) {
            ...
            for (k = 0; k < k1; k++) {
                cage_ijk[kk] = k;
                memcpy(cv, CV(cage_ijk[0], cage_ijk[1], cage_ijk[2]),
                       cage_sizeofcv);
                cv += cage_cvdim;
            }
        }
...
bool ON_NurbsCage::IsRational() const { return m_is_rat ? true : false; }
...
double *ON_NurbsCage::CV(int i, int j, int k) const {
    ...
    return (m_cv) ? (m_cv + i * m_cv_stride[0] + j * m_cv_stride[1] +
                     k * m_cv_stride[2])
                  : 0;
}
...
bool ON_NurbsCage::SetCV(int i, int j, int k, ON::point_style style,
                         const double *Point) {
    bool rc = true;
    int n;
    double w;
...
bool ON_NurbsCage::GetCV(int i, int j, int k, ON::point_style style,
                         double *Point) const {
    const double *cv = CV(i, j, k);
    if (!cv)
        return false;
...
bool ON_NurbsCage::SetKnot(int dir, int knot_index, double knot_value) {
    bool rc;
...
double ON_NurbsCage::Knot(int dir, int knot_index) const {
    double knot_value;
...
bool ON_NurbsCage::MakeRational() {
    if (!IsRational()) {
        ...
        if (m_cv_count[0] > 0 && m_cv_count[1] > 0 && m_cv_count[2] > 0 &&
            dim > 0) {
            int i, j, k;
            if (m_cv_stride[0] <= dim || m_cv_stride[1] <= dim ||
                m_cv_stride[2] <= dim) {
                // there's not room for the weight in the existing m_cv array -
                // adjust the strides
                double *new_cv = (double *)onmalloc(
                ...
                m_is_rat = 1;
                ReserveCVCapacity(m_cv_count[0] * m_cv_count[1] *
                                  m_cv_count[2] * (dim + 1));
                memcpy(m_cv, new_cv,
                       m_cv_count[0] * m_cv_count[1] * m_cv_count[2] *
                           (dim + 1) * sizeof(*m_cv));
...
bool ON_NurbsCage::ReserveKnotCapacity(int dir, int knot_capacity) {
    bool rc = false;
            ...
            if (m_knot[dir]) {
                if (m_knot_capacity[dir]) {
                    m_knot[dir] = (double *)onrealloc(
                        m_knot[dir], knot_capacity * sizeof(m_knot[dir][0]));
                    m_knot_capacity[dir] = m_knot[dir] ? knot_capacity : 0;
                }
                // else user supplied m_knot[dir] array.
            } else {
                m_knot[dir] =
                    (double *)onmalloc(knot_capacity * sizeof(m_knot[dir][0]));
                m_knot_capacity[dir] = m_knot[dir] ? knot_capacity : 0;
            }
        }
        rc = m_knot[dir] != 0;
...
void ON_MorphControl::MorphPointLocalizerHelper(
    const ON_3dPoint &point, double &w, double &clspt_max_dist,
    const ON_Localizer *&distloc) const {
    w = 1.0;
    clspt_max_dist = 0.0;
    distloc = 0;
...
void ON_MorphControl::MorphPointVarient1Helper(double t, double w,
    ...
    if (ON_UNSET_VALUE == m_nurbs_curve_domain.m_t[0]) {
        const_cast<ON_MorphControl *>(this)->m_nurbs_curve_domain =
            m_nurbs_curve0.Domain();
    }
...
void ON_MorphControl::MorphPointVarient2Helper(double s, double t, double w,
                                               const ON_Localizer *distloc,
                                               ON_3dPoint &Q,
                                               ON_3dVector *N) const {
    ON_3dPoint P0, P1;
    ...
    if (ON_UNSET_VALUE == m_nurbs_surface_domain[0].m_t[0]) {
        const_cast<ON_MorphControl *>(this)->m_nurbs_surface_domain[0] =
            m_nurbs_surface0.Domain(0);
        const_cast<ON_MorphControl *>(this)->m_nurbs_surface_domain[1] =
            m_nurbs_surface0.Domain(1);
    }

    bool bDuTweak = ((s <= m_nurbs_surface_domain[0][0] ||
                      s >= m_nurbs_surface_domain[0][1]) &&
                     !m_nurbs_surface0.IsClosed(0));
    bool bDvTweak = ((t <= m_nurbs_surface_domain[1][0] ||
                      t >= m_nurbs_surface_domain[1][1]) &&
                     !m_nurbs_surface0.IsClosed(1));
    ...
    if (bDuTweak || bDvTweak) {
        ...
        if (bDuTweak || bDvTweak) {
            ...
            if (2 ==
                ON_Solve3x2(&Du0.x, &Dv0.x, D.x, D.y, D.z, &u, &v, &e, &pr)) {
                // D = s*Du1 + t*Dv1;
                D.x = u * Du1.x + v * Dv1.x;
                D.y = u * Du1.y + v * Dv1.y;
                D.z = u * Du1.z + v * Dv1.z;
...
ON_BOOL32 ON_MorphControl::GetBBox(double *boxmin, double *boxmax,
                                   int bGrowBox) const {
    ON_BOOL32 rc = false;
    switch (m_varient) {
    case 1:
...
ON_BOOL32 ON_MorphControl::Transform(const ON_Xform &xform) {
    ON_BOOL32 rc = false;
...
bool ON_MorphControl::AddControlLocalizer(double support_distance,
    ...
    if (rc) {
        ...
        case 2: {
            ...
            localizer.m_d.Set(support_distance + falloff_distance,
                              support_distance);
            rc = true;
        } break;
        ...
        case 3: {
            ...
            rc = AddConvexPolygonLocalizer(planes, support_distance,
                                           falloff_distance);
        } break;
...
bool ON_MorphControl::AddSphereLocalizer(ON_3dPoint center,
                                         ...
                                         double falloff_distance) {
    bool rc =
        (center.IsValid() && support_distance >= 0.0 && falloff_distance > 0.0);
    if (rc) {
        ON_Localizer &localizer = m_localizers.AppendNew();
...
bool ON_MorphControl::AddCylinderLocalizer(ON_Line axis,
                                           ...
                                           double falloff_distance) {
    bool rc =
        (axis.IsValid() && support_distance >= 0.0 && falloff_distance > 0.0);
    if (rc) {
        ON_Localizer &localizer = m_localizers.AppendNew();
        rc = localizer.CreateCylinderLocalizer(
            axis.from, axis.Tangent(), support_distance + falloff_distance,
            support_distance);
    }
    return rc;
}
...
bool ON_MorphControl::AddBoxLocalizer(ON_BoundingBox bbox,
    ...
    ON_SimpleArray<ON_Plane> planes(6);
    bool rc =
        (bbox.IsValid() && support_distance >= 0.0 && falloff_distance > 0.0);
    if (rc) {
...
bool ON_MorphControl::AddPlaneLocalizer(const ON_Plane &plane,
    ...
    planes.Append(plane);
    return AddConvexPolygonLocalizer(planes, support_distance,
                                     falloff_distance);
}

bool ON_MorphControl::AddConvexPolygonLocalizer(
    const ON_SimpleArray<ON_Plane> &planes, double support_distance,
    double falloff_distance) {
    ...
    if (rc) {
        ...
        for (i = 0; i < count && rc; i++) {
            ...
            rc = localizer.CreatePlaneLocalizer(
                plane.origin, plane.zaxis, support_distance + falloff_distance,
                support_distance);
        }
    }
    return rc;
