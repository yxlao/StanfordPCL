...
const ON_3dPoint &ON_PointCloud::operator[](int i) const { return m_P[i]; }

ON_3dPoint ON_PointCloud::Point(ON_COMPONENT_INDEX ci) const {
    return (ON_COMPONENT_INDEX::pointcloud_point == ci.m_type &&
            ci.m_index >= 0 && ci.m_index < m_P.Count())
               ? m_P[ci.m_index]
               : ON_UNSET_POINT;
}
...
ON_PointCloud::ON_PointCloud(const ON_PointCloud &src) { *this = src; }
...
int ON_PointCloud::Dimension() const { return 3; }
...
ON_BOOL32 ON_PointCloud::GetBBox( // returns true if successful
    ...
    if (!m_bbox.IsValid()) {
        m_P.GetBBox((double *)&m_bbox.m_min.x, (double *)&m_bbox.m_max.x,
                    false);
    }
...
ON_BOOL32 ON_PointCloud::Transform(const ON_Xform &xform) {
...
bool ON_PointCloud::IsDeformable() const { return true; }

bool ON_PointCloud::MakeDeformable() { return true; }
...
int ON_PointCloud::PointCount() const { return m_P.Count(); }

void ON_PointCloud::AppendPoint(const ON_3dPoint &pt) { m_P.Append(pt); }

void ON_PointCloud::InvalidateBoundingBox() { m_bbox.Destroy(); }
...
bool ON_PointCloud::IsOrdered() const { return (m_flags & 1) ? true : false; }

bool ON_PointCloud::HasPlane() const { return (m_flags & 2) ? true : false; }
...
const ON_Plane &ON_PointCloud::Plane() { return m_plane; }
