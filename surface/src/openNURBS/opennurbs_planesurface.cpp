...
ON_PlaneSurface::ON_PlaneSurface(const ON_PlaneSurface &src) { *this = src; }
...
void ON_PlaneSurface::Dump(ON_TextLog &dump) const {
...
int ON_PlaneSurface::Dimension() const { return 3; }
...
ON_PlaneSurface::Transform(const ON_Xform &xform) {
    ...
    if (rc && fabs(fabs(xform.Determinant()) - 1.0) > ON_SQRT_EPSILON) {
        ...
        double x0, x1, y0, y1;
        rc = false;
        if (m_plane.ClosestPointTo(p, &x0, &y0) &&
            m_plane.ClosestPointTo(q, &x1, &y1)) {
            if (x0 < x1 && y0 < y1) {
...
int ON_PlaneSurface::SpanCount(int dir) const { return 1; }
...
int ON_PlaneSurface::Degree(int dir) const { return 1; }
...
ON_PlaneSurface::GetParameterTolerance(int dir,
    ...
    dir = (dir) ? 1 : 0;
    return ON_GetParameterTolerance(m_domain[dir][0], m_domain[dir][1], t,
                                    tminus, tplus);
}
...
ON_BOOL32
ON_PlaneSurface::IsClosed(int dir) const { return false; }

ON_BOOL32
ON_PlaneSurface::IsPeriodic(int dir) const { return false; }

ON_BOOL32
ON_PlaneSurface::IsSingular(int side) const { return false; }

bool ON_PlaneSurface::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
...
ON_BOOL32 ON_PlaneSurface::Trim(int dir, const ON_Interval &domain) {
    if (dir < 0 || dir > 1)
        return false;
...
bool ON_PlaneSurface::Extend(int dir, const ON_Interval &domain) {
    if (dir < 0 || dir > 1)
        return false;
    bool changed = false;
    ...
    if (domain[0] < Domain(dir)[0]) {
        changed = true;
        tdom[0] = domain[0];
        xdom[0] = m_extents[dir].ParameterAt(
            m_domain[dir].NormalizedParameterAt(domain[0]));
    }
    if (domain[1] > Domain(dir)[1]) {
        changed = true;
        tdom[1] = domain[1];
        xdom[1] = m_extents[dir].ParameterAt(
            m_domain[dir].NormalizedParameterAt(domain[1]));
    }
    if (!changed)
        return false;
...
ON_BOOL32 ON_PlaneSurface::Split(int dir, double c,
...
// the search fails.
ON_BOOL32 ON_PlaneSurface::GetLocalClosestPoint(
...
bool ON_PlaneSurface::SetExtents(int dir, ON_Interval extents,
                                 bool bSyncDomain) {
...
ON_BOOL32 ON_PlaneSurface::SetDomain(int dir, double t0, double t1) {
    bool rc = false;
...
void ON_ClippingPlaneInfo::Default() { memset(this, 0, sizeof(*this)); }
