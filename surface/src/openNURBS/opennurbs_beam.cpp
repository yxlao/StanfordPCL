    ...
    if (profile_count > 1) {
        for (int i = 0; i < profile_count; i++) {
            ...
            if (segment->Domain() != polycurve.SegmentDomain(i)) {
                if (text_log) {
                    text_log->Print("polycurve.Segment(%d).Domain() does not "
                                    "match polycurve.SegmentDomain(%d).\n",
                                    i, i);
                }
                return ON_ExtrusionPolyCurveProfileIsNotValid();
            }
...
bool ON_Extrusion::CleanupPolyCurveProfile(ON_PolyCurve &polycurve) {
    if (!ON_Extrusion::IsValidPolyCurveProfile(polycurve)) {
        ...
        for (i = 0; i < old_count; i++) {
            ...
            if (0 == old_segment)
                return false;
            if (2 != old_segment->Dimension() &&
                !old_segment->ChangeDimension(2))
                return false;
        }
            ...
            for (i = 0; i < new_polycurves.Count(); i++) {
                ...
                if (new_segment) {
                    for (int j = new_segment->Count() - 1; j >= 0; j--) {
                        new_segment->HarvestSegment(
                            j); // do not delete parts of input polycurve
                    }
                    delete new_segment;
                }
            }
...
bool ON_GetEndCapTransformation(ON_3dPoint P, ON_3dVector T, ON_3dVector U,
    ...
    if (N.z > ON_Extrusion::m_Nz_min && N.IsUnitVector()) {
        ...
        for (;;) {
            ...
            // S is a non-uniform scale that maps A to A and perpA to
            // 1/cosa*perpA. The scale distorts the profile so that after it is
            // rotated into the miter plane, the projection of the rotated
            // profile onto the xy-plane matches the original profile.
            ON_Xform S(0.0);
...
bool ON_Extrusion::SetPathAndUp(ON_3dPoint A, ON_3dPoint B, ON_3dVector up) {
    ...
    if (rc) {
        ...
        double d = up * D;
        if (!up.IsUnitVector() ||
            fabs(d) > distAB * ON_SQRT_EPSILON * 0.015625) {
...
int ON_Extrusion::PathParameter() const { return m_bTransposed ? 0 : 1; }

int ON_Extrusion::ProfileParameter() const { return m_bTransposed ? 1 : 0; }
...
static bool ProfileHelper(int desired_orientation, ON_Curve *profile) {
    ...
    if (!bbox.IsValid()) {
        ON_ERROR(
            "ON_Extrusion::Set/Add Profile - profile->BoundingBox() failed.");
        return false;
    }
    if (fabs(bbox.m_min.z) > ON_ZERO_TOLERANCE ||
        fabs(bbox.m_max.z) > ON_ZERO_TOLERANCE) {
        ON_ERROR("ON_Extrusion::Set/Add Profile - profile->BoundingBox() is "
                 "not in the world xy plane.");
        return false;
    }
    if (!profile->ChangeDimension(2)) {
        ON_ERROR("ON_Extrusion::Set/Add Profile - profile->ChangeDimension(2) "
                 "failed.");
        return false;
    }
    ...
    if (profile->IsClosed()) {
        ...
        case 0:
            if (-1 == profile_orientation) {
                if (!profile->Reverse()) {
                    ON_ERROR("ON_Extrusion::SetOuterProfile() - "
                             "profile->Reverse() failed.");
                    return false;
                }
                profile_orientation = 1;
            }
            if (1 == desired_orientation && 1 != profile_orientation) {
                ON_ERROR("ON_Extrusion::SetOuterProfile() - profile has wrong "
                         "orientation.");
                return false;
            }
            break;
        case -1:
            if (1 == profile_orientation) {
                if (!profile->Reverse()) {
                    ON_ERROR("ON_Extrusion::AddInnerProfile() - "
                             "profile->Reverse() failed.");
                    return false;
                }
                profile_orientation = -1;
            ...
            if (-1 != profile_orientation) {
                ON_ERROR("ON_Extrusion::AddInnerProfile() - profile has wrong "
                         "orientation.");
                return false;
            }
            break;
        default: {
            ON_ERROR("ON_Extrusion::Set/Add Profile - invalid "
                     "desired_orientation parameter.");
            return false;
        } break;
        }
    ...
    if (0 != polycurve) {
        ...
        for (int i = 0; i < polycurve->Count(); i++) {
            ...
            if (0 == segment) {
                ON_ERROR("ON_Extrusion::Set/Add Profile - ON_PolyCurve has "
                         "null segment.");
                return false;
            }
            if (!segment->SetDomain(polycurve->SegmentDomain(i))) {
                ON_ERROR("ON_Extrusion::Set/Add Profile - segment->SetDomain() "
                         "failed.");
                return false;
            }
        }
...
bool ON_Extrusion::SetOuterProfile(ON_Curve *outer_profile, bool bCap) {
    if (0 != m_profile) {
        ON_ERROR("ON_Extrusion::SetOuterProfile() called when m_profile is "
                 "already not null.");
        return false;
    }
...
bool ON_Extrusion::AddInnerProfile(ON_Curve *inner_profile) {
    if (m_profile_count < 1) {
        ON_ERROR(
            "ON_Extrusion::AddInnerProfile() called when m_profile_count < 1.");
        return false;
    }
    if (0 == m_profile) {
        ON_ERROR(
            "ON_Extrusion::AddInnerProfile() called when m_profile is null.");
        return false;
    }

    if (1 == m_profile_count && !m_profile->IsClosed()) {
        ON_ERROR("ON_Extrusion::AddInnerProfile() called when outer profile is "
                 "not closed.");
        return false;
    }
    ...
    if (m_profile_count > 1 && 0 == polycurve) {
        ON_ERROR("ON_Extrusion::AddInnerProfile() called when  m_profile_count "
                 "> 1 but m_profile is not an ON_PolyCurve.");
        return false;
    }
    if (m_profile_count > 1 && m_profile_count != polycurve->Count()) {
        ON_ERROR("ON_Extrusion::AddInnerProfile() called when  m_profile_count "
                 "> 1 but m_profile_count != m_profile->Count().");
        return false;
    }
    ...
    if (polycurve->SegmentDomain(m_profile_count) != inner_profile->Domain()) {
        ...
        // so we don't get bugs caused by fuzz as we adjust parameters when
        // evaluating.
...
const ON_PolyCurve *ON_Extrusion::PolyProfile() const {
    ...
    return (0 != poly_profile && m_profile_count == poly_profile->Count())
               ? poly_profile
               : 0;
}
...
int ON_Extrusion::GetProfileCurves(
    ON_SimpleArray<const ON_Curve *> &profile_curves) const {
    if (!m_profile)
        return 0;
...
ON_OBJECT_IMPLEMENT(ON_Extrusion, ON_Surface,
...
ON_BOOL32 ON_Extrusion::IsValid(ON_TextLog *text_log) const {
    ...
    if (m_profile_count > 1) {
        ...
        if (0 == c) {
            ON_Extrusion_IsNotValidMessage(
                text_log,
                "m_profile_count > 1 but m_profile is not an ON_PolyCurve.");
            return ON_ExtrusionIsNotValid();
        }
        if (m_profile_count != c->Count()) {
            ON_Extrusion_IsNotValidMessage(
                text_log, "m_profile_count > 1 but m_profile_count != "
                          "m_profile->SegmentCount().");
            return ON_ExtrusionIsNotValid();
        }

        if (!ON_Extrusion::IsValidPolyCurveProfile(*c, text_log)) {
            ON_Extrusion_IsNotValidMessage(
                text_log, "m_profile is not a valid ON_PolyCurve.");
            return ON_ExtrusionIsNotValid();
        }
        for (int i = 0; i < m_profile_count; i++) {
            ...
            if (0 == segment) {
                ON_Extrusion_IsNotValidMessage(
                    text_log, "m_profile_count > 1 but a "
                              "m_profile_count->SegmentCurve() is null.");
                return ON_ExtrusionIsNotValid();
            }
            if (!segment->IsClosed()) {
                ON_Extrusion_IsNotValidMessage(
                    text_log, "m_profile_count > 1 but a "
                              "m_profile_count->SegmentCurve() is not closed.");
                return ON_ExtrusionIsNotValid();
            }
        }
    ...
    if (!ON_IsValid(len) || len <= ON_Extrusion::m_path_length_min) {
        if (text_log)
            text_log->Print(
                "m_path has zero length <= ON_Extrusion::m_path_length_min.");
        return ON_ExtrusionIsNotValid();
    }
    ...
    if (!(0.0 <= m_t.m_t[0] && m_t.m_t[0] < m_t.m_t[1] && m_t.m_t[1] <= 1.0)) {
        ON_Extrusion_IsNotValidMessage(
            text_log, "m_t does not satisfy 0<=m_t[0]<m_t[1]<=1");
        return ON_ExtrusionIsNotValid();
    }
    ...
    if (fabs(len) > ON_SQRT_EPSILON) {
        ON_Extrusion_IsNotValidMessage(text_log,
                                       "m_up is not perpendicular to m_path.");
        return ON_ExtrusionIsNotValid();
    }
    ...
    if (m_bHaveN[0]) {
        if (!m_N[0].IsUnitVector()) {
            ON_Extrusion_IsNotValidMessage(text_log,
                                           "m_N[0] is not a unit vector.");
            return ON_ExtrusionIsNotValid();
        }
        if (!(m_N[0].z > ON_Extrusion::m_Nz_min)) {
            ON_Extrusion_IsNotValidMessage(
                text_log,
                "m_N[0].z is too small (<=ON_Extrusion::m_Nz_min) or negative");
            return ON_ExtrusionIsNotValid();
        }
    }
    if (m_bHaveN[1]) {
        if (!m_N[1].IsUnitVector()) {
            ON_Extrusion_IsNotValidMessage(text_log,
                                           "m_N[1] is not a unit vector.");
            return ON_ExtrusionIsNotValid();
        }
        if (!(m_N[1].z > ON_Extrusion::m_Nz_min)) {
            ON_Extrusion_IsNotValidMessage(
                text_log,
                "m_N[1].z is too small (<=ON_Extrusion::m_Nz_min) or negative");
            return ON_ExtrusionIsNotValid();
        }
    }
...
ON__UINT32 ON_Extrusion::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_bHaveN[0]), &m_bHaveN[0]);
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_bHaveN[1]), &m_bHaveN[1]);
    ...
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_bCap[0]), &m_bCap[0]);
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_bCap[1]), &m_bCap[1]);
    if (m_profile)
        current_remainder = m_profile->DataCRC(current_remainder);
...
ON_BOOL32 ON_Extrusion::Read(ON_BinaryArchive &binary_archive) {
    ...
    int major_version = 0;
    int minor_version = 0;
    bool rc = binary_archive.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK,
...
int ON_Extrusion::Dimension() const { return 3; }

static bool GetBoundingBoxHelper(const ON_Extrusion &extrusion,
                                 ON_BoundingBox &bbox, const ON_Xform *xform) {
...
ON_BOOL32 ON_Extrusion::GetBBox(double *boxmin, double *boxmax,
                                int bGrowBox) const {
    bool rc = false;
...
bool ON_Extrusion::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
...
static bool Profile2dTransform(ON_Extrusion &e, const ON_Xform &profile_xform,
                               bool bNeedReverse) {
    if (profile_xform.IsIdentity()) {
        // profile curves are unchanged (translation, rotation, 1D scale along
        // path, ... )
        return true;
    }
    ...
    bool rc = false;
    bool bNeedDeformable = (fabs(profile_xform.m_xform[0][0]) !=
                                fabs(profile_xform.m_xform[1][1]) ||
    ...
    if (0 != polyprofile) {
        ...
        for (int i = 0; i < polyprofile->Count(); i++) {
            ...
            if (bNeedReverse) {
                ...
                if (profile_segment->GetDomain(&t0, &t1)) {
                    // reverse the segment so the correct
                    // clounterclockwise/clockwise orientation is maintained
        ...
        if (bNeedDeformable && !e.m_profile->IsDeformable()) {
            ...
            rc = e.m_profile->Transform(profile_xform)
                     ? true
        ...
        if (rc && bNeedReverse) {
            ...
            if (e.m_profile->GetDomain(&t0, &t1)) {
                // reverse the segment so the correct
                // clounterclockwise/clockwise orientation is maintained after
...
ON_BOOL32 ON_Extrusion::Transform(const ON_Xform &xform) {
    ...
    // 12 July 2008 Dale Lear - this test fixes
    // http://dev.mcneel.com/bugtrack/?q=88130
    ...
    for (int i = 0; i < 2; i++) {
        QN[i] = m_N[i];
        if (m_bHaveN[i]) {
            const ON_3dVector N3d =
                m_N[i].x * X + m_N[i].y * m_up + m_N[i].z * T;
            ...
            QN[i].y = QU * QN3d;
            QN[i].z = QT * QN3d;
            if (!QN[i].Unitize() || QN[i].z < ON_Extrusion::m_Nz_min ||
    ...
    else if (!ON_IsValid(profile_xform.m_xform[1][1]) ||
             0.0 == profile_xform.m_xform[1][1]) {
    ...
    if (fabs(fabs(profile_xform.m_xform[0][0]) - profile_xform.m_xform[1][1]) <=
        profile_xform_tol) {
        profile_xform.m_xform[0][0] =
            ((profile_xform.m_xform[0][0] < 0.0) ? -1.0 : 1.0) *
    ...
    const double profile_det =
        profile_xform.m_xform[0][0] * profile_xform.m_xform[1][1];
    ...
    if (bHavePathShear) {
        ...
        ON_2dVector A(-path_shear.y,
                      path_shear.x); // path_shear is relative to xy plane.
        A.Unitize();
...
ON_Extrusion_BrepForm_FaceInfo::~ON_Extrusion_BrepForm_FaceInfo() {
    // when m_extrusion_srf is not null, its destructor deletes
    // m_extrusion_profile.
...
MakeCap2dCurvesHelper(ON_ClassArray<ON_Extrusion_BrepForm_FaceInfo> &finfo,
                      int is_capped, const ON_Xform &scale0,
...
static bool GetNextProfileSegmentDiscontinuity(const ON_Curve *profile_segment,
    ...
    return profile_segment->GetNextDiscontinuity(
        ON::Gsmooth_continuous, t0, t1, t, 0, 0,
        ON_DEFAULT_ANGLE_TOLERANCE_COSINE, ON_SQRT_EPSILON);
}
...
bool ON_Extrusion::ProfileIsKinked(int profile_index) const {
    ...
    if (!ON_IsValid(t0) || !(t0 < t1))
        return 0;
    t = t0;
    return (GetNextProfileSegmentDiscontinuity(profile, t0, t1, &t) && t0 < t &&
            t < t1);
}
...
int ON_Extrusion::GetProfileKinkParameters(
...
ON_Brep *ON_Extrusion::BrepForm(ON_Brep *brep, bool bSmoothFaces) const {
    ...
    ON_Xform xform0(1.0), xform1(1.0), scale0(1.0), scale1(1.0), rot0(1.0),
        rot1(1.0);
    if (!ON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[0]), T, m_up,
                                    m_bHaveN[0] ? &m_N[0] : 0, xform0, &scale0,
                                    &rot0))
        return 0;

    if (!ON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[1]), T, m_up,
                                    m_bHaveN[1] ? &m_N[1] : 0, xform1, &scale1,
                                    &rot1))
        return 0;
    ...
    if (is_capped < 0 || is_capped > 3)
        is_capped = 0; // don't let future changes or bugs in IsCapped() crash
                       // this code.
    ...
    for (int profile_index = 0; profile_index < profile_count;
         profile_index++) {
        ...
        profile_fi.Init();
        {
            ...
            profile_fi.m_profile_orientation =
                (profile_fi.m_bClosedProfile)
                    ? ON_ClosedCurveOrientation(*newprofile, 0)
                    : 0;

            if (is_capped && profile_fi.m_profile_orientation !=
                                 ((0 == profile_index) ? 1 : -1)) {
                // do not attempt to cap extrusions with incorrectly oriented
                // profiles
                is_capped = 0;
                cap_count = 0;
            }
            ...
            if (bSmoothFaces) {
                // It is important to use the original profile,
                // "profile_segment" and not the trimmed copy in the
                // GetNextDiscontinuity() loop so the code that creates this
                // brep divides the extrusion profile exactly the same way as
                // the code that calculates component indices.
                double t0 = ON_UNSET_VALUE;
                double t1 = ON_UNSET_VALUE;
                ...
                double t = t1;
                while (GetNextProfileSegmentDiscontinuity(profile_segment, t0,
                                                          t1, &t)) {
                    if (t0 < t && t < t1) {
                        ON_Curve *left_side = 0;
                        ON_Curve *right_side = 0;
                        if (newprofile->Split(t, left_side, right_side) &&
                            0 != left_side && 0 != right_side) {
        ...
        for (int finfo_index = fi0; finfo_index < fi1; finfo_index++) {
            ...
            if (fi.m_eid[eidmap[3]] >= 0)
                fi.m_bRev3d[eidmap[3]] =
                    (profile_fi.m_bRev3d[eidmap[1]]) ? false : true;

            if (profile_fi.m_bClosedProfile && finfo_index > fi0 &&
                finfo_index == fi1 - 1) {
                ...
                if (fi.m_eid[eidmap[1]] >= 0)
                    fi.m_bRev3d[eidmap[1]] =
                        (profile_fi.m_bRev3d[eidmap[3]]) ? false : true;
            }
            ...
            if (0 == face) {
                if (newbrep->m_S.Count() == brep_S_count ||
                    (newbrep->m_S.Count() > brep_S_count &&
                     0 == newbrep->m_S[brep_S_count])) {
                    delete face_srf;
                }
                if (newbrep != brep)
                    delete newbrep;
            ...
            const ON_BrepLoop *loop =
                (1 == face->LoopCount()) ? face->OuterLoop() : 0;
            ...
            if (0 == bottom_trim ||
                ON_BrepTrim::boundary != bottom_trim->m_type) {
                is_capped = 0;
                cap_count = 0;
                continue;
            }
    ...
    while (is_capped > 0 && cap_count == ((3 == is_capped) ? 2 : 1)) {
        // while(...) not a loop
        // - break's are used for flow control and there is a break at the
        // bottom of this scope.
        ...
        newbrep->m_C2.Reserve(newbrep->m_C2.Count() +
                              cap_count * finfo.Count());
        ...
        for (cap_index = 0; cap_index < 2; cap_index++) {
            ...
            int profile_index;
            for (profile_index = 0;
                 profile_index < profile_count && fi0 < finfo.Count();
                 profile_index++) {
                if (finfo[fi0].m_profile_index != profile_index)
                    break;
                ON_BrepLoop::TYPE loop_type = (0 == profile_index)
                                                  ? ON_BrepLoop::outer
                                                  : ON_BrepLoop::inner;
                fi1 = MakeCapLoopHelper(finfo, fi0, cap_index, capface,
                                        loop_type);
                if (fi1 <= fi0)
                    break;
                fi0 = fi1;
        ...
        if (2 == cap_index && 2 == cap_count && 3 == is_capped)
            ((CMyBrepIsSolidSetter *)newbrep)
                ->SetIsSolid(m_bTransposed ? 2 : 1);
...
bool ON_Extrusion::GetBrepFormComponentIndex(
    ON_COMPONENT_INDEX extrusion_ci, ON_COMPONENT_INDEX &brep_ci) const {
    const ON_Brep unused_brep;
    return GetBrepFormComponentIndex(extrusion_ci, ON_UNSET_VALUE, unused_brep,
                                     brep_ci);
}
...
static bool GetBrepFormFaceIndex(const ON_Extrusion &extrusion,
    ...
    if (ON_UNSET_VALUE == extrusion_profile_parameter) {
        if (extrusion_profile_index != profile_count) {
            ...
            if (0 == profile || !profile->GetDomain(&t0, &t1) ||
                !ON_IsValid(t0) || !ON_IsValid(t1) || !(t0 < t1)) {
                return false;
            }
        }
        ...
        for (int i = 0; i < profile_count; i++) {
            ...
            // exactly the same way as the code that calculates component
            // indices.
            if (0 == profile || !profile->GetDomain(&t0, &t1) ||
                !ON_IsValid(t0) || !ON_IsValid(t1) || !(t0 < t1)) {
                return false;
            ...
            if (bCountProfileDiscontinuities) {
                while (
                    GetNextProfileSegmentDiscontinuity(profile, t0, t1, &t)) {
                    if (t0 < t && t < t1) {
...
bool ON_Extrusion::GetBrepFormComponentIndex(
    ON_COMPONENT_INDEX extrusion_ci, double extrusion_profile_parameter,
    const ON_Brep &brep_form, ON_COMPONENT_INDEX &brep_ci) const {
    brep_ci.UnSet();
    int face_index = -1;
    ...
    bool bCountProfileDiscontinuities =
        (brep_face_count > profile_count + cap_count);
    ...
    case ON_COMPONENT_INDEX::extrusion_top_profile:
        if (extrusion_ci.m_index < 0 || extrusion_ci.m_index >= profile_count)
            return false;
        if (!GetBrepFormFaceIndex(*this, extrusion_ci.m_index,
    ...
    case ON_COMPONENT_INDEX::extrusion_wall_edge:
        if (extrusion_ci.m_index < 0 ||
            extrusion_ci.m_index >= 2 * profile_count)
            return false;
        if (!GetBrepFormFaceIndex(*this, extrusion_ci.m_index / 2,
                                  extrusion_profile_parameter,
                                  bCountProfileDiscontinuities, &face_index,
                                  &face_profile_domain))
            return false;
    ...
    case ON_COMPONENT_INDEX::extrusion_cap_surface:
        ...
        } else if (!GetBrepFormFaceIndex(*this, profile_count,
                                         extrusion_profile_parameter,
                                         bCountProfileDiscontinuities,
                                         &face_index, &face_profile_domain)) {
            return false;
        }
...
ON_BOOL32 ON_Extrusion::SetDomain(int dir, // 0 sets first parameter's domain, 1
...
ON_BOOL32 ON_Extrusion::GetSpanVector( // span "knots"
    int dir, // 0 gets first parameter's domain, 1 gets second parameter's
             // domain
...
ON_BOOL32 ON_Extrusion::GetSpanVectorIndex(
    ...
    if (1 - path_dir == dir && m_profile) {
        return m_profile->GetSpanVectorIndex(t, side, span_vector_index,
                                             span_interval);
    }
    return false;
}
...
int ON_Extrusion::Degree( // returns maximum algebraic degree of any span
                          // ( or a good estimate if curve spans are not
                          // algebraic )
    int dir // 0 gets first parameter's domain, 1 gets second parameter's domain
) const {
    const int path_dir = PathParameter();
...
ON_BOOL32
ON_Extrusion::GetParameterTolerance( // returns tminus < tplus: parameters
                                     // tminus <= s <= tplus
...
ON_BOOL32 ON_Extrusion::IsPlanar(ON_Plane *plane, double tolerance) const {
...
// ON_BOOL32 ON_Extrusion::IsSingular( // true if surface side is collapsed to a
// point
//      int        // side of parameter space to test
//                 // 0 = south, 1 = east, 2 = north, 3 = west
//      ) const
bool ON_Extrusion::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
                                        double t1, double *t, int *hint,
                                        int *dtype, double cos_angle_tolerance,
...
bool ON_Extrusion::IsContinuous(ON::continuity c, double s, double t, int *hint,
    ...
    return m_profile->IsContinuous(c, curvet, crv_hint, point_tolerance,
...
ON_BOOL32 ON_Extrusion::Transpose() // transpose surface parameterization (swap
                                    // "s" and "t")
{
...
ON_BOOL32 ON_Extrusion::Evaluate( // returns false if unable to evaluate
    ...
    if (num_der > 0) {
        ...
        for (i = num_der; i > 0; i--) {
            ...
            if (1 == i) {
                ...
                // xformD transform is applied to a curve derivative ((x,y) =
                // vector)
            ...
            // xformP transformation is applied to a curve derivative ((x,y) =
            // vector)
    ...
    if (m_bTransposed && num_der > 0) {
        ...
        const size_t sz =
            ((3 <= array_stride) ? 3 : array_stride) * sizeof(double);
        void *tmp = (sz <= sizeof(xform0)) ? ((void *)&xform0.m_xform[0][0])
                                           : onmalloc(sz);
        for (i = 1; i <= num_der; i++) {
...
ON_Curve *ON_Extrusion::IsoCurve(int dir, double c) const {
    ...
    if (1 == dir) {
        ...
        xform1.m_xform[0][0] =
            s0 * xform0.m_xform[0][0] + s1 * xform1.m_xform[0][0];
        xform1.m_xform[0][1] =
            s0 * xform0.m_xform[0][1] + s1 * xform1.m_xform[0][1];
        xform1.m_xform[0][2] =
            s0 * xform0.m_xform[0][2] + s1 * xform1.m_xform[0][2];
        xform1.m_xform[0][3] =
            s0 * xform0.m_xform[0][3] + s1 * xform1.m_xform[0][3];

        xform1.m_xform[1][0] =
            s0 * xform0.m_xform[1][0] + s1 * xform1.m_xform[1][0];
        xform1.m_xform[1][1] =
            s0 * xform0.m_xform[1][1] + s1 * xform1.m_xform[1][1];
        xform1.m_xform[1][2] =
            s0 * xform0.m_xform[1][2] + s1 * xform1.m_xform[1][2];
        xform1.m_xform[1][3] =
            s0 * xform0.m_xform[1][3] + s1 * xform1.m_xform[1][3];

        xform1.m_xform[2][0] =
            s0 * xform0.m_xform[2][0] + s1 * xform1.m_xform[2][0];
        xform1.m_xform[2][1] =
            s0 * xform0.m_xform[2][1] + s1 * xform1.m_xform[2][1];
        xform1.m_xform[2][2] =
            s0 * xform0.m_xform[2][2] + s1 * xform1.m_xform[2][2];
        xform1.m_xform[2][3] =
            s0 * xform0.m_xform[2][3] + s1 * xform1.m_xform[2][3];

        xform1.m_xform[3][0] =
            s0 * xform0.m_xform[3][0] + s1 * xform1.m_xform[3][0];
        xform1.m_xform[3][1] =
            s0 * xform0.m_xform[3][1] + s1 * xform1.m_xform[3][1];
        xform1.m_xform[3][2] =
            s0 * xform0.m_xform[3][2] + s1 * xform1.m_xform[3][2];
        xform1.m_xform[3][3] =
            s0 * xform0.m_xform[3][3] + s1 * xform1.m_xform[3][3];
...
//                  ) const
ON_BOOL32 ON_Extrusion::Trim(int dir, const ON_Interval &domain) {
    bool rc = false;
...
ON_BOOL32 ON_Extrusion::Split(int dir, double c,
...
bool ON_Extrusion::GetSurfaceParameterFromNurbFormParameter(
    double nurbs_s, double nurbs_t, double *surface_s,
    ...
    if (surface_s) {
        rc = m_profile ? (m_profile->GetCurveParameterFromNurbFormParameter(
                              nurbs_s, surface_s)
                              ? true
                              : false)
                       : false;
...
bool ON_Extrusion::GetNurbFormParameterFromSurfaceParameter(
    double surface_s, double surface_t, double *nurbs_s,
    ...
    if (nurbs_s) {
        rc = m_profile ? (m_profile->GetNurbFormParameterFromCurveParameter(
                              surface_s, nurbs_s)
                              ? true
                              : false)
                       : false;
...
ON_SumSurface *ON_Extrusion::SumSurfaceForm(ON_SumSurface *sum_surface) const {
    ...
    ON_SumSurface *sumsrf =
        (0 != sum_surface) ? sum_surface : new ON_SumSurface();
    if (0 == sumsrf) {
        delete curve0;
        delete curve1;
        return 0;
...
ON_Extrusion *ON_Extrusion::Pipe(const ON_Cylinder &cylinder,
                                 double other_radius, bool bCapBottom,
