...
bool ON_Localizer::CreateCylinderLocalizer(ON_3dPoint P, ON_3dVector V,
                                           double r0, double r1) {
    Destroy();
...
bool ON_Localizer::CreatePlaneLocalizer(ON_3dPoint P, ON_3dVector N, double h0,
                                        double h1) {
    Destroy();
...
bool ON_Localizer::CreateSphereLocalizer(ON_3dPoint P, double r0, double r1) {
    Destroy();
    if (P.IsValid() && ON_IsValid(r0) && ON_IsValid(r1) && r0 > 0.0 &&
        r1 > 0.0 && r0 != r1) {
        m_P = P;
        m_V.Zero();
        m_d.Set(r0, r1);
...
bool ON_SpaceMorph::Ev1Der(ON_3dPoint rst, ON_3dPoint &xyz, ON_3dVector &Dr,
...
double ON_SpaceMorph::Tolerance() const { return m_tolerance; }

void ON_SpaceMorph::SetTolerance(double tolerance) {
    m_tolerance = (ON_IsValid(tolerance) && tolerance > 0.0) ? tolerance : 0.0;
}

bool ON_SpaceMorph::QuickPreview() const { return m_bQuickPreview; }
...
bool ON_SpaceMorph::PreserveStructure() const { return m_bPreserveStructure; }

void ON_SpaceMorph::SetPreserveStructure(bool bPreserveStructure) {
    m_bPreserveStructure = bPreserveStructure ? true : false;
}

void ON_SpaceMorph::MorphPointList(int dim, int is_rat, int count, int stride,
                                   double *point) const {
    if (dim > 0 && stride >= (dim + (is_rat) ? 1 : 0) && count > 0 &&
        point != 0) {
        int i;
        if (is_rat) {
...
bool ON_Mesh::EvaluatePoint(const class ON_ObjRef &objref,
    ...
    case ON_COMPONENT_INDEX::meshtop_edge:
        if (5 == objref.m_evp.m_t_type &&
            fabs(objref.m_evp.m_t[0] + objref.m_evp.m_t[1] - 1.0) <=
                ON_SQRT_EPSILON) {
    ...
    case ON_COMPONENT_INDEX::mesh_face:
        if (4 == objref.m_evp.m_t_type &&
            fabs(objref.m_evp.m_t[0] + objref.m_evp.m_t[1] +
                 objref.m_evp.m_t[2] + objref.m_evp.m_t[3] - 1.0) <=
                ON_SQRT_EPSILON) {
