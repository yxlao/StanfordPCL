...
ON_BOOL32 ON_Cone::Create(const ON_Plane &p, double h, double r) {
    plane = p;
    height = h;
    radius = r;
    return IsValid();
...
const ON_3dVector &ON_Cone::Axis() const { return plane.zaxis; }

const ON_3dPoint &ON_Cone::ApexPoint() const { return plane.origin; }
...
double ON_Cone::AngleInRadians() const {
    return height == 0.0 ? (radius != 0.0 ? ON_PI : 0.0)
                         : atan(radius / height);
}
...
bool ON_Cone::ClosestPointTo(ON_3dPoint point, double *radial_parameter,
                             double *height_parameter) const {
    // untested code
    ...
    if (height_parameter) {
        ...
        v.x *= radius;
        v.y *= radius;
        ON_Line line(ON_origin, v.x * plane.xaxis + v.y * plane.yaxis +
                                    height * plane.zaxis);
...
ON_BOOL32 ON_Cone::Rotate(double sin_angle, double cos_angle,
                          const ON_3dVector &axis_of_rotation) {
    return Rotate(sin_angle, cos_angle, axis_of_rotation, plane.origin);
}

ON_BOOL32 ON_Cone::Rotate(double angle, const ON_3dVector &axis_of_rotation) {
...
ON_BOOL32 ON_Cone::Rotate(double sin_angle, double cos_angle,
    ...
    return plane.Rotate(sin_angle, cos_angle, axis_of_rotation,
                        center_of_rotation);
}

ON_BOOL32 ON_Cone::Rotate(double angle,            // angle in radians
...
ON_RevSurface *ON_Cone::RevSurfaceForm(ON_RevSurface *srf) const {
    ...
    if (IsValid()) {
        ...
        ON_LineCurve *line_curve =
            new ON_LineCurve(line, line_domain[0], line_domain[1]);
        if (srf)
            pRevSurface = srf;
        else
...
pBrep->m_C2.Count()-1 ); trim.m_tolerance[0] = 0.0; trim.m_tolerance[1] = 0.0;
