...
ON_OBJECT_IMPLEMENT(ON_SumSurface, ON_Surface,
...
ON_SumSurface *ON_SumSurface::New() { return new ON_SumSurface(); }
...
void ON_SumSurface::Destroy() {
    // GBA 24-Sept-2004. Since there now could be a surface tree it need to be
    // cleared out.
    DestroyRuntimeCache();
    for (int i = 0; i < 2; i++) {
        if (m_curve[i]) {
...
void ON_SumSurface::ClearBoundingBox() { m_bbox.Destroy(); }
...
ON_BOOL32 ON_SumSurface::SetDomain(int dir, // 0 sets first parameter's domain,
...
ON_BOOL32
ON_SumSurface::GetParameterTolerance( // returns tminus < tplus: parameters
                                      // tminus <= s <= tplus
...
ON_BOOL32 ON_SumSurface::IsPlanar(ON_Plane *plane, double tolerance) const {
    ON_Plane pln;
    ON_3dPoint center;
...
ON_BOOL32 ON_SumSurface::IsSingular(int side) const { return false; }

bool ON_SumSurface::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
    ...
    if (0 == dir || 1 == dir) {
        if (0 != m_curve[dir]) {
            rc = m_curve[dir]->GetNextDiscontinuity(
                c, t0, t1, t, (hint ? &hint[dir] : 0), dtype,
...
bool ON_SumSurface::IsContinuous(
    ...
    if (m_curve[0] && m_curve[1]) {
        ...
        rc = m_curve[0]->IsContinuous(
            desired_continuity, s, &crv_hint[0], point_tolerance, d1_tolerance,
            d2_tolerance, cos_angle_tolerance, curvature_tolerance);
        if (rc)
            rc = m_curve[1]->IsContinuous(desired_continuity, t, &crv_hint[1],
                                          point_tolerance, d1_tolerance,
...
ON_BOOL32 ON_SumSurface::Evaluate( // returns false if unable to evaluate
    ...
    if (dim > 0) {
        ...
        rc = m_curve[0]->Evaluate(s, nder, dim, v0, side0,
                                  hint ? &crv_hint[0] : 0);
        if (rc)
            rc = m_curve[1]->Evaluate(t, nder, dim, v1, side1,
                                      hint ? &crv_hint[1] : 0);
        if (rc) {
            int j, ds, dt, der;
            for (j = 0; j < dim; j++) {
...
int ON_SumTensor::DimensionA() const { return dim; }

int ON_SumTensor::DimensionB() const { return dim; }

int ON_SumTensor::DimensionC() const { return dim; }

bool ON_SumTensor::Evaluate(double a, const double *CurveA, double b,
                            const double *CurveB, double *SrfPoint) {
...
int ON_SumSurface::GetNurbForm(ON_NurbsSurface &nurbs_surface,
    ...
    if (dim > 0) {
        ...
        if (nurbs_curveA && nurbs_curveB) {
            ...
            if (!nurbs_surface.TensorProduct(*nurbs_curveA, *nurbs_curveB,
                                             sum_tensor))
                nurbs_surface.Destroy();
            else
                rc = (rcA >= rcB) ? rcA : rcB;
...
bool ON_SumSurface::GetSurfaceParameterFromNurbFormParameter(
    ...
    *surface_t = nurbs_t;
    if (m_curve[0]) {
        if (!m_curve[0]->GetCurveParameterFromNurbFormParameter(nurbs_s,
                                                                surface_s))
            rc = false;
    }

    if (m_curve[1]) {
        if (!m_curve[1]->GetCurveParameterFromNurbFormParameter(nurbs_t,
                                                                surface_t))
            rc = false;
    }
...
bool ON_SumSurface::GetNurbFormParameterFromSurfaceParameter(
    ...
    *nurbs_t = surface_t;
    if (m_curve[0]) {
        if (!m_curve[0]->GetNurbFormParameterFromCurveParameter(surface_s,
                                                                nurbs_s))
            rc = false;
    }

    if (m_curve[1]) {
        if (!m_curve[1]->GetNurbFormParameterFromCurveParameter(surface_t,
                                                                nurbs_t))
            rc = false;
    }
    return rc;
...
ON_BOOL32 ON_SumSurface::Trim(int dir, const ON_Interval &domain)
...
bool ON_SumSurface::Extend(int dir, const ON_Interval &domain) {
    if (dir < 0 || dir > 1)
        return false;
    if (IsClosed(dir))
        return false;
...
ON_BOOL32 ON_SumSurface::Split(int dir, double c,
