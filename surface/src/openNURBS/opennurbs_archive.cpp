...
int ON_FileStream::Close(FILE *fp) { return ((0 != fp) ? fclose(fp) : -1); }
...
ON__UINT64 ON_FileStream::Write(FILE *fp, ON__UINT64 count,
                                const void *buffer) {
    ON__UINT64 rc = 0;
    if (0 == fp || count <= 0 || 0 == buffer)
        return rc;
...
bool ON_FileStream::GetFileInformation(FILE *fp, ON__UINT64 *file_size,
...
ON_ReadChunkHelper::ON_ReadChunkHelper(ON_BinaryArchive &binary_archive)
    : m_binary_archive(binary_archive), m_bReadSuccess(0), m_chunk_tcode(0),
      m_chunk_value(0), m_bCallEndRead3dmChunk(0) {
    m_bReadSuccess =
...
bool ON_IsLongChunkTypecode(ON__UINT32 typecode) {
    // NOTE: RenderXXXX plug-in used zero as a typecode in material userdata,
    // sigh ...
...
static bool DownSizeChunkValue(ON__UINT32 typecode, ON__INT64 v64,
                               ON__INT32 *v32) {
    if (0 == v32)
        return true;
...
ON_BinaryArchive::ON_BinaryArchive(ON::archive_mode mode)
    : m_3dm_version(0), m_3dm_v1_layer_index(0), m_3dm_v1_material_index(0),
      m_error_message_mask(0), m_3dm_opennurbs_version(0),
      m_3dm_start_section_offset(0),
      m_active_table(ON_BinaryArchive::no_active_table), m_bDoChunkCRC(0),
      m_bad_CRC_count(0), m_endian(ON::Endian()), m_mode(mode) {
    ...
    m_bEmbedTextureBitmaps = false; // true to embed texture, bump, trace, and
                                    // wallpaper bitmaps (increases file size)
    m_bSaveRenderMeshes = false;    // true to save meshes used to render B-rep
                                    // objects (increases file size)
    m_bSaveAnalysisMeshes = false;  // true to save meshes used in surface
                                    // analysis (increases file size)
...
bool ON_BinaryArchive::BigSeekFromStart(ON__UINT64 offset) {
    ...
    return (offset > 2147483632) ? (SeekFromStart(2147483632) &&
                                    BigSeekForward(offset - 2147483632))
...
bool ON_BinaryArchive::ReadChar( // Read an array of 8 bit chars
    size_t count,                // number of chars to read
    char *p) {
    return ReadByte(count, p);
}

bool ON_BinaryArchive::ReadChar( // Read an array of 8 bit unsigned chars
    size_t count,                // number of unsigned chars to read
    unsigned char *p) {
    return ReadByte(count, p);
}

bool ON_BinaryArchive::ReadChar( // Read a single 8 bit char
    char *p) {
    return ReadByte(1, p);
}

bool ON_BinaryArchive::ReadChar( // Read a single 8 bit unsigned char
    unsigned char *p) {
    return ReadByte(1, p);
}

bool ON_BinaryArchive::ReadInt16( // Read an array of 16 bit integers
...
bool ON_BinaryArchive::ReadShort( // Read an array of 16 bit shorts
    size_t count,                 // number of unsigned chars to read
    short *p) {
#if defined(ON_COMPILER_MSC)
#pragma warning(push)
...
bool ON_BinaryArchive::ReadShort( // Read an array of 16 bit unsigned shorts
    size_t count,                 // number of unsigned chars to read
    unsigned short *p) {
    return ReadShort(count, (short *)p);
}

bool ON_BinaryArchive::ReadShort( // Read a single 16 bit short
    short *p) {
    return ReadShort(1, p);
}

bool ON_BinaryArchive::ReadShort( // Read a single 16 bit unsigned short
    unsigned short *p) {
    return ReadShort(1, p);
}

bool ON_BinaryArchive::ReadInt32( // Read an array of 32 bit integers
...
bool ON_BinaryArchive::ReadInt( // Read an array of integers
    size_t count,               // number of unsigned chars to read
    int *p) {
#if defined(ON_COMPILER_MSC)
#pragma warning(push)
...
bool ON_BinaryArchive::ReadInt( // Read an array of 32 bit integers
    size_t count,               // number of unsigned chars to read
    unsigned int *p) {
    return ReadInt(count, (int *)p);
}

bool ON_BinaryArchive::ReadInt( // Read a single 32 bit integer
    int *p) {
    return ReadInt(1, p);
}

bool ON_BinaryArchive::ReadInt( // Read a single 32 bit unsigned integer
    unsigned int *p) {
    return ReadInt(1, p);
}
...
bool ON_BinaryArchive::ReadLong( // Read an array of 32 bit integers
    size_t count,                // number of unsigned chars to read
    long *p) {
#if defined(ON_COMPILER_MSC)
#pragma warning(push)
...
bool ON_BinaryArchive::ReadLong( // Read a single 32 bit integer
    long *p) {
    return ReadLong(1, (long *)p);
}

bool ON_BinaryArchive::ReadLong( // Read a single 32 bit unsigned integer
    unsigned long *p) {
    return ReadLong(1, (long *)p);
}

bool ON_BinaryArchive::ReadFloat( // Read an array of floats
    size_t count,                 // number of unsigned chars to read
    float *p) {
...
bool ON_BinaryArchive::ReadFloat( // Read a single float
    float *p) {
    return ReadFloat(1, p);
}

bool ON_BinaryArchive::ReadDouble( // Read an array of IEEE 64 bit doubles
    size_t count,                  // number of unsigned chars to read
    double *p) {
...
bool ON_BinaryArchive::ReadDouble( // Read a single double
    double *p) {
...
bool ON_BinaryArchive::ReadPoint(ON_2dPoint &p) { return ReadDouble(2, &p.x); }

bool ON_BinaryArchive::ReadPoint(ON_3dPoint &p) { return ReadDouble(3, &p.x); }

bool ON_BinaryArchive::ReadPoint(ON_4dPoint &p) { return ReadDouble(4, &p.x); }
...
bool ON_BinaryArchive::WritePlaneEquation(
...
bool ON_BinaryArchive::ReadStringSize( // Read size of NULL terminated string
    ...
    if (rc) {
        ...
        if (0 != (0xF000000 & ui32)) {
            ON_ERROR("ON_BinaryArchive::ReadStringSize() length is impossibly "
                     "large");
            rc = false;
        } else if (ui32 > 5000) {
            ...
            if (0 != curchunk && 0 == (TCODE_SHORT & curchunk->m_typecode)) {
                if (curchunk->m_big_value < 0 ||
                    curchunk->m_big_value >=
                            0x0FFFFFFF // 268 million chars oughta be plenty
                        && ui32 > ((ON__UINT32)curchunk->m_big_value)) {
                    ON_ERROR("ON_BinaryArchive::ReadStringSize() length "
                             "exceeds current chunk size");
                    rc = false;
                }
            }
        }
...
bool ON_BinaryArchive::ReadString( // Read NULL terminated string
...
bool ON_BinaryArchive::ReadString( // Read NULL terminated unicode string
    size_t sizeof_string,          // length = value from ReadStringSize()
...
bool ON_BinaryArchive::ReadString(ON_String &s) {
    ...
    if (rc && sizeof_string > 0) {
        const int isizeof_string =
            (int)sizeof_string; // (int) converts 64 bits size_t
...
#pragma warning(disable : 4127)
    ...
    if (rc && sizeof_string > 0) {
        ...
        if (2 == sizeof(wchar_t)) {
            ...
            // UNICODE strings are stored in 3DM files as arrays of 2 byte
            // integers
            //
...
bool ON_BinaryArchive::WriteArray(
    const ON_SimpleArray<ON_DisplayMaterialRef> &a) {
    int i, count = a.Count();
    if (count < 0)
        count = 0;
    bool rc = WriteInt(count);
...
bool ON_BinaryArchive::ReadArray(ON_SimpleArray<ON_ClippingPlaneInfo> &a) {
    a.Empty();
    int count = 0;
    bool rc = ReadInt(&count);
    if (rc && count > 0) {
...
bool ON_BinaryArchive::WriteChar( // Write an array of 8 bit chars
    size_t count,                 // number of chars to write
    const char *p) {
    return WriteByte(count, p);
}

bool ON_BinaryArchive::WriteChar( // Write an array of 8 bit unsigned chars
    size_t count,                 // number of unsigned chars to write
    const unsigned char *p) {
    return WriteByte(count, p);
}

bool ON_BinaryArchive::WriteChar( // Write a single 8 bit char
    char c) {
    return WriteByte(1, &c);
}

bool ON_BinaryArchive::WriteChar( // Write a single 8 bit unsigned char
    unsigned char c) {
    return WriteByte(1, &c);
}

bool ON_BinaryArchive::WriteInt16( // Write an array of 16 bit shorts
...
bool ON_BinaryArchive::WriteShort( // Write an array of 16 bit shorts
    size_t count,                  // number of shorts to write
    const short *p) {
#if defined(ON_COMPILER_MSC)
#pragma warning(push)
...
bool ON_BinaryArchive::WriteShort( // Write an array of 16 bit unsigned shorts
    size_t count,                  // number of shorts to write
    const unsigned short *p) {
    return WriteShort(count, (const short *)p);
}

bool ON_BinaryArchive::WriteShort( // Write a single 16 bit short
    short s) {
    return WriteShort(1, &s);
}

bool ON_BinaryArchive::WriteShort( // Write a single 16 bit unsigned short
    unsigned short s) {
    return WriteShort(1, &s);
}

bool ON_BinaryArchive::WriteInt32( // Write an array of 32 bit integers
...
bool ON_BinaryArchive::ReadInt64( // Read an array of 64 bit integers
...
bool ON_BinaryArchive::WriteInt64( // Write an array of 64 bit integers
    size_t count,                  // number of ints to write
    const ON__INT64 *p) {
    bool rc = true;
    if (m_endian == ON::big_endian) {
...
bool ON_BinaryArchive::WriteInt( // Write an array of integers
    size_t count,                // number of ints to write
    const int *p) {
#if defined(ON_COMPILER_MSC)
#pragma warning(push)
...
bool ON_BinaryArchive::WriteInt( // Write an array of 32 bit integers
    size_t count,                // number of ints to write
    const unsigned int *p) {
    return WriteInt(count, (const int *)p);
}

bool ON_BinaryArchive::WriteInt( // Write a single 32 bit integer
    int i) {
    return WriteInt(1, &i);
}
...
bool ON_BinaryArchive::WriteLong( // Write an array of longs
...
bool ON_BinaryArchive::WriteLong( // Write a single long
    long i) {
    return WriteLong(1, &i);
}

bool ON_BinaryArchive::WriteLong( // Write a single unsigned long
    unsigned long i) {
    return WriteLong(1, &i);
}

bool ON_BinaryArchive::WriteFloat( // Write a number of IEEE floats
...
bool ON_BinaryArchive::WriteFloat( // Write a single float
    float f) {
    return WriteFloat(1, &f);
}

bool ON_BinaryArchive::WriteDouble( // Write a single double
    size_t count,                   // number of doubles
    const double *p) {
    bool rc = true;
    if (m_endian == ON::big_endian) {
...
bool ON_BinaryArchive::WriteDisplayMaterialRef(
    const ON_DisplayMaterialRef &dmr) {
...
// to load the string.
bool ON_BinaryArchive::WriteString( // Write NULL terminated string
    const char *s) {
    size_t sizeof_string = 0;
    if (s) {
...
bool ON_BinaryArchive::WriteString( // Write NULL terminated unicode string
...
bool ON_BinaryArchive::WriteArray(
    const ON_SimpleArray<ON_SurfaceCurvature> &a) {
    int count = a.Count();
    if (count < 0)
        count = 0;
    bool rc = WriteInt(count);
...
bool ON_BinaryArchive::WriteArray(
    const ON_SimpleArray<ON_ClippingPlaneInfo> &a) {
    int count = a.Count();
    if (count < 0)
        count = 0;
    bool rc = WriteInt(count);
    ...
    if (o)
        ...
        if (rc) {
            ...
            if (rc) {
                ...
                if (!EndWrite3dmChunk()) // end of TCODE_OPENNURBS_CLASS_UUID
                                         // chunk
                    rc = false;
            }
            if (!EndWrite3dmChunk())
...
bool ON_BinaryArchive::WriteObject(const ON_Object &o) {
    ...
    if (m_3dm_version <= 4 && ON::extrusion_object == o.ObjectType()) {
        ...
        if (0 != extrusion) {
            ...
            if (0 != v4object &&
                ON::extrusion_object !=
                    v4object->ObjectType() // no infinte recursion!
            ) {
    ...
    if (rc) {
        ...
        if (rc) {
            ...
            if (rc) {
                ...
                if (!rc) {
                    ON_ERROR(
                        "ON_BinaryArchive::WriteObject() o.Write() failed.");
                }
                if (!EndWrite3dmChunk()) // end of TCODE_OPENNURBS_CLASS_DATA
                                         // chunk
                    rc = false;
            }
        }
...
bool ON_BinaryArchive::WriteObjectUserData(const ON_Object &object) {
    ...
    for (ud = object.m_userdata_list; ud && rc; ud = ud->m_userdata_next) {
        ...
        if (0 == ON_UuidCompare(userdata_classid,
                                ON_UserData::m_ON_UserData_class_id.Uuid()))
            continue;
        if (0 == ON_UuidCompare(userdata_classid,
                                ON_Object::m_ON_Object_class_id.Uuid()))
            continue;
        if (0 ==
            ON_UuidCompare(userdata_classid,
        ...
        if (3 == m_3dm_version) {
            ...
            if (m_V3_plugin_id_list.BinarySearch(&ud->m_application_uuid,
                                                 ON_UuidCompare) < 0)
                continue;
        }
        ...
        if (0 == unknown_ud) {
            if (ud->IsUnknownUserData()) {
                ON_ERROR("ON_UnknownUserData::Cast(ud) is null and "
                         "ud->IsUnknownUserData() is true.");
                continue; // something's wrong
            }
        } else {
            if (!ud->IsUnknownUserData()) {
                ON_ERROR("ON_UnknownUserData::Cast(ud) is not null and "
                         "ud->IsUnknownUserData() is false.");
                continue; // something's wrong
            }
        }
        ...
        if (rc) {
            ...
            // wrap user data header info in an
            ...
            if (rc) {
                ...
                if (rc) {
                    ...
                    int ver =
                        unknown_ud ? unknown_ud->m_3dm_version : m_3dm_version;
                    rc = WriteInt(ver);
...
int ON_BinaryArchive::ReadObjectHelper(ON_Object **ppObject) {
    ...
    //            this could happen in cases where old code is attempting to
    //            read new objects.
    ON__UINT32 tcode;
    ...
    // all ON_Objects written by WriteObject are in a TCODE_OPENNURBS_CLASS
    // chunk
    ...
    //
    //  TCODE_OPENNURBS_CLASS, length = 20 + 3*sizeof_chunk_header +
    //  length_DATA_chunk + length_USER_DATA_chunk(s)
    //
    //    TCODE_OPENNURBS_CLASS_UUID, length = 20
    //      16 byte nil uuid
    //      4 byte TCODE_OPENNURBS_CLASS_UUID crc
    ...
    if (tcode != TCODE_OPENNURBS_CLASS) {
        ON_ERROR("ON_BinaryArchive::ReadObject() didn't find "
                 "TCODE_OPENNURBS_CLASS block.");
        rc = 0;
    } else if (length_TCODE_OPENNURBS_CLASS <
               expected_length_TCODE_OPENNURBS_CLASS_UUID +
                   sizeof_chunk_header) {
        ON_ERROR("ON_BinaryArchive::ReadObject() TCODE_OPENNURBS_CLASS chunk "
                 "length too small.");
        rc = 0;
    } else {
        ...
        for (;;) {
            ...
            if (tcode != TCODE_OPENNURBS_CLASS_UUID) {
                ...
                rc = 0;
                       ...
                       length_TCODE_OPENNURBS_CLASS_UUID) {
                ON_ERROR("ON_BinaryArchive::ReadObject() "
                         "TCODE_OPENNURBS_CLASS_UUID has invalid length");
                rc = 0;
            ...
            if (pObject) {
                pID = pObject->ClassId();
                if (uuid != pID->Uuid()) {
                    ON_ERROR("ON_BinaryArchive::ReadObject() - uuid does not "
                             "match intput pObject's class id.");
                    pID = 0;
                    rc = 2;
                    break;
                }
            ...
            if (!pID) {
                // If you get here and you are not calling ON::Begin() at some
                // point in your executable, then call ON::Begin() to force all
                // class definition to be linked. If you are callig ON::Begin(),
                // then either the uuid is garbage or you are attempting to read
                // an object with old code. Visit http://www.opennurbs.org to
                // get the latest OpenNURBS code.
                ON_WARNING("ON_BinaryArchive::ReadObject() "
                           "ON_ClassId::ClassId(uuid) returned NULL.");
                rc = 3;
                break;
            }
            ///////////////////////////////////////////////////////////////////////////////
            ...
            // /////////////////////////////////////////////////
            rc = BeginRead3dmBigChunk(&tcode,
                                      &length_TCODE_OPENNURBS_CLASS_DATA);
            if (!rc)
                break;
            if (tcode != TCODE_OPENNURBS_CLASS_DATA) {
                ...
                ON_ERROR("ON_BinaryArchive::ReadObject() "
                         "TCODE_OPENNURBS_CLASS_DATA chunk length too small");
                rc = 0;
            } else {
                if (!pObject) {
                    pObject = pID->Create();
                ...
                if (!pObject) {
                    ON_ERROR("ON_BinaryArchive::ReadObject() pID->Create() "
                             "returned NULL.");
                    rc = 0;
                } else {
            ...
            if (rc && 0 != pObject) {
                // read user data
                // /////////////////////////////////////////////////
                //   If TCODE_OPENNURBS_CLASS_USERDATA chunks exist, this reads
                //   them. ReadObjectUserData() stops when it reads a
                //   TCODE_OPENNURBS_CLASS_END chunk.
...
bool ON_BinaryArchive::ReadObjectUserDataAnonymousChunk(
    ...
    const int archive_3dm_version, const int archive_opennurbs_version,
    ON_UserData *ud) {
    ...
    if (ud->IsUnknownUserData() ||
        ...
        const size_t sizeof_buffer =
            (size_t)(length_TCODE_ANONYMOUS_CHUNK + 4 + SizeofChunkLength());
        void *freeme = 0;
        void *buffer =
            (sizeof_buffer <= sizeof(stack_buffer))
                ? &stack_buffer[0]
                : (freeme = onmalloc(
                       sizeof_buffer)); // generally, object userdata is small
                                        // we almost never use heap
        if (0 != buffer && sizeof_buffer == ReadBuffer(sizeof_buffer, buffer)) {
            ON_Read3dmBufferArchive memory_archive(sizeof_buffer, buffer, false,
                                                   archive_3dm_version,
            ...
            // when reading the chunk length of the TCODE_ANONYMOUS_CHUNK
            // wrapper.
...
void CUserDataHeaderInfo::Initialize() { memset(this, 0, sizeof(*this)); }
...
static bool ReadObjectUserDataHeaderHelper(ON_BinaryArchive &binary_archive,
    ...
    if (major_userdata_version == 2) {
        ...
        if (t != TCODE_OPENNURBS_CLASS_USERDATA_HEADER) {
            ON_ERROR("version 2.0 TCODE_OPENNURBS_CLASS_USERDATA chunk is "
                     "missing TCODE_OPENNURBS_CLASS_USERDATA_HEADER chunk.");
    ...
    if (major_userdata_version == 2) {
        if (minor_userdata_version >= 1) {
            ...
            if (minor_userdata_version >= 2) {
                ...
                if (rc)
                    rc = binary_archive.ReadInt(
                        &ud_header.m_3dm_opennurbs_version);
            }
        }
        if (!binary_archive
                 .EndRead3dmChunk()) // end of
    ...
    if (0 == ud_header.m_3dm_version ||
        0 == ud_header.m_3dm_opennurbs_version) {
        ...
        ud_header.m_3dm_opennurbs_version =
            binary_archive.ArchiveOpenNURBSVersion();
...
bool ON_BinaryArchive::ReadObjectUserData(ON_Object &object) {
    ...
    while (rc) {
        ...
        if (TCODE_OPENNURBS_CLASS_END == ch.m_chunk_tcode) {
            // A short TCODE_OPENNURBS_CLASS_END chunk marks the end of the
            // opennurbs class
            break; // done
        }
        ...
        rc = Read3dmChunkVersion(&major_userdata_version,
                                 &minor_userdata_version);
        if (!rc) {
            ON_ERROR("Unable to read TCODE_OPENNURBS_CLASS_USERDATA chunk "
                     "version numbers");
            break;
        }
        ...
        // we should be ready to read a TCODE_ANONYMOUS_CHUNK containing
        // userdata
        ...
        for (;;) {
            ...
            if (t != TCODE_ANONYMOUS_CHUNK) {
                ON_ERROR("Reading object user data - unable to find "
                         "TCODE_ANONYMOUS_CHUNK");
                rc = false;
                break;
            }
            if (length_TCODE_ANONYMOUS_CHUNK < 4) {
                ON_ERROR("Reading object user data - length of "
                         "TCODE_ANONYMOUS_CHUNK < 4");
                rc = false;
                break;
            }
            break;
        ...
        // attempt to get an instance of the userdata class that saved this
        // information
        ON_UserData *ud = 0;
        for (;;) {
            ...
            if (0 == ud) {
                ON_ERROR("Reading object user data - unable to create userdata "
                         "class");
                if (tmp)
                    delete tmp;
                tmp = 0;
        ...
        if (0 == ud) {
            // no luck on this one
            // One reason can be that the plug-in userdata class has something
            // wrong with its ON_OBJECT_DECLARE/ON_OBJECT_IMPLEMENT stuff.
            ON_ERROR("Unable to create object user data class. Flawed class id "
                     "information.");
        ...
        if (ON_UuidIsNil(ud->m_application_uuid)) {
            if (ON_UuidIsNil(ud_header.m_appid)) {
                ...
                case 2:
                    ...
                    // This id flags the userdata as being read from a V3
                    // archive.
...
int ON_BinaryArchive::Archive3dmVersion() const { return m_3dm_version; }
...
bool ON_BinaryArchive::BeginWrite3dmChunk(unsigned int tcode, int major_version,
                                          int minor_version) {
    bool rc = false;
    if (0 == tcode) {
        ON_ERROR("ON_BinaryArchive::BeginWrite3dmChunk - input tcode = 0");
    } else if (0 != (tcode & TCODE_SHORT)) {
        ON_ERROR("ON_BinaryArchive::BeginWrite3dmChunk - input tcode has short "
                 "flag set.");
    } else if (major_version <= 0) {
        ON_ERROR(
            "ON_BinaryArchive::BeginWrite3dmChunk - input major_version <= 0.");
    } else if (minor_version < 0) {
        ON_ERROR(
            "ON_BinaryArchive::BeginWrite3dmChunk - input minor_version < 0.");
    } else {
        ...
        if (c->m_bLongChunk) {
            if (c->m_do_crc16) {
                ...
                if (c->m_crc16) {
                    ...
                    m_bad_CRC_count++;
                    ON_ERROR("ON_BinaryArchive::EndWrite3dmChunk: CRC16 "
                             "computation error.");
                }
            ...
            if (offset < c->m_big_offset) {
                ON_ERROR(
                    "ON_BinaryArchive::EndWrite3dmChunk() - chunk length < 0");
                rc = false;
            } else {
                ...
                if (CurrentPosition() != offset) {
                    ON_ERROR("ON_BinaryArchive::EndWrite3dmChunk() - "
                             "CurrentPosition() != offset");
                    rc = false;
                }
            }
        } else {
            // "short" chunks are completely written by call to
            // BeginWrite3dmChunk().
            rc = true;
        }
    ...
    if (goo.m_typecode) {
        ...
        if (0 != (goo.m_typecode & TCODE_SHORT)) {
            if (goo.m_value == 0 || (goo.m_value > 0 && goo.m_goo)) {
                // write long chunk - do not use Begin/EndWrite3dmChunk()
                // because
...
bool ON_BinaryArchive::PeekAt3dmBigChunkType(unsigned int *typecode,
                                             ON__INT64 *big_value) {
    // does not change file position
    ...
    if (ReadMode()) {
        ...
        if (c) {
            ...
            if (!ON_IsLongChunkTypecode(c->m_typecode)) {
                ON_ERROR("ON_BinaryArchive::Seek3dmChunkFromStart() - current "
                         "chunk is not a long chunk");
                return false;
            }
            if (c->m_big_value < 0) {
                ON_ERROR("ON_BinaryArchive::Seek3dmChunkFromStart() called "
                         "with an active chunk that has m_value < 0");
                return false;
            }
            if (pos0 < c->m_big_offset ||
                pos0 > c->m_big_offset + c->Length()) {
                ON_ERROR("ON_BinaryArchive::Seek3dmChunkFromStart() called "
                         "with out of bounds current position");
                return false;
            }
            rc = BigSeekBackward(
                pos0 - c->m_big_offset); // pos0 >= c->m_offset, so this size_t
                                         // subtraction is ok
        } else {
            // set archive position to the beginning of archive chunks by
            // skipping 32 byte version info and any start section padding.
            size_t start_offset =
                ((m_3dm_start_section_offset > 0) ? m_3dm_start_section_offset
                                                  : 0);
...
bool ON_BinaryArchive::Seek3dmChunkFromCurrentPosition(
    ...
    if (ReadMode()) {
        ...
        while (pos > pos_prev || bFirstTime) {
            ...
            if (0 == t) {
                // zero is not a valid typecode - file is corrupt at or before
                // this position
                break;
            }
            ...
            if (TCODE_ENDOFTABLE == t && 0 != v64) {
                // TCODE_ENDOFTABLE chunks always have value = 0 - file is
                // corrupt at or before this position
                break;
            }
        }
        if (!rc) {
...
bool ON_BinaryArchive::BeginRead3dmBigChunk(unsigned int *typecode,
    ...
    ON__INT64 v = 0;
    m_bDoChunkCRC =
        false; // no CRC on chunk headers because length is written twice
    ...
    if (rc) {
        if (t == TCODE_ENDOFFILE) {
            ...
            // that Rhino 1.0 or the pre-February 2000 Rhino 1.1 saved and
            // wrote.
            ...
            if (rc && v >= 0 && ((ON__UINT64)v) >= SizeofChunkLength()) {
                ...
                if (rc) {
                    if (BigSeekForward(EOF_chunk_length)) {
                        ...
                        if (m_3dm_version <= 1) {
                            if (!AtEnd()) {
                                // Rhino v1 reads chunks with unknown typecodes
                                // as a block of "goo" and then saves them back
                                // into file.  When this happens to an eof
                                // marker, we get TCODE_ENDOFFILE chunks in the
                                // middle of a file. This can only happen in
                                // m_3dm_version = 1 files.
                                t = TCODE_ENDOFFILE_GOO;
                            }
                        } else {
                            // check that current position matches saved file
                            // size
                            if (pos2 != sizeof_file) {
                                ON_ERROR("ON_BinaryArchive::BeginRead3dmChunk()"
                                         " - Rogue eof marker in v2 file.\n");
                            }
                        }
                ...
                ON_ERROR(
                    "ON_BinaryArchive::BeginRead3dmChunk() - file is damaged.");
                rc = false;
                t = 0; // ?? file is trashed ??
            }
        } else {
...
bool ON_BinaryArchive::BeginRead3dmChunk(unsigned int expected_tcode,
    ...
    if (0 == expected_tcode) {
        ...
        ON_ERROR("ON_BinaryArchive::BeginRead3dmChunk - input expected_tcode "
                 "has short flag set.");
    } else if (0 == major_version) {
        ON_ERROR(
            "ON_BinaryArchive::BeginRead3dmChunk - input major_version NULL");
    } else if (0 == minor_version) {
        ON_ERROR(
            "ON_BinaryArchive::BeginRead3dmChunk - input minor_version NULL");
    } else {
        *major_version = 0;
        *minor_version = 0;
        ...
        if (expected_tcode != tcode) {
            ...
            ON_ERROR("ON_BinaryArchive::BeginRead3dmChunk - unexpected chunk "
                     "length");
            rc = false;
        } else {
            tcode = 0;
            ...
            if (rc) {
                if (expected_tcode != tcode || value < 8) {
                    ...
                    ON_ERROR("ON_BinaryArchive::BeginRead3dmChunk - unexpected "
                             "tcode or chunk length - archive driver or device "
                             "may be bad");
                    rc = false;
                } else {
...
bool ON_BinaryArchive::EndRead3dmChunk() { return EndRead3dmChunk(false); }
...
bool ON_BinaryArchive::EndRead3dmChunk(bool bSupressPartiallyReadChunkWarning) {
    ...
    if (c) {
        ...
        if (c->m_bLongChunk) {
            if (c->m_big_value < 0) {
                ON_ERROR("ON_BinaryArchive::EndRead3dmChunk - negative chunk "
                         "length");
            } else {
            ...
            if (c->m_do_crc16) {
                if (file_offset + 2 == end_offset) {
                    ...
                    if (rc) {
                        ...
                        if (c->m_crc16) {
                            m_bad_CRC_count++;
                            ON_ERROR("ON_BinaryArchive::EndRead3dmChunk: CRC16 "
                                     "error.");
                        }
                    }
                } else {
                ...
                if (file_offset + 4 == end_offset) {
                    ...
                    if (rc) {
                        ...
                        if (crc0 != crc1) {
                            m_bad_CRC_count++;
                            ON_ERROR("ON_BinaryArchive::EndRead3dmChunk: CRC32 "
                                     "error.");
                        }
                    }
                } else {
        ...
        if (file_offset < c->m_big_offset) {
            ON_ERROR("ON_BinaryArchive::EndRead3dmChunk: current position "
                     "before start of current chunk.");
            if (!BigSeekFromStart(end_offset))
                rc = false;
        } else if (file_offset > end_offset) {
            ON_ERROR("ON_BinaryArchive::EndRead3dmChunk: current position "
                     "after end of current chunk.");
            if (!BigSeekFromStart(end_offset))
                rc = false;
        } else if (file_offset != end_offset) {
            // partially read chunk - happens when chunks are skipped or old
            // code reads a new minor version of a chunk whnich has added
            // information.
            if (file_offset != c->m_big_offset) {
                if (m_3dm_version != 1 || (m_error_message_mask & 0x02) == 0) {
                    ...
                    if (file_v <= app_v && filev_date <= appv_date) {
                        ...
                        if (!bSupressPartiallyReadChunkWarning) {
                            ON_WARNING("ON_BinaryArchive::EndRead3dmChunk: "
                                       "partially read chunk - skipping bytes "
                                       "at end of current chunk.");
                        }
                    }
                }
            }
...
bool ON_BinaryArchive::EndWriteDictionary() {
    ...
    bool rc = (chunk_count > 0 &&
               TCODE_DICTIONARY == m_chunk[chunk_count - 1].m_typecode);
    if (rc) {
...
bool ON_BinaryArchive::EndWriteDictionaryEntry() {
    int chunk_count = m_chunk.Count();
    bool rc = (chunk_count > 0 &&
               TCODE_DICTIONARY_ENTRY == m_chunk[chunk_count - 1].m_typecode)
                  ? EndWrite3dmChunk()
                  : false;
    return rc;
}
...
bool ON_BinaryArchive::BeginReadDictionary(ON_UUID *dictionary_id,
    ...
    int minor_version = 0;
    bool rc =
        BeginRead3dmChunk(TCODE_DICTIONARY, &major_version, &minor_version);
    if (rc) {
        for (;;) {
            rc = (1 == major_version);
            if (!rc)
            ...
            rc = BeginRead3dmChunk(TCODE_DICTIONARY_ID, &major_version,
                                   &minor_version);
            if (!rc)
                break;
            for (;;) {
...
bool ON_BinaryArchive::PushBigChunk(ON__UINT32 typecode, ON__INT64 big_value) {
    ...
    if (0 == (TCODE_SHORT & typecode) &&
        (0 != typecode || 1 != Archive3dmVersion())) {
...
bool ON_BinaryArchive::EnableSave3dmRenderMeshes(
    ON_BOOL32 b /* default = true */) {
...
bool ON_BinaryArchive::EnableSave3dmAnalysisMeshes(
    ON_BOOL32 b /* default = true */) {
...
bool ON_BinaryArchive::SaveUserData() const { return m_bSaveUserData; }
...
bool ON_BinaryArchive::Write3dmStartSection(int version,
    ...
    // 3dm archive version numbers >= 50 indicate the file has 8 byte chunk
    // lengths. Rather than change the hundreds of places in Rhino that use
    // 5,6,7,..., the "version *= 10" line handles it.
    ...
    if (rc) {
        ...
        if (rc) {
            // write information that helps determine what code wrote the 3dm
            // file
            char s[2048];
            size_t s_len = 0;
            memset(s, 0, sizeof(s));
            sprintf(s, " 3DM I/O processor: OpenNURBS toolkit version %d",
                    ON::Version());
        ...
        if (!EndWrite3dmChunk()) // need to call EndWrite3dmChunk() even if a
                                 // WriteByte() has failed
            rc = false;
    }
    return rc;
}
    ...
    if (rc) {
        ...
        if (rc) {
            ...
            if (rc) {
                if (typecode != 1) {
                    ...
                    if (length > 0x00FFFFFF) {
                        ON_ERROR("ON_BinaryArchive::Read3dmStartSection - "
                                 "start section string is unreasonably long.");
                        rc = false;
                    } else {
                        int slen = (int)length;
            ...
            if (rc && m_3dm_version == 1) {
                ...
                // a V2 bodies.  We haven't been able to track down the
                // application that is creating these damaged files, but we can
                // detect them and read them because they all have a
                // TCODE_PROPERTIES_TABLE chunk right after the start comments
                // chunk and no valid V1 file has a chunk with a
                // TCODE_PROPERTIES_TABLE tcode.
                //
                // Rhino 1.1 version 31-May-2000 reads 2.0 files as goo.  If a
                // user saves this file for some reason (no instances have been
                // reported) the resulting file has a V1 header, V1 fluff, and a
                // V2 body. This code will cause opennurbs to read the V2 body.
                ...
                // detects these files.
                {
                    ...
                    while (bCheckChunks) {
                        ...
                        case TCODE_NAMED_VIEW:
                            ...
                            bCheckChunks =
                                BeginRead3dmBigChunk(&typecode, &length);
                            if (bCheckChunks)
                        ...
                        default:
                            if (TCODE_TABLE == (typecode & 0xFFFF0000)) {
                                ...
                                ON_WARNING(
                                    "ON_BinaryArchive::Read3dmStartSection(): "
                                    "Archive has V1 header and V2 body. "
                                    "Continuing to read V2 body.");
                                m_3dm_version = 2;
                                if (version)
                                    *version = 2;
                            }
...
bool ON_BinaryArchive::Read3dmProperties(ON_3dmProperties &prop) {
    ...
    if (m_3dm_version != 1) {
        ...
        for (;;) {
                    ...
                    if (name) {
                        ...
                        if (*name) {
                            ...
                            if (!on_strnicmp(name, "Rhinoceros", 10)) {
                                prop.m_Application.m_application_URL =
                                    "http://www.rhino3d.com";
                                details = name + 10;
                                while (*details && *details <= 32)
                                    details++;
                                while ((*details >= '0' && *details <= '9') ||
                                       *details == '.')
                                    details++;
                            ...
                            if (details && *details)
                                prop.m_Application.m_application_details =
                                    details;
                        }
                    }
                }
            ...
            // this call to EndRead3dmChunk() skips any unread portions of the
            // chunk
            if (!EndRead3dmChunk()) {
                rc = false;
                bRewindFilePointer = true;
...
bool ON_BinaryArchive::BeginWrite3dmTable(unsigned int typecode) {
    ...
    if (m_active_table != no_active_table) {
        ON_ERROR("ON_BinaryArchive::BeginWrite3dmTable() m_active_table != "
                 "no_active_table");
        return false;
    }
    if (m_chunk.Count()) {
...
bool ON_BinaryArchive::EndWrite3dmTable(unsigned int typecode) {
    ...
    if (0 == c || c->m_typecode != typecode) {
        ON_ERROR("ON_BinaryArchive::EndWrite3dmTable() "
                 "m_chunk.Last()->typecode != typecode");
        return false;
    }
...
bool ON_BinaryArchive::BeginRead3dmTable(unsigned int typecode) {
    ...
    if (m_active_table != no_active_table) {
        ON_ERROR("ON_BinaryArchive::BeginRead3dmTable() m_active_table != "
                 "no_active_table");
        return false;
    }
    if (m_chunk.Count()) {
        ...
        if (rc) {
            if (tcode != typecode) {
                if (typecode == TCODE_USER_TABLE) {
                    ...
                    rc = false;
                    bReadTable = false;
                } else if (typecode == TCODE_GROUP_TABLE &&
                           m_3dm_opennurbs_version < 200012210) {
                    // 3DM archives written before version 200012210 and before
                    // do not have group tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_FONT_TABLE &&
                           m_3dm_opennurbs_version < 200109180) {
                    // 3DM archives written before version 200109180 and before
                    // do not have font tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_DIMSTYLE_TABLE &&
                           m_3dm_opennurbs_version < 200109260) {
                    // 3DM archives written before version 200109260 and before
                    // do not have dimstyle tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_INSTANCE_DEFINITION_TABLE &&
                           m_3dm_opennurbs_version < 200205110) {
                    // 3DM archives written before version 200205110 and before
                    // do not have instance definition tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_HATCHPATTERN_TABLE &&
                           m_3dm_opennurbs_version < 200405030) {
                    // 3DM archives written before version 200405030 and before
                    // do not have hatch pattern tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_LINETYPE_TABLE &&
                           m_3dm_opennurbs_version < 200503170) {
                    // 3DM archives written before version 200503170 and before
                    // do not have linetype tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_TEXTURE_MAPPING_TABLE &&
                           m_3dm_opennurbs_version < 200511110) {
                    // 3DM archives written before version 200511110 and before
                    // do not have texture mapping tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else if (typecode == TCODE_HISTORYRECORD_TABLE &&
                           m_3dm_opennurbs_version < 200601180) {
                    // 3DM archives written before version 200601180 and before
                    // do not have history record tables
                    rc = true;
                    m_active_table = tt;
                    bReadTable = false;
                } else {
                    // A required table is not at the current position in the
                    // archive see if we can find it someplace else in the
                    // archive.  This can happen when old code encounters a
                    // table that was added later.
                    ...
                    if (TCODE_HATCHPATTERN_TABLE == tcode &&
                        ...
                        // Dale Lear
                        //   V3 files from 19 may 2004 on contained bogus hatch
                        //   pattern tables where the instance definition table
                        //   was supposed to be.
                        //
                        // Do not set rc in this code.  The goal of this code is
                        // to avoid seeking from the start of the file and
                        // posting an ON_ERROR alert about something we can work
                        // around and has been fixed in V4.
                        tcode = 0;
                        big_value = 0;
                    ...
                    if (bSeekFromStart) {
                        ON_ERROR(
                            "ON_BinaryArchive::BeginRead3dmTable() - current "
                            "file position not at start of table - searching");
            ...
            if (rc && bReadTable) {
                ...
                if (rc && tcode != typecode) {
                    ON_ERROR("ON_BinaryArchive::BeginRead3dmTable() - corrupt "
                             "table - skipping");
                    rc = false;
                    EndRead3dmChunk();
                } else if (rc) {
...
static const unsigned char *BufferToChunkValue(bool bReverseByteOrder,
    ...
    if (8 == sizeof_chunk_value) {
        buffer =
            BufferToINT64(bReverseByteOrder, buffer, buffer_end, chunk_value);
    } else {
        ON__UINT32 u32;
        ON__UINT64 u64;
...
static const unsigned char *EmergencyFindTable_UuidHelper(
    bool bReverseByteOrder, size_t sizeof_chunk_value,
    const unsigned char *buffer, const unsigned char *buffer_end,
    const ON__UINT32 expected_tcode, const ON_UUID *expected_uuid) {
    ON__INT64 chunk_value;
    ON__UINT32 c, cc;
    ON_UUID uuid;
    ...
    // breaks down as:
    //  16                       sizeof(uuid)
    //  +4                     + sizeof(TCODE_USER_TABLE_RECORD_HEADER chunk
    //  typecode) +sizeof_chunk_value    + sizeof(TCODE_USER_TABLE_RECORD_HEADER
    //  chunk length) +21                    + major ver, minor ver, bool,
    //  archive ver, 3dm ver +4                     +
    //  sizeof(TCODE_USER_TABLE_RECORD_HEADER chunk crc) +4 +
    //  sizeof(TCODE_USER_TABLE_UUID chunk crc)
    const bool bLookForUserTableRecordHeader =
        (TCODE_USER_TABLE_UUID == expected_tcode &&
         ((ON__UINT64)chunk_value) >=
             (16 + 4 + sizeof_chunk_value + 21 + 4 + 4));
    ...
    if (bLookForUserTableRecordHeader) {
        // make sure there is a TCODE_USER_TABLE_RECORD_HEADER chunk and skip
        // over it.
        buffer = BufferValidateTcode(bReverseByteOrder, buffer, buffer_end,
        ...
        buffer = BufferToChunkValue(bReverseByteOrder, sizeof_chunk_value,
                                    buffer, buffer_end, &header_length);
        if (0 == buffer)
            return 0;
        if (header_length < 25)
            return 0;
...
bool ON_BinaryArchive::FindMisplacedTable(ON__UINT64 filelength,
    ...
    const size_t sizeof_chunk_header =
        sizeof_chunk_typecode + sizeof_chunk_value;
    ...
    if (bFindUserTable && ON_UuidIsNil(class_uuid)) {
        ...
        ON_ERROR("ON_BinaryArchive::FindMisplacedTable - must provide plug-in "
                 "id when searching for user tables");
        return false;
    }
    ...
    while (!bAtEOF) {
        ...
        if (sizeof_read < 2 * sizeof_chunk_header ||
            sizeof_read > sizeof_buffer2048)
            break;
        ...
        if (table_tcode != tcode) {
            ...
            for (size_t i = 1; i <= sizeof_read - sizeof_chunk_typecode; i++) {
                tcode = !table_tcode;
                buffer = BufferToUINT32(bReverseByteOrder, &buffer2048[i],
                                        buffer_end, &tcode);
                if (0 == buffer || table_tcode == tcode) {
                    if (bAtEOF && sizeof_read > 0 && 0 != buffer &&
                        table_tcode == tcode) {
                        // this table starts within sizeof_buffer2048 bytes of
                        // the eof.
                        bAtEOF = false;
                    }
                    break;
            ...
            continue; // start again with archive positioned at the tcode we
                      // want
        }
        ...
        i64 = -1;
        buffer = BufferToChunkValue(bReverseByteOrder, sizeof_chunk_value,
                                    buffer, buffer_end, &i64);
        if (0 == buffer || i64 <= 0)
            continue;
        const ON__UINT64 length_of_table = (ON__UINT64)i64;

        if (length_of_table < 2 * sizeof_chunk_header + 4 + min_length_data) {
            if (sizeof_chunk_header == length_of_table &&
                2 != empty_table_status) {
                // see if we are at a TCODE_ENDOFTABLE chunk
                buffer = BufferValidateTcode(bReverseByteOrder, buffer,
                                             buffer_end, TCODE_ENDOFTABLE);
                if (0 != buffer) {
                    i64 = -1;
                    buffer = BufferToChunkValue(bReverseByteOrder,
                                                sizeof_chunk_value, buffer,
                                                buffer_end, &i64);
                    if (0 == i64) {
                        if (0 == empty_table_status) {
                            ...
                            // found 2 or more candidates for the end of table
                            // chunk
        ...
        if (bFindUserTable) {
            ...
            buffer = EmergencyFindTable_UuidHelper(
                ...
                TCODE_USER_TABLE_UUID, &class_uuid);
        ...
        if (bFindUserTable) {
            ON__UINT64 expected_length_of_table =
                length_of_user_uuid_and_header + sizeof_chunk_header +
                length_of_record;
            if (expected_length_of_table != length_of_table)
                continue;
        } else {
            if (length_of_record <
                4 * sizeof_chunk_header + 20 + min_length_data)
                continue;
            ...
            if (bFindObjectTable) {
                buffer =
                    BufferValidateTcode(bReverseByteOrder, buffer, buffer_end,
                                        TCODE_OBJECT_RECORD_TYPE);
                if (0 == buffer)
                    continue;
                // The TCODE_OBJECT_RECORD_TYPE is a shot chunk whose value is a
                // bitfield used to filter reading of objects.  Checking the
                // value will not help validate the record, but we need to skip
                // over it.
                buffer =
                    BufferToChunkValue(bReverseByteOrder, sizeof_chunk_value,
                                       buffer, buffer_end, 0);
                if (0 == buffer)
                    continue;
            }
            ...
            if (length_of_on_class <
                3 * sizeof_chunk_header + 20 + min_length_data)
                continue;
            ...
            if (length_of_data < min_length_data)
                continue;
            if (length_of_data + length_of_uuid_chunk +
                    2 * sizeof_chunk_header >
                length_of_on_class)
                continue;
        }
...
bool ON_BinaryArchive::FindTableInDamagedArchive(
    const unsigned int tcode_table, const unsigned int tcode_record,
    const ON_UUID class_uuid, const int min_length_data) {
    bool rc = FindMisplacedTable(0, tcode_table, tcode_record, class_uuid,
                                 min_length_data);
    return rc;
}
...
bool ON_BinaryArchive::EndRead3dmTable(unsigned int typecode) {
    ...
    if (m_3dm_version == 1) {
        if (m_chunk.Count() != 0) {
            ON_ERROR("ON_BinaryArchive::EndRead3dmTable() v1 file "
                     "m_chunk.Count() != 0");
            return false;
        }
        rc = true;
        ...
        if (m_active_table == group_table &&
            m_3dm_opennurbs_version < 200012210) {
            // 3DM archives written before version 200012210 and before do not
            // have group tables
            rc = true;
        } else if (m_active_table == font_table &&
                   m_3dm_opennurbs_version < 200109180) {
            // 3DM archives written before version 200109180 and before do not
            // have font tables
            rc = true;
        } else if (m_active_table == dimstyle_table &&
                   m_3dm_opennurbs_version < 200109260) {
            // 3DM archives written before version 200109260 and before do not
            // have dimstyle tables
            rc = true;
        } else if (m_active_table == instance_definition_table &&
                   m_3dm_opennurbs_version < 200205110) {
            // 3DM archives written before version 200205110 and before do not
            // have instance definition tables
            rc = true;
        } else if (m_active_table == hatchpattern_table &&
                   m_3dm_opennurbs_version < 200405030) {
            // 3DM archives written before version 200405030 and before do not
            // have hatch pattern tables
            rc = true;
        } else if (m_active_table == linetype_table &&
                   m_3dm_opennurbs_version < 200503170) {
            // 3DM archives written before version 200503170 and before do not
            // have linetype tables
            rc = true;
        } else if (m_active_table == texture_mapping_table &&
                   m_3dm_opennurbs_version < 200511110) {
            // 3DM archives written before version 200511110 and before do not
            // have texture mapping tables
            rc = true;
        } else if (m_active_table == historyrecord_table &&
                   m_3dm_opennurbs_version < 200601180) {
            // 3DM archives written before version 200601180 and before do not
            // have history record tables
            rc = true;
        } else {
            if (m_chunk.Count() != 1) {
                ON_ERROR("ON_BinaryArchive::EndRead3dmTable() v2 file "
                         "m_chunk.Count() != 1");
                return false;
            }
            const ON_3DM_BIG_CHUNK *c = m_chunk.Last();
            if (0 == c || c->m_typecode != typecode) {
                ON_ERROR("ON_BinaryArchive::EndRead3dmTable() "
                         "m_chunk.Last()->typecode != typecode");
                return false;
            }
            rc = EndRead3dmChunk();
    ...
    if (m_3dm_version != 1) {
        ...
        if (BeginRead3dmBigChunk(&tcode, &big_value)) {
            if (tcode == TCODE_BITMAP_RECORD) {
                ...
                if (!bitmap) {
                    ON_ERROR("ON_BinaryArchive::Read3dmBitmap() - corrupt "
                             "bitmap table");
                }
                if (ppBitmap)
                    *ppBitmap = bitmap;
                ...
                ON_ERROR(
                    "ON_BinaryArchive::Read3dmBitmap() - corrupt bitmap table");
            }
            EndRead3dmChunk();
        }
    }
...
bool ON_BinaryArchive::BeginWrite3dmLayerTable() {
    ...
    if (m_3dm_version != 1) {
        ...
        if (m_chunk.Count()) {
            ON_ERROR("ON_BinaryArchive::BeginWrite3dmLayerTable() - chunk "
                     "stack should be empty");
            return false;
        }
        if (m_active_table != no_active_table) {
            ON_ERROR("ON_BinaryArchive::BeginWrite3dmLayerTable() - "
                     "m_active_table != no_active_table");
        }
...
bool ON_BinaryArchive::Write3dmLayer(const ON_Layer &layer) {
    ...
    if (m_active_table != layer_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmLayer() - m_active_table != "
                 "layer_table");
    }

    if (m_3dm_version == 1) {
        // legacy version 1 layer information is in a top level TCODE_LAYER
        // chunk
        if (m_chunk.Count()) {
            ON_ERROR("ON_BinaryArchive::Write3dmLayer() - version 1 - chunk "
                     "stack should be empty");
            return false;
        }
...
bool ON_BinaryArchive::EndWrite3dmLayerTable() {
    ...
    if (m_3dm_version == 1) {
        if (m_active_table != layer_table) {
            ON_ERROR("ON_BinaryArchive::EndWrite3dmLayerTable() - "
                     "m_active_table != layer_table");
        }
        rc = true;
        m_active_table = no_active_table;
...
bool ON_BinaryArchive::BeginRead3dmLayerTable() {
    ...
    if (!rc) {
        ...
        rc = true; // there are 1.0 files written by the old IO toolkit that
                   // have no layers
    }
    return rc;
}
...
int ON_BinaryArchive::Read3dmV1LayerIndex(const char *sV1LayerName) const {
    ...
    if (ON::read3dm == m_mode && 0 == m_3dm_opennurbs_version &&
        1 == m_3dm_version && 0 != m_V1_layer_list && 0 != sV1LayerName &&
        0 != sV1LayerName[0]) {
...
bool ON_BinaryArchive::Read3dmV1Layer(ON_Layer *&layer) {
        ...
        for (;;) {
            ...
            case TCODE_LAYERNAME: {
                ...
                if (slen < 0 || slen > 10000) {
                    ON_ERROR("ON_BinaryArchive::Read3dmV1Layer() - invalid "
                             "layer name length");
                } else {
                    s.SetLength(slen);
    ...
    if (!rc && layer) {
        ...
        if (ON::read3dm == m_mode && 0 == m_3dm_opennurbs_version &&
            ...
            if (layer->LayerIndex() >= 0 && s_length > 0 && s_length < 256 &&
                0 != s_name && 0 != s_name[0]) {
                struct ON__3dmV1LayerIndex *p =
                    (struct ON__3dmV1LayerIndex *)oncalloc(
                        1,
                        sizeof(*p) + (s_length + 1) * sizeof(*p->m_layer_name));
                p->m_layer_name = (char *)(p + 1);
                p->m_layer_index = layer->LayerIndex();
                p->m_layer_name_length = s_length;
                memcpy(p->m_layer_name, s_name,
                       s_length * sizeof(*p->m_layer_name));
    ...
    if (m_3dm_version == 1) {
        ...
        if (BeginRead3dmBigChunk(&tcode, &big_value)) {
            if (tcode == TCODE_LAYER_RECORD) {
                ...
                if (!layer) {
                    ON_ERROR("ON_BinaryArchive::Read3dmLayer() - corrupt layer "
                             "table");
                }
            } else if (tcode != TCODE_ENDOFTABLE) {
                ON_ERROR(
                    "ON_BinaryArchive::Read3dmLayer() - corrupt layer table");
            }
            EndRead3dmChunk();
        }
    }
...
bool ON_BinaryArchive::EndRead3dmLayerTable() {
    ...
    if (m_3dm_version == 1) {
        if (m_active_table != layer_table) {
            ON_ERROR("ON_BinaryArchive::EndRead3dmLayerTable() - "
                     "m_active_table != no_active_table");
            rc = false;
...
bool ON_BinaryArchive::Write3dmGroup(const ON_Group &group) {
    bool rc = false;
    if (m_active_table != group_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmGroup() - m_active_table != "
                 "group_table");
    }
...
int ON_BinaryArchive::Read3dmGroup(ON_Group **ppGroup) {
    ...
    *ppGroup = 0;
    ...
    if (m_3dm_opennurbs_version < 200012210) {
        // 3DM archives written before version 200012210 and before do not have
        // group tables
        return 0;
    }
    ...
    if (BeginRead3dmBigChunk(&tcode, &big_value)) {
        if (tcode == TCODE_GROUP_RECORD) {
            ...
            if (!group) {
                ON_ERROR(
                    "ON_BinaryArchive::Read3dmGroup() - corrupt group table");
            }
        } else if (tcode != TCODE_ENDOFTABLE) {
...
bool ON_BinaryArchive::Write3dmFont(const ON_Font &font) {
    ...
    if (m_active_table != font_table) {
        ON_ERROR(
            "ON_BinaryArchive::Write3dmFont() - m_active_table != font_table");
    }
...
int ON_BinaryArchive::Read3dmFont(ON_Font **ppFont) {
    ...
    *ppFont = 0;
    ...
    if (m_3dm_opennurbs_version < 200109180) {
        // 3DM archives written before version 200109180 and before do not have
        // font tables
        return 0;
    }
...
bool ON_BinaryArchive::Write3dmDimStyle(const ON_DimStyle &dimstyle) {
    ...
    if (m_active_table != dimstyle_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmDimStyle() - m_active_table != "
                 "dimstyle_table");
    }
...
int ON_BinaryArchive::Read3dmDimStyle(ON_DimStyle **ppDimStyle) {
    ...
    *ppDimStyle = 0;
    ...
    if (m_active_table != dimstyle_table) {
        ON_ERROR("ON_BinaryArchive::BeginRead3dmDimStyleTable() - "
                 "m_active_table != no_active_table");
    }
    ...
    if (BeginRead3dmBigChunk(&tcode, &big_value)) {
        if (tcode == TCODE_DIMSTYLE_RECORD) {
            ...
            if (!dimstyle) {
                ON_ERROR("ON_BinaryArchive::Read3dmDimStyle() - corrupt "
                         "dimstyle table");
            }
        } else if (tcode != TCODE_ENDOFTABLE) {
            ON_ERROR(
                "ON_BinaryArchive::Read3dmDimStyle() - corrupt dimstyle table");
        }
        EndRead3dmChunk();
    }
    *ppDimStyle = dimstyle;
...
bool ON_BinaryArchive::BeginRead3dmHatchPatternTable() {
    ...
    if (!rc && m_3dm_opennurbs_version >= 200511010) {
        ...
        rc = FindMisplacedTable(
            0, TCODE_HATCHPATTERN_TABLE, TCODE_HATCHPATTERN_RECORD,
...
int ON_BinaryArchive::Read3dmHatchPattern(ON_HatchPattern **ppPattern) {
    ...
    *ppPattern = 0;
    ...
    if (m_active_table != hatchpattern_table) {
        ON_ERROR("ON_BinaryArchive::BeginRead3dmHatchPatternTable() - "
                 "m_active_table != hatchpattern_table");
    }
    if (m_3dm_opennurbs_version < 200405030) {
        // 3DM archives written before version 200405030 do not have
        // hatchpattern tables
        return 0;
    }
        ...
        if (tcode == TCODE_HATCHPATTERN_RECORD) {
            if (m_3dm_opennurbs_version < 200511010) {
                ...
                if (!pPat->Read(*this)) {
                    delete pPat;
                    pPat = NULL;
                    ON_ERROR("ON_BinaryArchive::Read3dmHatchPattern() - "
                             "corrupt hatch pattern table");
                }
            } else {
                ON_Object *p = 0;
                if (ReadObject(&p)) {
            ...
            ON_ERROR("ON_BinaryArchive::Read3dmHatchPattern() - corrupt hatch "
                     "pattern table");
        }
...
bool ON_BinaryArchive::Write3dmLinetype(const ON_Linetype &linetype) {
    ...
    if (m_active_table != linetype_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmLinetype() - m_active_table != "
                 "linetype_table");
    }
...
bool ON_BinaryArchive::BeginRead3dmLinetypeTable() {
    ...
    if (m_3dm_version < 4 || m_3dm_opennurbs_version < 200503170) {
        ...
        if (!rc) {
            ...
            //    functions when it makes sense.
            rc = FindMisplacedTable(
                0, TCODE_LINETYPE_TABLE, TCODE_LINETYPE_RECORD,
...
int ON_BinaryArchive::Read3dmLinetype(ON_Linetype **ppLinetype) {
    ...
    *ppLinetype = 0;
    ...
    if (m_active_table != linetype_table) {
        ON_ERROR("ON_BinaryArchive::BeginRead3dmLinetypeTable() - "
                 "m_active_table != linetype_table");
    }
    ...
    if (BeginRead3dmBigChunk(&tcode, &big_value)) {
        if (tcode == TCODE_LINETYPE_RECORD) {
            ...
            if (!linetype) {
                ON_ERROR("ON_BinaryArchive::Read3dmLinetype() - corrupt "
                         "linetype table");
            }
        } else if (tcode == TCODE_ENDOFTABLE) {
...
bool ON_BinaryArchive::Write3dmInstanceDefinition(
    const ON_InstanceDefinition &idef) {
    bool rc = false;
    if (m_active_table != instance_definition_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmInstanceDefinition() - "
                 "m_active_table != instance_definition_table");
    }
    ...
    if (c && c->m_typecode == TCODE_INSTANCE_DEFINITION_TABLE) {
        ...
        ON_ERROR("ON_BinaryArchive::Write3dmInstanceDefinition() must be "
                 "called in BeginWrite3dmInstanceDefinitionTable() block");
        rc = false;
    }
...
bool ON_BinaryArchive::BeginRead3dmInstanceDefinitionTable() {
    ...
    if (!rc) {
        ...
        //    functions when it makes sense.
        rc = FindMisplacedTable(
            0, TCODE_INSTANCE_DEFINITION_TABLE,
...
int ON_BinaryArchive::Read3dmInstanceDefinition(
    ...
    *ppInstanceDefinition = 0;
    ...
    if (m_active_table != instance_definition_table) {
        ON_ERROR("ON_BinaryArchive::BeginRead3dmInstanceDefinitionTable() - "
                 "m_active_table != no_active_table");
    }
    ...
    if (BeginRead3dmBigChunk(&tcode, &big_value)) {
        if (tcode == TCODE_INSTANCE_DEFINITION_RECORD) {
            ...
            if (!idef) {
                ON_ERROR("ON_BinaryArchive::Read3dmInstanceDefinition() - "
                         "corrupt instance definition table");
            }
        } else if (tcode != TCODE_ENDOFTABLE) {
            ON_ERROR("ON_BinaryArchive::Read3dmInstanceDefinition() - corrupt "
...
bool ON_BinaryArchive::Write3dmTextureMapping(
    ...
    if (!c || c->m_typecode != TCODE_TEXTURE_MAPPING_TABLE) {
        ON_ERROR("ON_BinaryArchive::Write3dmTextureMapping() - active chunk "
                 "typecode != TCODE_TEXTURE_MAPPING_TABLE");
    } else {
...
bool ON_BinaryArchive::BeginRead3dmTextureMappingTable() {
    ...
    if (!rc) {
        ...
        //    test file.
        rc = FindMisplacedTable(
            0, TCODE_TEXTURE_MAPPING_TABLE, TCODE_TEXTURE_MAPPING_RECORD,
...
int ON_BinaryArchive::Read3dmTextureMapping(
    ON_TextureMapping **ppTextureMapping) {
    int rc = 0;
    if (!ppTextureMapping)
        return 0;
    *ppTextureMapping = 0;
...
bool ON_BinaryArchive::Write3dmHistoryRecord(
    const ON_HistoryRecord &history_record) {
    bool rc = false;
    ...
    if (!c || c->m_typecode != TCODE_HISTORYRECORD_TABLE) {
        ON_ERROR("ON_BinaryArchive::Write3dmHistoryRecord() - active chunk "
                 "typecode != TCODE_HISTORYRECORD_TABLE");
    } else {
...
bool ON_BinaryArchive::BeginRead3dmHistoryRecordTable() {
    ...
    if (!rc) {
        ...
        //    test file.
        rc = FindMisplacedTable(
            0, TCODE_HISTORYRECORD_TABLE, TCODE_HISTORYRECORD_RECORD,
...
int ON_BinaryArchive::Read3dmHistoryRecord(ON_HistoryRecord *&history_record) {
    ...
    if (BeginRead3dmBigChunk(&tcode, &big_value)) {
        if (tcode == TCODE_HISTORYRECORD_RECORD) {
            ...
            if (!history_record) {
                ON_ERROR("ON_BinaryArchive::Read3dmHistoryRecord() - corrupt "
...
bool ON_BinaryArchive::Write3dmMaterial(const ON_Material &material) {
    ...
    if (m_active_table != material_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmMaterial() - m_active_table != "
                 "material_table");
    }
...
bool ON_BinaryArchive::BeginRead3dmMaterialTable() {
    ...
    if (!rc) {
        ...
        //    test file.
        rc =
            FindMisplacedTable(0, TCODE_MATERIAL_TABLE, TCODE_MATERIAL_RECORD,
...
bool ON_BinaryArchive::Read3dmV1AttributesOrMaterial(
    ON_3dmObjectAttributes *attributes, ON_Material *material,
    ON_BOOL32 &bHaveMat, unsigned int end_mark_tcode, ON__3dmV1_XDATA *xdata) {
    ...
    for (;;) {
        ...
        case TCODE_RGBDISPLAY:
            if (attributes) {
                ...
                ON__UINT32 rgb32 = (ON__UINT32)rgb64;
                u = rgb32;
                attributes->m_color.SetRGB(u % 256, (u >> 8) % 256,
                                           (u >> 16) % 256);
            }
            break;
        ...
        case TCODE_XDATA:
            ...
            if (attributes) {
                ...
                if (!on_stricmp("RhHidePrevLayer", xid)) {
                    ...
                    char *buffer =
                        (char *)alloca((sizeof_data + 1) * sizeof(buffer[0]));
                    buffer[0] = 0;
                    ...
                    if (ReadByte(sizeof_data, buffer)) {
                        ...
                        //{
                        //  xdata->m_type =
                        //  ON__3dmV1_XDATA::hidden_object_layer_name;
                    ...
                    if (ReadByte(sizeof_data, buffer)) {
                        ...
                        //{
                        //  xdata->m_type =
                        //  ON__3dmV1_XDATA::locked_object_layer_name;
                        //  xdata->m_string = buffer;
                        //}
                    }
                } else if (!on_stricmp("RhAnnotateArrow", xid) &&
                           24 == sizeof_data) {
                    ...
                    m_error_message_mask |=
                        0x0002; // disable v1 EndRead3dmChunk() partially read
                                // chunk warning
                }
        ...
        default:
            ...
            m_error_message_mask |= 0x02; // disable v1 EndRead3dmChunk()
    ...
    while (0 == rc) {
        ...
        case TCODE_RH_POINT:
            // v1 3d point
            {
                ...
                if (0 != point_chunk &&
                    ...
                    // (It appears that points with arrow xdata have this
                    // problem.) For these chunks we need to set the chunk
                    // length so EndRead3dmChunk() will keep going.
        ...
        case TCODE_MESH_OBJECT:
            // v1 mesh
            {
                ...
                if (!EndRead3dmChunk())
                    break;
                // attributes and material informtion follow the
                // TCODE_COMPRESSED_MESH_GEOMETRY chunk
        ...
        m_error_message_mask =
            saved_error_message_mask; // enable v1 EndRead3dmChunk() partially
                                      // read chunk warning
        if (!bEndReadChunk_rc) {
            rc = -1;
            break;
    ...
    if (m_3dm_version == 1) {
        ...
        if (BeginRead3dmBigChunk(&tcode, &big_value)) {
            if (tcode == TCODE_MATERIAL_RECORD) {
                ...
                if (!material) {
                    ON_ERROR("ON_BinaryArchive::Read3dmMaterial() - corrupt "
                             "material table");
                }
            } else if (tcode == TCODE_ENDOFTABLE) {
...
bool ON_BinaryArchive::Write3dmLight(const ON_Light &light,
    ...
    if (m_active_table != light_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmLight() - m_active_table != "
                 "light_table");
    }
    const ON_3DM_BIG_CHUNK *c = m_chunk.Last();
    if (c && c->m_typecode == TCODE_LIGHT_TABLE) {
        ...
        if (rc) {
            // WriteObject writes TCODE_OPENNURBS_CLASS chunk that contains
            // light definition
            ...
            if (rc && attributes) {
                ...
                if (rc) {
                    ...
                    if (!EndWrite3dmChunk())
                        rc = false;
                    if (rc && m_bSaveUserData && Archive3dmVersion() >= 4 &&
                        0 != attributes->FirstUserData()) {
                        // 14 May 2008 Dale Lear
                        //    Added support for saving light attribute userdata
                        rc = BeginWrite3dmChunk(
                            TCODE_LIGHT_RECORD_ATTRIBUTES_USERDATA, 0);
                        if (rc) {
                            // write user data
                            rc = WriteObjectUserData(*attributes);
                            if (rc) {
                                // Because I'm not using Write3dmObject() to
                                // write the attributes, the user data must be
                                // immediately followed by a short
                                // TCODE_OPENNURBS_CLASS_END chunk in order for
                                // ReadObjectUserData() to work correctly.
                                //
                                // The reason that this is hacked in is that V3
                                // files did not support attribute user data and
                                // doing it this way means that V3 can still
                                // read V4 files.
                                rc = BeginWrite3dmChunk(
                                    TCODE_OPENNURBS_CLASS_END, 0);
                                if (rc) {
                                    if (!EndWrite3dmChunk())
                                        rc = false;
                                }
...
int ON_BinaryArchive::Read3dmV1Light( // returns 0 at end of light table
    ...
    ON_3dmObjectAttributes
        *pAttributes // optional - if NOT NULL, object attributes are
                     //            returned here
) {
    ON_BOOL32 bHaveMat;
    ON_Material material;
...
int ON_BinaryArchive::Read3dmLight(ON_Light **ppLight,
    ...
    *ppLight = 0;
    if (m_active_table != light_table) {
        ON_ERROR(
            "ON_BinaryArchive::Read3dmLight() - m_active_table != light_table");
        ...
        if (BeginRead3dmBigChunk(&tcode, &big_value)) {
            if (tcode == TCODE_LIGHT_RECORD) {
                ...
                if (!light) {
                    ON_ERROR("ON_BinaryArchive::Read3dmLight() - corrupt light "
                             "table");
                } else {
                    *ppLight = light;
                    rc = 1;
                ...
                ON_ERROR(
                    "ON_BinaryArchive::Read3dmLight() - corrupt light table");
            } else
                rc = 0;
            ...
            while (rc == 1) {
                ...
                if (tcode == TCODE_LIGHT_RECORD_ATTRIBUTES && attributes) {
                    ...
                    if (0 != attributes) {
                        // 14 May 2008
                        //   Added support for reading user data on light
                        //   attributes
...
bool ON_BinaryArchive::Write3dmObject(
    const ON_Object &object, const ON_3dmObjectAttributes *attributes) {
    bool rc = false;
    if (m_active_table != object_table) {
        ON_ERROR("ON_BinaryArchive::Write3dmObject() - m_active_table != "
                 "object_table");
    }

    if (Archive3dmVersion() <= 2 &&
        object.ObjectType() == ON::pointset_object) {
    ...
    if (c && c->m_typecode == TCODE_OBJECT_TABLE) {
        ...
        if (rc) {
            ...
            rc = BeginWrite3dmChunk(TCODE_OBJECT_RECORD_TYPE,
                                    object.ObjectType());
            if (rc) {
                if (!EndWrite3dmChunk())
                    rc = false;
            ...
            if (rc && attributes) {
                ...
                if (rc) {
                    ...
                    if (rc && m_bSaveUserData && Archive3dmVersion() >= 4 &&
                        0 != attributes->FirstUserData()) {
                        // 19 October 2004
                        //   Added support for saving user data on object
                        //   attributes
                        rc = BeginWrite3dmChunk(
                            TCODE_OBJECT_RECORD_ATTRIBUTES_USERDATA, 0);
                        if (rc) {
                            // write user data
                            rc = WriteObjectUserData(*attributes);
                            if (rc) {
            ...
            ON_ERROR("ON_BinaryArchive::Write3dmObject() - active chunk "
                     "typecode != TCODE_OBJECT_TABLE");
        }
    }
    return rc;
}
...
bool ON_BinaryArchive::BeginRead3dmObjectTable() {
    ...
    if (!rc) {
        ...
        rc = FindMisplacedTable(
            0, TCODE_OBJECT_TABLE, TCODE_OBJECT_RECORD, ON_nil_uuid,
...
bool ON_BinaryArchive::ReadV1_TCODE_RH_POINT(
    ...
    rc = ReadPoint(pt);
    if (rc) {
        rc = Read3dmV1AttributesOrMaterial(pAttributes, NULL, bHaveMat,
                                           TCODE_ENDOFTABLE, &xdata);
    ...
    if (rc && pos0 > 0 && 0 != point_chunk && point_chunk == m_chunk.Last()) {
        if (TCODE_RH_POINT == point_chunk->m_typecode &&
            0 == point_chunk->m_big_value) {
...
bool ON_BinaryArchive::ReadV1_TCODE_ANNOTATION(
    unsigned int tcode, ON_Object **ppObject,
    ...
    *ppObject = NULL;
    ...
    case TCODE_TEXT_BLOCK: {
        ...
        if (rc && version == RHINO_TEXT_BLOCK_VERSION_1 ||
            ...
            // 11 November 2003 Dale Lear - see comments in
            ...
            // depending on the value of byobject, more fields might be read
            // here
            ...
            // 2 extra doubles were written into the file by mistake in version
            // 1
    ...
    case TCODE_ANGULAR_DIMENSION: {
        ...
        if (i == RHINO_ANGULAR_DIMENSION_VERSION_1) {
            ...
            // distances from apes to start and end of dimensioned lines - not
            // used
            if (!ReadDouble(4, d4))
                return false;
...
bool ON_BinaryArchive::ReadV1_TCODE_MESH_OBJECT(
    ...
    if (tcode == TCODE_COMPRESSED_MESH_GEOMETRY)
        for (;;) {
            ...
            mesh = new ON_Mesh(face_count, point_count,
    ...
    if (rc && mesh) {
        // attributes and material information follows the
        // TCODE_COMPRESSED_MESH_GEOMETRY chunk;
        ON_BOOL32 bHaveMat = false;
        Read3dmV1AttributesOrMaterial(pAttributes, NULL, bHaveMat,
                                      TCODE_ENDOFTABLE);
...
static ON_NurbsCurve *ReadV1_TCODE_LEGACY_SPLSTUFF(ON_BinaryArchive &file) {
    ...
    pNurbsCurve =
        new ON_NurbsCurve(dim, is_rat ? true : false, order, cv_count);
    ...
    for (;;) {
        ...
        if (order == 2 && cv_count == 2 &&
            ...
            // a few isolated old v1 3DM file created by Rhino 1.0 files have
            // lines with reversed knots.
...
static ON_NurbsSurface *ReadV1_TCODE_LEGACY_SRFSTUFF(ON_BinaryArchive &file) {
    ...
    int i, j, dim = 0, is_rat = 0, order[2], cv_count[2], is_closed[2],
              is_singular[2], form;
    ON_BoundingBox bbox;
    char c;
    ...
    pNurbsSurface = new ON_NurbsSurface(dim, is_rat ? true : false, order[0],
                                        order[1], cv_count[0], cv_count[1]);

    ON_BOOL32 rc = false;
    for (;;) {

        // read legacy v1 knot vectors
        if (!file.ReadDouble(order[0] + cv_count[0] - 2,
                             pNurbsSurface->m_knot[0]))
            break;
        if (!file.ReadDouble(order[1] + cv_count[1] - 2,
                             pNurbsSurface->m_knot[1]))
            break;
...
ON_Curve *ReadV1_TCODE_LEGACY_CRVSTUFF(ON_BinaryArchive &file) {
    ...
    for (;;) {
        ...
        for (i = 0; i < segment_count; i++) {
            ...
            if (segment->CVCount() > 2 || segment->Order() != 2 ||
                segment->IsRational()) {
        ...
        if (polycurve->Count() == 1) {
            ...
            for (i = 0; i < polycurve->Count(); i++) {
                pline->m_pline.Append(
                    polycurve->SegmentCurve(i)->PointAtStart());
            }
            pline->m_pline.Append(
...
bool ON_Brep::ReadV1_LegacyTrimStuff(
    ...
    ON_BOOL32 bHasMate =
        (c & 6); // bit 1 or 2 = 1 if "tedge" has non NULL "->twin"
    ON_BOOL32 bIsSeam =
        (c & 2); // bit 1 = 1 if "tedge->twin" belongs to same face
    ...
    if (bHasEdge) {
        ...
        ON_BrepTrim &trim =
            NewTrim(edge, revedge ? true : false, loop, curve2d_index);
        trim_index = trim.m_trim_index;
    } else {
        ON_BrepTrim &trim =
            NewTrim(revedge ? true : false, loop, curve2d_index);
        trim_index = trim.m_trim_index;
    }
    if (trim_index >= 0) {
...
bool ON_Brep::ReadV1_LegacyTrim(ON_BinaryArchive &file, ON_BrepFace &face,
                                ON_BrepLoop &loop) {
    bool rc = false;
...
bool ON_Brep::ReadV1_LegacyLoop(ON_BinaryArchive &file, ON_BrepFace &face) {
    bool rc = false;
...
bool ON_Brep::ReadV1_LegacyFaceStuff(ON_BinaryArchive &file) {
    ...
    if (!file.ReadInt(&twincnt))
        return false;
    short *glue =
        (twincnt > 0) ? (short *)ws.GetMemory(twincnt * sizeof(*glue)) : NULL;
    if (twincnt > 0) {
...
ReadV1_RHINOIO_NURBS_SURFACE_OBJECT_DATA(ON_BinaryArchive &file) {
    ...
    if (tcode == TCODE_RHINOIO_OBJECT_DATA)
        for (;;) {
            ...
            surface = new ON_NurbsSurface(dim, is_rat, order[0], order[1],
...
int ON_BinaryArchive::Read3dmV1Object(
    ...
    for (;;) {
        ...
        case TCODE_RHINOIO_OBJECT_NURBS_CURVE:
            ...
            if (0 != (ON::mesh_object & object_filter)) {
                if (ReadV1_TCODE_RHINOIO_OBJECT_NURBS_CURVE(ppObject,
                                                            pAttributes))
                    rc = 1;
                else
                    rc = -1;
        ...
        case TCODE_RHINOIO_OBJECT_NURBS_SURFACE:
            ...
            if (0 != (ON::mesh_object & object_filter)) {
                if (ReadV1_TCODE_RHINOIO_OBJECT_NURBS_SURFACE(ppObject,
                                                              pAttributes))
                    rc = 1;
                else
                    rc = -1;
...
ON_OBSOLETE_CCustomMeshUserData::ON_OBSOLETE_CCustomMeshUserData() {
    ...
    m_userdata_uuid = ON_OBSOLETE_CCustomMeshUserData::
...
int ON_BinaryArchive::Read3dmObject(
    ...
    //          1: success
    //          2: skipped filtered objects
    //          3: skipped new object (object's class UUID wasn't found in class
    //          list)
    ON_BOOL32 rc = -1;
    if (pAttributes)
        pAttributes->Default();
    ...
    if (m_3dm_version == 1) {
        ...
        if (BeginRead3dmBigChunk(&tcode, &length_TCODE_OBJECT_RECORD)) {
            if (tcode == TCODE_OBJECT_RECORD) {
                if (BeginRead3dmBigChunk(&tcode,
                                         &value_TCODE_OBJECT_RECORD_TYPE)) {
                    ...
                    case 1:
                        ...
                        break;
                    case 3:
                        rc = 3; // skipped object - assume it's just a newer
                                // object than this code reads
                        break;
                    default:
                ...
                ON_ERROR(
                    "ON_BinaryArchive::Read3dmObject() - corrupt object table");
                rc = -1;
            } else
                rc = 0;
            ...
            while (rc == 1) {
                tcode = 0;
                if (!BeginRead3dmBigChunk(
                        &tcode, &length_TCODE_OBJECT_RECORD_ATTRIBUTES)) {
                    rc = -1;
                    break;
                }
                    ...
                    if (0 != pAttributes) {
                        ...
                        if (!ReadObjectUserData(*pAttributes))
                            ...
                            ON_OBSOLETE_CCustomMeshUserData *ud =
                                ON_OBSOLETE_CCustomMeshUserData::Cast(
                                    pAttributes->GetUserData(
                                        ON_OBSOLETE_CCustomMeshUserData::
                                            m_ON_OBSOLETE_CCustomMeshUserData_class_id
                                                .Uuid()));
                            if (ud) {
                                ud->m_mp.m_bCustomSettingsEnabled =
                                    ud->m_bInUse ? true : false;
                                pAttributes->SetCustomRenderMeshParameters(
                                    ud->m_mp);
                                delete ud;
                            }
#endif
...
bool ON_BinaryArchive::BeginWrite3dmUserTable(const ON_UUID &plugin_id,
    ...
    if (!ON_UuidCompare(&ON_nil_uuid, &plugin_id)) {
        ON_ERROR("ON_BinaryArchive::BeginWrite3dmUserTable() - nil "
                 "usertable_uuid not permitted.");
        return false;
    }
    if (bSavingGoo) {
...
bool ON_BinaryArchive::Write3dmAnonymousUserTableRecord(
    const ON_UUID &plugin_id, int goo_3dm_version, int goo_opennurbs_version,
    const ON_3dmGoo &goo) {
...
bool ON_BinaryArchive::Write3dmAnonymousUserTable(const ON_3dmGoo &goo) {
    ...
    if (!c || c->m_typecode != TCODE_USER_RECORD) {
        ON_ERROR("ON_BinaryArchive::Write3dmAnonymousUserTable() - active "
                 "chunk not a TCODE_USER_RECORD.");
        rc = false;
    } else if (goo.m_typecode != TCODE_USER_RECORD) {
        ON_ERROR("ON_BinaryArchive::Write3dmAnonymousUserTable() - goo chunk "
                 "not a TCODE_USER_RECORD.");
        rc = false;
    } else {
...
bool ON_BinaryArchive::EndWrite3dmUserTable() {
    ...
    if (c && c->m_typecode == TCODE_USER_RECORD) {
        ...
        ON_ERROR("ON_BinaryArchive::EndWrite3dmUserTable() - not in a "
                 "TCODE_USER_RECORD chunk.");
        rc = false;
    }
    ...
    if (rc) {
        ...
        if (rc) {
            if (tcode != TCODE_USER_TABLE_UUID) {
                ON_ERROR("ON_BinaryArchive::BeginRead3dmUserTable() - missing "
                         "user table UUID");
                rc = false;
            } else {
                rc = ReadUuid(plugin_id);

                // Version 200910190 of OpenNURBS began writing a
                // TCODE_USER_TABLE_RECORD_HEADER section immediately after the
                // uuid.  This was possible because the uuid was wrapped in a
                // TCODE_USER_TABLE_UUID chunk.  The
                // TCODE_USER_TABLE_RECORD_HEADER contains information that
                // let's us determine what version of Rhino and opennurbs wrote
                // the user table.  We need to know this because "goo" can have
                // chunks with 4 byte lengths embedded in a file with 8 byte
                // chunk lengths.  If this information is missing, then we know
                // the "goo" must have 4 byte chunk lengths and we assume it is
                // from a V4 file.
                //
                // 37 + SizeofChunkLength() =
                //      16 bytes of uuid
                //    + 4 bytes of TCODE_USER_TABLE_RECORD_HEADER typecode
                //    + SizeofChunkLength() TCODE_USER_TABLE_RECORD_HEADER chunk
                //    length
        ...
        if (rc) {
            if (tcode != TCODE_USER_RECORD) {
                ON_ERROR("ON_BinaryArchive::BeginRead3dmUserTable() - missing "
                         "user table TCODE_USER_RECORD chunk.");
                EndRead3dmChunk();
                rc = false;
            }
        }
        ...
        if (rc && !bReadArchiveInfo) {
            // If the file we are reading is V4 or an early V5 file, then use
            // the version numbers from the file.  Otherwise, assume the goo is
            // from an early V5 file.  All we know for sure is that the chunk
            // lengths in the user table are 4 bytes.
...
bool ON_BinaryArchive::EndRead3dmUserTable() {
    if (m_chunk.Count() != 2) {
        ON_ERROR(
            "ON_BinaryArchive::EndRead3dmUserTable() m_chunk.Count() != 2");
        return false;
    }
    const ON_3DM_BIG_CHUNK *c = m_chunk.Last();
    ...
    if (rc) {
        ...
        if (rc) {
            if (tcode != TCODE_ENDOFTABLE) {
                ON_ERROR("ON_BinaryArchive::EndRead3dmTable() missing "
                         "TCODE_ENDOFTABLE marker.");
            }
            if (!EndRead3dmChunk())
                rc = false;
        }
...
bool ON_BinaryArchive::Write3dmEndMark() {
    Flush();
    if (m_chunk.Count() != 0) {
        ON_ERROR("ON_BinaryArchive::WriteEndMark() called with unfinished "
                 "chunks.\n");
        return false;
    }
    ON__UINT64 length = CurrentPosition(); // since no chunks are unfinished,
                                           // everything has been committed to
                                           // disk in either write mode.
...
bool ON_BinaryArchive::Read3dmEndMark(size_t *file_length) {
    ...
    const unsigned int saved_error_message_mask = m_error_message_mask;
    m_error_message_mask |=
        0x0001; // disable v1 ReadByte() error message at EOF
...
ON::endian ON_BinaryArchive::Endian() const // endian-ness of cpu
{
    return m_endian;
}

ON::archive_mode ON_BinaryArchive::Mode() const { return m_mode; }
...
void ON_BinaryArchive::UpdateCRC(size_t count, const void *p) {
    if (m_bDoChunkCRC) {
        ...
        if (c) {
            if (c->m_do_crc16)
                c->m_crc16 = ON_CRC16(c->m_crc16, count,
                                      p); // version 1 files had 16 bit CRC
            if (c->m_do_crc32)
                c->m_crc32 = ON_CRC32(c->m_crc32, count,
                                      p); // version 2 files have 32 bit CRC
        }
    }
}

int ON_BinaryArchive::BadCRCCount() const { return m_bad_CRC_count; }
...
bool ON_BinaryArchive::MaskReadError(ON__UINT64 sizeof_request,
    ...
    if (0 != (0x04 & m_error_message_mask) && 0 <= sizeof_request)
        return true;
    if (0 != (0x01 & m_error_message_mask) && 4 == sizeof_request &&
        0 == sizeof_read)
        return true;
...
ON__UINT64 ON_BinaryArchive::ReadBuffer(ON__UINT64 sizeof_buffer,
    ...
    m_error_message_mask |=
        0x04; // tell Read to tolerate hitting the end of the file
...
ON_BinaryFile::ON_BinaryFile(ON::archive_mode mode)
    : ON_BinaryArchive(mode), m_fp(0), m_memory_buffer_capacity(0),
      m_memory_buffer_size(0), m_memory_buffer_ptr(0), m_memory_buffer(0) {}

ON_BinaryFile::ON_BinaryFile(ON::archive_mode mode, FILE *fp)
    : ON_BinaryArchive(mode), m_fp(fp), m_memory_buffer_capacity(0),
      m_memory_buffer_size(0), m_memory_buffer_ptr(0), m_memory_buffer(0) {}
    ...
    if (count > 0) {
        if (!ReadMode()) {
...
#if defined(ON_DEBUG)
            {
                // this is slow (becuase of the call to CurrentPosition(), but
                // it really helps find bugs in IO code
                const ON_3DM_BIG_CHUNK *c = m_chunk.Last();
                if (c) {
                    ON__UINT64 file_offset = CurrentPosition();
                    if (file_offset < c->m_big_offset) {
                        ON_ERROR("ON_BinaryArchive::ReadByte will read before "
                                 "the start of the chunk.");
                    } else if (c->m_big_offset + c->Length() <
                               file_offset + count) {
                        ON_ERROR("ON_BinaryArchive::ReadByte will read past "
                                 "end of the chunk");
                    }
                }
            }
            ...
            if (readcount == count) {
                ...
                for (;;) {
                    if (0 != (m_error_message_mask & 0x01) && 0 == readcount &&
                        4 == count) {
                        // when reading v1 files, there are some situations
                        // where it is reasonable to attempt to read 4 bytes at
                        // the end of a file.
                        break;
                    }
...
void ON_BinaryFile::EnableMemoryBuffer(int buffer_capacity) {
    ...
    if (m_fp) {
        if (m_memory_buffer && m_memory_buffer_size > 0) {
            ...
            if (rc && m_memory_buffer_ptr != m_memory_buffer_size) {
                // if ( !fseek( m_fp, m_memory_buffer_size-m_memory_buffer_ptr,
                // SEEK_CUR ) )
                int delta =
                    (m_memory_buffer_ptr >= m_memory_buffer_size)
...
bool ON_BinaryFile::SeekFromCurrentPosition(int offset) {
    ...
    bool rc = false;
    if (m_fp) {
        if (m_memory_buffer && m_memory_buffer_ptr + offset >= 0 &&
...
ON_3dmGoo &ON_3dmGoo::operator=(const ON_3dmGoo &src) {
    if (this != &src) {
        ...
        m_value = src.m_value;
        m_goo = (m_value > 0 && src.m_goo)
                    ? (unsigned char *)onmemdup(src.m_goo, m_value)
                    : 0;
    }
    return *this;
}
...
bool ON_WriteOneObjectArchive(ON_BinaryArchive &archive, int version,
    ...
    const ON_Object *pObject = &object;
    {
        if (ON_BrepEdge::Cast(pObject)) {
            ...
            pObject =
                static_cast<const ON_SurfaceProxy *>(pObject)->ProxySurface();
        }
    }
    ...
    while (pObject) {
        rc = archive.Write3dmStartSection(
            version, "Archive created by ON_WriteOneObjectArchive "__DATE__
                     " "__TIME__);
        if (!rc)
            break;
...
static void Dump3dmChunk_ErrorReportHelper(size_t offset, const char *msg,
                                           ON_TextLog &dump) {
    int ioffset = (int)offset;
    dump.Print("** ERROR near offset %d ** %s\n", ioffset, msg);
}
static bool DumpChunk_PrintHeaderInfo(size_t offset0, ON__UINT32 typecode,
                                      ON__INT64 big_value,
...
static bool Dump3dmChunk_EndReadChunkHelper(ON_BinaryArchive &file,
                                            size_t offset0, ON__UINT32 tcode,
                                            ON__INT64 big_value,
                                            ON_TextLog &dump) {
    ...
    if (!rc) {
        Dump3dmChunk_ErrorReportHelper(offset1, "EndRead3dmChunk() failed.",
                                       dump);
    } else if (!bShortChunk) {
        ...
        const ON__INT64 extra =
            big_value - (delta + sizeof_crc - sizeof_chunk_header);
        if (extra < 0) {
            Dump3dmChunk_ErrorReportHelper(offset0, "Read beyond end of chunk.",
                                           dump);
            rc = false;
        }
    }
...
static bool Dump3dmChunk_UserDataHeaderHelper(size_t offset,
    ...
    ON_UUID userdata_appid =
        ON_nil_uuid; // id of plug-in that owns the user data
    ...
    for (;;) {
        if (2 == major_userdata_version) {
            ...
            if (!rc) {
                Dump3dmChunk_ErrorReportHelper(
                    offset,
                    "Unable to find the TCODE_OPENNURBS_CLASS_USERDATA_HEADER "
                    "chunk header in a TCODE_OPENNURBS_CLASS_USERDATA chunk.",
                    dump);
                break;
            }
            ...
            if (!rc) {
                Dump3dmChunk_ErrorReportHelper(
                    offset,
                    "Unable to read the TCODE_OPENNURBS_CLASS_USERDATA_HEADER "
                    "chunk header in a TCODE_OPENNURBS_CLASS_USERDATA chunk.",
                    dump);
                break;
            }
            if (TCODE_OPENNURBS_CLASS_USERDATA_HEADER != tcode) {
                Dump3dmChunk_ErrorReportHelper(
                    offset,
                    "Missing TCODE_OPENNURBS_CLASS_USERDATA_HEADER chunk "
                    "header in a TCODE_OPENNURBS_CLASS_USERDATA chunk.",
                    dump);
                Dump3dmChunk_EndReadChunkHelper(file, offset0, tcode, big_value,
                                                dump);
                rc = false;
                break;
            }
        ...
        if (!rc) {
            Dump3dmChunk_ErrorReportHelper(
                offset, "ReadUuid() failed to read the user data class id.",
                dump);
            break;
        }
        ...
        const ON_ClassId *pUserDataClassId =
            ON_ClassId::ClassId(userdata_classid);
        if (pUserDataClassId) {
        ...
        if (!rc) {
            Dump3dmChunk_ErrorReportHelper(
                offset, "ReadUuid() failed to read the user data item id.",
                dump);
            break;
        }
        ...
        if (!rc) {
            Dump3dmChunk_ErrorReportHelper(
                offset, "ReadInt() failed to read the user data copy count.",
                dump);
            break;
        }
        ...
        if (!rc) {
            Dump3dmChunk_ErrorReportHelper(
                offset, "ReadXform() failed to read the user data xform.",
                dump);
            break;
        }
        ...
        if (!rc) {
            Dump3dmChunk_ErrorReportHelper(
                offset,
                "ReadUuid() failed to read the user data app plug-in id.",
                dump);
            break;
        }
        ...
        if (!rc) {
            Dump3dmChunk_ErrorReportHelper(offset,
                                           "ReadBool() failed to read the user "
                                           "data header bSavedAsGoo value.",
                                           dump);
            break;
        }
        ...
        if (bLastSavedAsGoo)
            dump.Print("Userdata originally written by opennurbs %d in 3dm "
                       "version %d and saved as goo in this file.\n",
    ...
    if (bCallEndRead3dmChunk) {
        if (!Dump3dmChunk_EndReadChunkHelper(file, offset0, tcode, big_value,
                                             dump)) {
            if (rc) {
                Dump3dmChunk_ErrorReportHelper(
                    offset,
...
unsigned int ON_BinaryArchive::Dump3dmChunk(ON_TextLog &dump,
                                            int recursion_depth) {
    ...
    if (!rc) {
        Dump3dmChunk_ErrorReportHelper(offset0, "BeginRead3dmChunk() failed.",
                                       dump);
    } else {
        if (0 == typecode) {
            Dump3dmChunk_ErrorReportHelper(
            ...
            typecode_name = ON_BinaryArchive::TypecodeName(typecode);
            bShortChunk = (0 != (typecode & TCODE_SHORT));
            if (!DumpChunk_PrintHeaderInfo(offset0, typecode, big_value,
                                           typecode_name, dump)) {
                EndRead3dmChunk();
                return 0;
            }
            ...
            case TCODE_OBJECT_TABLE:
                // start of a table
                {
                    ...
                    for (;;) {
                        record_typecode =
                            Dump3dmChunk(dump, recursion_depth + 1);
                        if (!record_typecode) {
                            break;
                        }
            ...
            case TCODE_PROPERTIES_OPENNURBS_VERSION: {
                ...
                dump.Print("Version of opennurbs that wrote this file: %lld\n",
                           big_value);
                dump.PopIndent();
                if (0 == m_3dm_opennurbs_version && big_value > 0 &&
                    big_value <= 299999999)
                    ON_SetBinaryArchiveOpenNURBSVersion(*this,
                                                        (ON__INT32)big_value);
            } break;

            case TCODE_BITMAP_RECORD: {
                dump.PushIndent();
                unsigned int bitmap_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (0 == typecode)
            ...
            case TCODE_MATERIAL_RECORD: {
                dump.PushIndent();
                unsigned int material_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (0 == typecode)
            ...
            case TCODE_FONT_RECORD: {
                dump.PushIndent();
                unsigned int font_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (0 == typecode)
            ...
            case TCODE_DIMSTYLE_RECORD: {
                dump.PushIndent();
                unsigned int dimstyle_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (0 == typecode)
            ...
            case TCODE_TEXTURE_MAPPING_RECORD: {
                dump.PushIndent();
                unsigned int mapping_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (!typecode)
            ...
            case TCODE_HISTORYRECORD_RECORD: {
                dump.PushIndent();
                unsigned int history_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (!typecode)
            ...
            case TCODE_HATCHPATTERN_RECORD: {
                dump.PushIndent();
                unsigned int hatch_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (!typecode)
            ...
            case TCODE_INSTANCE_DEFINITION_RECORD: {
                dump.PushIndent();
                unsigned int idef_chunk_typecode =
                    Dump3dmChunk(dump, recursion_depth + 1);
                if (0 == typecode)
            ...
            case TCODE_OBJECT_RECORD_ATTRIBUTES: {
                ...
                if (big_value < 14) {
                    Dump3dmChunk_ErrorReportHelper(
                        offset0,
                        "Length of chunk is too small.  Should be >= 14.",
                        dump);
                } else {
                    ...
                    if (!Read3dmChunkVersion(&mj, &mn)) {
                        ...
                        Dump3dmChunk_ErrorReportHelper(
                            offset0, "ReadUuid() failed.", dump);
                    } else if (!ReadInt(&layer_index)) {
                        Dump3dmChunk_ErrorReportHelper(
                            offset0, "ReadInt() failed to read layer index.",
                            dump);
                    } else {
            ...
            case TCODE_OPENNURBS_CLASS: {
                ...
                for (;;) {
                    opennurbs_object_chunk_typecode =
                        Dump3dmChunk(dump, recursion_depth + 1);
                    if (!opennurbs_object_chunk_typecode) {
                        break;
                    }
                    if (TCODE_OPENNURBS_CLASS_END ==
                        opennurbs_object_chunk_typecode) {
                        break;
                    }
                    ...
                    default: {
                        Dump3dmChunk_ErrorReportHelper(
                            offset0, "Rogue chunk in OpenNURBS class record.",
                            dump);
                    }
                    }
                }
            ...
            case TCODE_OPENNURBS_CLASS_USERDATA: {
                if (!Read3dmChunkVersion(&major_userdata_version,
                                         &minor_userdata_version)) {
                    Dump3dmChunk_ErrorReportHelper(
                        offset0,
                        "Read3dmChunkVersion() failed to read "
                        "TCODE_OPENNURBS_CLASS_USERDATA chunk version.",
                        dump);
                } else {
                    dump.PushIndent();
                    dump.Print("UserData chunk version: %d.%d\n",
                               major_userdata_version, minor_userdata_version);
                    if (1 == major_userdata_version ||
                        2 == major_userdata_version) {
                        const size_t userdata_header_offset = CurrentPosition();
                        switch (major_userdata_version) {
                        case 1:
                        case 2: {
                            // version 1 user data header information was not
                            // wrapped in a chunk.
                            //
                            ...
                            if (Dump3dmChunk_UserDataHeaderHelper(
                                ...
                                // a TCODE_ANONYMOUS_CHUNK contains user data
                                // goo
                                int anon_typecode =
                                    Dump3dmChunk(dump, recursion_depth + 1);
                        ...
                        default:
                            if (major_userdata_version < 3) {
                                ...
                                dump.Print(
                                    "New user data format created after this "
                                    "diagnostic tool was written.\n");
                            }
                            break;
                        }
                    }
            ...
            case TCODE_OPENNURBS_CLASS_USERDATA_HEADER: {
                ...
                Dump3dmChunk_ErrorReportHelper(
                    offset0,
                    "Encountered TCODE_OPENNURBS_CLASS_USERDATA_HEADER chunk "
                    "outside a TCODE_OPENNURBS_CLASS_USERDATA chunk.",
                    dump);
            } break;
        ...
        if (!EndRead3dmChunk(true)) {
            ...
            ON__INT64 delta = (offset1 > offset0)
                                  ...
                                  : -((ON__INT64)(offset0 - offset1));
            const ON__INT64 extra =
                big_value - (delta - 4 - SizeofChunkLength());
            if (extra < 0) {
                Dump3dmChunk_ErrorReportHelper(
                    offset0, "Read beyond end of chunk.", dump);
            }
        }
    }
    return typecode;
...
ON_Read3dmBufferArchive::ON_Read3dmBufferArchive(size_t sizeof_buffer,
    ...
    : ON_BinaryArchive(ON::read3dm), m_p(0), m_buffer(0), m_sizeof_buffer(0),
      m_buffer_position(0), m_reserved1(0), m_reserved2(0), m_reserved3(0),
      m_reserved4(0) {
...
size_t ON_Read3dmBufferArchive::SizeOfBuffer() const { return m_sizeof_buffer; }
...
ON_Write3dmBufferArchive::ON_Write3dmBufferArchive(
    size_t initial_sizeof_buffer, size_t max_sizeof_buffer,
    int archive_3dm_version, int archive_opennurbs_version)
    : ON_BinaryArchive(ON::write3dm), m_p(0), m_buffer(0), m_sizeof_buffer(0),
      m_max_sizeof_buffer(max_sizeof_buffer), m_sizeof_archive(0),
      m_buffer_position(0), m_reserved1(0), m_reserved2(0), m_reserved3(0),
      m_reserved4(0) {
...
ON_Buffer *ON_BinaryArchiveBuffer::Buffer() const { return m_buffer; }
...
#if defined(ON_COMPILER_MSC)
      ,
      m_file_attributes_mask(0), m_h(0)
#else
      ,
      m_dir(0), m_current_file_attributes(0), m_current_file_size(0),
      m_current_create_time(0), m_current_last_modified_time(0),
...
ON__UINT64 ON_FileIterator::Count() const { return m_count; }
...
const wchar_t *ON_FileIterator::FirstFile(const wchar_t *directory_name,
    ...
    m_count++;
    m_fd.cFileName[(sizeof(m_fd.cFileName) / sizeof(m_fd.cFileName[0])) - 1] =
        0;
...
const wchar_t *ON_FileIterator::NextFile() {
    ...
    for (;;) {
        ...
        ON_ConvertUTF8ToWideChar(
            ...
            0, // error_code_point = null terminator inserted at point of
               // conversion error
...
#if defined(ON_COMPILER_MSC)
    if (0 != CurrentFileName() &&
        0 != (FILE_ATTRIBUTE_DIRECTORY & m_fd.dwFileAttributes)) {
        rc = true;
    }
#else
...
static ON__UINT64 SecondsSinceJanOne1970(FILETIME ft) {
    ...
    ON__UINT64 seconds_since_jan_1_1970 =
        seconds_since_jan_1_1601 - 11644473600;
...
void ON_SetDebugWriteObjectDirectory(const wchar_t *directory_name) {
    size_t i;
    const size_t maxi = sizeof(ON_sDebugWriteObjectDirectory) /
                            sizeof(ON_sDebugWriteObjectDirectory[0]) -
                        1;
...
const wchar_t *ON_DebugWriteObjectDirectory() {
    return (0 != ON_sDebugWriteObjectDirectory[0]
                ? ON_sDebugWriteObjectDirectory
                : 0);
}
...
int ON_DebugWriteObject(const ON_Object *pObject) {
    ...
    wchar_t filename[sizeof(ON_sDebugWriteObjectDirectory) /
                         sizeof(ON_sDebugWriteObjectDirectory[0]) +
                     64];
    size_t i;
    size_t maxi = sizeof(ON_sDebugWriteObjectDirectory) /
                      sizeof(ON_sDebugWriteObjectDirectory[0]) -
                  1;
    int N = -1;
    errno_t e;
