...
ON::object_type ON_Curve::ObjectType() const { return ON::curve_object; }
...
bool ON_Curve::SetDomain(ON_Interval domain) {
    return (domain.IsIncreasing() && SetDomain(domain[0], domain[1])) ? true
                                                                      : false;
}

ON_BOOL32 ON_Curve::SetDomain(double, double) {
    // this virtual function is overridden by curves that can change their
    // domain.
    return false;
}
...
ON_BOOL32 ON_Curve::GetParameterTolerance( // returns tminus < tplus: parameters
                                           // tminus <= s <= tplus
        ...
        if (span_count > 0) {
            ...
            if (GetSpanVector(s.Array())) {
                ...
                if (line.Length() > tolerance) {
                    ...
                    for (span_index = 0; span_index < span_count;
                         span_index++) {
...
bool ON_Curve::IsEllipse(const ON_Plane *plane, ON_Ellipse *ellipse,
...
bool ON_Curve::IsArcAt(double t, const ON_Plane *plane, ON_Arc *arc,
                       double tolerance, double *t0, double *t1) const {
    double k, k0, k1;
    int hint;
    if (!GetDomain(&k0, &k1))
        return false;
    ...
    // check sub-segments
    hint = 0;
    for (k = k0; k0 <= t && GetNextDiscontinuity(ON::G2_locus_continuous, k0,
                                                 k1, &k, &hint);
         k0 = k) {
        if (!(k > k0))
        ...
        if (t <= k) {
            ...
            //   (happens when t is the parameter between a line and arc
            //   segment.) The "k0 <= t" test is in the for() condition will
...
ON_BOOL32 ON_Curve::IsArc(const ON_Plane *plane, ON_Arc *arc,
                          double tolerance) const {
    ON_BOOL32 rc = false;
    double c0, c, t, delta;
    int n, i, span_index;
...
bool ON_Curve::GetNextDiscontinuity(ON::continuity c, double t0, double t1,
    ...
    if (t0 != t1) {
        ...
        if (bTestC0) {
            ...
            if ((t0 < domain[1] && t1 >= domain[1]) ||
                (t0 > domain[0] && t1 <= domain[0])) {
                if (IsClosed()) {
                    if (bTestD1 || bTestT) {
                        // need to check locus continuity at start/end of closed
                        // curve.
                        ON_3dPoint Pa, Pb;
...
ON_3dPoint ON_Curve::PointAtStart() const { return PointAt(Domain().Min()); }

ON_3dPoint ON_Curve::PointAtEnd() const { return PointAt(Domain().Max()); }

ON_BOOL32 ON_Curve::SetStartPoint(ON_3dPoint start_point) { return false; }

ON_BOOL32 ON_Curve::SetEndPoint(ON_3dPoint end_point) { return false; }
...
ON_BOOL32 ON_Curve::EvTangent(double t, ON_3dPoint &point, ON_3dVector &tangent,
    ...
    if (rc && !tangent.Unitize()) {
        if (Ev2Der(t, point, D1, D2, side, hint)) {
            ...
            if (rc) {
                ...
                double tplus = 0.0;
                if (domain.IsIncreasing() &&
                    GetParameterTolerance(t, &tminus, &tplus)) {
                    ON_3dPoint p;
                    ON_3dVector d1, d2;
                    double eps = 0.0;
...
ON_BOOL32 ON_Curve::EvCurvature(double t, ON_3dPoint &point,
                                ON_3dVector &tangent, ON_3dVector &kappa,
                                int side, int *hint) const {
    ON_3dVector d1, d2;
...
const ON_Interval *
ON_Curve_IsShortSubdomainHelper(const ON_Curve *curve,
...
*/
bool ON_MatchCurveEnds(ON_Curve *curve0, int end0, ON_Curve *curve1, int end1,
                       double gap_tolerance = 0.0);

bool ON_MatchCurveEnds(ON_Curve *curve0, int end0, ON_Curve *curve1, int end1,
                       double gap_tolerance) {
    ON_BOOL32 rc = false;
    if (0 != curve0 && 0 != curve1 && end0 >= 0 && end0 <= 1 && end1 >= 0 &&
        end1 <= 1) {
        ...
        if (!rc) {
            ...
            for (i = 0; i <= 1; i++) {
                ...
                while (ON::ctPolycurve == ct) {
                    ...
                    if (0 == polycurve)
                        break;
                    c = polycurve->SegmentCurve(e ? (polycurve->Count() - 1)
                                                  : 0);
                    if (0 == c)
                        return false;
                    ct = ON_CurveType(c);
            ...
            if (fix[0] >= 1000 || fix[1] < fix[0]) {
                ...
                if (P0.x == P1.x)
                    P.x = P0.x;
                else if (fixPoint[0].x != ON_UNSET_VALUE &&
                         fixPoint[1].x == ON_UNSET_VALUE)
                    P.x = fixPoint[0].x;
                else if (fixPoint[0].x == ON_UNSET_VALUE &&
                         fixPoint[1].x != ON_UNSET_VALUE)
                    P.x = fixPoint[1].x;
                if (P0.y == P1.y)
                    P.y = P0.y;
                else if (fixPoint[0].y != ON_UNSET_VALUE &&
                ...
                if (P0.z == P1.z)
                    P.z = P0.z;
                else if (fixPoint[0].z != ON_UNSET_VALUE &&
                         fixPoint[1].z == ON_UNSET_VALUE)
                    P.z = fixPoint[0].z;
                else if (fixPoint[0].z == ON_UNSET_VALUE &&
                         fixPoint[1].z != ON_UNSET_VALUE)
                    P.z = fixPoint[1].z;
                ON_BOOL32 rc0 =
                    end0 ? curve0->SetEndPoint(P) : curve0->SetStartPoint(P);
                ON_BOOL32 rc1 =
                    end1 ? curve1->SetEndPoint(P) : curve1->SetStartPoint(P);
                rc = (rc0 && rc1);
            }
            if (rc) {
...
static bool ForceMatchArcs(ON_ArcCurve &Arc0, int end0, ON_ArcCurve &Arc1,
                           int end1)
...

{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (ct[i] == ON::ctArc)
            ...
            if (ct[i] == ON::ctLine || ct[i] == ON::ctNurbs ||
                ct[i] == ON::ctPolyline)
                bIsArc[i] = false;
            else
                return false;
    ...
    if (bIsArc[0]) {
        ...
        if (bIsArc[1]) {
            ...
            ON_3dPoint P0 =
                (end0) ? seg[0]->PointAtEnd() : seg[0]->PointAtStart();
            ON_3dPoint P1 =
                (end1) ? seg[1]->PointAtEnd() : seg[1]->PointAtStart();
            P = 0.5 * (P0 + P1);
        }
    }
    ...
    if (bMove[0]) {
        ON_BOOL32 brc =
            (end0) ? seg[0]->SetEndPoint(P) : seg[0]->SetStartPoint(P);
        if (!brc)
            rc = false;
    }
    if (bMove[1]) {
        ON_BOOL32 brc =
            (end1) ? seg[1]->SetEndPoint(P) : seg[1]->SetStartPoint(P);
        if (!brc)
            rc = false;
    }
...
bool ON_NurbsCurve::RepairBadKnots(double knot_tolerance, bool bRepair) {
    bool rc = false;
    if (m_order >= 2 && m_cv_count > m_order && 0 != m_cv && 0 != m_knot &&
                m_dim > 0 && m_cv_stride >= (m_is_rat)
            ? (m_dim + 1)
            : m_dim && 0 != m_cv && 0 != m_knot &&
                  m_knot[m_cv_count - 1] - m_knot[m_order - 2] >
                      knot_tolerance) {
        ...
        for (i = m_cv_count - 2; i > m_order - 2; i--) {
            if (m_knot[m_cv_count - 1] - m_knot[i] > knot_tolerance) {
                if (i < m_cv_count - 2) {
                    rc = true;
                    if (bRepair) {
                        // remove extra knots but do not change end point
                        // location
                        DestroyRuntimeCache();
                        ...
                        for (i = m_cv_count - 1; i < m_cv_count + m_order - 2;
                             i++)
                            m_knot[i] = domain[1];
                        onfree(cv);
                        cv = 0;
        ...
        // make sure first span has m_knot[m_order-1] - m_knot[m_order-2] >
        // knot_tolerance
            ...
            if (m_knot[i] - m_knot[m_order - 2] > knot_tolerance) {
                if (i > m_order - 1) {
                    ...
                    if (bRepair) {
                        ...
                        for (j0 = 0, j1 = i; j1 < m_cv_count + m_order - 2;
                             j0++, j1++)
                            m_knot[j0] = m_knot[j1];
                        m_cv_count -= i;
                        ClampEnd(2);
                ...
                if (m_knot[i + m_order - 1] - m_knot[i] <= knot_tolerance) {
                    ...
                    if (bRepair) {
                        ...
                        for (j0 = i, j1 = i + 1; j1 < m_cv_count; j0++, j1++)
                            memcpy(CV(j0), CV(j1), sizeof_cv);
                        for (j0 = i, j1 = i + 1; j1 < m_cv_count + m_order - 2;
                             j0++, j1++)
                            m_knot[j0] = m_knot[j1];
                        m_cv_count--;
                    } else {
...
bool ON_Curve::FirstSpanIsLinear(double min_length, double tolerance) const {
    ...
    if (0 != poly_curve) {
        ...
        return (0 != segment)
                   ? segment->FirstSpanIsLinear(min_length, tolerance)
                   : false;
    }
...
bool ON_Curve::LastSpanIsLinear(double min_length, double tolerance) const {
    const ON_NurbsCurve *nurbs_curve = ON_NurbsCurve::Cast(this);
    if (0 != nurbs_curve) {
        return nurbs_curve->SpanIsLinear(nurbs_curve->m_cv_count -
                                             nurbs_curve->m_order,
    ...
    const ON_PolyCurve *poly_curve = ON_PolyCurve::Cast(this);
    if (0 != poly_curve) {
        const ON_Curve *segment =
            poly_curve->SegmentCurve(poly_curve->Count() - 1);
        return (0 != segment) ? segment->LastSpanIsLinear(min_length, tolerance)
                              : false;
    }
...
bool ON_NurbsCurve::SpanIsLinear(int span_index, double min_length,
                                 double tolerance) const {
    if (m_dim < 2 || m_dim > 3)
        return false;
    ...
    if (!(m_knot[span_index + m_order - 2] <
...
ON_Curve *ON_TrimCurve(const ON_Curve &curve, ON_Interval trim_parameters) {
    ...
    if (trim_parameters.IsDecreasing() && curve.IsClosed()) {
        ...
        if (!right_crv->Trim(
                ON_Interval(curve_domain[0], trim_parameters[1]))) {
            delete left_crv;
            delete right_crv;
            return 0;
        }
...
// virtual
int ON_Curve::GetNurbForm(ON_NurbsCurve &nurbs_curve, double tolerance,
                          const ON_Interval *subdomain) const {
    return 0;
}

// virtual
int ON_Curve::HasNurbForm() const { return 0; }

ON_NurbsCurve *ON_Curve::NurbsCurve(ON_NurbsCurve *pNurbsCurve,
                                    double tolerance,
                                    const ON_Interval *subdomain) const {
...
ON_BOOL32
ON_Curve::GetCurveParameterFromNurbFormParameter(double nurbs_t,
...
ON_BOOL32
ON_Curve::GetNurbFormParameterFromCurveParameter(double curve_t,
        ...
        if (rc && major_version == 1) {
            ...
            if (rc) {
                ...
                for (i = 0; rc && i < count && rc; i++) {
                    ...
                    if (rc && flag == 1) {
                        p = 0;
                        rc = file.ReadObject(&p) ? true
                                                 : false; // polymorphic curves
...
// Makes continuous ON_PolyCurves and ON_Polycurves. Appends results to
// OutCurves. returns number of curves appended.
...

{
    ...
    // polycurve endarray[i][0] - 1, and the start of IC[i] is interior to the
    // polycurve. if endarray[i][1] > 0, then the end of IC[i] is interior.  if
    // both endarray[i][0] > 0 and endarray[i][1] > 0, then they are equal.
    ...
    for (i = 0; i < EData.Count(); i++) {
        ...
        if (endarray[ED.id[0]][ED.end[0]] > 0 ||
            endarray[ED.id[1]][ED.end[1]] > 0)
            continue; // one of these endpoints has already been join to a
                      // closer end
        if (endarray[ED.id[0]][1 - ED.end[0]] == 0) {
            if (endarray[ED.id[1]][1 - ED.end[1]] == 0) { // new curve
                ...
                // second curve is part of an existing sequence. Insert or
                // append first curve.
            ...
            // first curve is part of an existing sequence. Insert or append
            // second curve.
            ON_SimpleArray<CurveJoinSeg> &SArray =
            ...
            if (endarray[ED.id[0]][1 - ED.end[0]] ==
                endarray[ED.id[1]][1 - ED.end[1]])
                ...
                if (SArray0[0].id == ED.id[0]) {
                    if (SArray1[0].id == ED.id[1]) {
                        ...
                        endarray[ED.id[0]][ED.end[0]] =
                            endarray[ED.id[1]][ED.end[1]] = segid0;
                        SArray1.SetCount(0);
                    } else {
                        int j;
                        ...
                        endarray[ED.id[0]][ED.end[0]] =
                            endarray[ED.id[1]][ED.end[1]] = segid1;
                        SArray0.SetCount(0);
                    }
    ...
    for (i = 0; i < SegsArray.Count(); i++) {
        ...
        if (SArray.Count() < 2)
            continue;
        if (!bPreserveDirection) { // if number of reversed segs is more than
                                   // half, reverse.
            int count = 0;
            int j;
    ...
    /* This was added by greg to fix big curves that are nearly, but not quite,
 closed. It causes problems when the curve is tiny. for(i=0;
 i<OutCurves.Count(); i++){ if(!OutCurves[i]->IsClosed()){ ON_3dPoint s=
 OutCurves[i]->PointAtStart(); ON_3dPoint e = OutCurves[i]->PointAtEnd();
    ...
    if (count > 1 && ON_IsValid(t)) {
        ...
        if (bEnableSnap && !rc) {
            ...
            if (index >= 0 && index < count - 1) {
                ...
                // 22 October 2003 Dale Lear - added this case to match the
                // index==count case above. If we get here, then t < m_t[0]
                if (m_t[0] - t <= ktol) {
...
bool ON_SortLines(int line_count, const ON_Line *line_list, int *index,
    ...
    for (Ni = 1; Ni < line_count; Ni++) {
        ...
        */
        ...
        startd = StartP.DistanceTo(
            line_list[index[start_i]].from); // "from" is correct here
        endd = EndP.DistanceTo(
            line_list[index[end_i]].from); // "from" is correct here
...
bool ON_SortCurves(int curve_count, const ON_Curve *const *curve_list,
    ...
    if (curve_count < 1 || 0 == curve_list || 0 == curve_list[0] ||
        0 == index || 0 == bReverse) {
        if (index) {
    ...
    for (i = 0; i < curve_count; i++) {
        ...
        if (!curve->EvPoint(d[0], line.from, 1) ||
            !curve->EvPoint(d[1], line.to, -1)) {
            rc = false;
        }
    }
