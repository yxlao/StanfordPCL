    ...
    if (rc) {
        ...
        if (rc) {
            for (fi = 0; rc && fi < face_count; fi++) {
                const ON_Mesh *mesh = file.Save3dmRenderMeshes()
                                          ? brep->m_F[fi].m_render_mesh
                                          : 0;
                b = mesh ? 1 : 0;
                rc = file.WriteChar(b);
                if (rc && mesh) {
        ...
        if (rc) {
            for (fi = 0; rc && fi < face_count; fi++) {
                const ON_Mesh *mesh = file.Save3dmAnalysisMeshes()
                                          ? brep->m_F[fi].m_analysis_mesh
                                          : 0;
                b = mesh ? 1 : 0;
                rc = file.WriteChar(b);
                if (rc && mesh) {
...
static void ReadFillInMissingBoxes(ON_Brep &brep) {
...
ON_BOOL32 ON_Brep::Read(ON_BinaryArchive &file) {
    ...
    if (rc && major_version == 2) {
        ...
        if (rc && minor_version >= 1) {
            ...
            rc = file.BeginRead3dmBigChunk(&tcode,
...
bool ON_Brep::ReadOld200(ON_BinaryArchive &file, int minor_version) {
        ...
        for (fbi = 0; rc && fbi < fbcnt; fbi++) {
            ...
            for (bti = 0; rc && bti < btcnt; bti++) {
                ...
                if (trim.m_trim_index != i) {
                    ON_ERROR("ON_Brep::ReadOld200 - trim.m_trim_index out of "
                             "synch.");
                    // rc = false;
                    // break;
                }
                ...
                if (rc)
                    rc = file.ReadChar(
                        &b); // true if legacy trim managed 3d edge
                if (rc)
                ...
                if (b) {
                    if (trim.m_ei < 0 || trim.m_ei >= c3_count) {
                        trim.m_ei = -1;
                        ON_ERROR(
                            "ON_Brep::ReadOld201 - trim.m_ei out of range.");
                        rc = false;
                        break;
                    }
                }
    ...
    if (rc) {
        ...
        if (!rc) {
            ON_ERROR("ON_Brep::ReadOld201() - trouble reading render/analysis "
                     "meshes");
            rc = true;
        }
    }
