...
double ON_SpanTolerance(int order, int cv_count, const double *knot,
                        int span_index) {
...
int ON_KnotCount(int order, int cv_count) { return (order + cv_count - 2); }
...
int ON_KnotMultiplicity(int order,          // order (>=2)
...
int ON_KnotVectorSpanCount(int order,         // order (>=2)
...
bool ON_GetKnotVectorSpanVector(int order,          // order (>=2)
                                int cv_count,       // cv count
                                const double *knot, // knot[] array
                                double *s           // s[] array
) {
...
int ON_NextNurbsSpanIndex(int order, int cv_count, const double *knot,
                          int span_index)
...
int ON_GetSpanIndices(int order, int cv_count, const double *knot,
                      int *span_indices)
  ...
  */
{
    ...
    while (span_index != next_span_index) {
        ...
        next_span_index =
            ON_NextNurbsSpanIndex(order, cv_count, knot, span_index);
        if (next_span_index < 0)
...
double ON_SuperfluousKnot(int order, int cv_count, const double *knot,
                          int end) {
    double k;
...

{
    ...
    if ((end == 0 || end == 2) && knot[0] != knot[order - 2])
        rc = false;
    if ((end == 1 || end == 2) &&
        knot[cv_count - 1] != knot[order + cv_count - 3])
        rc = false;
    return rc;
}
...
bool ON_KnotVectorHasBezierSpans(int order,         // order (>=2)
                                 int cv_count,      // cv count
                                 const double *knot // knot[] array
) {
    ...
    if (order >= 2 && cv_count >= order &&
        ...
        knot[0] == knot[order - 2] &&
        knot[cv_count - 1] == knot[knot_count - 1])
        return true;
    return false;
}
...
bool ON_SetKnotVectorDomain(int order, int cv_count, double *knot, double t0,
                            double t1) {
    bool rc = false;
    if (order < 2 || cv_count < order || 0 == knot || t0 >= t1 ||
        !ON_IsValid(t0) || !ON_IsValid(t1)) {
...
bool ON_GetKnotVectorDomain(int order, int cv_count, const double *knot,
                            double *k0, double *k1) {
...
bool ON_IsValidKnotVector(int order, int cv_count, const double *knot,
    ...
    if (!(knot[order - 2] < knot[order - 1])) {
        if (text_log) {
            text_log->Print(
                "Knot vector order=%d and knot[%d]=%g >= knot[%d]=%g (should "
    ...
    if (!(knot[cv_count - 2] < knot[cv_count - 1])) {
        if (text_log) {
            text_log->Print(
                "Knot vector cv_count=%d and knot[%d]=%g >= knot[%d]=%g "
                "(should have knot[cv_count-2] < knot[cv_count-1]).\n",
    ...
    while (i--) {
        if (!(*k1 >= *k0)) {
            if (text_log) {
                text_log->Print("Knot vector must be increasing but "
                                "knot[%d]=%g > knot[%d]=%g\n",
                                order + cv_count - 4 - i, *k0,
                                order + cv_count - 3 - i, *k1);
            }
            return ON_KnotVectorIsNotValid(bSilentError);
        }
        k0++;
    ...
    while (i--) {
        if (!(*k1 > *k0)) {
            if (text_log) {
                text_log->Print(
                    "Knot vector order = %d but knot[%d]=%g >= knot[%d]=%g\n",
...
bool ON_ClampKnotVector(
    ...
    // sets initial/final order-2 knot values to match
    // knot[order-2]/knot[cv_count-1]
    bool rc = false;
    int i, i0;
...
bool ON_MakeKnotVectorPeriodic(
    ...
    case 2:
        if (cv_count < 4) {
            ON_ERROR("ON_MakePeriodicKnotVector(): illegal input degree=1, "
                     "cv_count<4");
            return false;
        } else {
            return true;
    ...
    case 3:
        if (cv_count < 4) {
            ON_ERROR("ON_MakePeriodicKnotVector(): illegal input degree=2, "
                     "cv_count<5");
            return false;
        }
        break;
    default:
        if (cv_count < 2 * order - 2) {
            ON_ERROR("ON_MakePeriodicKnotVector(): illegal input degree>=3, "
                     "cv_count<2*degree");
            return false;
        }
        break;
...
ON_DECL
bool ON_MakeClampedUniformKnotVector(int order, int cv_count, double *knot,
                                     double delta) {
    bool rc = false;
...
//   true if successful
ON_DECL
bool ON_MakePeriodicUniformKnotVector(int order, int cv_count, double *knot,
                                      double delta) {
    bool rc = false;
    if (order >= 2 && cv_count >= order && knot != NULL && delta > 0.0) {
        double k = 0.0;
...
bool ON_GetGrevilleKnotVector( // get knots from Greville abcissa
    ...
    if (bPeriodic) {
        ...
        for (j = g_count + degree, gi = 1; j < g_count + 2 * degree;
             j++, gi++) {
...
bool ON_ClampKnotVector(
    ...
    double *
        cv, // NULL or cv array with room for at least knot_multiplicity new cvs
    double
    ...
    if (knot && order >= 2 && cv_count >= order) {
        if (end == 0 || end == 2) {
            if (cv) {
                ON_EvaluateNurbsDeBoor(cv_dim, order, cv_stride, cv, knot, 1,
                                       0.0, knot[order - 2]);
            }
            i0 = order - 2;
            for (i = 0; i < i0; i++)
...
static bool ON_InsertSingleKnot(
    int cv_dim, int order, int cv_stride,
    double *cv,   // NULL or array of length at least order*cv_stride+cv_dim
    double *knot, // array of length at least 2*order-1 and existing knot values
                  // in knot[0], ..., knot[2*order-3]
...
int ON_InsertKnot(
    ...
    int span_index = ON_NurbsSpanIndex(order, cv_count, knot, knot_value, 1,
                                       hint ? *hint : 0);
    knot += span_index;
    if (cv)
    ...
    if (span_index == 0) {
        if (knot_value < knot[order - 1]) {
            if (knot_value <= knot[order - 2] + knot_tolerance) {
                ON_ERROR("ON_InsertKnot(): requested knot_value at start of "
                         "NURBS domain");
                return 0;
            }
        }
    ...
    if (span_index == cv_count - order) {
        if (knot_value > knot[order - 2] &&
            ...
            ON_ERROR(
                "ON_InsertKnot(): requested knot_value at end of NURBS domain");
            return 0;
        }
    }
    ...
    int j;
    if (knot_value == knot[order - 2]) {
        for (j = order - 2; m < knot_multiplicity && knot[j - m] == knot_value;
             m++)
            ; // empty for
    } else if (knot_value == knot[order - 1]) {
        for (j = order - 1; m < knot_multiplicity && knot[j + m] == knot_value;
             m++)
            ; // empty for
    }
    ...
    double *new_knot = (double *)onmalloc(
        ((2 * degree + m) + (order + m) * cv_dim) * sizeof(*new_knot));
    if (!new_knot) {
    ...
    if (cv) {
        ...
        for (j = 0; j < order; j++) {
            memcpy(new_cv + j * cv_dim, cv + j * cv_stride,
                   cv_dim * sizeof(*new_cv));
        }
    }
    ...
    rc = 0;
    while (m > 0) {
        if (!ON_InsertSingleKnot(cv_dim, order, cv_dim, new_cv, new_knot,
                                 knot_value))
            break;
        m--;
        if (new_cv)
    ...
    if (rc > 0) {
        ...
        int i0 =
            ON_KnotCount(order, cv_count) - 1; // knot[i0] = last input knot
        int i1 = i0 + rc;
        ...
        // update knot vector
        memcpy(knot + degree, new_knot + degree,
               (degree + rc) * sizeof(*new_knot));

        if (cv) {
            // make room for rc many new CVs
            i0 = (cv_count - 1) *
                 cv_stride; // cv[i0] = last coord of last input cv */
            i1 = i0 + rc * cv_stride;
            j = cv_count - order;
            while (j--) {
