...
ON_OBJECT_IMPLEMENT(ON_Material, ON_Object,
                    "60B5DBBC-E660-11d3-BFE4-0010830122F0");

double ON_Material::m_max_shine = 255.0f;

double ON_Material::MaxShine() { return m_max_shine; }
...
ON_BOOL32
ON_Material::IsValid(ON_TextLog *text_log) const { return true; }
...
ON_UUID ON_Material::MaterialPlugInUuid() const { return m_plugin_id; }

void ON_Material::SetMaterialPlugInUuid(ON_UUID u) { m_plugin_id = u; }
                ...
                for (;;) {
                    ...
                    if (rc) {
                        if (1 == texmajver) {
                            ...
                            for (i = 0; i < count && rc; i++) {
                                int trc =
                                    file.ReadObject(m_textures.AppendNew());
                                if (trc <= 0)
                                    rc = false;
                                else if (trc > 1)
                                    m_textures.Remove();
...
bool ON_Material::ReadV3Helper(ON_BinaryArchive &file, int minor_version) {
    ...
    bool rc = true;
    {
        ...
        if (rc && !str.IsEmpty()) {
            ON_Texture &texture =
                m_textures[AddTexture(str, ON_Texture::bitmap_texture)];
            if (2 == i) {
        ...
        if (rc && !str.IsEmpty()) {
            ON_Texture &texture =
                m_textures[AddTexture(str, ON_Texture::bump_texture)];
            if (2 == i) {
                texture.m_mode = ON_Texture::decal_texture;
            } else {
                texture.m_mode = ON_Texture::modulate_texture;
        ...
        if (rc && !str.IsEmpty()) {
            ON_Texture &texture =
                m_textures[AddTexture(str, ON_Texture::emap_texture)];
            if (2 == i) {
                texture.m_mode = ON_Texture::decal_texture;
            } else {
                texture.m_mode = ON_Texture::modulate_texture;
...
ON::object_type ON_Material::ObjectType() const { return ON::material_object; }

int ON_Material::FindTexture(const wchar_t *filename, ON_Texture::TYPE type,
                             int i0) const {
    int i, count = m_textures.Count();
...
int ON_Material::DeleteTexture(const wchar_t *filename, ON_Texture::TYPE type) {
    ...
    if (!filename && type == ON_Texture::no_texture_type) {
        ...
        for (i = m_textures.Count() - 1; i >= 0; i--) {
            if (type != ON_Texture::no_texture_type &&
                type != m_textures[i].m_type)
                continue;
            if (filename && m_textures[i].m_filename.CompareNoCase(filename))
                continue;
            m_textures.Remove(i);
...
double ON_Material::Shine() const { return m_shine; }
...
double ON_Material::Transparency() const { return m_transparency; }
...
int ON_Texture::Compare(const ON_Texture &other) const {
    ...
    while (!rc) {
        ...
        rc = memcmp(&m_blend_A[0], &other.m_blend_A[0],
                    3 * sizeof(m_blend_A[0]));
        if (rc)
            break;

        rc = memcmp(&m_blend_RGB[0], &other.m_blend_RGB[0],
                    3 * sizeof(m_blend_RGB[0]));
        if (rc)
            break;
...
ON_Color ON_Material::Ambient() const { return m_ambient & 0x00FFFFFF; }

ON_Color ON_Material::Diffuse() const { return m_diffuse & 0x00FFFFFF; }

ON_Color ON_Material::Emission() const { return m_emission & 0x00FFFFFF; }

ON_Color ON_Material::Specular() const { return m_specular & 0x00FFFFFF; }

void ON_Material::SetAmbient(ON_Color c) { m_ambient = c; }

void ON_Material::SetDiffuse(ON_Color c) { m_diffuse = c; }

void ON_Material::SetEmission(ON_Color c) { m_emission = c; }

void ON_Material::SetSpecular(ON_Color c) { m_specular = c; }

int ON_Material::MaterialIndex() const { return m_material_index; }

void ON_Material::SetMaterialIndex(int i) { m_material_index = i; }

const wchar_t *ON_Material::MaterialName() const { return m_material_name; }
...
ON_OBJECT_IMPLEMENT(ON_Texture, ON_Object,
...
ON_BOOL32 ON_TextureMapping::IsValid(ON_TextLog *text_log) const {
    ...
    if (m_texture_space !=
        ON_TextureMapping::TextureSpaceFromInt(m_texture_space)) {
        if (text_log) {
            text_log->Print("ON_TextureMapping m_texture_space = %d is not a "
                            "valid value.\n",
                            m_texture_space);
        }
        return false;
...
bool ON_TextureMapping::TileTextureCoordinate(int dir, double count,
                                              double offset) {
    bool rc = false;
...
static int IntersectBoxRayHelper(const ON_3dPoint &rst, const ON_3dVector &n,
                                 int dir, double *s) {
    /*
    returns:
...
int ON_TextureMapping::EvaluateSphereMapping(const ON_3dPoint &P,
    ...
    double longitude =
        (0.0 != rst.y || 0.0 != rst.x) ? atan2(rst.y, rst.x) : 0.0;
    double latitude =
        (0.0 != rst.z) ? atan2(rst.z, ((const ON_2dVector *)(&rst.x))->Length())
                       : 0.0;
    if (latitude > ON_PI)
        latitude -= 2.0 * ON_PI;
    ...
    if (ON_TextureMapping::ray_projection == mapping_proj) {
        ...
        if (m_bCapped) {
            ...
            if (fabs(1.0 + rst.z) < t && fabs(1.0 - rst.z) < t) {
                ...
                if (fabs(1.0 + Q.z) > ON_SQRT_EPSILON ||
                    (Q.x * Q.x + Q.y * Q.y) >
                        1.0 + 2.0 * ON_SQRT_EPSILON + ON_EPSILON) {
        ...
        if (rc >= 0) {
            ...
            if (0 == side0) {
                ...
                t1 = rst.z + t0 * n.z;
                if (t1 >= -(1.0 + ON_SQRT_EPSILON) &&
                    t1 <= 1.0 + ON_SQRT_EPSILON) {
...
class CBrepMappingData {
  public:
    CBrepMappingData(ON__UINT32 brepDataCRC = 0,
                     ON_UUID brepModelObjectId = ON_nil_uuid) {
        m_brepDataCRC = brepDataCRC;
        m_brepModelObjectId = brepModelObjectId;
    }
...
class CBrepMappingDataCache {
  public:
    ...
    bool GetBrepFaceMappingData(const ON_Brep *pBrep, int face,
                                CBrepFaceMappingData &brepFaceMappingDataOut);

  protected:
    ON_ClassArray<CBrepMappingData> m_aDataBlocks[0x20];
};

bool CBrepMappingDataCache::GetBrepFaceMappingData(
    const ON_Brep *pBrep, int face,
    CBrepFaceMappingData &brepFaceMappingDataOut) {
    ...
    const int brepMappingDataIndex = m_aDataBlocks[blockIndex].BinarySearch(
        &brepMappingData, CBrepMappingData::Compare);

    if (0 <= brepMappingDataIndex) {
        const CBrepMappingData &brepMappingData =
            m_aDataBlocks[blockIndex][brepMappingDataIndex];
...
void ON_TextureMapping::SetAdvancedBrepMappingToolFunctions(
    TEXMAP_INTERSECT_LINE_SURFACE pFnILS,
...
int ON_TextureMapping::Evaluate(const ON_3dPoint &P, const ON_3dVector &N,
                                ON_3dPoint *T, const ON_Xform &P_xform,
...
int ON_TextureMapping::Evaluate(const ON_3dPoint &P, const ON_3dVector &N,
    ...
    case mesh_mapping_primitive:
        ...
        break;
    case srf_mapping_primitive:
        rc = EvaluateSurfaceMapping(P, N, ON_Surface::Cast(m_mapping_primitive),
                                    T);
        break;
    case brep_mapping_primitive:
        ...
        if (0 != m_mapping_primitive) {
            ...
            case ON_TextureMapping::mesh_mapping_primitive: {
                ...
                if (mesh->HasTextureCoordinates()) {
                    ...
                    //   Including m_T[] in crc32 per Jussi and Andy email
                    //   discussion. This is probably correct because users will
                    //   expect the "picture" on the mesh to be applied to the
                    //   target in a visual way.
                    const ON_2fPoint *tex = mesh->m_T.Array();
                    crc32 = ON_CRC32(crc32, mesh->m_T.Count() * sizeof(tex[0]),
                                     tex);
                }
            } break;
            ...
            case ON_TextureMapping::srf_mapping_primitive: {
                const ON_Surface *surface =
                    ON_Surface::Cast(m_mapping_primitive);
                if (0 == surface)
                    break;
...
bool ON_TextureMapping::HasMatchingTextureCoordinates(
    const ON_Mesh &mesh, const ON_Xform *mesh_xform) const {
...
bool ON_TextureMapping::HasMatchingTextureCoordinates(
    const ON_MappingTag &tag, const ON_Xform *mesh_xform) const {
    bool rc = false;
    ...
    if (tag.m_mapping_crc == MappingCRC()) {
        ...
        if (ON_TextureMapping::srfp_mapping != m_type && mesh_xform &&
            mesh_xform->IsValid() && !mesh_xform->IsZero() &&
...
static bool GetSPTCHelper(const ON_Mesh &mesh, const ON_TextureMapping &mapping,
                          float *tc, int tc_stride) {
    ...
    bool bHaveUVWXform = mapping.m_uvw.IsValid() &&
                         !mapping.m_uvw.IsIdentity() && !mapping.m_uvw.IsZero();
...
bool ON_TextureMapping::GetTextureCoordinates(
    const ON_Mesh &mesh, ON_SimpleArray<ON_3fPoint> &T,
    const ON_Xform *mesh_xform, bool bLazy, ON_SimpleArray<int> *Tside) const {
    if (Tside)
        Tside->SetCount(0);
    ...
    if (bLazy) {
        ...
        for (tci = 0; tci < tccount; tci++) {
            if (vcnt == mesh.m_TC[tci].m_T.Count()) {
                if (HasMatchingTextureCoordinates(mesh.m_TC[tci].m_tag,
                                                  mesh_xform)) {
                    T = mesh.m_TC[tci].m_T;
                    return true;
                }
    ...
    if (ON_TextureMapping::srfp_mapping == m_type) {
        ...
        const ON_3fVector *mesh_N =
            mesh.HasVertexNormals() ? mesh.m_N.Array() : 0;
        ...
        if (clspt_projection == m_projection &&
            ...
            if (PT && NT) {
                ...
                for (i = 0; i < vcnt; i++) {
                    ...
                    w = (0.0 != w) ? 1.0 / w : 1.0;
                    P.x = w *
                          (PT[0] * f[0] + PT[1] * f[1] + PT[2] * f[2] + PT[3]);
                    P.y = w *
                          (PT[4] * f[0] + PT[5] * f[1] + PT[6] * f[2] + PT[7]);
                    P.z = w * (PT[8] * f[0] + PT[9] * f[1] + PT[10] * f[2] +
                               PT[11]);
            ...
            for (i = 0; i < vcnt; i++) {
                ...
                P.z =
                    w * (PT[8] * f[0] + PT[9] * f[1] + PT[10] * f[2] + PT[11]);
                sd = Evaluate(P, N, &tc);
                T[i] = tc;
                if (Tsd)
                    Tsd[i] = sd;
...
bool ON_TextureMapping::GetTextureCoordinates(
    const ON_Mesh &mesh, ON_SimpleArray<ON_2fPoint> &T,
    const ON_Xform *mesh_xform, bool bLazy, ON_SimpleArray<int> *Tside) const {
    bool rc = false;
    if (Tside)
        Tside->SetCount(0);
...
//    if ( bRotateTexture )
//    {
//      x2.m_xform[0][0] =  0.0; x2.m_xform[0][1] = -1.0; x2.m_xform[0][3]
//      = 1.0; x2.m_xform[1][0] =  1.0; x2.m_xform[1][1] =  0.0;
//    }
...
class ON__CChangeTextureCoordinateHelper {
    ...
    // IT IS A PRIVATE HELPER CLASS.
  public:
    ON__CChangeTextureCoordinateHelper(ON_Mesh &mesh, int newvcnt,
                                       float *&mesh_T);
    int DupVertex(int vi);
    void ChangeTextureCoordinate(int *Fvi, int fvi, float x, float y,
  ...
  private:
    ...
    ON__CChangeTextureCoordinateHelper(
        const ON__CChangeTextureCoordinateHelper &);
    ON__CChangeTextureCoordinateHelper &
    operator=(const ON__CChangeTextureCoordinateHelper &);
};

void ON__CChangeTextureCoordinateHelper::ChangeTextureCoordinate(
...
int ON__CChangeTextureCoordinateHelper::DupVertex(int vi) {
    ...
    if (vi >= 0 && vi < m_vuse_count) {
        if (m_vuse[vi] <= 1)
            return vi; // only one face uses this vertex - no need to dup the
                       // vertex
...
static int IntersectBoxSideRayHelper(int side, const ON_3dPoint &rst,
                                     const ON_3dVector &n, double *s) {
    /*
    returns:
      0 = ray parallel to sides
      1 = ray hit left side (x=-1)
    ...
    */
    ...
    if (fabs(Q.x - t1) > ON_SQRT_EPSILON || fabs(Q.y) > 1.0e8 ||
        fabs(Q.z) > 1.0e8) {
        *s = ON_UNSET_VALUE;
        return 0;
    }
...
static bool EvBoxSideTextureCoordinateHelper2(
    int side, const ON_TextureMapping &box_mapping, const ON_3dPoint &P,
    const ON_3dVector &N, ON_3dPoint *T) {
    // side flag
    //  1 =  left side (x=-1)
    //  2 =  right side (x=+1)
    //  3 =  back side (y=-1)
...
static bool EvBoxSideTextureCoordinateHelper1(
    const ON_Mesh &mesh, const ON_Xform *mesh_xform, int vi, int side,
    const ON_TextureMapping &box_mapping, float *Tx, float *Ty) {
    bool rc = false;
    ON_3dPoint P, tc;
    ON_3dVector N(0.0, 0.0, 0.0);
    ...
    if (mesh_N &&
        ON_TextureMapping::ray_projection == box_mapping.m_projection) {
        // calculation uses mesh vertex normal
        if (PT && NT) {
            // need to transform vertex and normal
            // before calculating texture coordinates
...
static void AdjustSingleBoxTextureCoordinatesHelper(
    ON_Mesh &mesh, const ON_Xform *mesh_xform, float *mesh_T, int mesh_T_stride,
    const int *Tsd, const ON_TextureMapping &box_mapping) {
    const int vcnt = mesh.m_V.Count();
    ...
    for (fi = 0; fi < fcnt; fi++) {
        ...
        // This 'continue' only works for faces having one or more of its tc's
        // in (0,1)x(0,1). I have commented it out as a fix to RR 90329.
        // if ( d <= 0.0f )
        //  continue;
        ...
        for (j = 0; j < fvicnt; j++) {
            if (sd[j] != side) {
                ...
                if (EvBoxSideTextureCoordinateHelper1(mesh, mesh_xform, Fvi[j],
                                                      side, box_mapping, &tc.x,
                                                      &tc.y)) {
    ...
    for (k = 0; k < mflist_count; k++) {
        ...
        for (j = 0; j < fvicnt; j++) {
            if (mf.bNewV[j]) {
                helper.ChangeTextureCoordinate(fvi, j, mf.tc[j].x, mf.tc[j].y,
...
static void AdjustMeshPeriodicTextureCoordinatesHelper(
    ON_Mesh &mesh, const ON_Xform *mesh_xform, float *mesh_T, int mesh_T_stride,
    const int *Tsd, double two_pi_tc, const ON_TextureMapping &mapping) {
    ...
    const bool bSphereCheck =
        (ON_TextureMapping::sphere_mapping == mapping_type);
    const bool bCylinderCheck =
        (Tsd && ON_TextureMapping::cylinder_mapping == mapping_type);
    const bool bBoxCheck =
        (Tsd && ON_TextureMapping::box_mapping == mapping_type);

    if (bBoxCheck && ON_TextureMapping::single == mapping.m_texture_space) {
        AdjustSingleBoxTextureCoordinatesHelper(mesh, mesh_xform, mesh_T,
                                                mesh_T_stride, Tsd, mapping);
        return;
    }
        ...
        if (0 != (8 & q)) {
            ...
            if (8 == ftc.quad[0]) {
                ...
                if (ON_UNSET_FLOAT != t0 || ON_UNSET_FLOAT != t1) {
                    ...
                    ftc.quad[0] =
                        ((ftc.Tx[0] < ang0) ? 1 : ((ftc.Tx[0] > ang1) ? 4 : 0));
                    q |= ftc.quad[0];
                    ftc.fi = fi;
                }
            }
            if (8 == ftc.quad[1]) {
                ...
                if (ON_UNSET_FLOAT != t0 || ON_UNSET_FLOAT != t1) {
                    ...
                    ftc.quad[1] =
                        ((ftc.Tx[1] < ang0) ? 1 : ((ftc.Tx[1] > ang1) ? 4 : 0));
                    q |= ftc.quad[1];
                    ftc.fi = fi;
                }
            }
            if (8 == ftc.quad[2]) {
                ...
                if (ON_UNSET_FLOAT != t0 || ON_UNSET_FLOAT != t1) {
                    ...
                    ftc.quad[2] =
                        ((ftc.Tx[2] < ang0) ? 1 : ((ftc.Tx[2] > ang1) ? 4 : 0));
                    if (!k) {
                        ftc.Tx[3] = ftc.Tx[2];
            ...
            if (8 == ftc.quad[3] && Fvi[2] != Fvi[3]) {
                ...
                if (ON_UNSET_FLOAT != t0 || ON_UNSET_FLOAT != t1) {
                    ...
                    ftc.quad[3] =
                        ((ftc.Tx[3] < ang0) ? 1 : ((ftc.Tx[3] > ang1) ? 4 : 0));
                    q |= ftc.quad[3];
                    ftc.fi = fi;
                }
            }
        ...
        if (5 == (5 & q)) {
            ...
            if (two_pi_tc == 1.0) {
                ...
                if (f0 >= f1) {
                    ...
                    // If a vertex is on the left side, clamp its tc to
                    // two_pi_tc.
                    if (1 == ftc.quad[0]) {
                        ftc.Tx[0] = twopitc;
                        ftc.fi = fi;
...
// If there are unused vertices, this may not work correctly - but it is very
// fast.
static inline bool HasSharedVertices(const ON_Mesh &mesh) {
    return mesh.m_V.Count() <
           ((mesh.TriangleCount() * 3) + (mesh.QuadCount() * 4));
}
...
ON_Mesh::SetCachedTextureCoordinates(const class ON_TextureMapping &mapping,
    ...
    if (bLazy && TC &&
        mapping.HasMatchingTextureCoordinates(TC->m_tag, mesh_xform))
        return TC;
    ...
    if (bSeamCheck && m_F.Count() > 0 && TC->m_T.Count() == m_V.Count()) {
        ...
        AdjustMeshPeriodicTextureCoordinatesHelper(
            *this, mesh_xform, mesh_T, mesh_T_stride, Tsd ? Tside.Array() : 0,
            two_pi_tc, mp);
...
int ON_MaterialRef::Compare(const ON_MaterialRef &other) const {
    ...
    if (!rc)
        rc = ON_UuidCompare(m_material_backface_id,
                            other.m_material_backface_id);
    return rc;
}
...
bool ON_MappingRef::Transform(const ON_Xform &xform) {
    ...
    if (count > 0) {
        for (ON_MappingChannel *mapping_channel = m_mapping_channels.Array();
             count--; mapping_channel++) {
            mapping_channel->m_object_xform =
                xform * mapping_channel->m_object_xform;
        }
    }
    return true;
}
...
void ON_RenderingAttributes::Default() { m_materials.Destroy(); }
        ...
        for (i = 0; i < count - 1; i++) {
            ...
            for (j = i + 1; j < count; j++) {
                if (!ON_UuidCompare(&plugin_id, &mr[j].m_plugin_id)) {
                    if (text_log) {
                        text_log->Print(
                            L"ON_RenderingAttributes error: m_materials[%d] "
                            L"and m_materials[%d] have the same plug-in id.\n",
                            i, j);
                    }
                    return false;
                }
    ...
    if ((count = m_mappings.Count()) > 1) {
        ...
        for (i = 0; i < count - 1; i++) {
            ...
            for (j = i + 1; j < count; j++) {
                if (!ON_UuidCompare(&plugin_id, &mr[j].m_plugin_id)) {
                    if (text_log) {
                        text_log->Print(L"ON_ObjectRenderingAttributes error: "
                                        L"m_mappings[%d] and m_mappings[%d] "
                                        L"have the same plug-in id.\n",
                                        i, j);
                    }
                    return false;
                }
...
int ON_ObjectRenderingAttributes::Compare(
    const ON_ObjectRenderingAttributes &other) const {
    ...
    if (!rc) {
        ...
        if (!rc) {
            rc = ((int)(m_bCastsShadows ? 1 : 0)) -
                 ((int)(other.m_bCastsShadows ? 1 : 0));
            if (!rc) {
...
ON_ObjectRenderingAttributes::AddMappingRef(const ON_UUID &plugin_id) {
    ...
    int count;
    if ((count = m_mappings.Count()) > 0) {
        for (mr = const_cast<ON_MappingRef *>(m_mappings.Array()); count--;
             mr++) {
            if (plugin_id == mr->m_plugin_id)
                break;
        }
...
bool ON_ObjectRenderingAttributes::DeleteMappingRef(const ON_UUID &plugin_id) {
    ...
    if (mr)
        m_mappings.Remove(
            (int)(mr - m_mappings.Array())); // safe ptr to in conversion
    return (0 != mr);
}
...
ON_ObjectRenderingAttributes::MappingChannel(const ON_UUID &plugin_id,
    ...
    if (mr) {
        ...
        if ((count = mr->m_mapping_channels.Count()) > 0) {
            for (const ON_MappingChannel *mc = mr->m_mapping_channels.Array();
                 count--; mc++) {
...
bool ON_ObjectRenderingAttributes::AddMappingChannel(
    const ON_UUID &plugin_id, int mapping_channel_id,
    ...
    if (!mr) {
        ...
        mc.m_mapping_index = -1; // 27th October 2011 John Croudy - constructor
                                 // is not called by AppendNew().
...
bool ON_ObjectRenderingAttributes::DeleteMappingChannel(
    const ON_UUID &plugin_id, const ON_UUID &mapping_id) {
    ON_MappingRef *mr = const_cast<ON_MappingRef *>(MappingRef(plugin_id));
    return mr ? mr->DeleteMappingChannel(mapping_id) : false;
}

bool ON_ObjectRenderingAttributes::ChangeMappingChannel(
    const ON_UUID &plugin_id, int old_mapping_channel_id,
    int new_mapping_channel_id) {
    ON_MappingRef *mr = const_cast<ON_MappingRef *>(MappingRef(plugin_id));
    return mr ? mr->ChangeMappingChannel(old_mapping_channel_id,
...
ON_MappingRef::MappingChannel(const ON_UUID &mapping_id) const {
    ...
    if ((count = m_mapping_channels.Count()) > 0) {
        for (const ON_MappingChannel *mc = m_mapping_channels.Array(); count--;
             mc++) {
            if (mapping_id == mc->m_mapping_id)
                return mc;
        }
...
bool ON_MappingRef::AddMappingChannel(int mapping_channel_id,
    ...
    if ((i = m_mapping_channels.Count()) > 0) {
        for (const ON_MappingChannel *mc = m_mapping_channels.Array(); i--;
             mc++) {
            if (mapping_channel_id == mc->m_mapping_channel_id) {
    ...
    mc.m_mapping_channel_id = mapping_channel_id;
    mc.m_mapping_id = mapping_id;
    mc.m_mapping_index = -1; // 27th October 2011 John Croudy - constructor is
                             // not called by AppendNew().
    mc.m_object_xform.Identity();
...
bool ON_TextureMapping::SetBoxMapping(const ON_Plane &plane, ON_Interval dx,
                                      ON_Interval dy, ON_Interval dz,
                                      bool bCapped) {
...
bool ON_TextureMapping::GetMappingPlane(ON_Plane &plane, ON_Interval &dx,
                                        ON_Interval &dy,
...
bool ON_TextureMapping::GetMappingBox(ON_Plane &plane, ON_Interval &dx,
                                      ON_Interval &dy, ON_Interval &dz) const {
