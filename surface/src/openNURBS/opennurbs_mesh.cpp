...
static const ON_3dPoint *Mesh_dV(const ON_Mesh &mesh) {
    if (mesh.HasDoublePrecisionVertices() &&
        mesh.DoublePrecisionVerticesAreValid()) {
...
static // LEAVE THIS STATIC
    void
    ON_MeshSurfaceHelper(ON_Mesh *mesh, int vcnt_s, int vcnt_t,
...
ON_Mesh *ON_MeshSurface(const ON_Surface &surface, int u_count, const double *u,
                        int v_count, const double *v, ON_Mesh *mesh) {
    int i, j;
    ...
    for (i = 0; i < u_count; i++) {
        for (j = 0; j < v_count; j++) {
            ...
            mesh->m_T.AppendNew().Set(
                (float)mesh->m_srf_domain[0].NormalizedParameterAt(u[i]),
    ...
    if (bNeedHelper) {
        ON_MeshSurfaceHelper(mesh, u_count, v_count, srfIsClosed,
                             srfIsSingular);
    }
...
static ON_Mesh *ON_MeshSurface(const ON_Surface &surface, int mesh_density,
                               bool bDoublePrecisionMesh, ON_Mesh *mesh) {
    ...
    ON_3dPointArray &D =
        bDoublePrecisionMesh ? mesh->DoublePrecisionVertices() : dummy_array;
    ...
    if (bNeedHelper) {
        ON_MeshSurfaceHelper(mesh, vcnt_s, vcnt_t,
...
ON_Mesh *ON_MeshSurface(const ON_Surface &surface, int mesh_density,
                        ON_Mesh *mesh) {
    return ON_MeshSurface(surface, mesh_density, false, mesh);
}
...
ON_OBJECT_IMPLEMENT(ON_Mesh, ON_Geometry,
...
bool ON_MeshFace::IsValid(int mesh_vertex_count) const {
    return (vi[0] >= 0 && vi[0] < mesh_vertex_count && vi[1] >= 0 &&
            vi[1] < mesh_vertex_count && vi[2] >= 0 &&
            vi[2] < mesh_vertex_count && vi[3] >= 0 &&
            vi[3] < mesh_vertex_count && vi[0] != vi[1] && vi[1] != vi[2] &&
            vi[2] != vi[0] &&
            (vi[2] == vi[3] || (vi[0] != vi[3] && vi[1] != vi[3])));
}

bool ON_MeshFace::IsValid(int mesh_vertex_count, const ON_3fPoint *V) const {
...
bool ON_MeshFace::IsValid(int mesh_vertex_count, const ON_3dPoint *V) const {
    if (!IsValid(mesh_vertex_count))
        return false;
    if (!(V[vi[0]] != V[vi[1]]))
        return false;
...
bool ON_MeshFace::Repair(int mesh_vertex_count) {
    ...
    if (vi[2] >= 0 && vi[2] < mesh_vertex_count && f.vi[0] != vi[2] &&
        f.vi[1] != vi[2])
        f.vi[fvi_count++] = vi[2];

    if (vi[3] >= 0 && vi[3] < mesh_vertex_count && f.vi[0] != vi[3] &&
        f.vi[1] != vi[3] && f.vi[2] != vi[3])
        f.vi[fvi_count++] = vi[3];
...
bool ON_MeshFace::Repair(int mesh_vertex_count, const ON_3fPoint *V) {
    ON_MeshFace f;
    int fvi_count = 0;
    f.vi[0] = f.vi[1] = f.vi[2] = f.vi[3] = -1;
    ...
    if (vi[3] >= 0 && vi[3] < mesh_vertex_count && f.vi[0] != vi[3] &&
        f.vi[1] != vi[3] && f.vi[2] != vi[3] && V[f.vi[0]] != V[vi[3]] &&
        V[f.vi[1]] != V[vi[3]]) {
...
bool ON_MeshFace::Repair(int mesh_vertex_count, const ON_3dPoint *V) {
    ON_MeshFace f;
    int fvi_count = 0;
    f.vi[0] = f.vi[1] = f.vi[2] = f.vi[3] = -1;
...
ON_Mesh::ON_Mesh()
    : m_packed_tex_rotate(0), m_parent(0), m_mesh_parameters(0),
      m_invalid_count(0), m_quad_count(0), m_triangle_count(0),
      m_mesh_is_closed(0), m_mesh_is_manifold(0), m_mesh_is_oriented(0),
      m_mesh_is_solid(0), m_mtree(0) {
    m_top.m_mesh = this;
    m_srf_scale[0] = 0.0;
    m_srf_scale[1] = 0.0;
    m_kstat[0] = 0;
...
ON_Mesh::ON_Mesh(
    ...
    : m_V(initial_vertex_capacity), m_F(initial_facet_capacity),
...
ON__UINT32 ON_Mesh::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder =
        ON_CRC32(current_remainder, m_V.Count() * sizeof(p[0]), p);
    current_remainder = ON_CRC32(
        current_remainder, m_F.Count() * sizeof(ON_MeshFace), m_F.Array());
    const ON_3fVector *v = m_N.Array();
    current_remainder =
        ON_CRC32(current_remainder, m_N.Count() * sizeof(v[0]), v);
...
ON_BOOL32 ON_Mesh::IsValid(ON_TextLog *text_logx) const {
    ...
    if (m_S.Count() > 0 && m_S.Count() != vertex_count) {
        if (text_log) {
            text_log->Print(
                "ON_Mesh.m_S.Count() = %d (should be 0 or %d=vertex_count).\n",
    ...
    if (HasVertexNormals()) {
        ...
        for (vi = 0; vi < vertex_count; vi++) {
            x = m_N[vi][0] * m_N[vi][0] + m_N[vi][1] * m_N[vi][1] +
                m_N[vi][2] * m_N[vi][2];
            if (x < 0.985 || x > 1.015) {
                if (text_log) {
                    text_log->Print(
    ...
    // Greg Arden 9 May 2003. Fixes TRR#10604.  Attempt to detect meshes with
    // non-finite vertices by testing the bounding box.
    int i;
    for (i = 0; i < 3; i++) {
        if (!ON_IsValid(m_vbox[0][i]) || !ON_IsValid(m_vbox[1][i])) {
            if (text_log) {
                text_log->Print("ON_Mesh.m_vbox is not finite.  Check for "
                                "invalid vertices\n");
            }
            return ON_MeshIsNotValid(bSilentError);
        }
    ...
    while (HasDoublePrecisionVertices()) {
        ...
        if (!bSynchronized) {
            if (text_log) {
                text_log->Print("Single and double precision vertices are not "
                                "synchronized.\n");
            }
            return ON_MeshIsNotValid(bSilentError);
        }
        ...
        if (!bValidDoubles) {
            if (text_log) {
                text_log->Print("Double precision vertices appear to be ok but "
                                "are not marked as valid\n");
            }
            return ON_MeshIsNotValid(bSilentError);
        }
        ...
        if (!bValidFloats) {
            if (text_log) {
                text_log->Print("Single precision vertices appear to be ok but "
                                "are not marked as valid\n");
            }
            return ON_MeshIsNotValid(bSilentError);
        }
    ...
    if (0 != dV) {
            ...
            if (!m_F[fi].IsValid(vertex_count, dV)) {
                if (text_log) {
                    if (!m_F[fi].IsValid(vertex_count))
                        text_log->Print("ON_Mesh.m_F[%d].vi[] has invalid "
                                        "vertex indices.\n",
                                        fi);
                    else
                        text_log->Print("ON_Mesh.m_F[%d] has degenerate double "
                                        "precision vertex locations.\n",
                                        fi);
                }
                return ON_MeshIsNotValid(bSilentError);
            }
        ...
        for (fi = 0; fi < facet_count; fi++) {
            ...
            //  {
            //    if ( !m_F[fi].IsValid( vertex_count) )
            //      text_log->Print("ON_Mesh.m_F[%d].vi[] has invalid vertex
            //      indices.\n",fi);
            //    else
            //      text_log->Print("ON_Mesh.m_F[%d] has degenerate float
            //      precision vertex locations.\n",fi);
            //  }
            //  return ON_MeshIsNotValid(bSilentError);
            //}
...
void ON_Mesh::Dump(ON_TextLog &dump) const {
    ...
    dump.Print("m_srf_domain: (%g,%g)x(%g,%g)\n", m_srf_domain[0][0],
               m_srf_domain[0][1], m_srf_domain[1][0], m_srf_domain[1][1]);
    ...
    dump.Print("%d mesh vertices:\n", m_V.Count());
    {
        ...
        for (i = 0; i < vcount; i++) {
            if (i == half_max && 2 * half_max < vcount) {
                ...
                if (0 != D) {
                    q = D[i];
                    dump.Print("m_V[%d] = (%.17g,%.17g,%.17g) D = "
                               "(%.17g,%.17g,%.17g)\n",
        ...
        for (i = 0; i < fcount; i++) {
            if (i == half_max && 2 * half_max < fcount) {
                ...
                dump.Print("m_F[%d].vi = (%d,%d,%d)\n", i, m_F[i].vi[0],
                           m_F[i].vi[1], m_F[i].vi[2]);
            else
                dump.Print("m_F[%d].vi = (%d,%d,%d,%d)\n", i, m_F[i].vi[0],
                           m_F[i].vi[1], m_F[i].vi[2], m_F[i].vi[3]);
        }
        dump.PopIndent();
    }
...
bool ON_Mesh::WriteFaceArray(int vcount, int fcount,
...
bool ON_Mesh::Write_2(int Vcount, ON_BinaryArchive &file) const {
    ...
    if (Vcount > 0) {
        ...
        if (e == ON::big_endian) {
            ...
            file.ToggleByteOrder(Vcount * 3, 4, m_V.Array(),
                                 (void *)m_V.Array());
            file.ToggleByteOrder(Ncount * 3, 4, m_N.Array(),
                                 (void *)m_N.Array());
            file.ToggleByteOrder(Tcount * 2, 4, m_T.Array(),
                                 (void *)m_T.Array());
            file.ToggleByteOrder(Kcount * 2, 8, m_K.Array(),
                                 (void *)m_K.Array());
            file.ToggleByteOrder(Ccount, 4, m_C.Array(), (void *)m_C.Array());
        }
        if (rc)
            rc = file.WriteCompressedBuffer(Vcount * sizeof(ON_3fPoint),
                                            m_V.Array());
        if (rc)
            rc = file.WriteCompressedBuffer(Ncount * sizeof(ON_3fVector),
                                            m_N.Array());
        if (rc)
            rc = file.WriteCompressedBuffer(Tcount * sizeof(ON_2fPoint),
                                            m_T.Array());
        if (rc)
            rc = file.WriteCompressedBuffer(
                Kcount * sizeof(ON_SurfaceCurvature), m_K.Array());
        if (rc)
            rc = file.WriteCompressedBuffer(Ccount * sizeof(ON_Color),
                                            m_C.Array());
        if (e == ON::big_endian) {
            // These calls restore the m_V[], m_N[], m_T[], m_K[] and m_C[]
            // arrays to the correct big endian runtime byte order.  This must
            // be done even if rc is false.
            file.ToggleByteOrder(Vcount * 3, 4, m_V.Array(),
                                 (void *)m_V.Array());
            file.ToggleByteOrder(Ncount * 3, 4, m_N.Array(),
                                 (void *)m_N.Array());
    ...
    if (vcount > 0) {
        ...
        if (rc && sz) {
            if (sz == vcount * sizeof(m_V[0])) {
                ...
                if (rc)
                    rc =
                        file.ReadCompressedBuffer(sz, m_V.Array(), &bFailedCRC);
                if (rc)
                    m_V.SetCount(vcount);
            } else {
                ON_ERROR("ON_Mesh::Read - compressed vertex point buffer size "
                         "is wrong.");
        ...
        if (rc && sz) {
            if (sz == vcount * sizeof(m_N[0])) {
                ...
                if (rc)
                    rc =
                        file.ReadCompressedBuffer(sz, m_N.Array(), &bFailedCRC);
                if (rc)
                    m_N.SetCount(vcount);
            } else {
                ON_ERROR("ON_Mesh::Read - compressed vertex normal buffer size "
                         "is wrong.");
                rc = false; // buffer is wrong size
            }
        }
        ...
        if (rc && sz) {
            if (sz == vcount * sizeof(m_T[0])) {
                ...
                if (rc)
                    rc =
                        file.ReadCompressedBuffer(sz, m_T.Array(), &bFailedCRC);
                if (rc)
                    m_T.SetCount(vcount);
            } else {
                ON_ERROR("ON_Mesh::Read - compressed texture coordinate buffer "
                         "size is wrong.");
                rc = false; // buffer is wrong size
            }
        }
        ...
        if (rc && sz) {
            if (sz == vcount * sizeof(m_K[0])) {
                ...
                if (rc)
                    rc =
                        file.ReadCompressedBuffer(sz, m_K.Array(), &bFailedCRC);
                if (rc)
                    m_K.SetCount(vcount);
            } else {
                ON_ERROR("ON_Mesh::Read - compressed vertex curvature buffer "
                         "size is wrong.");
                rc = false; // buffer is wrong size
            }
        }
        ...
        if (rc && sz) {
            if (sz == vcount * sizeof(m_C[0])) {
                ...
                if (rc)
                    rc =
                        file.ReadCompressedBuffer(sz, m_C.Array(), &bFailedCRC);
                if (rc)
                    m_C.SetCount(vcount);
            } else {
                ON_ERROR("ON_Mesh::Read - compressed vertex color buffer size "
                         "is wrong.");
                rc = false; // buffer is wrong size
            }
        }
        ...
        if (e == ON::big_endian) {
            ...
            file.ToggleByteOrder(m_V.Count() * 3, 4, m_V.Array(),
                                 (void *)m_V.Array());
            file.ToggleByteOrder(m_N.Count() * 3, 4, m_N.Array(),
                                 (void *)m_N.Array());
            file.ToggleByteOrder(m_T.Count() * 2, 4, m_T.Array(),
                                 (void *)m_T.Array());
            file.ToggleByteOrder(m_K.Count() * 2, 8, m_K.Array(),
                                 (void *)m_K.Array());
            file.ToggleByteOrder(m_C.Count() * 3, 4, m_C.Array(),
                                 (void *)m_C.Array());
        }
    }
...
ON_BOOL32 ON_Mesh::Write(ON_BinaryArchive &file) const {
    ...
    // archive int value meaning: -1 = unknown 0 = mesh is not closed, 1 = mesh
    // is closed
    i = -1;
    ...
    if (rc && vcount > 0) {
        ...
        const ON::endian e = file.Endian();
        if (e == ON::big_endian) {
            file.ToggleByteOrder(Scount * 2, 8, m_S.Array(),
                                 (void *)m_S.Array());
        }
        if (rc)
            rc = file.WriteCompressedBuffer(Scount * sizeof(ON_2dPoint),
                                            m_S.Array());
        if (e == ON::big_endian) {
            file.ToggleByteOrder(Scount * 2, 8, m_S.Array(),
                                 (void *)m_S.Array());
...
static const ON_UUID obsolete_default_srfp_mapping_id = {
    ...
    0x61a6,
    0x45a7,
    {0xaa, 0xee, 0x9a, 0xed, 0x7e, 0xf4, 0xe3, 0x16}};

// overrides virtual ON_Object::Write
ON_BOOL32 ON_TextureMapping::Write(ON_BinaryArchive &file) const {
...
ON_BOOL32 ON_TextureMapping::Read(ON_BinaryArchive &file) {
    Default();
    ...
    bool rc = file.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &major_version,
                                     &minor_version);
    if (rc) {
        if (1 == major_version) {
            ...
            for (;;) {
                ...
                if (!rc)
                    break;
                if (0 == ON_UuidCompare(&obsolete_default_srfp_mapping_id,
                                        &m_mapping_id))
...
static void GetSurfaceParametersHelper(const ON_Mesh &mesh, double tex_x,
                                       double tex_y, double *srf_s,
                                       double *srf_t) {
...
ON_BOOL32 ON_Mesh::Read(ON_BinaryArchive &file) {
    ...
    if (rc && (1 == major_version || 3 == major_version)) {
        ...
        // int value meaning: -1 = unknown 0 = mesh is not closed, 1 = mesh is
        // closed
        i = -1;
        if (rc)
            rc = file.ReadInt(&i);
        if (rc) {
            ...
            case 1: // closed;
                SetClosed(1);
                break;
            case 2: // 13 April 2010 Dale Lear - "2" value is obsolete but
                    // appears in old files
                SetClosed(1);
                break;
            }
        }
        ...
        if (3 == major_version) {
            if (minor_version >= 3) {
                ...
                if (rc && vcount > 0) {
                    ...
                    if (rc && sz) {
                        if (sz == vcount * sizeof(ON_2dPoint)) {
                            ...
                            if (rc)
                                rc = file.ReadCompressedBuffer(sz, m_S.Array(),
                                                               &bFailedCRC);
                            if (rc)
                                m_S.SetCount(vcount);
                            if (ON::big_endian == file.Endian()) {
                                file.ToggleByteOrder(m_S.Count() * 2, 8,
                                                     m_S.Array(),
                                                     (void *)m_S.Array());
                            }
                        } else {
                            ON_ERROR("ON_Mesh::Read - surface parameter buffer "
                                     "size is wrong.");
                            if (rc &&
                                file.ArchiveOpenNURBSVersion() <= 201011049 &&
                                0 == (sz % sizeof(ON_2dPoint)) &&
                                sz >= sizeof(ON_2dPoint)) {
                                // Before 201011049 there was a bug that let
                                // m_S[] arrays with the wrong size get saved in
                                // files. There was also a bug in the Rhino .OBJ
                                // file reader that created meshes with m_S[]
                                // arrays that had the wrong size.  The next 4
                                // lines of code let us read the junk, discard
                                // it and then successfully read the rest of the
                                // file.
                ...
                if (minor_version >= 4 &&
                    file.ArchiveOpenNURBSVersion() >= 200606010) {
                    if (rc)
                        rc = m_Ttag.Read(file);
                    if (minor_version >= 5) {
                        if (rc)
        ...
        if (0 == m_S.Count() && m_V.Count() > 0 && HasTextureCoordinates() &&
            ...
            m_packed_tex_domain[1].IsInterval() && 0 == m_Ttag.m_mapping_crc &&
...
ON::object_type ON_Mesh::ObjectType() const { return ON::mesh_object; }

int ON_Mesh::Dimension() const { return 3; }

#if defined(ON_COMPILER_MSC)
// Disable the MSC /W4 warning
//   C4189: 'breakpoint_here_for_bad_vbox' : local variable is initialized but
//   not referenced
// on the line
...
float ON_FloatFloor(double x) {
    ...
    // float value that is mathematically larger than the value of x.
    //
    const double e = (x < 0.0) ? (1.0 + 0.5 * ON_FLOAT_EPSILON)
                               : (1.0 - 0.5 * ON_FLOAT_EPSILON);
    float f;
    f = (float)(e * x);
    return f;
}
    ...
    if (facet_count >= 1 && vertex_count >= 3) {
        ...
        if (m_vbox[0][0] > m_vbox[1][0]) {
            ...
            while (HasDoublePrecisionVertices() &&
                ...
                rc = ON_GetPointListBoundingBox(3, 0, vertex_count, 3, &dV[0].x,
                                                &dbbox[0][0], &dbbox[1][0],
                                                false);
                if (!rc)
                    break;
                ...
                if (SinglePrecisionVerticesAreValid()) {
                    ON_GetPointListBoundingBox(3, 0, vertex_count, 3, &m_V[0].x,
                                               &fbbox[0][0], &fbbox[1][0],
                                               true);
                }
                break;
...
bool ON_Mesh::IsDeformable() const { return true; }

bool ON_Mesh::MakeDeformable() { return true; }
...
ON_BOOL32 ON_Mesh::Transform(const ON_Xform &xform) {
    ...
    const bool bSyncheddV = bIsValid_fV && bIsValid_dV &&
                            HasSynchronizedDoubleAndSinglePrecisionVertices();
    ...
    if (bSyncheddV) {
        ...
        rc =
            ON_TransformPointList(3, false, vertex_count, 3, &m_V[0][0], xform);
    }
    ...
    if (rc && HasPrincipalCurvatures()) {
        if (fabs(fabs(d) - 1.0) > ON_SQRT_EPSILON) {
            ...
            if (0.0 != scale && 0.0 != d && scale == xform.m_xform[1][1] &&
                ...
                for (int j = 0; j < 4; j++) {
                    if (m_kstat[j])
                        m_kstat[j]->Set(m_kstat[j]->m_style, m_K.Count(),
                                        m_K.Array(), m_N.Array());
                }
            } else {
                ON_ERROR("ON_Mesh::Transform() cannot apply this transform to "
                         "curvatures.\n");
                rc = false;
            }
        }
...
ON_BOOL32 ON_Mesh::SwapCoordinates(int i, int j // indices of coords to swap
) {
    if (i == j)
        return true;
    ...
    const int vertex_count = VertexCount();
    ON_BOOL32 rc =
        ON_SwapPointListCoordinates(vertex_count, 3, &m_V[0][0], i, j);
...
int ON_Mesh::GetVertexEdges(int vertex_index_count, const int *vertex_index,
                            bool bNoDuplicates,
    ...
    // 03/12/2007 TimH. The line below appears to be a typo.  Using the
    // following line works better. const int mesh_fcount = m_V.Count();
    ...
    if (TopologyExists() && mesh_vcount == m_top.m_topv_map.Count() &&
        ...
        for (fi = 0; fi < mesh_fcount; fi++) {
            ...
            for (fvi = 0; fvi < 4; fvi++) {
                vi = f_vi[fvi];
                if (ON_BinarySearchIntArray(vi, vertex_index,
                                            vertex_index_count)) {
    ...
    if (bNoDuplicates && edges.Count() > edges_count0) {
        ...
        ON_qsort(edges.Array() + edges_count0, edges.Count() - edges_count0,
                 sizeof(edge_ends), ON_MeshGetVertexEdges_Compare2dex);
...
int ON_Mesh::GetMeshEdges(ON_SimpleArray<ON_2dex> &edges) const {
    ...
    if (edges.Count() > edges_count0) {
        ON_qsort(edges.Array() + edges_count0, edges.Count() - edges_count0,
                 sizeof(e), ON_MeshGetVertexEdges_Compare2dex);
...
bool ON_Mesh::IsPointInside(ON_3dPoint test_point, double tolerance,
...
bool ON_Mesh::IsManifold(bool bTopologicalTest, bool *pbIsOriented,
    ...
    if (vcount > 0 && fcount > 0) {
        ...
        if (bTopologicalTest) {
            ...
            ON_Sort(ON::quick_sort, vid, m_V.Array(), vcount, sizeof(m_V[0]),
        ...
        for (i = 0; i < fcount; i++) {
            fvi = (f++)->vi;
            if (fvi[0] >= 0 && fvi[0] < vcount && fvi[1] >= 0 &&
                fvi[1] < vcount && fvi[2] >= 0 && fvi[2] < vcount &&
        ...
        if (ecount >= 4) {
            ...
            ON_qsort(edge, ecount, sizeof(edge[0]),
...
static void ON_hsort_3dex(ON_3dex *e, size_t nel) {
    ...
    for (;;) {
        ...
        j = (k << 1) + 1;
        while (j <= i_end) {
            if (j < i_end && (e[j].i < e[j + 1].i ||
                              (e[j].i == e[j + 1].i &&
                               (e[j].j < e[j + 1].j || (e[j].j == e[j + 1].j &&
                                                        e[j].k < e[j + 1].k)))))
                j++;
...
static int *GetVidHelper(const int Vcount, const ON_3fPoint *fV,
                         const ON_3dPoint *dV, int first_vid, int *Vid,
                         int *Vindex) {
...
static const ON_3dPoint *MeshDoublePrecisionVertices(const ON_Mesh &mesh) {
    if (mesh.HasDoublePrecisionVertices() &&
        mesh.DoublePrecisionVerticesAreValid()) {
        const ON_3dPointArray &a = mesh.DoublePrecisionVertices();
        if (a.Count() == mesh.m_V.Count())
...
static void ON_Mesh_SetClosedHelper(bool bClosedOnly, const ON_Mesh &mesh,
    ...
    //       If bClosedOnly is false, also sets m_mesh_is_manifold and
    //       m_mesh_is_oriented
    int is_closed = 0;
    char is_manifold = 2;
    char is_oriented = 2;
    for (;;) {
        ...
        int *Vid = mesh.GetVertexLocationIds(
            1, (Vcount * sizeof(*Vid) <= sizeof(Vidbuffer) ? &Vidbuffer[0] : 0),
            0);
        if (0 == Vid) {
        ...
        // build an edge list where the "vertex" indices identify unique 3d
        // locations
        ...
        if (!bClosedOnly || 1 == is_closed)
            for (i = 0; i < E_count; /*empty iterator*/) {
                ...
                if (E.k == E_list[i].k) {
                    // opposite face normals along this edge - mesh is not
                    // oriented
                    is_oriented = 2;
                }

                if (++i >= E_count || E.i != E_list[i].i ||
                    E.j != E_list[i].j) {
                    // two faces share this edge
                    continue;
                }

                // three or more faces share this edge - mesh is not oriented
                // manifold
                is_oriented = 2;
                is_manifold = 2;
                if (0 == is_closed) {
                ...
                for (i++; i < E_count; i++) {
                    if (E.i != E_list[i].i || E.j != E_list[i].j) {
                        // the edges E and Eid_list[i] are in different
                        // locations
                        break;
                    }
                }
...
bool ON_Mesh::IsClosed() const {
    if (m_mesh_is_closed <= 0 || m_mesh_is_closed > 2) {
        ...
        ON_Mesh_SetClosedHelper(true, *this, m_mesh_is_manifold,
                                m_mesh_is_oriented);
    }
...
bool ON_Mesh::IsManifold() const {
    if (m_mesh_is_manifold <= 0 || m_mesh_is_manifold > 2) {
        // thread safe lazy evaluation
        ON_Mesh_SetClosedHelper(false, *this, m_mesh_is_manifold,
                                m_mesh_is_oriented);
    }
...
bool ON_Mesh::SetVertex(int vertex_index, const ON_3dPoint &vertex_location) {
    bool rc = false;
...
bool ON_Mesh::SetVertex(int vertex_index, const ON_3fPoint &vertex_location) {
    bool rc = false;
    int vertex_count = m_V.Count();
    if (vertex_index >= 0 && vertex_index <= vertex_count) {
        bool bIsValid_fV = false;
...
bool ON_Mesh::SetVertexNormal(int vertex_index, const ON_3dVector &normal) {
    bool rc = false;
    // use double precision for unitizing normal
    ON_3dVector unit_vector = normal;
    const bool bUnitVector = unit_vector.Unitize();
    ON_3fVector v((float)unit_vector.x, (float)unit_vector.y,
                  (float)unit_vector.z);
...
bool ON_Mesh::SetVertexNormal(int vertex_index, const ON_3fVector &normal) {
    ON_3dVector v(normal.x, normal.y, normal.z);
    return SetVertexNormal(vertex_index, v);
}

bool ON_Mesh::SetTextureCoord(int vertex_index, double s,
                              double t // texture coordinates
) {
...
bool ON_Mesh::SetTriangle(int face_index, int a, int b, int c // vertex indices
) {
    return SetQuad(face_index, a, b, c, c);
}

bool ON_Mesh::SetQuad(int face_index, int a, int b, int c,
                      int d // vertex indices
) {
    bool rc = false;
...
int ON_Mesh::FaceCount() const { return m_F.Count(); }
...
int ON_Mesh::VertexCount() const { return m_V.Count(); }
...
bool ON_Mesh::GetCurvatureStats( // returns true if successful
    ON::curvature_style kappa_style, ON_MeshCurvatureStats &stats) const {
    bool rc = false;
    stats.Destroy();
    int ksi;
    switch (kappa_style) {
    ...
    if (ksi >= 0 && ksi <= 3 && HasPrincipalCurvatures()) {
        ...
        if (!m_kstat[ksi]) {
            ...
            p->m_kstat[ksi]->Set(kappa_style, m_K.Count(), m_K.Array(),
                                 m_N.Array());
        }
        if (p->m_kstat[ksi]) {
            stats = *p->m_kstat[ksi];
...
bool ON_Mesh::TopologyExists() const { return (1 == m_top.m_b32IsValid); }
...
void ON_Mesh::DestroyTopology() { m_top.Destroy(); }

bool ON_MeshFace::IsTriangle() const { return vi[2] == vi[3]; }

bool ON_MeshFace::IsQuad() const { return vi[2] != vi[3]; }
...
bool ON_Mesh::ComputeFaceNormal(int fi) {
    ...
    if (fi >= 0 && fi < fcount && HasFaceNormals()) {
        ...
        n = ON_CrossProduct(
            a, b); // works for triangles, quads, and nonplanar quads
        rc = n.Unitize();
        m_FN[fi] = n;
    }
    return rc;
...
bool ON_Mesh::CombineCoincidentVertices(
    ...
    double cos_normal_angle // = -1.0  // cosine(break angle) -1.0 will merge
...
static int CompareMeshPoint(const void *a, const void *b, void *ptr) {
    ...
    int i = (int)(((const char *)a) -
                  mp->p0); // the (int) is for 64 bit size_t conversion
...
bool ON_Mesh::CombineIdenticalVertices(bool bIgnoreVertexNormals,
    ...
    if (vertex_count > 0) {
        ...
        ON_Sort(ON::quick_sort, index,
                mp.p0, // data buffer
                vertex_count, sizeof(*mp.p0), CompareMeshPoint, &mp);
        ...
        if (remap_vertex_count > 0 && remap_vertex_count < vertex_count) {
            ...
            if (mesh.m_V.Capacity() > 4 * mesh.m_V.Count() &&
                mesh.m_V.Capacity() > 50) {
...
void ON_Mesh::Append(int mesh_count, const ON_Mesh *const *meshes) {
    ...
    // The calls to Has*() must happen before the m_V[] and m_F[] arrays get
    // enlarged Allow the appendage of VertexNormals, TextureCoordinates,
    // PrincipalCurvatures to empty meshes by checking for 0 == vcount0 && 0 ==
    // fcount0
    ...
    for (mi = 0; mi < mesh_count; mi++) {
        ...
        if (bHasDoubles &&
            !m->HasSynchronizedDoubleAndSinglePrecisionVertices())
            bHasDoubles = false;
    ...
    if (bHasDoubles) {
        ...
        if (0 == dv && 0 == m_V.Count()) {
            DoublePrecisionVertices(); // creates ON_MeshDoubleVertices info on
                                       // "this"
            dv = ON_MeshDoubleVertices::Get(this);
        }
        if (0 == dv)
...
static ON_MeshParameters FastRenderMeshParameters() {
    ...
    //{
    //  // 16 July, 2002 - copied V2 hard coded "jagged and faster" render mesh
    //  settings
    //  //
...
const ON_MeshParameters
    ON_MeshParameters::FastRenderMesh(FastRenderMeshParameters());
...
static int ON_MeshParameters_CompareDouble(double t0, double t1,
...
int ON_MeshParameters::CompareGeometrySettings(
    const ON_MeshParameters &src) const {
    int rc;
    ...
    rc = ON_MeshParameters_CompareDouble(m_relative_tolerance,
                                         src.m_relative_tolerance, 0.0);
    if (rc)
        return rc;
    ...
    rc = ON_MeshParameters_CompareDouble(m_min_edge_length,
                                         src.m_min_edge_length, 0.0);
    if (rc)
        return rc;

    rc = ON_MeshParameters_CompareDouble(m_max_edge_length,
                                         src.m_max_edge_length, 0.0);
    if (rc)
        return rc;

    rc = ON_MeshParameters_CompareDouble(m_grid_aspect_ratio,
                                         src.m_grid_aspect_ratio, 0.0);
    if (rc)
        return rc;
    ...
    rc = ON_MeshParameters_CompareDouble(m_refine_angle, src.m_refine_angle,
                                         0.0);
    if (rc)
        return rc;

    rc = ON_MeshParameters_CompareDouble(m_grid_amplification,
                                         src.m_grid_amplification, 1.0);
    if (rc)
        return rc;
...
ON__UINT32 ON_MeshParameters::DataCRC(ON__UINT32 current_remainder) const {
    ON__UINT32 crc = ON_CRC32(current_remainder, sizeof(m_bComputeCurvature),
                              &m_bComputeCurvature);
...
bool ON_MeshParameters::Write(ON_BinaryArchive &file) const {
    ...
    if (rc) {
        ...
        if (rc)
            rc = file.WriteDouble(5.0 * ON_PI /
                                  180.0); // obsolete m_combine_angle field
        int mft = m_face_type;
        if (mft < 0 || mft > 2) {
...
bool ON_MeshCurvatureStats::Set(
    ON::curvature_style kappa_style, int Kcount, const ON_SurfaceCurvature *K,
    ...
    if (rc) {
        ...
        for (i = 0; i < Kcount; i++) {
            ...
            //  if ( fabs(tangent.y) <= ON_SQRT_EPSILON && fabs(tangent.z) <=
            //  ON_SQRT_EPSILON )
            //    tangent.Zero();
            //  else
            //    tangent.Unitize();
            ...
            //  if ( fabs(tangent.x) <= ON_SQRT_EPSILON && fabs(tangent.z) <=
            //  ON_SQRT_EPSILON )
            //    tangent.Zero();
            //  else
            //    tangent.Unitize();
            ...
            //  if ( fabs(tangent.x) <= ON_SQRT_EPSILON && fabs(tangent.y) <=
            //  ON_SQRT_EPSILON )
            //    tangent.Zero();
            //  else
            //    tangent.Unitize();
...
bool ON_Mesh::ConvertTrianglesToQuads(double angle_tol_radians,
    ...
    struct EDGEINFO *EI =
        (struct EDGEINFO *)ws.GetMemory(edge_count * sizeof(*EI));
...
bool ON_Mesh::ConvertQuadsToTriangles() {
    ...
    if (fcount > 0 && QuadCount() > 0) {
        ...
        const ON_3dPoint *dV =
            (vcount > 0 && HasDoublePrecisionVertices() &&
             (HasSynchronizedDoubleAndSinglePrecisionVertices() ||
              DoublePrecisionVerticesAreValid()))
        ...
        const double rel_tol =
            8.0 * ((0 != dV) ? ON_EPSILON : ON_FLOAT_EPSILON);
            ...
            if (f0.IsValid(vcount) && f0.IsQuad()) {
                ...
                if (0 == idmin) // m_V[f0.vi[0]] == m_V[f0.vi[1]] (nearly)
                {
                    ...
                    f0.vi[2] = f0.vi[3];
                           ...
                           idmin) // m_V[f0.vi[1]] == m_V[f0.vi[2]] (nearly)
                {
                    // degenerate quad - remove duplicate vertex
                    int vi0 = f0.vi[0];
                    f0.vi[0] = f0.vi[2];
                    ...
                    f0.vi[3] = vi0;
                           ...
                           idmin) // m_V[f0.vi[2]] == m_V[f0.vi[3]] (nearly)
                {
                    // degenerate quad - remove duplicate vertex
                    f0.vi[2] = f0.vi[1];
                    f0.vi[1] = f0.vi[0];
                    ...
                    f0.vi[3] = f0.vi[2];
                           ...
                           idmin) // m_V[f0.vi[3]] == m_V[f0.vi[0]] (nearly)
                {
                    // degenerate quad - remove duplicate vertex
                    f0.vi[3] = f0.vi[2];
                } else {
    ...
    if (fcount > 0 && vcount > 0) {
        ...
        if (rc) {
            ...
            for (vi = 0; vi < vcount; vi++) {
                ...
                if (!n.Unitize()) {
                    // this vertex is not used by a face or the face normals
                    // cancel out. set a unit z normal and press on.
                    n.Set(0, 0, 1);
                }
                m_N.Append(n);
            }
...
bool ON_Mesh::TransposeTextureCoordinates() {
    ...
    bool bSrfParamTag =
        (!m_Ttag.IsSet() || m_Ttag.IsDefaultSurfaceParameterMapping());
...
bool ON_Mesh::ReverseTextureCoordinates(int dir) {
    ...
    if (bPackedRegion && bSrfParamTag) {
        ...
        double s;
        m_packed_tex_domain[dir]
            .Swap(); // Swap() is correct - Reverse() is wrong.
        for (i = 0; i < vcount; i++) {
            ON_2fPoint &tc = m_T[i];
    ...
    if (bHasSurfaceParameters) {
        ...
        if (HasPrincipalCurvatures()) {
            for (vi = 0; vi < vcount; vi++) {
                ...
                // convert texture coordinates to normalizied texture
                // coordinates
                srf_st = &m_S[vi];
                s = srf_st->x;
                t = srf_st->y;
                // 19 April 2006 Dale Lear - added side setter so singular
                // normals
                //                           are correctly evaluated RR 12482
                side =
                    (smax == s) ? ((tmax == t) ? 3 : 2) : ((tmax == t) ? 4 : 1);
                srf.Ev2Der(s, t, point, Ds, Dt, Dss, Dst, Dtt, side, hint);
                ON_EvNormal(side, Ds, Dt, Dss, Dst, Dtt, normal);
                ON_EvPrincipalCurvatures(Ds, Dt, Dss, Dst, Dtt, normal, &kgauss,
                                         &kmean, &m_K[vi].k1, &m_K[vi].k2, K1,
                                         K2); // m_K[vi].e1, m_K[vi].e2 );
                m_V[vi] =
                    &point.x; // use ON_3fPoint double* conversion (quiets gcc)
                if (bHasVertexNormals)
                    m_N[vi] = &normal.x; // use ON_3fVector double* conversion
                                         // (quiets gcc)
            }
            InvalidateCurvatureStats();
            ...
            for (vi = 0; vi < vcount; vi++) {
                ...
                m_N[vi] =
                    &normal
                         .x; // use ON_3fVector double* conversion (quiets gcc)
            }
        } else {
            for (vi = 0; vi < vcount; vi++) {
                // ON_2fPoint& tex = m_T[vi];
...
static int GetPointMap(int pt_count, const ON_3fPoint *fV, const ON_3dPoint *dV,
    ...
    if (pt_count > 0 && (dV || fV)) {
        ...
        if (dV)
            ON_Sort(ON::quick_sort, index, dV, pt_count, sizeof(*dV),
                    (ON_COMPAR_LPVOID_LPVOID)compare3dPoint);
        else
            ON_Sort(ON::quick_sort, index, fV, pt_count, sizeof(*fV),
        ...
        if (dV) {
            for (vt0 = 0; vt0 < pt_count; vt0 = vt1, max_pt_index++) {
                dp0 = dV[index[vt0]];
                for (vt1 = vt0 + 1; vt1 < pt_count &&
                                    0 == compare3dPoint(&dp0, dV + index[vt1]);
                     vt1++) {
                    // empty
                }
                while (vt0 < vt1) {
            ...
            for (vt0 = 0; vt0 < pt_count; vt0 = vt1, max_pt_index++) {
                fp0 = fV[index[vt0]];
                for (vt1 = vt0 + 1; vt1 < pt_count &&
                                    0 == compare3fPoint(&fp0, fV + index[vt1]);
                     vt1++) {
                    // empty
                }
                while (vt0 < vt1) {
...
int ON_Mesh::CullDegenerateFaces() {
    ...
    if (fcount > 0) {
        // use GetTopologyVertexMap() instead of calling Topology() because this
        // mesh may have lots of bogus faces that need to be culled.
...
//   ON_SurfaceCurvature
//
double ON_SurfaceCurvature::GaussianCurvature() const { return k1 * k2; }

double ON_SurfaceCurvature::MeanCurvature() const { return 0.5 * (k1 + k2); }
...
//  if ( fabs(tangent.x) <= ON_SQRT_EPSILON && fabs(tangent.y) <=
//  ON_SQRT_EPSILON && fabs(tangent.z) <= ON_SQRT_EPSILON )
//    tangent.Zero();
//  else
//    tangent.Unitize();
...
ON_MeshTopology::ON_MeshTopology()
    : m_mesh(0), m_memchunk(0), m_b32IsValid(0) {}
...
int ON_MeshTopology::TopVertexCount() const { return m_topv.Count(); }

//////////
// number of topoligical edges
int ON_MeshTopology::TopEdgeCount() const { return m_tope.Count(); }

//////////
// number of topoligical faces (same as m_mesh.FaceCount())
int ON_MeshTopology::TopFaceCount() const { return m_topf.Count(); }
...
int ON_MeshTopology::TopEdge(int vtopi0, int vtopi1) const {
    ...
    if (vtopi0 < vtopi1) {
        ...
        const ON_MeshTopologyEdge *tope =
            m_tope.Array(); // to speed array access
        i0 = 0;
        i1 = tope_count;
        while (i0 < i1) {
            ei = (i0 + i1) / 2;
...
static void ON_ReverseIntArray(int *e,    // array of ints
...
bool ON_MeshTopology::SortVertexEdges(int topvi) const {
    ...
    {
        int *e3f =
            e1f + topv.m_tope_count; // e3f[] = edges with 3 or more faces
    ...
    if (e1fcnt + e2fcnt != topv.m_tope_count) {
        ON_ERROR("ON_MeshTopology::SortVertexEdges() input vertex had bogus "
                 "m_topei[]");
        return false;
    }
    int fi = -1;
    ...
    while (vei < topv.m_tope_count) {
        if (next_topei >= 0) {
            ...
            ON_ERROR("ON_MeshTopology::SortVertexEdges() input vertex had "
                     "bogus topology information.");
            return false;
        }
        ...
        if (vei0 < vei1) {
            if (1 == elist_dir) {
                ...
                //   edges new_tope[vei0],...,new_tope[vei1-1] are radially
                //   sorted but the order is not counterclockwise with respect
                //   to the normal of the face attached to the first edge. So,
                //   this group of edges in new_tope[] needs to be reversed.
...
bool ON_MeshTopologyFace::IsTriangle() const {
    return (m_topei[2] == m_topei[3] && m_topei[0] != m_topei[1]) ? true
                                                                  : false;
}
    ...
    for (fi = 0; fi < f_count; fi++) {
        ...
        if (topf.m_topei[0] != 0 || topf.m_topei[1] != 0 ||
            ...
            for (j = 0; j < jmax; j++) {
                ...
                // topedge m_tope[topf.m_topei[j]] ENDS at topvtx
                // m_topv[tfvi[j]]
                if (topf.m_reve[j]) {
...
void ON_MeshTopology::Dump(ON_TextLog &dump) const {
    ...
    for (ei = 0; ei < tope_count; ei++) {
        ...
        dump.Print("tope %d: topv%d to topvv%d (", ei, e.m_topvi[0],
                   e.m_topvi[1]);
...
static int GetEidHelper(const int Vcount, const int Fcount,
                        const ON_MeshFace *F, const int *Vid,
...
int ON_Mesh::GetMeshFaceSideList(const int *Vid,
    ...
    if (0 == Elist) {
        Elist =
            (struct ON_MeshFaceSide *)onmalloc(4 * Fcount * sizeof(Elist[0]));
        if (0 == Elist)
            return 0;
    }
...
bool ON_MeshTopology::Create() {
    ...
    while (0 == b32IsValid || -1 == b32IsValid) {
        // while() is for flow control - this is a while() {... break;}
        // statment.
        Destroy();
        b32IsValid = 0;
...

        {
            ...
            for (vt0 = 0; vt0 < vcount; vt0 = vt1) {
                ...
                if (topvi != m_topv_map[vindex[vt0]]) {
                    ...
                    ON_ERROR("ON_MeshTopology::Create() - topvi != vertex id "
                             "from GetVertexLocationIds()");
                }
#endif
                ON_MeshTopologyVertex &topv = m_topv.AppendNew();
                topv.m_vi = vindex + vt0;
                for (vt1 = vt0 + 1;
                     vt1 < vcount && topvi == m_topv_map[vindex[vt1]]; vt1++) {
                    // empty
                }
        ...
        if (topv_count >= 2) {
            ...
            if (fcount > 0 && vcount > 0) {
                // When working on this code be sure to test bug# 9271 and 9254
                // and file fsv_r4.3dm
                ON_Workspace ws;
                struct ON_MeshFaceSide *e =
                    (struct ON_MeshFaceSide *)ws.GetMemory(4 * fcount *
                                                           sizeof(*e));
                ...
                if (ecnt > 0) {
                    ...
                    ecnt = m_tope.Count();
                    int *ve_count =
                        (int *)onmalloc(topv_count * sizeof(*ve_count));
                    // set ve_count[topvi] = number of edges that begin or end
                    // at m_topv[topvi]
                    ...
                    // allocate and distribute storage for the mopv.m_topei[]
                    // array
                    ...
                    memset(m_topf.Array(), 0,
                           fcount * sizeof(ON_MeshTopologyFace));
                    for (fi = 0; fi < fcount; fi++) {
                    ...
                    for (ei = 0; ei < ecnt; ei++) {
                        ...
                        for (efi = 0; efi < tope.m_topf_count; efi++) {
                            // Because ON_MeshFace.vi[2] == ON_MeshFace.vi[3]
                            // for triangles, we have topf.m_topei[j] BEGIN at
                            // ON_MeshFace.vi[(j+3)%4] and END at
                            // ON_MeshFace.vi[j]
                            fi = tope.m_topfi[efi];
                            f = m_mesh->m_F[fi];
                    ...
                    for (fi = 0; fi < fcount; fi++) {
                        ON_MeshTopologyFace &f = m_topf[fi];
                        bool bIsGood = false;
                        if (f.m_topei[0] >= 0 && f.m_topei[1] >= 0 &&
                            f.m_topei[2] >= 0 && f.m_topei[0] != f.m_topei[1] &&
...
static int AddToPartition(ON_Mesh *mesh, ON_SimpleArray<int> &pmark, int vi,
                          int partition_mark, int fi0) {
    bool b = true;
    ...
    if (!i) {
        ...
        if (i == partition_mark - 2)
            pmark[vi] =
                partition_mark - 1; // vertex vi shared between two partitions
        else {
...
bool ON_MeshPartition_IsValid(const ON_MeshPartition &p, const ON_Mesh &mesh) {
    ...
    if (partcount) {
        if (p.m_part[0].fi[0] != 0 ||
            p.m_part[partcount - 1].fi[1] != mesh.m_F.Count())
            rc = false;
    }
    return rc;
}
...
const ON_MeshPartition *ON_Mesh::CreatePartition(
    ...
    int partition_max_triangle_count // maximum number of triangles in a
                                     // partition
) {
    ON_Workspace ws;
    ...
    if (m_partition) {
        bNeedFaceSort = false;
        if (m_partition->m_partition_max_triangle_count >
                partition_max_triangle_count ||
            m_partition->m_partition_max_vertex_count >
                partition_max_vertex_count)
            DestroyPartition();
    }
    if (!m_partition) {
        // TODO: create one
        ...
        if (vertex_count <= partition_max_vertex_count &&
            ...
            //  ON_2fPoint fcenter;
            //  ON_BoundingBox bbox =
            //  ON_PointListBoundingBox(2,0,m_T.Count(),2,&m_T[0].x); const
            //  ON_Interval txdom(bbox.m_min.x,bbox.m_max.x); const ON_Interval
            //  tydom(bbox.m_min.y,bbox.m_max.y); for ( fi = 0; fi < face_count;
            //  fi++ ) {
            //    fvi = m_F[fi].vi;
            //    if ( fvi[2] == fvi[3] ) {
            ...
            //    }
            //    else {
            //      fcenter = 0.25f*(m_T[fvi[0]] + m_T[fvi[1]] + m_T[fvi[2]] +
            //      m_T[fvi[3]]);
            //    }
            //    fpt[fi].x =
            //    (int)floor(txdom.NormalizedParameterAt(fcenter.x)*100);
            //    fpt[fi].y =
            //    (int)floor(tydom.NormalizedParameterAt(fcenter.y)*100);
            //    fpt[fi].z = 0;
            //  }
            //}
            // else
            ...
            //    }
            //    else {
            //      fcenter = 0.25f*(m_V[fvi[0]] + m_V[fvi[1]] + m_V[fvi[2]] +
            //      m_V[fvi[3]]);
            //    }
            //    fpt[fi].x =
            //    (int)floor(vxdom.NormalizedParameterAt(fcenter.x)*100);
            //    fpt[fi].y =
            //    (int)floor(vydom.NormalizedParameterAt(fcenter.y)*100);
            //    fpt[fi].z =
            //    (int)floor(vzdom.NormalizedParameterAt(fcenter.z)*100);
            //  }
            //}
            ...
            pmark.Zero();
            int fi0, fi1, partition_mark, partition_vertex_count,
            ...
            for (partition_mark = 3, fi0 = 0; fi0 < face_count;
                ...
                for (fi1 = fi0;
                     fi1 < face_count &&
                     partition_triangle_count + 2 <=
                         partition_max_triangle_count &&
                     partition_vertex_count + 4 <= partition_max_vertex_count;
                     fi1++) {
                    fvi = m_F[fi1].vi;
                    partition_triangle_count++;
                    if (AddToPartition(this, pmark, fvi[0], partition_mark,
                                       fi0))
                        partition_vertex_count++;
                    if (AddToPartition(this, pmark, fvi[1], partition_mark,
                                       fi0))
                        partition_vertex_count++;
                    if (AddToPartition(this, pmark, fvi[2], partition_mark,
                                       fi0))
                        partition_vertex_count++;
                    if (fvi[2] != fvi[3]) {
                        partition_triangle_count++; // quads = 2 triangles
                        if (AddToPartition(this, pmark, fvi[3], partition_mark,
                                           fi0))
                            partition_vertex_count++;
                    }
                }
                ...
                if (partition_triangle_count >
                    m_partition->m_partition_max_triangle_count)
                    m_partition->m_partition_max_triangle_count =
                        partition_triangle_count;
                if (partition_vertex_count >
                    m_partition->m_partition_max_vertex_count)
                    m_partition->m_partition_max_vertex_count =
                        partition_vertex_count;
            }
            ...
            ON_Sort(ON::quick_sort, pamv, vmap, vertex_count, sizeof(vmap[0]),
                    compare_vmap);
            for (fi = 0; fi < face_count; fi++) {
                fvi = m_F[fi].vi;
                fvi[0] = pamv[fvi[0]];
            ...
            int vi0, vi1, vi2, vi3;
            for (vi2 = 0; vi2 < vertex_count && pmark[vi2] < 2;
                 vi2++) { /*empty for body*/
            }
            vi3 = vi2;
...
const ON_MeshPartition *ON_Mesh::Partition() const { return m_partition; }
...
ON_Mesh *ON_Mesh::MeshPart(const ON_MeshPart &mesh_part, ON_Mesh *mesh) const {
    if (this == mesh) {
    ...
    ON_Mesh *submesh =
        (0 != mesh)
            ? mesh
            : new ON_Mesh(mesh_part.triangle_count, mesh_part.vertex_count,
                          bHasVertexNormals, bHasTextureCoordinates);
...
ON_OBJECT_IMPLEMENT(ON_MeshVertexRef, ON_Geometry,
                    "C547B4BD-BDCD-49b6-A983-0C4A7F02E31A");

ON_OBJECT_IMPLEMENT(ON_MeshEdgeRef, ON_Geometry,
                    "ED727872-463A-4424-851F-9EC02CB0F155");

ON_OBJECT_IMPLEMENT(ON_MeshFaceRef, ON_Geometry,
    ...
    if (-1 != m_top_vi) {
        ...
        if (-1 != m_mesh_vi) {
            ...
            if (i >= topv.m_v_count) {
                if (0 != text_log) {
                    text_log->Print(
                        "m_mesh_vi=%d is not in "
                        "m_top->m_topv[m_top_vi=%d].m_vi[] array.\n",
                        m_mesh_vi, m_top_vi);
                }
                return false;
            }
...
int ON_MeshVertexRef::Dimension() const { return 3; }

ON_BOOL32 ON_MeshVertexRef::GetBBox(double *boxmin, double *boxmax,
                                    ON_BOOL32 bGrowBox) const {
    bool rc = false;
    ON_3dPoint v = Point();
    if (v.IsValid()) {
        rc = ON_GetPointListBoundingBox(3, 0, 1, 3, &v.x, boxmin, boxmax,
                                        bGrowBox ? true : false);
    }
    return rc;
}

ON_BOOL32 ON_MeshVertexRef::Transform(const ON_Xform &xform) { return false; }
...
ON_3dPoint ON_MeshVertexRef::Point() const {
    ...
    if (0 != m_mesh) {
        int vi = m_mesh_vi;
        if (-1 == vi && m_top_vi >= 0 &&
            m_top_vi < m_mesh->m_top.m_topv.Count()) {
...
// overrides of virtual ON_Geometry functions
int ON_MeshEdgeRef::Dimension() const { return 3; }

ON_BOOL32 ON_MeshEdgeRef::GetBBox(double *boxmin, double *boxmax,
                                  ON_BOOL32 bGrowBox) const {
    bool rc = false;
    ON_Line line = Line();
    if (line.from.IsValid() && line.to.IsValid()) {
        rc = ON_GetPointListBoundingBox(3, 0, 2, 3, &line.from.x, boxmin,
                                        boxmax, bGrowBox ? true : false);
    }
    return rc;
}

ON_BOOL32 ON_MeshEdgeRef::Transform(const ON_Xform &xform) { return false; }
...
// overrides of virtual ON_Geometry functions
int ON_MeshFaceRef::Dimension() const { return 3; }

ON_BOOL32 ON_MeshFaceRef::GetBBox(double *boxmin, double *boxmax,
                                  ON_BOOL32 bGrowBox) const {
    bool rc = false;
    if (0 != m_mesh && m_mesh_fi >= 0 && m_mesh_fi < m_mesh->m_F.Count()) {
        ...
        rc = ON_GetPointListBoundingBox(3, 0, 4, 3, &v[0].x, boxmin, boxmax,
                                        bGrowBox ? true : false);
    }
    return rc;
}

ON_BOOL32 ON_MeshFaceRef::Transform(const ON_Xform &xform) { return false; }
    ...
    if (ci.m_index >= 0) {
        ...
        case ON_COMPONENT_INDEX::meshtop_vertex:
            if (ci.m_index < m_topv.Count()) {
                ...
                if (m_topv[ci.m_index].m_vi &&
                    1 == m_topv[ci.m_index].m_v_count) {
...
static bool ON_ClosestPointToTriangleHelper(ON_3dPoint A, ON_3dPoint B,
                                            ON_3dPoint C, ON_3dPoint P,
    ...
    ON_3dVector N(V.y * W.z - W.y * V.z, V.z * W.x - W.z * V.x,
                  V.x * W.y - W.x * V.y);

    if (N.Unitize()) {
        s = (A.x - P.x) * N.x + (A.y - P.y) * N.y + (A.z - P.z) * N.z;
        ON_3dPoint Q(P.x + s * N.x - A.x, P.y + s * N.y - A.y,
                     P.z + s * N.z - A.z);

        if (2 == ON_Solve3x2(&V.x, &W.x, Q.x, Q.y, Q.z, &r, &s, &t, &pr)) {
            *a = 1.0 - r - s;
            *b = r;
            *c = s;
            rc = true;
...
bool ON_ClosestPointToTriangle(ON_3dPoint A, ON_3dPoint B, ON_3dPoint C,
                               ON_3dPoint P, double *a, double *b, double *c) {
    bool rc;
    int i;
    double x, y, z, d;
...
bool ON_ClosestPointToTriangleFast(const ON_3dPoint &R, const ON_3dPoint &S,
                                   const ON_3dPoint &T, ON_3dPoint Q, double *r,
                                   double *s, double *t) {
...
// bc[i] is count[i] doubles and will be filled in with barycentric coordinates
...

{
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < count[i]; j++) {
            ...
            for (k = 0; k < count[1 - i]; k++) {
                double t;
                if (L[i][j].ClosestPointTo(P[1 - i][k], &t) && t > 0.0 &&
                    t < 1.0) {
        ...
        if (count[i] == 4) {
            ...
            for (k = 0; k < count[1 - i]; k++) {
                double t;
                if (L[i][4].ClosestPointTo(P[1 - i][k], &t) && t > 0.0 &&
                    t < 1.0) {
    ...
    for (i = 0; i < count[0]; i++) {
        ...
        for (j = 0; j < count[1]; j++) {
            double s, t;
            if (ON_Intersect(L[0][i], L[1][j], &s, &t) && s > 0.0 && t > 0.0 &&
                s < 1.0 && t < 1.0) {
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < count[1 - i]; j++) {
            double a, b, c;
            if (ON_ClosestPointToTriangle(P[i][0], P[i][1], P[i][2],
                                          P[1 - i][j], &a, &b, &c) &&
                a > 0.0 && b > 0.0 && c > 0.0) {
            ...
            if (count[i] == 4) {
                if (ON_ClosestPointToTriangle(P[i][2], P[i][3], P[i][0],
                                              P[1 - i][j], &a, &b, &c) &&
                    a > 0.0 && b > 0.0 && c > 0.0) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        for (j = 0; j < count[1 - i]; j++) {
            ...
            double a, b, c;
            if (ON_ClosestPointToTriangle(P[i][0], P[i][1], P[i][2], Q, &a, &b,
                                          &c) &&
                a > 0.0 && b > 0.0 && c > 0.0) {
                int k;
                for (k = 0; k < count[1 - i]; k++)
        ...
        if (count[i] == 4) {
            ...
            for (j = 0; j < count[1 - i]; j++) {
                ...
                ON_3dPoint Q = L[1 - i][j].PointAt(t);
                double a, b, c;
                if (ON_ClosestPointToTriangle(P[i][2], P[i][3], P[i][0], Q, &a,
                                              &b, &c) &&
                    a > 0.0 && b > 0.0 && c > 0.0) {
                    int k;
                    for (k = 0; k < count[1 - i]; k++)
...
bool ON_ClosestPointBetweenTriangles(const ON_3dPoint A[3],
                                     const ON_3dPoint B[3], double a[3],
                                     double b[3])
...
bool ON_ClosestPointBetweenQuads(const ON_3dPoint A[4], const ON_3dPoint B[4],
...
ON_3dVector ON_TriangleNormal(const ON_3dPoint &A, const ON_3dPoint &B,
                              const ON_3dPoint &C) {
...
bool ON_GetTrianglePlaneEquation(const ON_3dPoint &A, const ON_3dPoint &B,
                                 const ON_3dPoint &C, double *a, double *b,
                                 double *c, double *d, double *evaluation_tol) {
...
int ON_LineTriangleIntersect(const ON_3dPoint &A, const ON_3dPoint &B,
                             const ON_3dPoint &C, const ON_3dPoint &P,
                             const ON_3dPoint &Q, double abc[2][3], double t[2],
                             double tol) {
    ON_3dPoint L, T;
    ...
    if (fabs(pdepth) > tol || fabs(qdepth) > tol) {
        ...
        L.y = p * P.y + q * Q.y;
        L.z = p * P.z + q * Q.z;
        if (!ON_ClosestPointToTriangleFast(A, B, C, L, &tri.x, &tri.y,
                                           &tri.z)) {
            return 0;
        }
        bool bCheck = false;
    ...
    if (!ON_ClosestPointToTriangleFast(A, B, C, P, &abc[0][0], &abc[0][1],
                                       &abc[0][2])) {
        return 0;
    }
    if (!ON_ClosestPointToTriangleFast(A, B, C, Q, &abc[1][0], &abc[1][1],
                                       &abc[1][2])) {
        return 0;
    }
...
const bool *ON_Mesh::HiddenVertexArray() const {
    return ((m_hidden_count > 0 && m_H.Count() == m_V.Count()) ? m_H.Array()
                                                               : 0);
}
...
bool ON_Mesh::VertexIsHidden(int meshvi) const {
    const int vcount = m_V.Count();
    return ((m_hidden_count > 0 && meshvi >= 0 && meshvi < vcount &&
             vcount == m_H.Count())
                ? m_H[meshvi]
                : false);
}
...
bool ON_Mesh::FaceIsHidden(int meshfi) const {
    ...
    if (bHiddenVertex && meshfi >= 0 && meshfi < m_F.Count()) {
        ...
        if (bHiddenVertex[f.vi[0]] || bHiddenVertex[f.vi[1]] ||
            bHiddenVertex[f.vi[2]] || bHiddenVertex[f.vi[3]])
            return true;
    }
    return false;
}
...
int ON_MappingTag::Compare(const ON_MappingTag &other, bool bCompareId,
                           bool bCompareCRC, bool bCompareXform) const {
    int rc = 0;
    if (!rc && bCompareId) {
...
bool ON_MappingTag::Read(ON_BinaryArchive &archive) {
    ...
    for (;;) {
        ...
        if (!rc)
            break;
        if (0 ==
            ON_UuidCompare(&obsolete_default_srfp_mapping_id, &m_mapping_id))
            m_mapping_id = ON_nil_uuid;
...
ON_OBJECT_IMPLEMENT(ON_MeshDoubleVertices, ON_UserData,
...
ON_BOOL32 ON_MeshDoubleVertices::IsValid(ON_TextLog *) const { return true; }
...
ON__UINT32 ON_MeshDoubleVertices::DataCRC(ON__UINT32 current_remainder) const {
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_fcount), &m_fcount);
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_dcount), &m_dcount);
...
void ON_Mesh::EnableDoublePrecisionVertices(
...
ON__UINT32 ON_MeshDoubleVertices::DoubleCRC() const { return m_dV.DataCRC(0); }
...
ON_3dPointArray &ON_Mesh::DoublePrecisionVertices() {
    ...
    if (0 == dv) {
        dv = ON_MeshDoubleVertices::Attach(this);
        bUpdate = true;
    } else if (dv->m_dcount != dv->m_dV.Count() ||
               dv->m_dCRC != dv->DoubleCRC()) {
...
#error Never copy this class definition or put this definition in a header file!
  ...
  public:
    static ON_PerObjectMeshParameters *FindOrCreate(const ON_Object *,
                                                    bool bCreate);
...
ON_OBJECT_IMPLEMENT(ON_PerObjectMeshParameters, ON_UserData,
                    "B5628CA9-82C4-4CAE-9883-487B3E4AB28B");

ON_PerObjectMeshParameters *
ON_PerObjectMeshParameters::FindOrCreate(const ON_Object *object,
                                         bool bCreate) {
    if (0 == object)
        return 0;
    ON_PerObjectMeshParameters *ud =
        ON_PerObjectMeshParameters::Cast(object->GetUserData(
            ON_PerObjectMeshParameters::m_ON_PerObjectMeshParameters_class_id
                .Uuid()));
    if (!ud && bCreate) {
...
ON_BOOL32 ON_PerObjectMeshParameters::Read(ON_BinaryArchive &binary_archive) {
    ...
    int major_version = 0;
    int minor_version = 0;
    if (!binary_archive.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &major_version,
                                          &minor_version))
        return false;
...
// virtual ON_UserData override
ON_BOOL32 ON_PerObjectMeshParameters::Archive() const { return true; }
...
bool ON_3dmObjectAttributes::SetCustomRenderMeshParameters(
