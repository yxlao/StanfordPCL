...
bool ON_Arc::Create(const ON_2dPoint &P,     // [IN] start point
...
bool ON_Arc::Create(const ON_3dPoint &P,     // [IN] start point
...
ON_3dPoint ON_Arc::StartPoint() const { return PointAt(m_angle[0]); }

ON_3dPoint ON_Arc::MidPoint() const { return PointAt(m_angle.Mid()); }

ON_3dPoint ON_Arc::EndPoint() const { return PointAt(m_angle[1]); }
...
bool ON_Arc::GetBoundingBox(ON_BoundingBox &bbox, int bGrowBox) const {
    if (bGrowBox) {
...
double ON_Arc::AngleRadians() const { return m_angle[1] - m_angle[0]; }

double ON_Arc::AngleDegrees() const { return (AngleRadians() / ON_PI) * 180.0; }

ON_Interval ON_Arc::Domain() const { return m_angle; }

ON_Interval ON_Arc::DomainRadians() const { return m_angle; }
...
bool ON_ArcCurve::IsContinuous(
    ON::continuity c, double t,
    int *,  // hint                - formal parameter intentionally ignored in
            // this virtual function
    double, // point_tolerance     - formal parameter intentionally ignored in
            // this virtual function
    double, // d1_tolerance        - formal parameter intentionally ignored in
            // this virtual function
    double, // d2_tolerance        - formal parameter intentionally ignored in
            // this virtual function
    double, // cos_angle_tolerance - formal parameter intentionally ignored in
            // this virtual function
    double  // curvature_tolerance - formal parameter intentionally ignored in
            // this virtual function
) const {
...
double ON_Arc::Length() const { return fabs(AngleRadians() * radius); }
...
bool ON_Arc::ClosestPointTo(const ON_3dPoint &pt, double *t) const {
    /*
    double tt, a;
    if ( !t )
      t =&tt;
    ...
    */
    ...
    if (rc) {
        ...
        // Greg Arden April 14 2003. Changed test from ">" to ">=" this ensures
        // that closest point to a circle at the seam will return the least
        // parameter value.
        while (s >= twopi)
            s -= twopi;
...
ON_3dPoint ON_Arc::ClosestPointTo(const ON_3dPoint &pt) const {
    double t = m_angle[0];
    ClosestPointTo(pt, &t);
    return PointAt(t);
}
