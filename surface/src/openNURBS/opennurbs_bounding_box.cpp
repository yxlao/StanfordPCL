...
ON_BoundingBox::ON_BoundingBox()
    : m_min(1.0, 0.0, 0.0), m_max(-1.0, 0.0, 0.0) {}
...
ON_3dPoint ON_BoundingBox::Min() const { return m_min; }

ON_3dPoint ON_BoundingBox::Max() const { return m_max; }

ON_3dVector ON_BoundingBox::Diagonal() const { return m_max - m_min; }

ON_3dPoint ON_BoundingBox::Center() const { return 0.5 * (m_max + m_min); }
...
int ON_ClippingRegion::InViewFrustum(int count, const ON_3fPoint *p) const {
    const double *xform;
    const float *cv;
    double x, w;
...
int ON_ClippingRegion::InViewFrustum(int count, const ON_3dPoint *p) const {
    const double *xform;
    const double *cv;
    double x, w;
    unsigned int out, all_out, some_out;
...
int ON_ClippingRegion::InViewFrustum(int count, const ON_4dPoint *p) const {
    const double *xform;
    const double *cv;
    double x, w;
    unsigned int out, all_out, some_out;
    ...
    for (i = count; i--; cv += 4) {
        ...
        w = xform[12] * cv[0] + xform[13] * cv[1] + xform[14] * cv[2] +
            xform[15] * cv[3];
        x = xform[0] * cv[0] + xform[1] * cv[1] + xform[2] * cv[2] +
            xform[3] * cv[3];
        if (x < -w)
            out = 0x01;
        else if (x > w)
            out = 0x02;
        x = xform[4] * cv[0] + xform[5] * cv[1] + xform[6] * cv[2] +
            xform[7] * cv[3];
        if (x < -w)
            out |= 0x04;
        else if (x > w)
            out |= 0x08;
        x = xform[8] * cv[0] + xform[9] * cv[1] + xform[10] * cv[2] +
            xform[11] * cv[3];
        if (x < -w)
            out |= 0x10;
        else if (x > w)
...
int ON_ClippingRegion::InClipPlaneRegion(int count, const ON_3fPoint *p) const {
    ...
    for (i = count; i--; cv += 3) {
        ...
        // if ( m_clip_plane_count )
        {
            ...
            while (j--) {
                x = cpeqn->x * cv[0] + cpeqn->y * cv[1] + cpeqn->z * cv[2] +
                    cpeqn->d;
                if (x < 0.0)
                    out |= cpbit;
                cpbit <<= 1;
...
int ON_ClippingRegion::InClipPlaneRegion(int count, const ON_3dPoint *p) const {
    const ON_PlaneEquation *cpeqn;
    const double *cv;
    double x;
    unsigned int out, all_out, some_out, cpbit;
...
int ON_ClippingRegion::InClipPlaneRegion(int count, const ON_4dPoint *p) const {
    const ON_PlaneEquation *cpeqn;
    const double *cv;
    double x;
    unsigned int out, all_out, some_out, cpbit;
...
unsigned int ON_ClippingRegion::TransformPoint(const ON_4dPoint &P,
    ...
    if (x < -w)
        ...
        out |= 0x02;
    y = xform[4] * cv[0] + xform[5] * cv[1] + xform[6] * cv[2] +
        xform[7] * cv[3];
    if (y < -w)
        out |= 0x04;
    else if (y > w)
        out |= 0x08;
    z = xform[8] * cv[0] + xform[9] * cv[1] + xform[10] * cv[2] +
        xform[11] * cv[3];
    if (z < -w)
        out |= 0x10;
    else if (z > w)
...
int ON_ClippingRegion::TransformPoints(int count, ON_4dPoint *p,
...
bool ON_ClippingRegion::GetLineClipPlaneParamters(ON_4dPoint P0, ON_4dPoint P1,
                                                  double *t0,
                                                  double *t1) const {
...
int ON_BoundingBox::IsVisible(const ON_Xform &bbox2c) const {
    ...
    some_out =
        0; // will be != 0 if some portion of box is outside visible region
    all_out = 0xFFFFFFFF; // will be == 0 if some portion of box is inside
                          // visible region
    ...
    while (i--) {
        ...
        while (j--) {
            ...
            while (k--) {
                ...
                all_out &= out;
                if (some_out && !all_out) {
                    // box intersects visble region but is not completely inside
                    // it.
                    return 1;
                }
                bz = m_max.z;
...
bool ON_BoundingBox::IsPointIn(const ON_3dPoint &p, int bStrictlyIn) const {
    bool bIn = false;
    if (bStrictlyIn) {
        bIn = m_min.x < p.x && p.x < m_max.x && m_min.y < p.y &&
              p.y < m_max.y && m_min.z < p.z && p.z < m_max.z;
    } else {
        bIn = m_min.x <= p.x && p.x <= m_max.x && m_min.y <= p.y &&
              p.y <= m_max.y && m_min.z <= p.z && p.z <= m_max.z;
    }
...
int ON_BoundingBox::GetClosestPoint(const ON_Line &line, ON_3dPoint &box_point,
    ...
    //   in the plane that doesn't intersect the 2d-box in the plane.  The
    //   projection
    //   of the 3d closest point is the closest point of this 2d closest point
    //   problem.
    int k[3];
    for (i = 0; i < 3; i++) {
        ...
        if (!overlap.Intersection(over[(i + 1) % 3], over[(i + 2) % 3])) {
            ...
            // Solve for 2d-closest point between closest corner and projected
            // line
...
bool Intersect(ON_Interval A, ON_Interval B, ON_Interval &AB) {
    ...
    } else if (B.m_t[0] <= A.m_t[0] && A.m_t[0] <= B.m_t[1] &&
               B.m_t[1] <= A.m_t[1]) {
        AB.Set(A.m_t[0], B.m_t[1]);
    } else if (A.m_t[0] <= B.m_t[0] && B.m_t[0] <= B.m_t[1] &&
               B.m_t[1] <= A.m_t[1]) {
...
bool ON_BoundingBox::GetClosestPoint(
    ...
    ON_3dPoint
        &this_point, // point on "this" box that is closest to "other" box
    ON_3dPoint
        &other_point // point on "other" box that is closest to "this" box
) const {
    ON_BoundingBox b;
...
bool ON_BoundingBox::GetFarPoint(
    const ON_BoundingBox &other_box, // "other" bounding box
    ON_3dPoint &this_point, // point on "this" box that is farthest from "other"
                            // box point
    ON_3dPoint &other_point // point on "other" box that is farthest from "this"
                            // box point
) const {
    if (!IsValid() || !other_box.IsValid())
        return false;
...
bool ON_BoundingBox::Intersection(const ON_BoundingBox &a) {
...
bool ON_BoundingBox::Union(const ON_BoundingBox &a, const ON_BoundingBox &b) {
    if (a.IsValid()) {
        if (b.IsValid()) {
            m_min.x = (a.m_min.x <= b.m_min.x) ? a.m_min.x : b.m_min.x;
            m_min.y = (a.m_min.y <= b.m_min.y) ? a.m_min.y : b.m_min.y;
...
bool ON_BoundingBox::Set(int dim, int is_rat, int count, int stride,
                         const double *points, int bGrowBox) {
...
bool ON_BoundingBox::Set(const ON_SimpleArray<ON_4dPoint> &a, int bGrowBox) {
    ...
    return ON_GetPointListBoundingBox(3, 1, count, 4, p, *this, bGrowBox != 0,
                                      0);
}
...
bool ON_BoundingBox::Set(const ON_SimpleArray<ON_3dPoint> &a, int bGrowBox) {
    const int count = a.Count();
    const double *p = (count > 0) ? &a.Array()->x : 0;
    return ON_GetPointListBoundingBox(3, 0, count, 3, p, *this, bGrowBox != 0,
                                      0);
}
...
bool ON_BoundingBox::Set(const ON_SimpleArray<ON_2dPoint> &a, int bGrowBox) {
    const int count = a.Count();
    const double *p = (count > 0) ? &a.Array()->x : 0;
    return ON_GetPointListBoundingBox(2, 0, count, 2, p, *this, bGrowBox != 0,
                                      0);
}

ON_BoundingBox ON_PointListBoundingBox(int dim, int is_rat, int count,
                                       int stride, const double *points) {
    ON_BoundingBox bbox;
    ON_GetPointListBoundingBox(dim, is_rat, count, stride, points, bbox, false,
                               0);
    return bbox;
}

bool ON_GetPointListBoundingBox(int dim, int is_rat, int count, int stride,
                                const double *points,
                                ON_BoundingBox &tight_bbox, int bGrowBox,
    ...
    if (count > 0 && dim > 0 && points &&
        (count == 1 || stride >= dim + is_rat)) {
        ON_BoundingBox bbox;
...
bool ON_GetPointListBoundingBox(int dim, int is_rat, int count, int stride,
                                const double *points, double *boxmin,
...
bool ON_GetPointListBoundingBox(int dim, int is_rat, int count, int stride,
                                const float *points, float *boxmin,
                                float *boxmax, int bGrowBox)
/*****************************************************************************
Bounding Box of a set of points
...
ON_BoundingBox ON_PointGridBoundingBox(int dim, ON_BOOL32 is_rat,
                                       int point_count0, int point_count1,
                                       int point_stride0, int point_stride1,
                                       const double *p) {
    ON_BoundingBox bbox;
    if (dim > 3) {
        // strides control stepping - no need to waste time on coordinates we
        // don't return
        dim = 3;
    }
    ON_GetPointGridBoundingBox(dim, is_rat, point_count0, point_count1,
                               point_stride0, point_stride1, p, &bbox.m_min.x,
                               &bbox.m_max.x, false);
    return bbox;
}

bool ON_GetPointGridBoundingBox(int dim, int is_rat, int point_count0,
                                int point_count1, int point_stride0,
                                int point_stride1, const double *p,
    ...
    for (i = 0; i < point_count0; i++) {
        if (!ON_GetPointListBoundingBox(dim, is_rat, point_count1,
                                        point_stride1, p + i * point_stride0,
...
bool ON_BeyondSinglePrecision(const ON_BoundingBox &bbox, ON_Xform *xform) {
    ...
    if (bbox.IsValid()) {
        ...
        bool bTooFar = (bbox.m_min.x >= too_far || bbox.m_min.y >= too_far ||
                        bbox.m_min.z >= too_far || bbox.m_max.x <= -too_far ||
                        bbox.m_max.y <= -too_far || bbox.m_max.z <= -too_far);
        bool bTooBig = (bbox.m_min.x <= -too_big || bbox.m_min.y <= -too_big ||
                        bbox.m_min.z <= -too_big || bbox.m_max.x >= too_big ||
                        bbox.m_max.y >= too_big || bbox.m_max.z >= too_big);
        if (bTooFar || bTooBig) {
            rc = true;
            if (0 != xform) {
...
double ON_BoundingBoxTolerance(int dim, const double *bboxmin,
...
#pragma warning(disable : 4127)
    ...
    ON_ASSERT_OR_RETURN(dim > 0 && bboxmin != NULL && bboxmax != NULL,
                        tolerance);
    for (i = 0; i < dim; i++) {
...
static double ON_BBoxMinimumDistanceToHelper(const ON_BoundingBox &bbox,
    ...
    //   returns d > 0.0 if the line misses the box and the minimum dist is >=
    //   d. returns ON_UNSET_VALUE if the line misses the box but the minimum
    //   distance is not easily bounded away from zero.
    ...
    if (bbox.m_min.x <= line_bbox.m_min.x &&
        line_bbox.m_max.x <= bbox.m_max.x) {
        if (bbox.m_min.y <= line_bbox.m_min.y &&
            line_bbox.m_max.y <= bbox.m_max.y) {
            // The fact that MinimumDistanceTo(line_bbox) == 0.0 implies
            // that the z-extents of the line intersects this bounding box.
            return 0.0;
        } else if (bbox.m_min.z <= line_bbox.m_min.z &&
                   line_bbox.m_max.z <= bbox.m_max.z) {
            // The fact that MinimumDistanceTo(line_bbox) == 0.0 implies
            // that the y-extents of the line intersects this bounding box.
            return 0.0;
        }
    } else if (bbox.m_min.y <= line_bbox.m_min.y &&
               line_bbox.m_max.y <= bbox.m_max.y &&
               bbox.m_min.z <= line_bbox.m_min.z &&
               line_bbox.m_max.z <= bbox.m_max.z) {
        // The fact that MinimumDistanceTo(line_bbox) == 0.0 implies
