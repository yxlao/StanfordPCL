...
double
ON_PlaneEquation::MinimumValueAt(const ON_SurfaceLeafBox &srfleafbox) const {
    // The working function is part of the Rhino SDK.
    return 0.0;
}

double
ON_PlaneEquation::MaximumValueAt(const ON_SurfaceLeafBox &srfleafbox) const {
    // The working function is part of the Rhino SDK.
    return 0.0;
}

double ON_PlaneEquation::MinimumValueAt(
    const class ON_CurveLeafBox &crvleafbox) const {
    // The working function is part of the Rhino SDK.
    return 0.0;
}
...
bool ON_Curve::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
    ...
    for (int span_index = 0; span_index <= N.m_cv_count - N.m_order;
         span_index++) {
        if (!(N.m_knot[span_index + N.m_order - 2] <
...
ON_PolylineCurve *ON_Curve::MeshCurve(ON_MeshCurveParameters &,
                                      ON_PolylineCurve *, bool,
                                      const ON_Interval *) {
    // The working function is part of the Rhino SDK.
    return 0;
}
