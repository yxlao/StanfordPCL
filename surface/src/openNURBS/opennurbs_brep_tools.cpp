...
static const ON_BrepEdge *FindLinearEdge(const ON_Brep &brep, int vi0,
                                         int vi1) {
...
ON_BrepFace *ON_Brep::NewRuledFace(const ON_BrepEdge &edgeA, bool bRevEdgeA,
...
bool ON_BrepFace::SetMesh(ON::mesh_type mt, ON_Mesh *mesh) {
    // TODO move next to ON_BrepFace::Mesh() when opennurbs_brep.cpp is
    // available
    bool rc = true;
    switch (mt) {
    case ON::render_mesh:
...
void ON_Brep::SetTolerancesBoxesAndFlags(
    ON_BOOL32 bLazy, ON_BOOL32 bSetVertexTolerances,
    ON_BOOL32 bSetEdgeTolerances, ON_BOOL32 bSetTrimTolerances,
    ON_BOOL32 bSetTrimIsoFlags, ON_BOOL32 bSetTrimTypeFlags,
    ON_BOOL32 bSetLoopTypeFlags, ON_BOOL32 bSetTrimBoxes) {
    int ei, ti, li;
...
ON_BrepFace *ON_Brep::NewFace(ON_Surface *pSurface, int vid[4], int eid[4],
                              ON_BOOL32 bRev3d[4]) {
    m_bbox.Destroy();
    m_is_solid = 0;
...
ON_BrepLoop *ON_Brep::NewOuterLoop(int face_index, int vid[4], int eid[4],
                                   ON_BOOL32 bRev3d[4]) {
    m_is_solid = 0;
    ...
    for (i = 0; i < 4; i++) {
        if (bRev3d[i])
            bRev3d[i] =
                1; // do this so we can use 1-bRev3d[i] as an array index
    }
    ...
    for (i = 0; i < 4; i++) {
        if (eid[i] != -1) {
            ...
            if (vi0 < 0 || vi1 < 0) {
                ON_ERROR("ON_Brep::NewFace(ON_Surface*,...) error: Bad edge "
                         "vertex informtion.");
                return 0;
            }
            if (vid[i] == -1)
                ...
                ON_ERROR("ON_Brep::NewFace(ON_Surface*,...) error: Edge and "
                         "vertex informtion do not match.");
                return 0;
            }
            if (vid[(i + 1) % 4] == -1)
                vid[(i + 1) % 4] = vi1;
        ...
        if (bIsClosed[i]) {
            ...
            if (eid[j] == -1 && eid[k] != -1) {
                ...
                if (eid[j] != eid[k] || bRev3d[j] != 1 - bRev3d[k]) {
                    ON_ERROR(
                        "Bad edge information passed to ON_Brep::NewFace.");
                    return 0;
                }
            }
        }
    ...
    // if surface has singularities or is closed, make sure vertex and edge
    ...
    for (i = 0; i < 4; i++) {
        if (c3[i]) {
            ...
            ON_BrepEdge &edge =
                NewEdge(m_V[vid[i0]], m_V[vid[i1]], AddEdgeCurve(c3[i]));
    ...
    ON_Surface::ISO srf_iso[4] = {ON_Surface::S_iso, ON_Surface::E_iso,
    ...
    if (box_corners) {
        ...
        */
        ...
        for (fi = 0; fi < 6; fi++) {
            ...
            si = brep->AddSurface(ON_NurbsSurfaceQuadrilateral(
            ...
            trim0.m_type = (trim0.m_vi[0] != trim0.m_vi[1])
                               ? ON_BrepTrim::mated
            ...
            trim1.m_type = (trim1.m_vi[0] != trim1.m_vi[1])
                               ? ON_BrepTrim::mated
                               : ON_BrepTrim::singular;
            ...
            trim2.m_type = (trim2.m_vi[0] != trim2.m_vi[1])
                               ? ON_BrepTrim::mated
                               : ON_BrepTrim::singular;
            ...
            trim3.m_type = (trim3.m_vi[0] != trim3.m_vi[1])
                               ? ON_BrepTrim::mated
                               : ON_BrepTrim::singular;
    ...
    if (corners) {
        ...
        */
        ...
        for (fi = 0; fi < 5; fi++) {
            ...
            if (f[fi].e[3] >= 0)
                // 4 sided face
                srf = ON_NurbsSurfaceQuadrilateral(v0->point, v1->point,
                                                   v2->point, v3->point);
            else
                // 3 sided face
                srf = ON_NurbsSurfaceQuadrilateral(
                    v0->point, v1->point, v1->point + (v2->point - v0->point),
                    v2->point);
            ...
            if (f[fi].e[3] >= 0) {
                // south side of surface
                c2i = brep->AddTrimCurve(new ON_LineCurve(p0, p1));
                brep->NewTrim(*e0, f[fi].bRev[0], loop, c2i).m_iso =
                    ON_Surface::S_iso;

                // east side of surface
                c2i = brep->AddTrimCurve(new ON_LineCurve(p1, p2));
                brep->NewTrim(*e1, f[fi].bRev[1], loop, c2i).m_iso =
                    ON_Surface::E_iso;

                // north side of surface
                c2i = brep->AddTrimCurve(new ON_LineCurve(p2, p3));
                brep->NewTrim(*e2, f[fi].bRev[2], loop, c2i).m_iso =
                    ON_Surface::N_iso;

                // west side of surface
                c2i = brep->AddTrimCurve(new ON_LineCurve(p3, p0));
                brep->NewTrim(*e3, f[fi].bRev[3], loop, c2i).m_iso =
                    ON_Surface::W_iso;
            } else {
                // south side of surface
                c2i = brep->AddTrimCurve(new ON_LineCurve(p0, p1));
...
ON_Brep *ON_BrepCylinder(const ON_Cylinder &cylinder, ON_BOOL32 bCapBottom,
                         ON_BOOL32 bCapTop, ON_Brep *pBrep) {
    ON_BOOL32 bArcLengthParameterization = true;
    ON_Brep *brep = NULL;
    if (pBrep)
        pBrep->Destroy();
...
ON_Brep *ON_BrepRevSurface(ON_RevSurface *&pRevSurface, ON_BOOL32 bCapStart,
                           ON_BOOL32 bCapEnd, ON_Brep *pBrep) {
    ON_Brep *brep = 0;
    if (pBrep)
        pBrep->Destroy();
...
static ON_BOOL32 AddC3Curve(const ON_Curve *c3,
    ...
    if (ON_PolyCurve::Cast(c3)) {
        ...
        const ON_PolylineCurve *pline =
            static_cast<const ON_PolylineCurve *>(c3);
...
bool ON_Brep::NewPlanarFaceLoop(int face_index, ON_BrepLoop::TYPE loop_type,
    ...
    if (face_index < 0 || face_index >= m_F.Count() || boundary.Count() < 1)
        return false;
    ON_BrepFace &face = m_F[face_index];
    const ON_PlaneSurface *pPlaneSurface =
        ON_PlaneSurface::Cast(face.SurfaceOf());
    if (!pPlaneSurface)
        return false;
    ...
    if (i < count) {
        ON_ERROR(
            "ON_Brep::NewPlanarFaceLoop - null 3d curve in boundary[] array");
        if (bDuplicateCurves) {
    ...
    for (i = 0; i < count; i++) {
        ...
        ON_BrepEdge &edge =
            NewEdge(m_V[vi0 + i], m_V[vi0 + ((i + 1) % count)], c3i);
...
ON_Brep *ON_BrepFromMesh(const ON_MeshTopology &mesh_topology,
                         ON_BOOL32 bTrimmedTriangles, ON_Brep *pBrep) {
        ...
        for (fi = 0; fi < face_count; fi++) {
            ...
            if (bTriangle) {
                if (bTrimmedTriangles) {
                    // trimmed triangle
                    srf_3d_corner[3] =
                        srf_3d_corner[2] - srf_3d_corner[1] + srf_3d_corner[0];
                } else {
                    // singular triangle
            ...
            ON_Surface *pSurface = ON_NurbsSurfaceQuadrilateral(
                srf_3d_corner[0], srf_3d_corner[1], srf_3d_corner[2],
                srf_3d_corner[3]);
            ...
            if (bTriangle && bTrimmedTriangles) {
                ...
                for (lti = 0; lti < 3; lti++) {
                    ...
                    const ON_MeshTopologyEdge &mesh_edge =
                        mesh_topology.m_tope[edge_index];
                    ...
                    ON_BrepTrim &trim = brep->NewTrim(
                        brep_edge, mesh_face.m_reve[fei] ? true : false, loop,
                        c2i);
                    trim.m__legacy_2d_tol = 0.0;
                    trim.m__legacy_3d_tol = 0.0;
                    trim.m_tolerance[0] = 0.0;
                    ...
                    trim.m_type = (mesh_edge.m_topf_count > 1)
                                      ? ON_BrepTrim::mated
                ...
                for (lti = 0; lti < 4; lti++) {
                    ...
                    if (bTriangle && lti == 3) {
                        ...
                        brep->NewSingularTrim(brep->m_V[fvi[0]], loop,
                                              quad_iso[lti], c2i);
                    } else {
                        fei = (lti + 1) % 4;
                        edge_index = mesh_face.m_topei[fei];
                        ...
                        const ON_MeshTopologyEdge &mesh_edge =
                            mesh_topology.m_tope[edge_index];
                        ON_BrepTrim &trim = brep->NewTrim(
                            brep_edge, mesh_face.m_reve[fei] ? true : false,
                            loop, c2i);
                        trim.m__legacy_2d_tol = 0.0;
                        trim.m__legacy_3d_tol = 0.0;
                        trim.m_tolerance[0] = 0.0;
...
bool ON_Brep::ChangeVertex(int old_vi, int new_vi, bool bClearTolerances) {
    ...
    // clear type bits
    old_vi =
        (int)(old_v - m_V.Array()); // the (int) is for 64 bit size_t conversion
    ...
    for (vei = 0; vei < old_v->m_ei.Count(); vei++) {
        ...
        if (0 == edge)
            continue;
        // edges that start/end at the same vertex are listed twice in
        // old_v->m_ei[].
...
ON_BOOL32 ON_BrepEdge::SetStartPoint(ON_3dPoint start_point) { return false; }

ON_BOOL32 ON_BrepEdge::SetEndPoint(ON_3dPoint end_point) { return false; }
...
ON_BOOL32 ON_BrepTrim::SetEndPoint(ON_3dPoint end_point) { return false; }
...
bool ON_Brep::CloseTrimGap(ON_BrepTrim &trim0, ON_BrepTrim &trim1) {
    ...
    case ON_Surface::E_iso:
        ...
        switch (coord0_lock) {
        case 0:
            if (ON_Surface::x_iso == trim0.m_iso &&
                ON_Surface::x_iso != trim1.m_iso)
                p.x = p1.x; // trim1 is on surface edge
            else if (ON_Surface::x_iso != trim0.m_iso &&
                     ON_Surface::x_iso == trim1.m_iso)
                p.x = p0.x; // trim0 is on surface edge
            else {
                // longest one wins
                if (p0.DistanceTo(trim0.PointAtStart()) >=
                    p1.DistanceTo(trim1.PointAtEnd()))
                    p.x = p0.x;
                else
                    p.x = p1.x;
            }
...
bool ON_Brep::CollapseEdge(int edge_index, bool bCloseTrimGap,
                           int vertex_index) {
...
bool ON_Brep::RemoveNesting(bool bExtractSingleSegments, bool bEdges,
                            bool bTrimCurves) {
    bool rc = false;
    // TODO
    int i, count;
        ...
        for (i = 0; i < count; i++) {
            ...
            if (0 != polycurve) {
                ...
                if (bExtractSingleSegments && 1 == polycurve->Count()) {
                    // TODO - extract segment and update edge's proxy
                    // information
                }
            }
        }
        ...
        for (i = 0; i < count; i++) {
            ...
            if (0 != polycurve) {
                if (polycurve->RemoveNestingEx())
                    rc = true;
                if (bExtractSingleSegments && 1 == polycurve->Count()) {
                    // TODO - extract segment and update trims's proxy
                    // information
                }
            }
        }
    ...
    while (b < tcount) {
        ...
        while (c < tcount) {
            ...
            if (bIsSlit[next_trim]) {
                ...
                int atid =
                    (pE->m_ti[0] == T.m_trim_index) ? pE->m_ti[1] : pE->m_ti[0];
                next_trim = -1;
                for (i = 0; i < tcount; i++) {
                    if (L.m_ti[i] == atid) {
                        next_trim = i;
...
bool ON_BrepRemoveSlits(ON_BrepFace &F) {
    ...
    // After all loops have been split, call ON_Brep::RemoveSlits() on the
    // result.
...

{
    ...
    // Find a manifold edge that joins the two faces and combine the loops by
    // removing the trims at that edge.
