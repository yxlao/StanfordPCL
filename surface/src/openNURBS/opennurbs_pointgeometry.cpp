...
ON_OBJECT_IMPLEMENT(ON_Point, ON_Geometry,
                    "C3101A1D-F157-11d3-BFE7-0010830122F0");
...
ON::object_type ON_Point::ObjectType() const { return ON::point_object; }

int ON_Point::Dimension() const { return 3; }

ON_BOOL32 ON_Point::GetBBox(double *boxmin, double *boxmax,
                            ON_BOOL32 bGrowBox) const {
    return ON_GetPointListBoundingBox(3, 0, 1, 3, &point.x, boxmin, boxmax,
                                      bGrowBox ? true : false);
}

bool ON_Point::IsDeformable() const { return true; }

bool ON_Point::MakeDeformable() { return true; }
