...
ON_BOOL32 ON_InstanceDefinition::IsValid(ON_TextLog *text_log) const {
    ...
    case linked_and_embedded_def:
        if (m_source_archive.IsEmpty()) {
            if (text_log) {
                text_log->Print("ON_InstanceDefinition is linked or embedded "
                                "but m_source_archive is empty.\n");
            }
            return false;
        }
        break;
...
ON_BOOL32 ON_InstanceDefinition::Write(ON_BinaryArchive &binary_archive) const {
    ...
    // m_idef_update_type was an unsigned int and got changed to an enum.  Read
    // and write as an unsigned int to support backwards compatibility
    if (rc)
...
ON_BOOL32 ON_InstanceDefinition::Read(ON_BinaryArchive &binary_archive) {
    int major_version = 0;
    int minor_version = 0;
    ...
    bool rc =
        binary_archive.Read3dmChunkVersion(&major_version, &minor_version);
    if (rc) {
        if (major_version != 1)
            rc = false;
        // version 1.0 fields
        ...
        // m_idef_update_type was an unsigned int and got changed to an enum.
        // Read and write as an unsigned int to support backwards compatibility
        ...
        if (minor_version >= 2) {
            ...
            if (ON::custom_unit_system != m_us.m_unit_system &&
                ...
                m_us.m_custom_unit_scale =
                    ON::UnitScale(m_us.m_unit_system, ON::meters);
            } else {
                m_us.m_custom_unit_scale = 0.0;
            }
...
int ON_InstanceDefinition::Dimension() const { return 3; }

ON_BOOL32 ON_InstanceDefinition::GetBBox(double *boxmin, double *boxmax,
...
ON_BOOL32 ON_InstanceDefinition::Transform(const ON_Xform &xform) {
    // instance DEFs cannot be transformed
    return false;
}

const wchar_t *ON_InstanceDefinition::Name() const { return m_name; }
...
const wchar_t *ON_InstanceDefinition::URL() const { return m_url; }
...
const wchar_t *ON_InstanceDefinition::URL_Tag() const { return m_url_tag; }
...
ON_UUID ON_InstanceDefinition::Uuid() const { return m_uuid; }

void ON_InstanceDefinition::SetUuid(ON_UUID uuid) { m_uuid = uuid; }
...
void ON_InstanceDefinition::SetSourceArchive(
    ...
    if (m_source_archive.IsEmpty())
        ...
        m_idef_update_type =
            ON_InstanceDefinition::IdefUpdateType(idef_update_type);
}
...
const ON_UnitSystem &ON_InstanceDefinition::UnitSystem() const { return m_us; }
...
ON_BOOL32 ON_InstanceRef::Write(ON_BinaryArchive &binary_archive) const {
...
ON_BOOL32 ON_InstanceRef::Read(ON_BinaryArchive &binary_archive) {
    int major_version = 0;
    int minor_version = 0;
    bool rc =
        binary_archive.Read3dmChunkVersion(&major_version, &minor_version);
...
// virtual ON_Geometry overrides
int ON_InstanceRef::Dimension() const { return 3; }

ON_BOOL32 ON_InstanceRef::GetBBox(double *boxmin, double *boxmax,
                                  ON_BOOL32 bGrowBox) const {
    if (!boxmin || !boxmax) {
        bGrowBox = false;
    } else if (bGrowBox) {
        bGrowBox =
            ON_BoundingBox(ON_3dPoint(boxmin), ON_3dPoint(boxmax)).IsValid();
    }
...
ON_BOOL32 ON_InstanceRef::Transform(const ON_Xform &xform) {
...
ON__IDefLayerSettingsUserData::FindOrCreate(const ON_InstanceDefinition &idef,
                                            bool bCreate) {
    ON__IDefLayerSettingsUserData *ud = ON__IDefLayerSettingsUserData::Cast(
        idef.GetUserData(ON__IDefLayerSettingsUserData::
                             m_ON__IDefLayerSettingsUserData_class_id.Uuid()));
    if (!ud && bCreate) {
...
ON__IDefLayerSettingsUserData &ON__IDefLayerSettingsUserData::operator=(
    const ON__IDefLayerSettingsUserData &src) {
    if (this != &src) {
        DestroyHelper();
...
static int compareUuidIndexId(const void *a, const void *b) {
    return ON_UuidIndex::CompareId((const ON_UuidIndex *)a,
                                   (const ON_UuidIndex *)b);
}

void ON_InstanceDefinition::UpdateLinkedIdefReferenceFileLayerRuntimeId(
...
void ON_InstanceDefinition::UpdateLinkedIdefParentLayerSettings(
    ...
    if (0 != ud &&
        ud->m_idef_layer_table_parent_layer != linked_idef_parent_layer) {
        ...
        if (0 != linked_idef_parent_layer) {
            ud->m_idef_layer_table_parent_layer =
                new ON_Layer(*linked_idef_parent_layer);
        }
    }
}
...
void ON_InstanceDefinition::UpdateLinkedIdefReferenceFileLayerSettings(
    ...
    ON_UuidIndex *iddex =
        (ON_UuidIndex *)ws.GetMemory(layer_count * sizeof(iddex[0]));
    ...
    for (i = 0; i < layer_count; i++) {
        ...
        if (0 == layer)
            continue;
        layer->SaveSettings(0,
                            false); // remove any saved settings on input layers
    ...
    for (i = 0; i < ud->m_layers.UnsignedCount(); i++) {
        ...
        for (;;) {
            ...
            layer_settings[idx0->m_i]->SaveSettings(
                settings,
                false); // saves the layer settings found in linked file
            layer_settings[idx0->m_i]->Set(
                settings, *layer); // applies modifications found on idef
            break;
        }
        delete layer;
    }
...
void ON_InstanceDefinition::UpdateLinkedIdefLayerSettings(
    ...
    for (unsigned int i = 0; i < layer_count; i++) {
        ...
        if (!ud->m_runtime_layer_id_map.FindId1(layer1->m_layer_id,
                                                &layerId.m_layer_id))
            layerId.m_layer_id = layer1->m_layer_id;
        ON_Layer **pp =
            (ON_Layer **)bsearch(&layerPtrId, ud_layers, ud_layers_count,
...
ON__IDefAlternativePathUserData::FindOrCreate(const ON_InstanceDefinition &idef,
    ...
    ON__IDefAlternativePathUserData *ud =
        ON__IDefAlternativePathUserData::Cast(idef.GetUserData(
            ON__IDefAlternativePathUserData::
                m_ON__IDefAlternativePathUserData_class_id.Uuid()));
    if (!ud && bCreate) {
...
ON__IDefAlternativePathUserData::ON__IDefAlternativePathUserData() {
    m_userdata_uuid = ON__IDefAlternativePathUserData::
                          m_ON__IDefAlternativePathUserData_class_id.Uuid();
    m_application_uuid = ON_opennurbs_id;
    m_userdata_copycount = 1;
    CreateHelper();
...
ON__IDefAlternativePathUserData &ON__IDefAlternativePathUserData::operator=(
    const ON__IDefAlternativePathUserData &src) {
    if (this != &src) {
        DestroyHelper();
        ON_UserData::operator=(src);
...
void ON_InstanceDefinition::SetAlternateSourceArchivePath(
    ...
    if (0 != alternate_source_archive_path &&
        0 == alternate_source_archive_path[0])
        alternate_source_archive_path = 0;
    ON__IDefAlternativePathUserData *ud =
        ON__IDefAlternativePathUserData::FindOrCreate(
