...
ON_Ellipse::ON_Ellipse(const ON_Plane &p, double rx, double ry) {
    Create(p, rx, ry);
}
...
ON_BOOL32 ON_Ellipse::IsCircle() const {
    double r0 = radius[0];
    return (ON_IsValid(r0) &&
            fabs(r0 - radius[1]) <= fabs(r0) * ON_ZERO_TOLERANCE && IsValid())
               ? true
               : false;
}

double ON_Ellipse::Radius(int i) const { return radius[(i) ? 1 : 0]; }

const ON_3dPoint &ON_Ellipse::Center() const { return plane.origin; }
...
bool ON_Ellipse::GetFoci(ON_3dPoint &F1, ON_3dPoint &F2) const {
    ...
    const ON_3dVector &majorAxis =
        (radius[0] >= radius[1]) ? plane.xaxis : plane.yaxis;
    F1 = plane.origin + f * majorAxis;
    F2 = plane.origin - f * majorAxis;
    return true;
}

const ON_3dVector &ON_Ellipse::Normal() const { return plane.zaxis; }

const ON_Plane &ON_Ellipse::Plane() const { return plane; }

ON_3dPoint ON_Ellipse::PointAt(double t) const {
    return plane.PointAt(cos(t) * radius[0], sin(t) * radius[1]);
}

ON_3dVector ON_Ellipse::DerivativeAt(int d,   // desired derivative ( >= 0 )
                                     double t // parameter
) const {
    double r0 = radius[0];
    double r1 = radius[1];
...
ON_3dVector ON_Ellipse::TangentAt(double t // parameter
) const {
...
ON_3dVector ON_Ellipse::CurvatureAt(double t // parameter
) const {
    ON_3dVector T, K;
            ...
            if (radius[0] != radius[1]) {
                ...
                // solve for closest point using Brent's algorithm
                {
                    ...
                    *t = (t0 == 2.0 * ON_PI) ? 0.0 : t0;
                    for (i = 0; true; i++) {
                        ...
                        if (i >= 100) {
                            ...
                            if (sqrt(d0) <= ON_ZERO_TOLERANCE ||
                                sqrt(d0) <=
                                    ON_SQRT_EPSILON * E0.DistanceTo(Center())) {
                            ...
                            if (V.Unitize()) {
                                ...
                                // pretty close.
                                if (fabs(V * T) <=
                                    0.087155742747658173558064270837474)
                                    return true;
                            }
                    ...
                    rc = ON_FindLocalMinimum(distSqToEllipse, p, t0, et, t1,
...
double ON_Ellipse::EquationAt(const ON_2dPoint &p // coordinates in plane
) const {
    double e, x, y;
...
ON_2dVector ON_Ellipse::GradientAt(const ON_2dPoint &p // coordinates in plane
) const {
    ON_2dVector g;
    if (radius[0] != 0.0 && radius[1] != 0.0) {
...
ON_BOOL32 ON_Ellipse::Rotate(double sin_angle, double cos_angle,
                             const ON_3dVector &axis) {
    return plane.Rotate(sin_angle, cos_angle, axis);
}

ON_BOOL32 ON_Ellipse::Rotate(double angle, const ON_3dVector &axis) {
...
ON_BOOL32 ON_Ellipse::Translate(const ON_3dVector &delta) {
