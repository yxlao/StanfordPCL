    ...
    if (this != &src) {
        ...
        if (0 != m_object && m_bDeleteObject) {
            if (0 != m_ref_count)
                ...
                *m_ref_count =
                    2; // 2 because this and src reference same m_object
...
ONX_Model::ONX_Model()
    : m_3dm_file_version(0), m_3dm_opennurbs_version(0), m_file_length(0),
      m_crc_error_count(0) {
    ...
    if (!m__object_table_bbox.IsValid() && m_object_table.Count() > 0) {
        ...
        for (i = 0; i < object_count; i++) {
            const ON_Geometry *geo =
                ON_Geometry::Cast(m_object_table[i].m_object);
            if (geo)
...
void ONX_Model::GetRenderMaterial(const ON_3dmObjectAttributes &attributes,
    ...
    case ON::material_from_layer:
        if (attributes.m_layer_index >= 0 &&
            ...
            material_index =
                m_layer_table[attributes.m_layer_index].RenderMaterialIndex();
        break;
...
void ONX_Model::GetLinetype(const ON_3dmObjectAttributes &attributes,
    ...
    case ON::linetype_from_layer:
        if (attributes.m_layer_index >= 0 &&
            attributes.m_layer_index < m_layer_table.Count())
            linetype_index =
                m_layer_table[attributes.m_layer_index].LinetypeIndex();
        break;
...
void ONX_Model::GetLinetype(int object_index, ON_Linetype &linetype) const {
    if (object_index < 0 || object_index >= m_object_table.Count()) {
        linetype.Default();
...
void ONX_DumpView(ON_TextLog &dump, const ON_3dmView &view) { view.Dump(dump); }
...
static bool CheckForCRCErrors(ON_BinaryArchive &archive, ONX_Model &model,
                              ON_TextLog *error_log, const char *sSection) {
...
class ON__CIndexPair {
  public:
    static int CompareOldIndex(const ON__CIndexPair *a,
                               const ON__CIndexPair *b);
    static int CompareOldAndNewIndex(const ON__CIndexPair *a,
                                     const ON__CIndexPair *b);
    int m_old_index; // value in model.m_..._table[m_table_index].m_..._index;
                     // (Read from file)
    int m_new_index; // index in model.m_..._table[] array
};

int ON__CIndexPair::CompareOldIndex(const ON__CIndexPair *a,
                                    const ON__CIndexPair *b) {
    return (a->m_old_index - b->m_old_index);
}

int ON__CIndexPair::CompareOldAndNewIndex(const ON__CIndexPair *a,
                                          const ON__CIndexPair *b) {
    int i;
...
class ON__CIndexMaps {
  public:
    ON__CIndexMaps(ONX_Model &model)
        : m_model(model), m_bRemapLayerIndex(0), m_bRemapMaterialIndex(0),
          m_bRemapLinetypeIndex(0), m_bRemapGroupIndex(0), m_bRemapFontIndex(0),
          m_bRemapDimstyleIndex(0), m_bRemapHatchPatternIndex(0),
          m_layer_count(0), m_group_count(0), m_material_count(0),
          m_linetype_count(0), m_font_count(0), m_dimstyle_count(0),
          m_hatch_pattern_count(0), m_default_layer_index(0),
          m_default_group_index(-1), m_default_material_index(-1),
          m_default_linetype_index(-1), m_default_font_index(0),
          m_default_dimstyle_index(0), m_default_hatch_pattern_index(-1) {
        CreateHelper();
    }
    ...
    */
    ...
    Returns:
      new hatch pattern index to use or -1 if old_hatch_pattern_index makes no
    sense.
    */
...
int ON__CIndexMaps::CreateHelper() {
    ...
    for (i = 0; i < m_material_count; i++) {
        ...
        m_model.m_material_id_index.AddUuidIndex(material.m_material_id, i,
                                                 false);
    }
    ...
    for (i = 0; i < m_model.m_object_table.Count(); i++) {
        ...
        m_model.m_object_id_index.AddUuidIndex(mo.m_attributes.m_uuid, i,
                                               false);
    }
...
int ON__CIndexMaps::RemapGeometryAttributes(ON_Object *object) {
    ...
    case ON::hatch_object: {
        ...
        if (hatch_object) {
            ...
            int new_hatch_pattern_index =
                RemapHatchPatternIndex(old_hatch_pattern_index);
...
int ON__CIndexMaps::RemapGeometryAndObjectAttributes(
    ONX_Model_Object &model_object) {
    int geometry_change_count =
        RemapGeometryAttributes(const_cast<ON_Object *>(model_object.m_object));
    int attribute_change_count =
        RemapObjectAttributes(model_object.m_attributes);
...
int ON__CIndexMaps::RemapModel() {
    ...
    for (i = 0; i < m_model.m_light_table.Count(); i++) {
        change_count +=
            RemapObjectAttributes(m_model.m_light_table[i].m_attributes);
    }

    for (i = 0; i < m_model.m_object_table.Count(); i++) {
        change_count +=
            RemapGeometryAndObjectAttributes(m_model.m_object_table[i]);
    }

    return change_count;
}

static int RemapIndexHelper(int old_index, bool bRemapIndex, int count,
                            int default_index,
...
int ON__CIndexMaps::RemapLayerIndex(int old_layer_index) const {
    return RemapIndexHelper(old_layer_index, m_bRemapLayerIndex, m_layer_count,
                            m_default_layer_index, m_layer_map);
}

int ON__CIndexMaps::RemapMaterialIndex(int old_material_index) const {
    return RemapIndexHelper(old_material_index, m_bRemapMaterialIndex,
                            m_material_count, m_default_material_index,
                            m_material_map);
}

int ON__CIndexMaps::RemapLinetypeIndex(int old_linetype_index) const {
    return RemapIndexHelper(old_linetype_index, m_bRemapLinetypeIndex,
                            m_linetype_count, m_default_linetype_index,
                            m_linetype_map);
}

int ON__CIndexMaps::RemapGroupIndex(int old_group_index) const {
    return RemapIndexHelper(old_group_index, m_bRemapGroupIndex, m_group_count,
                            m_default_group_index, m_group_map);
}

int ON__CIndexMaps::RemapFontIndex(int old_font_index) const {
    return RemapIndexHelper(old_font_index, m_bRemapFontIndex, m_font_count,
                            m_default_font_index, m_font_map);
}

int ON__CIndexMaps::RemapDimstyleIndex(int old_dimstyle_index) const {
    return RemapIndexHelper(old_dimstyle_index, m_bRemapDimstyleIndex,
                            m_dimstyle_count, m_default_dimstyle_index,
                            m_dimstyle_map);
}

int ON__CIndexMaps::RemapHatchPatternIndex(int old_hatch_pattern_index) const {
    return RemapIndexHelper(old_hatch_pattern_index, m_bRemapHatchPatternIndex,
                            m_hatch_pattern_count,
                            m_default_hatch_pattern_index, m_hatch_pattern_map);
}
...
int ON__CIndexMaps::RemapRenderingAttributes(ON_RenderingAttributes &ra) {
    ...
    for (i = ra.m_materials.Count() - 1; i >= 0; i--) {
        ...
        if (-1 == mr.m_material_index ||
            mr.m_material_index >= m_material_count) {
            if (ON_nil_uuid == mr.m_material_id)
                mr.m_material_index = -1;
            else if (!m_model.m_material_id_index.FindUuid(
                         mr.m_material_id, &mr.m_material_index))
                mr.m_material_index = -1;
        } else if (m_model.m_material_table[mr.m_material_index]
                       .m_material_id != mr.m_material_id) {
            ...
            if (!ON_UuidIsNil(mr.m_material_id) &&
                ...
                mr.m_material_id =
                    m_model.m_material_table[mr.m_material_index].m_material_id;
            }
        }
        ...
        if (-1 == mr.m_material_backface_index ||
            ...
            if (ON_nil_uuid == mr.m_material_backface_id)
                ...
                mr.m_material_backface_index = -1;
        } else if (m_model.m_material_table[mr.m_material_backface_index]
                       .m_material_id != mr.m_material_backface_id) {
            new_material_index = -1;
            if (!ON_UuidIsNil(mr.m_material_backface_id) &&
                ...
                mr.m_material_backface_id =
                    m_model.m_material_table[mr.m_material_backface_index]
                        .m_material_id;
            }
        }
...
bool ONX_Model::Read(ON_BinaryArchive &archive, ON_TextLog *error_log) {
    ...
    if (!archive.Read3dmStartSection(&m_3dm_file_version,
    ...
    if (!archive.Read3dmProperties(m_properties)) {
        if (error_log)
            error_log->Print(
                "ERROR: Unable to read properties section. "
                "(ON_BinaryArchive::Read3dmProperties() returned false.)\n");
        return false;
    } else if (CheckForCRCErrors(archive, *this, error_log,
                                 "properties section"))
        return_code = false;
    ...
    if (!archive.Read3dmSettings(m_settings)) {
        if (error_log)
            error_log->Print(
                "ERROR: Unable to read settings section. "
    ...
    if (archive.BeginRead3dmBitmapTable()) {
        ...
        if (!archive.EndRead3dmBitmapTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt bitmap table. "
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt bitmap table. "
    ...
    if (archive.BeginRead3dmTextureMappingTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt render texture_mapping found. "
        ...
        if (CheckForCRCErrors(archive, *this, error_log,
                              "render texture_mapping table"))
            return_code = false;
    } else {
        if (error_log) {
            error_log->Print(
                "WARNING: Missing or corrupt render texture_mapping table. "
                "(ON_BinaryArchive::BeginRead3dmTextureMappingTable() returned "
                "false.)\n");
            error_log->Print("-- Attempting to continue.\n");
        }
        return_code = false;
    ...
    if (archive.BeginRead3dmMaterialTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt render material found. "
        ...
        if (!archive.EndRead3dmMaterialTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt render material table. "
        ...
        if (error_log) {
            error_log->Print(
                ...
                "(ON_BinaryArchive::BeginRead3dmMaterialTable() returned "
                "false.)\n");
            error_log->Print("-- Attempting to continue.\n");
        }
        return_code = false;
    ...
    if (archive.BeginRead3dmLinetypeTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt render linetype found. "
        ...
        if (!archive.EndRead3dmLinetypeTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt render linetype table. "
                                 "(ON_BinaryArchive::EndRead3dmLinetypeTable() "
                                 "returned false.)\n");
            return false;
        }
        if (CheckForCRCErrors(archive, *this, error_log,
                              "render linetype table"))
            return_code = false;
    } else {
        if (error_log) {
            error_log->Print(
                "WARNING: Missing or corrupt render linetype table. "
                "(ON_BinaryArchive::BeginRead3dmLinetypeTable() returned "
                "false.)\n");
            error_log->Print("-- Attempting to continue.\n");
        }
        return_code = false;
    ...
    if (archive.BeginRead3dmLayerTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt layer found. "
                        "(ON_BinaryArchive::Read3dmLayer() < 0.)\n");
                    error_count++;
                    if (error_count > max_error_count)
                        return false;
                    error_log->Print("-- Attempting to continue.\n");
        ...
        if (!archive.EndRead3dmLayerTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt render layer table. "
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt layer table. "
    ...
    if (archive.BeginRead3dmGroupTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt group found. "
                        "(ON_BinaryArchive::Read3dmGroup() < 0.)\n");
                    error_count++;
                    if (error_count > max_error_count)
                        return false;
                    error_log->Print("-- Attempting to continue.\n");
        ...
        if (!archive.EndRead3dmGroupTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt group table. "
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt group table. "
    ...
    if (archive.BeginRead3dmFontTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt font found. "
                        "(ON_BinaryArchive::Read3dmFont() < 0.)\n");
                    error_count++;
                    if (error_count > max_error_count)
                        return false;
                    error_log->Print("-- Attempting to continue.\n");
    ...
    if (archive.BeginRead3dmDimStyleTable()) {
        ...
        if (!archive.EndRead3dmDimStyleTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt dimstyle table. "
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt dimstyle table. "
    ...
    if (archive.BeginRead3dmLightTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt render light found. "
        ...
        if (!archive.EndRead3dmLightTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt render light table. "
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt render light table. "
    ...
    if (archive.BeginRead3dmHatchPatternTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt hatchpattern found. "
        ...
        if (!archive.EndRead3dmHatchPatternTable()) {
            if (error_log)
                error_log->Print(
                    ...
                    "(ON_BinaryArchive::EndRead3dmHatchPatternTable() returned "
                    "false.)\n");
            return false;
        }
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt hatchpattern table. "
    ...
    if (archive.BeginRead3dmInstanceDefinitionTable()) {
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Corrupt instance definition found. "
        ...
        if (!archive.EndRead3dmInstanceDefinitionTable()) {
            if (error_log)
                error_log->Print(
                    "ERROR: Corrupt instance definition table. "
        ...
        if (error_log) {
            error_log->Print(
                "WARNING: Missing or corrupt instance definition table. "
    ...
    if (archive.BeginRead3dmObjectTable()) {
        ...
        // For example, if you just wanted to just read points and meshes, you
        // would use object_filter = ON::point_object | ON::mesh_object;
        int object_filter = 0;
        ...
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: Object table entry %d is corrupt. "
            ...
            if (pObject) {
                ...
                if (error_log) {
                    if (rc == 2)
                        error_log->Print("WARNING: Skipping object table entry "
                                         "%d because it's filtered.\n",
                                         count);
                    else if (rc == 3)
                        error_log->Print(
                            "WARNING: Skipping object table entry %d because "
                            "it's newer than this code.  Update your OpenNURBS "
                            "toolkit.\n",
                            count);
                    else
                        error_log->Print("WARNING: Skipping object table entry "
                                         "%d for unknown reason.\n",
                                         count);
                }
            }
        }
        ...
        if (!archive.EndRead3dmObjectTable()) {
            if (error_log)
                error_log->Print("ERROR: Corrupt object light table. "
        ...
        if (error_log) {
            error_log->Print("WARNING: Missing or corrupt object table. "
    ...
    if (archive.BeginRead3dmHistoryRecordTable()) {
        for (count = 0; true; count++) {
            ...
            if (rc < 0) {
                if (error_log) {
                    error_log->Print(
                        "ERROR: History record table entry %d is corrupt. "
            ...
            if (m_crc_error_count != archive.BadCRCCount()) {
                if (error_log) {
                    error_log->Print("ERROR: History record table entry %d is "
                                     "corrupt. (CRC errors).\n",
                                     count);
                    error_log->Print("-- Attempting to continue.\n");
                }
            ...
            if (pHistoryRecord) {
                ...
                if (error_log) {
                    error_log->Print("WARNING: Skipping history record table "
                                     "entry %d for unknown reason.\n",
                                     count);
                }
            }
        }
        ...
        if (error_log) {
            error_log->Print(
                ...
                "(ON_BinaryArchive::BeginRead3dmHistoryRecordTable() returned "
                "false.)\n");
            error_log->Print("-- Attempting to continue.\n");
        }
        return_code = false;
    ...
    for (count = 0; true; count++) {
        ...
        if (!archive.BeginRead3dmUserTable(plugin_id, &bGoo,
                                           &usertable_3dm_version,
                                           &usertable_opennurbs_version)) {
    ...
    if (!archive.Read3dmEndMark(&m_file_length)) {
        if (archive.Archive3dmVersion() != 1) {
            ...
            if (error_log)
                error_log->Print(
                    "ERROR: ON_BinaryArchive::Read3dmEndMark(&m_file_length) "
                    "returned false.\n");
        }
    }
...
bool ONX_Model::Write(const char *filename, int version,
                      const char *sStartSectionComment, ON_TextLog *error_log) {
    bool rc = false;
    if (0 != filename) {
...
bool ONX_Model::Write(const wchar_t *filename, int version,
                      const char *sStartSectionComment, ON_TextLog *error_log) {
    bool rc = false;
    if (0 != filename) {
...
bool ONX_Model::Write(ON_BinaryArchive &archive, int version,
                      const char *sStartSectionComment, ON_TextLog *error_log) {
    int i;
    ...
    if (!IsValid(error_log)) {
        ...
        if (error_log)
            error_log->Print("ONX_Model::Write Your model is not valid and "
                             "will not be saved.\n");
        return false;
    }
    ...
    if (version < 2 || version > 5) {
        ...
        version = 5;
        if (error_log)
            error_log->Print("ONX_Model::Write version parameter = %d; it must "
                             "be 2, 3, or 4.\n",
                             version);
    }
    ...
    if (!ok) {
        ...
        if (error_log)
            error_log->Print(
                ...
                "a file is locked, a disk is locked, or something along those "
                "lines.\n");
        return false;
    }
    ...
    if (!ok) {
        ...
        if (error_log)
            error_log->Print(
                ...
                "Your m_properties information is not valid or basic file "
                "writing failed.\n");
        return false;
    }
    ...
    if (!ok) {
        ...
        if (error_log)
            error_log->Print(
                ...
                "Your m_settings information is not valid or basic file "
                "writing failed.\n");
        return false;
    }
    ...
    if (!ok) {
        if (error_log)
            error_log->Print("ONX_Model::Write "
                             "archive.BeginWrite3dmBitmapTable() failed.\n");
        return false;
    }
    for (i = 0; ok && i < m_bitmap_table.Count(); i++) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write "
                    "archive.Write3dmBitmap(m_bitmap_table[%d]) failed.\n",
                    i);
        }
    }
    ...
    if (archive.Archive3dmVersion() >= 4) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write "
                    "archive.BeginWrite3dmTextureMappingTable() failed.\n");
            return false;
        }
        for (i = 0; ok && i < m_mapping_table.Count(); i++) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print("ONX_Model::Write "
                                     "archive.Write3dmTextureMapping(m_mapping_"
                                     "table[%d]) failed.\n",
                                     i);
            }
        }
        if (!archive.EndWrite3dmTextureMappingTable()) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.EndWrite3dmTextureMappingTable() "
                    "failed.\n");
            return false;
        }
        if (!ok)
    ...
    if (!ok) {
        if (error_log)
            error_log->Print("ONX_Model::Write "
                             "archive.BeginWrite3dmMaterialTable() failed.\n");
        return false;
    }
    for (i = 0; ok && i < m_material_table.Count(); i++) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write "
                    "archive.Write3dmMaterial(m_material_table[%d]) failed.\n",
                    i);
        }
    }
    if (!archive.EndWrite3dmMaterialTable()) {
        if (error_log)
            error_log->Print("ONX_Model::Write "
                             "archive.EndWrite3dmMaterialTable() failed.\n");
        return false;
    }
    if (!ok)
        return false;
    ...
    if (archive.Archive3dmVersion() >= 4) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.BeginWrite3dmLinetypeTable() "
                    "failed.\n");
            return false;
        }
        for (i = 0; ok && i < m_linetype_table.Count(); i++) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print("ONX_Model::Write "
                                     "archive.Write3dmLinetype(m_linetype_"
                                     "table[%d]) failed.\n",
                                     i);
            }
        }
        if (!archive.EndWrite3dmLinetypeTable()) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.EndWrite3dmLinetypeTable() "
                    "failed.\n");
            return false;
        }
        if (!ok)
    ...
    for (i = 0; ok && i < m_layer_table.Count(); i++) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.Write3dmLayer(m_layer_table[%d]) "
                    "failed.\n",
                    i);
        }
    }
    ...
    for (i = 0; ok && i < m_group_table.Count(); i++) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.Write3dmGroup(m_group_table[%d]) "
                    "failed.\n",
                    i);
        }
    }
    ...
    if (archive.Archive3dmVersion() >= 3) {
        ...
        if (!ok) {
            ...
            if (error_log)
                error_log->Print("ONX_Model::Write "
                                 "archive.BeginWrite3dmFontTable() failed.\n");
            return false;
        }
        for (i = 0; ok && i < m_font_table.Count(); i++) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print(
                        "ONX_Model::Write "
                        "archive.Write3dmFont(m_font_table[%d]) failed.\n",
                        i);
            }
        }
        if (!archive.EndWrite3dmFontTable()) {
            if (error_log)
                error_log->Print("ONX_Model::Write "
                                 "archive.EndWrite3dmFontTable() failed.\n");
            return false;
        }
        if (!ok)
            return false;
    ...
    if (archive.Archive3dmVersion() >= 3) {
        ...
        if (!ok) {
            ...
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.BeginWrite3dmDimStyleTable() "
                    "failed.\n");
            return false;
        }
        for (i = 0; ok && i < m_dimstyle_table.Count(); i++) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print("ONX_Model::Write "
                                     "archive.Write3dmDimStyle(m_dimstyle_"
                                     "table[%d]) failed.\n",
                                     i);
            }
        }
        if (!archive.EndWrite3dmDimStyleTable()) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.EndWrite3dmDimStyleTable() "
                    "failed.\n");
            return false;
        }
        if (!ok)
    ...
    for (i = 0; ok && i < m_light_table.Count(); i++) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.Write3dmLight(m_light_table[%d]) "
                    "failed.\n",
                    i);
        }
    }
    ...
    if (archive.Archive3dmVersion() >= 4) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.BeginWrite3dmHatchPatternTable() "
                    "failed.\n");
            return false;
        }
        for (i = 0; ok && i < m_hatch_pattern_table.Count(); i++) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print("ONX_Model::Write "
                                     "archive.Write3dmHatchPattern(m_hatch_"
                                     "pattern_table[%d]) failed.\n",
                                     i);
            }
        }
        if (!archive.EndWrite3dmHatchPatternTable()) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write archive.EndWrite3dmHatchPatternTable() "
                    "failed.\n");
            return false;
        }
        if (!ok)
    ...
    if (archive.Archive3dmVersion() >= 3) {
        ...
        for (i = 0; ok && i < m_idef_table.Count(); i++) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print("ONX_Model::Write "
                                     "archive.Write3dmInstanceDefinition(m_"
                                     "IDef_table[%d]) failed.\n",
                                     i);
            }
        }
        if (!archive.EndWrite3dmInstanceDefinitionTable()) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write "
                    "archive.EndWrite3dmInstanceDefinitionTable() failed.\n");
            return false;
        }
        if (!ok)
            return false;
    ...
    if (!ok) {
        if (error_log)
            error_log->Print("ONX_Model::Write "
                             "archive.BeginWrite3dmObjectTable() failed.\n");
        return false;
    }
        ...
        if (0 != m_object_table[i].m_object) {
            ...
            if (!ok) {
                if (error_log)
                    error_log->Print(
                        "ONX_Model::Write "
                        "archive.Write3dmObject(m_IDef_table[%d]) failed.\n",
                        i);
            }
        }
    }
    ...
    if (archive.Archive3dmVersion() >= 4) {
        ...
        if (!ok) {
            if (error_log)
                error_log->Print(
                    "ONX_Model::Write "
                    "archive.BeginWrite3dmHistoryRecordTable() failed.\n");
            return false;
        }
        ...
        if (!archive.EndWrite3dmHistoryRecordTable()) {
            if (error_log)
                error_log->Print("ONX_Model::Write "
                                 "archive.EndWrite3dmHistoryTable() failed.\n");
            return false;
        }
        if (!ok)
            return false;
    ...
    for (i = 0; ok && i < m_userdata_table.Count(); i++) {
        ...
        if (ON_UuidIsNotNil(ud.m_uuid)) {
            if (!archive.Write3dmAnonymousUserTableRecord(
                    ud.m_uuid, ud.m_usertable_3dm_version,
    ...
    if (ON_UuidIsNotNil(object_uuid)) {
        ...
        if (object_count > 0) {
            if (object_count != m_object_id_index.Count()) {
                ...
                ON_UuidIndexList *p =
                    const_cast<ON_UuidIndexList *>(&m_object_id_index);
                p->Empty();
                ...
                for (i = 0; i < object_count; i++) {
                    ...
                    if (ON_UuidIsNil(id)) {
                        ...
                        ON_CreateUuid(id);
                        *(const_cast<ON_UUID *>(
                            &m_object_table[i].m_attributes.m_uuid)) = id;
                    }
    ...
    if (ON_UuidIsNotNil(idef_uuid)) {
        ...
        if (idef_count > 0) {
            if (idef_count != m_idef_id_index.Count()) {
                ...
                ON_UuidIndexList *p =
                    const_cast<ON_UuidIndexList *>(&m_idef_id_index);
                p->Empty();
                ...
                for (i = 0; i < idef_count; i++) {
                    ...
                    if (ON_UuidIsNil(id)) {
                        ...
                        ON_CreateUuid(id);
                        (const_cast<ON_InstanceDefinition *>(&m_idef_table[i]))
                            ->m_uuid = id;
                    }
                    if (!p->AddUuidIndex(id, i, true)) {
...
int ONX_Model::UsesIDef(const ON_InstanceRef &iref, ON_UUID idef_uuid) const {
    ...
    for (j = 0; j < iref_idef.m_object_uuid.Count(); j++) {
        ...
        if (obj.m_object->ObjectType() == ON::instance_reference) {
            ...
            if (0 != pNestedIRef) {
                if (0 ==
                    ON_UuidCompare(idef_uuid,
                                   pNestedIRef->m_instance_definition_uuid))
                    return 2;
...
bool ONX_Model::SetDocumentUserString(const wchar_t *key,
    ...
    if (0 != key && 0 != key[0]) {
        ON_UUID doc_userstring_id =
            ON_DocumentUserStringList::m_ON_DocumentUserStringList_class_id
                .Uuid();
        for (int i = 0; i < m_userdata_table.Count(); i++) {
            ONX_Model_UserData &ud = m_userdata_table[i];
            if (ud.m_uuid == doc_userstring_id) {
                if (TCODE_USER_RECORD == ud.m_goo.m_typecode &&
                    ud.m_goo.m_value != 0) {
                    ON_Read3dmBufferArchive ba(
                        (unsigned int)ud.m_goo.m_value, ud.m_goo.m_goo, false,
                        m_3dm_file_version, m_3dm_opennurbs_version);
                    ON_Object *p = 0;
                    if (ba.ReadObject(&p)) {
                        ON_DocumentUserStringList *sl =
                        ...
                        if (0 != sl) {
                            ...
                            if (rc) {
                                ...
                                ON_Write3dmBufferArchive newgoo(
                                    ud.m_goo.m_value + 1024, 0,
                                    m_3dm_file_version, ON::Version());
                                if (newgoo.BeginWrite3dmUserTable(
                                        doc_userstring_id, false, 0, 0) &&
                                    newgoo.WriteObject(sl) &&
                                    newgoo.EndWrite3dmUserTable()) {
                                    if (newgoo.SizeOfArchive() > 0 &&
                                        newgoo.SizeOfArchive() <=
                                            0xFFFFFFFF // max goo size if 4GB
                                                       // because we used an
                                                       // unsigned int back in
                                                       // the ice ages
                                    ) {
                                        // update the "goo"
                                        unsigned char *goo =
                                            (unsigned char *)
                                                newgoo.HarvestBuffer();
                                        unsigned int value =
                                            (unsigned int)
                                                newgoo.SizeOfArchive();
                                        if (0 != goo && value > 0) {
    ...
    for (int i = 0; i < m_userdata_table.Count(); i++) {
        ...
        if (ud.m_uuid == doc_userstring_id) {
            if (TCODE_USER_RECORD == ud.m_goo.m_typecode &&
                ...
                ON_Object *p = 0;
                if (ba.ReadObject(&p)) {
                    const ON_DocumentUserStringList *sl =
                        ON_DocumentUserStringList::Cast(p);
                    if (0 != sl) {
...
static int AuditTextureMappingTableHelper(ONX_Model &model, bool bAttemptRepair,
                                          int *repair_count,
                                          ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mapping.m_mapping_index != i) {
            if (text_log) {
                text_log->Print("m_mapping_table[%d].m_mapping_index == %d "
                                "(should be %d)\n",
...
static int AuditGroupTableHelper(ONX_Model &model, bool bAttemptRepair,
                                 int *repair_count, ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
...
static int AuditFontTableHelper(ONX_Model &model, bool bAttemptRepair,
                                int *repair_count, ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
...
static int AuditDimStyleTableHelper(ONX_Model &model, bool bAttemptRepair,
                                    int *repair_count, ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (dimstyle.m_dimstyle_index != i) {
            if (text_log) {
                text_log->Print("m_dimstyle_table[%d].m_dimstyle_index == %d "
                                "(should be %d)\n",
...
static int AuditHatchPatternTableHelper(ONX_Model &model, bool bAttemptRepair,
                                        int *repair_count,
                                        ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (hatchpattern.m_hatchpattern_index != i) {
            if (text_log) {
                text_log->Print("m_hatch_pattern_table[%d].m_hatchpattern_"
                                "index == %d (should be %d)\n",
...
static int AuditObjectAttributesHelper(ONX_Model &model,
                                       ...
                                       int parent_index, bool bAttemptRepair,
                                       int *repair_count,
                                       ON_TextLog *text_log) {
    int repcount = 0;
    int errcount = 0;
    ...
    if (linetype_index < -1 ||
        linetype_index >= model.m_linetype_table.Count()) {
        errcount++;
        if (text_log) {
            text_log->Print(parent_name, parent_index);
            text_log->Print("m_linetype_index = %d is not valid.",
                            linetype_index);
        }
    ...
    if (material_index < -1 ||
        material_index >= model.m_material_table.Count()) {
        errcount++;
        if (text_log) {
            text_log->Print(parent_name, parent_index);
            text_log->Print("m_material_index = %d is not valid.",
                            material_index);
        }
...
static int AuditLightTableHelper(ONX_Model &model, bool bAttemptRepair,
                                 int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (light.m_light_index != i) {
            if (text_log) {
                text_log->Print(
                    "m_light_table[%d].m_light_index == %d (should be %d)\n", i,
        ...
        int attrc = AuditObjectAttributesHelper(
            model, xlight.m_attributes, L"m_light_table[%d].m_attributes", i,
            bAttemptRepair, repair_count, text_log);
...
static int AuditMaterialTableHelper(ONX_Model &model, bool bAttemptRepair,
                                    int *repair_count, ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mat.MaterialIndex() != i) {
            if (text_log) {
                text_log->Print("m_material_table[%d].MaterialIndex() == %d "
                                "(should be %d)\n",
...
static int AuditLinetypeTableHelper(ONX_Model &model, bool bAttemptRepair,
                                    int *repair_count, ON_TextLog *text_log) {
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (lt.LinetypeIndex() != i) {
            if (text_log) {
                text_log->Print("m_linetype_table[%d].LinetypeIndex() == %d "
                                "(should be %d)\n",
...
static int AuditLayerTableHelper(ONX_Model &model, bool bAttemptRepair,
                                 int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    if (repair_count)
        *repair_count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (i != j) {
            if (text_log) {
                text_log->Print("m_layer_table[%d] and m_layer_table[%d] have "
                                "same layer name.\n",
                                i, j);
            }
            if (bAttemptRepair) {
...
static int AuditIdsHelper(ON_SimpleArray<ON_UuidIndex> &id_list,
                          ON_UuidIndexList *index_list, bool bAttemptRepair,
                          int *repair_count, ON_TextLog *text_log,
    ...
    if (count > 0) {
        ...
        if (index_list) {
            ...
            for (i = 0; i < count; i++) {
                index_list->AddUuidIndex(id_list[i].m_id, id_list[i].m_i,
                                         false);
            }
        }
    }
...
static int AuditObjectIdsHelper(ONX_Model &model, bool bAttemptRepair,
                                int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    ...
    if (count > 0) {
        ...
        for (i = 0; i < count; i++) {
            id_list.AppendNew().m_id =
                model.m_object_table[i].m_attributes.m_uuid;
        }
        rc = AuditIdsHelper(id_list, &model.m_object_id_index, bAttemptRepair,
                            repair_count, text_log,
                            L"m_object_table[%d].m_attributes.m_uuid is nil.",
                            L"m_object_table[%d] and [%d] have the same id.");
        if (rc && bAttemptRepair) {
            for (i = 0; i < count; i++) {
                model.m_object_table[id_list[i].m_i].m_attributes.m_uuid =
                    id_list[i].m_id;
            }
        }
    }
...
static int AuditLightIdsHelper(ONX_Model &model, bool bAttemptRepair,
                               int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    ...
    if (count > 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (ON_UuidCompare(light.m_light.m_light_id,
                               light.m_attributes.m_uuid)) {
                mismatch_count++;
                if (text_log) {
                    text_log->Print(
        ...
        if (rc && bAttemptRepair) {
            for (i = 0; i < count; i++) {
                ONX_Model_RenderLight &light =
                    model.m_light_table[id_list[i].m_i];
        ...
        for (i = 0; i < count; i++) {
            ONX_Model_RenderLight &light = model.m_light_table[i];
            int oi = -1;
            if (model.m_object_id_index.FindUuid(light.m_light.m_light_id,
                                                 &oi)) {
                rc++;
                if (text_log) {
                    text_log->Print(L"m_light_table[%d] and m_object_table[%d] "
                                    L"have same id.",
                                    i, oi);
                }
                if (bAttemptRepair) {
                    ...
                    light_id.m_i = -1;
                    if (ON_CreateUuid(light_id.m_id) &&
                        ON_nil_uuid != light_id.m_id) {
                        if (!model.m_object_id_index.FindUuid(light_id.m_id) &&
                            id_list.BinarySearch(&light_id,
                                                 ON_UuidIndex::CompareId) < 0) {
                            if (repair_count)
                                *repair_count = *repair_count + 1;
                            light.m_light.m_light_id = light_id.m_id;
                            light.m_attributes.m_uuid =
                                light.m_light.m_light_id;
                            if (text_log)
                                text_log->Print(" Repaired.");
                        }
                    }
...
static int AuditIDefIdsHelper(ONX_Model &model, bool bAttemptRepair,
                              int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    ...
    if (count > 0) {
        ...
        rc = AuditIdsHelper(id_list, &model.m_idef_id_index, bAttemptRepair,
                            repair_count, text_log,
...
static int AuditMappingIdsHelper(ONX_Model &model, bool bAttemptRepair,
                                 int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    ...
    if (count > 0) {
        ...
        rc = AuditIdsHelper(id_list, &model.m_mapping_id_index, bAttemptRepair,
                            repair_count, text_log,
                            L"m_mapping_table[%d].m_mapping_id is nil.",
                            L"m_mapping_table[%d] and[%d] are the same.");
        if (rc && bAttemptRepair) {
            for (i = 0; i < count; i++) {
                model.m_mapping_table[id_list[i].m_i].m_mapping_id =
                    id_list[i].m_id;
            }
        }
    }
...
static int AuditMaterialIdsHelper(ONX_Model &model, bool bAttemptRepair,
                                  int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    ...
    if (count > 0) {
        ...
        rc = AuditIdsHelper(id_list, &model.m_material_id_index, bAttemptRepair,
                            repair_count, text_log,
                            L"m_material_table[%d].m_material_id is nil.",
                            L"m_material_table[%d] and[%d] are the same.");
        if (rc && bAttemptRepair) {
            for (i = 0; i < count; i++) {
                model.m_material_table[id_list[i].m_i].m_material_id =
                    id_list[i].m_id;
            }
        }
    }
...
static int AuditModelIdsHelper(ONX_Model &model, bool bAttemptRepair,
                               int *repair_count, ON_TextLog *text_log,
...
static int AuditIDefTableHelper(ONX_Model &model, bool bAttemptRepair,
                                int *repair_count, ON_TextLog *text_log) {
    int rc = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (!ONX_IsValidName(idef_name)) {
            if (text_log)
                text_log->Print(
                    "m_idef_table[%d].Name() = \"%S\" is not valid.\n", i,
                    idef_name);
            idef_ok = false;
        } else {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (j = k = 0; j < idef.m_object_uuid.Count(); k++) {
            ...
            if (obj_index < 0) {
                if (text_log)
                    text_log->Print("m_idef_table[%d].m_object_uuid[%d] is not "
                                    "an object's uuid.\n",
                                    i, k);
                if (bAttemptRepair) {
                ...
                if (ON::idef_object != obj.m_attributes.Mode()) {
                    if (text_log)
                        text_log->Print(
                            "Object with uuid "
                            "m_idef_table[%d].m_object_uuid[%d] does not have "
                            "m_attributes.Mode()=ON::idef_object.\n",
                            i, k);
                    if (bAttemptRepair) {
                        idef.m_object_uuid.Remove(j);
                        if (repair_count)
                    ...
                    if (text_log)
                        text_log->Print("Object with uuid "
                                        "m_idef_table[%d].m_object_uuid[%d] "
                                        "has NULL m_object.\n",
                                        i, k);
                    if (bAttemptRepair) {
                        idef.m_object_uuid.Remove(j);
                ...
                } else if (obj.m_object->ObjectType() ==
                           ON::instance_reference) {
                    const ON_InstanceRef *pNestedRef =
                        ON_InstanceRef::Cast(obj.m_object);
                    if (pNestedRef) {
                        if (model.UsesIDef(*pNestedRef, idef.Uuid())) {
                            if (text_log)
                                text_log->Print(
                                    "m_idef_table[%d].m_object_uuid[%d] is a "
                                    "circular reference.\n",
                                    i, k);
                            if (bAttemptRepair) {
                                idef.m_object_uuid.Remove(j);
...
static int AuditObjectTableHelper(ONX_Model &model, bool bAttemptRepair,
                                  int *repair_count, ON_TextLog *text_log) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (0 == obj.m_object) {
            ...
            if (text_log) {
                text_log->Print(
                    "m_object_table[%d].m_object->IsValid() = false.\n", i);
                text_log->PushIndent();
                text_log->PushIndent();
        ...
        int attrc = AuditObjectAttributesHelper(
            model, obj.m_attributes, L"m_object_table[%d].m_attributes", i,
            bAttemptRepair, repair_count, text_log);
        if (0 == rc && attrc)
            rc = attrc;
    }
...
static int AuditHistoryRecordTableHelper(ONX_Model &model, bool bAttemptRepair,
                                         int *repair_count,
                                         ON_TextLog *text_log) {
...
int ONX_Model::Audit(bool bAttemptRepair, int *repair_count,
                     ON_TextLog *text_log, ON_SimpleArray<int> *warnings) {
    int warning_count = 0;
    int rc = 0, i;
    int repcnt;
    if (repair_count)
    ...
    repcnt = 0;
    i = AuditTextureMappingTableHelper(*this, bAttemptRepair, &repcnt,
                                       text_log);
    if (repair_count)
        *repair_count += repcnt;
    if (i < 0)
...
RDKObjectUserDataHelper(const ON_UserData *objectud) {
    ...
    static const ON_UUID CRhRdkUserData_object_id = {
        ...
        0x1525,
        0x43dd,
        {0xA6, 0x3C, 0xC8, 0x4A, 0xC5, 0x80, 0x69, 0x11}};
    ...
    static const ON_UUID CRhRdkUserData_userdata_uuid = {
        ...
        0xCF67,
        0x416c,
        {0x80, 0x0D, 0x22, 0x02, 0x3A, 0xE1, 0xBE, 0x21}};

    const ON_UnknownUserData *unknown_ud = ON_UnknownUserData::Cast(objectud);

    bool rc = (0 != unknown_ud && unknown_ud->m_sizeof_buffer > 0 &&
               0 != unknown_ud->m_buffer &&
               0 == ON_UuidCompare(CRhRdkUserData_object_id,
                                   unknown_ud->m_unknownclass_uuid) &&
               0 == ON_UuidCompare(CRhRdkUserData_userdata_uuid,
                                   unknown_ud->m_userdata_uuid));
...
bool ONX_Model::GetRDKObjectInformation(const ON_Object &object,
    ...
    if (0 != ud) {
        ...
        for (ud = object.FirstUserData(); 0 != ud && 0 == unknown_ud;
             ud = ud->Next()) {
            unknown_ud = RDKObjectUserDataHelper(ud);
        }
    }
    ...
    ON_Read3dmBufferArchive a(unknown_ud->m_sizeof_buffer, unknown_ud->m_buffer,
                              false, unknown_ud->m_3dm_version,
    ...
    if (1 == version) {
        ...
        if (0 != sArray && 0 != sArray[0]) {
            ...
            int wLen = ON_ConvertUTF8ToWideChar(sArray, -1, 0, 0, &error_status,
                                                0, 0, 0);
            if (wLen > 0 && 0 == error_status) {
                rdk_xml_object_data.SetLength(wLen + 2);
                wLen = ON_ConvertUTF8ToWideChar(
                    sArray, -1, rdk_xml_object_data.Array(), wLen + 1,
                    &error_status, 0, 0, 0);
                if (wLen > 0 && 0 == error_status)
            ...
            if (0 != error_status) {
                ON_ERROR(
                    "RDK xml object information is not a valid UTF-8 string.");
            }
        }
    }
...
bool ONX_Model::IsRDKDocumentInformation(const ONX_Model_UserData &docud) {
    ...
    static const ON_UUID rdk_plugin_id = {
        ...
        0x4A2F,
        0x401D,
        {0xBF, 0x5E, 0x3B, 0x87, 0x74, 0x1C, 0x1B, 0x1B}};
...
bool ONX_Model::GetRDKDocumentInformation(const ONX_Model_UserData &docud,
    ...
    ON_Read3dmBufferArchive a(docud.m_goo.m_value, docud.m_goo.m_goo, false,
    ...
    if (1 == version) {
        ...
        if (0 != sArray && 0 != sArray[0]) {
            ...
            if (wLen > 0 && 0 == error_status) {
                ...
                wLen = ON_ConvertUTF8ToWideChar(
                    sArray, -1, rdk_xml_document_data.Array(), wLen + 1,
                    &error_status, 0, 0, 0);
                if (wLen > 0 && 0 == error_status)
            ...
            if (0 != error_status) {
                ON_ERROR(
                    "RDK xml document settings is not a valid UTF-8 string.");
            }
        }
    }
