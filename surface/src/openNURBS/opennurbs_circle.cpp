...
ON_Circle::ON_Circle(const ON_2dPoint &P, const ON_2dPoint &Q,
                     const ON_2dPoint &R) {
    Create(P, Q, R);
}

ON_Circle::ON_Circle(const ON_3dPoint &P, const ON_3dPoint &Q,
                     const ON_3dPoint &R) {
    Create(P, Q, R);
}

double ON_Circle::Radius() const { return radius; }

double ON_Circle::Diameter() const { return 2.0 * radius; }

const ON_3dPoint &ON_Circle::Center() const { return plane.origin; }

const ON_3dVector &ON_Circle::Normal() const { return plane.zaxis; }

const ON_Plane &ON_Circle::Plane() const { return plane; }
...
double ON_Circle::Circumference() const { return fabs(2.0 * ON_PI * radius); }
...
bool ON_Circle::Create(const ON_Plane &pln, const ON_3dPoint &C, double r) {
    ON_Plane p = pln;
    p.origin = C;
    p.UpdateEquation();
    return Create(p, r);
...
bool ON_Circle::Create(const ON_2dPoint &P,     // [IN] point P
...
bool ON_Circle::Create(const ON_3dPoint &P,     // [IN] point P
...
ON_3dVector ON_Circle::DerivativeAt(int d,   // desired derivative ( >= 0 )
                                    double t // parameter
) const {
    double r0 = radius;
    double r1 = radius;
...
double ON_Circle::EquationAt(const ON_2dPoint &p // coordinates in plane
) const {
    double e, x, y;
    if (radius != 0.0) {
        x = p.x / radius;
...
ON_2dVector ON_Circle::GradientAt(const ON_2dPoint &p // coordinates in plane
) const {
    ON_2dVector g;
    if (radius != 0.0) {
...
bool ON_Circle::Rotate(double sin_angle, double cos_angle,
                       const ON_3dVector &axis) {
    return plane.Rotate(sin_angle, cos_angle, axis);
}

bool ON_Circle::Rotate(double angle, const ON_3dVector &axis) {
    return plane.Rotate(angle, axis);
}

bool ON_Circle::Rotate(double sin_angle, double cos_angle,
                       const ON_3dVector &axis, const ON_3dPoint &point) {
...
bool ON_Circle::GetRadianFromNurbFormParameter(double NurbParameter,
...
bool ON_Circle::GetNurbFormParameterFromRadian(double RadianParameter,
