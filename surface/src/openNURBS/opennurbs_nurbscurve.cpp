...
ON_OBJECT_IMPLEMENT(ON_NurbsCurve, ON_Curve,
                    "4ED7D4DD-E947-11d3-BFE5-0010830122F0");

ON_NurbsCurve *ON_NurbsCurve::New() { return new ON_NurbsCurve(); }

ON_NurbsCurve *ON_NurbsCurve::New(const ON_NurbsCurve &nurbs_curve) {
    return new ON_NurbsCurve(nurbs_curve);
}

ON_NurbsCurve *ON_NurbsCurve::New(const ON_BezierCurve &bezier_curve) {
    return new ON_NurbsCurve(bezier_curve);
}

ON_NurbsCurve *ON_NurbsCurve::New(int dimension, ON_BOOL32 bIsRational,
                                  int order, int cv_count) {
...
ON_NurbsCurve::ON_NurbsCurve(int dim, ON_BOOL32 bIsRational, int order,
                             int cv_count) {
    ON__SET__THIS__PTR(m_s_ON_NurbsCurve_ptr);
    Initialize();
...
ON__UINT32 ON_NurbsCurve::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_is_rat), &m_is_rat);
    current_remainder = ON_CRC32(current_remainder, sizeof(m_order), &m_order);
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_cv_count), &m_cv_count);
...
int ON_NurbsCurve::Dimension() const { return m_dim; }

bool ON_NurbsCurve::IsRational() const { return m_is_rat ? true : false; }

int ON_NurbsCurve::CVSize() const {
    return (m_dim > 0) ? (m_is_rat ? (m_dim + 1) : m_dim) : 0;
}

int ON_NurbsCurve::Order(void) const { return m_order; }

int ON_NurbsCurve::CVCount(void) const { return m_cv_count; }
...
double ON_NurbsCurve::Knot(int i) const { return (m_knot) ? m_knot[i] : 0.0; }

int ON_NurbsCurve::KnotMultiplicity(int i) const {
    return ON_KnotMultiplicity(m_order, m_cv_count, m_knot, i);
}

const double *ON_NurbsCurve::Knot() const { return m_knot; }

double ON_NurbsCurve::SuperfluousKnot(int end) const {
    return (m_knot) ? ON_SuperfluousKnot(m_order, m_cv_count, m_knot, end)
                    : 0.0;
}
...
bool ON_NurbsCurve::CreateClampedUniformNurbs(int dimension, int order,
                                              int point_count,
...
bool ON_NurbsCurve::CreatePeriodicUniformNurbs(int dimension, int order,
                                               int point_count,
                                               const ON_3dPoint *point,
                                               double knot_delta) {
    bool rc = (dimension >= 1 && dimension <= 3 && point != NULL);
    ...
    if (rc) {
        ...
        for (i = 0; i < point_count; i++)
            SetCV(i, ON::intrinsic_point_style, point[i]);
        for (i = 0; i <= order - 2; i++)
            SetCV(m_cv_count - m_order + 1 + i, ON::intrinsic_point_style,
                  CV(i));
    }
    if (rc)
...
bool ON_NurbsCurve::Create(int dim,          // dimension (>= 1)
...
void ON_NurbsCurve::EmergencyDestroy() { Initialize(); }
...
static void ON_NurbsCurveCopyHelper(const ON_NurbsCurve &src,
    ...
    if (src.m_knot) {
        ...
        memcpy(dest.m_knot, src.m_knot,
               dest.KnotCount() * sizeof(*dest.m_knot));
    }
    if (src.m_cv) {
        // copy cv array
        ...
        if (src_stride == dst_stride) {
            memcpy(dst_cv, src_cv,
                   dest.m_cv_count * dst_stride * sizeof(*dst_cv));
        } else {
            int i;
...
void ON_NurbsCurve::Dump(ON_TextLog &dump) const {
    ...
    dump.Print("Control Points  %d %s points\n"
               "  index               value\n",
               m_cv_count, (m_is_rat) ? "rational" : "non-rational");
    if (!m_cv) {
        dump.Print("  NULL cv array\n");
    } else {
        dump.PrintPointList(m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv,
                            "  CV");
    }
}
...
static bool ON_ControlPointsAreValid(int cv_size, int cv_count, int cv_stride,
                                     const double *cv, ON_TextLog *text_log) {
    int i, j;
    if (0 == cv) {
        if (text_log) {
    ...
    if (cv_stride < cv_size) {
        if (text_log) {
            text_log->Print("cv_stride = %d and cv_size = %d (cv_stride must "
                            "be >= cv_size).\n",
                            cv_stride, cv_size);
        }
        return ON_ControlPointsAreNotValid();
...
ON_BOOL32 ON_NurbsCurve::IsValid(ON_TextLog *text_log) const {
    ...
    if (!ON_ControlPointsAreValid(CVSize(), m_cv_count, m_cv_stride, m_cv,
                                  text_log)) {
        if (text_log) {
    ...
    if (m_is_rat) {
        ...
        for (i = 0; i < m_cv_count; i++, w += m_cv_stride) {
            ...
            if (zcount >= m_order) {
                if (text_log) {
                    text_log->Print("ON_NurbsCurve.m_cv has zero weights for "
                                    "CV[%d],...,CV[%d].\n",
                                    i - m_order + 1, i);
                }
                return ON_NurbsCurveIsNotValid(); // denominator is zero for
                                                  // entire span
            }
            ...
            if (m_knot[i] == m_knot[i + m_order - 2]) {
                ...
                if (sign == 0.0) {
                    ...
                    if (text_log) {
                        text_log->Print(
                            "ON_NurbsCurve.m_cv has a zero denominator in the "
...
ON_BOOL32 ON_NurbsCurve::GetBBox( // returns true if successful
    ...
    return ON_GetPointListBoundingBox(m_dim, m_is_rat, m_cv_count, m_cv_stride,
                                      m_cv, boxmin, boxmax,
                                      bGrowBox ? true : false);
}
...
ON_BOOL32 ON_NurbsCurve::Transform(const ON_Xform &xform) {
    ...
    return ON_TransformPointList(m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv,
                                 xform);
}

bool ON_NurbsCurve::IsDeformable() const { return true; }

bool ON_NurbsCurve::MakeDeformable() { return true; }

bool ON_PolylineCurve::IsDeformable() const { return true; }

bool ON_PolylineCurve::MakeDeformable() { return true; }

ON_BOOL32 ON_NurbsCurve::Write(ON_BinaryArchive &file // open binary file
) const {
    ...
    if (rc) {
        ...
        count =
            (m_cv && cv_size > 0 && m_cv_count > 0 && m_cv_stride >= cv_size)
                ? m_cv_count
                : 0;
        if (rc)
            rc = file.WriteInt(count);
...
ON_BOOL32 ON_NurbsCurve::Read(ON_BinaryArchive &file // open binary file
) {
    // NOTE - check legacy I/O code if changed
    Destroy();
    int major_version = 0;
...
ON_Interval ON_NurbsCurve::Domain() const {
    ON_Interval d;
    if (!ON_GetKnotVectorDomain(m_order, m_cv_count, m_knot, &d.m_t[0],
                                &d.m_t[1]))
        d.Destroy();
    return d;
}
...
int ON_NurbsCurve::Degree() const { return m_order >= 2 ? m_order - 1 : 0; }
...
int ON_NurbsCurve::IsPolyline(ON_SimpleArray<ON_3dPoint> *pline_points,
            ...
            for (i = 0; i < span_count; i++, k++) {
                if (k[m_order - 2] < k[m_order - 1]) {
                    ...
                    for (j = 1; j < m_order - 1; j++) {
                        ...
                        // this test.
                        if (fabs(P.x - Q.x) >
                            (fabs(P.x) + fabs(Q.x)) * 0.00001 +
                                ON_ZERO_TOLERANCE)
                            return 0;
                        if (fabs(P.y - Q.y) >
                            (fabs(P.y) + fabs(Q.y)) * 0.00001 +
                                ON_ZERO_TOLERANCE)
                            return 0;
                        if (fabs(P.z - Q.z) >
                            (fabs(P.z) + fabs(Q.z)) * 0.00001 +
                                ON_ZERO_TOLERANCE)
                            return 0;
                    }
                    rc++;
...
ON_BOOL32
ON_NurbsCurve::GetParameterTolerance( // returns tminus < tplus: parameters
                                      // tminus <= s <= tplus
...
ON_NurbsCurve::Evaluate( // returns false if unable to evaluate
    ...
    int span_index = ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, t, side,
                                       (hint) ? *hint : 0);

    if (-2 == side || 2 == side) {
        // 9 November 2010 Dale Lear - ON_TuneupEvaluationParameter fix
        //   When evluation passes through ON_CurveProxy or ON_PolyCurve
        //   reparamterization and the original side parameter was -1 or +1, it
        //   is changed to -2 or +2 to indicate that if t is numerically closed
        //   to an end paramter, then it should be tuned up to be at the end
        //   paramter.
        double a = t;
        if (ON_TuneupEvaluationParameter(side, m_knot[span_index + m_order - 2],
                                         m_knot[span_index + m_order - 1],
                                         &a)) {
            // recalculate span_index
            t = a;
            span_index = ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, t, side,
                                           span_index);
        }
    }

    rc = ON_EvaluateNurbsSpan(m_dim, m_is_rat, m_order, m_knot + span_index,
...
ON_NurbsCurve::IsPeriodic() const {
    ON_BOOL32 bIsPeriodic =
        ON_IsKnotVectorPeriodic(m_order, m_cv_count, m_knot);
    if (bIsPeriodic) {
        int i0 = m_order - 2;
        int i1 = m_cv_count - 1;
...
bool ON_IsCurvatureDiscontinuity(const ON_3dVector Km, const ON_3dVector Kp,
    ...
    return ON_IsCurvatureDiscontinuity(
        Km, Kp, cos_angle_tolerance, curvature_tolerance, zero_curvature,
        radius_tolerance,
        0.001 // relative_tolerance - used to be hard coded in this version of
              // the function
    );
}

bool ON_IsG2CurvatureContinuous(const ON_3dVector Km, const ON_3dVector Kp,
                                double cos_angle_tolerance,
                                double curvature_tolerance) {
    ...
    //  get test results.
    double rel_tol =
        0.05; // disables using curvature_tolerance to flag unequal scalars
    ...
    if (cos_Kangle_tolerance > 0.95) {
        ...
        if (cos_angle_tolerance < 0.0) {
            ...
            cos_Kangle_tolerance =
                2.0 * cos_Kangle_tolerance * cos_Kangle_tolerance -
    ...
    return !ON_IsCurvatureDiscontinuity(
        Km, Kp, cos_Kangle_tolerance, curvature_tolerance,
        ON_ZERO_CURVATURE_TOLERANCE,
        ON_UNSET_VALUE, // no absolute delta radius tolerance
        rel_tol);
}

bool ON_IsGsmoothCurvatureContinuous(const ON_3dVector Km, const ON_3dVector Kp,
                                     double cos_angle_tolerance,
                                     double curvature_tolerance) {
    double rel_tol =
        1.0; // disables using curvature_tolerance to flag unequal scalars
...
bool ON_NurbsCurve::GetNextDiscontinuity(ON::continuity c, double t0, double t1,
    ...
    if (m_order <= 2)
        c = ON::PolylineContinuity(
            c); // no need to check 2nd derivatives that are zero
    ...
    ki = ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, t0, (t0 > t1) ? -1 : 1,
                           *hint);
    double segtol = (fabs(m_knot[ki]) + fabs(m_knot[ki + 1]) +
                     fabs(m_knot[ki + 1] - m_knot[ki])) *
                    ON_SQRT_EPSILON;
    ...
    if (t0 < t1) {
        int ii = ki + m_order - 2;
        if (t0 < m_knot[ii + 1] && t1 > m_knot[ii + 1] &&
            (m_knot[ii + 1] - t0) <= segtol && ii + 2 < m_cv_count) {
            t0 = m_knot[ii + 1];
            ki = ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, t0, 1, *hint);
        }
        if (bLineWiggleTest)
            bSpanIsLinear =
                SpanIsLinear(ki, is_linear_min_length, is_linear_tolerance);
        *hint = ki;
        ki += m_order - 2;
        while (ki < m_cv_count - 1 && m_knot[ki] <= t0)
            ki++;
        if (ki >= m_cv_count - 1) {
            if (input_c != c && t0 < m_knot[m_cv_count - 1] &&
                t1 >= m_knot[m_cv_count - 1]) {
                // have to do locus end test
                return ON_Curve::GetNextDiscontinuity(
                    input_c, t0, t1, t, hint, dtype, cos_angle_tolerance,
                    curvature_tolerance);
            }
            return false;
        }
        ...
        *hint = ki;
        ...
        while (ki > m_order - 2 && m_knot[ki] >= t0)
            ki--;
        if (ki <= m_order - 2) {
            if (input_c != c && t0 > m_knot[m_order - 2] &&
                t1 < m_knot[m_order - 2]) {
                // have to do locus end test
                return ON_Curve::GetNextDiscontinuity(
                    input_c, t0, t1, t, hint, dtype, cos_angle_tolerance,
    ...
    while (search_domain[0] < m_knot[ki] && m_knot[ki] < search_domain[1]) {
        ...
        if (m_knot[ki] == m_knot[ki + delta]) {
            ...
            if (bTestTangent) {
                ...
                if (d < cos_angle_tolerance) {
                    ...
                    *t = m_knot[ki];
                    ...
                    bool bIsCurvatureContinuous =
                        (ON::Gsmooth_continuous == c)
                            ? ON_IsGsmoothCurvatureContinuous(
                                  Km, Kp, cos_angle_tolerance,
                                  curvature_tolerance)
                            : ON_IsG2CurvatureContinuous(Km, Kp,
                                                         cos_angle_tolerance,
                                                         curvature_tolerance);
                    if (!bIsCurvatureContinuous) {
                        ...
                        //   the one used in
                        //   ON_PolyCurve::GetNextDiscontinuity() and
                        ...
                        if (bSpanIsLinear !=
                            ((delta_ki < 0)
                                 ...
                                 : SpanIsLinear(ki - m_order + 2,
                                                is_linear_min_length,
                                                is_linear_tolerance))) {
                            // we are at a transition between a line segment and
                            // a wiggle
                            *dtype = 3;
                            *t = m_knot[ki];
                            return true;
                ...
                if (!(D1m - D1p).IsTiny(D1m.MaximumCoordinate() *
                                        ON_SQRT_EPSILON)) {
                    *dtype = 1;
                    *t = m_knot[ki];
                    return true;
                } else if (bEv2ndDer) {
                    if (!(D2m - D2p).IsTiny(D2m.MaximumCoordinate() *
                                            ON_SQRT_EPSILON)) {
                        *dtype = 2;
                        *t = m_knot[ki];
                        return true;
    ...
    if (m_order < m_cv_count && desired_continuity != ON::C0_continuous) {
...

        {
            ...
            if (m_knot[ii] + segtol < m_knot[ii + 1] - segtol) {
                if (fabs(t - m_knot[ii]) <= segtol && ii > m_order - 2) {
                    t = m_knot[ii];
                } else if (fabs(t - m_knot[ii + 1]) <= segtol &&
                           ii + 2 < m_cv_count) {
                    t = m_knot[ii + 1];
                    ki = ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, t, 1,
                                           *hint);
                }
            }
        }
        ...
        *hint = ki;
        ...
        if (ki > m_order - 2 && ki < m_cv_count - 1 && m_knot[ki] == t) {
            ...
            int knot_mult =
                ON_KnotMultiplicity(m_order, m_cv_count, m_knot, ki);
            ...
            rc = ON_Curve::IsContinuous(
                desired_continuity, t, hint, point_tolerance, d1_tolerance,
                d2_tolerance, cos_angle_tolerance, curvature_tolerance);

            if (rc && ON::Gsmooth_continuous == desired_continuity &&
                knot_mult == m_order - 1 && ki > m_order - 2 &&
                ki < m_cv_count - 1) {
                // See if we are transitioning from linear to non-linear
                const double is_linear_tolerance = 1.0e-8;
                const double is_linear_min_length = 1.0e-8;
                if (SpanIsLinear(ki - m_order + 2, is_linear_min_length,
                                 is_linear_tolerance) !=
                    SpanIsLinear(ki - 2 * m_order + 3, is_linear_min_length,
                                 is_linear_tolerance))
                    rc = false;
            }
        }
...
ON_NurbsCurve::SetCV(int i, ON::point_style style, const double *Point) {
    ...
    case ON::euclidean_rational: // input Point is euclidean rational
        if (IsRational()) {
            // NURBS curve is rational - convert euclean point to homogeneous
            // form
            w = Point[m_dim];
            for (k = 0; k < m_dim; k++)
...
ON_NurbsCurve::Reverse() {
    ...
    ON_BOOL32 rc1 =
        ON_ReversePointList(m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv);
    return rc0 && rc1;
}
...
bool ON_NurbsCurve::GetGrevilleAbcissae( // see ON_GetGrevilleAbcissa() for
                                         // details
...
bool ON_NurbsCurve::ReserveKnotCapacity(int desired_capacity) {
    ...
    if (desired_capacity > m_knot_capacity) {
        if (!m_knot) {
            ...
            // the m_knot[] array.
            m_knot =
                (double *)onrealloc(m_knot, desired_capacity * sizeof(*m_knot));
            if (!m_knot) {
                rc = false;
                m_knot_capacity = 0;
            } else {
...
bool ON_NurbsCurve::ConvertSpanToBezier(int span_index,
                                        ON_BezierCurve &bez) const {
    bool rc = false;
    if (span_index >= 0 && span_index <= m_cv_count - m_order && m_knot &&
        m_cv) {
        ...
        if (rc) {
            ...
            if (knot[m_order - 2] < knot[m_order - 1])
                ON_ConvertNurbSpanToBezier(cvdim, bez.m_order, bez.m_cv_stride,
                                           bez.m_cv, knot, knot[m_order - 2],
                                           knot[m_order - 1]);
            else
                rc = false;
        }
...
bool ON_NurbsCurve::MakePiecewiseBezier(bool bSetEndWeightsToOne) {
    ...
    if (!rc && IsValid()) {
        ...
        int ki, spani, i;
        for (ki = m_order - 2, spani = 0;
             ki < m_cv_count - 1 && spani < span_count; ki++) {
            if (m_knot[ki] < m_knot[ki + 1]) {
                bez[spani].Create(m_dim, m_is_rat, m_order);
                for (i = 0; i < m_order; i++)
                    bez[spani].SetCV(i, ON::intrinsic_point_style,
                                     CV(i + ki - m_order + 2));
                ON_ConvertNurbSpanToBezier(
                    cvdim, bez[spani].m_order, bez[spani].m_cv_stride,
                    bez[spani].m_cv, m_knot + ki - m_order + 2, m_knot[ki],
                    m_knot[ki + 1]);
                spani++;
            }
        }
        ...
        for (spani = 0; spani < span_count; spani++) {
            for (i = 0; i < m_order; i++) {
                SetCV(spani * (m_order - 1) + i, ON::intrinsic_point_style,
                      bez[spani].CV(i));
            }
    ...
    if (rc && bSetEndWeightsToOne && m_is_rat) {
        // 2 June 2003 Dale Lear - added support for bSetEndWeightsToOne=true
        // option.
        double w0, w1;
        ON_BezierCurve bez;
        bez.m_dim = m_dim;
...
double ON_NurbsCurve::ControlPolygonLength() const {
    double length = 0.0;
    ON_GetPolylineLength(m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv,
                         &length);
    return length;
}
...
bool ON_NurbsCurve::InsertKnot(double knot_value, int knot_multiplicity) {
    ...
    if (knot_value == t0) {
        if (knot_multiplicity == degree) {
            ...
            ON_ERROR("ON_InsertKnot(): knot_value = t0 and 1 < "
                     "knot_multiplicity < degree.");
            rc = false;
        }
        return rc;
    ...
    if (knot_value == t1) {
        if (knot_multiplicity == degree) {
            ...
            ON_ERROR("ON_InsertKnot(): knot_value = t1 and 1 < "
                     "knot_multiplicity < degree.");
            rc = false;
        }
        return rc;
    ...
    int span_index =
        ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, knot_value, 0, 0);
    ...
    int new_knot_count =
        ON_InsertKnot(knot_value, knot_multiplicity, CVSize(), m_order,
                      m_cv_count, m_cv_stride, m_cv, m_knot, &span_hint);
    if (new_knot_count > 0) {
        ...
        if (ON_MakeKnotVectorPeriodic(m_order, m_cv_count, m_knot)) {
            ...
            for (i0 = 0, i1 = m_cv_count - degree; i0 < degree; i0++, i1++) {
                if (span_index < degree - 1)
                    SetCV(i1, ON::intrinsic_point_style,
                          CV(i0)); // cv[i1] = cv[i0]
                else
                    SetCV(i0, ON::intrinsic_point_style,
                          CV(i1)); // cv[i0] = cv[i1]
            }
        } else {
            ClampEnd(2);
...
static ON_BOOL32
GetRaisedDegreeCV(int old_order, int cvdim, int old_cvstride,
...

{
    ...
    for (i = 0; i < new_degree; i++) {
        ...
        if (!ON_EvaluateNurbsBlossom(cvdim, old_order, old_cvstride, oldCV,
                                     oldkn, t, P)) {
            onfree((void *)t);
            onfree((void *)P);
            return false;
...

{
    ...
    for (i = 0; i < span_count; i++) {
        ...
        for (j = skip; j < N.Order(); j++) { // calculate each cv of the span
            GetRaisedDegreeCV(M.Order(), cvdim, M.m_cv_stride, cvM, knotM,
                              knotN, j, cvN);
            cvN += N.m_cv_stride;
        }
...
bool ON_NurbsCurve::ChangeDimension(int desired_dimension) {
    ...
    if (desired_dimension < m_dim) {
        ...
        const int cv_size1 =
            m_is_rat ? desired_dimension + 1 : desired_dimension;
        const int cv_stride1 =
            (m_cv_stride < cv_size1) ? cv_size1 : m_cv_stride;
...
bool ON_NurbsCurve::Append(const ON_NurbsCurve &c) {
    ...
    if (IsRational() && !c.IsRational() || c.Degree() < Degree() ||
        !c.IsClamped(0) || c.Dimension() < Dimension()) {
        ON_NurbsCurve tmp(c);
...
ON_BOOL32 ON_NurbsCurve::Trim(const ON_Interval &in) {
    ...
    // Greg Arden 28 April 2003.  Do not change any curve that is trimmed to its
    // entire domain.
    ...
    split_t = t;
    if (TweakSplitTrimParameter(m_knot[ki + order - 2], m_knot[ki + order - 1],
                                split_t))
        ki = ON_NurbsSpanIndex(order, m_cv_count, m_knot, split_t, side, ki);

    if (!ON_EvaluateNurbsDeBoor(cv_dim, order, m_cv_stride, CV(ki), m_knot + ki,
                                side, 0.0, t)) {
...

{
    ...
    if (domain[0] < Domain()[0]) {
        ClampEnd(0);
        ON_EvaluateNurbsDeBoor(cvdim, Order(), m_cv_stride, CV(0), m_knot, 1,
                               0.0, domain[0]);
    ...
    if (domain[1] > Domain()[1]) {
        ClampEnd(1);
        int i = CVCount() - Order();
        ON_EvaluateNurbsDeBoor(cvdim, Order(), m_cv_stride, CV(i), m_knot + i,
                               -1, 0.0, domain[1]);
...
ON_BOOL32 ON_NurbsCurve::Split(
    ...
    ON_Curve *
        &left_crv, // left portion returned here (must be an ON_NurbsCurve)
    ON_Curve *
        &right_crv // right portion returned here (must be an ON_NurbsCurve)
) const {
    ...
    if (IsValid() && t > m_knot[m_order - 2] && t < m_knot[m_cv_count - 1]) {
        ...
        if (TweakSplitTrimParameter(m_knot[span_index + m_order - 2],
            ...
            span_index = ON_NurbsSpanIndex(m_order, m_cv_count, m_knot, split_t,
                                           1, span_index);
        }
        ...
        if (span_index >= 0 && span_index <= m_cv_count - m_order) {
            ...
            if (span_index > 0 && split_t == m_knot[span_index + m_order - 2]) {
                ...
                int k;
                for (k = 0;
                     left_cv_count >= m_order && k <= span_index + m_order - 2;
                     k++) {
            ...
            if (right->m_cv != m_cv || span_index > 0) {
                ...
                for (i = 0; i < right_cv_count; i++) {
                    memmove(right->m_cv + i * cvdim, CV(i + span_index),
                            sizeof_cv);
                }
            }
            if (right->m_knot != m_knot || span_index > 0) {
                ...
                memmove(right->m_knot, m_knot + span_index,
                        i * sizeof(right->m_knot[0]));
            }
            ...
            ON_EvaluateNurbsDeBoor(cvdim, m_order, cvdim, left->CV(i),
                                   left->m_knot + i, -1, 0.0, t);
            for (i = left->m_cv_count - 1;
                 i < ON_KnotCount(left->m_order, left->m_cv_count); i++)
                left->m_knot[i] = t;
            left->ClampEnd(2); // 26 June 2003 Dale Lear

            // trim left end of right NURBS
            ON_EvaluateNurbsDeBoor(cvdim, m_order, cvdim, right->m_cv,
                                   right->m_knot, +1, 0.0, t);
...
int ON_NurbsCurve::GetNurbForm(
    ON_NurbsCurve &curve, double tolerance,
    const ON_Interval
        *subdomain // OPTIONAL subdomain of ON::ProxyCurve::Domain()
) const {
    int rc = 1;
...
int ON_NurbsCurve::HasNurbForm() const { return 1; }
...
bool ON_NurbsCurve::ClampEnd(
    ...
    return ON_ClampKnotVector(CVSize(), m_order, m_cv_count, m_cv_stride, m_cv,
                              m_knot, end);
}
...
static bool ON_IsDuplicateKnotVector(int order, int cv_count,
    ...
    if (rc) {
        ...
        if (bIgnoreParameterization) {
            ...
            const ON_Interval other_dom(other_knot[order - 2],
                                        other_knot[cv_count - 1]);
            double k, other_k;
...
static bool ON_IsDuplicatePointList(int dim, int is_rat, int count, int stride,
                                    const double *cv, int other_stride,
                                    const double *other_cv, double tolerance) {
    bool rc = (dim > 0 || is_rat >= 0 && is_rat <= 1 && count > 0 &&
                              abs(stride) >= dim + is_rat &&
                              abs(other_stride) >= (dim + is_rat) && 0 != cv &&
                              0 != other_cv);
    if (rc) {
        if (tolerance < 0.0)
            tolerance = 0.0;
...
bool ON_NurbsCurve::IsDuplicate(const ON_NurbsCurve &other,
    ...
    if (!rc && m_dim == other.m_dim && m_is_rat == other.m_is_rat &&
        ...
        // compare knots
        rc = ON_IsDuplicateKnotVector(m_order, m_cv_count, m_knot, other.m_knot,
                                      bIgnoreParameterization);

        // compare control points
        if (rc)
            rc = ON_IsDuplicatePointList(m_dim, m_is_rat ? 1 : 0, m_cv_count,
                                         m_cv_stride, m_cv, other.m_cv_stride,
                                         other.m_cv, tolerance);
    }
    return rc;
}
...
bool ON_NurbsSurface::IsDuplicate(const ON_NurbsSurface &other,
    ...
    if (!rc && m_dim == other.m_dim && m_is_rat == other.m_is_rat &&
        ...
        // compare knots
        rc = ON_IsDuplicateKnotVector(m_order[0], m_cv_count[0], m_knot[0],
                                      other.m_knot[0], bIgnoreParameterization);
        if (rc)
            rc = ON_IsDuplicateKnotVector(m_order[1], m_cv_count[1], m_knot[1],
                                          other.m_knot[1],
                                          bIgnoreParameterization);
        ...
        for (i = 0; i < m_cv_count[0] && rc; i++) {
            rc = ON_IsDuplicatePointList(
                m_dim, m_is_rat ? 1 : 0, m_cv_count[1], m_cv_stride[1],
                CV(i, 0), other.m_cv_stride[1], other.CV(i, 0), tolerance);
        }
    }
    return rc;
}

bool ON_Brep::IsDuplicate(const ON_Brep &other, double tolerance) const {
...
bool ON_NurbsCurve::Reparameterize(double c) {
    ...
    return ON_ReparameterizeRationalNurbsCurve(c, m_dim, m_order, m_cv_count,
                                               m_cv_stride, m_cv, m_knot);
}

bool ON_ReparameterizeRationalNurbsCurve(double c, int dim, int order,
                                         int cv_count, int cvstride, double *cv,
                                         double *knot) {
    // Reference
...
bool ON_ChangeRationalNurbsCurveEndWeights(int dim, int order, int cv_count,
                                           int cvstride, double *cv,
                                           double *knot, double w0, double w1) {
    double r, s, v0, v1;
    int i, j;
    ...
    if (r != s) {
        ...
        if (ON_IsValid(v0) && ON_IsValid(v1) && 0.0 != v0) {
            ...
            if (!ON_IsValid(r))
                return false;
            if (!ON_ReparameterizeRationalNurbsCurve(r, dim, order, cv_count,
                                                     cvstride, cv, knot))
                return false;
        }
    }
