...
ON_OBJECT_IMPLEMENT(ON_RevSurface, ON_Surface,
...
ON_RevSurface *ON_RevSurface::New() { return new ON_RevSurface(); }
...
ON_RevSurface::ON_RevSurface()
    : m_curve(0), m_axis(ON_origin, ON_zaxis), m_angle(0.0, 2.0 * ON_PI),
      m_t(0.0, 2.0 * ON_PI), m_bTransposed(0) {
...
ON_BOOL32 ON_RevSurface::IsValid(ON_TextLog *text_log) const {
    ...
    if (!m_angle.IsIncreasing()) {
        if (text_log)
            text_log->Print("ON_RevSurface.m_angle = (%g,%g) (should be an "
                            "increasing interval)\n",
                            m_angle[0], m_angle[1]);
        return false;
    }
    ...
    if (length > 2.0 * ON_PI + ON_ZERO_TOLERANCE) {
        if (text_log)
            text_log->Print("ON_RevSurface.m_angle.Length() = %g (should be <= "
                            "2*pi radians).\n",
                            length);
        return false;
    }
    ...
    if (!m_t.IsIncreasing()) {
        if (text_log)
            text_log->Print("ON_RevSurface.m_t = (%g,%g) (should be an "
                            "increasing interval)\n",
                            m_t[0], m_t[1]);
        return false;
    }
...
int ON_RevSurface::Dimension() const { return 3; }
...
ON_BOOL32 ON_RevSurface::Evaluate( // returns false if unable to evaluate
    ...
    if (curve_dim == 2 || curve_dim == 3) {
        ...
        rc = m_curve->Evaluate(t, der_count, v_stride, v, curve_side, hint)
                 ? true
                 : false;
        if (rc) {
            ...
            // convert location coordinates to local frame with origin at
            // m_axis.from
            {
            ...
            for (i = 1; i <= der_count; i++) {
                dst_i = v_stride * ((i + 1) * (i + 2) / 2 - 1);
                pt = ON_3dPoint(v +
                                dst_i); // pt = curve derivative in world coords
                v[dst_i++] = pt * xaxis;
                v[dst_i++] = pt * yaxis;
                v[dst_i] = pt * zaxis;
            }
...
int ON_RevolutionTensor::DimensionA() const { return 2; }

int ON_RevolutionTensor::DimensionB() const { return 3; }

int ON_RevolutionTensor::DimensionC() const { return 3; }

bool ON_RevolutionTensor::Evaluate(double a, const double *ArcPoint, double b,
...
int ON_RevSurface::GetNurbForm(class ON_NurbsSurface &srf,
...
bool ON_RevSurface::GetSurfaceParameterFromNurbFormParameter(
    ...
    *surface_t = nurbs_t;
    ...
    if (m_curve) {
        if (!m_curve->GetCurveParameterFromNurbFormParameter(nurbs_t,
                                                             surface_t))
            rc = false;
    }
...
bool ON_RevSurface::GetNurbFormParameterFromSurfaceParameter(
    ...
    *nurbs_t = surface_t;
    ...
    if (m_curve) {
        if (!m_curve->GetNurbFormParameterFromCurveParameter(surface_t,
                                                             nurbs_t))
            rc = false;
    }
...
ON_Curve *ON_RevSurface::IsoCurve(int dir, double c) const {
    ...
    if (dir == 0) {
        ...
        circle.plane.yaxis =
            ON_CrossProduct(circle.plane.zaxis, circle.plane.xaxis);
...
ON_BOOL32 ON_RevSurface::Trim(int dir, const ON_Interval &domain) {
    ...
    if (dir == 0) {
        ...
        if (!dom.IsIncreasing() || !m_t.IsIncreasing() ||
            !m_angle.IsIncreasing())
            return false;
        ...
        if (fabs(d) > ON_ZERO_TOLERANCE &&
            fabs(d) <= 2.0 * ON_PI + ON_ZERO_TOLERANCE) {
            m_angle = a;
            m_t = domain;
            rc = true;
        }
...
bool ON_RevSurface::Extend(int dir, const ON_Interval &domain) {
    if (dir != 0 && dir != 1)
        return false;
    if (IsClosed(dir))
        return false;
...
ON_BOOL32 ON_RevSurface::Split(int dir, double c,
...
bool ON_RevSurface::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
...
ON_BOOL32 ON_RevSurface::IsSingular(
    int side) const // 0 = south, 1 = east, 2 = north, 3 = west
{
    ON_BOOL32 rc = false;
    ON_3dPoint P, Q;
    double d, tol;
...
ON_BOOL32 ON_RevSurface::IsPlanar(ON_Plane *plane, double tolerance) const {
    ON_BOOL32 rc = false;
    if (IsValid()) {
...
ON_BOOL32
ON_RevSurface::GetParameterTolerance( // returns tminus < tplus: parameters
                                      // tminus <= s <= tplus
...
ON_BOOL32 ON_RevSurface::SetDomain(int dir, // 0 sets first parameter's domain,
...
void ON_RevSurface::ClearBoundingBox() { m_bbox.Destroy(); }
...
ON_BOOL32 ON_RevSurface::GetBBox( // returns true if successful
    ...
    if (!rc) {
        ...
        if (rc) {
            // Dec 16 2010 - Chuck - if the angle range does not include 0,
            // m_curve is not part of the surface. bbox = cbox;
            ...
            for (i = 0; i < 8; i++) {
                P = corners[i];
                abox.Set(P, false);
                while (m_axis.ClosestPointTo(
                    P, &t)) // not a loop - used for flow control
                {
                    abox.Set(arc.plane.origin, true);
                    // If we cannot construct a valid arc, then P and the point
                    // on the axis are added to the bounding box.  One case
                    // where this happens is when P is on the axis of
                    // revolution.  See bug 84354.
                    ...
                    if (fabs(arc.plane.xaxis * arc.plane.zaxis) > 0.0001)
                        break;
                    arc.plane.yaxis =
                        ON_CrossProduct(arc.plane.zaxis, arc.plane.xaxis);
...
ON_BOOL32 ON_RevSurface::IsSpherical(ON_Sphere *sphere,
                                     double tolerance) const {
    ON_BOOL32 rc = false;
    if (m_curve) {
        ON_Plane plane;
            ...
            if (m_curve->IsArc(&plane, &arc, tolerance)) {
                ...
                if (P.DistanceTo(arc.Center()) <= tolerance) {
                    ...
                    if (sphere) {
                        ...
                        sphere->plane.xaxis = ON_CrossProduct(
                            sphere->plane.zaxis, sphere->plane.yaxis);
...
bool ON_Surface::IsSphere(ON_Sphere *sphere, double tolerance) const {
    ...
    ON_Arc arc0;
    int bIsArc0 =
        crv->IsArc(0, &arc0, tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
    delete crv;
    crv = 0;
    if (!bIsArc0)
        return false;
    ...
    ON_Arc arc1;
    int bIsArc1 =
        crv->IsArc(0, &arc1, tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
    delete crv;
    crv = 0;
    if (!bIsArc1)
        return false;
...
bool ON_Surface::IsCylinder(ON_Cylinder *cylinder, double tolerance) const {
    ...
    if (!rc && !rs) {
        ...
        ON_Line line;
        int bIsLine = 0;
        int bIsArc = crv->IsArc(
            0, &arc, tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
        if (!bIsArc) {
            bIsLine =
                crv->IsLinear(tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
            if (bIsLine) {
        ...
        if (tol < tolerance)
            tol = tolerance;
        double r =
            arc.plane.origin.DistanceTo(arc.plane.ClosestPointTo(line.from));
            ...
            for (int ii = i ? 1 : 0; ii <= 4; ii++) {
                ...
                for (int j = 0; j < sc1; j++) {
                    for (int jj = j ? 1 : 0; jj <= 4; jj++) {
                        v = 0.25 * ((4 - jj) * t[j] + jj * t[j + 1]);
                        P = PointAt(u, v);
                        r = arc.plane.origin.DistanceTo(
                            arc.plane.ClosestPointTo(P));
                        if (fabs(arc.radius - r) > tol) {
                            onfree(s);
                            return false;
                        }
...
bool ON_Surface::IsCone(ON_Cone *cone, double tolerance) const {
    ...
    int bIsLine = 0;
    int bIsArc =
        crv->IsArc(0, &arc, tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
    if (!bIsArc) {
        bIsLine =
            crv->IsLinear(tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
        if (bIsLine) {
    ...
    if (!crv)
        return false;
    if (!bIsArc)
        bIsArc = crv->IsArc(0, &arc,
                            tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
    else if (!bIsLine) {
        bIsLine =
            crv->IsLinear(tolerance > ON_ZERO_TOLERANCE ? tolerance : 0.0);
        if (bIsLine) {
...
static bool ON__IsCylConeHelper(const ON_Line &axis, const ON_Curve *curve,
                                double tolerance, ON_Plane &plane,
                                ON_Line &line, double r[2], double &h) {
    if (!axis.IsValid())
        return false;
    if (!curve)
        return false;
...
ON_BOOL32 ON_RevSurface::IsCylindrical(ON_Cylinder *cylinder,
                                       double tolerance) const {
    ON_Cylinder c;
    ON_Line line;
    double r[2] = {0.0, 0.0};
    ...
    if (!ON_IsValid(tolerance) || tolerance <= 0.0)
        tolerance = ON_ZERO_TOLERANCE;
    if (!ON__IsCylConeHelper(m_axis, m_curve, tolerance, c.circle.plane, line,
                             r, h))
        return false;
...
ON_BOOL32 ON_RevSurface::IsConical(ON_Cone *cone, double tolerance) const {
    ON_Cone c;
    ON_Line line;
    double r[2] = {0.0, 0.0};
    double h = 0.0;
...
static double ON_ClosestPointAngleHelper(const ON_Line &axis,
                                         const ON_Curve &curve,
                                         ON_Interval angle_domain,
                                         const ON_3dPoint &test_point,
                                         ON_3dPoint &curve_test_point) {
    // this assumes curve (revolute) is coplanar with axis and midpoint of curve
    // is not on the axis
    ...
    if (fabs(sin_angle) < ON_EPSILON)
        sin_angle = 0.0;
    if (fabs(cos_angle) < ON_EPSILON)
        cos_angle = 0.0;
    ...
    if (sin_angle != 0.0 || cos_angle != 0.0) {
        ...
        if (!angle_domain.Includes(angle)) {
            // NOTE: angle[0] and angle[1] can both be negative (-2pi,pi) for
            // example
...
double ON_ClosestPointAngle(const ON_Line &axis, const ON_Curve &curve,
                            ON_Interval angle_domain,
                            const ON_3dPoint &test_point,
                            ON_3dPoint &curve_test_point, double *sine_angle,
                            double *cosine_angle) {
    double angle = ON_ClosestPointAngleHelper(axis, curve, angle_domain,
                                              test_point, curve_test_point);
    if (!ON_IsValid(angle)) {
        // happens when test point is on the axis of revolution.
        // For example, bug 42808 when the cyl is picked first and the sphere
        // second. This behavior is long standing.
