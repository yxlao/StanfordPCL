...
bool ON_BezierCage::Read(ON_BinaryArchive &archive) {
    ...
    if (rc) {
        while (rc) {
            if (major_version != 1) {
                ON_ERROR("ON_BezierCage::Read - old code unable to read new "
                         "version of chunk");
                rc = false;
                break;
            }
...
ON_BezierCage::ON_BezierCage(int dim, bool is_rat, int order0, int order1,
                             int order2)
    : m_dim(0), m_is_rat(0), m_cv_capacity(0), m_cv(0) {
    m_order[0] = 0;
    m_order[1] = 0;
...
ON_BezierCage::ON_BezierCage(const ON_3dPoint *box_corners, int order0,
...
bool ON_BezierCage::IsValid() const {
    ...
    if (m_cv_capacity > 0 &&
        m_cv_capacity < cvdim * m_order[0] * m_order[1] * m_order[2])
        return false;
    ...
    if (!m_cv) {
        ...
        for (i = 0; i < m_order[0]; i++) {
            for (j = 0; j < m_order[1]; j++) {
                ...
                dump.PrintPointList(m_dim, m_is_rat, m_order[2], m_cv_stride[2],
                                    CV(i, j, 0), sPreamble);
            }
            if (i < m_order[0] - 1)
                dump.Print("\n");
...
int ON_BezierCage::Dimension() const { return m_dim; }

bool ON_BezierCage::Create(const ON_BoundingBox &bbox, int order0, int order1,
                           int order2) {
    /*
              7______________6
              |\             |\
...
bool ON_BezierCage::Create(int dim, bool is_rat, int order0, int order1,
                           int order2) {
    if (m_cv_capacity < 1)
        m_cv = 0;
    m_dim = (dim > 0) ? dim : 0;
...
bool ON_BezierCage::Create(const ON_3dPoint *box_corners, int order0,
                           int order1, int order2) {
    int i, j, k;
    double r, s, t;
...
bool ON_BezierCage::GetBBox( // returns true if successful
    ...
    int i, j;
    bool rc =
        (m_order[0] > 0 && m_order[1] > 0 && m_order[2] > 0) ? true : false;
    for (i = 0; rc && i < m_order[0]; i++)
        for (j = 0; rc && j < m_order[1]; j++) {
            rc = ON_GetPointListBoundingBox(m_dim, m_is_rat, m_order[2],
                                            m_cv_stride[2], CV(i, j, 0), boxmin,
                                            boxmax, bGrowBox);
            bGrowBox = true;
        }
    return rc;
...
bool ON_BezierCage::Transform(const ON_Xform &xform) {
    ...
    if (rc) {
        ...
        for (i = 0; rc && i < m_order[0]; i++) {
            for (j = 0; rc && j < m_order[1]; j++) {
                rc = ON_TransformPointList(m_dim, m_is_rat, m_order[2],
...
bool ON_BezierCage::Rotate(double sin_angle,        // sin(angle)
...
bool ON_BezierCage::Rotate(double angle,            // angle in radians
                           const ON_3dVector &axis, // axis of rotation
                           const ON_3dPoint &center // center of rotation
) {
...
bool ON_BezierCage::Evaluate(     // returns false if unable to evaluate
    ...
    i = cvdim * sizeof(*vtmp);
    vtmp = m_is_rat ? (i < ((int)(10 * 4 * sizeof(vtmparray[0])))
                           ? vtmparray
                           : (double *)alloca(i))
                    : v;
    memset(vtmp, 0, i);
...
ON_3dPoint ON_BezierCage::PointAt(double r, double s, double t) const {
    ON_3dPoint pt;
    if (m_dim <= 3) {
        pt.x = 0.0;
        pt.y = 0.0;
...
bool ON_BezierCage::IsRational() const { return m_is_rat ? true : false; }
...
bool ON_BezierCage::SetCV(int i, int j, int k, ON::point_style style,
                          const double *Point) {
    bool rc = true;
    int n;
    double w;
...
bool ON_BezierCage::GetCV(int i, int j, int k, ON::point_style style,
                          double *Point) const {
    const double *cv = CV(i, j, k);
    if (!cv)
        return false;
...
bool ON_BezierCage::MakeRational() {
    if (!IsRational()) {
        ...
        { for ( cvi = m_order[0]-1; cvi >= 0; cvi-- ) { old_cv =
        CV(cvi,cvj)+dim-1; *new_cv-- = 1.0; for ( j = 0; j < dim; j++ ) {
                  *new_cv-- = *old_cv--;
                }
              }
            }
...
bool ON_BezierCageMorph::Create(ON_3dPoint P0, ON_3dPoint P1, ON_3dPoint P2,
                                ON_3dPoint P3, int point_countX,
                                int point_countY, int point_countZ) {
    if (point_countX < 2 || point_countY < 2 || point_countZ < 2 ||
        !P0.IsValid() || !P1.IsValid() || !P2.IsValid() || !P3.IsValid()) {
    ...
    if (m_bValid) {
        ...
        box_corners[7] = P3 + Y;
        m_bValid = m_rst2xyz.Create(box_corners, point_countX, point_countY,
                                    point_countZ);
        m_xyz2rst = xform;
    } else {
        ON_ERROR("ON_BezierCageMorph::Create - invalid input - P0,P1,P2,P3 are "
                 "coplanar");
        m_rst2xyz.Destroy();
    }
    return m_bValid;
