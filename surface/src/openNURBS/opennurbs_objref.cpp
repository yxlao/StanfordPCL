...
void ON_COMPONENT_INDEX::Set(ON_COMPONENT_INDEX::TYPE type, int index) {
    m_type = type;
    m_index = index;
}
...
int ON_COMPONENT_INDEX::Compare(const ON_COMPONENT_INDEX *a,
...
ON_ObjRef::ON_ObjRef()
    : m_uuid(ON_nil_uuid), m_geometry(0), m_parent_geometry(0),
      m_geometry_type(ON::unknown_object_type), m_runtime_sn(0),
      m_point(ON_UNSET_POINT), m_osnap_mode(ON::os_none), m__proxy1(0),
      m__proxy2(0), m__proxy_ref_count(0) {}
...
ON_ObjRef::ON_ObjRef(const ON_ObjRef &src)
    : m_uuid(src.m_uuid), m_geometry(src.m_geometry),
      ...
      m_geometry_type(src.m_geometry_type), m_runtime_sn(src.m_runtime_sn),
      m_point(src.m_point), m_osnap_mode(src.m_osnap_mode), m_evp(src.m_evp),
      m__iref(src.m__iref), m__proxy1(src.m__proxy1), m__proxy2(src.m__proxy2),
...
bool ON_ObjRef::SetParentIRef(const ON_InstanceRef &iref, ON_UUID iref_id,
                              int idef_geometry_index) {
    bool rc = false;

    if (m__iref.Count() > 0) {
        // nested irefs
        if (0 == m__proxy2 || 0 == m__proxy_ref_count ||
            *m__proxy_ref_count <= 0) {
            return false;
        }
        ...
        if (m_geometry->ComponentIndex().m_type !=
        ...
        if (parent_brep) {
            ...
            const ON_Geometry *brep_component =
                proxy_brep->BrepComponent(m_component_index);
            if (!brep_component) {
...
void ON_ObjRef::SetProxy(ON_Object *proxy1, ON_Object *proxy2,
    ...
    if (0 != m__proxy_ref_count) {
        if (*m__proxy_ref_count > 1) {
            ...
            *m__proxy_ref_count = 0;
            ...
            ON_ERROR("ON_ObjRef::DecrementReferenceCount() *m__proxy_ref_count "
                     "<= 0");
        }
    }
...
ON_ObjRef_IRefID::ON_ObjRef_IRefID()
    : m_iref_uuid(ON_nil_uuid), m_iref_xform(0.0), m_idef_uuid(ON_nil_uuid),
      m_idef_geometry_index(0), m_geometry_xform(0.0) {}
