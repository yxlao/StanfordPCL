...
ON_3dPoint &ON_Line::operator[](int i) { return (i <= 0) ? from : to; }
...
double ON_Line::Length() const { return from.DistanceTo(to); }

ON_3dVector ON_Line::Direction() const { return (to - from); }
...
bool ON_Line::Rotate(double sin_angle,        // sin(angle)
...
bool ON_Line::Rotate(double a,                // angle in radians
                     const ON_3dVector &axis, // axis of rotation
                     const ON_3dPoint &center // center of rotation
) {
...
int ON_ArePointsOnLine( // returns 0=no, 1 = yes, 2 = pointset is (to tolerance)
...
bool ON_Line::GetBoundingBox(ON_BoundingBox &bbox, int bGrowBox) const {
...
bool ON_Line::IsFartherThan(double d, const ON_Line &L) const {
    ...
    if (!ON_Intersect(*this, L, &a, &b)) {
        ...
        if (Direction() * L.Direction() >= 0.0) {
            ...
            // If ( b >= 0.0), then this->from and L(b) are a pair of closest
            // points.
            if (b < 0.0) {
            ...
            // If ( b >= 0.0), then this->to and L(b) are a pair of closest
            // points.
            if (b < 0.0) {
