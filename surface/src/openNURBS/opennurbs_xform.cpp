...
static int Inv(const double *src, double dst[4][4], double *determinant,
               double *pivot) {
        ...
        if (x > 0.0) {
            ...
            if (x > 0.0) {
                ...
                if (x > 0.0) {
                    ...
                    // 17 August 2011 Dale Lear
                    //   The result is slightly more accurate when using
                    //   division instead of multiplying by the inverse of
                    //   M[3][3]. If there is any speed penalty at this point in
                    //   history, the accuracy is more important than the
                    //   additional clocks.
                    // c = 1.0/M[3][3];
                    // d *= c;
                    // ScaleRow( I, c, 3 );
...
ON_Xform::ON_Xform(const ON_3dPoint &P, const ON_3dVector &X,
...
ON_Xform ON_Xform::operator*(const ON_Xform &rhs) const {
    ...
    m[0][0] = m_xform[0][0] * p[0] + m_xform[0][1] * p[4] +
              m_xform[0][2] * p[8] + m_xform[0][3] * p[12];
    m[0][1] = m_xform[0][0] * p[1] + m_xform[0][1] * p[5] +
              m_xform[0][2] * p[9] + m_xform[0][3] * p[13];
    m[0][2] = m_xform[0][0] * p[2] + m_xform[0][1] * p[6] +
              m_xform[0][2] * p[10] + m_xform[0][3] * p[14];
    m[0][3] = m_xform[0][0] * p[3] + m_xform[0][1] * p[7] +
              m_xform[0][2] * p[11] + m_xform[0][3] * p[15];

    m[1][0] = m_xform[1][0] * p[0] + m_xform[1][1] * p[4] +
              m_xform[1][2] * p[8] + m_xform[1][3] * p[12];
    m[1][1] = m_xform[1][0] * p[1] + m_xform[1][1] * p[5] +
              m_xform[1][2] * p[9] + m_xform[1][3] * p[13];
    m[1][2] = m_xform[1][0] * p[2] + m_xform[1][1] * p[6] +
              m_xform[1][2] * p[10] + m_xform[1][3] * p[14];
    m[1][3] = m_xform[1][0] * p[3] + m_xform[1][1] * p[7] +
              m_xform[1][2] * p[11] + m_xform[1][3] * p[15];

    m[2][0] = m_xform[2][0] * p[0] + m_xform[2][1] * p[4] +
              m_xform[2][2] * p[8] + m_xform[2][3] * p[12];
    m[2][1] = m_xform[2][0] * p[1] + m_xform[2][1] * p[5] +
              m_xform[2][2] * p[9] + m_xform[2][3] * p[13];
    m[2][2] = m_xform[2][0] * p[2] + m_xform[2][1] * p[6] +
              m_xform[2][2] * p[10] + m_xform[2][3] * p[14];
    m[2][3] = m_xform[2][0] * p[3] + m_xform[2][1] * p[7] +
              m_xform[2][2] * p[11] + m_xform[2][3] * p[15];

    m[3][0] = m_xform[3][0] * p[0] + m_xform[3][1] * p[4] +
              m_xform[3][2] * p[8] + m_xform[3][3] * p[12];
    m[3][1] = m_xform[3][0] * p[1] + m_xform[3][1] * p[5] +
              m_xform[3][2] * p[9] + m_xform[3][3] * p[13];
    m[3][2] = m_xform[3][0] * p[2] + m_xform[3][1] * p[6] +
              m_xform[3][2] * p[10] + m_xform[3][3] * p[14];
    m[3][3] = m_xform[3][0] * p[3] + m_xform[3][1] * p[7] +
              m_xform[3][2] * p[11] + m_xform[3][3] * p[15];
...
void ON_Xform::Zero() { memset(m_xform, 0, sizeof(m_xform)); }
...
void ON_Xform::Scale(const ON_Plane &plane, double x_scale_factor,
...
void ON_Xform::Shear(const ON_Plane &plane, const ON_3dVector &x1,
...
void ON_Xform::PlanarProjection(const ON_Plane &plane) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        q[i] =
            m_xform[i][0] * p[0] + m_xform[i][1] * p[1] + m_xform[i][2] * p[2];
    }
    for (i = 0; i < 3; i++) {
        m_xform[3][i] = 0.0;
        m_xform[i][3] = p[i] - q[i];
...
void ON_Xform::ActOnLeft(double x, double y, double z, double w,
                         double v[4]) const {
    if (v) {
        v[0] = m_xform[0][0] * x + m_xform[0][1] * y + m_xform[0][2] * z +
               m_xform[0][3] * w;
        v[1] = m_xform[1][0] * x + m_xform[1][1] * y + m_xform[1][2] * z +
               m_xform[1][3] * w;
        v[2] = m_xform[2][0] * x + m_xform[2][1] * y + m_xform[2][2] * z +
               m_xform[2][3] * w;
        v[3] = m_xform[3][0] * x + m_xform[3][1] * y + m_xform[3][2] * z +
               m_xform[3][3] * w;
    }
}

void ON_Xform::ActOnRight(double x, double y, double z, double w,
                          double v[4]) const {
    if (v) {
        v[0] = m_xform[0][0] * x + m_xform[1][0] * y + m_xform[2][0] * z +
               m_xform[3][0] * w;
        v[1] = m_xform[0][1] * x + m_xform[1][1] * y + m_xform[2][1] * z +
               m_xform[3][1] * w;
        v[2] = m_xform[0][2] * x + m_xform[1][2] * y + m_xform[2][2] * z +
               m_xform[3][2] * w;
        v[3] = m_xform[0][3] * x + m_xform[1][3] * y + m_xform[2][3] * z +
               m_xform[3][3] * w;
    }
}
    ...
    if (m_xform[3][0] != 0.0 || m_xform[3][1] != 0.0 || m_xform[3][2] != 0.0 ||
        ...
        if (fabs(det) <= ON_SQRT_EPSILON) {
            ...
            if (sx == 0.0 || sy == 0.0 || sz == 0.0 || fabs(sx - sy) > tol ||
                ...
                double yz = (Y * Z) / (sy * sz);
                double zx = (Z * X) / (sz * sx);
                if (fabs(xy) > dottol || fabs(yz) > dottol ||
                    fabs(zx) > dottol) {
                    // shear or worse
                    rc = 0;
                } else {
...
double ON_Xform::GetSurfaceNormalXform(ON_Xform &N_xform) const {
    ...
    memcpy(&N_xform.m_xform[0][0], &m_xform[0][0],
           3 * sizeof(N_xform.m_xform[0][0]));
    N_xform.m_xform[0][3] = 0.0;
    memcpy(&N_xform.m_xform[1][0], &m_xform[1][0],
           3 * sizeof(N_xform.m_xform[0][0]));
    N_xform.m_xform[1][3] = 0.0;
    memcpy(&N_xform.m_xform[2][0], &m_xform[2][0],
           3 * sizeof(N_xform.m_xform[0][0]));
...
void ON_Xform::Rotation(double angle,
...
void ON_Xform::Rotation(ON_3dVector start_dir, ON_3dVector end_dir,
...
void ON_Xform::Rotation(double sin_angle, double cos_angle, ON_3dVector axis,
                        ON_3dPoint center) {
    Identity();
    ...
    if (fabs(sin_angle) >= 1.0 - ON_SQRT_EPSILON &&
        ...
        if (sin_angle < 0.0)
            ...
            sin_angle = 1.0;
    } else if (fabs(cos_angle) >= 1.0 - ON_SQRT_EPSILON &&
        ...
        if (cos_angle < 0.0)
            ...
            cos_angle = 1.0;
    } else if (fabs(cos_angle * cos_angle + sin_angle * sin_angle - 1.0) >
               ON_SQRT_EPSILON) {
    ...
    if (sin_angle != 0.0 || cos_angle != 1.0) {
        ...
        if (center.x != 0.0 || center.y != 0.0 || center.z != 0.0) {
            m_xform[0][3] =
                -((m_xform[0][0] - 1.0) * center.x + m_xform[0][1] * center.y +
                  m_xform[0][2] * center.z);
            m_xform[1][3] =
                -(m_xform[1][0] * center.x + (m_xform[1][1] - 1.0) * center.y +
                  m_xform[1][2] * center.z);
            m_xform[2][3] =
                -(m_xform[2][0] * center.x + m_xform[2][1] * center.y +
...
void ON_Xform::Rotation(
    const ON_3dVector
        &X0, // initial frame X (X,Y,Z = right handed orthonormal frame)
...
void ON_Xform::Rotation(const ON_Plane &plane0, const ON_Plane &plane1) {
    Rotation(plane0.origin, plane0.xaxis, plane0.yaxis, plane0.zaxis,
...
bool ON_Xform::CameraToClip(ON_BOOL32 bPerspective, double left, double right,
                            double bottom, double top, double near_dist,
                            double far_dist) {
    double dd;
    ...
    if (!bPerspective) {
        ...
        // = m_xform[0][2] = 0.0; d = 1.0/(bottom-top); m_xform[1][1] = -2.0*d;
        // m_xform[1][3] = (bottom+top)*d; m_xform[1][0] = m_xform[1][2] = 0.0;
        // d = 1.0/(far_dist-near_dist);
        // m_xform[2][2] = 2.0*d;  m_xform[2][3] = (far_dist+near_dist)*d;
        // m_xform[2][0] = m_xform[2][1] = 0.0; m_xform[3][0] = m_xform[3][1] =
        // m_xform[3][2] = 0.0; m_xform[3][3] = 1.0;
...
bool ON_Xform::ClipToCamera(ON_BOOL32 bPerspective, double left, double right,
                            double bottom, double top, double near_dist,
                            double far_dist) {
    // see comments in tl2_xform.h for details.
    double dd;
    ...
    if (!bPerspective) {
        ...
        // m_xform[2][0] = m_xform[2][1] = m_xform[2][2] = 0.0; m_xform[2][3] =
        // -1.0;
...
bool ON_Xform::ClipToScreen(double left, double right, double bottom,
                            double top, double near_z, double far_z) {
    // see comments in tl2_xform.h for details.
...
bool ON_Xform::ScreenToClip(double left, double right, double bottom,
                            double top, double near_z, double far_z) {
    // see comments in tl2_xform.h for details.
    ON_Xform c2s;
...
bool ON_Xform::IntervalChange(int dir, ON_Interval old_interval,
    ...
    if (dir >= 0 && dir <= 3 && old_interval[0] != ON_UNSET_VALUE &&
        ...
        if (new_interval != old_interval) {
            ...
            ;
            double d = (new_interval[0] * old_interval[1] -
                        new_interval[1] * old_interval[0]) /
                       old_interval.Length();
            m_xform[dir][dir] = s;
            m_xform[dir][3] = d;
        }
