...
double ON_ArrayDotDifference(int dim, const double *A, const double *B,
                             const double *C) {
    // returns A o ( B - C )
...
void ON_Array_aA_plus_B(int dim, double a, const double *A, const double *B,
                        double *aA_plus_B) {
    if (dim > 0) {
        while (dim--)
            *aA_plus_B++ = a * *A++ + *B++;
...
void ON_Array_aA_plus_B(int dim, float a, const float *A, const float *B,
                        float *aA_plus_B) {
    if (dim > 0) {
        while (dim--)
            *aA_plus_B++ = a * *A++ + *B++;
...
int ON_DecomposeVector(const ON_3dVector &V, const ON_3dVector &A,
                       const ON_3dVector &B, double *x, double *y) {
    int rank;
    double pr;
...
ON_Pullback3dVector( // use to pull 3d vector back to surface parameter space
    ...
    double distance, // signed distance from vector location to closet point on
                     // surface < 0 if point is below with respect to Du x Dv
    ...
    if (distance != 0.0) {
        ...
        if (rc) {
            ...
            rc = ON_DecomposeVector(vector, ds + distance * ns,
                                    dt + distance * nt, &pullback.x,
                                    &pullback.y);
        }
    } else {
...
ON_GetParameterTolerance(
    ...
    if (rc) {
        if (t < t0)
            ...
            t = t1;
        double dt = (t1 - t0) * 8.0 * ON_SQRT_EPSILON +
                    (fabs(t0) + fabs(t1)) * ON_EPSILON;
        if (dt >= t1 - t0)
            dt = 0.5 * (t1 - t0);
...
ON_EvNormal(int limit_dir, const ON_3dVector &Du, const ON_3dVector &Dv,
            const ON_3dVector &Duu, const ON_3dVector &Duv,
            const ON_3dVector &Dvv, ON_3dVector &N) {
    ...
    if (ON_EvJacobian(DuoDu, DuoDv, DvoDv, NULL)) {
         ...
         * Du X Dv = 0.  Then
         *
         * |Su X Sv|^2 = u^2*AoA + 2uv*AoB + v^2*BoB + cubic and higher order
         * terms
         *
         ...
         *
         * All I need is the direction, so I just need to compute a*A + b*B as
         * carefully as possible.  Note that (3)  a*A + b*B = Du X (a*Duv +
         * b*Dvv)  - Dv X (a*Duu + b*Duv). Formaula (3) requires fewer flops
         * than using formulae (1) and (2) to compute a*A + b*B.  In addition,
         * when |Du| << |Dv| or |Du| >> |Dv|, formula (3) reduces the number of
         * subtractions between numbers of similar size.  Since the (nearly)
         * zero first partial is the most common is more common than the
         * (nearly) (anti) parallel case, I'll use formula (3).  If you're
         * reading this because you're not getting the right answer and you
         * can't find any bugs, you might want to try using formulae (1) and
         * (2).
         *
         * The limit_dir argument determines which direction is used to compute
         * the limit.
         *                  |
...
bool ON_EvTangent(const ON_3dVector &D1, // first derivative
...
bool ON_EvSectionalCurvature(const ON_3dVector &S10, const ON_3dVector &S01,
                             const ON_3dVector &S20, const ON_3dVector &S11,
                             const ON_3dVector &S02,
                             const ON_3dVector &planeNormal, ON_3dVector &K) {
    ...
    // M1 = ON_CrossProduct(a*S20 + b*S11, S01) + ON_CrossProduct(S10, a*S11 +
    // b*S02);
...
ON_BOOL32 ON_IsContinuous(ON::continuity desired_continuity, ON_3dPoint Pa,
                          ON_3dVector D1a, ON_3dVector D2a, ON_3dPoint Pb,
                          ON_3dVector D1b, ON_3dVector D2b,
...
#define MAX_HALF_N 26
    static const double bc[((MAX_HALF_N - 2) * (MAX_HALF_N - 1)) / 2 +
                           MAX_HALF_N - 2] = {15.0,
                                              20.0,
                                              28.0,
                                              56.0,
    ...
    if (n % 2)
        return ON_BinomialCoefficient(i - 1, j) +
               ON_BinomialCoefficient(i, j - 1);
...
ON_DECL
double ON_TrinomialCoefficient(int i, int j, int k) {
    return (ON_BinomialCoefficient(i, j + k) * ON_BinomialCoefficient(j, k));
}

ON_BOOL32
ON_IsValidPointList(int dim, int is_rat, int count, int stride,
                    const float *p) {
    return (dim > 0 && stride >= (is_rat ? (dim + 1) : dim) && count >= 0 &&
            p != NULL)
               ? true
               : false;
}
...
ON_BOOL32
ON_IsValidPointGrid(int dim, ON_BOOL32 is_rat, int point_count0,
                    int point_count1, int point_stride0, int point_stride1,
                    const double *p) {
...
bool ON_ReversePointList(int dim, int is_rat, int count, int stride,
                         double *p) {
...
ON_BOOL32
ON_ReversePointGrid(int dim, ON_BOOL32 is_rat, int point_count0,
                    int point_count1, int point_stride0, int point_stride1,
                    double *p, int dir) {
    ON_BOOL32 rc = false;
    if (!dir) {
        rc = ON_ReversePointGrid(dim, is_rat, point_count1, point_count0,
                                 point_stride1, point_stride0, p, 1);
    } else {
        int i;
...
bool ON_SwapPointListCoordinates(int count, int stride, float *p, int i,
                                 int j) {
    float t;
    int k;
...
bool ON_SwapPointListCoordinates(int count, int stride, double *p, int i,
                                 int j) {
    double t;
    int k;
    if (!ON_IsValidPointList(stride, 0, count, stride, p))
...
ON_BOOL32
ON_SwapPointGridCoordinates(int point_count0, int point_count1,
                            int point_stride0, int point_stride1, double *p,
...
bool ON_TransformPointList(int dim, int is_rat, int count, int stride,
                           float *point, const ON_Xform &xform) {
    bool rc = true;
    double x, y, z, w;
    ...
    if (is_rat) {
        ...
        case 1:
            while (count--) {
                x = xform.m_xform[0][0] * point[0] +
                    xform.m_xform[0][3] * point[1];
                w = xform.m_xform[3][0] * point[0] +
                    xform.m_xform[3][3] * point[1];
                point[0] = (float)x;
                point[1] = (float)w;
                point += stride;
            }
        ...
        case 2:
            while (count--) {
                x = xform.m_xform[0][0] * point[0] +
                    xform.m_xform[0][1] * point[1] +
                    xform.m_xform[0][3] * point[2];
                y = xform.m_xform[1][0] * point[0] +
                    xform.m_xform[1][1] * point[1] +
                    xform.m_xform[1][3] * point[2];
                w = xform.m_xform[3][0] * point[0] +
                    xform.m_xform[3][1] * point[1] +
        ...
        default: // dim >= 3
            while (count--) {
                x = xform.m_xform[0][0] * point[0] +
                    xform.m_xform[0][1] * point[1] +
                    xform.m_xform[0][2] * point[2] +
                    xform.m_xform[0][3] * point[dim];
                y = xform.m_xform[1][0] * point[0] +
                    xform.m_xform[1][1] * point[1] +
                    xform.m_xform[1][2] * point[2] +
                    xform.m_xform[1][3] * point[dim];
                z = xform.m_xform[2][0] * point[0] +
                    xform.m_xform[2][1] * point[1] +
                    xform.m_xform[2][2] * point[2] +
                    xform.m_xform[2][3] * point[dim];
                w = xform.m_xform[3][0] * point[0] +
                    xform.m_xform[3][1] * point[1] +
                    xform.m_xform[3][2] * point[2] +
                    xform.m_xform[3][3] * point[dim];
                point[0] = (float)x;
                point[1] = (float)y;
                point[2] = (float)z;
                point[dim] = (float)w;
        ...
        case 2:
            while (count--) {
                w = xform.m_xform[3][0] * point[0] +
                    xform.m_xform[3][1] * point[1] + xform.m_xform[3][3];
                if (w == 0.0) {
                    rc = false;
                    w = 1.0;
                } else
                    w = 1.0 / w;
                x = xform.m_xform[0][0] * point[0] +
                    xform.m_xform[0][1] * point[1] + xform.m_xform[0][3];
                y = xform.m_xform[1][0] * point[0] +
                    xform.m_xform[1][1] * point[1] + xform.m_xform[1][3];
                point[0] = (float)(w * x);
                point[1] = (float)(w * y);
                point += stride;
            }
...
bool ON_TransformPointList(int dim, int is_rat, int count, int stride,
                           double *point, const ON_Xform &xform) {
    bool rc = true;
    double x, y, z, w;
...
ON_TransformPointGrid(int dim, int is_rat, int point_count0, int point_count1,
    ...
    double *pt = point;
    for (i = 0; i < point_count0; i++) {
        if (!ON_TransformPointList(dim, is_rat, point_count1, point_stride1, pt,
                                   xform)) {
            rc = false;
        } else if (!i) {
            rc = true;
...
ON_TransformVectorList(int dim, int count, int stride, float *vector,
    ...
    case 2:
        while (count--) {
            x = xform.m_xform[0][0] * vector[0] +
                xform.m_xform[0][1] * vector[1];
            y = xform.m_xform[1][0] * vector[0] +
                xform.m_xform[1][1] * vector[1];
            vector[0] = (float)x;
            vector[1] = (float)y;
            vector += stride;
        }
    ...
    default: // dim >= 3
        while (count--) {
            ...
            z = xform.m_xform[2][0] * vector[0] +
                xform.m_xform[2][1] * vector[1] +
...
int ON_ComparePointList( // returns
                         ...
                         //  0: first == second
                         // +1: first > second
    int dim, ON_BOOL32 is_rat, int point_count, int point_strideA,
    const double *pointA, int point_strideB, const double *pointB) {
    int i, rc = 0, rc1 = 0;
...
ON_BOOL32
ON_IsPointListClosed(int dim, int is_rat, int count, int stride,
                     const double *p) {
    ON_BOOL32 rc = false;
    if (count >= 4 &&
        0 == ON_ComparePoint(dim, is_rat, p, p + stride * (count - 1))) {
        // a bunch of points piled on top of each other is not considered to be
        // closed.
...
ON_BOOL32
ON_IsPointGridClosed(int dim, ON_BOOL32 is_rat, int point_count0,
                     int point_count1, int point_stride0, int point_stride1,
                     const double *p, int dir) {
    ON_BOOL32 rc = false;
    if (point_count0 > 0 && point_count1 > 0 && p != NULL) {
        ...
        rc = (0 ==
              ON_ComparePointList(dim, is_rat, count, stride, p0, stride, p1))
                 ? true
                 : false;
    }
    return rc;
...
int ON_SolveQuadraticEquation(double a, double b, double c, double *r0,
                              double *r1)
...
int ON_Solve2x2(double m00, double m01, double m10, double m11, double d0,
                double d1, double *x_addr, double *y_addr, double *pivot_ratio)
...
int ON_Solve3x2(const double col0[3], const double col1[3], double d0,
                double d1, double d2, double *x_addr, double *y_addr,
                double *err_addr, double *pivot_ratio)
...
double ON_SolveNxN(bool bFullPivot, bool bNormalize, int n, double *M[],
                   double B[], double X[]) {
    if (n <= 0 || 0 == M || 0 == B || 0 == X)
        return 0.0;
    ...
    for (n0 = 0; n0 < n; n0++) {
        ...
        x = 1.0 / M[n0][n0];
        // M[n0][n0] = 1.0; // cosmetic because M[n0][n0] will never be used
        // again
        B[n0] *= x;
        for (j = n0 + 1; j < n; j++)
            M[n0][j] *= x;
        ...
        for (i = n0 + 1; i < n; i++) {
            x = -M[i][n0];
            if (0.0 != x) {
                // M[i][n0] = 0.0; // cosmetic because M[i][n0] will never be
                // used again
                B[i] += x * B[n0];
                for (j = n0 + 1; j < n; j++) {
                    M[i][j] += x * M[n0][j];
    ...
    for (j = n - 1; j >= 0; j--) {
        for (i = 0; i < j; i++) {
            ...
            if (x != 0) {
                B[i] += x * B[j];
                // M[i][j] += x; // cosmetic because M[i][j] will never be used
                // again
            }
        }
    }
...
int ON_Solve4x4(const double row0[4], const double row1[4],
                const double row2[4], const double row3[4], double d0,
                double d1, double d2, double d3, double *x_addr, double *y_addr,
                double *z_addr, double *w_addr, double *pivot_ratio) {
...
int ON_Solve3x3(const double row0[3], const double row1[3],
                const double row2[3], double d0, double d1, double d2,
...
void ON_Sort(ON::sort_algorithm method, int *index, const void *data,
             size_t count, size_t sizeof_element,
...
OUTPUT:
  index
    Sorted so that datum at index[i] is less than or equal to datum at
index[i+1] COMMENTS: If you want the data sorted in place, then use ON_qsort()
or ON_hsort(). EXAMPLE:
  // ...
REFERENCE:
  KNUTH
*/
{
    ...
    isizeof_element =
        (unsigned int)sizeof_element; // (int) converts 64 bit size_t
    ...
    if (ON::quick_sort == method) {
        ...
        for (;;) {
            ...
            while (j <= icount) {
                if (j < icount &&
                    context.compar2(context.qdata + idx[j],
                                    context.qdata + idx[j + 1]) < 0) {
                    j++;
                }
...
void ON_Sort(ON::sort_algorithm method, int *index, const void *data,
             size_t count, size_t sizeof_element,
             int (*compar)(const void *, const void *, void *), void *p) {
    tagON_SORT_CONTEXT context;
    unsigned int *idx;
    const void *tmp;
    unsigned int i, j, k, tmpi, icount, isizeof_element;
    ...
    if (ON::quick_sort == method) {
        ...
        for (;;) {
            ...
            while (j <= icount) {
                ...
                if (context.compar3(tmp, context.qdata + idx[j],
                                    context.users_context) < 0) {
                    idx[i] = idx[j];
                    i = j;
                    j = (j << 1) + 1;
...
const double *ON_BinarySearchDoubleArray(double key, const double *base,
                                         size_t nel) {
    if (nel > 0 && base) {
        size_t i;
        double d;
    ...
    (t)*f    (t))
                      ---------------------------------------------------------------------
                                               w(t)
                                                                          ...
                                                                          (i)
    (j) (The missing summands look like  ON_BinomialCoefficient(i,j)*w   * f )
    */
...
Parameters:
  ...
  v - [in/out]
    ...
    The input array contains derivatives of the numerator and denominator
                functions in the order X, W, Xs, Ws, Xt, Wt, Xss, Wss, Xst, Wst,
Xtt, Wtt, ... In general, the (i,j)-th derivatives are in the (dim+1) entries of
v[] v[k], ..., answer[k+dim], where	k = ((i+j)*(i+j+1)/2 + j)*v_stride. In
the output v[] array the derivatives of X are replaced with the derivatives of F
and the derivatives of W are divided by w = v[dim]. Returns: True if input is
valid; i.e., v[dim] != 0. See Also: ON_EvaluateQuotientRule
  ON_EvaluateQuotientRule3
*/
...
bool ON_EvaluateQuotientRule2(int dim, int der_count, int v_stride, double *v) {
    ...
    if (der_count) {
        ...
        if (der_count > 1) {
            ...
            x = v +
                3 * v_stride; // x = Xss, x[v_stride] = Xst, x[2*v_stride] = Xtt
            wss = -x[dim];    // wss = -wss/W
            ...
            if (der_count > 2) {
                ...
                for (n = 3; n <= der_count; n++) {
                    for (j = 0; j <= n; j++) {
                        // f = Ds^i Dt^j
                        // 13 Jan 2005 Dale Lear bug fix - added missing a
                        ...
                        for (ii = 0; ii <= i; ii++) {
                            ...
                            for (jj = ii ? 0 : 1; jj <= j; jj++) {
                                ...
                                wt = -ws * ON_BinomialCoefficient(jj, j - jj) *
                                     v[Fn];
                                q = n - q;
                                Fn = ((q * (q + 1)) / 2 + j - jj) *
                                     v_stride; // X(i-ii,j-jj) = v[Fn]
                                for (q = 0; q < dim; q++)
                                    f[q] += wt * v[Fn + q];
                            }
                        }
...
bool ON_EvaluateQuotientRule3(int dim, int der_count, int v_stride, double *v) {
    ...
    if (der_count) {
        ...
        if (der_count > 1) {
            // 2nd derivatives
            f = v; // f = F, f[v_stride] = Fr, f[2*v_stride] = Fs, f[3*v_stride]
                   // = Ft
                    ...
                    for (i = n; i >= 0; i--) {
                        for (j = n - i; j >= 0; j--) {
                            ...
                            for (ii = 0; ii <= i; ii++) {
                                ...
                                for (jj = 0; jj <= j; jj++) {
                                    ws =
                                        wr * ON_BinomialCoefficient(jj, j - jj);
                                    for (kk = (ii || jj) ? 0 : 1; kk <= k;
                                         kk++) {
                                        q = ii + jj + kk;
                                        Fn = q - ii;
                                        Fn = v_stride *
                                                 (q * (q + 1) * (q + 2) / 6 +
                                                  Fn * (Fn + 1) / 2 + kk) +
                                             dim;
                                        // wt = -(i choose ii)*(j choose jj)*(k
                                        // choose kk)*W(ii,jj,kk)
                                        wt =
                                            -ws *
                                            ON_BinomialCoefficient(kk, k - kk) *
                                            v[Fn];
                                        q = n - q;
                                        Fn = q - i + ii;
...
void ON_EPC_WARNING(const char *msg) { ON_Warning(__FILE__, __LINE__, msg); }
...
ON_BOOL32 ON_EvPrincipalCurvatures(
    ...
    return ON_EvPrincipalCurvatures(Ds, Dt, l, m, n, N, gauss, mean, kappa1,
                                    kappa2, K1, K2);
}
...
ON_BOOL32 ON_EvPrincipalCurvatures(
    ...
    // return ON_WRONG_EvPrincipalCurvatures( Ds, Dt, Dss, Dst, Dtt, N, gauss,
    // mean, kappa1, kappa2, K1, K2 );
    ...
    const double trace =
        (g * l - 2.0 * f * m + e * n) * x; // = 2*(mean curvature)
    if (gauss)
        *gauss = det;
    if (mean)
        *mean = 0.5 * trace;

    {
        ...
        double tol =
            fabs(det) *
            1.0e-12; // 31 March 2008 Dale Lear - added tol to fix 32091
        if (x < 4.0 * det - tol) {
            if (det <= ON_EPSILON) {
                k1 = k2 = 0.0;
                if (gauss)
        ...
        if (fabs(gggg - det) > 1.0e-4 * fabs(det)) {
            ON_EPC_WARNING(
                "ON_EvPrincipalCurvatures() Det(shape op) != gaussian");
        }
        if (fabs(tttt - trace) > 1.0e-4 * fabs(trace)) {
            ON_EPC_WARNING(
                "ON_EvPrincipalCurvatures() Trace(shape op) != trace");
        }
...

        {
            ...
            if (fabs(k1 - k2) > 1.0e-6 * (fabs(k1) + fabs(k2))) {
                ...
                // use ShapeOp(Ds) = d(N) along s, etc., to figure out that with
                // respect to Du,Dv basis for tangent space, ShapeOp is given by
                // the matrix
                //
                //            a  b
                // ShapeOp =
                ...
                //  Since I'm looking for eigen vectors, I can ignore scale
                //  factor "x".
                // So I need to solve
                //
                //          a   b
                ...
                for (ki = 0; ki < 2; ki++) {
...
#if defined(ON_TEST_EV_KAPPAS)
                    x = (a - kk) * (d - kk) -
                        b * c; // kk = eigen value of ShapeOp means
                               // x should be zero
                    if (fabs(x) > 1.0e-8) {
                        if (0 == ki) {
                            ON_EPC_WARNING(
                                "ON_EvPrincipalCurvatures() |det(shape op - "
                                "[k1,k1])| > 1.0e-8");
                        } else {
                            ON_EPC_WARNING(
                                "ON_EvPrincipalCurvatures() |det(shape op - "
                    ...
                    if (fabs(x) > 1.0e-8 || fabs(y) > 1.0e-8) {
                        if (0 == ki) {
                            ON_EPC_WARNING("ON_EvPrincipalCurvatures() (shape "
                                           "op k1 eigenvector is noisy).");
                        } else {
                            ON_EPC_WARNING("ON_EvPrincipalCurvatures() (shape "
                                           "op k2 eigenvector is noisy).");
                        }
                    }
#endif
                ...
                bFixK1 = bFixK2 = false;
                {
                    ...
                    if (fabs(x) >= 1.0e-4) {
                        ON_EPC_WARNING(
                            "ON_EvPrincipalCurvatures() K1*N > 1.0e-4.");
                        bFixK1 = true;
                    }
                    ...
                    if (fabs(x) >= 1.0e-4) {
                        ON_EPC_WARNING(
                            "ON_EvPrincipalCurvatures() K2*N > 1.0e-4.");
                        bFixK2 = true;
                    }
                }
                ...
                if (!bFixK1 && !bFixK2) {
                    ...
                    if (fabs(x) >= 1.0e-4) {
#if defined(ON_TEST_EV_KAPPAS)
                        {
                            ...
                            if (!bSecondTry) {
                                ON_EPC_WARNING(
                                    "ON_EvPrincipalCurvatures() K1*K2 > 0.1.");

                                // 15 July 2005 - Dale Lear
                                //        There is a bug in converting the 2d
                                //        eigenvectors Into 3d K1 and K2.  I'll
                                //        look into it later.
                                bSecondTry = true;
                                double ggg, mmm, kkk1, kkk2;
                                ON_3dVector KKK1, KKK2;
                                ON_EvPrincipalCurvatures(Ds, Dt, Dss, Dst, Dtt,
                                                         N, &ggg, &mmm, &kkk1,
                                                         &kkk2, KKK1, KKK2);
                                bSecondTry = false;
                            }
...
ON_3dVector ON_NormalCurvature(const ON_3dVector &S10, const ON_3dVector &S01,
                               const ON_3dVector &S20, const ON_3dVector &S11,
                               const ON_3dVector &S02,
...
OUTPUT:
   The return value is the normal curvature vector for the surface in the
direction of UnitTangent Normal curvature vector ((anti)parallel to UnitNormal).
...
*****************************************************************************/
{
    ...
    a = b = 0.0;
    // solve T = a*S10 + b*S01
    rc = ON_Solve3x2(S10, S01, UnitTangent.x, UnitTangent.y, UnitTangent.z, &a,
                     &b, &e, &pr);
    if (rc < 2) {
        NormalCurvature.Zero();
    } else {
        // compute 2nd derivative of 3d curve(t) = srf( u0 + a*t, v0 + b*t ) at
        // t=0
...
bool ON_GetPolylineLength(int dim, ON_BOOL32 is_rat, int count, int stride,
                          const double *P, double *length)
/* first point  = {P[0], ... }
...
#define SUM_SIZE 128
    ...
    if (stride == 0)
        stride = (dim + is_rat);
    if (dim < 1 || count < 2 || stride < ((is_rat) ? dim + 1 : dim) || !P ||
        !length)
        return false;
...
int ON_Evaluator::EvaluateHessian(const double *parameters, double *value,
    ...
    if (m_parameter_count == 1) {
        ...
        if (0 != hessian) {
            ...
            for (i = 0; i < m_parameter_count; i++) {
                memset(hessian[0], 0,
                       m_parameter_count * sizeof(hessian[i][0]));
            }
        }
    }
...
bool ON_Evaluator::Periodic(int parameter_index) const {
    return ((m_parameter_count == m_bPeriodicParameter.Count() &&
             m_parameter_count > 0)
...
double ON_Max(double a, double b) { return (a < b) ? b : a; }

float ON_Max(float a, float b) { return (a < b) ? b : a; }

int ON_Max(int a, int b) { return (a < b) ? b : a; }

double ON_Min(double a, double b) { return (a < b) ? a : b; }

float ON_Min(float a, float b) { return (a < b) ? a : b; }

int ON_Min(int a, int b) { return (a < b) ? a : b; }
