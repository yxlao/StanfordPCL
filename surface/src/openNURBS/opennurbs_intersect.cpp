...
bool ON_IntersectLineLine(const ON_Line &lineA, const ON_Line &lineB, double *a,
...
bool ON_Intersect(const ON_BoundingBox &bbox, const ON_Line &line, double tol,
    ...
    if (fabs(d) < 1.0 && (fabs(mn) >= fabs(d) * M || fabs(mx) >= fabs(d) * M)) {
        // the value of mn/d or mx/d is too large for a realistic answer to be
        // computed
        return false;
    }
    d = 1.0 / d;
    ...
    const int j =
        (i + (fabs(v[(i + 1) % 3]) > fabs(v[(i + 2) % 3]) ? 1 : 2)) % 3;
    a = chord.from[j];
    b = chord.to[j];
    mn = bbox.m_min[j];
    mx = bbox.m_max[j];
...
bool ON_Intersect(const ON_Line &lineA, const ON_Line &lineB,
                  double *lineA_parameter, double *lineB_parameter) {
...
bool ON_Intersect(const ON_Line &line, const ON_Plane &plane,
    ...
    if (d == 0.0) {
        ...
        fd = fabs(d);
        if (fd > 1.0 &&
            (fabs(a) >= ON_DBL_MAX / fd || fabs(b) >= ON_DBL_MAX / fd)) {
...
bool ON_Intersect(const ON_Plane &R, const ON_Plane &S, const ON_Plane &T,
                  ON_3dPoint &P) {
    double pr = 0.0;
    const int rank = ON_Solve3x3(&R.plane_equation.x, &S.plane_equation.x,
                                 &T.plane_equation.x, -R.plane_equation.d,
                                 -S.plane_equation.d, -T.plane_equation.d, &P.x,
                                 &P.y, &P.z, &pr);
    return (rank == 3) ? true : false;
}

int ON_Intersect(const ON_Plane &plane, const ON_Sphere &sphere,
                 ON_Circle &circle) {
    ...
    if (ON_IsValid(sphere_radius) && ON_IsValid(d) &&
        d <= sphere_radius + tol) {
        if (sphere_radius > 0.0) {
            ...
            circle.radius =
                (d > 4.0 * ON_EPSILON) ? sphere_radius * sqrt(d) : 0.0;
        } else
            circle.radius = 0.0;
...
static int Intersect2dLineCircle(ON_2dPoint line_from, // 2d line from point
                                 ON_2dPoint line_to, double r, double tol,
                                 double *t0, double *t1) {
...
int ON_Intersect( // returns 0 = no intersections,
    ...
    ON_Line axis;
    axis.from = cylinder.circle.plane.origin +
                cylinder.height[0] * cylinder.circle.plane.zaxis;
    axis.to = cylinder.circle.plane.origin +
              cylinder.height[1] * cylinder.circle.plane.zaxis;
    if (axis.Length() <= tol) {
    ...
    if (d >= cylinder_radius - tol) {
        ...
        if (bFiniteCyl) {
            V = V -
                (V * cylinder.circle.plane.zaxis) * cylinder.circle.plane.zaxis;
        }
        V.Unitize();
        ...
        int qerc = ON_SolveQuadraticEquation(
            ax + ay, bx + by, cx + cy - cylinder_radius * cylinder_radius, &t0,
            &t1);
        if (qerc == 2) {
            // complex roots - ignore (tiny) imaginary part caused by
            // computational noise.
            t1 = t0;
        }
        ...
        if (d <= ON_ZERO_TOLERANCE) {
            ...
            ON_3dVector V = line_point - axis_point;
            if (bFiniteCyl) {
                V = V - (V * cylinder.circle.plane.zaxis) *
                            cylinder.circle.plane.zaxis;
            }
            V.Unitize();
            B = axis_point + cylinder_radius * V;
            rc = 1;
...
int ON_Intersect(const ON_Sphere &sphere0, const ON_Sphere &sphere1,
                 ON_Circle &circle)
