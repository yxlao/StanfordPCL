...
const ON_Surface *ON_SurfaceProxy::ProxySurface() const { return m_surface; }

bool ON_SurfaceProxy::ProxySurfaceIsTransposed() const { return m_bTransposed; }
...
ON_SurfaceProxy::Transform(
    const ON_Xform & // xform - formal parameter intentionally ignored in this
                     // virtual function
) {
...
ON_SurfaceProxy::GetParameterTolerance(int dir,
    ...
    return (m_surface) ? m_surface->GetParameterTolerance(dir, t, tminus, tplus)
                       : false;
}
...
ON_Surface::ISO
ON_SurfaceProxy::IsIsoparametric( // returns isoparametric status based on
                                  // bounding box
...
ON_BOOL32 ON_SurfaceProxy::IsPlanar(ON_Plane *plane, double tolerance) const {
    ON_BOOL32 rc = false;
    if (m_surface) {
...
bool ON_SurfaceProxy::GetNextDiscontinuity(int dir, ON::continuity c, double t0,
    ...
    if (0 != m_surface && dir >= 0 && dir <= 1) {
        rc = m_surface->GetNextDiscontinuity(
            m_bTransposed ? 1 - dir : dir, c, t0, t1, t, hint, dtype,
...
bool ON_SurfaceProxy::IsContinuous(
    ...
    if (m_surface) {
        ...
        rc = m_surface->IsContinuous(
            desired_continuity, s, t, hint, point_tolerance, d1_tolerance,
...
ON_SurfaceProxy::Evaluate( // returns false if unable to evaluate
    ...
    return (m_surface)
               ? m_surface->Evaluate(s, t, der_count, v_stride, v, side, hint)
               : false;
}

ON_Curve *ON_SurfaceProxy::IsoCurve(int dir, double c) const {
    ON_Curve *isocurve = 0;
...
int ON_SurfaceProxy::
    ...
    ON_BOOL32 rc =
        (m_surface) ? m_surface->GetNurbForm(nurbs, tolerance) : false;
    if (rc && m_bTransposed) {
        rc = nurbs.Transpose();
    }
    return rc;
