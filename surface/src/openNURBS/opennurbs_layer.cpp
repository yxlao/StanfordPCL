...
ON_OBJECT_IMPLEMENT(ON_Layer, ON_Object,
...
const wchar_t *ON_Layer::LayerNameReferenceDelimiter() { return L" : "; }

const wchar_t *ON_Layer::LayerNamePathDelimiter() { return L"::"; }
...
ON_Layer::Write(ON_BinaryArchive &file // serialize definition to binary archive
    ...
    while (rc) {
        ...
        // Starting with version 200312110, this value is zero.  For files
        // written with earlier versions, the number was a "model index" value
        // that was set to something >= 1, but never used.  We have to continue
        // to read/write an integer here so that old/new versions of opennurbs
        // can read files written by new/old versions.
        i = 0;
        rc = file.WriteInt(i);
        if (!rc)
            break;
        ...
        {
            // OBSOLETE LINE STYLE if ( rc ) rc = file.WriteLineStyle(
            // LineStyle() ); Starting with version 200503170, this section is
            // "officially" not used. Prior to that, it was old ON_LineStyle
            // information that has never been used.
            short s = 0;
            if (rc)
...
ON_Layer::Read(ON_BinaryArchive &file // restore definition from binary archive
    ...
    if (rc && major_version == 1) {
        ...
        {
            // OBSOLETE line style was never used - read and discard the next 20
            // bytes
            short s;
            double x;
            if (rc)
                    ...
                    if (rc && minor_version >= 4) {
                        ...
                        if (rc && minor_version >= 5) {
                            ...
                            if (rc && minor_version >= 6 &&
                                ...
                                // Some files saved with opennurbs version
                                // 200505110 do not contain correctly written
                                // m_parent_layer_id and m_bExpanded values. It
                                // is ok to default these values.
        ...
        ON_ERROR(
            "ON_Layer::Read() encountered a layer written by future code.");
        rc = false;
    }
...
ON::object_type ON_Layer::ObjectType() const { return ON::layer_object; }
...
const ON_wString &ON_Layer::LayerName() const { return m_name; }

void ON_Layer::SetColor(ON_Color c) { m_color = c; }

void ON_Layer::SetPlotColor(ON_Color c) { m_plot_color = c; }

ON_Color ON_Layer::Color() const { return m_color; }
...
int ON_Layer::LinetypeIndex() const { return m_linetype_index; }

bool ON_Layer::IsVisible() const { return m_bVisible; }

void ON_Layer::SetVisible(bool bVisible) {
    m_bVisible = (bVisible ? true : false);
}

void ON_Layer::SetLocked(bool bLocked) { m_bLocked = (bLocked ? true : false); }

bool ON_Layer::IsLocked() const { return m_bLocked; }

bool ON_Layer::IsVisibleAndNotLocked() const {
    return (m_bVisible && !m_bLocked);
}

bool ON_Layer::IsVisibleAndLocked() const { return (m_bVisible && m_bLocked); }
...
int ON_Layer::RenderMaterialIndex() const { return m_material_index; }
...
int ON_Layer::LayerIndex() const { return m_layer_index; }
...
int ON_Layer::IgesLevel() const { return m_iges_level; }

double ON_Layer::PlotWeight() const { return m_plot_weight_mm; }
...
#error Never copy this class definition or put this definition in a header file!
  ...
  public:
    ...
    double
        m_plot_weight_mm; // ON_UNSET_VALUE means use ON_Layer::m_plot_weight_mm
    ...
    static int Compare(const ON__LayerPerViewSettings *a,
                       const ON__LayerPerViewSettings *b);

    static int CompareViewportId(const ON__LayerPerViewSettings *a,
                                 const ON__LayerPerViewSettings *b);
...
int ON__LayerPerViewSettings::CompareViewportId(
    const ON__LayerPerViewSettings *a, const ON__LayerPerViewSettings *b) {
...
bool ON__LayerPerViewSettings::Write(ON_BinaryArchive &binary_archive) const {
    ...
    for (;;) {
        ...
        rc = binary_archive.WriteInt(1, &bits);
        if (!rc)
            break;
        if (0 == bits)
...
class /*NEVER EXPORT THIS CLASS DEFINITION*/ ON__LayerExtensions
    : public ON_UserData {
#if !defined(ON_BOZO_VACCINE_3E4904E6E9304fbcAA42EBD407AEFE3B)
#error Never copy this class definition or put this definition in a header file!
#endif
  ...
  public:
    ...
    ViewportSettings(const ON_Layer &layer, const ON_UUID &viewport_id,
                     bool bCreate);

    static void DeleteViewportSettings(
        const ON_Layer &layer,
        const ON__LayerPerViewSettings *vp_settings_to_delete);

    static ON__LayerExtensions *LayerExtensions(const ON_Layer &layer,
                                                bool bCreate);

    // per viewport overrides of color, linetype, plot color, plot weight, and
    // visibility
...
ON__LayerExtensions *ON__LayerExtensions::LayerExtensions(const ON_Layer &layer,
                                                          bool bCreate) {
    ON__LayerExtensions *ud = ON__LayerExtensions::Cast(layer.GetUserData(
        ON__LayerExtensions::m_ON__LayerExtensions_class_id.Uuid()));
    unsigned char *layer__runtme_flags =
        ((unsigned char *)&layer.m_bExpanded) + sizeof(layer.m_bExpanded);
    if (!ud) {
        if (bCreate) {
            ...
            if (0x00 != *layer__runtme_flags) {
                ...
                //   This permits ON_Layer visibility and color queries to check
                //   for userdata.
            ...
            if (0x01 != *layer__runtme_flags) {
                ...
                //   Setting this bit prevents ON_Layer visibility and color
                //   queries that are often used in time critical code from
                //   performing the time consuming check for userdata that
                //   rarely exists.
        ...
        // Set ON_Layer::m__runtime_flags = 0x00
        //   This permits ON_Layer visibility and color queries to check for
        //   userdata.
        *layer__runtme_flags = 0x00;
    }
    return ud;
...
// virtual ON_UserData override
ON_BOOL32 ON__LayerExtensions::Archive() const { return !IsEmpty(); }
...
ON__LayerPerViewSettings *ON__LayerExtensions::ViewportSettings(
    const ON_Layer &layer, const ON_UUID &viewport_id, bool bCreate) {
    if (!ON_UuidIsNil(viewport_id)) {
        ON__LayerExtensions *ud =
            ON__LayerExtensions::LayerExtensions(layer, bCreate);
        if (ud) {
            int i;
            ...
            for (i = 0; i < vp_settings_count; i++) {
                if (0 == memcmp(&viewport_id, &vp_settings[i].m_viewport_id,
                                sizeof(ON_UUID)))
                    return (vp_settings + i);
            }
            if (bCreate) {
                ON__LayerPerViewSettings &new_vp_settings =
                    ud->m_vp_settings.AppendNew();
                vp_settings =
                    ud->m_vp_settings.Array(); // appending can grow the array
...
void ON__LayerExtensions::DeleteViewportSettings(
    ...
    const ON__LayerPerViewSettings *vp_settings_to_delete) {
    ON__LayerExtensions *ud =
        ON__LayerExtensions::LayerExtensions(layer, false);
    if (ud) {
...
void ON_Layer::SetColor(ON_Color layer_color, const ON_UUID &viewport_id) {
    ...
    if (ON_UuidIsNil(viewport_id)) {
        ...
        ON__LayerPerViewSettings *vp_settings =
            ON__LayerExtensions::ViewportSettings(*this, viewport_id, bSet);
        if (vp_settings) {
...
ON_Color ON_Layer::Color(const ON_UUID &viewport_id) const {
    ...
    ON__LayerPerViewSettings *vp_settings =
        ON__LayerExtensions::ViewportSettings(*this, viewport_id, false);
    ...
    if (ON_UuidIsNil(viewport_id)) {
        ...
        if (vp_settings) {
            vp_settings->m_plot_color =
                (bSet) ? plot_color : ON_Color::UnsetColor;
            if (!bSet && !vp_settings->ActiveElements())
                ON__LayerExtensions::DeleteViewportSettings(*this, vp_settings);
        }
    }
...
ON_Color ON_Layer::PlotColor(const ON_UUID &viewport_id) const {
    ...
    return (vp_settings && vp_settings->m_plot_color != ON_UNSET_COLOR)
               ? vp_settings->m_plot_color
               : PlotColor(); // 2-Nov-2009 Dale Fugier, modified to call
                              // default PlotColor()
}
    ...
    if (!ON_IsValid(plot_weight_mm)) {
        ...
        if (vp_settings) {
            vp_settings->m_plot_weight_mm =
                (bSet) ? plot_weight_mm : ON_UNSET_VALUE;
            if (!bSet && !vp_settings->ActiveElements())
                ON__LayerExtensions::DeleteViewportSettings(*this, vp_settings);
        }
    }
...
double ON_Layer::PlotWeight(const ON_UUID &viewport_id) const {
    ...
    return (vp_settings && (vp_settings->m_plot_weight_mm >= 0.0 ||
                            -1.0 == vp_settings->m_plot_weight_mm))
               ? vp_settings->m_plot_weight_mm
               : m_plot_weight_mm;
}
...
bool ON_Layer::IsVisible(const ON_UUID &viewport_id) const {
    ...
    if (ON_UuidIsNil(viewport_id)) {
        ...
        if (!m_bVisible) {
            ...
            ON__LayerExtensions *ud =
                ON__LayerExtensions::LayerExtensions(*this, false);
            if (0 != ud) {
        ...
        if (vp_settings && vp_settings->m_visible) {
            ...
            if (2 == vp_settings->m_visible)
                return false; // per viewport OFF setting overrides layer
                              // setting
        }
    }
...
void ON_Layer::SetVisible(bool bVisible, const ON_UUID &viewport_id) {
    if (ON_UuidIsNil(viewport_id)) {
        ...
        ON__LayerPerViewSettings *vp_settings =
            ON__LayerExtensions::ViewportSettings(*this, viewport_id, true);
        if (vp_settings) {
            vp_settings->m_visible =
                (bVisible) ? 1  // layer is always on in this viewport
...
void ON_Layer::DeletePerViewportColor(const ON_UUID &viewport_id) {
    if (ON_UuidIsNil(viewport_id)) {
        ...
        if (0 != ud) {
            ...
            if (ud->IsEmpty()) {
                delete ud;
                // setting m__runtime_flags prevents searching for
                // ON__LayerExtensions userdata and speeds up visibility and
                // color queries.
...
bool ON_Layer::HasPerViewportSettings(const ON_UUID &viewport_id) const {
    ...
    if (ON_UuidIsNil(viewport_id)) {
        ...
        if (ud && ud->m_vp_settings.Count() > 0)
            rc = true;
    } else if (0 != ON__LayerExtensions::ViewportSettings(*this, viewport_id,
                                                          false)) {
        rc = true;
    }
    return rc;
...
bool ON_Layer::CopyPerViewportSettings(ON_UUID source, ON_UUID destination) {
    bool rc = false;
    if (ON_UuidIsNotNil(source) && ON_UuidIsNotNil(destination) &&
        ON_UuidCompare(source, destination) != 0) {
        ON__LayerPerViewSettings *pSourceSettings =
            ON__LayerExtensions::ViewportSettings(*this, source, false);
        if (pSourceSettings) {
            // Copy source settings, since pSourceSettings pointer may become
            // bogus after creating the destination settings
            ON__LayerPerViewSettings src = *pSourceSettings;
            ON__LayerPerViewSettings *pDestinationSettings =
                ON__LayerExtensions::ViewportSettings(*this, destination, true);
...
void ON_Layer::CullPerViewportSettings(int viewport_id_count,
    ...
    if (0 != ud) {
        if (viewport_id_count <= 0) {
            ...
            delete ud;
            // setting m__runtime_flags prevents searching for
            // ON__LayerExtensions userdata and speeds up visibility and color
            // queries.
            m__runtime_flags = 0x01;
            ...
            for (i = ud->m_vp_settings.Count(); i--; /*empty iterator*/) {
                ...
                for (j = 0; j < viewport_id_count; j++) {
                    if (0 ==
                        memcmp(&viewport_id_list[i], &vp_id, sizeof(vp_id)))
                        break;
                }
                if (j >= viewport_id_count) {
                    // ud->m_vp_settings[i].m_viewport_id is NOT in
                    // viewport_id_list[]
                    ud->m_vp_settings.Remove(i);
                }
            }
...
void ON_Layer::Set(unsigned int settings, const ON_Layer &settings_values) {
    if (0 != (ON_Layer::userdata_settings & settings)) {
        ...
        // now restore any original user data that was not present on
        // settings_values.
...
#error Never copy this class definition or put this definition in a header file!
  ...
  public:
    ...
    static ON__LayerSettingsUserData *LayerSettings(const ON_Layer &layer,
                                                    bool bCreate);
  ...
  public:
    enum {
        valid_settings =
            (ON_Layer::color_settings | ON_Layer::plot_color_settings |
...
ON_OBJECT_IMPLEMENT(ON__LayerSettingsUserData, ON_UserData,
...
ON__LayerSettingsUserData::LayerSettings(const ON_Layer &layer, bool bCreate) {
    ON__LayerSettingsUserData *ud =
        ON__LayerSettingsUserData::Cast(layer.GetUserData(
            ON__LayerSettingsUserData::m_ON__LayerSettingsUserData_class_id
                .Uuid()));
    if (!ud && bCreate) {
