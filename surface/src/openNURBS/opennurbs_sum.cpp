...
void ON_Sum::Plus(double x) {
    if (x > 0.0) {
        ...
        if (m_pos_sum1_count == sum1_max_count) {
            m_pos_sum2[m_pos_sum2_count++] =
                SortAndSum(m_pos_sum1_count, m_pos_sum1);
            m_pos_sum1_count = 0;
            if (m_pos_sum2_count == sum2_max_count) {
                m_pos_sum3[m_pos_sum3_count++] =
                    SortAndSum(m_pos_sum2_count, m_pos_sum2);
                m_pos_sum2_count = 0;
        ...
        if (m_neg_sum1_count == sum1_max_count) {
            m_neg_sum2[m_neg_sum2_count++] =
                SortAndSum(m_neg_sum1_count, m_neg_sum1);
            m_neg_sum1_count = 0;
            if (m_neg_sum2_count == sum2_max_count) {
                m_neg_sum3[m_neg_sum3_count++] =
                    SortAndSum(m_neg_sum2_count, m_neg_sum2);
                m_neg_sum2_count = 0;
...
void ON_Sum::operator=(double x) { Begin(x); }
...
double ON_Sum::Total(double *error_estimate) {
    ...
    if (error_estimate) {
        *error_estimate =
            m_sum_err + ON_EPSILON * (fabs(m_pos_sum) + fabs(m_neg_sum));
    }
