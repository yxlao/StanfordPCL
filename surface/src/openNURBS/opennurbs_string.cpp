...
void ON_String::Create() { m_s = (char *)pEmptyaString; }
...
void ON_String::EmergencyDestroy() { Create(); }

void ON_String::EnableReferenceCounting(bool bEnable) {
    // TODO fill this in;
}

bool ON_String::IsReferenceCounted() const { return true; }
...
void ON_String::CreateArray(int capacity) {
    Destroy();
    if (capacity > 0) {
        ON_aStringHeader *p = (ON_aStringHeader *)onmalloc(
            sizeof(ON_aStringHeader) + (capacity + 1) * sizeof(*m_s));
        p->ref_count = 1;
        p->string_length = 0;
...
int ON_String::Length() const { return Header()->string_length; }
...
char ON_String::operator[](int i) const { return m_s[i]; }
...
int ON_String::Find(unsigned char c) const { return Find((char)c); }
...
int ON_String::Remove(const char chRemove) {
    ...
    *pstrDest = 0;
    int nCount = (int)(pstrSource -
                       pstrDest); // the (int) is for 64 bit size_t conversion
...
ON_String ON_String::Mid(int i) const { return Mid(i, Length() - i); }
...
bool ON_CheckSum::SetBufferCheckSum(size_t size, const void *buffer,
                                    time_t time) {
    bool rc = false;
    Zero();
    if (size != 0 && buffer != 0) {
...
bool ON::GetFileStats(const wchar_t *filename, size_t *filesize,
...
bool ON::GetFileStats(FILE *fp, size_t *filesize, time_t *create_time,
...
bool ON_CheckSum::CheckBuffer(size_t size, const void *buffer) const {
    if (m_size != size)
        return false;
    if (0 == size)
        return true;
...
bool ON_CheckSum::CheckFile(FILE *fp, bool bSkipTimeCheck) const {
    if (!fp)
        return false;
...
void ON_CheckSum::Dump(ON_TextLog &text_log) const {
    ...
    unsigned long long u; // 8 bytes in windows and gcc - should be at least as
                          // big as a size_t or time_t.
    ...
    if (!IsSet())
        ...
        text_log.Print(
            L"Last Modified Time: %u (seconds since January 1, 1970, UCT)\n",
            u);
        text_log.Print(
            L"CRC List: %08x, %08x, %08x, %08x, %08x, %08x, %08x, %08x\n",
