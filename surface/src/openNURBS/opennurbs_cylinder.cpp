...
bool ON_Cylinder::Create(const ON_Circle &c) { return Create(c, 0.0); }

bool ON_Cylinder::Create(const ON_Circle &c, double h) {
    circle = c;
    if (h > 0.0) {
        height[0] = 0.0;
        height[1] = h;
...
bool ON_Cylinder::IsValid() const { return circle.IsValid(); }

bool ON_Cylinder::IsFinite() const { return height[0] != height[1]; }

const ON_3dVector &ON_Cylinder::Axis() const { return circle.plane.zaxis; }

const ON_3dPoint &ON_Cylinder::Center() const { return circle.plane.origin; }

double ON_Cylinder::Height() const { return height[1] - height[0]; }

ON_Circle ON_Cylinder::CircleAt(double t // linear parameter
) const {
    ON_Circle c = circle;
    if (t != 0.0)
...
ON_Line ON_Cylinder::LineAt(double s // angular parameter
) const {
...
bool ON_Cylinder::Rotate(double sin_angle, double cos_angle,
                         const ON_3dVector &axis // axis of rotation
) {
    return Rotate(sin_angle, cos_angle, axis, circle.plane.origin);
}

bool ON_Cylinder::Rotate(double angle,           // angle in radians
                         const ON_3dVector &axis // axis of rotation
) {
...
bool ON_Cylinder::Rotate(double sin_angle,        // sin(angle)
  ...
  if ( pRevSurface )
  {
    ...
    else
    {
      ...
      for ( int capcount = 0; capcount < 2; capcount++ )
      {
        ...
        ON_BrepTrim& trim = pBrep->NewTrim( edge, true, loop,
pBrep->m_C2.Count()-1 ); for ( int eti = 0; eti < edge.m_ti.Count(); eti++ )
