...
bool ON_Sphere::IsValid() const {
    return (ON_IsValid(radius) && radius > 0.0 && plane.IsValid()) ? true
                                                                   : false;
}
...
ON_Circle ON_Sphere::LatitudeRadians(double a) const {
    return ON_Circle(PointAt(0.0, a), PointAt(0.5 * ON_PI, a),
                     PointAt(ON_PI, a));
}
...
ON_3dPoint ON_Sphere::Center() const { return plane.origin; }

ON_3dPoint ON_Sphere::NorthPole() const { return PointAt(0.0, 0.5 * ON_PI); }

ON_3dPoint ON_Sphere::SouthPole() const { return PointAt(0.0, -0.5 * ON_PI); }

double ON_Sphere::Radius() const { return radius; }

double ON_Sphere::Diameter() const { return 2.0 * radius; }

bool ON_Sphere::ClosestPointTo(ON_3dPoint point, double *longitude,
...
bool ON_Sphere::Rotate(double sin_angle, double cos_angle,
                       const ON_3dVector &axis // axis of rotation
) {
    return Rotate(sin_angle, cos_angle, axis, plane.origin);
}

bool ON_Sphere::Rotate(double angle,           // angle in radians
                       const ON_3dVector &axis // axis of rotation
) {
...
int ON_Sphere::GetNurbForm(ON_NurbsSurface &s) const {
    ...
    if (IsValid()) {
        ...
        ON_3dPoint p[9] = {
            plane.origin + x,     plane.origin + x + y, plane.origin + y,
...
ON_RevSurface *ON_Sphere::RevSurfaceForm(ON_RevSurface *srf) const {
    ...
    if (IsValid()) {
        ...
        ON_ArcCurve *arc_curve =
            new ON_ArcCurve(arc, -0.5 * ON_PI, 0.5 * ON_PI);
        if (srf)
            pRevSurface = srf;
        else
