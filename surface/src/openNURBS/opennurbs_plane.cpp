...
bool ON_Plane::GetDistanceToBoundingBox(const ON_BoundingBox &Box, double *min,
                                        double *max) const {
    // min and max signed distance.  Returns false if plane normal has zero
    // length.
...
bool ON_Plane::operator==(const ON_Plane &other) const {
    return (origin == other.origin && xaxis == other.xaxis &&
            yaxis == other.yaxis && zaxis == other.zaxis)
               ? true
               : false;
}
...
bool ON_Plane::CreateFromEquation(const double e[4] // equation of plane
) {
    bool b = false;
...
bool ON_Plane::CreateFromPoints(const ON_3dPoint &P, // point on the plane
                                const ON_3dPoint &Q, // point on the plane
                                const ON_3dPoint &R  // point on the plane
) {
    origin = P;
...
bool ON_Plane::Rotate(double s,               // sin(angle)
                      double c,               // cos(angle)
...
bool ON_Plane::Rotate(double angle,           // angle in radians
                      const ON_3dVector &axis // axis of rotation
) {
...
bool ON_Plane::Rotate(double sin_angle,        // sin(angle)
...
bool ON_Plane::Rotate(double a,                   // angle in radians
                      const ON_3dVector &axis,    // axis of rotation
...
int ON_ArePointsOnPlane( // returns 0=no, 1 = yes, 2 = pointset is (to
                         // tolerance) a single point on the line
    int dim,             // 2 or 3
