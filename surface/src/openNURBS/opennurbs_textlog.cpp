...
ON_TextLog::ON_TextLog()
    : m_pFile(0), m_pString(0), m_indent(""), m_beginning_of_line(1),
      m_indent_size(0) {
    SetFloatFormat("%g");
    SetDoubleFormat("%.17g");
}
...
ON_TextLog::ON_TextLog(FILE *pFile)
    : m_pFile(pFile), m_pString(0), m_indent(""), m_beginning_of_line(1),
      m_indent_size(0) {
    SetFloatFormat("%g");
    SetDoubleFormat("%.17g");
}
...
ON_TextLog::ON_TextLog(ON_wString &wstr)
    : m_pFile(0), m_pString(&wstr), m_indent(""), m_beginning_of_line(1),
      m_indent_size(0) {
    SetFloatFormat("%g");
    SetDoubleFormat("%.17g");
}
...
void ON_TextLog::GetDoubleFormat(ON_String &s) const { s = m_double_format; }
...
void ON_TextLog::GetFloatFormat(ON_String &s) const { s = m_float_format; }
...
void ON_TextLog::SetIndentSize(int s) { m_indent_size = (s > 0) ? s : 0; }
...
void ON_TextLog::Print(const ON_Xform &xform) {
    if (xform.IsIdentity()) {
        ...
        Print(m_double4_format, xform[0][0], xform[0][1], xform[0][2],
              xform[0][3]);
        Print("\n");
        Print(m_double4_format, xform[1][0], xform[1][1], xform[1][2],
              xform[1][3]);
        Print("\n");
        Print(m_double4_format, xform[2][0], xform[2][1], xform[2][2],
              xform[2][3]);
        Print("\n");
        Print(m_double4_format, xform[3][0], xform[3][1], xform[3][2],
              xform[3][3]);
        Print("\n");
    }
}
...
void ON_TextLog::Print(const ON_UUID &uuid) {
    Print("%08X-%04X-%04x-%02X%02X-%02X%02X%02X%02X%02X%02X", uuid.Data1,
...
void ON_TextLog::PrintTime(const struct tm &t) {
    ...
    const char *sMonName[13] = {"January",   "February", "March",    "April",
    ...
    Print("%s %s %02d %02d:%02d:%02d %4d", sDayName[wday], sMonName[mon],
          t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec, t.tm_year + 1900);
}

void ON_TextLog::PrintPointList(int dim, int is_rat, int count, int stride,
                                const double *P, const char *sPreamble) {
    double w, x;
    int i, j, cvdim;
...
void ON_TextLog::PrintPointGrid(int dim, int is_rat, int point_count0,
    ...
    for (i = 0; i < point_count0; i++) {
        ...
        PrintPointList(dim, is_rat, point_count1, point_stride1,
                       P + i * point_stride0, s);
    }
}
...
void ON_TextLog::Print(const ON_Matrix &M, const char *sPreamble,
                       int precision) {
    double x;
    ...
    for (row_index = 0; row_index < row_count; row_index++) {
        ...
        Print("%5d:", row_index);
        if (precision > 3) {
            for (column_index = 0; column_index < column_count;
                 column_index++) {
...
void ON_TextLog::PrintWrappedText(const wchar_t *s, int line_length) {
    ...
    if (s && *s && line_length > 0) {
        ...
        wchar_t *sLine =
            (wchar_t *)ws.GetMemory((max_line_length + 1) * sizeof(*sLine));
