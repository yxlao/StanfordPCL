...
static void InitParVars(ON_RTreePartitionVars *a_parVars, int a_maxRects,
                        int a_minFill);
static void GetBranches(ON_RTreeNode *a_node, ON_RTreeBranch *a_branch,
...
static ON_RTreeBBox CombineRectHelper(const ON_RTreeBBox *a_rectA,
                                      const ON_RTreeBBox *a_rectB);
static double CalcRectVolumeHelper(const ON_RTreeBBox *a_rect);
static bool OverlapHelper(const ON_RTreeBBox *a_rectA,
                          const ON_RTreeBBox *a_rectB);
static double
DistanceToCapsuleAxisHelper(const struct ON_RTreeCapsule *a_capsule,
                            const ON_RTreeBBox *a_rect);
static void ClassifyHelper(int a_index, int a_group,
                           struct ON_RTreePartitionVars *a_parVars);
static bool SearchHelper(const ON_RTreeNode *a_node, ON_RTreeBBox *a_rect,
                         ON_RTreeSearchResultCallback &a_result);
static bool SearchHelper(const ON_RTreeNode *a_node, const ON_RTreeBBox *a_rect,
...
static size_t MemPoolBlkSize(size_t leaf_count) {
    ...
    if (leaf_count > 0) {
        ...
        if (5 * leaf_count < 4 * ON_RTree_MAX_NODE_COUNT)
            node_count = 3;
        else if (5 * leaf_count <
                 4 * ON_RTree_MAX_NODE_COUNT * ON_RTree_MAX_NODE_COUNT)
    ...
    // to store allocation bookkeeping information.  The "+ 2*sizeof(void*)
    // allows for up to two pointers of "lame" system overhead per allocation.
    // The goal is to prevent the "real" allocation from being just a hair
    // bigger that a multiple of pagesize.
    if (sz < sizeof_blklink + nodes_per_blk * sizeof_node + 2 * sizeof(void *))
        nodes_per_blk--; // prevent memory manager overhead from allocating
                         // another page
...
ON_RTreeMemPool::ON_RTreeMemPool(size_t leaf_count)
    : m_nodes(0), m_list_nodes(0), m_buffer(0), m_buffer_capacity(0),
      m_blk_list(0), m_sizeof_blk(0), m_sizeof_heap(0) {
...
struct ON_RTreeListNode *ON_RTreeMemPool::AllocListNode() {
    struct ON_RTreeListNode *list_node =
        (struct ON_RTreeListNode *)m_list_nodes;
    if (list_node) {
...
size_t ON_RTreeMemPool::SizeOf() const { return m_sizeof_heap; }
...
bool ON_RTreeIterator::PushChildren(StackElement *sp, bool bFirstChild) {
    ...
    while (0 != node && node->m_level >= 0 && node->m_count > 0) {
        ...
        sp->m_node = node;
        sp->m_branchIndex =
            bFirstChild ? 0 : node->m_count - 1; // 0 for first child
    }
    return false;
}
...
ON_RTree::ON_RTree(size_t leaf_count)
    : m_root(0), m_reserved(0), m_mem_pool(leaf_count) {}
...
bool ON_RTree::Insert2d(const double a_min[2], const double a_max[2],
                        int a_element_id) {
...
bool ON_RTree::Insert(const double a_min[ON_RTree_NODE_DIM],
                      const double a_max[ON_RTree_NODE_DIM], int a_element_id) {
    bool rc;
    ON_RTreeBBox rect;
...
bool ON_RTree::Remove2d(const double a_min[2], const double a_max[2],
                        int a_dataId) {
    const double min3d[3] = {a_min[0], a_min[1], 0.0};
    const double max3d[3] = {a_max[0], a_max[1], 0.0};
...
bool ON_RTree::Remove(const double a_min[ON_RTree_NODE_DIM],
                      const double a_max[ON_RTree_NODE_DIM], int a_dataId) {
    bool rc = false;
    if (0 != m_root) {
        ON_RTreeBBox rect;
        memcpy(rect.m_min, a_min, sizeof(rect.m_min));
...
bool ON_RTree::Remove(const double a_min[ON_RTree_NODE_DIM],
                      const double a_max[ON_RTree_NODE_DIM], void *a_dataId) {
    bool rc = false;
    if (0 != m_root) {
        ON_RTreeBBox rect;
        memcpy(rect.m_min, a_min, sizeof(rect.m_min));
...
bool ON_RTree::Search(const double a_min[ON_RTree_NODE_DIM],
                      const double a_max[ON_RTree_NODE_DIM],
                      bool ON_MSC_CDECL a_resultCallback(void *a_context,
                                                         ON__INT_PTR a_data),
                      void *a_context) const {
...
bool ON_RTree::Search(struct ON_RTreeSphere *a_sphere,
                      bool ON_MSC_CDECL a_resultCallback(void *a_context,
                                                         ON__INT_PTR a_id),
                      void *a_context) const {
...
bool ON_RTree::Search(struct ON_RTreeCapsule *a_capsule,
                      bool ON_MSC_CDECL a_resultCallback(void *a_context,
                                                         ON__INT_PTR a_id),
                      void *a_context) const {
...
static void PairSearchHelper(const ON_RTreeBranch *a_branchA,
    ...
    while (branchB < branchBmax) {
        if (PairSearchOverlapHelper(&a_branchA->m_rect, &branchB->m_rect,
                                    a_result->m_tolerance)) {
...
static void PairSearchHelper(const ON_RTreeNode *a_nodeA,
    ...
    while (branchA < branchAmax) {
        if (PairSearchOverlapHelper(&branchA->m_rect, &a_branchB->m_rect,
                                    a_result->m_tolerance)) {
...
static void PairSearchHelper(const ON_RTreeNode *a_nodeA,
    ...
    while (branchA < branchAmax) {
        for (branchB = a_nodeB->m_branch; branchB < branchBmax; branchB++) {
            if (PairSearchOverlapHelper(&branchA->m_rect, &branchB->m_rect,
                                        a_result->m_tolerance)) {
                if (a_nodeA->m_level > 0) {
                    if (a_nodeB->m_level > 0)
                        PairSearchHelper(branchA->m_child, branchB->m_child,
                                         a_result);
                    else
...
bool ON_RTree::Search(const ON_RTree &a_rtreeA, const ON_RTree &a_rtreeB,
                      double tolerance, ON_SimpleArray<ON_2dex> &a_result) {
    ...
    r.m_tolerance =
        (ON_IsValid(tolerance) && tolerance > 0.0) ? tolerance : 0.0;
    r.m_result = &a_result;
...
typedef bool (*ON_RTreePairSearchCallbackBool)(void *, ON__INT_PTR,
                                               ON__INT_PTR);
    ...
    while (branchB < branchBmax) {
        if (PairSearchOverlapHelper(&a_branchA->m_rect, &branchB->m_rect,
            ...
            if (a_nodeB->m_level > 0) {
                ...
                a_result->m_resultCallback(a_result->m_context, a_branchA->m_id,
                                           branchB->m_id);
            }
        }
        branchB++;
...
PairSearchHelperBool(const ON_RTreeBranch *a_branchA,
    ...
    while (branchB < branchBmax) {
        if (PairSearchOverlapHelper(&a_branchA->m_rect, &branchB->m_rect,
                                    a_result->m_tolerance)) {
            if (a_nodeB->m_level > 0) {
                if (!PairSearchHelperBool(a_branchA, branchB->m_child,
                                          a_result))
                    return false;
            } else {
    ...
    while (branchA < branchAmax) {
        if (PairSearchOverlapHelper(&branchA->m_rect, &a_branchB->m_rect,
            ...
            if (a_nodeA->m_level > 0) {
                ...
                a_result->m_resultCallback(a_result->m_context, branchA->m_id,
                                           a_branchB->m_id);
            }
        }
        branchA++;
...
PairSearchHelperBool(const ON_RTreeNode *a_nodeA,
    ...
    while (branchA < branchAmax) {
        if (PairSearchOverlapHelper(&branchA->m_rect, &a_branchB->m_rect,
                                    a_result->m_tolerance)) {
            if (a_nodeA->m_level > 0) {
                if (!PairSearchHelperBool(branchA->m_child, a_branchB,
                                          a_result))
                    return false;
            } else {
                if (!a_result->m_resultCallbackBool(
...
static bool
PairSearchHelperBool(const ON_RTreeNode *a_nodeA, const ON_RTreeNode *a_nodeB,
                     ON_RTreePairSearchCallbackResultBool *a_result) {
    // DO NOT ADD ANYTHING TO THIS FUNCTION
    const ON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
    ...
    while (branchA < branchAmax) {
        for (branchB = a_nodeB->m_branch; branchB < branchBmax; branchB++) {
            if (PairSearchOverlapHelper(&branchA->m_rect, &branchB->m_rect,
                ...
                if (a_nodeA->m_level > 0) {
                    if (a_nodeB->m_level > 0) {
                        if (!PairSearchHelperBool(branchA->m_child,
                                                  branchB->m_child, a_result))
                            return false;
                    } else {
                        if (!PairSearchHelperBool(branchA->m_child, branchB,
                                                  a_result))
                            return false;
                    }
                } else if (a_nodeB->m_level > 0) {
                    if (!PairSearchHelperBool(branchA, branchB->m_child,
                                              a_result))
                        return false;
                } else {
                    if (!a_result->m_resultCallbackBool(
...
bool ON_RTree::Search(const ON_RTree &a_rtreeA, const ON_RTree &a_rtreeB,
    ...
    // Do not return false if PairSearchHelperBool() returns false.  The only
    // reason PairSearchHelperBool() returns false is that the user specified
    // resultCallback() terminated the search. This way a programmer with the
    // ability to reason can distinguish between a terminiation and a failure to
    // start because input is missing.
...
const ON_RTreeNode *ON_RTree::Root() const { return m_root; }
...
size_t ON_RTree::SizeOf() const { return m_mem_pool.SizeOf(); }

static void NodeCountHelper(const ON_RTreeNode *node, size_t &node_count,
                            size_t &wasted_branch_count, size_t &leaf_count) {
    if (0 == node)
        return;
    node_count++;
    ...
    if (node->m_level > 0) {
        for (int i = 0; i < node->m_count; i++) {
            NodeCountHelper(node->m_branch[i].m_child, node_count,
                            wasted_branch_count, leaf_count);
        }
    } else
...
bool ON_RTree::InsertRectRec(ON_RTreeBBox *a_rect, ON__INT_PTR a_id,
                             ON_RTreeNode *a_node, ON_RTreeNode **a_newNode,
                             int a_level) {
    int index;
...
#if defined(ON_COMPILER_MSC) && 4 == ON_SIZEOF_POINTER
#pragma warning(push)
// Disable warning C4312: 'type cast' : conversion from 'ON__INT_PTR' to
// 'ON_RTreeNode *' of greater size
...
bool ON_RTree::InsertRect(ON_RTreeBBox *a_rect, ON__INT_PTR a_id,
                          ON_RTreeNode **a_root, int a_level) {
...
bool ON_RTree::AddBranch(ON_RTreeBranch *a_branch, ON_RTreeNode *a_node,
...
void ON_RTree::SplitNode(ON_RTreeNode *a_node, ON_RTreeBranch *a_branch,
                         ON_RTreeNode **a_newNode) {
    ON_RTreePartitionVars localVars;
    int level;

    // Load all the branches into a buffer, initialize a_node to be empty
    level = a_node->m_level; // save m_level (The InitNode() call in GetBranches
                             // will set it to -1)
...
double CalcRectVolumeHelper(const ON_RTreeBBox *a_rect) {
    ...
    // Bounding sphere volume calculation is slower, but helps certain merge
    // cases
#if (3 == ON_RTree_NODE_DIM)
...
static void GetBranches(ON_RTreeNode *a_node, ON_RTreeBranch *a_branch,
    ...
    for (int index = 1; index < ON_RTree_MAX_NODE_COUNT + 1; ++index) {
        a_parVars->m_coverSplit = CombineRectHelper(
            &a_parVars->m_coverSplit, &a_parVars->m_branchBuf[index].m_rect);
    }
    a_parVars->m_coverSplitArea =
        CalcRectVolumeHelper(&a_parVars->m_coverSplit);
...
static void ChoosePartition(ON_RTreePartitionVars *a_parVars, int a_minFill) {
    ...
    while (
        ((a_parVars->m_count[0] + a_parVars->m_count[1]) <
         a_parVars->m_total) &&
        ...
        for (int index = 0; index < a_parVars->m_total; ++index) {
            if (!a_parVars->m_taken[index]) {
                ...
                ON_RTreeBBox rect0 =
                    CombineRectHelper(curRect, &a_parVars->m_cover[0]);
                ON_RTreeBBox rect1 =
                    CombineRectHelper(curRect, &a_parVars->m_cover[1]);
                double growth0 =
                    CalcRectVolumeHelper(&rect0) - a_parVars->m_area[0];
                double growth1 =
                    CalcRectVolumeHelper(&rect1) - a_parVars->m_area[1];
...
void ON_RTree::LoadNodes(ON_RTreeNode *a_nodeA, ON_RTreeNode *a_nodeB,
                         ON_RTreePartitionVars *a_parVars) {
    for (int index = 0; index < a_parVars->m_total; ++index) {
...
static void PickSeeds(ON_RTreePartitionVars *a_parVars) {
    ...
    for (int index = 0; index < a_parVars->m_total; ++index) {
        area[index] =
            CalcRectVolumeHelper(&a_parVars->m_branchBuf[index].m_rect);
    }
    ...
    for (int indexA = 0; indexA < a_parVars->m_total - 1; ++indexA) {
        for (int indexB = indexA + 1; indexB < a_parVars->m_total; ++indexB) {
            ON_RTreeBBox oneRect =
                CombineRectHelper(&a_parVars->m_branchBuf[indexA].m_rect,
                                  &a_parVars->m_branchBuf[indexB].m_rect);
            waste =
                CalcRectVolumeHelper(&oneRect) - area[indexA] - area[indexB];
            if (waste > worst) {
                worst = waste;
                seed0 = indexA;
                seed1 = indexB;
...
void ClassifyHelper(int a_index, int a_group,
                    ON_RTreePartitionVars *a_parVars) {
    ...
    if (a_parVars->m_count[a_group] == 0) {
        ...
        a_parVars->m_cover[a_group] =
            CombineRectHelper(&a_parVars->m_branchBuf[a_index].m_rect,
                              &a_parVars->m_cover[a_group]);
    }
    a_parVars->m_area[a_group] =
        CalcRectVolumeHelper(&a_parVars->m_cover[a_group]);
    ++a_parVars->m_count[a_group];
}

// Delete a data rectangle from an index structure.
// Pass in a pointer to a ON_RTreeBBox, the tid of the record, ptr to ptr to
// root node. Returns 1 if record not found, 0 if success. RemoveRect provides
// for eliminating the root.

bool ON_RTree::RemoveRect(ON_RTreeBBox *a_rect, ON__INT_PTR a_id,
                          ON_RTreeNode **a_root) {
        ...
        while (reInsertList) {
            ...
            for (int index = 0; index < tempNode->m_count; ++index) {
                InsertRect(&(tempNode->m_branch[index].m_rect),
                           tempNode->m_branch[index].m_id, a_root,
                           tempNode->m_level);
            }
...
bool ON_RTree::RemoveRectRec(ON_RTreeBBox *a_rect, ON__INT_PTR a_id,
                             ON_RTreeNode *a_node,
                ...
                if (!RemoveRectRec(a_rect, a_id,
                    ...
                    if (a_node->m_branch[index].m_child->m_count >=
                        ...
                        DisconnectBranch(a_node,
                                         index); // Must return after this call
...
// static bool OverlapHelper(const struct ON_RTreeSphere* a_sphere, const
// ON_RTreeBBox* a_rect)
//{
//  double d[3], t, r;
//  const double* mn;
...
static double DistanceToBoxHelper(const double *pt, double r,
                                  const ON_RTreeBBox *a_rect) {
...
DistanceToCapsuleAxisHelper(const struct ON_RTreeCapsule *a_capsule,
    ...
    if (0.0 == a_capsule->m_domain[0]) {
        ...
        L[0][0] =
            s[0] * a_capsule->m_point[0][0] + s[1] * a_capsule->m_point[1][0];
        L[0][1] =
            s[0] * a_capsule->m_point[0][1] + s[1] * a_capsule->m_point[1][1];
        L[0][2] =
            s[0] * a_capsule->m_point[0][2] + s[1] * a_capsule->m_point[1][2];
    }
...
static bool SearchHelper(const ON_RTreeNode *a_node, ON_RTreeBBox *a_rect,
                         ON_RTreeSearchResultCallback &a_result) {
    // NOTE:
    //  Some versions of ON_RTree::Search shrink a_rect as the search
    //  progresses.
    int i, count;
    ...
    if ((count = a_node->m_count) > 0) {
        ...
        if (a_node->IsInternalNode()) {
            ...
            for (i = 0; i < count; ++i) {
                if (OverlapHelper(a_rect, &branch[i].m_rect)) {
                    if (!a_result.m_resultCallback(a_result.m_context,
                                                   branch[i].m_id)) {
...
static bool SearchHelper(const ON_RTreeNode *a_node,
                         ...
                         ON_RTreeSearchResultCallback &a_result) {
    // NOTE:
    //  Some versions of ON_RTree::Search shrink a_sphere as the search
    //  progresses.
    ...
    if ((count = a_node->m_count) > 0) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            r[i] = DistanceToBoxHelper(sphere_center, sphere_radius,
                                       &branch[i].m_rect);
            if (r[i] <= closest_d) {
                closest_d = r[i];
                closest_i = i;
        ...
        if (closest_i >= 0) {
            if (a_node->IsInternalNode()) {
                // a_node is an internal node - search m_branch[].m_child as
                // needed. Search a closer node first to avoid worst case search
                // times in calculations where the calls to
                // a_result.m_resultCallback() reduce a_sphere->m_radius as
                // results are found.  Closest point calculations are an
                // example.
                if (!SearchHelper(branch[closest_i].m_child, a_sphere,
                                  a_result)) {
                    // callback canceled search
                    return false;
                }
                ...
                for (i = 0; i < count; ++i) {
                    ...
                    if (i != closest_i && r[i] <= a_sphere->m_radius) {
                        if (!SearchHelper(branch[i].m_child, a_sphere,
                                          a_result)) {
                            return false; // Don't continue searching
                        }
                    }
                ...
                // in calculations where the calls to
                // a_result.m_resultCallback() reduce a_sphere->m_radius as
                // results are found.  Closest point calculations are an
                // example.
                if (!a_result.m_resultCallback(a_result.m_context,
                                               branch[closest_i].m_id)) {
                    // callback canceled search
                    return false;
                }

                for (i = 0; i < count; ++i) {
                    // Note that the calls to a_result.m_resultCallback() can
                    // reduce the value of a_sphere->m_radius.
                    if (i != closest_i && r[i] <= a_sphere->m_radius) {
                        if (!a_result.m_resultCallback(a_result.m_context,
                                                       branch[i].m_id)) {
                            // callback canceled search
    ...
    if ((count = a_node->m_count) > 0) {
        ...
        if (a_node->IsInternalNode()) {
            ...
            if (count > 1) {
                ...
                r[0] =
                    DistanceToCapsuleAxisHelper(a_capsule, &branch[0].m_rect);
                r[1] = DistanceToCapsuleAxisHelper(a_capsule,
                                                   &branch[count - 1].m_rect);
                i = (r[0] <= r[1]) ? 0 : count - 1;
                if ((r[i ? 1 : 0] <= a_capsule->m_radius &&
                     !SearchHelper(branch[i].m_child, a_capsule, a_result)) ||
                    (r[i ? 0 : 1] <= a_capsule->m_radius &&
                     !SearchHelper(branch[count - 1 - i].m_child, a_capsule,
                                   a_result))) {
                    // callback canceled search
                    return false;
                }
            ...
            for (i = 0; i < count; ++i) {
                r[0] =
                    DistanceToCapsuleAxisHelper(a_capsule, &branch[i].m_rect);
                if (r[0] <= r[1]) {
...
static bool SearchHelper(const ON_RTreeNode *a_node, const ON_RTreeBBox *a_rect,
    ...
    if ((count = a_node->m_count) > 0) {
        ...
        if (a_node->IsInternalNode()) {
            ...
            for (i = 0; i < count; ++i) {
                if (OverlapHelper(a_rect, &branch[i].m_rect)) {
                    // The (void*) cast is safe because branch[i].m_id is an
                    // ON__INT_PTR
#if defined(ON_COMPILER_MSC) && 4 == ON_SIZEOF_POINTER
#pragma warning(push)
// Disable warning C4312: 'type cast' : conversion from 'const ON__INT_PTR' to
