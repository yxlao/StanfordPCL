...
ON_CompressStream::ON_CompressStream()
    : m_out_callback_function(0), m_out_callback_context(0), m_in_size(0),
      m_out_size(0), m_in_crc(0), m_out_crc(0), m_implementation(0),
      m_reserved(0) {}
...
bool ON_CompressStream::In(ON__UINT64 size, const void *uncompressed_buffer) {
    ...
    struct ON_ZlibImplementation *imp =
        (struct ON_ZlibImplementation *)m_implementation;
    z_stream &strm = imp->m_strm;
    if (0 != strm.avail_in || 0 != strm.next_in) {
        // strm.avail_in is always zero when we leave an ON_CompressStream
        // function.
        ErrorHandler();
        return false;
    }

    const ON__UINT32 sizeof_out_buffer =
        (ON__UINT32)(sizeof(imp->m_zlib_out_buffer));
    ...
    for (int counter = 512; counter > 0; counter--) {
        ...
        if (strm.avail_in <= 0) {
            ...
            m_in_size += sz;
            m_in_crc =
                ON_CRC32(m_in_crc, (size_t)sz,
                         uncompressed_buffer); // (size_t) cast is safe because
                                               // sz <= max_sz = 0x7FFFFFF0
            strm.next_in = (z_Bytef *)uncompressed_buffer;
            strm.avail_in = (ON__UINT32)sz;
            uncompressed_buffer =
                ((const unsigned char *)uncompressed_buffer) + sz;
            size -= sz;
            counter =
                512; // added input - reset the counter that detects stalls
        }
        ...
        if (deflate_output_count > 0) {
            ...
            rc = (0 != m_out_callback_function)
                     ? m_out_callback_function(m_out_callback_context,
                                               deflate_output_count, out_buffer)
                     : Out(m_out_callback_context, deflate_output_count,
                           out_buffer);
            if (!rc)
                break;
    ...
    for (int counter = 512; counter > 0; counter--) {
        ...
        if (deflate_output_count > 0) {
            ...
            ON__UINT32 compressed_crc1 =
                ON_CRC32(m_out_crc, deflate_output_count, out_buffer);
            ON__UINT64 compressed_size1 =
                m_out_size + ((ON__UINT64)deflate_output_count);
...
ON__UINT64 ON_CompressStream::InSize() const { return m_in_size; }

ON__UINT64 ON_CompressStream::OutSize() const { return m_out_size; }

ON__UINT32 ON_CompressStream::InCRC() const { return m_in_crc; }

ON__UINT32 ON_CompressStream::OutCRC() const { return m_out_crc; }
...
bool ON_UncompressStream::In(ON__UINT64 size, const void *compressed_buffer) {
    ...
    z_stream &strm = imp->m_strm;
    if (0 != strm.avail_in || 0 != strm.next_in) {
        // strm.avail_in is always zero when we leave an ON_UncompressStream
        // function.
        ErrorHandler();
        return false;
    }
    ...
    for (int counter = 512; counter > 0; counter--) {
        ...
        if (strm.avail_in <= 0) {
            ...
            m_in_crc =
                ON_CRC32(m_in_crc, (size_t)sz,
                         compressed_buffer); // (size_t) cast is safe because sz
                                             // <= max_sz = 0x7FFFFFF0
        ...
        if (inflate_output_count > 0) {
            ...
            // in the buffer argument.
            ON__UINT32 out_crc1 =
                ON_CRC32(m_out_crc, inflate_output_count, out_buffer);
            ON__UINT64 out_size1 = m_out_size + inflate_output_count;

            rc = (0 != m_out_callback_function)
                     ? m_out_callback_function(m_out_callback_context,
                                               inflate_output_count, out_buffer)
                     : Out(m_out_callback_context, inflate_output_count,
                           out_buffer);
            if (!rc)
                break;
...
ON__UINT64 ON_UncompressStream::InSize() const { return m_in_size; }

ON__UINT64 ON_UncompressStream::OutSize() const { return m_out_size; }

ON__UINT32 ON_UncompressStream::InCRC() const { return m_in_crc; }

ON__UINT32 ON_UncompressStream::OutCRC() const { return m_out_crc; }
