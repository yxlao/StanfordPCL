...
bool ON_IsValid(double x) { return ON_IS_VALID(x); }

bool ON_IsValidFloat(float x) { return ON_IS_VALID_FLOAT(x); }
...
double &ON_Interval::operator[](int i) { return m_t[(i <= 0) ? 0 : 1]; }

double ON_Interval::operator[](int i) const { return m_t[(i <= 0) ? 0 : 1]; }

double ON_Interval::Min() const { return m_t[IsDecreasing() ? 1 : 0]; }

void ON_Interval::Destroy() { Set(ON_UNSET_VALUE, ON_UNSET_VALUE); }
...
double ON_Interval::ParameterAt(double x) const {
    return (ON_IS_VALID(x) ? ((1.0 - x) * m_t[0] + x * m_t[1])
                           : ON_UNSET_VALUE);
}
...
double ON_Interval::NormalizedParameterAt( // returns x so that min*(1.0-x) +
                                           // max*x = input
    double t) const {
    if (!ON_IS_VALID(t))
...
ON_Interval ON_Interval::NormalizedParameterAt( // returns x so that min*(1.0-x)
                                                // + max*x = input
    ON_Interval t) const {
...
double ON_Interval::Max() const { return m_t[IsDecreasing() ? 0 : 1]; }

double ON_Interval::Mid() const { return 0.5 * (m_t[0] + m_t[1]); }
...
bool ON_Interval::IsEmptyInterval() const {
    return (m_t[0] == ON_UNSET_VALUE && m_t[1] == ON_UNSET_VALUE) ? true
                                                                  : false;
}
...
bool ON_Interval::MakeIncreasing() // returns true if resulting interval
                                   // IsIncreasing()
{
    if (IsDecreasing()) {
        Swap();
...
bool ON_Interval::Includes(double t, bool bTestOpenInterval) const {
    bool rc = false;
...
bool ON_Interval::Includes(const ON_Interval &other, bool bProperSubSet) const {
...
bool ON_3dVector::Decompose( // Computes a, b, c such that this vector = a*X +
                             // b*Y + c*Z
                             //
    ...
    row2[1] = row1[2];
    row2[2] = Z * Z;
    rank = ON_Solve3x3(row0, row1, row2, (*this) * X, (*this) * Y, (*this) * Z,
                       a, b, c, &pivot_ratio);
...
bool ON_3dVector::PerpendicularTo(const ON_3dPoint &P0, const ON_3dPoint &P1,
                                  const ON_3dPoint &P2) {
...
void ON_2dPoint::Transform(const ON_Xform &xform) {
    double xx, yy, ww;
    if (xform.m_xform) {
        ww = xform.m_xform[3][0] * x + xform.m_xform[3][1] * y +
             xform.m_xform[3][3];
        if (ww != 0.0)
            ww = 1.0 / ww;
...
void ON_3dPoint::Transform(const ON_Xform &xform) {
    ...
    if (xform.m_xform) {
        ww = xform.m_xform[3][0] * x + xform.m_xform[3][1] * y +
             xform.m_xform[3][2] * z + xform.m_xform[3][3];
        if (ww != 0.0)
            ww = 1.0 / ww;
        xx = ww * (xform.m_xform[0][0] * x + xform.m_xform[0][1] * y +
...
void ON_4dPoint::Transform(const ON_Xform &xform) {
    ...
    if (xform.m_xform) {
        xx = xform.m_xform[0][0] * x + xform.m_xform[0][1] * y +
             xform.m_xform[0][2] * z + xform.m_xform[0][3] * w;
        yy = xform.m_xform[1][0] * x + xform.m_xform[1][1] * y +
             xform.m_xform[1][2] * z + xform.m_xform[1][3] * w;
        zz = xform.m_xform[2][0] * x + xform.m_xform[2][1] * y +
             xform.m_xform[2][2] * z + xform.m_xform[2][3] * w;
        ww = xform.m_xform[3][0] * x + xform.m_xform[3][1] * y +
             xform.m_xform[3][2] * z + xform.m_xform[3][3] * w;
        x = xx;
        y = yy;
        z = zz;
        w = ww;
...
void ON_2fPoint::Rotate(double angle,            // angle in radians
                        const ON_2fPoint &center // center of rotation
) {
    Rotate(sin(angle), cos(angle), center);
}

void ON_2fPoint::Rotate(double sin_angle,        // sin(angle)
...
void ON_3fPoint::Rotate(double angle,            // angle in radians
...
void ON_3fPoint::Rotate(double sin_angle,        // sin(angle)
                        double cos_angle,        // cos(angle)
                        const ON_3fVector &axis, // axis of rotation
                        const ON_3fPoint &center // center of rotation
) {
...
bool ON_4dPoint::Normalize() {
    ...
    if (c > 0.0) {
        ...
        const double s =
            1.0 /
            (c * sqrt(f[0] * f[0] + f[1] * f[1] + f[2] * f[2] + f[3] * f[3]));
        x *= s;
        y *= s;
        z *= s;
        w *= s;
...
void ON_3fVector::Transform(const ON_Xform &xform) {
    ...
    const double dy = y;
    const double dz = z;
    double xx = xform.m_xform[0][0] * dx + xform.m_xform[0][1] * dy +
                xform.m_xform[0][2] * dz;
    double yy = xform.m_xform[1][0] * dx + xform.m_xform[1][1] * dy +
                xform.m_xform[1][2] * dz;
    double zz = xform.m_xform[2][0] * dx + xform.m_xform[2][1] * dy +
                xform.m_xform[2][2] * dz;
    x = (float)xx;
    y = (float)yy;
    z = (float)zz;
}
...
void ON_3dPoint::Rotate(double angle,            // angle in radians
...
void ON_3dPoint::Rotate(double sin_angle,        // sin(angle)
                        double cos_angle,        // cos(angle)
                        const ON_3dVector &axis, // axis of rotation
                        const ON_3dPoint &center // center of rotation
) {
...
void ON_2dPoint::Rotate(double angle,            // angle in radians
                        const ON_2dPoint &center // center of rotation
) {
    Rotate(sin(angle), cos(angle), center);
}

void ON_2dPoint::Rotate(double sin_angle,        // sin(angle)
                        double cos_angle,        // cos(angle)
                        const ON_2dPoint &center // center of rotation
) {
    ON_Xform rot;
...
void ON_2dVector::Rotate(double angle // angle in radians
) {
    Rotate(sin(angle), cos(angle));
}

void ON_2dVector::Rotate(double sin_angle, // sin(angle)
                         double cos_angle  // cos(angle)
) {
    ON_Xform rot;
...
void ON_3fVector::Rotate(double angle,           // angle in radians
                         const ON_3fVector &axis // axis of rotation
) {
    Rotate(sin(angle), cos(angle), axis);
}

void ON_3fVector::Rotate(double sin_angle,       // sin(angle)
                         double cos_angle,       // cos(angle)
                         const ON_3fVector &axis // axis of rotation
) {
    // bool rc = false;
...
void ON_3dVector::Rotate(double angle,           // angle in radians
                         const ON_3dVector &axis // axis of rotation
) {
    Rotate(sin(angle), cos(angle), axis);
}

void ON_3dVector::Rotate(double sin_angle,       // sin(angle)
                         double cos_angle,       // cos(angle)
                         const ON_3dVector &axis // axis of rotation
) {
    // bool rc = false;
...
bool ON_IsOrthogonalFrame(const ON_3dVector &X, const ON_3dVector &Y,
                          const ON_3dVector &Z) {
    // returns true if X, Y, Z is an orthogonal frame
...
bool ON_IsOrthonormalFrame(const ON_3dVector &X, const ON_3dVector &Y,
                           const ON_3dVector &Z) {
    // returns true if X, Y, Z is an orthonormal frame
...
bool ON_IsRightHandFrame(const ON_3dVector &X, const ON_3dVector &Y,
                         const ON_3dVector &Z) {
    // returns true if X, Y, Z is an orthonormal right hand frame
...
const ON_3dVector ON_UNSET_VECTOR(ON_UNSET_VALUE, ON_UNSET_VALUE,
                                  ON_UNSET_VALUE);
...
float ON_2fPoint::operator[](int i) const { return (i <= 0) ? x : y; }
...
bool ON_3fPoint::operator<=(const ON_3fPoint &p) const {
    ...
    return (
        (x < p.x)
            ? true
            : ((x == p.x)
                   ? ((y < p.y) ? true : (y == p.y && z <= p.z) ? true : false)
                   : false));
}
...
void ON_3fPoint::Zero() { x = y = z = 0.0; }
...
ON_4fPoint ON_4fPoint::operator/(float d) const {
    const float one_over_d = 1.0f / d;
    return ON_4fPoint(x * one_over_d, y * one_over_d, z * one_over_d,
                      w * one_over_d);
}
...
void ON_4fPoint::Zero() { x = y = z = w = 0.0; }
...
ON_2fVector ON_2fVector::operator-() const { return ON_2fVector(-x, -y); }
...
float ON_2fVector::operator[](int i) const { return ((i <= 0) ? x : y); }
...
double ON_2fVector::LengthSquared() const { return (x * x + y * y); }

double ON_2fVector::Length() const { return ON_Length2d((double)x, (double)y); }

void ON_2fVector::Zero() { x = y = 0.0; }
...
bool ON_2fVector::IsZero() const { return (x == 0.0f && y == 0.0f); }
...
bool ON_2fVector::PerpendicularTo(const ON_2fPoint &p, const ON_2fPoint &q) {
...
ON_3fVector ON_3fVector::operator-() const { return ON_3fVector(-x, -y, -z); }
...
bool ON_3fVector::operator<=(const ON_3fVector &v) const {
    ...
    return (
        (x < v.x)
            ? true
            : ((x == v.x)
                   ? ((y < v.y) ? true : (y == v.y && z <= v.z) ? true : false)
                   : false));
}
...
double ON_3fVector::LengthSquared() const { return (x * x + y * y + z * z); }

double ON_3fVector::Length() const {
    return ON_Length3d((double)x, (double)y, (double)z);
}

void ON_3fVector::Zero() { x = y = z = 0.0; }
...
float ON_TripleProduct(const ON_3fVector &a, const ON_3fVector &b,
                       const ON_3fVector &c) {
    // = a o (b x c )
...
double ON_2dPoint::operator[](int i) const { return (i <= 0) ? x : y; }
...
void ON_3dPoint::Zero() { x = y = z = 0.0; }
...
ON_4dPoint ON_4dPoint::operator/(double d) const {
    const double one_over_d = 1.0 / d;
    return ON_4dPoint(x * one_over_d, y * one_over_d, z * one_over_d,
                      w * one_over_d);
}
...
void ON_4dPoint::Zero() { x = y = z = w = 0.0; }
...
ON_2dVector ON_2dVector::operator-() const { return ON_2dVector(-x, -y); }
...
double ON_2dVector::operator[](int i) const { return ((i <= 0) ? x : y); }
...
double ON_2dVector::LengthSquared() const { return (x * x + y * y); }
...
double ON_2dVector::Length() const { return ON_Length2d(x, y); }

double ON_2dVector::WedgeProduct(const ON_2dVector &B) const {
    return x * B.y - y * B.x;
}

void ON_2dVector::Zero() { x = y = 0.0; }
...
bool ON_2dVector::IsZero() const { return (x == 0.0 && y == 0.0); }
...
// set this vector to be perpendicular to a line defined by 2 points
bool ON_2dVector::PerpendicularTo(const ON_2dPoint &p, const ON_2dPoint &q) {
    return PerpendicularTo(q - p);
}
...
ON_3dVector ON_3dVector::operator-() const { return ON_3dVector(-x, -y, -z); }
...
double ON_3dVector::LengthSquared() const { return (x * x + y * y + z * z); }
...
double ON_3dVector::Length() const { return ON_Length3d(x, y, z); }

void ON_3dVector::Zero() { x = y = z = 0.0; }
...
bool ON_3dVector::IsZero() const { return (x == 0.0 && y == 0.0 && z == 0.0); }
...
double ON_TripleProduct(const ON_3dVector &a, const ON_3dVector &b,
                        const ON_3dVector &c) {
    // = a o (b x c )
    return (a.x * (b.y * c.z - b.z * c.y) + a.y * (b.z * c.x - b.x * c.z) +
            a.z * (b.x * c.y - b.y * c.x));
...
bool ON_4dPoint::IsValid() const {
    return (ON_IS_VALID(x) && ON_IS_VALID(y) && ON_IS_VALID(z) &&
            ON_IS_VALID(w))
               ? true
               : false;
}
...
bool ON_PlaneEquation::Transform(const ON_Xform &xform) {
    ...
    if (rc) {
        ...
        if (rc) {
            ...
            const double zz = z;
            const double dd = d;
            x = T.m_xform[0][0] * xx + T.m_xform[1][0] * yy +
                T.m_xform[2][0] * zz + T.m_xform[3][0] * dd;
            y = T.m_xform[0][1] * xx + T.m_xform[1][1] * yy +
                T.m_xform[2][1] * zz + T.m_xform[3][1] * dd;
            z = T.m_xform[0][2] * xx + T.m_xform[1][2] * yy +
                T.m_xform[2][2] * zz + T.m_xform[3][2] * dd;
            d = T.m_xform[0][3] * xx + T.m_xform[1][3] * yy +
                T.m_xform[2][3] * zz + T.m_xform[3][3] * dd;
        }
    }
    return rc;
}
...
double *ON_PlaneEquation::ValueAt(int Pcount, const ON_3fPoint *P,
                                  double *value, double value_range[2]) const {
    if (Pcount <= 0 || 0 == P)
        return 0;
...
double *ON_PlaneEquation::ValueAt(int Pcount, const ON_3dPoint *P,
                                  double *value, double value_range[2]) const {
    if (Pcount <= 0 || 0 == P)
        return 0;
    ...
    if (0 != value_range) {
        value[0] = s =
            e[0] * (P[0].x) + e[1] * (P[0].y) + e[2] * (P[0].z) + e[3];
        value_range[0] = s;
        value_range[1] = s;
        for (i = 1; i < Pcount; i++) {
            value[i] = s =
                e[0] * (P[i].x) + e[1] * (P[i].y) + e[2] * (P[i].z) + e[3];
            if (s < value_range[0])
                value_range[0] = s;
            else if (s > value_range[1])
                value_range[1] = s;
...
ON_3dPoint ON_PlaneEquation::ClosestPointTo(ON_3dPoint P) const {
    const double t =
        -(x * P.x + y * P.y + z * P.z + d) / (x * x + y * y + z * z);
...
double ON_PlaneEquation::MaximumAbsoluteValueAt(bool bRational, int point_count,
                                                int point_stride,
                                                const double *points,
    ...
    if (ON_IsValid(stop_value)) {
        if (bRational) {
            w = points[3];
            w = (0.0 != w) ? 1.0 / w : 1.0;
            max_value = fabs(x * w * points[0] + y * w * points[1] +
                             z * w * points[2] + points[3]);
            ...
            for (point_count--; point_count--; /*empty iterator*/) {
                ...
                w = points[3];
                w = (0.0 != w) ? 1.0 / w : 1.0;
                value = fabs(x * w * points[0] + y * w * points[1] +
                             z * w * points[2] + points[3]);
                if (value > max_value) {
...
double ON_PlaneEquation::MaximumValueAt(bool bRational, int point_count,
    ...
    if (ON_IsValid(stop_value)) {
        if (bRational) {
            w = points[3];
            w = (0.0 != w) ? 1.0 / w : 1.0;
            max_value = x * w * points[0] + y * w * points[1] +
                        z * w * points[2] + points[3];
            if (max_value > stop_value)
                return max_value;
            for (point_count--; point_count--; /*empty iterator*/) {
                ...
                w = points[3];
                w = (0.0 != w) ? 1.0 / w : 1.0;
                value = x * w * points[0] + y * w * points[1] +
                        z * w * points[2] + points[3];
                if (value > max_value) {
                    if ((max_value = value) > stop_value)
                        return max_value;
...
double ON_PlaneEquation::MinimumValueAt(bool bRational, int point_count,
                                        int point_stride, const double *points,
                                        double stop_value) const {
    ...
    if (ON_IsValid(stop_value)) {
        if (bRational) {
            w = points[3];
            w = (0.0 != w) ? 1.0 / w : 1.0;
            min_value = x * w * points[0] + y * w * points[1] +
                        z * w * points[2] + points[3];
...
bool ON_PlaneEquation::IsNearerThan(const ON_BezierCurve &bezcrv, double s0,
                                    double s1, int sample_count,
...
bool ON_BoundingBox::IsValid() const {
    return (m_min.x <= m_max.x && ON_IS_VALID(m_min.x) &&
            ON_IS_VALID(m_max.x) && m_min.y <= m_max.y &&
            ON_IS_VALID(m_min.y) && ON_IS_VALID(m_max.y) &&
...
void ON_BoundingBox::Dump(ON_TextLog &text_log) const {
    ...
    text_log.Print("%.17g to %.17g, %.17g to %.17g, %.17g to %.17g\n", m_min.x,
                   m_max.x, m_min.y, m_max.y, m_min.z, m_max.z);
}

bool ON_IsDegenrateConicHelper(double A, double B, double C, double D,
                               double E) {
    //
...
bool ON_GetConicEquationThrough6Points(int stride, const double *points2d,
                                       double conic[6], double *max_pivot,
                                       double *min_pivot, double *zero_pivot) {
    ...
    // 1,1,1,1,0 on the diagonal. The vector (A,B,C,D,E) =
    // N*Transpose(0,0,0,0,1) will be in the kernel of the original M[][]. The
    // conic through the six points(
    // scale*(pts[0]-pts[5]),...,scale*(pts[4]-pts[5]),(0,0) ) is Ax^2 + Bxy +
    // Cy^2 + Dx + Ey = 0. We need to apply the inverse of the scale and then
    // translate by (pts[5].x,pts[5].y) to get the equation of the conic through
...
bool ON_IsConicEquationAnEllipse(const double conic[6], ON_2dPoint &center,
                                 ON_2dVector &major_axis,
                                 ON_2dVector &minor_axis, double *major_radius,
                                 double *minor_radius) {
    double A, C, D, E, F, x0, y0;
    double X[2], Y[2], P[2];

    if (!ON_IsValid(conic[0]) || !ON_IsValid(conic[1]) ||
        !ON_IsValid(conic[2]) || !ON_IsValid(conic[3]) ||
        !ON_IsValid(conic[4]) || !ON_IsValid(conic[5])) {
        return false;
    }
...
bool ON_GetEllipseConicEquation(double a, double b, double x0, double y0,
                                double alpha, double conic[6]) {
    if (0 == conic)
        return false;
