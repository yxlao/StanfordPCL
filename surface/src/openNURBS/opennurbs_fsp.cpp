...
size_t ON_FixedSizePool::SizeofElement() const { return m_sizeof_element; }
...
bool ON_FixedSizePool::Create(size_t sizeof_element,
    ...
    if (block_element_capacity <= 0) {
        ...
        block_element_capacity =
            (page_count * page_size - overhead) / m_sizeof_element;
...
void ON_FixedSizePool::ReturnAll() {
    if (0 != m_first_block) {
        ...
        m_al_element_array =
            (void *)(((char *)m_al_block) + 2 * sizeof(void *));
...
void ON_FixedSizePool::EmergencyDestroy() { memset(this, 0, sizeof(*this)); }
...
void *ON_FixedSizePool::AllocateElement() {
    ...
    if (0 != m_al_element_stack) {
        ...
        if (0 == m_al_block || 0 == m_al_count) {
            ...
            if (0 == next_block) {
                // This if clause is used when we need to allocate a new block
                // from the heap
                if (0 == m_sizeof_element) {
                    ON_ERROR("ON_FixedSizePool::AllocateElement - you must "
                             "call ON_FixedSizePool::Create with a valid "
                ...
                *((void **)(((char *)p) + sizeof(void *))) =
                    ((char *)p) +
                    (2 * sizeof(void *) + m_al_count * m_sizeof_element);
                if (0 == m_first_block) {
                    m_first_block = p;
                    // If the call to Create() specified a positive
                    // element_count_estimate, then m_sizeof_block needs to be
                    // reset for any future block allocations.

                } else {
                    // If m_first_block != 0, then m_al_block is nonzero (or
                    // memory for this class has been trashed)
                    *((void **)m_al_block) = p;
                }
                m_al_block = p;
            } else {
        ...
        m_al_element_array =
            (void *)(((char *)m_al_element_array) + m_sizeof_element);
...
void ON_FixedSizePool::ReturnElement(void *p) {
    if (p) {
        if (m_active_element_count <= 0) {
            ...
            ON_ERROR(
                "ON_FixedSizePool::ReturnElement - no active elements exist.");
        } else {
            m_active_element_count--;
...
void *ON_FixedSizePool::NextElement() {
    if (m_it_element) {
        ...
        if (m_it_element == m_al_element_array) {
            m_it_block = 0;
            m_it_element = 0;
        } else if (m_it_element ==
                   *((void **)(((char *)m_it_block) + sizeof(void *)))) {
            // m_it_element  = "end" pointer which means we are at the end of
            // m_it_block
            m_it_block = *((void **)m_it_block); // m_it_block = "next" block
            m_it_element =
                (0 != m_it_block) // m_it_element points to first element in
                                  // m_it_block
...
void *ON_FixedSizePool::FirstElement(size_t element_index) {
    ...
    if (element_index >= 0 && element_index < m_total_element_count) {
        for (block = (const char *)m_first_block; 0 != block;
             block = next_block) {
            if (block == m_al_block) {
                next_block = 0;
            ...
            if (element_index < block_count) {
                ...
                m_it_element =
                    ((void *)(block + (2 * sizeof(void *) +
                                       element_index * m_sizeof_element)));
                break;
            }
...
void *ON_FixedSizePool::NextBlock(size_t *block_element_count) {
    if (0 != m_it_block && m_it_block != m_al_block &&
...
void *ON_FixedSizePool::Element(size_t element_index) const {
    ...
    if (element_index >= 0) {
        for (block = (const char *)m_first_block; 0 != block;
            ...
            block_count = (block_end - block) / m_sizeof_element;
            if (element_index < block_count)
                return ((void *)(block + (2 * sizeof(void *) +
                                          element_index * m_sizeof_element)));
            element_index -= block_count;
        }
    }
