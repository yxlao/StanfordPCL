...
ON_OBJECT_IMPLEMENT(ON_PolyCurve, ON_Curve,
...
bool ON_PolyCurve::IsValid(bool bAllowGaps, ON_TextLog *text_log) const {
    ...
    if (rc && m_t.Count() != count + 1) {
        if (text_log)
            text_log->Print("Polycurve segment count = %d and m_t.Count()=%d "
                            "(should be segment count+1)\n",
                            count, m_t.Count());
        rc = false;
    }
    ...
    for (segment_index = 0; segment_index < count && rc; segment_index++) {
        ...
        if (rc && m_t[segment_index] >= m_t[segment_index + 1]) {
            if (text_log)
                text_log->Print("Polycurve m_t[%d]=%g and m_t[%d]=%g (should "
                                "be increasing)\n",
        ...
        if (rc && count > 1 && !bAllowGaps &&
            m_segment[segment_index]->IsClosed()) {
            if (text_log)
                text_log->Print(
    ...
    if (rc && !bAllowGaps) {
        ...
        if (segment_index > 0) {
            ...
            if (text_log)
                text_log->Print("Polycurve end of segment[%d] != start of "
...
ON_BOOL32 ON_PolyCurve::Write(ON_BinaryArchive &file // open binary file
) const {
...
ON_BOOL32 ON_PolyCurve::Read(ON_BinaryArchive &file // open binary file
) {
    // NOTE - if changed, check legacy I/O code
    Destroy();
    int major_version = 0;
    ...
    if (rc) {
        ...
        for (segment_index = 0; segment_index < segment_count && rc;
             segment_index++) {
            obj = 0;
            crv = 0;
            ...
            if (rc) {
                ...
                if (crv) {
                    ...
                    ON_ERROR("ON_PolyCurve::Read() - non ON_Curve object in "
                             "segment list\n");
                    delete obj;
                    rc = false;
                }
        ...
        if (rc && m_segment.Count() > 0 && m_segment.Count() == segment_count &&
            ...
            d1 = in1.Length();
            for (segment_index = 1; segment_index < segment_count;
                 segment_index++) {
                t = m_t[segment_index];
                in0 = in1;
                d0 = d1;
    ...
    if (rc) {
        ...
        if (old_count == 1) {
            ...
            if (old_dom.Includes(k, true)) {
                int segment_index =
                    ON_NurbsSpanIndex(2, old_count + 1, m_t.Array(), k, 0, 0);
                ...
                if (k >= m_t[segment_index + 1])
                    return false;
                int new_count =
                    (k == m_t[segment_index]) ? old_count : old_count + 1;
                ON_Curve *sleft = 0;
                ON_Curve *sright = 0;
                if (new_count > old_count) {
                    ON_Interval subdom(m_t[segment_index],
                                       m_t[segment_index + 1]);
                ...
                if (rc && segment_index < old_count) {
                    ...
                    new_t.Append(old_count - segment_index - 1,
                                 m_t.Array() + segment_index + 1);
                    int j = new_t.Count();
                    ...
                    memset(m_segment.Array(), 0,
                           m_segment.Capacity() * sizeof(*m_segment.Array()));
...
int ON_PolyCurve::IsPolyline(ON_SimpleArray<ON_3dPoint> *pline_points,
    ...
    if (seg_count == 1) {
        ...
        if (pline_t && rc > 0) {
            ...
            if (sdom != cdom) {
                for (i = 0; i < pline_t->Count(); i++)
                    (*pline_t)[i] = sdom.ParameterAt(
                        cdom.NormalizedParameterAt((*pline_t)[i]));
            }
        }
    } else if (seg_count > 1) {
        ...
        for (seg_i = 0; seg_i < seg_count; seg_i++) {
            ...
            seg_t.SetCount(0);
            seg_rc = m_segment[seg_i]->IsPolyline(
                pline_points ? &seg_points : 0, pline_t ? &seg_t : 0);
            if (seg_rc < 2) {
                if (pline_points)
                    pline_points->SetCount(0);
                if (pline_t)
            ...
            if (pline_t) {
                ...
                if (sdom != cdom) {
                    for (i = 0; i < seg_t.Count(); i++)
                        seg_t[i] = sdom.ParameterAt(
                            cdom.NormalizedParameterAt(seg_t[i]));
                }
                if (pline_t->Count() > 0)
                    pline_t->Remove();
...
ON_PolyCurve::IsPlanar(
    ...
    if (count == 1 && crv)
        ...
        if (!GetTestPlane(*this, test_plane)) {
            ...
            for (i = 1; i < count; i++) {
                if (m_segment[i]) {
                    Q = m_segment[i]->PointAt(
                        m_segment[i]->Domain().ParameterAt(0.5));
    ...
    if (count > 1) {
        ...
        for (i = 1; i < count; i++) {
            ...
            if (ON_ComparePoint(3, false, &P0.x, &P1.x)) {
                ...
                const double tol0 = arc0
                                        ? (arc0->m_arc.radius *
                                           arc0->m_arc.AngleRadians() * 1.0e-10)
                                        : 0.0;
                const double tol1 = arc1
                                        ? (arc1->m_arc.radius *
                                           arc1->m_arc.AngleRadians() * 1.0e-10)
                                        : 0.0;
                if (tol < tol0)
                    tol = tol0;
                if (tol < tol1)
...
bool ON_PolyCurve::GetNextDiscontinuity(ON::continuity c, double t0, double t1,
    ...
    if (count > 0 && t0 != t1) {
        ...
        int segment_index = ON_NurbsSpanIndex(2, count + 1, m_t, t0,
                                              (t0 > t1) ? -1 : 1, segment_hint);
        curve_hint =
            (hint && segment_hint == segment_index) ? ((*hint) >> 14) : 0;
        ...
        for (/*empty*/;
            ...
            rc = crv->GetNextDiscontinuity(c, s0, s1, &s, &curve_hint, dtype,
                                           cos_angle_tolerance,
                                           curvature_tolerance);
            if (rc) {
                double kink_t;
                if (sdom == cdom) {
                    ...
                    double t_tol = (fabs(t0) + fabs(t1) + fabs(t0 - t1)) *
                                   ON_ZERO_TOLERANCE;
                    if (kink_t <= tmin + t_tol || kink_t >= tmax - t_tol) {
                        ...
                        if (e < 1.0)
                            ...
                            e = 1000.0;
                        double s_tol = (fabs(s0) + fabs(s1) + fabs(s0 - s1)) *
                                       ON_ZERO_TOLERANCE * e;
                        ...
                        rc = crv->GetNextDiscontinuity(
                            c, s0, s1, &s, &curve_hint, dtype,
                            cos_angle_tolerance, curvature_tolerance);
                        if (rc) {
                            kink_t =
                                sdom.ParameterAt(cdom.NormalizedParameterAt(s));
            ...
            if (t0 > t1) {
                if (sdom[0] <= t1) // this line is correct - search is
                                   // decreasing towards t1
                {
            ...
            case ON::G1_continuous:
                ...
                crv1->Ev1Der(crv1_t, Pp, D1p,
                             -crv0_side); // corresponding point on next curve
            ...
            case ON::Gsmooth_continuous:
                crv->Ev2Der(crv0_t, Pm, D1m, D2m,
                            crv0_side); // point on this curve
                crv1->Ev2Der(crv1_t, Pp, D1p, D2p,
                             -crv0_side); // corresponding point on next curve
                if (c == ON::C2_continuous) {
                    if (!(D1m - D1p).IsTiny(D1m.MaximumCoordinate() *
                        ...
                        if (dtype)
                            *dtype = 1;
                    } else if (!(D2m - D2p).IsTiny(D2m.MaximumCoordinate() *
                                                   ON_SQRT_EPSILON)) {
                        rc = true;
                        if (dtype)
                            *dtype = 2;
                    ...
                    if (Tm * Tp < cos_angle_tolerance) {
                        ...
                        if (!bIsCurvatureContinuous) {
                            ...
                            bool b0 = seg0->LastSpanIsLinear(
                                is_linear_min_length, is_linear_tolerance);
                            bool b1 = seg1->FirstSpanIsLinear(
                                is_linear_min_length, is_linear_tolerance);
                            if (b0 != b1) {
                                rc = true;
                                if (dtype)
        ...
        if (!rc && input_c != c) {
            ...
            rc = ON_Curve::GetNextDiscontinuity(input_c, t0, t1, t, NULL, dtype,
                                                cos_angle_tolerance,
                                                curvature_tolerance);
        }
    }
...
bool ON_PolyCurve::IsContinuous(
    ...
    if (count > 0) {
        ...
        int segment_index =
            ON_NurbsSpanIndex(2, count + 1, m_t, t, 1, segment_hint);

        {
            ...
            double segtol =
                ...
                ON_SQRT_EPSILON;
            if (m_t[segment_index] + segtol < m_t[segment_index + 1] - segtol) {
                if (fabs(t - m_t[segment_index]) <= segtol &&
                    segment_index > 0) {
                    t = m_t[segment_index];
                } else if (fabs(t - m_t[segment_index + 1]) <= segtol &&
                           segment_index + 1 < count) {
                    t = m_t[segment_index + 1];
                    segment_index = ON_NurbsSpanIndex(2, count + 1, m_t, t, 1,
                                                      segment_hint);
                }
            }
        }
        ...
        if (m_t[segment_index] < t && t < m_t[segment_index + 1]) {
            ...
            if (segment_curve) {
                ...
                rc = segment_curve->IsContinuous(
                    desired_continuity, t, &curve_hint, point_tolerance,
            ...
            if (segment_index == 0 && t == m_t[0])
                ...
                if (0 != rc && ON::Gsmooth_continuous == desired_continuity &&
                    ...
                    const int i0 = (t >= m_t[segment_index]) ? segment_index - 1
                                                             : segment_index;
                    if (i0 >= 0 && t == m_t[i0 + 1]) {
...
bool ON_TuneupEvaluationParameter(int side, double s0,
...
ON_BOOL32 ON_PolyCurve::Evaluate( // returns false if unable to evaluate
    ...
    if (count > 0 && dim > 0 && dim <= v_stride) {
        segment_hint = (hint) ? (*hint & 0x3FFF) : 0;
        int segment_index =
            ON_NurbsSpanIndex(2, count + 1, m_t, t, side, segment_hint);
        if (-2 == side || 2 == side) {
            // 9 November 2010 Dale Lear - ON_TuneupEvaluationParameter fix
            //   When evluation passes through ON_CurveProxy or ON_PolyCurve
            //   reparamterization and the original side parameter was -1 or +1,
            //   it is changed to -2 or +2 to indicate that if t is numerically
            //   closed to an end paramter, then it should be tuned up to be at
            //   the end paramter.
            double a = t;
            if (ON_TuneupEvaluationParameter(side, m_t[segment_index],
                ...
                t = a;
                segment_index = ON_NurbsSpanIndex(2, count + 1, m_t, t, side,
                                                  segment_index);
            }
        }
        ...
        if (c) {
            ...
            if (s0 != s1) {
                ...
                if (s0 == t0 && s1 == t1) {
                    ...
                    if (fabs(t1 - t0) <
                        (ON_ZERO_TOLERANCE + ON_EPSILON * fabs(t0))) {
                ...
                curve_hint = (hint && segment_hint == segment_index)
                                 ? ((*hint) >> 14)
                                 : 0;
                ...
                if (rc) {
                    if (der_count > 0 && s1 - s0 != t1 - t0 && t0 != t1) {
                        // Adjust segment derivative evaluation bug by applying
                        // chain rule to get polycurve derivative value.
...
int ON_PolyCurve::Count() const { return m_segment.Count(); }
...
ON_Curve *ON_PolyCurve::FirstSegmentCurve() const { return SegmentCurve(0); }
...
ON_BOOL32 ON_PolyCurve::Prepend(ON_Curve *c) { return Insert(0, c); }

ON_BOOL32 ON_PolyCurve::Append(ON_Curve *c) { return Insert(Count(), c); }
...
ON_BOOL32 ON_PolyCurve::Remove() { return Remove(Count() - 1); }
...
ON_BOOL32 ON_PolyCurve::Remove(int segment_index) {
    ...
    if (segment_index >= 0 && segment_index < segment_count) {
        ...
        // GBA 18 September 2003. m_t array not properly updated when last
        // segment removed.
        if (segment_index >= 1) {
            double *d = m_t.Array();
        ...
        // GBA 12/02/02.  When removing the last segment remove both m_t values
        // so the polycurve will have the same state as ON_PolyCurve().
        if (segment_count == 1)
            m_t.Empty();
        else
...
int ON_PolyCurve::SegmentIndex(ON_Interval sub_domain, int *segment_index0,
...
ON_PolyCurve::GetCurveParameterFromNurbFormParameter(double nurbs_t,
    ...
    if (curve) {
        ...
        if (in != cdom) {
            ...
            rc =
                curve->GetCurveParameterFromNurbFormParameter(nurbs_t, curve_t);
            if (rc)
...
ON_PolyCurve::GetNurbFormParameterFromCurveParameter(double curve_t,
    ...
    if (curve) {
        ...
        if (in != cdom) {
            ...
            rc =
                curve->GetNurbFormParameterFromCurveParameter(curve_t, nurbs_t);
            if (rc)
...
ON_BOOL32 ON_PolyCurve::Trim(const ON_Interval &domain) {
    ...
    if (m_t.Count() < 2 || segment_count + 1 != m_t.Count() ||
        !domain.IsIncreasing()) {
        // bogus input
        return false;
    }
    ...
    if (s0 < 0 || s0 > s1 || s1 >= segment_count) {
        ...
        // the only way that (s0 < 0 || s0 > s1 || s1 >= segment_count) can be
        // true is if something is seriously wrong with the m_t[] values.
        return false;
    }
    ...
    const double fuzz = 0.001; // Greg says: anything small and > 1.0e-6 will
                               // work about the same
    ...
    if (bTrimFirstSegment) {
        ...
        if (c_dom != s_dom) {
            // need to convert polycurve parameters to "real" segment curve
            // parameters
            trim_c_dom[0] =
                c_dom.ParameterAt(s_dom.NormalizedParameterAt(trim_s_dom[0]));
            trim_c_dom[1] =
                c_dom.ParameterAt(s_dom.NormalizedParameterAt(trim_s_dom[1]));
            if (!trim_c_dom.IsIncreasing()) {
                if (s_dom.NormalizedParameterAt(trim_s_dom[0]) >= 1.0 - fuzz &&
                    s1 > 0) {
                    // We were trying to throw away all but a microscopic bit on
                    // the right end of the first segment of a multi segment
                    // polycurve and the parameter conversion killed the "real"
                    // trim interval. In this case, we can just throw away the
                    // first segment.
                    ...
                    m_t.Remove(0);
                    m_segment.Remove(0);
                    s1--; // removing a segment, means s1=(index of last valid
                          // segment) has to get decremented.
                } else
                    return false;
            }
        } else {
        ...
        if (bTrimFirstSegment && trim_c_dom != c_dom) {
            ...
            if (!curve->Trim(trim_c_dom)) {
                ...
                if (c_dom.NormalizedParameterAt(trim_c_dom[0]) >= 1.0 - fuzz &&
                    s1 > 0) {
                ...
                m_t[0] = actual_trim_domain[0]; // will be tweaked below when
                                                // we've finished.
    ...
    if (bTrimLastSegment) {
        ...
        if (0 == curve)
            return false; // bogus polycurve (m_segment[s1] array has a null
                          // pointer)
        ...
        if (c_dom != s_dom) {
            trim_c_dom[1] =
                c_dom.ParameterAt(s_dom.NormalizedParameterAt(trim_s_dom[1]));
            if (!trim_c_dom.IsIncreasing()) {
                if (s_dom.NormalizedParameterAt(trim_s_dom[1]) <= fuzz &&
                    s1 > 0) {
                    // We were trying to throw away all but a microscopic bit on
                    // the left end of the last segment of a multi segment
                    // polycurve and the parameter conversion killed the "real"
                    // trim interval. In this case, we can just throw away the
                    // last segment.
                    ...
                    m_segment.Remove(); // remove last array entry in the
                                        // m_segment[] array
                    s1--; // removing a segment, means s1=(index of last valid
                          // segment) has to get decremented.
                } else
        ...
        if (bTrimLastSegment && c_dom != trim_c_dom) {
            ...
            if (!curve->Trim(trim_c_dom)) {
                ...
                if (c_dom.NormalizedParameterAt(trim_c_dom[1]) <= fuzz &&
                    s1 > 0) {
                    // We were trying to throw away all but a microscopic bit on
                    // the left end of the last segment of a multi segment
                    // polycurve and the segment curve's trimmer failed.  I'm
                    // assuming the failure was caused because the part that
                    // would be left was too short. In this case, we can just
                    // throw away the last segment.
                    bTrimLastSegment = false;
                    curve = 0;
                    delete m_segment[s1];
                    m_segment[s1] = 0;
                ...
                m_t[m_t.Count() - 1] =
                    actual_trim_domain[1]; // will be tweaked below when we've
                                           // finished.
        }
    }
    ...
    if (Domain()[0] > domain[0]) {
        ...
        if (changed) {
            if (seg->Domain() == DesiredDom)
                ...
                m_t[0] = sdom.ParameterAt(
                    cdom.NormalizedParameterAt(seg->Domain()[0]));
        }
    }
    if (Domain()[1] < domain[1]) {
        ...
        if (chgd) {
            if (seg->Domain() == DesiredDom)
                ...
                m_t[Count()] = sdom.ParameterAt(
                    cdom.NormalizedParameterAt(seg->Domain()[1]));
            changed = true;
        }
    }
...
ON_PolyCurve::Split(double split_parameter,
    ...
    */
    ...
    if (split_at_break)
        ...
        c = (c_dom == s_dom) ? split_parameter
                             : c_dom.ParameterAt(s_dom.NormalizedParameterAt(
                                   split_parameter));
    ...
    if (!split_at_break && c_dom.Includes(c, true)) {
        if (!seg_curve->Split(c, seg_left, seg_right)) {
            double fuzz =
                0.001; // anything small and > 1.0e-6 will work about the same
    ...
    if (seg_left && seg_right) {
        ...
        if ((c == c_dom[0] && si == 0) || // attempting split at curve start
            (c == c_dom[1] &&
             si == m_segment.Count())) // attempting split at curve end
            return false;
...
static void Flatten(ON_PolyCurve *poly, ON_Interval pdom,
...
bool ON_PolyCurve::HasSynchronizedSegmentDomains() const {
    ...
    for (i = 0; i < count; i++) {
        t0 = -ON_UNSET_VALUE;
        t1 = ON_UNSET_VALUE;
        if (0 != c[i] && c[i]->GetDomain(&t0, &t1) && t0 == t[i] &&
            t1 == t[i + 1]) {
            continue;
        }
        return false;
...
void ON_PolyCurve::RemoveNesting() { RemoveNestingEx(); }
...
bool ON_PolyCurve::ParameterSearch(double t, int &index,
                                   bool bEnableSnap) const {
    return ON_Curve::ParameterSearch(t, index, bEnableSnap, m_t,
                                     ON_SQRT_EPSILON);
}

const ON_CurveArray &ON_PolyCurve::SegmentCurves() const { return m_segment; }
