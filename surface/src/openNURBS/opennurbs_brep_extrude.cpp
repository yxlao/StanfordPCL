...
ON_BrepExtrudeHelper_MakeSides(ON_Brep &brep, int loop_index,
    ...
    side_face_index.Reserve(side_face_index.Count() +
                            loop_trim_count); // index of new face above
                                              // brep.m_L[loop_index].m_ti[lti]
    ...
    for (lti = 0; lti < loop_trim_count; lti++) {
        ...
        // get side surface for new face
        {
            ...
            if (trim.m_ei >= 0 && trim.m_ei < edge_count0) {
                ...
                sum_srf = ON_BrepExtrudeHelper_MakeSumSrf(path_curve, base_edge,
                                                          trim.m_bRev3d);
            }
        }
        if (!sum_srf)
        ...
        if (first_face_east_trim_index >= 0 &&
            ...
            const ON_BrepTrim &first_face_east_trim =
                brep.m_T[first_face_east_trim_index];
            ...
            *side_face_index.Last() = side_face->m_face_index;
            ...
            if (first_face_east_trim_index < 0)
                first_face_east_trim_index =
                    brep.m_L[side_face->m_li[0]].m_ti[1];
        }
    }
...
static bool ON_BrepExtrudeHelper_MakeTopLoop(
    ON_Brep &brep, ON_BrepFace &top_face, int bottom_loop_index,
    const ON_3dVector path_vector,
    const int
        *side_face_index // array of brep.m_L[bottom_loop_index].m_ti.Count()
                         // face indices
) {
    bool rc = true;
    ...
    for (lti = 0; lti < loop_trim_count; lti++) {
        if (side_face_index[lti] >= 0) {
            ...
            top_vertex_index[(lti + 1) % loop_trim_count] =
                side_north_trim.m_vi[1];
            ...
            if (side_face_index[lti_prev] < 0 &&
                side_face_index[lti_next] < 0 && top_vertex_index[lti] < 0 &&
                ...
                if (bottom_ti >= 0 && bottom_ti < brep.m_T.Count() &&
                    ...
                    const ON_BrepTrim &bottom_trim_prev =
                        brep.m_T[bottom_ti_prev];
                    const ON_BrepTrim &bottom_trim = brep.m_T[bottom_ti];
                    const ON_BrepTrim &bottom_trim_next =
                        brep.m_T[bottom_ti_next];
                    if (ON_BrepTrim::seam == bottom_trim_prev.m_type &&
                        ...
                        if (vi >= 0 && vi < brep.m_V.Count()) {
                            ON_BrepVertex &top_vertex = brep.NewVertex(
                                brep.m_V[vi].point + path_vector, 0.0);
            ...
            for (lti = 0; lti < loop_trim_count; lti++) {
                if (top_vertex_index[lti] == -1) {
                    for (i = lti + 1; i < loop_trim_count; i++) {
                        if (ON_BrepTrim::singular !=
                            brep.m_T[bottom_loop.m_ti[i - 1]].m_type)
                            break;
    ...
    for (lti = 0; lti < loop_trim_count; lti++) {
        ...
        ON_BrepEdge &top_edge =
            brep.NewEdge(brep.m_V[e_vi0], brep.m_V[e_vi1], top_c3i);
        ...
        for (mate_lti = lti + 1; mate_lti < loop_trim_count; mate_lti++) {
            ...
            top_trim_bRev3d[mate_lti] =
                bottom_mate_trim.m_bRev3d ? true : false;
            break;
        }
    }
    ...
    for (lti = 0; lti < loop_trim_count; lti++) {
        ...
        int top_c2i =
            (0 != top_c2) ? brep.AddTrimCurve(top_c2) : bottom_trim.m_c2i;
        top_trim_index = -1;
        if (bottom_trim.m_type == ON_BrepTrim::singular &&
            top_vertex_index[lti] >= 0) {
            top_trim_index =
                brep.NewSingularTrim(brep.m_V[top_vertex_index[lti]], top_loop,
                                     bottom_trim.m_iso, top_c2i)
                    .m_trim_index;
        } else if (bottom_trim.m_type != ON_BrepTrim::singular &&
                   top_edge_index[lti] >= 0 &&
                   top_edge_index[lti] < brep.m_E.Count()) {
            ON_BrepEdge &top_edge = brep.m_E[top_edge_index[lti]];
            top_trim_index =
                brep.NewTrim(top_edge, top_trim_bRev3d[lti], top_loop, top_c2i)
                    .m_trim_index;
        } else {
            ON_ERROR(
                "ON_BrepExtrudeHelper_MakeTopLoop ran into capping trouble.");
            rc = false;
            break;
        }
...
static bool ON_BrepExtrudeHelper_MakeCap(ON_Brep &brep, int bottom_loop_index,
                                         const ON_3dVector path_vector,
...
int ON_BrepExtrudeFace(ON_Brep &brep, int face_index,
                       const ON_Curve &path_curve, bool bCap) {
    ...
    if (brep.m_F[face_index].m_li.Count() == 1) {
        rc = ON_BrepExtrudeLoop(brep, brep.m_F[face_index].m_li[0], path_curve,
                                bCap);
    } else {
        ON_3dVector path_vector;
        ON_SimpleArray<int> side_face_index;
        ...
        if (new_side_trim_count == 0)
            return false;
        ON_BrepExtrudeHelper_ReserveSpace(brep, new_side_trim_count,
                                          bCap ? 1 : 0);
        ...
        for (fli = 0; fli < face_loop_count && rc; fli++) {
            ...
            rc = ON_BrepExtrudeHelper_MakeSides(brep, li, path_curve, bCap,
                                                side_face_index);
        }
        ...
        if (bCap && rc && outer_loop_index >= 0) {
            ...
            bCap = ON_BrepExtrudeHelper_MakeCap(
                brep, outer_loop_index, path_vector,
...
int ON_BrepExtrudeLoop(ON_Brep &brep, int loop_index,
                       const ON_Curve &path_curve, bool bCap) {
    ON_SimpleArray<int> side_face_index; // index of new face above
                                         // brep.m_L[loop_index].m_ti[lti]
    ON_3dVector path_vector;
    ...
    // can only cap closed loops ( for now, just test for inner and outer
    // loops).
        ...
        for (ei = 0; ei < ecount0; ei++) {
            if (bSideEdge[ei]) {
                ...
                if (!topvimap[bottomvi0]) {
                    ...
                    ON_BrepVertex &topv = brep.NewVertex(bottomv.point + height,
                                                         bottomv.m_tolerance);
    ...
    if (bOK)
        for (ei = 0; ei < ecount0; ei++) {
            if (bCap || bSideEdge[ei]) {
                ...
                ON_BrepEdge &tope =
                    brep.NewEdge(topv0, topv1, c3i, 0, bottome.m_tolerance);
            ...
            for (int vei = 0; vei < bottomv.m_ei.Count(); vei++) {
                if (bSideEdge[bottomv.m_ei[vei]] && topvimap[vi]) {
                    ...
                    if (!c3) {
                        ...
                        int c3i = brep.AddEdgeCurve(c3);
                        const ON_BrepEdge &e =
                            brep.NewEdge(bottomv, topv, c3i, 0, 0.0);
            ...
            for (int fli = 0; fli < loop_count; fli++) {
                ...
                for (int lti = 0; lti < loop_trim_count; lti++) {
                    ...
                    if (bottomt.m_ei >= 0) {
                        ...
                        // singular trim
                        ON_BrepVertex &topv =
                            brep.m_V[topvimap[bottomt.m_vi[0]]];
                        topt = &brep.NewSingularTrim(topv, topl, bottomt.m_iso,
                                                     c2i);
                    }
            ...
            if (bSideEdge[ei] && topeimap[ei]) {
                ...
                if (sideveimap[vid[0]] && sideveimap[vid[1]]) {
                    ...
                    if (!face) {
                        ...
                        bool bBottomFaceRev =
                            brep.m_F[(loop0.m_fi != face->m_face_index)
                                         ? loop0.m_fi
                                         : loop1.m_fi]
                                .m_bRev;
    ...
    for (lti = 0; lti < loop_trim_count; lti++) {
        ...
        // get side surface for new face
        {
            ...
            if (trim.m_ei >= 0 && trim.m_ei < edge_count0) {
                ...
                eid[0] = base_edge.m_edge_index;
                bRev3d[0] = (trim.m_bRev3d ? false : true);
                cone_srf = ON_BrepExtrudeHelper_MakeConeSrf(
                    apex_point, base_edge, bRev3d[0]);
            }
        }
        if (!cone_srf)
            continue;
