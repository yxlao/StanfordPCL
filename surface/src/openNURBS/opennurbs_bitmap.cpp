...
static BITMAPINFO *ON_WindowsBitmapHelper_AllocBMI(size_t sizeof_palette,
                                                   size_t sizeof_image) {
    // In theory,
...
bool ON_WindowsBitmap::Create(int width, int height,
...
ON_WindowsBitmap &ON_WindowsBitmap::operator=(const ON_WindowsBitmap &src) {
    if (this != &src) {
        ...
        if (src.m_bmi) {
            ...
            m_bmi =
                ON_WindowsBitmapHelper_AllocBMI(sizeof_palette, sizeof_image);
            if (m_bmi) {
                m_bFreeBMI = 1;
                m_bmi->bmiHeader = src.m_bmi->bmiHeader;
                if (sizeof_palette > 0) {
                    memcpy(&m_bmi->bmiColors[0], &src.m_bmi->bmiColors[0],
                           sizeof_palette);
                }
                if (sizeof_image > 0) {
                    m_bits =
                        (unsigned char *)&m_bmi->bmiColors[PaletteColorCount()];
                    if (src.m_bits)
...
int ON_WindowsBitmap::PaletteColorCount() const {
    return m_bmi ? ON_WindowsBitmapHelper_PaletteColorCount(
...
const unsigned char *ON_WindowsBitmap::Bits(int scan_index) const {
    ...
    const unsigned char *bits =
        m_bmi ? (const unsigned char *)&m_bmi->bmiColors[PaletteColorCount()]
              : 0;
    if (bits && sizeof_scan && scan_index >= 0 && scan_index < Height()) {
        bits += (sizeof_scan * scan_index);
    } else {
...
ON_Color ON_WindowsBitmap::Pixel(int column_index,
    ...
    if (m_bmi && column_index >= 0 && column_index <= Width() && scanbits &&
        ...
        case 1:
            ...
            palindex =
                (scanbits[column_index / 8] >> (7 - (column_index % 8))) & 0x01;
            b = m_bmi->bmiColors[palindex].rgbBlue;
            g = m_bmi->bmiColors[palindex].rgbGreen;
            r = m_bmi->bmiColors[palindex].rgbRed;
            a = m_bmi->bmiColors[palindex].rgbReserved;
...
bool ON_WindowsBitmap::WriteUncompressed(ON_BinaryArchive &file) const {
    ...
    if (rc) {
        ...
        if (sizeof_image > 0 && rc) {
            if (rc)
                rc = file.WriteByte(sizeof_image,
                                    &m_bmi->bmiColors[color_count]);
        }
    }
...
bool ON_WindowsBitmap::ReadUncompressed(ON_BinaryArchive &file) {
    ...
    if (rc) {
        bmiHeader.biSize = sizeof(bmiHeader);
        const size_t sizeof_palette = ON_WindowsBitmapHelper_SizeofPalette(
    ...
    if (rc) {
        if (bContiguousBitmap) {
            ...
            rc = file.WriteCompressedBuffer(
                sizeof_buffer, (0 != m_bmi) ? m_bmi->bmiColors : 0);
        } else {
            // 28 July 2003
...
bool ON_WindowsBitmap::ReadCompressed(ON_BinaryArchive &file) {
    ...
    if (rc) {
        ...
        if (!m_bmi) {
            ...
            if (rc) {
                const size_t sizeof_colors =
                    color_count * sizeof(*m_bmi->bmiColors);
                if (sizeof_buffer == sizeof_colors ||
                    sizeof_buffer == sizeof_colors + sizeof_image) {
                    // palette and image bits are compressed into one or two
                    // chunks
                    rc = file.ReadCompressedBuffer(
                        sizeof_buffer, m_bmi->bmiColors, &bFailedCRC);
                    if (rc && sizeof_image > 0 &&
                        ...
                        if (rc) {
                            if (sizeof_buffer == sizeof_image) {
                                // image bits are compressed into a
                                // separatechunk
                                rc = file.ReadCompressedBuffer(
                                    sizeof_buffer,
                                    &m_bmi->bmiColors[color_count],
                                ...
                                ON_ERROR("ON_WindowsBitmap::ReadCompressed() "
                                         "image bits buffer size mismatch\n");
                                rc = false;
                            }
                        }
                    }
                } else {
                    ON_ERROR("ON_WindowsBitmap::ReadCompressed() buffer size "
                             "mismatch\n");
                    rc = false;
                }
            }
...
bool ON_WindowsBitmap::Create(const BITMAPINFO *bmi, const unsigned char *bits,
                              bool bCopy) {
    bool rc = false;
    Destroy();
    m_bFreeBMI = 0;
    ...
    if (0 != bmi) {
        if (bCopy) {
            ...
            m_bmi = ON_WindowsBitmapHelper_AllocBMI(sizeof_palette,
                                                    (bCopy ? sizeof_image : 0));
            if (0 != m_bmi) {
                rc = true;
...
ON_WindowsBitmap::ON_WindowsBitmap(const BITMAPINFO *src)
    ...
    if (0 != src) {
        int color_count = ON_WindowsBitmapHelper_PaletteColorCount(
            src->bmiHeader.biClrUsed, src->bmiHeader.biBitCount);
        Create(src, (const unsigned char *)(&src->bmiColors[color_count]),
               false);
    }
}
        ...
        if (rc) {
            ...
            case 1:
                if (rc) {
                    ...
                    if (rc)
                        rc = file.ReadCompressedBuffer(m_sizeof_buffer,
                                                       m_buffer, &bFailedCRC);
                }
                break;
            }
