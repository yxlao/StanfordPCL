...
bool ON_OffsetSurface::SetBaseSurface(ON_Surface *base_surface, bool bManage) {
...
ON_BOOL32 ON_OffsetSurface::GetBBox(double *bbox_min, double *bbox_max,
...
ON_BOOL32 ON_OffsetSurface::Evaluate(double s, double t, int der_count,
                                     int v_stride, double *v, int side,
                                     int *hint) const {
    ...
    if (rc) {
        double
            darray[21]; // 21 = ((5+1)*(5+2)/2) = enough room for der_count <= 5
        double *d =
            (der_count > 5)
        ...
        if (rc) {
            ON_3dVector N;
            ON_EvNormal(side, srf_value[1], srf_value[2], srf_value[3],
                        srf_value[4], srf_value[5], N);
            v[0] += d[0] * N.x;
            v[1] += d[0] * N.y;
            v[2] += d[0] * N.z;
            ...
            if (der_count > 0) {
                ON_3dVector Ns, Nt;
                ON_EvNormalPartials(srf_value[1], srf_value[2], srf_value[3],
                                    srf_value[4], srf_value[5], Ns, Nt);
...
void ON_BumpFunction::EvaluateHelperLinearBump(double t, double dt,
                                               int der_count,
                                               double *value) const {
    value[0] = t;
    if (der_count > 0) {
...
void ON_BumpFunction::EvaluateHelperQuinticBump(double t, double dt,
                                                int der_count,
                                                double *value) const {
    // c(t) = (1-t)^3 * (1 + 3t + 6t^2)
    // bool neg = (t<0.0);
...
void ON_BumpFunction::Evaluate(double s, double t, int der_count,
                               double *value) const {
    double tmp[20];
    double *xvalue;
    double *yvalue;
...
bool ON_OffsetSurfaceFunction::SetSideTangency(int side, bool bEnable) {
    bool rc = false;
    if (0 <= side && side < 4) {
...
double ON_OffsetSurfaceFunction::DistanceAt(double s, double t) const {
    double d = 0.0;
    EvaluateDistance(s, t, 0, &d);
    return d;
}

bool ON_OffsetSurfaceFunction::EvaluateDistance(double s, double t, int num_der,
                                                double *value) const {
    ...
    if (rc) {
        double barray[21];
        double *bump_value =
            (vcnt > 21) ? (double *)onmalloc(vcnt * sizeof(bump_value[0]))
                        : barray;
...
ON_3dPoint ON_OffsetSurfaceFunction::PointAt(double s, double t) const {
    // double d = 0.0;
    ON_3dPoint P;
    ON_3dVector N;
    if (0 != m_srf) {
...
bool ON_OffsetSurfaceFunction::SetOffsetPoint(double s, double t,
    ...
    if (ON_IsValid(s) && ON_IsValid(t) && ON_IsValid(distance) &&
        ...
        const double dTol =
            ON_SQRT_EPSILON; // tiny border around untrimmed edges
    ...
    if (!m_bValid && 0 != m_srf && count > 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (offset_value.m_radius > 0.0) {
                ...
                ON_3dPoint Pt;
                ON_3dVector Ds, Dt;
                if (m_srf->Ev1Der(offset_value.m_s, offset_value.m_t, Pt, Ds,
                                  Dt)) {
                    ds = (ds0 > ds1) ? ds0 : ds1;
                    dt = (dt0 > dt1) ? dt0 : dt1;
                    a = Ds.Length();
            ...
            } else if (bump.m_point.x == m_domain[0][0] &&
                       bump.m_point.y == m_domain[1][0]) {
            ...
            } else if (bump.m_point.x == m_domain[0][1] &&
                       bump.m_point.y == m_domain[1][0]) {
...
double ON_BumpFunction::ValueAt(double s, double t) const {
    double v;
    Evaluate(s, t, 0, &v);
    return v;
}
