...
void ON_EvaluatedeCasteljau(int dim, int order, int side, int cv_stride,
                            double *cv, double t)
/*****************************************************************************
...
INPUT:
  ...
  cv     array of order*cv_stride doubles that specify the Bezier's control
         vertices.
  cv_stride ( >= dim) number of doubles between cv's (typically a multiple of
dim). t      If side <= 0, then t must be > 0.0. If side >  0, then t must be
< 1.0. OUTPUT: cv If side <= 0, the input cv's are replaced with the cv's for
...
bool ON_IncreaseBezierDegree(int dim, ON_BOOL32 is_rat, int order,
                             int cv_stride, double *cv)
/*****************************************************************************
...
EXAMPLE:
  raise_degree(TL_BEZIER* bez)
  {
    ...
    bez->cv = (double*) onrealloc ( bez->cv,
(bez->order+1)*(bez->dim+bez->is_rat) ); TL_IncreaseBezierDegree (
bez->dim+bez->is_rat, bez->order,bez->cv,bez->cv ); bez->order++;
  }
REFERENCE:
  BOHM-01, Page 7.
RELATED FUNCTIONS:
...
bool ON_RemoveBezierSingAt1(int dim, int order, int cv_stride, double *cv) {
    ...
    while (order > 1 && cv[CVlen - 1] == 0.0) {
        ...
        for (i = 0; i < order; i++) {
            for (k = 0; k < cvdim; k++)
                cv[i * cv_stride + k] =
                    (order * cv[i * cv_stride + k]) / (order - i);
        }
        CVlen -= cvdim;
    }
    while (order < ord0)
...
bool ON_EvaluateBezier(int dim,              // dimension
...
*****************************************************************************/
{
    ...
    CV = (double *)((sizeofCV <= sizeof(stack_buffer))
                        ? stack_buffer
...
bool ON_EvaluateNurbsBasis(int order, const double *knot, double t, double *N) {
    ...
    OUTPUT:
      ...
      value returned is the value of the polynomial that agrees with N_i^k on
    the half open domain [ knot[d-1], knot[d] )
    ...
    COMMENTS:
      ...
      where N_{i} are the degree d b-spline basis functions and C_{i} are the
    control vertices.  The knot[] array length d+n-1 and satisfies
      ...
      N_{i}(t) = 0 if i <= k-d
               = 0 if i >= k+2
               = B[i-k+d-1] if k-d+1 <= i <= k+1, where B[] is computed by the
    call TL_EvNurbBasis( d+1, knot+k-d+1, t, B );
...
bool ON_EvaluateNurbsBasisDerivatives(int order, const double *knot,
     ...
     *
     * Actually, the above is true when knot[d-1] <= t < knot[d].  Otherwise,
     * the values returned are the values of the polynomials that agree with
     * N_i^k on the half open domain [ knot[d-1], knot[d] )
     *
     * Ref: The NURBS Book
     */
    double dN, c;
    ...
    a0 = (double *)alloca((order * (2 + ((d + 1) >> 1)))
                          << 3); /* d for a0, d for a1, d*order/2 for dk[]'s and
                                    slop to avoid /2 */
    a1 = a0 + order;
    ...
    dk--;
    /* dk[1] = 1/{t[d]-t[0], t[d+1]-t[1], ..., t[2d-2] - t[d-2], t[2d-1] -
     * t[d-1]} = diffs needed for 1rst derivative dk[2] = 1/{t[d]-t[1],
     * t[d+1]-t[2], ..., t[2d-2] - t[d-1]} = diffs needed for 2nd derivative
     * ...
     * dk[d] = 1/{t[d] - t[d-1]}
    ...
    for (i = 0; i < order; i++) {
        ...
        for (k = 1; k <= der_count; k++) {
            ...
            if (jmax < k) {
                while (j <= jmax) {
                    dN += (a1[j] = (a0[j] - a0[j - 1]) * dk[k][i + j - k]) *
                          N[i + j];
                    j++;
                }
            } else {
...
static bool ON_EvaluateNurbsNonRationalSpan(
    int dim,            // dimension
    int order,          // order
...
static bool ON_EvaluateNurbsRationalSpan(
    int dim,            // dimension
    int order,          // order
    const double *knot, // knot[] array of (2*order-2) doubles
    int cv_stride,      // cv_stride >= (is_rat)?dim+1:dim
    ...
    rc = ON_EvaluateNurbsNonRationalSpan(dim + 1, order, knot, cv_stride, cv,
                                         der_count, t, hv_stride, hv);
    if (rc) {
...
bool ON_EvaluateNurbsSpan(
    ...
    if (knot[0] == knot[order - 2] && knot[order - 1] == knot[2 * order - 3]) {
        ...
        rc = ON_EvaluateBezier(dim, is_rat, order, cv_stride, cv,
                               knot[order - 2], knot[order - 1], der_count, t,
                               v_stride, v);
    } else {
        // generic NURBS span evaluation
        rc = (is_rat)
                 ? ON_EvaluateNurbsRationalSpan(dim, order, knot, cv_stride, cv,
                                                der_count, t, v_stride, v)
                 : ON_EvaluateNurbsNonRationalSpan(dim, order, knot, cv_stride,
                                                   cv, der_count, t, v_stride,
                                                   v);
    }
    return rc;
}
...
bool ON_EvaluateNurbsSurfaceSpan(
    int dim, ON_BOOL32 is_rat, int order0, int order1, const double *knot0,
    const double *knot1, int cv_stride0, int cv_stride1,
            ...
            if (der_count > 2) {
                ...
                for (d = 3; d <= der_count; d++) {
                    ...
                    for (j0 = 0; j0 < order0; j0++) {
                        cv = cv0 + j0 * cv_stride0;
                        for (j1 = 0; j1 < order1; j1++) {
                            for (d0 = d, d1 = 0; d0 > der_count0 && d1 <= d1max;
                                 d0--, d1++) {
 ...
 *                (1-a[i,j])*D[i-1,j-1] + a[i,j]*D[i-1,j], if 0 < i <= d =
 * degree
 *
 ...
 */
{
    double workarray[21], alpha0, alpha1, t0, t1, dt, *delta_t, *free_delta_t,
        *cv0, *cv1;
    const double *k0, *k1;
    int degree, i, j, k;
    ...
    if (side < 0) {
        ...
        if (side == -2)
            ...
            side = -1;
            if (degree > 21)
                delta_t = free_delta_t =
                    (double *)onmalloc(degree * sizeof(*delta_t));
        }
        ...
        if (side != -2) {
            ...
            dt = t - t0;
            // cv += order*cv_dim; // Chuck-n-Dale 21 Sep bug fix change cv_dim
            // to cv_stride
            cv += order * cv_stride;
            k = order;
            while (--k) {
        ...
        if (side == 1) {
             ...
             * delta_t = {knot[order-1] - t, knot[order] -  t, ..
             * knot[2*order-3] - t}
             */
            k1 = knots;
            k = degree;
...
bool ON_EvaluateNurbsBlossom(
    int cvdim, int order, int cv_stride,
    ...
    for (i = 0; i < cvdim; i++) { // do one coordinate at a time
        ...
        for (j = 1; j < order; j++) {
            for (k = j; k < order; k++) { // djk
                space[k - j] =
                    (knot[degree + k - j] - t[j - 1]) /
                        (knot[degree + k - j] - knot[k - 1]) * space[k - j] +
                    (t[j - 1] - knot[k - 1]) /
                        (knot[degree + k - j] - knot[k - 1]) * space[k - j + 1];
            }
        }
...
void ON_ConvertNurbSpanToBezier(int cvdim, int order, int cvstride, double *cv,
