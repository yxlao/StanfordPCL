...
ON_OBJECT_IMPLEMENT(ON_Leader, ON_Annotation,
...
int ON_Annotation::Dimension() const { return 3; }
...
void ON_LinearDimension::SetTextToDefault() { SetUserText(L"<>"); }
...
void ON_Leader::EmergencyDestroy() { ON_Annotation::EmergencyDestroy(); }
...
int ON_AnnotationArrow::Dimension() const { return 3; }

ON_BOOL32 ON_AnnotationArrow::GetBBox(double *boxmin, double *boxmax,
                                      ON_BOOL32 bGrowBox) const {
    ON_BOOL32 rc = ON_GetPointListBoundingBox(3, false, 1, 3, m_tail, boxmin,
                                              boxmax, bGrowBox ? true : false);
    if (rc)
        rc = ON_GetPointListBoundingBox(3, false, 1, 3, m_head, boxmin, boxmax,
                                        true);
    return rc;
}
...
ON_3dVector ON_AnnotationArrow::Vector() const { return (m_head - m_tail); }

ON_3dPoint ON_AnnotationArrow::Head() const { return m_head; }

ON_3dPoint ON_AnnotationArrow::Tail() const { return m_tail; }
