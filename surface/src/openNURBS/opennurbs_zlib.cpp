...
size_t ON_BinaryArchive::WriteDeflate( // returns number of bytes written
    ...
    /*
      ...
      correctly. Each call of inflate() or deflate() is limited to input and
      output chunks of the maximum value that can be stored in the compiler's
      "unsigned int" type, but there is no limit to the number of chunks. Note
      however that the strm.total_in and strm_total_out counters may be limited
      to 4 GB. These counters are provided as a convenience and are not used
      internally by inflate() or deflate(). The application can easily set up
      its own counters updated after each call of inflate() or deflate() to
      count beyond 4 GB. compress() and uncompress() may be limited to 4 GB,
      since they operate in a single call. gzseek() and gztell() may be limited
      to 4 GB depending on how zlib is compiled. See the zlibCompileFlags()
      function in zlib.h.

          The word "may" appears several times above since there is a 4 GB limit
          only if the compiler's "long" type is 32 bits. If the compiler's
      "long" type is 64 bits, then the limit is 16 exabytes.
    */
    ...
    // counter guards prevents infinte loops if there is a bug in zlib return
    // codes.
    int counter = 512;
...
bool ON_BinaryArchive::ReadInflate(
    ...
    bool bValidCompressedBuffer = false;
    {
        ...
        if (tcode == TCODE_ANONYMOUS_CHUNK && big_value > 4 &&
            sizeof___outbuffer > 0 && 0 != out___buffer) {
            // read compressed buffer from the archive
            sizeof__inbuffer =
                (size_t)(big_value -
                         4); // the last 4 bytes in this chunk are a 32 bit crc
    ...
    if (!bValidCompressedBuffer && 0 != out___buffer &&
    ...
    while (rc && counter > 0) {
        ...
        if (my_avail_in > 0 && m_zlib.strm.avail_in < max_avail) {
            if (0 == m_zlib.strm.avail_in || 0 == m_zlib.strm.next_in) {
                ...
                // The call to inflate() left some input in
                // m_zlib.strm.next_in[], but I can increase
                // m_zlib.strm.avail_in.
                d = max_avail - m_zlib.strm.avail_in;
                if (d > my_avail_in)
                    d = my_avail_in;
                m_zlib.strm.avail_in += (unsigned int)d;
...
ON_CompressedBuffer::ON_CompressedBuffer()
    : m_sizeof_uncompressed(0), m_sizeof_compressed(0), m_crc_uncompressed(0),
      m_crc_compressed(0), m_method(0), m_sizeof_element(0),
      m_buffer_compressed_capacity(0), m_buffer_compressed(0) {}
    ...
    if (this != &src) {
        ...
        if (src.m_buffer_compressed && src.m_sizeof_compressed > 0) {
            ...
            if (m_buffer_compressed) {
                ...
                memcpy(m_buffer_compressed, src.m_buffer_compressed,
                       m_sizeof_compressed);
            }
        }
    }
...
bool ON_CompressedBuffer::Write(ON_BinaryArchive &binary_archive) const {
    ...
    for (;;) {
        ...
        rc = binary_archive.WriteSize(
            (m_buffer_compressed && m_sizeof_compressed > 0)
                ? m_sizeof_compressed
                : 0);
        if (!rc)
            break;
        ...
        if (m_buffer_compressed && m_sizeof_compressed > 0) {
            rc = binary_archive.WriteByte(m_sizeof_compressed,
                                          m_buffer_compressed);
            if (!rc)
                break;
        }
...
bool ON_CompressedBuffer::Read(ON_BinaryArchive &binary_archive) {
    ...
    for (;;) {
        ...
        if (m_sizeof_compressed > 0) {
            ...
            if (m_buffer_compressed) {
                m_buffer_compressed_capacity = m_sizeof_compressed;
                rc = binary_archive.ReadByte(m_sizeof_compressed,
                                             m_buffer_compressed);
            } else {
...
bool ON_CompressedBuffer::Compress(
    ...
    if (bToggleByteOrder) {
        ON_BinaryFile::ToggleByteOrder(
            (int)(sizeof__inbuffer / m_sizeof_element), m_sizeof_element,
            inbuffer, (void *)inbuffer);
    }
    ...
    if (m_method) {
        if (!CompressionInit(&helper)) {
            ...
            CompressionEnd(&helper);
            if (sizeof_compressed > 0 &&
                sizeof_compressed == m_sizeof_compressed) {
                rc = true;
                if (2 * m_buffer_compressed_capacity >
                    3 * m_sizeof_compressed) {
                    // release memory we don't need
                    m_buffer_compressed_capacity = m_sizeof_compressed;
                    m_buffer_compressed = onrealloc(
                        m_buffer_compressed, m_buffer_compressed_capacity);
                }
            } else {
                Destroy();
                m_method = 0;
    ...
    if (rc) {
        ...
        m_crc_compressed =
            ON_CRC32(0, m_sizeof_compressed, m_buffer_compressed);
    }

    return rc;
}

bool ON_CompressedBuffer::Uncompress(void *outbuffer, int *bFailedCRC) const {
    bool rc = false;
    ...
    if (rc) {
        ON__UINT32 uncompressed_crc =
            ON_CRC32(0, m_sizeof_uncompressed, outbuffer);
...
bool ON_CompressedBuffer::WriteChar(size_t count, const void *buffer) {
    bool rc = true;
    if (count > 0 && buffer) {
        if (count + m_sizeof_compressed > m_buffer_compressed_capacity) {
            size_t delta =
                count + m_sizeof_compressed - m_buffer_compressed_capacity;
            if (delta < 2048)
                delta = 2048;
        ...
        memcpy(((char *)m_buffer_compressed) + m_sizeof_compressed, buffer,
               count);
...
bool ON_CompressedBuffer::InflateHelper(
    ...
    const size_t max_avail = 0x7FFFFFF0; // See max_avail comment in
...
bool ON_CompressedBuffer::CompressionInit(
    struct ON_CompressedBufferHelper *helper) const {
    bool rc = false;
