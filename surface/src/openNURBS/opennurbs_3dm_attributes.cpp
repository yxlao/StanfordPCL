...
bool ON_3dmObjectAttributes::operator==(
    const ON_3dmObjectAttributes &other) const {
...
/*
ON_3dmObjectAttributes& ON_3dmObjectAttributes::operator=(const
...
const ON_UUID ON_ObsoletePageSpaceObjectId = {
    ...
    0x2131,
    0x4fb8,
    {0xb9, 0xc6, 0x55, 0x24, 0x85, 0x9b, 0x98, 0xb8}};
...
bool ON_3dmObjectAttributes::ReadV5Helper(ON_BinaryArchive &file) {
    ...
    if (rc && 0 != itemid) {
        ON_ERROR(
            "Bug in ON_3dmObjectAttributes::ReadV5Helper or WriteV5Helper");
    }
...
ON_BOOL32 ON_3dmObjectAttributes::Read(ON_BinaryArchive &file) {
    ...
    if (rc && major_version == 1) {
        ...
        while (rc) {
            // OBSOLETE if (rc) rc = file.ReadLineStyle(m_line_style); // 23
            // March 2005 Dale Lear replaced with
            short s = 0;
            double x;
            rc = file.ReadShort(&s);
            if (!rc)
                break;
            if (file.Archive3dmVersion() < 4 ||
                ...
                // in V4.
                m_object_decoration =
                    ON::ObjectDecoration((s & ON::both_arrowhead));
            }
            rc = file.ReadShort(&s);
            if (!rc)
                break;
        ...
        if (rc)
            m_color_source =
                (unsigned char)ON::ObjectColorSource(m_color_source);

        if (rc)
            rc = file.ReadChar(&m_linetype_source);
        if (rc)
            m_linetype_source =
                (unsigned char)ON::ObjectLinetypeSource(m_linetype_source);

        if (rc)
            rc = file.ReadChar(&m_material_source);
        if (rc)
            m_material_source =
                (unsigned char)ON::ObjectMaterialSource(m_material_source);
            ...
            if (rc && minor_version >= 2) {
                ...
                if (rc && minor_version >= 3) {
                    ...
                    if (rc && minor_version >= 4) {
                        ...
                        if (rc)
                            m_plot_color_source =
                                (unsigned char)ON::PlotColorSource(
                                    m_plot_color_source);
                        if (rc)
                            rc = file.ReadColor(m_plot_color);
                        if (rc)
                        ...
                        if (rc)
                            m_plot_weight_source =
                                (unsigned char)ON::PlotWeightSource(
                                    m_plot_weight_source);
                        if (rc)
                            rc = file.ReadDouble(&m_plot_weight_mm);
                                ...
                                if (rc) {
                                    ...
                                    if (rc && count > 0) {
                                        ...
                                        for (i = 0; i < count && rc; i++) {
                                            ...
                                            if (rc) {
                                                // Assigning an object to a page
                                                // started out as using dmrs.
                                                // The way the runtime info is
                                                // saved has changed, but, at
                                                // this point, I can't change
                                                // the way the information is
                                                // saved in the file and it
                                                // doesn't matter.
                                                if (0 ==
                                                    ON_UuidCompare(
                                                        &ON_ObsoletePageSpaceObjectId,
                                                        &dmr.m_display_material_id)) {
                                                    m_viewport_id =
                                                        dmr.m_viewport_id;
                                                    m_dmref.Remove();
...
ON_BOOL32 ON_3dmObjectAttributes::Write(ON_BinaryArchive &file) const {
    ...
    if (rc) {
        // OBSOLETE if (rc) rc = file.WriteLineStyle(m_line_style); // 23 March
        // 2005 Dale Lear
        short s;
...
unsigned int ON_3dmObjectAttributes::SizeOf() const {
    unsigned int sz = sizeof(*this) - sizeof(ON_Object) +
                      ...
                      m_url.Length() * sizeof(wchar_t) + m_group.SizeOfArray() +
                      ON_Object::SizeOf();
    return sz;
}
...
bool ON_3dmObjectAttributes::IsVisible() const { return m_bVisible; }
...
unsigned int ON_3dmObjectAttributes::ApplyParentalControl(
    ...
    return ApplyParentalControl(parents_attributes, bogus_layer,
                                control_limits);
}

unsigned int ON_3dmObjectAttributes::ApplyParentalControl(
    const ON_3dmObjectAttributes &parents_attributes,
    const ON_Layer &parent_layer, unsigned int control_limits) {
    unsigned int rc = 0;
...
int ON_3dmObjectAttributes::GroupCount() const { return m_group.Count(); }
...
int ON_3dmObjectAttributes::GetGroupList(
    ON_SimpleArray<int> &group_list) const {
    group_list = m_group;
...
ON_BOOL32 ON_3dmObjectAttributes::IsInGroups(int group_count,
    ...
    if (group_count > 0 && group_list) {
        ...
        // in practice these arrays will be very short and this search will be
        // fast
        int i, j;
...
ON_BOOL32 ON_3dmObjectAttributes::IsInGroups(
    const ON_SimpleArray<int> &group_list) const {
...
void ON_3dmObjectAttributes::RemoveFromAllGroups() { m_group.Destroy(); }

bool ON_3dmObjectAttributes::FindDisplayMaterialId(
    const ON_UUID &viewport_id, ON_UUID *display_material_id) const {
    bool rc = false;
    if (m_dmref.Count() > 0) {
        ON_DisplayMaterialRef search_material, found_material;
        search_material.m_viewport_id = viewport_id;
        if (0 !=
            (rc = FindDisplayMaterialRef(search_material, &found_material))) {
            ...
            if (search_material.m_display_material_id != ON_nil_uuid) {
                while (i--) {
                    if ((m_dmref[i].m_display_material_id ==
                        ...
                        (m_dmref[i].m_viewport_id ==
                         search_material.m_viewport_id)) {
                        if (found_material)
        ...
        if (bCheckViewportId || bCheckMaterialId) {
            while (i--) {
                if (bCheckViewportId && m_dmref[i].m_viewport_id != viewport_id)
                    continue;
                if (bCheckMaterialId &&
                    m_dmref[i].m_display_material_id != display_material_id)
                    continue;
...
const ON_UUID ON_DisplayMaterialRef::m_invisible_in_detail_id = {
    ...
    0xe7ad,
    0x4a01,
    {0xa2, 0xaa, 0x41, 0xda, 0xe6, 0xbe, 0x7e, 0xcb}};
...
bool ON_DisplayMaterialRef::operator==(
    const ON_DisplayMaterialRef &other) const {
