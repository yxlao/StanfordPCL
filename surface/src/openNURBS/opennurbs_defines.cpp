...
static int on__hack__tolower(int c) {
    ...
    if (c <= 0) {
        ...
        case 197:    // UNICODE Latin capital letter A with ring above (A ring
                     // above)
            i = 229; // UNICODE Latin small letter A with ring above   (a ring
                     // above)
            break;
...
// Disable the MSC /W4 unreachable code warning for the call to
// on__hack__wcsicmp()
#pragma warning(push)
    ...
    if (0 != g_s__windows_locale_id) {
        if (0 == g_s__windows_locale_os) {
            ...
            int rc = ::CompareStringW(g_s__windows_locale_id,
                                      NORM_IGNORECASE | NORM_IGNOREWIDTH, s1,
                                      -1, s2, -1);
    ...
    if (!IsEmpty()) {
        ...
        if (0 != g_s__windows_locale_id) {
            if (0 == g_s__windows_locale_os) {
                ...
                int max_len_out =
                    2 * len_in + 16; // if 2x for wide char expansion
                ON_String out;
                out.ReserveArray(max_len_out + 1);
                out.SetLength(max_len_out + 1);

                // Wide char conversion
                int rc = ::LCMapStringA(g_s__windows_locale_id, LCMAP_UPPERCASE,
                                        in.Array(), len_in, out.Array(),
                                        max_len_out);
                ...
                // Wide char conversion
                int rc =
                    ::LCMapStringW(g_s__windows_locale_id, LCMAP_UPPERCASE,
                                   Array(), len_in, out.Array(), max_len_out);
                if (rc > 0 && rc <= max_len_out) {
                    out.SetLength(rc);
                    operator=(
                        out); // very fast - simply changes reference count
                    return;
                }
            }
        }
    ...
    if (!IsEmpty()) {
        ...
        if (0 != g_s__windows_locale_id) {
            if (0 == g_s__windows_locale_os) {
                ...
                int rc = ::LCMapStringA(g_s__windows_locale_id, LCMAP_LOWERCASE,
                                        in.Array(), len_in, out.Array(),
                                        max_len_out);
                if (rc > 0 && rc <= max_len_out) {
                    out.SetLength(rc);
                ...
                int rc =
                    ::LCMapStringW(g_s__windows_locale_id, LCMAP_LOWERCASE,
                                   Array(), len_in, out.Array(), max_len_out);
                if (rc > 0 && rc <= max_len_out) {
                    out.SetLength(rc);
                    operator=(
...
wchar_t *on_wcsrev(wchar_t *s) {
    ...
    for (i = 0, j--; i < j; i++, j--) {
        w = s[i];
        if (w >= 0xD800 && w <= 0xDBFF && s[i + 1] >= 0xDC00 &&
            s[i + 1] <= 0xDFFF) {
...
int on_WideCharToMultiByte(const wchar_t *lpWideCharStr, int cchWideChar,
    ...
    const wchar_t *p1 = 0;
    int count = ON_ConvertWideCharToUTF8(
        false, lpWideCharStr, cchWideChar, lpMultiByteStr, cchMultiByte,
        &error_status, error_mask, error_code_point, &p1);
    if (0 != error_status) {
        ON_ERROR("Error converting UTF-16 encoded wchar_t string to UTF-8 "
                 "encoded char string.");
    }
    return count;
}

int on_MultiByteToWideChar(const char *lpMultiByteStr, int cchMultiByte,
    ...
    ON__UINT32 error_code_point = 0xFFFD;
    const char *p1 = 0;
    int count = ON_ConvertUTF8ToWideChar(
        lpMultiByteStr, cchMultiByte, lpWideCharStr, cchWideChar, &error_status,
        error_mask, error_code_point, &p1);
    if (0 != error_status) {
        ON_ERROR("Error converting UTF-8 encoded char string to UTF-16 encoded "
                 "wchar_t string.");
    }
    return count;
}
...
const char *ON::Revision() { return OPENNURBS_SVN_REVISION; }
...
double ON::UnitScale(const class ON_UnitSystem &us_from,
                     ON::unit_system us_to) {
    double scale = 1.0;
    ON::unit_system us0 = us_from.m_unit_system;
    if (ON::custom_unit_system == us0) {
        if (us_from.m_custom_unit_scale > 0.0 &&
            ON_IsValid(us_from.m_custom_unit_scale)) {
...
double ON::UnitScale(const class ON_UnitSystem &u_and_t_from,
...
double ON::UnitScale(ON::unit_system u0, // from
    ...
    if (u0 != u1 && u1 != ON::custom_unit_system && ((int)u1) > 0 &&
        ((int)u1) < 26
...
ON::endian ON::Endian(int i) { // convert integer to endian enum
...
ON::curvature_style ON::CurvatureStyle(int i) {
    ...
    case max_curvature:
        ...
        // unsigned normal curvature with respect to sections cut perp to world
        // x axis
...
/*enum view_type // This is already in the header. I commented it out to see if
it would compile and it does. John Croudy.
{
  model_view_type     = 0,
