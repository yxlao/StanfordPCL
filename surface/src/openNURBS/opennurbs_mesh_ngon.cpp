...
bool ON_MeshNgonList::ReserveNgonCapacity(int capacity) {
    ...
    if (capacity > m_ngons_capacity) {
        m_ngons =
            (ON_MeshNgon *)onrealloc(m_ngons, capacity * sizeof(m_ngons[0]));
        if (0 == m_ngons) {
            m_ngons_capacity = 0;
            m_ngons_count = 0;
            rc = false;
...
int ON_MeshNgonList::NgonCount() const { return m_ngons_count; }

ON_MeshNgon *ON_MeshNgonList::Ngon(int Ngon_index) const {
    return (Ngon_index < 0 || Ngon_index >= m_ngons_count)
               ? 0
               : m_ngons + Ngon_index;
}
...
ON_MeshNgonUserData::ON_MeshNgonUserData() {
    m_userdata_uuid =
        ON_MeshNgonUserData::m_ON_MeshNgonUserData_class_id.Uuid();
...
ON_MeshNgonUserData::ON_MeshNgonUserData(const ON_MeshNgonUserData &src)
    : ON_UserData(src), m_mesh_F_count(src.m_mesh_F_count),
      m_mesh_V_count(src.m_mesh_V_count) {
    m_ngon_list =
        (0 != src.m_ngon_list) ? new ON_MeshNgonList(*src.m_ngon_list) : 0;
}
...
static bool ON_ValidateNgon(const ON_MeshNgon *ngon, int mesh_V_count,
                            int mesh_F_count) {
    unsigned int j;
    if (0 == ngon || ngon->N < 0)
        return false;
...
static bool ON_ValidateMeshNgonUserData(ON_MeshNgonUserData *ngud,
    ...
    if (0 == ngud->m_mesh_V_count && 0 == ngud->m_mesh_F_count) {
        ...
        // Set validation counts to -1 so we never do this slow validation
        // again.
        ...
        for (i = 0; i < ngon_count; i++) {
            if (!ON_ValidateNgon(ngud->m_ngon_list->Ngon(i), mesh_V_count,
                                 mesh_F_count))
                return false;
        }
    ...
    return (ngud->m_mesh_F_count == mesh_F_count &&
            ngud->m_mesh_V_count == mesh_V_count);
}
...
class ON_MeshNgonList *ON_Mesh::ModifyNgonList() {
    ...
    if (0 == ngud) {
        ...
        AttachUserData(ngud);
    } else if (0 != ngud->m_ngon_list &&
               !ON_ValidateMeshNgonUserData(ngud, *this)) {
