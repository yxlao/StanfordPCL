...
ON_BOOL32 ON_Torus::IsValid(ON_TextLog *text_log) const {
    ...
    if (minor_radius <= 0.0) {
        ...
        if (text_log)
            text_log->Print(
                "ON_Torus.major_radius = %g (should be > minor_radius=%g)\n",
...
ON_BOOL32 ON_Torus::Create(const ON_Plane &major_plane, double major__radius,
                           double minor__radius) {
    plane = major_plane;
...
ON_3dPoint ON_Torus::Center() const { return plane.origin; }

ON_3dVector ON_Torus::Axis() const { return plane.zaxis; }

double ON_Torus::MajorRadius() const { return major_radius; }

double ON_Torus::MinorRadius() const { return minor_radius; }
...
ON_BOOL32 ON_Torus::ClosestPointTo(ON_3dPoint test_point,
    ...
    ON_BOOL32 rc =
        major_circle.ClosestPointTo(test_point, &major_angle_radians);
...
ON_BOOL32 ON_Torus::Rotate(double sin_angle, double cos_angle,
                           const ON_3dVector &axis // axis of rotation
) {
    return Rotate(sin_angle, cos_angle, axis, plane.origin);
}

ON_BOOL32 ON_Torus::Rotate(double angle,           // angle in radians
                           const ON_3dVector &axis // axis of rotation
) {
