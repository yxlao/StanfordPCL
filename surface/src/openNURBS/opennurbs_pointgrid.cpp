...
ON_OBJECT_IMPLEMENT(ON_PointGrid, ON_Geometry,
...
ON_PointGrid::ON_PointGrid(const ON_PointGrid &src) { *this = src; }

ON_PointGrid::~ON_PointGrid() { Destroy(); }

int ON_PointGrid::Dimension() const { return 3; }
...
double *ON_PointGrid::PointArray() {
    return (m_point_count[0] > 0 && m_point_count[1] > 0) ? &m_point[0].x
                                                          : NULL;
}
...
ON_3dPoint *ON_PointGrid::operator[](int i) {
    return (0 <= i && i < m_point_count[0])
               ? m_point.Array() + i * m_point_stride0
               : 0;
}
    ...
    if (this != &src) {
        ...
        if (PointCount() > 0) {
            ...
            if (m_point_stride0 == src.m_point_stride0) {
                memcpy(m_point.Array(), src.m_point.Array(),
                       PointCount() * sizeof(ON_3dPoint));
            } else {
                int i, j;
...
void ON_PointGrid::Dump(ON_TextLog &dump) const {
    ...
    if (m_point.Count() < 1) {
        ...
        dump.PrintPointGrid(3, false, m_point_count[0], m_point_count[1],
                            3 * m_point_stride0, 3, &m_point[0].x, "  point");
    }
}

ON_BOOL32 ON_PointGrid::IsValid(ON_TextLog *text_log) const {
    ON_BOOL32 rc = false;
    if (ON_IsValidPointGrid(3, false, m_point_count[0], m_point_count[1],
...
ON_BOOL32 ON_PointGrid::GetBBox( // returns true if successful
    ...
    return ON_GetPointGridBoundingBox(3, 0, m_point_count[0], m_point_count[1],
                                      m_point_stride0 * 3, 3, &m_point[0].x,
                                      boxmin, boxmax, bGrowBox ? true : false);
}

bool ON_PointGrid::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
    ...
    int i;
    for (i = 0; i < m_point_count[0]; i++) {
        if (ON_GetPointListBoundingBox(3, 0, m_point_count[1], 3, &m_point[i].x,
                                       tight_bbox, bGrowBox, xform))
            bGrowBox = true;
    }
...
ON_BOOL32 ON_PointGrid::Transform(const ON_Xform &xform) {
    ...
    return ON_TransformPointGrid(3, false, m_point_count[0], m_point_count[1],
                                 m_point_stride0 * 3, 3, Point(0, 0), xform);
}

// virtual ON_Geometry::IsDeformable() override
bool ON_PointGrid::IsDeformable() const { return true; }

// virtual ON_Geometry::MakeDeformable() override
bool ON_PointGrid::MakeDeformable() { return true; }

ON_BOOL32 ON_PointGrid::SwapCoordinates(int i,
                                        int j // indices of coords to swap
) {
    return ON_SwapPointGridCoordinates(m_point_count[0], m_point_count[1],
                                       m_point_stride0 * 3, 3, Point(0, 0), i,
                                       j);
}

ON_BOOL32 ON_PointGrid::Write(ON_BinaryArchive & // open binary file
) const {
    // TODO
    return false;
}

ON_BOOL32 ON_PointGrid::Read(ON_BinaryArchive & // open binary file
) {
    // TODO
    return false;
}

ON::object_type ON_PointGrid::ObjectType() const { return ON::pointset_object; }

ON_BOOL32
ON_PointGrid::IsClosed(int dir) const {
    return ON_IsPointGridClosed(3, 0, m_point_count[0], m_point_count[1],
                                m_point_stride0 * 3, 3, &m_point[0].x, dir);
}

ON_BOOL32
ON_PointGrid::Reverse(int dir) {
    return ON_ReversePointGrid(3, false, m_point_count[0], m_point_count[1],
                               m_point_stride0 * 3, 3, Point(0, 0), dir);
}
