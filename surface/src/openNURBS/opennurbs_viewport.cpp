...
ON_OBJECT_IMPLEMENT(ON_Viewport, ON_Geometry,
...
static bool ON__IsCameraFrameUnitVectorHelper(const ON_3dVector &v) {
    ...
    return (v.x != ON_UNSET_VALUE && v.y != ON_UNSET_VALUE &&
            v.z != ON_UNSET_VALUE && fabs(v.Length() - 1.0) <= 1.0e-6);
}

static bool ON__IsCameraFramePerpindicular(const ON_3dVector &unit_vector0,
                                           const ON_3dVector &unit_vector1) {
    return (fabs(unit_vector0.x * unit_vector1.x +
                 unit_vector0.y * unit_vector1.y +
...
bool ON_GetViewportRotationAngles(
    ...
    const ON_3dVector &Y, const ON_3dVector &Z,
    ...
    // If si = sin(ai) and ci = cos(ai), then the relationship between the
    // camera frame and the angles is defined by the matrix equation C =
    // R3*R2*R1, where:
    //
...

    {
        ...
        dy = Y * Y;
        dz = Z * Z;
        if (fabs(dx - 1.0) <= eps && fabs(dy - 1.0) <= eps &&
            fabs(dz - 1.0) <= eps) {
            dx = X * Y;
            dy = Y * Z;
            dz = Z * X;
...
double ON_Viewport::PerspectiveMinNearDist() const { return m__MIN_NEAR_DIST; }
...
//
// ON_Viewport::DefaultMinNearOverFar = 0.001    // used in Rhino 3.0 beta
// testing until 11 Sep 2002 ON_Viewport::DefaultMinNearOverFar = 0.01     //
// used for Rhino 3.0 CD1 and CD2 ON_Viewport::DefaultMinNearOverFar = 0.000001
// // used for Rhino 3.0 CD3 ON_Viewport::DefaultMinNearOverFar = 0.0001   //
// used for Rhino 4.0 Fixes RR 8902
...
const ON_3dVector
    ON_Viewport::Default3dCameraDirection(-0.43301270189221932338186158537647,
                                          0.75, -0.5);
...
ON_BOOL32 ON_Viewport::Read(ON_BinaryArchive &file) {
    ...
    if (rc && major_version == 1) {
        ...
        if (m_bValidCamera) {
            if (!m_CamLoc.IsValid() || !m_CamUp.IsValid() ||
                !m_CamDir.IsValid()) {
                ON_ERROR("ON_Viewport.m_bValidCamera in file was true and it "
                         "should be false.");
        ...
        if (m_bValidFrustum) {
            if (!ON_IsValid(m_frus_left) || !ON_IsValid(m_frus_right) ||
                ...
                ON_ERROR("ON_Viewport.m_bValidFrustum in file was true and it "
                         "should be false.");
...
bool ON_Viewport::IsValidCamera() const { return (m_bValidCamera); }

bool ON_Viewport::IsValidFrustum() const { return (m_bValidFrustum); }
...
int ON_Viewport::Dimension() const { return 3; }
...
bool ON_Viewport::GetNearPlaneEquation(
    ON_PlaneEquation &near_plane_equation) const {
    bool rc = m_bValidCamera && m_bValidFrustum;
    if (rc) {
        near_plane_equation.ON_3dVector::operator=(m_CamZ);
        near_plane_equation.d = -near_plane_equation.ON_3dVector::operator*(
            m_CamLoc - m_frus_near * m_CamZ);
    }
    return rc;
}
...
bool ON_Viewport::GetFarPlaneEquation(
    ON_PlaneEquation &far_plane_equation) const {
    bool rc = m_bValidCamera && m_bValidFrustum;
    if (rc) {
        far_plane_equation.ON_3dVector::operator=(m_CamZ);
        far_plane_equation.d = -far_plane_equation.ON_3dVector::operator*(
            m_CamLoc - m_frus_far * m_CamZ);
    }
    return rc;
}

bool ON_Viewport::GetViewPlane(double view_plane_depth,
                               ON_Plane &view_plane) const {
    bool rc = IsValidFrustum() && IsValidCamera();
    if (rc) {
...
bool ON_Viewport::GetViewPlaneEquation(
    double view_plane_depth, ON_PlaneEquation &view_plane_equation) const {
    bool rc = m_bValidCamera && m_bValidFrustum;
    if (rc) {
...
bool ON_Viewport::GetNearRect(ON_3dPoint &left_bottom, ON_3dPoint &right_bottom,
                              ON_3dPoint &left_top,
...
bool ON_Viewport::GetFarRect(ON_3dPoint &left_bottom, ON_3dPoint &right_bottom,
                             ON_3dPoint &left_top,
                             ON_3dPoint &right_top) const {
    ON_Plane far_plane;
    ...
    if (rc) {
        ...
        y = 1.0 / y;
        left_bottom =
            far_plane.PointAt(s * x * m_frus_left, s * y * m_frus_bottom);
        right_bottom =
            far_plane.PointAt(s * x * m_frus_right, s * y * m_frus_bottom);
...
bool ON_Viewport::GetViewPlaneRect(double view_plane_depth,
    ...
    if (rc) {
        double s =
            IsPerspectiveProjection() ? view_plane_depth / m_frus_near : 1.0;
        double x = 1.0, y = 1.0;
        GetViewScale(&x, &y);
        x = 1.0 / x;
        y = 1.0 / y;
        left_bottom =
            view_plane.PointAt(s * x * m_frus_left, s * y * m_frus_bottom);
        right_bottom =
            view_plane.PointAt(s * x * m_frus_right, s * y * m_frus_bottom);
        left_top = view_plane.PointAt(s * x * m_frus_left, s * y * m_frus_top);
        right_top =
            view_plane.PointAt(s * x * m_frus_right, s * y * m_frus_top);
    }
    return rc;
}

ON_BOOL32 ON_Viewport::GetBBox(double *boxmin, double *boxmax,
    ...
    corners[8] = m_CamLoc;
    if (rc) {
        rc = ON_GetPointListBoundingBox(3, 0, 9, 3, &corners[0].x, boxmin,
...
ON_3dPoint ON_Viewport::CameraLocation() const { return m_CamLoc; }

ON_3dVector ON_Viewport::CameraDirection() const { return m_CamDir; }

ON_3dVector ON_Viewport::CameraUp() const { return m_CamUp; }

bool ON_Viewport::CameraLocationIsLocked() const { return m_bLockCamLoc; }

bool ON_Viewport::CameraDirectionIsLocked() const { return m_bLockCamDir; }

bool ON_Viewport::CameraUpIsLocked() const { return m_bLockCamUp; }
...
bool ON_Viewport::GetDollyCameraVector(
    ...
    if (rc) {
        ...
        if (dx == 0.0 || dy == 0.0 || dz == 0.0)
            ...
            ON_3dPoint c0((x0 - port_left) / dx - 1.0,
                          (y0 - port_bottom) / dy - 1.0, z);
            ON_3dPoint c1((x1 - port_left) / dx - 1.0,
                          (y1 - port_bottom) / dy - 1.0, z);
...
bool ON_Viewport::GetCameraFrame(double *CameraLocation, double *CameraX,
                                 double *CameraY, double *CameraZ) const {
    if (CameraLocation) {
...
ON_3dVector ON_Viewport::CameraX() const { return m_CamX; }

ON_3dVector ON_Viewport::CameraY() const { return m_CamY; }

ON_3dVector ON_Viewport::CameraZ() const { return m_CamZ; }

bool ON_Viewport::IsCameraFrameWorldPlan(int *xindex, int *yindex,
                                         int *zindex) {
    int i;
    int ix = 0;
    int iy = 0;
...
bool ON_Viewport::GetCameraExtents(
    ...
    if (wbox.GetCorners(corners)) {
        rc = GetCameraExtents(corners.Count(), 3, &corners.Array()[0].x, cbox,
                              bGrowBox);
    }
    return rc;
}
...
bool ON_Viewport::GetCameraExtents(
    // returns bounding box in camera coordinates - this is useful information
    // for setting view frustrums to include the point list
    ON_3dPoint &worldSphereCenter, double worldSphereRadius,
    ON_BoundingBox &cbox, // bounding box in camera coordinates
    int bGrowBox          // set to true to grow existing box
) const {
    bool rc = false;
...
bool ON_Viewport::ChangeToParallelProjection(bool bSymmetricFrustum) {
    ...
    ChangeToSymmetricFrustum(bSymmetricFrustum, bSymmetricFrustum,
                             ON_UNSET_VALUE);
    ...
    if (!ON_IsValid(target_distance) || !m_bValidFrustum ||
        !ON_IsValid(m_frus_near) || m_frus_near <= 0.0 ||
        target_distance <= m_frus_near) {
        target_distance =
            0.0; // makes it easier to check for valid target distance
    }
    ...
    if (rc) {
        if (ON::perspective_view == old_projection) {
            ...
            if (target_distance > 0.0 && 0.0 < m_frus_near &&
...
static bool ChangeFromParallelToPerspectiveHelper(ON_Viewport &vp,
    ...
    if (!vp.GetFrustum(&frus_left, &frus_right, &frus_bottom, &frus_top,
                       &frus_near, &frus_far))
        return false;
    ...
    if (frus_near < 1.0e-8 && frus_far >= 1.0e-7) {
        frus_near = 1.0e-8;
        vp.SetFrustum(frus_left, frus_right, frus_bottom, frus_top, frus_near,
                      frus_far);
        vp.GetFrustum(&frus_left, &frus_right, &frus_bottom, &frus_top,
                      &frus_near, &frus_far);
    }
...
bool ON_Viewport::ChangeToPerspectiveProjection(double target_distance,
    ...
    if (!ChangeFromParallelToPerspectiveHelper(*this, target_distance,
                                               lens_length))
        rc = false;
...
static bool GetTwoPointPerspectiveUpAndDirHelper(
    const ON_3dVector &up, const ON_3dVector &CamDir, const ON_3dVector &CamY,
    const ON_3dVector &CamZ, ON_3dVector &new_up, ON_3dVector &new_dir) {
    // get up direction
    ON_3dVector unit_up;
    ON_3dVector unit_dir;
    if (up.IsZero() && CamY.IsValid() && CamY.IsUnitVector()) {
        new_up = CamY;
        if (fabs(new_up.z) >= fabs(new_up.y) &&
            fabs(new_up.z) >= fabs(new_up.x))
            new_up.Set(0.0, 0.0, new_up.z < 0.0 ? -1.0 : 1.0);
        else if (fabs(new_up.y) >= fabs(new_up.z) &&
                 fabs(new_up.y) >= fabs(new_up.x))
    ...
    if (CamDir.IsValid() && !CamDir.IsTiny()) {
        ...
        unit_dir = new_dir;
        if (unit_dir.Unitize() &&
            ON__IsCameraFramePerpindicular(unit_up, unit_dir))
            return true;
...
bool ON_Viewport::ChangeToTwoPointPerspectiveProjection(double target_distance,
    ...
    if (rc) {
        ...
        if (!GetTwoPointPerspectiveUpAndDirHelper(up, m_CamDir, m_CamY, m_CamZ,
                                                  new_up, new_dir)) {
            rc = false;
        } else {
            ...
            if (center_point.IsValid() &&
                (new_loc - center_point) * m_CamZ > 0.0) {
                ON_Xform rot;
...
ON::view_projection ON_Viewport::Projection() const { return m_projection; }
...
bool ON_Viewport::SetFrustum(double frus_left, double frus_right,
                             double frus_bottom, double frus_top,
                             double frus_near, double frus_far) {
    bool rc = false;
    if (ON_IsValid(frus_left) && ON_IsValid(frus_right) &&
        ON_IsValid(frus_top) && ON_IsValid(frus_bottom) &&
        ON_IsValid(frus_near) && ON_IsValid(frus_far) &&
        frus_left < frus_right && frus_bottom < frus_top && 0.0 < frus_near &&
        frus_near < frus_far) {
        if (IsPerspectiveProjection() &&
            (frus_near <= 1.0e-8 || frus_far > 1.0001e8 * frus_near)) {
            ON_ERROR(
                "ON_Viewport::SetFrustum - Beyond float precision perspective "
...
bool ON_Viewport::GetFrustum(double *frus_left, double *frus_right,
                             double *frus_bottom, double *frus_top,
...
bool ON_Viewport::SetScreenPort(int port_left, int port_right, int port_bottom,
                                int port_top,
                                int port_near, // = 0
                                int port_far   // = 0
) {
...
bool ON_Viewport::GetScreenPort(int *port_left, int *port_right,
...
bool ON_Viewport::GetScreenPortAspect(double &aspect) const {
    ...
    aspect = (m_bValidPort && ON_IsValid(height) && ON_IsValid(width) &&
              height != 0.0)
                 ? fabs(width / height)
                 : 0.0;
...
bool ON_ViewportFromRhinoView(ON::view_projection projection,
                              ...
                              double rhvp_angle1, double rhvp_angle2,
...
*****************************************************************************/
{
    ...
    */
    ...
    // that are using this class to actually draw a picture, can make a
    // subsequent call to SetScreenPort().
    ...
    // Use this function to obtain standard view information from a Rhino
    // VIEWPORT view. The Rhino viewport has many entries.  As of 17 October,
    // 1997 all Rhino world to clipping transformation information is derived
    // from the VIEWPORT fields:
    //
    ...
    // A Rhino 1.0 VIEWPORT structure describes the camera's location,
    // direction, and  orientation by specifying a rotation transformation that
    // is applied to an initial frame.  The rotation transformation is defined
    ...
    vp.SetFrustum(frustum_left, frustum_right, frustum_bottom, frustum_top,
                  near_clipping_distance, far_clipping_distance);

    // Windows specific stuff that requires knowing size of client area in
    // pixels
...
bool ON_Viewport::GetCameraAngle(double *angle, double *angle_h,
                                 double *angle_w) const {
    bool rc = false;
    if (angle)
        *angle = 0.0;
    if (angle_h)
    ...
    if (GetFrustum(&left, &right, &bot, &top, &near_dist, NULL)) {
        ...
        if (near_dist > 0.0 && ON_IsValid(near_dist)) {
            if (angle)
                *angle =
                    atan(sqrt(half_w * half_w + half_h * half_h) / near_dist);
            if (angle_h)
...
bool ON_Viewport::GetCameraAngle(double *angle) const {
    double angle_h = 0.0;
    double angle_w = 0.0;
...
bool ON_Viewport::SetCameraAngle(double angle) {
    ...
    if (angle > 0.0 && angle < 0.5 * ON_PI * (1.0 - ON_SQRT_EPSILON)) {
        if (GetFrustum(NULL, NULL, NULL, NULL, &near_dist, &far_dist) &&
            ...
            rc = SetFrustum(-half_w, half_w, -half_h, half_h, near_dist,
                            far_dist);
        }
    }
    return rc;
...
bool ON_Viewport::GetCamera35mmLensLength(double *lens_length) const {
    ...
    *lens_length = 0.0;
    ...
    view_r = (half_w <= half_h || IsTwoPointPerspectiveProjection()) ? half_w
                                                                     : half_h;
    film_r = 12.0;
    if (view_r <= 0.0)
        return false;
...
bool ON_Viewport::SetCamera35mmLensLength(double lens_length) {
    ...
    frus_bottom *= s;
    frus_top *= s;
    return SetFrustum(frus_left, frus_right, frus_bottom, frus_top, frus_near,
                      frus_far);
}
...
bool ON_Viewport::GetXform(ON::coordinate_system srcCS,
    ...
    case ON::camera_cs:
        ...
        case ON::clip_cs:
            if (m_bValidFrustum) {
                ON_Xform cam2clip;
                cam2clip.CameraToClip(IsPerspectiveProjection(), m_frus_left,
                                      m_frus_right, m_frus_bottom, m_frus_top,
                                      m_frus_near, m_frus_far);
    ...
    case ON::clip_cs:
        ...
        case ON::camera_cs:
            if (m_bValidFrustum) {
                ON_Xform clip2cam;
                clip2cam.ClipToCamera(IsPerspectiveProjection(), m_frus_left,
                                      m_frus_right, m_frus_bottom, m_frus_top,
                                      m_frus_near, m_frus_far);
        ...
        case ON::screen_cs:
            if (m_bValidPort) {
                xform.ClipToScreen(m_port_left, m_port_right, m_port_bottom,
                                   m_port_top, m_port_near, m_port_far);
                rc = true;
            }
            break;
    ...
    case ON::screen_cs:
        ...
        case ON::clip_cs:
            if (m_bValidPort) {
                xform.ScreenToClip(m_port_left, m_port_right, m_port_bottom,
                                   m_port_top, m_port_near, m_port_far);
                rc = true;
            }
            break;
...
bool ON_Viewport::GetFrustumLine(double screenx, double screeny,
    ...
    if (rc) {
        ...
        c.x = s2c.m_xform[0][0] * screenx + s2c.m_xform[0][1] * screeny +
              s2c.m_xform[0][3];
        c.y = s2c.m_xform[1][0] * screenx + s2c.m_xform[1][1] * screeny +
              s2c.m_xform[1][3];
        c.z = 1.0;
        line.to = c2w * c; // line.to = near plane mouse point in world coords
        c.z = -1.0;
        line.from =
            c2w * c; // line.from = far plane mouse point in world coords
...
static double clipDist(const double *camLoc, const double *camZ,
                       const double *P) {
    return (camLoc[0] - P[0]) * camZ[0] + (camLoc[1] - P[1]) * camZ[1] +
           (camLoc[2] - P[2]) * camZ[2];
}

bool ON_Viewport::SetFrustumNearFar(const double *box_min,
    ...
    if (!ON_IsValid(box_min[0]) || !ON_IsValid(box_min[1]) ||
        !ON_IsValid(box_min[2]))
        return false;
    if (!ON_IsValid(box_max[0]) || !ON_IsValid(box_max[1]) ||
        !ON_IsValid(box_max[2]))
        return false;
    ...
    if (GetCameraFrame(camLoc, NULL, NULL, camZ)) {
        ...
        if (IsPerspectiveProjection())
            rc = SetFrustumNearFar(n, f, m__MIN_NEAR_DIST, m__MIN_NEAR_OVER_FAR,
                                   0.5 * (n + f));
        else
...
bool ON_Viewport::SetFrustumNearFar(const double *center, double radius) {
    bool rc = false;
    double n, f, d;
    double camLoc[3], camZ[3], P[3];
...
bool ON_Viewport::SetFrustumNearFar(double n, double f) {
    ...
    if (ON_IsValid(n) && ON_IsValid(f) && n > 0.0 && f > n) {
        if (GetFrustum(&frus_left, &frus_right, &frus_bottom, &frus_top,
            ...
            frus_near = n;
            frus_far = f;
            rc = SetFrustum(frus_left, frus_right, frus_bottom, frus_top,
                            frus_near, frus_far);
        } else {
            if (IsPerspectiveProjection() &&
                (n <= 1.0e-8 || f > 1.0001e8 * n)) {
                ON_ERROR("ON_Viewport::SetFrustum - bogus perspective "
                         "m_frus_near/far values - will crash MS OpenGL");
            }
            m_frus_near = n;
            m_frus_far = f;
            rc = true;
...
bool ON_Viewport::GetWorldToScreenScale(const ON_3dPoint &P,
                                        double *scale) const {
    if (scale) {
        ON_Xform w2s;
        ON_3dVector X;
...
bool ON_Viewport::GetCoordinateSprite(int size, int scrx, int scry,
...
static ON_BOOL32 GetRelativeScreenCoordinates(int port_left, int port_right,
                                              int port_bottom, int port_top,
                                              ON_BOOL32 bSortPoints, int &x0,
...
bool ON_Viewport::ZoomToScreenRect(int x0, int y0, int x1, int y1) {
    ...
    if (!GetScreenPort(&port_left, &port_right, &port_bottom, &port_top,
                       &port_near, &port_far))
        return false;
    ...
    if (!GetFrustum(&frus_left, &frus_right, &frus_bottom, &frus_top,
        ...
        return false;
    double s0, t0, s1, t1;
    if (!GetRelativeScreenCoordinates(port_left, port_right, port_bottom,
                                      port_top, true, x0, y0, x1, y1, s0, t0,
                                      s1, t1))
        return false;
                                        ...
                                        int x0, int y0, int x1, int y1 )
{
  ...
  int port_left, port_right, port_bottom, port_top;
  if ( !GetScreenPort( &port_left, &port_right, &port_bottom, &port_top, NULL,
NULL ) ) return false; int dx = (x0+x1)/2; int dy = (y0+y1)/2; int cx =
(port_left+port_right)/2; int cy = (port_bottom+port_top)/2; if (
!DollyAlongScreenChord( dx, dy, cx, cy ) ) return false; dx = cx - dx; dy = cy -
dy; x0 += dx; x1 += dx; y0 += dy; y1 += dy;
  ...
  double s0, t0, s1, t1;
  if ( !GetRelativeScreenCoordinates(port_left, port_right, port_bottom,
port_top, true, x0,y0,x1,y1, s0,t0,s1,t1) ) return false;
...
bool ON_Viewport::Extents(double angle, const ON_3dPoint &center,
                          double radius) {
    if (!IsValid())
        return false;
...
bool ON_Viewport::GetPointDepth(ON_3dPoint point, double *near_dist,
                                double *far_dist, bool bGrowNearFar) const {
    bool rc = false;
    if (point.x != ON_UNSET_VALUE) {
        double depth = (m_CamLoc - point) * m_CamZ;
        if (0 != near_dist && (*near_dist == ON_UNSET_VALUE || !bGrowNearFar ||
                               *near_dist > depth))
            *near_dist = depth;
        if (0 != far_dist &&
...
bool ON_Viewport::GetBoundingBoxDepth(ON_BoundingBox bbox, double *near_dist,
                                      double *far_dist,
    ...
    for (;;) {
        ...
        if (Pcount < 8) {
                ...
                if (Linout[i] == (k & Linout[i]) &&
                    ...
                    if (bPerspectiveProjection) {
                        ...
                        if (t[0] < 0.0) {
                            if (0 == j) {
                                P[Pcount++] = m_CamLoc;
                                j = 1; // j = 1 indicates m_CamLoc has been
                                       // added to P[].
                            }
                            t[0] = t[1];
                        }
            ...
            // C[e[*][0]] and C[E[*][1]]
            const unsigned int e[12][2] = {{0, 1}, {2, 3}, {4, 5}, {6, 7},
                                           {0, 2}, {1, 3}, {4, 6}, {5, 7},
                                           {0, 4}, {1, 5}, {2, 6}, {3, 7}};
        ...
        if (0 != near_dist &&
            ...
            *near_dist = t[0];
        if (0 != far_dist &&
            (!bGrowNearFar || !ON_IsValid(*far_dist) || t[1] > *far_dist))
            *far_dist = t[1];
...
bool ON_Viewport::GetSphereDepth(ON_Sphere sphere, double *near_dist,
                                 double *far_dist, bool bGrowNearFar) const {
...
bool ON_Viewport::SetFrustumNearFar(double near_dist, double far_dist,
                                    double min_near_dist,
                                    double min_near_over_far,
                                    double target_dist) {
    if (!ON_IsValid(near_dist) || !ON_IsValid(far_dist) ||
        near_dist > far_dist) {
        return false;
    }
    ...
    if (IsPerspectiveProjection()) {
        ...
        // The 1.0001 fudge factor is to ensure successive calls to this
        // function give identical results.
            ...
            if (ON_IsValid(target_dist) && near_dist < target_dist &&
                ...
                if (target_dist / far_dist < min_near_over_far) {
                    if (near_dist / target_dist >= sqrt(min_near_over_far)) {
                        ...
                        // move far_dist to within striking distance of the
                        // target and let STEP 2 fine tune things.
                ...
                if (near_dist / target_dist < min_near_over_far) {
                    if (target_dist / far_dist <= sqrt(min_near_over_far) &&
                        ...
                        // move near_dist to within striking distance of the
                        // target and let STEP 2 fine tune things.
                ...
                if (b > 0.0) {
                    ...
                    // 19 Jan 2010, Mikko:
                    // Reordered the operations to guarantee n==near_dist and
                    // f==far_dist when s==1.0. The old system generated bogus
                    // problem reports when the dist difference was big.
                    ...
                    if (m < 0.95 || m > 1.05) {
                        ON_ERROR("ON_Viewport::SetFrustumNearFar arithmetic "
                                 "problem 2.");
                    }
#endif
                    ...
                    near_dist = min_near_over_far * far_dist;
                       ...
                       fabs(far_dist - target_dist) >
                           fabs(near_dist - target_dist)) {
                far_dist = near_dist / min_near_over_far;
            } else {
                near_dist = min_near_over_far * far_dist;
            }
...
bool ON_Viewport::GetFrustumLeftPlaneEquation(
    ON_PlaneEquation &left_plane_equation) const {
    bool rc = m_bValidCamera && m_bValidFrustum;
    if (rc) {
        ...
        if (IsPerspectiveProjection()) {
            ON_2dVector v(m_frus_near, m_frus_left);
            if (0 != (rc = v.Unitize())) {
                left_plane_equation.ON_3dVector::operator=(v.x *m_CamX +
                                                           v.y * m_CamZ);
                left_plane_equation.d =
                    -left_plane_equation.ON_3dVector::operator*(m_CamLoc);
            }
        } else {
            left_plane_equation.ON_3dVector::operator=(m_CamX);
            left_plane_equation.d = -left_plane_equation.ON_3dVector::operator*(
                m_CamLoc + m_frus_left * m_CamX);
        }
    }
    return rc;
}
...
bool ON_Viewport::GetFrustumRightPlaneEquation(
    ON_PlaneEquation &right_plane_equation) const {
    bool rc = m_bValidCamera && m_bValidFrustum;
    if (rc) {
        ...
        if (IsPerspectiveProjection()) {
            ON_2dVector v(m_frus_near, -m_frus_right);
            if (0 != (rc = v.Unitize())) {
                right_plane_equation.ON_3dVector::operator=(-v.x *m_CamX +
                                                            v.y * m_CamZ);
            ...
            right_plane_equation.d =
                -right_plane_equation.ON_3dVector::operator*(
...
bool ON_Viewport::GetFrustumBottomPlaneEquation(
    ...
    if (rc) {
        ...
        if (IsPerspectiveProjection()) {
            ON_2dVector v(m_frus_near, m_frus_bottom);
            if (0 != (rc = v.Unitize())) {
                bottom_plane_equation.ON_3dVector::operator=(v.x *m_CamY +
                                                             v.y * m_CamZ);
            ...
            bottom_plane_equation.d =
                -bottom_plane_equation.ON_3dVector::operator*(
...
bool ON_Viewport::GetFrustumTopPlaneEquation(
    ON_PlaneEquation &top_plane_equation) const {
    bool rc = m_bValidCamera && m_bValidFrustum;
    if (rc) {
        ...
        if (IsPerspectiveProjection()) {
            ON_2dVector v(m_frus_near, -m_frus_top);
            if (0 != (rc = v.Unitize())) {
                top_plane_equation.ON_3dVector::operator=(-v.x *m_CamY +
                                                          v.y * m_CamZ);
                top_plane_equation.d =
                    -top_plane_equation.ON_3dVector::operator*(m_CamLoc);
            }
        } else {
            top_plane_equation.ON_3dVector::operator=(-m_CamY);
            top_plane_equation.d = -top_plane_equation.ON_3dVector::operator*(
                m_CamLoc + m_frus_top * m_CamY);
        }
    }
    return rc;
}
...
void ON_Viewport::GetViewScale(double *x, double *y) const {
    ...
    if (!m_clip_mods.IsIdentity() && 0.0 == m_clip_mods.m_xform[3][0] &&
        ...
        //   See comments in SetViewScale about why we are ignoring the test for
        //   1 on either sx or sy
...
bool ON_Viewport::SetViewScale(double x, double y) {
    ...
    // 04 November 2011 S. Baer (RR93636)
    //   This function is used for printer calibration and it is commonly
    //   possible to need to apply a scale in both x and y.  The reason for the
    //   need of x or y to be one is because the view scale is encoded in the
    //   clip mod xform and it is hard to be sure that we could accurately
    //   extract these values when calling GetViewScale.  Removing the
    //   requirement to have one of the values == 1
    bool rc = false;
...
double ON_Viewport::ClipCoordDepthBias(double relative_depth_bias,
                                       double clip_z, double clip_w) const {
    double d;
    if (m_frus_far > m_frus_near && 0.0 != relative_depth_bias &&
        0.0 != clip_w) {
        if (ON::perspective_view == m_projection) {
            // To get the formula for the code in this claus:
            //
            // Set M = [Camera2Clip]*[translation by
            // (0,0,relative_depth_bias*(f-n)]*[Clip2Camera] Note that M maps
            // clipping coordinates to clipping coordinates.
            //
...
bool ON_Viewport::GetClipCoordDepthBiasXform(double relative_depth_bias,
    ...
    while (0.0 != relative_depth_bias && m_frus_far > m_frus_near) {
        if (ON::perspective_view == m_projection) {
            ...
            if (!cam2clip.CameraToClip(true, m_frus_left, m_frus_right,
                                       m_frus_bottom, m_frus_top, m_frus_near,
                                       m_frus_far))
                break;
            if (!clip2cam.ClipToCamera(true, m_frus_left, m_frus_right,
                                       m_frus_bottom, m_frus_top, m_frus_near,
                                       m_frus_far))
                break;
            cam_delta.m_xform[2][3] =
                relative_depth_bias * (m_frus_far - m_frus_near);
...
ON_Xform ON_Viewport::ClipModXform() const { return m_clip_mods; }
...
ON_3dPoint ON_Viewport::FrustumCenterPoint(double target_distance) const {
    ...
    if (m_bValidFrustum) {
        s = (ON::perspective_view == m_projection && m_frus_near > 0.0)
                ...
                : 0.5;
        dx = FrustumIsLeftRightSymmetric() ? 0.0
                                           : s * (m_frus_right + m_frus_left);
        dy = FrustumIsTopBottomSymmetric() ? 0.0
    ...
    target_point.x =
        (m_CamLoc.x + dx * m_CamX.x + dy * m_CamY.x + dz * m_CamZ.x);
    target_point.y =
        (m_CamLoc.y + dx * m_CamX.y + dy * m_CamY.y + dz * m_CamZ.y);
    target_point.z =
        (m_CamLoc.z + dx * m_CamX.z + dy * m_CamY.z + dz * m_CamZ.z);

    return target_point;
}

ON_3dPoint ON_Viewport::TargetPoint() const { return m_target_point; }
...
double ON_Viewport::TargetDistance(bool bUseFrustumCenterFallback) const {
    ...
    if (m_bValidCamera) {
        ...
        if (bUseFrustumCenterFallback && ON_UNSET_VALUE == d &&
...
ON_UUID ON_Viewport::ViewportId(void) const { return m_viewport_id; }
...
bool ON_IntersectViewFrustumPlane(const ON_Viewport &vp,
    ...
    for (i = 0; i < ppt_count; i++) {
        plane.ClosestPointTo(ppt_list[i].m_P, &ppt_list[i].m_Q.x,
                             &ppt_list[i].m_Q.y);
    ...
    for (i = 1; i < ppt_count; i++) {
        ...
        ppt_list[i].m_negcotangle =
            (0.0 >= ppt_list[i].m_Q.y) ? -ON_DBL_MAX
    ...
    for (i = 2; i < ppt_count; i++) {
        if ((ppt_list[i].m_Q.y - ppt_list[i0].m_Q.y) * D.x <=
            ...
            // ppt_list[i0], ppt_list[i1], ppt_list[i] is a "right" turn or
            // colinear. Drop ppt_list[i1].
            i1 = i;
        } else {
...
void ON_Viewport::GetPerspectiveClippingPlaneConstraints(
    ON_3dPoint camera_location, unsigned int depth_buffer_bit_depth,
    double *min_near_dist, double *min_near_over_far) {
    double nof, n, d;
