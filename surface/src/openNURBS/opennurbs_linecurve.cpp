...
ON_OBJECT_IMPLEMENT(ON_LineCurve, ON_Curve,
...
ON_LineCurve::ON_LineCurve(const ON_LineCurve &src) { *this = src; }
...
int ON_LineCurve::Dimension() const { return m_dim; }
...
ON_LineCurve::GetBBox( // returns true if successful
    ...
    return ON_GetPointListBoundingBox(m_dim, false, 2, 3, m_line.from, boxmin,
...
bool ON_LineCurve::IsDeformable() const { return true; }

bool ON_LineCurve::MakeDeformable() { return true; }
...
ON_BOOL32 ON_LineCurve::Write(ON_BinaryArchive &file // open binary file
) const {
...
ON_BOOL32 ON_LineCurve::Read(ON_BinaryArchive &file // open binary file
) {
    int major_version = 0;
    int minor_version = 0;
...
ON_Interval ON_LineCurve::Domain() const { return m_t; }
    ...
    if (rc && m_dim != desired_dimension) {
        if (desired_dimension == 2) {
            ...
            if (2 == m_dim) {
                ...
                if (ON_UNSET_VALUE != m_line.from.x &&
                    ON_UNSET_VALUE == m_line.from.z)
                    m_line.from.z = 0.0;
                if (ON_UNSET_VALUE != m_line.from.x &&
                    ON_UNSET_VALUE == m_line.to.z)
                    m_line.from.z = 0.0;
            }
            m_dim = 3;
        }
...
int ON_LineCurve::SpanCount() const { return 1; }
...
int ON_LineCurve::Degree() const { return 1; }
...
int ON_LineCurve::IsPolyline(ON_SimpleArray<ON_3dPoint> *pline_points,
...
int ON_LineCurve::GetNurbForm(ON_NurbsCurve &c, double tolerance,
...
ON_BOOL32 ON_LineCurve::Split(double t, ON_Curve *&left_side,
...

{
    ...
    if (m_t.Includes(t, true)) {
        ...
        if (left_side && !left_line) {
            ON_ERROR(
                "ON_LineCurve::Split - input left_side not an ON_LineCurve*");
            return false;
        }
        if (right_side && !right_line) {
            ON_ERROR(
                "ON_LineCurve::Split - input right_side not an ON_LineCurve*");
            return false;
        }
        if (!left_line) {
