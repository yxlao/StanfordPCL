...
ON_OBJECT_IMPLEMENT(ON_CurveProxy, ON_Curve,
...
ON_CurveProxy::ON_CurveProxy(const ON_Curve *c, ON_Interval domain)
    : m_real_curve(c), m_bReversed(0), m_real_curve_domain(domain),
...
ON__UINT32 ON_CurveProxy::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_bReversed), &m_bReversed);
    current_remainder = ON_CRC32(current_remainder, sizeof(m_real_curve_domain),
                                 &m_real_curve_domain);
    current_remainder =
...
bool ON_CurveProxy::ProxyCurveIsReversed() const { return m_bReversed; }
...
const ON_Curve *ON_CurveProxy::ProxyCurve() const { return m_real_curve; }
...
void ON_CurveProxy::Dump(ON_TextLog &dump) const {
...
ON_CurveProxy::GetBBox( // returns true if successful
    ...
    return (m_real_curve) ? m_real_curve->GetBBox(boxmin, boxmax, bGrowBox)
                          : false;
}
...
ON_Interval ON_CurveProxy::Domain() const { return m_this_domain; }
...
// Do not change SpanCount() without making sure it gives the correct result for
// use in GetSpanVector()
...
// Do not change GetSpanVector() without making sure it is consistent with
// SpanCount()
ON_BOOL32 ON_CurveProxy::GetSpanVector(double *d) const {
  ...
  return rc;
    ...
    if (m_real_curve_domain[1] <= domain[0] ||
        m_real_curve_domain[0] >= domain[1]) {
        onfree((void *)rsv);
        return false;
    }
...
ON_CurveProxy::GetParameterTolerance(double t,       // t = parameter in domain
    ...
    ON_BOOL32 rc = (m_real_curve) ? m_real_curve->GetParameterTolerance(
                                        RealCurveParameter(t), tminus, tplus)
                                  : false;
    if (rc) {
        if (tminus)
...
int ON_CurveProxy::IsPolyline(ON_SimpleArray<ON_3dPoint> *pline_points,
    ...
    if (m_real_curve_domain == cdom) {
        ...
        if (rc) {
            ...
            if (pline_t &&
                (m_bReversed || m_real_curve_domain != m_this_domain)) {
                int i;
                for (i = 0; i < rc; i++) {
...
bool ON_CurveProxy::GetNextDiscontinuity(ON::continuity c, double t0, double t1,
    ...
    if (0 != m_real_curve) {
        ...
        if (realcrv_rc) {
            ...
            if (!(t0 < thiscrv_t && thiscrv_t < t1) &&
                !(t1 < thiscrv_t && thiscrv_t < t0)) {
                realcrv_rc = false;
                realcrv_dtype = 0;
                ...
                if ((s0 < s2 && s2 < s1) || (s1 < s2 && s2 < s0)) {
                    realcrv_rc = m_real_curve->GetNextDiscontinuity(
                        parametric_c, s2, s1, &s, hint, &realcrv_dtype,
    ...
    if (m_real_curve) {
        if (m_real_curve_domain != m_real_curve->Domain()) {
            ...
            case ON::G2_locus_continuous:
                if (t >= Domain()[1]) {
                    // Since the proxy curve is using a subset of the real
                    // curve, the proxy can't be closed.  So if the query
                    // parameter is >= domain max, the curve cannot be locus
                    // continuous.
                    rc = false;
                } else {
        ...
        if (rc)
            rc = m_real_curve->IsContinuous(
                desired_continuity, RealCurveParameter(t), hint,
...
ON_BOOL32 ON_CurveProxy::Trim(const ON_Interval &domain) {
    bool rc = false;
...
ON_BOOL32 ON_CurveProxy::Split(double t, ON_Curve *&left_side,
...
int ON_CurveProxy::
    GetNurbForm( // returns 0: unable to create NURBS representation
        ...
        double tolerance, // (>=0)
        const ON_Interval
            *sub_domain // OPTIONAL subdomain of ON::ProxyCurve::Domain()
    ) const {
    ON_BOOL32 rc = false;
    if (m_real_curve) {
...
ON_CurveProxy::GetCurveParameterFromNurbFormParameter(double nurbs_t,
    ...
    if (m_real_curve) {
        ...
        rc = real_crv->GetCurveParameterFromNurbFormParameter(
            RealCurveParameter(nurbs_t), curve_t);
        if (rc)
...
ON_CurveProxy::GetNurbFormParameterFromCurveParameter(double curve_t,
    ...
    if (m_real_curve) {
        ...
        rc = real_crv->GetNurbFormParameterFromCurveParameter(
            RealCurveParameter(curve_t), nurbs_t);
        if (rc)
