...
bool ON_UnitSystem::Read(ON_BinaryArchive &file) {
    ...
    if (!file.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &major_version,
                                &minor_version))
        return false;
...
void ON_UnitSystem::Dump(ON_TextLog &dump) const {
    ...
    case ON::custom_unit_system:
        if (m_custom_unit_name.Length() > 0)
            sUnitSystem.Format(L"%s (= %g meters)", m_custom_unit_name.Array(),
                               m_custom_unit_scale);
        else
            sUnitSystem.Format("user defined unit (= %g meters)",
                               m_custom_unit_scale);
        break;
    default:
...
void ON_3dmUnitsAndTolerances::Default() {
    ...
    m_absolute_tolerance =
        0.001; // = 0.01;       // Dale Lear: Changed March 2006
    m_angle_tolerance =
        ON_PI / 180.0; // = ON_PI/60.0; // Dale Lear: Changed 5 April 2006
...
ON_3dmUnitsAndTolerances::ON_3dmUnitsAndTolerances()
    : m_absolute_tolerance(0.0), m_angle_tolerance(0.0),
      m_relative_tolerance(0.0), m_distance_display_mode(ON::decimal),
      m_distance_display_precision(3) {
    Default();
}
...
ON_3dmUnitsAndTolerances::ON_3dmUnitsAndTolerances(
    const ON_3dmUnitsAndTolerances &src)
    : m_absolute_tolerance(0.0), m_angle_tolerance(0.0),
      m_relative_tolerance(0.0), m_distance_display_mode(ON::decimal),
      m_distance_display_precision(3) {
    Default();
...
bool ON_3dmUnitsAndTolerances::Write(ON_BinaryArchive &file) const {
    ...
    if (i < 0 || i > 20) {
        ON_ERROR("ON_3dmUnitsAndTolerances::Write() - "
                 "m_distance_display_precision out of range.");
        i = 3;
    }
    if (rc)
        rc = file.WriteInt(i);
...
bool ON_3dmUnitsAndTolerances::Read(ON_BinaryArchive &file) {
    ...
    if (rc && version >= 100 && version < 200) {
        ...
        if (version >= 101) {
            ...
            if (m_distance_display_precision < 0 ||
                m_distance_display_precision > 20)
...
ON_3dmAnnotationSettings::ON_3dmAnnotationSettings(
    const ON_3dmAnnotationSettings &src) {
    Default();
    *this = src;
}
...
void ON_3dmAnnotationSettings::SetWorldViewTextScale(
...
void ON_3dmAnnotationSettings::SetWorldViewHatchScale(
...
bool ON_3dmAnnotationSettings::Write(ON_BinaryArchive &file) const {
    ...
    // March 22, 2010 - Global DimScale abandoned and moved into DimStyles, so
    // now in older files, the dimscale values are multiplied into the DimStyle
    // lengths and DimScale is written as 1.0
    if (rc)
...
/*
ON_3dmConstructionPlane::ON_3dmConstructionPlane(const ON_3dmConstructionPlane&
src)
{
  Default();
  *this = src;
...
bool ON_3dmViewPosition::Read(ON_BinaryArchive &file) {
    ...
    // if people put bogus values in a file, tune them up to something that will
    // work
...
bool ON_3dmPageSettings::IsValid(ON_TextLog *text_log) const {
    ...
    if (m_width_mm != 0.0 || m_height_mm != 0.0) {
        if (!ON_IsValid(m_width_mm) || m_width_mm <= 0.0) {
            if (text_log) {
                text_log->Print("ON_3dmPageSettings has m_width_mm = %g "
                                "(should be > 0.0).\n",
                                m_width_mm);
            }
            rc = false;
        ...
        if (!ON_IsValid(m_height_mm) || m_height_mm <= 0.0) {
            if (text_log) {
                text_log->Print("ON_3dmPageSettings has m_height_mm = %g "
                                "(should be > 0.0).\n",
                                m_height_mm);
            }
            rc = false;
        ...
        if (!ON_IsValid(m_top_margin_mm) || m_top_margin_mm < 0.0) {
            if (text_log) {
                text_log->Print("ON_3dmPageSettings has m_top_margin_mm = %g "
                                "(should be >= 0.0).\n",
                                m_top_margin_mm);
            }
            rc = false;
        ...
        if (!ON_IsValid(m_bottom_margin_mm) || m_bottom_margin_mm < 0.0) {
            if (text_log) {
                text_log->Print("ON_3dmPageSettings has m_bottom_margin_mm = "
                                "%g (should be >= 0.0).\n",
                                m_bottom_margin_mm);
            }
            rc = false;
        ...
        if (!ON_IsValid(m_left_margin_mm) || m_left_margin_mm < 0.0) {
            if (text_log) {
                text_log->Print("ON_3dmPageSettings has m_left_margin_mm = %g "
                                "(should be >= 0.0).\n",
                                m_left_margin_mm);
            }
            rc = false;
        ...
        if (!ON_IsValid(m_right_margin_mm) || m_right_margin_mm < 0.0) {
            if (text_log) {
                text_log->Print("ON_3dmPageSettings has m_right_margin_mm = %g "
                                "(should be >= 0.0).\n",
                                m_right_margin_mm);
            }
            rc = false;
        ...
        if (m_left_margin_mm + m_right_margin_mm >= m_width_mm) {
            if (text_log) {
                text_log->Print(
                    "ON_3dmPageSettings has m_left_margin_mm+m_right_margin_mm "
                    "= %g > %g = m_width_mm.\n",
        ...
        if (m_top_margin_mm + m_bottom_margin_mm >= m_height_mm) {
            if (text_log) {
                text_log->Print(
                    "ON_3dmPageSettings has m_top_margin_mm+m_bottom_margin_mm "
                    "= %g > %g = m_height_mm.\n",
...
void ON_3dmView::Dump(ON_TextLog &dump) const {
    ...
    ON_BOOL32 bValidFrustum =
        m_vp.GetFrustum(&frus_left, &frus_right, &frus_bottom, &frus_top,
                        &frus_near, &frus_far);
    int port_left, port_right, port_bottom, port_top, port_near, port_far;
    ON_BOOL32 bValidPort =
        m_vp.GetScreenPort(&port_left, &port_right, &port_bottom, &port_top,
                           &port_near, &port_far);
    ...
    if (bValidCamera) {
        dump.Print("viewport camera frame\n"
                   ...
                   "  Y = %g, %g, %g\n"
                   "  Z = %g, %g, %g\n",
                   camLoc.x, camLoc.y, camLoc.z, camX.x, camX.y, camX.z, camY.x,
                   camY.y, camY.z, camZ.x, camZ.y, camZ.z);
        ON_3dPoint target_point = TargetPoint();
        double target_distance = target_point.DistanceTo(camLoc);
        dump.Print("camera target\n"
                   "  distance = %g\n"
                   "  point = %g,%g,%g\n",
                   target_distance, target_point.x, target_point.y,
                   target_point.z);
    }
    ...
    if (bValidFrustum) {
        dump.Print("view frustum\n"
                   ...
                   "  bottom = %g, top   = %g\n"
                   "  near   = %g, far   = %g\n",
                   frus_left, frus_right, frus_bottom, frus_top, frus_near,
                   frus_far);
    }
    ...
    if (bValidPort) {
        ...
        dump.Print("viewport window screen location\n"
                   ...
                   "  bottom = %4d, top   = %4d\n"
                   "  near   = %4d, far   = %4d\n",
                   port_left, port_right, port_bottom, port_top, port_near,
                   port_far);
    }
    ...
    dump.Print("relative viewport window position in application frame window\n"
               ...
               100.0 * rel_left, 100.0 * rel_right, 100.0 * rel_bottom,
               100.0 * rel_top);
...
void ON_3dmView::Default() {
    ...
    m_vp.Initialize();
    // ON_3dmView::m_target is obsolete - keep it in sync with
    // m_vp.m_target_point
    ...
    if (m_vp.Projection() == ON::parallel_view) {
        m_cplane.m_plane.CreateFromFrame(m_cplane.m_plane.origin,
                                         m_vp.CameraX(), m_vp.CameraY());
    }
...
bool ON_3dmView::SetTargetPoint(ON_3dPoint target_point) {
    ...
    OBSOLETE_3DM_VIEW_TARGET =
        m_vp.TargetPoint(); // keep obsolete m_target in sync with
                            // m_vp.m_target_point
    return rc;
}
...
bool ON_3dmView::IsValid(ON_TextLog *text_log) const {
    ...
    while (rc) {
        ...
        case ON::model_view_type:
            if (m_page_settings.m_width_mm != 0.0 ||
                m_page_settings.m_height_mm != 0.0) {
                if (text_log) {
                    text_log->Print(
                        "ON_3dmView has m_view_type = ON::model_view_type but "
                        "m_page_settings width,height = (%g,%g) (both should "
                        "be zero).\n",
            ...
            // if (    m_nested_view_position.m_min.x != 0.0 ||
            // m_nested_view_position.m_max.x != 0.0
            //     || m_nested_view_position.m_min.y != 0.0 ||
            //     m_nested_view_position.m_max.y != 0.0
            //     || m_nested_view_position.m_min.z != 0.0 ||
            //     m_nested_view_position.m_max.z != 0.0 )
            //{
            //  if ( text_log )
            //  {
            //    text_log->Print("ON_3dmView has m_view_type =
            //    ON::model_view_type and m_nested_view_position is not
            //    identically zero.\n");
            //  }
            //  rc = false;
            //}
            ...
            //  {
            //    text_log->Print("ON_3dmView has m_view_type =
            //    ON::model_view_type and m_parent_viewport_id is not nil\n");
            //  }
            //  rc = false;
            //}
            break;
        case ON::page_view_type:
            ...
            //  {
            //    text_log->Print("ON_3dmView has m_view_type =
            //    ON::page_view_type and m_nested_view_position is not
            //    identically zero.\n");
            //  }
            //  rc = false;
            //}
            ...
            //  {
            //    text_log->Print("ON_3dmView has m_view_type =
            //    ON::page_view_type and m_parent_viewport_id is not nil\n");
            //  }
            //  rc = false;
            //}
            if (m_page_settings.m_width_mm <= 0.0 ||
                ...
                if (text_log) {
                    text_log->Print(
                        "ON_3dmView has m_view_type = ON::page_view_type but "
                        "page width,height = (%g,%g)\n",
                        m_page_settings.m_width_mm,
                        m_page_settings.m_height_mm);
                }
                rc = false;
        ...
        case ON::nested_view_type:
            if (m_page_settings.m_width_mm != 0.0 ||
                ...
                if (text_log) {
                    text_log->Print(
                        "ON_3dmView has m_view_type = ON::nested_view_type but "
                        "m_page_settings width,height = (%g,%g) (both should "
                        "be zero).\n",
                        m_page_settings.m_width_mm,
                        m_page_settings.m_height_mm);
            ...
            //  {
            //    text_log->Print("ON_3dmView has m_view_type =
            //    ON::nested_view_type and m_parent_viewport_id is nil.\n");
            //  }
            //  rc = false;
            //}

            // if ( !m_nested_view_position.IsValid()
            //     || m_nested_view_position.m_min.x >=
            //     m_nested_view_position.m_max.x
            //     || m_nested_view_position.m_min.y >=
            //     m_nested_view_position.m_max.y
            //     || m_nested_view_position.m_min.z !=
            //     m_nested_view_position.m_max.z )
            //{
            //  if ( text_log )
            //  {
            //    text_log->Print("ON_3dmView has m_view_type =
        ...
        default:
            if (text_log) {
                text_log->Print(
                    "ON_3dmView m_view_type = %d (illegal enum value)\n",
                    m_view_type);
            }
            rc = false;
            break;
...
bool ON_3dmView::Write(ON_BinaryArchive &file) const {
    ...
    if (rc) {
        rc = file.BeginWrite3dmChunk(TCODE_VIEW_SHOWCONGRID,
    ...
    if (rc) {
        rc = file.BeginWrite3dmChunk(TCODE_VIEW_SHOWCONAXES,
    ...
    if (rc) {
        rc =
            file.BeginWrite3dmChunk(TCODE_VIEW_SHOWWORLDAXES, m_bShowWorldAxes);
        if (rc) {
            if (!file.EndWrite3dmChunk())
                rc = false;
        }
    ...
    if (rc && file.Archive3dmVersion() >= 4) {
        ...
        if (rc) {
            ...
            while (rc) {
                ...
                memset(&obsolete_parent_viewport_id, 0,
                       sizeof(obsolete_parent_viewport_id));
...
bool ON_3dmView::Read(ON_BinaryArchive &file) {
    ...
    if (bHaveViewport && bHaveTargetPoint && target_point.IsValid() &&
        ...
        SetTargetPoint(
            target_point); // sets both this->m_target and m_vp.m_target_point
    } else {
...
int ON_EarthAnchorPoint::CompareEarthLocation(const ON_EarthAnchorPoint *a,
    ...
    int i = a->m_earth_basepoint_elevation_zero -
            b->m_earth_basepoint_elevation_zero;
    if (i != 0)
        return i;
...
int ON_EarthAnchorPoint::CompareModelDirection(const ON_EarthAnchorPoint *a,
    ...
    int i = ON_ComparePoint(3, false, &a->m_model_basepoint.x,
                            &b->m_model_basepoint.x);
    if (!i) {
        i = ON_ComparePoint(3, false, &a->m_model_north.x, &b->m_model_north.x);
        if (!i) {
            i = ON_ComparePoint(3, false, &a->m_model_east.x,
                                &b->m_model_east.x);
        }
    }
    return i;
...
bool ON_EarthAnchorPoint::GetModelToEarthXform(
    const ON_UnitSystem &model_unit_system, ON_Xform &model_to_earth) const {
    ...
    const double earth_polar_radius =
        6356750.0; // Earth's radius at poles (meters)
    ...
    const double model_to_meters_scale =
        ON::UnitScale(model_unit_system, ON::meters);
    const double north_south_scale =
        model_to_meters_scale / meters_per_degree_latitude;
...
static bool ON_3dmSettings_Read_v1_TCODE_NAME(ON_BinaryArchive &file,
                                              ON_wString &str) {
...
static bool ON_3dmSettings_Read_v1_TCODE_VIEW(ON_BinaryArchive &file,
                                              ON_3dmView &view) {
    ...
    while (rc) {
        ...
        ON_ViewportFromRhinoView(
            ...
            target_point, angle1, angle2, angle3, viewsize, cameradist,
            100, // screen_width,
            100, // screen_height,
            view.m_vp);
...
static bool ON_3dmSettings_Read_v1_TCODE_NAMED_VIEW(ON_BinaryArchive &file,
                                                    ON_3dmView &view) {
...
static bool ON_3dmSettings_Read_v1_TCODE_VIEWPORT(ON_BinaryArchive &file,
                                                  ON_3dmView &view) {
    ...
    for (chunk_count = 0; rc; chunk_count++) {
        ...
        case TCODE_VIEWPORT_TRACEINFO: {
            ...
            if (rc)
                rc = ON_3dmSettings_Read_v1_TCODE_NAME(
                    file, view.m_trace_image.m_bitmap_filename);
        } break;

        case TCODE_VIEWPORT_WALLPAPER:
            rc = ON_3dmSettings_Read_v1_TCODE_NAME(
                file, view.m_wallpaper_image.m_bitmap_filename);
            break;
...
bool ON_3dmSettings::Read_v1(ON_BinaryArchive &file) {
    ...
    for (chunk_count = 0; rc; chunk_count++) {
        ...
        case TCODE_UNIT_AND_TOLERANCES:
            bGotSomething = true;
            rc = ON_3dmSettings_Read_v1_TCODE_UNIT_AND_TOLERANCES(
                file, m_ModelUnitsAndTolerances);
            break;
        }
    ...
    while (rc) {
        ...
        case TCODE_SETTINGS_CURRENT_LAYER_INDEX:
            if (big_value < -1 || big_value > 0x7FFFFFFF) {
                ON_ERROR("ON_3dmSettings::Read_v2() - "
                         "TCODE_SETTINGS_CURRENT_LAYER_INDEX - invalid layer "
                         "index value");
            } else {
        ...
        case TCODE_SETTINGS_CURRENT_FONT_INDEX:
            if (big_value < -1 || big_value > 0x7FFFFFFF) {
                ON_ERROR("ON_3dmSettings::Read_v2() - "
                         "TCODE_SETTINGS_CURRENT_FONT_INDEX - invalid font "
                         "index value");
            } else {
        ...
        case TCODE_SETTINGS_CURRENT_DIMSTYLE_INDEX:
            if (big_value < -1 || big_value > 0x7FFFFFFF) {
                ON_ERROR("ON_3dmSettings::Read_v2() - "
                         "TCODE_SETTINGS_CURRENT_DIMSTYLE_INDEX - invalid "
                         "dimstyle index value");
            } else {
                // in archives with opennurbs version >= 200106100
        ...
        case TCODE_SETTINGS_CURRENT_WIRE_DENSITY:
            if (big_value < -2 || big_value > 0x7FFFFFFF) {
                ON_ERROR("ON_3dmSettings::Read_v2() - "
                         "TCODE_SETTINGS_CURRENT_WIRE_DENSITY - invalid "
                         "current_wire_density value");
            } else {
            ...
            for (;;) {
                ...
                if (1 == major_version) {
                    ...
                    if (minor_version >= 1) {
                        // Added 6 Feb 2006
                        int mjv = 1, mnv = 1;
                        rc = file.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK, &mjv,
                                                    &mnv);
                        if (rc) {
                                ...
                                if (minor_version >= 4) {
                                    ...
                                    if (rc && minor_version >= 5) {
                                        ...
                                        if (rc && minor_version >= 6) {
                                            // 7 June 2006
                                            m_CustomRenderMeshSettings.Read(
                                                file);
                                        }
                                    }
                                }
        ...
        default:
            // information added in future will be skipped by
...
static bool ON_3dmSettings_Write_v1_TCODE_UNIT_AND_TOLERANCES(
    ON_BinaryArchive &file,
    const ON_3dmUnitsAndTolerances &UnitsAndTolerances) {
    bool rc = true;
    int v = 1, us = 0;
    if (rc)
...
bool ON_3dmSettings::Write_v1(ON_BinaryArchive &file) const {
    ...
    if (rc) {
        rc = ON_3dmSettings_Write_v1_TCODE_UNIT_AND_TOLERANCES(
            file, m_ModelUnitsAndTolerances);
        if (!file.EndWrite3dmChunk())
            rc = false;
    }
...
bool ON_3dmSettings::Write_v2(ON_BinaryArchive &file) const {
    ...
    // TCODE_SETTINGS_PLUGINLIST - plugins that may have saved userdata in the
    // file
