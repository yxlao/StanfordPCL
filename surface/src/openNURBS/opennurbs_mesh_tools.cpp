...
bool ON_Mesh::SwapEdge_Helper(int topei, bool bTestOnly) {
    ...
    if (tope.m_topf_count != 2 || tope.m_topvi[0] == tope.m_topvi[1] ||
        tope.m_topvi[0] < 0 || tope.m_topvi[1] < 0 ||
        tope.m_topvi[0] >= topv_count || tope.m_topvi[1] >= topv_count) {
        return false;
    }
...
bool ON_Mesh::SwapEdge(int topei) { return SwapEdge_Helper(topei, false); }
...
bool ON_Mesh::CollapseEdge(int topei) {
    ...
    // create a ON__MESHEDGE for each face (usually one or two) that uses the
    // edge
    ...
    for (efi = 0; efi < tope.m_topf_count; efi++) {
        ...
        for (fvi = 0; fvi < 4; fvi++) {
            ...
            me.topvi1 = top.m_topv_map[me.vi1];
            if (me.vi0 != me.vi1) {
                if ((me.topvi0 == tope.m_topvi[0] &&
                     me.topvi1 == tope.m_topvi[1]) ||
                    (me.topvi0 == tope.m_topvi[1] &&
                     me.topvi1 == tope.m_topvi[0])) {
                    if (me.vi0 > me.vi1) {
                        int i = me.vi0;
                        me.vi0 = me.vi1;
                        me.vi1 = i;
    ...
    ON_qsort(me_list, me_list_count, sizeof(me_list[0]),
             (QSORTCMPFUNC)CompareMESHEDGE);
    ...
    if (topv0.m_v_count == 1 || topv1.m_v_count == 1) {
        ...
        if (bHasVertexNormals) {
            ...
            ON_3dVector N = N0 + N1;
            if (!N.Unitize()) {
                N = (topv0.m_v_count == 1) ? mesh.m_N[topv0.m_vi[0]]
                                           : mesh.m_N[topv1.m_vi[0]];
            }
            newN = N;
        }
        if (bHasTextureCoordinates) {
    ...
    for (topv_end = 0; topv_end < 2; topv_end++) {
        ...
        for (vei = 0; vei < topv.m_tope_count; vei++) {
            ...
            for (efi = 0; efi < e.m_topf_count; efi++) {
                ...
                for (fvi = 0; fvi < 4; fvi++) {
                    nvi.oldvi = f.vi[fvi];
                    ON__NEWVI *p = (ON__NEWVI *)bsearch(
                        &nvi, old2new_map, old2new_map_count,
                        sizeof(old2new_map[0]), (QSORTCMPFUNC)CompareNEWVI);
                ...
                if (bChangedFace) {
                    if (!f.IsValid(V_count)) {
                        ...
                        if (f.vi[0] == f.vi[1] || f.vi[1] == f.vi[2] ||
                            f.vi[2] == f.vi[0] || f.vi[2] != f.vi[3]) {
...
ON_Mesh *ON_ControlPolygonMesh(const ON_NurbsSurface &nurbs_surface,
                               bool bCleanMesh, ON_Mesh *input_mesh) {
    int u0 = 0;
    ...
    bool bIsClosed0 =
        bPeriodic0 ? true : (nurbs_surface.IsClosed(0) ? true : false);
    bool bPeriodic1 = nurbs_surface.IsPeriodic(1) ? true : false;
    bool bIsClosed1 =
        bPeriodic1 ? true : (nurbs_surface.IsClosed(1) ? true : false);

    if (bPeriodic0) {
        u1 -= (nurbs_surface.Degree(0) - 1);
        while (u1 < nurbs_surface.CVCount(0) && gu[u0] < d0[0] &&
               gu[u1 - 1] <= d0[1]) {
            u0++;
            u1++;
        }
    ...
    if (bPeriodic1) {
        ...
        while (v1 < nurbs_surface.CVCount(1) && gv[v0] < d1[0] &&
               gv[v1 - 1] <= d1[1]) {
            v0++;
            v1++;
        }
    ...
    if (bCleanMesh) {
        ...
        if (badfi.Count() > 0) {
            ...
            // Added crash protection. At this point mesh is NULL if it
            // contained all bad faces.
            if (mesh)
