...
static void ON_DebuggerBreakpoint() {
} // <- Good location for a debugger breakpoint.
...
int ON_GetErrorCount(void) { return ON_ERROR_COUNT; }

int ON_GetWarningCount(void) { return ON_WARNING_COUNT; }

int ON_GetMathErrorCount(void) { return ON_MATH_ERROR_COUNT; }

int ON_GetDebugBreak(void) { return ON_DEBUG_BREAK_OPTION ? true : false; }
...
void ON_MathError(const char *sModuleName, const char *sErrorType,
    ...
    ON_Error(__FILE__, __LINE__,
             "Math library or floating point ERROR # %d module=%s type=%s "
             "function=%s",
             ON_MATH_ERROR_COUNT,
...
static bool ON_PrintErrorHeader(int type, // 0 = warning, 1 = error, 2 = assert
...
#define ON_SPRINTF3(s, count, fname, ln)                                       \
    sprintf_s(sMessage, sz, s, count, fname, ln)
        ...
        if (0 == type) {
            if (ON_WARNING_COUNT < 50) {
                ...
                if (sFunctionName && sFunctionName[0])
                    ON_SPRINTF4(
                        "openNURBS WARNING # %d %s.%d %s(): ", ON_WARNING_COUNT,
                        sFileName, line_number, sFunctionName);
                else
                    ON_SPRINTF3("openNURBS WARNING # %d %s.%d ",
                                ON_WARNING_COUNT, sFileName, line_number);
                bPrintMessage = true;
            } else if (50 == ON_ERROR_COUNT) {
                ON_SPRINTF1("openNURBS WARNING # %d - Too many warnings.  No "
                            "more printed messages.",
                            ON_WARNING_COUNT);
                bPrintMessage = true;
            }
            ...
            if (ON_ERROR_COUNT < 50) {
                ...
                if (sFunctionName && sFunctionName[0])
                    ON_SPRINTF4(
                        "openNURBS ERROR # %d %s.%d %s(): ", ON_ERROR_COUNT,
                        sFileName, line_number, sFunctionName);
                else
                    ON_SPRINTF3("openNURBS ERROR # %d %s.%d ", ON_ERROR_COUNT,
                                sFileName, line_number);
                bPrintMessage = true;
            } else if (50 == ON_ERROR_COUNT) {
                ON_SPRINTF1("openNURBS ERROR # %d - Too many errors.  No more "
                            "printed messages.",
                            ON_ERROR_COUNT);
                bPrintMessage = true;
            }
...
void ON_ErrorEx(const char *sFileName, int line_number,
                const char *sFunctionName, const char *sFormat, ...) {
    ON_IncrementErrorCount();

    bool bPrintMessage =
        ON_PrintErrorHeader(1, sFileName, line_number, sFunctionName);
...
void ON_WarningEx(const char *sFileName, int line_number,
                  const char *sFunctionName, const char *sFormat, ...) {
    ON_IncrementWarningCount();

    bool bPrintMessage =
        ON_PrintErrorHeader(0, sFileName, line_number, sFunctionName);
...
void ON_Assert(int bCondition, const char *sFileName, int line_number,
               const char *sFormat, ...) {
    if (!bCondition) {
        ON_IncrementErrorCount();
...
void ON_AssertEx(int bCondition, const char *sFileName, int line_number,
                 const char *sFunctionName, const char *sFormat, ...) {
    if (!bCondition) {
        ON_IncrementErrorCount();

        bool bPrintMessage =
            ON_PrintErrorHeader(2, sFileName, line_number, sFunctionName);
