...
bool ON_Font::CreateFontFromFaceName(const wchar_t *facename, bool bBold,
                                     bool bItalic) {
    PurgeUserData();
    Defaults();
...
ON_BOOL32 ON_Font::IsValid(ON_TextLog *text_log) const {
    return (m_font_name.Length() > 0 && m_font_index >= 0 &&
            m_facename[0] > 32 && m_facename[64] == 0);
}
...
//
// Interface
void ON_Font::SetFontName(const wchar_t *s) { m_font_name = s; }

void ON_Font::SetFontName(const char *s) { m_font_name = s; }

void ON_Font::GetFontName(ON_wString &s) const { s = m_font_name; }
...
#if defined(ON_OS_WINDOWS_GDI)
static int CALLBACK ON__IsSymbolFontFaceNameHelper(ENUMLOGFONTEX *,
                                                   NEWTEXTMETRICEX *, DWORD,
                                                   LPARAM) {
    ...
    if (s && s[0]) {
        ...
        if (hdc) {
            ...
            if (7 == ::EnumFontFamiliesEx(
                         hdc, &logfont,
                         (FONTENUMPROC)ON__IsSymbolFontFaceNameHelper, 0, 0)) {
                rc = true;
            }
            ::ReleaseDC(NULL, hdc);
        }
...
void ON_Font::GetFontFaceName(ON_wString &s) const { s = m_facename; }
...
void ON_Font::SetFontIndex(int i) { m_font_index = i; }

int ON_Font::FontIndex() const { return m_font_index; }

double ON_Font::LinefeedRatio() const { return m_linefeed_ratio; }

void ON_Font::SetLinefeedRatio(double d) { m_linefeed_ratio = d; }

int ON_Font::FontWeight() const { return m_font_weight; }
...
bool ON_Font::IsItalic() const { return m_font_italic; }

void ON_Font::SetIsItalic(bool b) { SetItalic(b); }
...
bool ON_Font::IsBold() const { return (FontWeight() >= bold_weight); }

void ON_Font::SetBold(bool bBold) {
    SetFontWeight(bBold ? bold_weight : normal_weight);
}

bool ON_Font::IsUnderlined() const { return m_font_underlined; }
...
void ON_Font::UpdateImplementationSettings() {
    ...
    if (b) {
        ...
        m_logfont.lfCharSet =
            ON_Font::IsSymbolFontFaceName(m_logfont.lfFaceName)
            ...
            if (hdc) {
                ...
                if (font) {
                    ...
                    if (numPoints > 2) {
                        ...
                        LPPOINT pPoints =
                            (LPPOINT)onmalloc(numPoints * sizeof(*pPoints));
                        LPBYTE pTypes =
                            (LPBYTE)onmalloc(numPoints * sizeof(*pTypes));
                        if (pTypes && pPoints) {
                            // Get the points and types from the current path
                            numPoints =
                                ::GetPath(hdc, pPoints, pTypes, numPoints);
                            if (numPoints > 2) {
...
const LOGFONT &ON_Font::LogFont() const { return m_logfont; }
...
bool ON_Font::CompareFontCharacteristics(ON_Font &other_font,
