...
bool ON_2dPointArray::Transform(const ON_Xform &xform) {
    return ON_TransformPointList(2, false, Count(), 2, (m_a) ? &m_a[0].x : 0,
                                 xform);
}
...
bool ON_3dPointArray::GetBoundingBox(ON_BoundingBox &bbox, int bGrowBox) const {
...
bool ON_3dPointArray::Transform(const ON_Xform &xform) {
    return ON_TransformPointList(3, false, Count(), 3, (m_a) ? &m_a[0].x : 0,
                                 xform);
}
...
bool ON_3dPointArray::Rotate(double sin_angle, double cos_angle,
...
bool ON_3dPointArray::Rotate(double angle_in_radians,
                             const ON_3dVector &axis_of_rotation,
                             const ON_3dPoint &center_of_rotation) {
    return Rotate(sin(angle_in_radians), cos(angle_in_radians),
                  axis_of_rotation, center_of_rotation);
}
...
bool ON_3dPointArray::Create(int point_dimension, int bRational,
                             int point_count, int point_stride,
                             const double *points) {
    bool rc = false;
    if (point_dimension >= 2 && point_dimension <= 3 && point_count > 0 &&
                points && point_stride >= bRational
            ? (point_dimension + 1)
            : point_dimension) {
        rc = true;
        int i;
...
bool ON_4dPointArray::Transform(const ON_Xform &xform) {
    return ON_TransformPointList(3, true, Count(), 4, (m_a) ? &m_a[0].x : 0,
                                 xform);
}
...
bool ON_3dVectorArray::GetBBox(double boxmin[3], double boxmax[3],
                               bool bGrowBox) const {
    return ON_GetPointListBoundingBox(
        3, false, Count(), 3, (m_a) ? &m_a[0].x : 0, boxmin, boxmax, bGrowBox);
}
...
bool ON_2fVectorArray::GetBBox(float boxmin[2], float boxmax[2],
                               bool bGrowBox) const {
    return ON_GetPointListBoundingBox(
        2, false, Count(), 2, (m_a) ? &m_a[0].x : 0, boxmin, boxmax, bGrowBox);
}
...
ON_UuidList::ON_UuidList(const ON_UuidList &src)
    : ON_SimpleArray<ON_UUID>(src), m_sorted_count(src.m_sorted_count),
...
int ON_UuidList::Count() const { return m_count - m_removed_count; }
...
ON_UuidIndexList::ON_UuidIndexList(int capacity)
    : ON_SimpleArray<ON_UuidIndex>(capacity > 32 ? capacity : 32),
      m_sorted_count(0), m_removed_count(0) {}
...
ON_UuidIndexList::ON_UuidIndexList(const ON_UuidIndexList &src)
    : ON_SimpleArray<ON_UuidIndex>(src), m_sorted_count(src.m_sorted_count),
      m_removed_count(src.m_removed_count) {}
...
bool ON_UuidIndexList::AddUuidIndex(ON_UUID uuid, int index,
...
int ON_UuidIndexList::Count() const { return m_count - m_removed_count; }
...
ON_UuidPairList::ON_UuidPairList(const ON_UuidPairList &src)
    : ON_SimpleArray<ON_UuidPair>(src), m_sorted_count(src.m_sorted_count),
      m_removed_count(src.m_removed_count) {}
...
bool ON_UuidPairList::AddPair(ON_UUID id1, ON_UUID id2,
                              bool bCheckForDupicates) {
...
int ON_UuidPairList::Count() const { return m_count - m_removed_count; }
...
void ON_UuidPairList::ImproveSearchSpeed() {
    if (((unsigned int)m_count) > m_sorted_count) {
        ...
        if (m_removed_count > 0) {
            ...
            while (m_count > 0 && !compar_uuidpair_id1id2(
                                      &removed_uuid, m_a + (m_count - 1))) {
                m_count--;
            }
            m_removed_count = 0;
        }
...
ON_UuidPair *ON_UuidPairList::SearchHelper(const ON_UUID *id1) const {
    ...
    // NOTE:
    //   The pair (ON_max_uuid,ON_max_uuid) never appears in the sorted portion
    //   of the array.
    ON_UuidPair *p =
        (m_sorted_count > 0)
            ? (ON_UuidPair *)bsearch(
    ...
    if (0 == p) {
        ...
        for (i = m_sorted_count; i < m_count; i++) {
            if (0 == ON_UuidList::CompareUuid(id1, &m_a[i].m_uuid[0]) &&
                (ON_max_uuid != m_a[i].m_uuid[0] ||
                 ON_max_uuid != m_a[i].m_uuid[1])) {
                p = m_a + i;
                break;
            }
        }
...
int ON_2dexMap::Count() const { return m_count; }

const ON_2dex *ON_2dexMap::Array() const { return m_a; }

void ON_2dexMap::Reserve(int capacity) {
    ON_SimpleArray<ON_2dex>::Reserve(capacity);
}

ON_2dex ON_2dexMap::operator[](int i) const { return m_a[i]; }
