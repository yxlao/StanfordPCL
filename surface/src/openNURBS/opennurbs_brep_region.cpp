...
ON_Brep *ON_Brep::SubBrep(int subfi_count, const int *subfi,
                          ON_Brep *sub_brep) const {
    ...
    for (i = 0; i < m_E.Count(); i++) {
        if (Emap[i]) {
            ...
            ON_BrepEdge &sub_edge =
                sub_brep->NewEdge(sub_v0, sub_v1, sub_brep->m_C3.Count() - 1, 0,
                                  edge.m_tolerance);
            Emap[i] = sub_edge.m_edge_index;
            sub_edge.CopyUserData(edge);
            // March 29, 2010 Lowell - Copy user fields
            memcpy(&sub_edge.m_edge_user, &edge.m_edge_user,
                   sizeof(sub_edge.m_edge_user));
        } else
            Emap[i] = -1;
    }
    ...
    for (i = 0; i < subfi_count; i++) {
        ...
        // March 29, 2010 Lowell - Copy user fields
        memcpy(&sub_face.m_face_user, &face.m_face_user,
               sizeof(sub_face.m_face_user));
        ...
        for (fli = 0; fli < face.m_li.Count(); fli++) {
            ...
            memcpy(&sub_loop.m_loop_user, &loop.m_loop_user,
                   sizeof(sub_loop.m_loop_user));
            ...
            for (lti = 0; lti < loop.m_ti.Count(); lti++) {
                ...
                // March 29, 2010 Lowell - copy ptonsrf type
                    ...
                    ON_BrepTrim &sub_trim =
                        sub_brep->NewTrim(false, sub_loop, -1);
                    sub_trim.m_type = ON_BrepTrim::ptonsrf;
                    ON_BrepVertex &sub_vertex =
                        sub_brep->m_V[Vmap[trim.m_vi[0]]];
                    sub_trim.m_vi[0] = sub_trim.m_vi[1] =
                        sub_vertex.m_vertex_index;
                } else {
                    return 0;
                }
                ON_BrepTrim &sub_trim =
                    sub_brep->m_T[sub_brep->m_T.Count() - 1];
