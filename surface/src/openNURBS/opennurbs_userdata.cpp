...
ON_UserData::ON_UserData()
    : m_userdata_uuid(ON_nil_uuid), m_application_uuid(ON_nil_uuid),
      m_userdata_copycount(0), m_userdata_xform(1), m_userdata_owner(0),
      m_userdata_next(0) {}

ON_UserData::ON_UserData(const ON_UserData &src)
    : ON_Object(src), m_userdata_uuid(src.m_userdata_uuid),
...
ON_BOOL32 ON_UserData::IsValid(ON_TextLog *text_log) const {
    ...
    if (0 == ON_UuidCompare(m_userdata_uuid, ON_UserData::ClassId()->Uuid())) {
        if (0 != text_log) {
            text_log->Print("invalid userdata - m_userdata_uuid in use. Use "
                            "guidgen to get a unique id.\n");
        }
        return false;
    }

    if (Archive() && 0 == ON_UuidCompare(ClassId()->Uuid(),
                                         ON_UserData::ClassId()->Uuid())) {
        ...
        if (0 != text_log) {
            text_log->Print(
                "invalid userdata - classes derived from ON_UserData that get "
                "saved in 3dm archives must have a class id and name defined "
                "by ON_OBJECT_DECLARE/ON_OBJECT_IMPLEMENT.\n");
        }
        return false;
    }
...
ON_Object *ON_UserData::Owner() const { return m_userdata_owner; }

ON_UserData *ON_UserData::Next() const { return m_userdata_next; }
...
ON_BOOL32 ON_UserData::IsUnknownUserData() const {
    return (ClassId() == &ON_UnknownUserData::m_ON_UnknownUserData_class_id)
               ? true
               : false;
}

ON_BOOL32 ON_UserData::GetDescription(ON_wString &description) { return true; }
...
ON_UnknownUserData::ON_UnknownUserData()
    : m_unknownclass_uuid(ON_nil_uuid), m_sizeof_buffer(0), m_buffer(0),
      m_3dm_version(0), m_3dm_opennurbs_version(0) {}

ON_UnknownUserData::ON_UnknownUserData(const ON_UnknownUserData &src)
    : ON_UserData(src), m_unknownclass_uuid(ON_nil_uuid), m_sizeof_buffer(0),
      m_buffer(0), m_3dm_version(0), m_3dm_opennurbs_version(0) {
...
ON_UnknownUserData::operator=(const ON_UnknownUserData &src) {
    if (this != &src) {
        ...
        m_userdata_uuid = src.m_userdata_uuid;
        m_application_uuid =
            src.m_application_uuid; // fix added 26 January 2010

        if (m_userdata_copycount > 0 && src.m_sizeof_buffer > 0 &&
            src.m_buffer) {
            m_unknownclass_uuid = src.m_unknownclass_uuid;
            m_sizeof_buffer = src.m_sizeof_buffer;
            m_buffer = onmemdup(src.m_buffer, src.m_sizeof_buffer);
...
unsigned int ON_UnknownUserData::SizeOf() const {
    return ON_UserData::SizeOf() +
           (sizeof(ON_UnknownUserData) - sizeof(ON_UserData)) + m_sizeof_buffer;
}

ON_BOOL32 ON_UnknownUserData::GetDescription(ON_wString &s) {
    s = "Unknown user data. (Definition of class was not available when object "
        "was read.)";
    return true;
}
...
ON_BOOL32 ON_UnknownUserData::IsValid(ON_TextLog *text_log) const {
    ...
    if (rc) {
        ...
        rc =
            ON_UuidCompare(&m_unknownclass_uuid, &ON_UnknownUserData_classuuid);
    }
...
class ON_UnknownUserDataArchive : public ON_BinaryArchive {
  ...
  public:
    ...
    size_t CurrentPosition( // current offset (in bytes) into archive ( like
                            // ftell() )
    ) const;
    bool SeekFromCurrentPosition( // seek from current position ( like fseek(
                                  // ,SEEK_CUR) )
        ...
        if (pID) {
            ...
            if (pObject) {
                ...
                if (!ud)
                    ...
                    // use class's Read() function to initialize class members
                    // from buffer
...
bool ON_UserDataHolder::MoveUserDataTo(const ON_Object &source_object,
                                       bool bAppend) {
    if (!bAppend) {
...
ON_UserStringList::ON_UserStringList() {
    ...
    m_application_uuid =
        ON_opennurbs4_id; // opennurbs.dll reads/writes this userdata
...
ON_BOOL32 ON_UserStringList::Archive() const { return true; }
...
int ON_UserStringList::SetUserStrings(int count, const ON_UserString *us,
                                      bool bReplace) {
    int added_count = 0;
    int i;
    ...
    ON_2dex *hash =
        (ON_2dex *)onmalloc((count0_plus_count + count) * sizeof(hash[0]));
...
int ON_Object::SetUserStrings(int count, const ON_UserString *user_strings,
                              bool bReplace) {
...
ON_BOOL32 ON_DocumentUserStringList::Read(ON_BinaryArchive &binary_archive) {
    ...
    if (2 == c) {
        ...
        int major_version = 0;
        int minor_version = 0;
        rc = binary_archive.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK,
