...
class ON_HatchExtra : public ON_UserData {
  ...
  public:
    ...
    static const ON_HatchExtra *HatchExtension(const ON_Hatch *pHatch,
                                               bool bCreate);
...
ON_HatchExtra *ON_HatchExtra::HatchExtension(ON_Hatch *pHatch, bool bCreate) {
    ...
    if (pHatch) {
        pExtra = ON_HatchExtra::Cast(pHatch->GetUserData(
            ON_HatchExtra::m_ON_HatchExtra_class_id.Uuid()));
        if (pExtra == 0 && bCreate) {
...
ON_BOOL32 ON_HatchExtra::Archive() const { return true; }
...
ON_2dPoint ON_HatchExtra::BasePoint() const { return m_basepoint; }
...
bool ON_HatchLine::operator==(const ON_HatchLine &src) const {
    return (m_angle == src.m_angle && m_base == src.m_base &&
            m_offset == src.m_offset && m_dashes == src.m_dashes);
}
...
double ON_HatchLine::Angle() const { return m_angle; }
...
ON_2dPoint ON_HatchLine::Base() const { return m_base; }

void ON_HatchLine::SetBase(const ON_2dPoint &base) { m_base = base; }

ON_2dVector ON_HatchLine::Offset() const { return m_offset; }

void ON_HatchLine::SetOffset(const ON_2dVector &offset) { m_offset = offset; }

int ON_HatchLine::DashCount() const { return m_dashes.Count(); }
...
void ON_HatchLine::GetLineData(double &angle, ON_2dPoint &base,
                               ON_2dVector &offset,
...
ON_HatchPattern::ON_HatchPattern()
    : m_hatchpattern_index(-1), m_hatchpattern_id(ON_nil_uuid),
...
void ON_HatchPattern::SetFillType(eFillType type) { m_type = type; }
...
const wchar_t *ON_HatchPattern::Name() const { return m_hatchpattern_name; }
...
const wchar_t *ON_HatchPattern::Description() const { return m_description; }

void ON_HatchPattern::SetIndex(int i) { m_hatchpattern_index = i; }

int ON_HatchPattern::Index() const { return m_hatchpattern_index; }

//  Line HatchPattern functions

int ON_HatchPattern::HatchLineCount() const { return m_lines.Count(); }
...
void ON_HatchPattern::RemoveAllHatchLines() { m_lines.Empty(); }
...
const ON_Curve *ON_HatchLoop::Curve() const { return m_p2dCurve; }
...
ON_HatchLoop::eLoopType ON_HatchLoop::Type() const { return m_type; }

void ON_HatchLoop::SetType(eLoopType type) { m_type = type; }

//  class ON_Hatch
/////////////////////////////////////////////////////////////////
ON_OBJECT_IMPLEMENT(ON_Hatch, ON_Geometry,
...
ON_Hatch::ON_Hatch(const ON_Hatch &src)
    : ON_Geometry(src), m_plane(src.m_plane),
...
ON::object_type ON_Hatch::ObjectType() const { return ON::hatch_object; }

int ON_Hatch::Dimension() const { return 3; }
...
int ON_Hatch::PatternIndex() const { return m_pattern_index; }

void ON_Hatch::SetPatternIndex(int index) { m_pattern_index = index; }

ON_BOOL32 ON_Hatch::GetBBox(double *bmin, double *bmax,
...
bool ON_Hatch::GetTightBoundingBox(ON_BoundingBox &tight_bbox, int bGrowBox,
...
bool ON_Hatch::Create(const ON_Plane &plane,
                      ...
                      int pattern_index, double pattern_rotation,
...
const ON_Plane &ON_Hatch::Plane() const { return m_plane; }

void ON_Hatch::SetPlane(const ON_Plane &plane) { m_plane = plane; }

double ON_Hatch::PatternRotation() const { return m_pattern_rotation; }

void ON_Hatch::SetPatternRotation(double rotation) {
    m_pattern_rotation = rotation;
}

double ON_Hatch::PatternScale() const { return m_pattern_scale; }
...
int ON_Hatch::LoopCount() const { return m_loops.Count(); }

void ON_Hatch::AddLoop(ON_HatchLoop *pLoop) { m_loops.Append(pLoop); }
