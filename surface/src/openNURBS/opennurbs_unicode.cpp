...
int ON_EncodeUTF8(ON__UINT32 u, ON__UINT8 sUTF8[6]) {
    ...
    if (u <= 0x3FFFFFF) {
        // 5 byte UTF8 encoding: 111110xx, 10xxxxxx, 10xxxxxx, 10xxxxxx,
        // 10xxxxxx
    ...
    if (u <= 0x7FFFFFFF) {
        // 6 byte UTF8 encoding: 111110xx, 10xxxxxx, 10xxxxxx, 10xxxxxx,
        // 10xxxxxx, 10xxxxxx
...
static int ON_DecodeUTF8Helper(const ON__UINT8 *sUTF8, int sUTF8_count,
                               ON__UINT32 *value, unsigned int *error_status) {
...
#define OVERLONG_ENCODING 8
    ...
    if (0xC0 == (0xE0 & c)) {
        ...
        if (0x80 != (0xC0 & c)) {
            *error_status |=
                INVALID_CONTINUATION_VALUE; // invalid UTF=8 continuation value
            return 0;
        }
        u *= 64;
        u |= (0x3F & c);
        if (u <= 0x7F) {
            *error_status |=
                OVERLONG_ENCODING; // overlong 2 byte character encoding
        }
        *value = u;
        return 2;
    }
    ...
    if (0xE0 == (0xF0 & c)) {
        ...
        if (u <= 0x7FF) {
            *error_status |=
                OVERLONG_ENCODING; // overlong 3 byte character encoding
        }
        *value = u;
        return 3;
    }
    ...
    if (0xF0 == (0xF8 & c)) {
        ...
        if (u <= 0xFFFF) {
            *error_status |=
                OVERLONG_ENCODING; // overlong 4 byte character encoding
        }
        *value = u;
        return 4;
    }
    ...
    if (0xF8 == (0xFC & c)) {
        // 5 byte character encoding: 111110xx, 10xxxxxx, 10xxxxxx, 10xxxxxx,
        // 10xxxxxx
        if (sUTF8_count < 5) {
            *error_status |= INPUT_BUFFER_TOO_SHORT; // input buffer too short
            return 0;
        ...
        if (u <= 0x1FFFFF) {
            *error_status |=
                OVERLONG_ENCODING; // overlong 5 byte character encoding
        }
        *value = u;
        return 5;
    }
    ...
    if (0xFC == (0xFE & c)) {
        // 6 byte character encoding: 110xxxxx, 10xxxxxx, 10xxxxxx, 10xxxxxx,
        // 10xxxxxx, 10xxxxxx
        if (sUTF8_count < 6) {
            *error_status |= INPUT_BUFFER_TOO_SHORT; // input buffer too short
            return 0;
        ...
        if (u <= 0x3FFFFFF) {
            *error_status |=
                OVERLONG_ENCODING; // overlong 6 byte character encoding
        }
        *value = u;
        return 6;
    }
...
int ON_DecodeUTF8(const ON__UINT8 *sUTF8, int sUTF8_count,
    ...
    if (i0 < sUTF8_count && u0 >= 0xD800 && u0 <= 0xDBFF &&
        ...
        u1 = 0xFFFFFFFF;
        i1 = ON_DecodeUTF8Helper(sUTF8 + i0, sUTF8_count - i0, &u1,
                                 &error_status);
        if (i1 > 0 && (0 == error_status || 8 == error_status)) {
            ...
            u0 = 0xFFFFFFFF;
            if (2 == ON_ConvertUTF16ToUTF32(false, sUTF16, 2, &u0, 1,
                                            &error_status, 0, 0, 0) &&
...
int ON_EncodeUTF16(ON__UINT32 unicode_code_point, ON__UINT16 sUTF16[2]) {
    ...
    if (unicode_code_point <= 0x10FFFF) {
        ...
        sUTF16[0] = (ON__UINT16)(
            0xD800 + (unicode_code_point /
                      0x400)); // high surrogate value (0xD800 ... 0xDBFF)
        sUTF16[1] = (ON__UINT16)(
            0xDC00 + (unicode_code_point &
...
int ON_DecodeSwapByteUTF16(const ON__UINT16 *sUTF16, int sUTF16_count,
    ...
    if (sUTF16_count >= 2 && w0 < 0xDC00) {
        ...
        if (w1 >= 0xDC00 && w1 < 0xE000) {
            ...
            ulo = w1;
            *unicode_code_point =
                (uhi - 0xD800) * 0x400 + (ulo - 0xDC00) + 0x10000;
            return 2;
        }
    }
...
int ON_ConvertWideCharToUTF8(int bTestByteOrder, const wchar_t *sWideChar,
    ...
    case sizeof(ON__UINT16):
        ...
        rc = ON_ConvertUTF16ToUTF8(
            bTestByteOrder, (const ON__UINT16 *)sWideChar, sWideChar_count,
            (ON__UINT8 *)sUTF8, sUTF8_count, error_status, error_mask,
            error_code_point, (const ON__UINT16 **)sNextWideChar);
        break;

    case sizeof(ON__UINT32):
        // assume wchar_t strings are UTF-32 encoded
        rc = ON_ConvertUTF32ToUTF8(
            bTestByteOrder, (const ON__UINT32 *)sWideChar, sWideChar_count,
            (ON__UINT8 *)sUTF8, sUTF8_count, error_status, error_mask,
            error_code_point, (const ON__UINT32 **)sNextWideChar);
        break;
...
int ON_ConvertUTF8ToWideChar(const char *sUTF8, int sUTF8_count,
    ...
    case sizeof(ON__UINT16):
        // assume wchar_t strings are UTF-16 encoded
        rc = ON_ConvertUTF8ToUTF16((const ON__UINT8 *)sUTF8, sUTF8_count,
    ...
    case sizeof(ON__UINT32):
        // assume wchar_t strings are UTF-32 encoded
        rc = ON_ConvertUTF8ToUTF32((const ON__UINT8 *)sUTF8, sUTF8_count,
