...
bool ON_Polyline::IsValid(double tolerance) const {
    ...
    if (tolerance > 0.0) {
        ...
        if (rc && m_count < 4 &&
            m_a[0].DistanceTo(m_a[m_count - 1]) <= tolerance)
            rc = false;
    } else {
...
int ON_Polyline::Clean(double tolerance) {
    ...
    if (m_count > 2) {
        ...
        while (m_count > 2 &&
               m_a[m_count - 2].DistanceTo(m_a[m_count - 1]) <= tolerance) {
...
int ON_Polyline::PointCount() const { return m_count; }
...
bool ON_Polyline::ClosestPointTo(const ON_3dPoint &point, double *t,
                                 int segment_index0, int segment_index1) const {
    bool rc = false;
    int segment_index;
    ...
    if (t) {
        ...
        for (segment_index = segment_index0; segment_index < segment_index1;
             segment_index++) {
            double seg_length =
                m_a[segment_index].DistanceTo(m_a[segment_index + 1]);
...
bool ON_Polyline::CreateStarPolygon(const ON_Circle &circle,
                                    double other_radius, int side_count) {
