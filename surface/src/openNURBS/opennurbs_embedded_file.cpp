...
ON_Buffer::ON_Buffer()
    : m_buffer_size(0), m_current_position(0), m_first_segment(0),
      m_last_segment(0), m_current_segment(0), m_error_handler(0),
      m_last_error(0) {
    memset(m_reserved, 0, sizeof(m_reserved));
}

void ON_Buffer::Destroy() { ChangeSize(0); }
...
struct ON_BUFFER_SEGMENT {
    ...
    ON__UINT64 m_segment_position1; // position of the first byte in the next
                                    // segment When a segment is the last one in
                                    // an ON_Buffer, is is common for
                                    // m_segment_position1 > m_buffer_size.
    unsigned char
        *m_segment_buffer; // null or an array of length (m_segment_position1 -
                           // m_segment_position0)
    void *m_reserved;
};
...
int ON_Buffer::Compare(const ON_Buffer &a, const ON_Buffer &b) {
    ...
    while (0 != aseg && 0 != bseg && size < buffer_size) {
        if (0 == asegsize) {
            ...
            asegsize =
                (size_t)(aseg->m_segment_position1 - aseg->m_segment_position0);
            aoffset = 0;
        }
        ...
        if (0 == bsegsize) {
            ...
            bsegsize =
                (size_t)(bseg->m_segment_position1 - bseg->m_segment_position0);
            boffset = 0;
        }
        ...
        rc = memcmp(aseg->m_segment_buffer + aoffset,
                    bseg->m_segment_buffer + boffset, (size_t)sz);
        if (0 != rc)
            return ((rc < 0) ? -1 : 1);
        aoffset += sz;
        boffset += sz;
...
ON_Buffer::~ON_Buffer() {
    ChangeSize(
        0); // frees all heap and zeros everything but m_current_position.
    m_current_position = 0;
}
...
bool ON_Buffer::Seek(ON__INT64 offset, int origin) {
    ...
    if (offset < 0) {
        ...
        pos1 = pos0 - (ON__UINT64)(
                          -offset); // overflow cannot happen in this operation
    } else if (offset > 0) {
        // current position can be >= m_buffer_size
        pos1 = pos0 +
               (ON__UINT64)(offset); // overflow is possible in this operation
        if (pos1 <= pos0) {
            // overflow
            ON_ERROR("Attempt to seek to a position that is too large for "
                     "64-bit unsigned int storage.");
            return false;
        }
    } else {
        pos1 = pos0;
...
bool ON_Buffer::SeekFromStart(ON__INT64 offset) { return Seek(offset, 0); }

bool ON_Buffer::SeekFromCurrentPosition(ON__INT64 offset) {
    return Seek(offset, 1);
}

bool ON_Buffer::SeekFromEnd(ON__INT64 offset) { return Seek(offset, 2); }
    ...
    if (0 == m_buffer_size) {
        ...
        if (m_buffer_size == m_last_segment->m_segment_position1)
            ...
            ON__UINT64 sizeof_segment_buffer =
                m_buffer_size - m_last_segment->m_segment_position0;
            struct ON_BUFFER_SEGMENT *prev_segment =
                m_last_segment->m_prev_segment;
            void *last_buffer = (0 != m_last_segment->m_segment_buffer &&
                                 m_last_segment->m_segment_buffer !=
                                     (unsigned char *)(m_last_segment + 1))
                                    ? m_last_segment->m_segment_buffer
                                    : 0;
            ...
            if (0 != new_last_segment) {
                if (new_last_segment != m_last_segment || 0 != last_buffer) {
                    new_last_segment->m_segment_buffer =
                        (unsigned char *)(new_last_segment + 1);
                    if (0 != last_buffer) {
...
bool ON_Buffer::ChangeSize(ON__UINT64 buffer_size) {
    if (buffer_size <= 0) {
        ...
        while (0 != p0) {
            ...
            if (0 != p0->m_segment_buffer &&
                (void *)(p0->m_segment_buffer) != (void *)(p0 + 1))
        ...
        while (0 != m_last_segment) {
            if (m_last_segment->m_segment_position0 < buffer_size) {
                ...
                if (0 != m_last_segment->m_segment_buffer &&
                    ...
                    memset(
                        ...
                        0,
                        (size_t)(m_last_segment->m_segment_position1 -
                                 buffer_size));
                }
            ...
            if (0 != p)
                p->m_next_segment = 0;
            if (0 != m_last_segment->m_segment_buffer &&
                (void *)(m_last_segment->m_segment_buffer) !=
                    (void *)(m_last_segment + 1))
...
void ON_Buffer::Copy(const ON_Buffer &src) {
    ...
    for (src_seg = src.m_first_segment; 0 != src_seg;
         src_seg = src_seg->m_next_segment) {
        ...
        ON__UINT64 segment_buffer_size =
            (0 != src_seg->m_segment_buffer)
                ...
                : 0;
        dst_seg = (struct ON_BUFFER_SEGMENT *)onmalloc(
            sizeof(*dst_seg) + ((size_t)segment_buffer_size));
        ...
        if (segment_buffer_size > 0) {
            ...
            memcpy(dst_seg->m_segment_buffer, src_seg->m_segment_buffer,
                   (size_t)segment_buffer_size); // segment_buffer_size always <
                                                 // 0xFFFFFFFF
        }
    ...
    // 27 June, 2001 Dale Lear: Should this copy m_last_error and
    // m_error_handler? Not sure.
}

static bool ON_Buffer_IsNotValid() { return false; }
...
bool ON_Buffer::IsValid(const ON_TextLog *text_log) const {
    ...
    for (seg = m_first_segment; seg != 0; seg = seg->m_next_segment) {
        if (prev_seg != seg->m_prev_segment)
            return ON_Buffer_IsNotValid();
        if (0 != prev_seg &&
            prev_seg->m_segment_position1 != seg->m_segment_position0)
            return ON_Buffer_IsNotValid();
...
bool ON_Buffer::AtEnd() const { return (m_current_position == m_buffer_size); }

ON__UINT64 ON_Buffer::Size() const { return m_buffer_size; }
...
ON__UINT32 ON_Buffer::CRC32(ON__UINT32 current_remainder) const {
    ...
    for (seg = m_first_segment; 0 != seg; seg = seg->m_next_segment) {
        ...
        if (seg->m_segment_position0 > seg->m_segment_position1) {
            // This is really bad!  If you can determine how the corruption
            // occurs, plase make a bug report and tell Dale Lear as soon as
            // possible.
        ...
        if (0 == prev_seg) {
            if (0 != seg->m_segment_position0) {
                ...
                ON_ERROR("corrupt buffer - first segment has non-zero value "
                         "for position0.");
            }
            ...
            // If you can determine how the corruption occured, please
            // make a bug report and assign it to Dale Lear.
            ON_ERROR("corrupt buffer - previous segment's position1 !- "
                     "segment's position0.");
        }
        ...
        if (seg_size + size > m_buffer_size) {
            if (seg != m_last_segment || seg->m_next_segment) {
                // If you can determine how the corruption occured, please
                // make a bug report and assign it to Dale Lear.
                ON_ERROR("corrupt buffer - segments contain more bytes than "
                         "m_buffer_size.");
            }
            seg_size = m_buffer_size - size;
        }

        current_remainder = ON_CRC32(current_remainder, (size_t)seg_size,
                                     seg->m_segment_buffer);
        size += seg_size;
        if (size >= m_buffer_size) {
            if (seg != m_last_segment || 0 != seg->m_next_segment ||
                size > m_buffer_size) {
                // If you can determine how the corruption occured, please
                // make a bug report and assign it to Dale Lear.
...
ON__UINT64 ON_Buffer::CurrentPosition() const { return m_current_position; }
...
bool ON_Buffer::SetCurrentSegment(bool bWritePending) {
    ...
    if (0 == m_current_segment)
        m_current_segment = (m_current_position <= m_buffer_size / 2)
                                ? m_first_segment
                                : m_last_segment;
...
ON__UINT64 ON_Buffer::Write(ON__UINT64 size, const void *buffer) {
    ...
    while (size > 0) {
        if (0 == m_current_segment) {
            ...
            const ON__UINT64 padding_size =
                4 * sizeof(void *); // room for os heap info
            ...
            ON__UINT64 malloc_size =
                (0 != m_last_segment && m_last_segment->m_segment_position1 >
            ...
            // (size_t) cast is safe because malloc_size is always <=
            // max_malloc_size = 16*page_size <  0xFFFFFFFF
            m_current_segment =
                (struct ON_BUFFER_SEGMENT *)onmalloc((size_t)malloc_size);
            memset(m_current_segment, 0, (size_t)malloc_size);
            m_current_segment->m_prev_segment = m_last_segment;
            m_current_segment->m_segment_buffer =
                (unsigned char *)(m_current_segment + 1);
            if (0 != m_last_segment) {
        ...
        if (m_current_position >= m_current_segment->m_segment_position1) {
            // happens when a seek puts the current position beyond the end of
            // the buffer.
        ...
        ON__UINT64 offset =
            m_current_position - m_current_segment->m_segment_position0;
        ON__UINT64 sz =
            (m_current_segment->m_segment_position1 - m_current_position);

        if (sz > size)
            sz = size;
        memcpy(m_current_segment->m_segment_buffer + offset, buffer,
               (size_t)sz);
...
ON__UINT64 ON_Buffer::Read(ON__UINT64 size, void *buffer) {
    ...
    while (size > 0) {
        if (0 == m_current_segment ||
            0 == m_current_segment->m_segment_buffer) {
            ON_ERROR("Corrupt ON_Buffer");
            return 0;
        }
        ...
        if (sz > size)
            sz = size;
        memcpy(buffer, m_current_segment->m_segment_buffer + offset,
               (size_t)sz);
        m_current_position += sz;
        rc += sz;
        size -= sz;
        ...
        if (size > 0) {
            if (m_current_position == m_buffer_size &&
                m_current_segment == m_last_segment) {
                // This is a common situation that occures when the read request
                // is for a size larger than the remaining number of bytes in
                // the buffer. For example, when repeatedly reading into a fixed
                // size buffer until reasing the end of the file. This is not an
                // error condition.
                break;
            }
            m_current_segment = m_current_segment->m_next_segment;
...
ON__UINT32 ON_Buffer::LastError() const { return m_last_error; }

void ON_Buffer::ClearLastError() { m_last_error = 0; }
    ...
    for (;;) {
        ...
        for (struct ON_BUFFER_SEGMENT *seg = m_first_segment;
            ...
            if (seg->m_segment_position1 <= seg->m_segment_position0)
                continue;
            ON__UINT64 seg_size =
                (seg->m_segment_position1 - seg->m_segment_position0);
            if (seg_size + size > m_buffer_size)
                seg_size = m_buffer_size - size;
...
bool ON_Buffer::ReadFromBinaryArchive(ON_BinaryArchive &archive) {
    ...
    if (rc) {
        ...
        if (buffer_crc != saved_buffer_crc ||
            m_buffer_size != saved_buffer_size) {
            // The buffer's contents have been damaged.
            ON_ERROR("The buffer contents were corrupted during, writing, "
                     "storage or reading.");
        }
    } else {
        Destroy();
...
static bool ON_Buffer_StreamCallback(void *context, ON__UINT32 size,
                                     const void *buffer) {
...
bool ON_Buffer::Compress(ON_Buffer &compressed_buffer) const {
    ...
    ON_Buffer *out =
        (this == &compressed_buffer) ? new ON_Buffer() : &compressed_buffer;
    ...
    for (;;) {
        ...
        for (seg = m_first_segment; 0 != seg; seg = seg->m_next_segment) {
            ...
            if (!compressor.In(pos1 - seg->m_segment_position0,
                               seg->m_segment_buffer))
                break;
            prev_seg = seg;
        }
...
bool ON_Buffer::Uncompress(ON_Buffer &uncompressed_buffer) const {
    ...
    for (;;) {
        ...
        for (seg = m_first_segment; 0 != seg; seg = seg->m_next_segment) {
            ...
            if (!uncompressor.In(pos1 - seg->m_segment_position0,
                                 seg->m_segment_buffer))
                break;
            prev_seg = seg;
        }
...
bool ON_EmbeddedFile::Create(const wchar_t *file_full_path_name,
                             bool bCompress) {
    Destroy();
...
ON_EmbeddedFile::ON_EmbeddedFile()
    : m_id(ON_nil_uuid), m_reserved(0), m_file_size(0), m_file_time(0),
      m_file_crc(0), m_buffer_crc(0), m_bCompressedBuffer(0) {
    m_reserved3[0] = 0;
    m_reserved3[1] = 0;
    m_reserved3[2] = 0;
    m_reserved3[3] = 0;
...
ON_EmbeddedFile::ON_EmbeddedFile(const ON_EmbeddedFile &src)
    : ON_Object(src), m_id(src.m_id), m_full_file_name(src.m_full_file_name),
      m_relative_file_name(src.m_relative_file_name), m_reserved(0),
      m_file_size(src.m_file_size), m_file_time(src.m_file_time),
      m_file_crc(src.m_file_crc), m_buffer_crc(src.m_buffer_crc),
      m_buffer(src.m_buffer), m_bCompressedBuffer(m_bCompressedBuffer) {
    m_reserved3[0] = 0;
    m_reserved3[1] = 0;
    m_reserved3[2] = 0;
    m_reserved3[3] = 0;
...
static bool CompressedStreamHandler(void *context, ON__UINT32 size,
                                    const void *buffer) {
    return (size == ((ON_Buffer *)context)->Write(size, buffer));
}

bool ON_EmbeddedFile::Create(FILE *fp, bool bCompress) {
    Destroy();
...
static bool UncompressedToFileHandler(void *context, ON__UINT32 size,
                                      const void *buffer) {
...
static bool UncompressedToBufferHandler(void *context, ON__UINT32 size,
                                        const void *buffer) {
    struct BufferAndSize *bs = (struct BufferAndSize *)context;
    size_t sz =
        (bs->buffer_size < size) ? (size_t)bs->buffer_size : (size_t)size;
    if (sz > 0) {
...
bool ON_EmbeddedFile::Extract(void *out_buffer) const {
    ON_Workspace ws;
    ...
    for (;;) {
        ...
        if (m_bCompressedBuffer) {
            ...
            file_size += buffer_size;
            file_crc = ON_CRC32(file_crc, (size_t)buffer_size, buffer);
            size_t sz = (bs.buffer_size < buffer_size) ? (size_t)bs.buffer_size
                                                       : (size_t)buffer_size;
            if (sz > 0) {
                memcpy(bs.buffer, buffer, sz);
                bs.buffer_size -= sz;
...
ON_UUID ON_EmbeddedFile::Id() const { return m_id; }
...
void ON_EmbeddedFile::SetId(ON_UUID id) { m_id = id; }

ON__UINT64 ON_EmbeddedFile::FileSize() const { return m_file_size; }

ON__UINT64 ON_EmbeddedFile::FileLastModifiedTime() const { return m_file_time; }

ON__UINT32 ON_EmbeddedFile::FileCRC() const { return m_file_crc; }

static bool ON_EmbeddedFileIsNotValid() { return ON_IsNotValid(); }
...
ON_BOOL32 ON_EmbeddedFile::IsValid(ON_TextLog *text_log) const {
    ...
    if (!m_bCompressedBuffer) {
        if (m_file_size != m_buffer.Size()) {
            if (0 != text_log)
                text_log->Print(
                    "Uncompressed buffer - m_file_size != m_buffer.Size(0)");
            return ON_EmbeddedFileIsNotValid();
        }
...
ON_BOOL32 ON_EmbeddedFile::Read(ON_BinaryArchive &archive) {
    ...
    for (;;) {
        ...
        if (!archive.BeginRead3dmChunk(TCODE_ANONYMOUS_CHUNK,
