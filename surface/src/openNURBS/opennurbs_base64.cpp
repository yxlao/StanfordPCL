...
const bool ON_DecodeBase64::Error() const { return (1 == m_status); }
...
const char *ON_DecodeBase64::Decode(const char *base64str) {
    ...
    for (;;) {
        ...
        u.i = m_cache[3] +
              64 * (m_cache[2] + 64 * (m_cache[1] + 64 * m_cache[0]));
        *outbuf++ = u.b[2];
        *outbuf++ = u.b[1];
        *outbuf++ = u.b[0];
        m_output_count += 3;
...
bool ON_Base64EncodeStream::In(ON__UINT64 in_buffer_size,
    ...
    ON_Base64EncodeImplementation *imp =
        (ON_Base64EncodeImplementation *)m_implementation;
    if (imp->m_in_buffer_size > 0) {
        ...
        if (in_buffer_size < sz) {
            ...
            sz = (ON__UINT32)
                in_buffer_size; // cast is safe because in_buffer_size <
        ...
        rc = (0 != m_out_callback_function)
                 ? m_out_callback_function(m_out_callback_context, 76,
                                           imp->m_out_buffer)
    ...
    while (in_buffer_size >= 57) {
        ...
        EncodeBase64Helper57((const unsigned char *)in_buffer,
                             imp->m_out_buffer);
        crc1 = ON_CRC32(m_out_crc, 76, imp->m_out_buffer);
        rc = (0 != m_out_callback_function)
                 ? m_out_callback_function(m_out_callback_context, 76,
    ...
    imp->m_in_buffer_size =
        (ON__UINT32)in_buffer_size; // safe cast - sizeof_buffer < 57
...
bool ON_Base64EncodeStream::End() {
    ...
    if (imp->m_in_buffer_size > 0) {
        ...
        ON__UINT32 crc1 =
            ON_CRC32(m_out_crc, out_buffer_size, imp->m_out_buffer);
        rc = (0 != m_out_callback_function)
                 ? m_out_callback_function(m_out_callback_context,
                                           out_buffer_size, imp->m_out_buffer)
                 : Out(m_out_callback_context, out_buffer_size,
                       imp->m_out_buffer);
        if (rc) {
            m_in_crc =
                ON_CRC32(m_in_crc, imp->m_in_buffer_size, imp->m_in_buffer);
...
ON__UINT64 ON_Base64EncodeStream::InSize() const { return m_in_size; }

ON__UINT64 ON_Base64EncodeStream::OutSize() const { return m_out_size; }

ON__UINT32 ON_Base64EncodeStream::InCRC() const { return m_in_crc; }

ON__UINT32 ON_Base64EncodeStream::OutCRC() const { return m_out_crc; }
