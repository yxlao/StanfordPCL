...
double ON_HairlinePrintWidth() { return 0.0001; }
...
ON_OBJECT_IMPLEMENT(ON_Linetype, ON_Object,
...
ON_BOOL32 ON_Linetype::IsValid(ON_TextLog *text_log) const {
    ...
    if (1 == count) {
        if (m_segments[0].m_length <= 0.0) {
            if (text_log)
                text_log->Print("ON_Linetype bogus single segment linetype - "
                                "length <= 0.0 (it must be > 0)\n");
            return false;
        }
        ...
        for (i = 0; i < count; i++) {
            ...
            if (i) {
                if (m_segments[i].m_seg_type == m_segments[i - 1].m_seg_type) {
                    if (text_log)
                        text_log->Print("ON_Linetype consecutive segments have "
                                        "same type.\n");
                    return false;
                }

                if (0.0 == m_segments[i].m_length &&
                    0.0 == m_segments[i - 1].m_length) {
                    if (text_log)
                        text_log->Print("ON_Linetype consecutive segments have "
                                        "length zero.\n");
                    return false;
...
int ON_Linetype::LinetypeIndex() const { return m_linetype_index; }
...
int ON_Linetype::SegmentCount() const { return m_segments.Count(); }
...
bool ON_Linetype::SetSegment(int index, double length,
