...
ON_OBJECT_IMPLEMENT(ON_BrepVertex, ON_Point,
                    "60B5DBC0-E660-11d3-BFE4-0010830122F0");

ON_BrepVertex::ON_BrepVertex()
    : m_vertex_index(-1), m_tolerance(ON_UNSET_VALUE) {
...
ON_BrepVertex::IsValid(ON_TextLog *text_log) const {
    ...
    for (vei = 0; vei < ve_count; vei++) {
        ...
        if (ei < 0) {
            if (text_log)
                text_log->Print("ON_BrepVertex m_ei[%d] = %d.  m_ei[] values "
                                "should be >= 0\n",
                                vei, ei);
            return false;
        }
...
void ON_BrepVertex::Dump(ON_TextLog &dump) const {
...
ON_3dPoint ON_BrepVertex::Point() const { return point; }

double ON_BrepVertex::Tolerance() const { return m_tolerance; }

int ON_BrepVertex::EdgeCount() const { return m_ei.Count(); }

////////////////////////////////////////////////////////////////
//   Class ON_BrepEdge
////////////////////////////////////////////////////////////////

ON_OBJECT_IMPLEMENT(ON_BrepEdge, ON_CurveProxy,
                    "60B5DBC1-E660-11d3-BFE4-0010830122F0");

ON_BrepEdge::ON_BrepEdge()
    : ON_CurveProxy(0), m_edge_index(-1), m_c3i(-1),
      m_tolerance(ON_UNSET_VALUE), m_brep(0) {
    memset(&m_edge_user, 0, sizeof(m_edge_user));
    m_vi[0] = m_vi[1] = -1;
}

ON_BrepEdge::ON_BrepEdge(int edge_index)
    : ON_CurveProxy(0), m_edge_index(edge_index), m_c3i(-1),
      m_tolerance(ON_UNSET_VALUE), m_brep(0) {
    memset(&m_edge_user, 0, sizeof(m_edge_user));
    m_vi[0] = m_vi[1] = -1;
}
...
ON_BOOL32 ON_BrepEdge::IsValid(ON_TextLog *text_log) const {
    ...
    if (!rc) {
        ...
        if (text_log) {
            text_log->Print(
                "ON_BrepEdge.m_brep = NULL (should point to parent ON_Brep)\n");
        }
        rc = false;
    }
...
ON_Brep *ON_BrepEdge::Brep() const { return m_brep; }

ON_BrepTrim *ON_BrepEdge::Trim(int eti) const {
    return (m_brep && eti >= 0 && eti < m_ti.Count()) ? m_brep->Trim(m_ti[eti])
                                                      : 0;
}

int ON_BrepEdge::TrimCount() const { return m_ti.Count(); }
...
ON_OBJECT_IMPLEMENT(ON_BrepTrim, ON_CurveProxy,
                    "60B5DBC2-E660-11d3-BFE4-0010830122F0");

ON_BrepTrim::ON_BrepTrim()
    : m_trim_index(-1), m_c2i(-1), m_ei(-1), m_bRev3d(false),
      m_type(ON_BrepTrim::unknown), m_iso(ON_Surface::not_iso), m_li(-1),
      m__legacy_2d_tol(ON_UNSET_VALUE), m__legacy_3d_tol(ON_UNSET_VALUE),
      m__legacy_flags(0), m_brep(0) {
...
ON_BrepTrim::ON_BrepTrim(int trim_index)
    : m_trim_index(trim_index), m_c2i(-1), m_ei(-1), m_bRev3d(false),
      m_type(ON_BrepTrim::unknown), m_iso(ON_Surface::not_iso), m_li(-1),
      m__legacy_2d_tol(ON_UNSET_VALUE), m__legacy_3d_tol(ON_UNSET_VALUE),
      m__legacy_flags(0), m_brep(0) {
    memset(&m_trim_user, 0, sizeof(m_trim_user));
...
ON_Brep *ON_BrepTrim::Brep() const { return m_brep; }
...
ON_BOOL32 ON_BrepTrim::IsValid(ON_TextLog *text_log) const {
    ...
    if (i == ON_BrepTrim::slit) {
        if (text_log) {
            text_log->Print("trim.m_type = ON_BrepTrim::slit is not valid. "
...
ON_OBJECT_IMPLEMENT(ON_BrepLoop, ON_Geometry,
...
ON_BrepLoop::ON_BrepLoop(int loop_index)
    : m_loop_index(loop_index), m_type(ON_BrepLoop::unknown), m_fi(-1),
      m_brep(0) {
    memset(&m_loop_user, 0, sizeof(m_loop_user));
}

ON_Brep *ON_BrepLoop::Brep() const { return m_brep; }
...
int ON_BrepLoop::TrimCount() const { return m_ti.Count(); }
...
ON_OBJECT_IMPLEMENT(ON_BrepFace, ON_SurfaceProxy,
                    "60B5DBC4-E660-11d3-BFE4-0010830122F0");

ON_BrepFace::ON_BrepFace()
    : ON_SurfaceProxy(0), m_face_index(-1), m_si(-1), m_bRev(false),
      m_face_material_channel(0), m_render_mesh(0), m_analysis_mesh(0),
      m_preview_mesh(0), m_brep(0) {
    m_face_uuid = ON_nil_uuid;
    memset(&m_face_user, 0, sizeof(m_face_user));
}

ON_BrepFace::ON_BrepFace(int face_index)
    : ON_SurfaceProxy(0), m_face_index(face_index), m_si(-1), m_bRev(false),
      m_face_material_channel(0), m_render_mesh(0), m_analysis_mesh(0),
      m_preview_mesh(0), m_brep(0) {
    m_face_uuid = ON_nil_uuid;
    memset(&m_face_user, 0, sizeof(m_face_user));
...
ON_Brep *ON_BrepFace::Brep() const { return m_brep; }

ON_BrepLoop *ON_BrepFace::Loop(int lti) const {
    return (m_brep && lti >= 0 && lti < m_li.Count()) ? m_brep->Loop(m_li[lti])
                                                      : 0;
}

int ON_BrepFace::LoopCount() const { return m_li.Count(); }
...
ON_OBJECT_IMPLEMENT(ON_Brep, ON_Geometry,
...
ON__UINT32 ON_BrepVertex::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder =
        ON_CRC32(current_remainder, sizeof(m_tolerance), &m_tolerance);
...
ON__UINT32 ON_BrepEdge::DataCRC(ON__UINT32 current_remainder) const {
    ...
    current_remainder =
        ON_CRC32(current_remainder, 2 * sizeof(m_vi[0]), &m_vi[0]);
...
int ON_Brep::GetMesh(ON::mesh_type mt,
...
int ON_Brep::Dimension() const { return (m_V.Count() > 0) ? 3 : 0; }

static void ON_BrepTransformSwapSrfHelper(ON_Brep &brep,
                                          ON_NurbsSurface *nurbs_srf, int si) {
...
ON_BOOL32 ON_Brep::Transform(const ON_Xform &xform) {
    ...
    for (i = 0; i < count; i++) {
        ...
        //     normal.
        bool bEvMesh =
            (fabs(det) <= ON_SQRT_EPSILON || xform[3][0] != 0.0 ||
        ...
        if (0 != face.m_analysis_mesh) {
            ...
            if (!bEvAnalysisMesh ||
                !face.m_analysis_mesh->EvaluateMeshGeometry(*srf))
...
int ON_Brep::AddTrimCurve(ON_Curve *pC) {
    ...
    if (0 != pC) {
        ...
        if (dim != 2) {
            ON_ERROR("ON_Brep::AddTrimCurve() go a non-2d curve - changing dim "
                     "to 2.");
...
int ON_Brep::AddEdgeCurve(ON_Curve *pC) {
    ...
    if (0 != pC) {
        ...
        if (dim != 3) {
            ...
            ON_ERROR("ON_Brep::AddEdgeCurve() got a non-3d curve - changing "
                     "dim to 3.");
...
ON_BrepEdge &ON_Brep::NewEdge(int c3i) {
    int ei = m_E.Count();
...
ON_BrepEdge &ON_Brep::NewEdge(ON_BrepVertex &v0, ON_BrepVertex &v1, int c3i,
...
bool ON_Brep::SetEdgeCurve(ON_BrepEdge &edge, int c3_index,
    ...
    if (c3_index == -1 && !sub_domain) {
        ...
        if (!sub_domain || (sub_domain->IsIncreasing() &&
                            curve_domain.Includes(*sub_domain))) {
            edge.m_c3i = c3_index;
...
bool ON_Brep::SetTrimCurve(ON_BrepTrim &trim, int c2_index,
                           const ON_Interval *sub_domain) {
    bool rc = false;
...
ON_BrepTrim &ON_Brep::NewTrim(int c2i) {
    m_is_solid = 0;
    int ti = m_T.Count();
...
ON_BrepTrim &ON_Brep::NewTrim(ON_BrepEdge &edge, ON_BOOL32 bRev3d, int c2i) {
    m_is_solid = 0;
...
ON_BrepTrim &ON_Brep::NewTrim(ON_BrepEdge &edge, ON_BOOL32 bRev3d,
                              ON_BrepLoop &loop, int c2i) {
    m_is_solid = 0;
...
ON_BrepTrim &ON_Brep::NewTrim(ON_BOOL32 bRev3d, ON_BrepLoop &loop, int c2i) {
    m_is_solid = 0;
    ON_BrepTrim &trim = NewTrim(c2i);
    trim.m_bRev3d = bRev3d ? true : false;
    trim.m_li = loop.m_loop_index;
...
ON_BrepTrim &ON_Brep::NewSingularTrim(const ON_BrepVertex &vertex,
                                      ON_BrepLoop &loop, ON_Surface::ISO iso,
                                      int c2i) {
...
ON_BrepLoop &ON_Brep::NewLoop(ON_BrepLoop::TYPE looptype, ON_BrepFace &face) {
    m_is_solid = 0;
...
bool ON_Brep::SetTrimIsoFlags(ON_BrepTrim &trim) {
    ...
    if (trim.m_li >= 0 && trim.m_li < m_L.Count()) {
        ...
        if (fi >= 0 && fi < m_F.Count()) {
            ...
            if (pS) {
                const ON_Curve *pC =
                    (trim.m_c2i >= 0 && trim.m_c2i < m_C2.Count())
                        ? m_C2[trim.m_c2i]
                        : 0;
                if (pC) {
...
bool ON_Brep::SetTrimTypeFlags(ON_BOOL32 bLazy) {
    bool rc = true;
    int fi;
    const int fcnt = m_F.Count();
    for (fi = 0; fi < fcnt; fi++) {
...
bool ON_Brep::SetTrimTypeFlags(ON_BrepFace &face, ON_BOOL32 bLazy) {
    bool rc = true;
    int fli;
    const int face_loop_count = face.m_li.Count();
    for (fli = 0; fli < face_loop_count; fli++) {
...
bool ON_Brep::SetTrimTypeFlags(ON_BrepLoop &loop, ON_BOOL32 bLazy) {
    bool rc = true;
    int lti;
    const int loop_trim_count = loop.m_ti.Count();
    for (lti = 0; lti < loop_trim_count; lti++) {
...
ON_BrepTrim::TYPE ON_Brep::TrimType(const ON_BrepTrim &trim,
                                    ON_BOOL32 bLazy) const {
    ON_BrepTrim::TYPE trim_type = bLazy ? trim.m_type : ON_BrepTrim::unknown;
    int eti, other_ti;

    if (trim_type == ON_BrepTrim::unknown && trim.m_li >= 0 &&
        trim.m_li < m_L.Count()) {
...
bool ON_Brep::SetTrimTypeFlags(ON_BrepTrim &trim, ON_BOOL32 bLazy) {
    if (!bLazy || trim.m_type == ON_BrepTrim::unknown)
        trim.m_type = TrimType(trim, false);
    return ((trim.m_type != ON_BrepTrim::unknown) ? true : false);
}

bool ON_Brep::GetTrim2dStart(int trim_index, ON_2dPoint &P) const
...
bool ON_Brep::GetTrim2dEnd(int trim_index, ON_2dPoint &P) const
...
bool ON_Brep::GetTrim3dStart(int trim_index, ON_3dPoint &P) const {
...
bool ON_Brep::GetTrim3dEnd(int trim_index, ON_3dPoint &P) const
...
bool ON_Brep::IsValidTrim(int trim_index, ON_TextLog *text_log) const {
    if (trim_index < 0 || trim_index >= m_T.Count()) {
        if (text_log) {
            text_log->Print("brep trim_index = %d (should be >=0 and "
                            "<%d=brep.m_T.Count()).\n",
    ...
    if (c2_dim != 2) {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_c2i = %d and ON_Brep.m_C2[%d]->Dimension() "
                            "= %d (should be 2).\n",
    ...
    if (!c2_domain.Includes(trim_domain)) {
        if (text_log) {
            ...
            text_log->Print("trim.Domain() = (%g,%g) is not included in "
                            ...
                            trim_domain[0], trim_domain[1], trim.m_c2i,
                            c2_domain[0], c2_domain[1]);
            text_log->PopIndent();
        }
        return false;
    }
    ...
    if (vi0 < 0 || vi0 >= m_V.Count()) {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_vi[0] = %d (should be >= 0 and < "
                            "%d=brep.m_V.Count()).\n",
    ...
    if (vi1 < 0 || vi1 >= m_V.Count()) {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_vi[1] = %d (should be >= 0 and < "
                            "%d=brep.m_V.Count()).\n",
    ...
    if (trim.m_type == ON_BrepTrim::singular) {
        ...
        if (ei != -1) {
            if (text_log) {
                text_log->Print("brep.m_T[%d] trim is not valid.\n",
                                trim_index);
                text_log->PushIndent();
                text_log->Print("trim.m_type = singular but trim.m_ei = %d "
                                "(should be -1)\n",
                                ei);
                text_log->PopIndent();
            }
        ...
        if (vi0 != vi1) {
            if (text_log) {
                ...
                text_log->PushIndent();
                text_log->Print(
                    "trim.m_type = singular but trim.m_vi[] = [%d,%d] (the "
                    "m_vi[] values should be equal).\n",
                    vi0, vi1);
                text_log->PopIndent();
            }
            return false;
        ...
        if (ei < 0 || ei >= m_E.Count()) {
            if (text_log) {
                ...
                text_log->PushIndent();
                text_log->Print(
                    "trim.m_type != singular and trim.m_ei = %d (m_ei should "
                    "be >=0 and <brep.m_E.Count()=%d\n",
                    trim.m_ei, m_E.Count());
                text_log->PopIndent();
            }
            return false;
        ...
        if (edge.m_vi[trim.m_bRev3d ? 1 : 0] != vi0) {
            if (text_log) {
                ...
                text_log->Print(
                    "trim.m_vi[0] != "
                    "brep.m_E[trim.m_ei=%d].m_vi[trim.m_bRev3d?1:0]\n",
                    ei);
                text_log->PopIndent();
            }
            return false;
        ...
        if (edge.m_vi[trim.m_bRev3d ? 0 : 1] != vi1) {
            if (text_log) {
                ...
                text_log->Print(
                    "trim.m_vi[1] != "
                    "brep.m_E[trim.m_ei=%d].m_vi[trim.m_bRev3d?0:1]\n",
                    ei);
                text_log->PopIndent();
            }
            return false;
        ...
        if (trim_domain == c2_domain && pC->IsClosed()) {
            ...
            if (vi0 != vi1) {
                if (text_log) {
                    ...
                    text_log->Print(
                        "trim.m_vi[] = [%d,%d] but "
                        "brep.m_C2[trim.m_c2i=%d]->IsClosed()=true\n",
                        vi0, vi1, trim.m_c2i);
                    text_log->PopIndent();
                }
                return false;
            ...
            // vi0 != vi1
            // TODO: check that trim start/end is not a closed surface seam
            // point.
        }
        int i;
                ...
                if (other_trim.m_li == trim.m_li) {
                    if (trim.m_type != ON_BrepTrim::seam) {
                        if (text_log) {
                            ...
                            text_log->PushIndent();
                            text_log->Print(
                                "trim.m_type!=seam but brep.m_E[trim.m_ei=%d] "
                                "references two trims in loop trim.m_li=%d.\n",
                                trim.m_ei, trim.m_li);
                            text_log->PopIndent();
                        }
                        return false;
    ...
    case ON_BrepTrim::unknown: {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_type = unknown (should be set to the "
                            "correct ON_BrepTrim::TYPE value)\n");
            text_log->PopIndent();
        }
        return false;
    } break;
    case ON_BrepTrim::boundary: {
        ...
        if (edge.m_ti.Count() > 1) {
            if (text_log) {
                text_log->Print("brep.m_T[%d] trim is not valid.\n",
                                trim_index);
                text_log->PushIndent();
                text_log->Print("trim.m_type = boundary but "
                                "brep.m_E[trim.m_ei=%d] has 2 or more trims.\n",
                                trim.m_ei);
                text_log->PopIndent();
            }
            return false;
        ...
        if (loop.m_type != ON_BrepLoop::outer &&
            ...
            if (text_log) {
                ...
                text_log->PushIndent();
                text_log->Print(
                    "trim.m_type = boundary but brep.m_L[trim.m_li=%d].m_type "
                    "is not inner or outer.\n",
                    trim.m_li);
                text_log->PopIndent();
            }
            return false;
    ...
    case ON_BrepTrim::mated: {
        ...
        if (edge.m_ti.Count() < 2) {
            if (text_log) {
                ...
                text_log->PushIndent();
                text_log->Print(
                    "trim.m_type = mated but brep.m_E[trim.m_ei=%d] only "
        ...
        if (loop.m_type != ON_BrepLoop::outer &&
            ...
            if (text_log) {
                ...
                text_log->PushIndent();
                text_log->Print(
                    "trim.m_type = mated but brep.m_L[trim.m_li=%d].m_type is "
                    "not inner or outer.\n",
                    trim.m_li);
                text_log->PopIndent();
            }
    ...
    case ON_BrepTrim::seam: {
            ...
            if (other_trim.m_type == ON_BrepTrim::seam &&
                ...
                if (other_ti < 0)
                    ...
                    if (text_log) {
                        ...
                        text_log->PushIndent();
                        text_log->Print("All three trims have m_type = seam "
                                        "m_ei=%d and m_li = %d.\n",
                                        trim.m_ei, trim.m_li);
                        text_log->PopIndent();
                    }
        ...
        if (other_ti < 0) {
            if (text_log) {
                text_log->Print("brep.m_T[%d] trim is not valid.\n",
                                trim_index);
                text_log->PushIndent();
                text_log->Print("trim.m_type = seam but its other trim is not "
                                "in the loop.\n");
                text_log->PopIndent();
            }
            return false;
        ...
        // 31 Jan 2002 - The definition of a seam trim is a trim that is
        // connected to
        //               an edge, is part of loop, and exactly one other trim in
        //               the same loop is connected to the same edge.  This can
        //               happen on the interior of a surface (like an annulus in
        //               a plane) and on non-manifold edges.
        // if ( trim.m_iso != ON_Surface::W_iso && trim.m_iso !=
        // ON_Surface::N_iso &&
        //     trim.m_iso != ON_Surface::E_iso && trim.m_iso !=
        //     ON_Surface::S_iso )
        //{
        //  if ( text_log )
        //  {
        //    text_log->Print("brep.m_T[%d] trim is not valid.\n",trim_index);
        //    text_log->PushIndent();
        //    text_log->Print("trim.m_type = seam but trim.m_iso !=
        //    N/S/E/W_iso\n"); text_log->PopIndent();
        //  }
        //  return false;
        //}
    ...
    case ON_BrepTrim::ptonsrf: {
        ...
        if (trim.m_pbox.m_min.x != trim.m_pbox.m_max.x ||
            ...
            // m_pbox must be a single point that defines surface parameters of
            // the point.
            return false;
        }
        if (trim.m_pbox.m_min.x == ON_UNSET_VALUE ||
            trim.m_pbox.m_min.y == ON_UNSET_VALUE ||
            trim.m_pbox.m_min.z != 0.0) {
            // m_pbox must be a single point that defines surface parameters of
            // the point.
            return false;
    ...
    case ON_BrepTrim::slit: {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_type = ON_BrepTrim::slit (should be set to "
                            "the correct ON_BrepTrim::TYPE value)\n");
            text_log->PopIndent();
        }
        return false;
    } break;
    ...
    case ON_BrepTrim::trim_type_count: {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_type = type_count (should be set to the "
                            "correct ON_BrepTrim::TYPE value)\n");
            text_log->PopIndent();
        }
        return false;
    ...
    default: {
        if (text_log) {
            text_log->Print("brep.m_T[%d] trim is not valid.\n", trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_type = garbage (should be set to the "
                            "correct ON_BrepTrim::TYPE value)\n");
            text_log->PopIndent();
        }
        return false;
...
bool ON_Brep::IsValidLoop(int loop_index, ON_TextLog *text_log) const {
    if (loop_index < 0 || loop_index >= m_L.Count()) {
        if (text_log) {
            text_log->Print("brep loop_index = %d (should be >=0 and "
                            "<%d=brep.m_L.Count()).\n",
    ...
    if (loop.m_type != ON_BrepLoop::outer &&
        loop.m_type != ON_BrepLoop::inner && loop.m_type != ON_BrepLoop::slit) {
        if (text_log) {
            text_log->Print("brep.m_L[%d] loop is not valid.\n", loop_index);
            text_log->PushIndent();
            text_log->Print(
            ...
            if (loop.m_ti[i] == ti) {
                if (text_log) {
                    text_log->Print("brep.m_L[%d] loop is not valid.\n",
                                    loop_index);
                    text_log->PushIndent();
                    text_log->Print("loop.m_ti[%d] = loop.m_ti[%d] = %d (trim "
                                    "index can only appear once)\n",
                                    lti, i, ti);
                    text_log->PopIndent();
                }
                return false;
            ...
            if (trim.m_type != ON_BrepTrim::seam) {
                if (text_log) {
                    text_log->Print("brep.m_L[%d] slit loop is not valid.\n",
                                    loop_index);
                    text_log->PushIndent();
                    text_log->Print("brep.m_T[loop.m_ti[%d]=%d].m_type = %d "
                                    "(should be %d = seam)\n",
            ...
            case ON_Surface::N_iso: {
                if (text_log) {
                    text_log->Print("brep.m_L[%d] slit loop is not valid.\n",
                                    loop_index);
                    text_log->PushIndent();
                    text_log->Print("brep.m_T[loop.m_ti[%d]=%d].m_iso = "
                                    "E/W/N/S_iso (should be interior)\n",
                                    lti, ti);
                    text_log->PopIndent();
                }
                return false;
            ...
            if (dx > xtol || dy > ytol) {
                if (text_log) {
                    text_log->Print("brep.m_L[%d] loop is not valid.\n",
                                    loop_index);
                    text_log->PushIndent();
                    text_log->Print("end of brep.m_T[loop.m_ti[%d]=%d]=(%g,%g) "
                                    "and start \n",
...
bool ON_Brep::IsValidFace(int face_index, ON_TextLog *text_log) const {
    if (face_index < 0 || face_index >= m_F.Count()) {
        if (text_log) {
            text_log->Print("brep face_index = %d (should be >=0 and "
                            "<%d=brep.m_F.Count()).\n",
        ...
        for (i = 0; i < fli; i++) {
            if (face.m_li[i] == li) {
                if (text_log) {
                    ...
                    text_log->PushIndent();
                    text_log->Print(
                        "face.m_li[%d]=face.m_li[%d]=%d (a loop index should "
                        "appear once in face.m_li[])\n",
                        fli, i, li);
                    text_log->PopIndent();
                }
                return false;
        ...
        if (loop.m_fi != face_index) {
            if (text_log) {
                text_log->Print("brep.m_F[%d] face is not valid.\n",
                                face_index);
                text_log->PushIndent();
                text_log->Print("face.m_li[%d]=%d but brep.m_L[%d].m_fi=%d "
                                "(m_fi should be %d)\n",
            ...
            if (loop.m_type != ON_BrepLoop::slit &&
                ...
                if (text_log) {
                    text_log->Print("brep.m_F[%d] face is not valid.\n",
                                    face_index);
                    text_log->PushIndent();
                    text_log->Print("brep.m_L[face.m_li[%d]=%d].m_type is not "
                                    "inner or slit.\n",
                                    fli, li);
                    text_log->PopIndent();
                }
...
bool ON_Brep::IsValidEdge(int edge_index, ON_TextLog *text_log) const {
    if (edge_index < 0 || edge_index >= m_E.Count()) {
        if (text_log)
            text_log->Print("brep edge_index = %d (should be >=0 and "
                            "<%d=brep.m_E.Count() ).\n",
        ...
        if (!bFoundIt) {
            if (text_log) {
                text_log->Print("brep.m_E[%d] edge is not valid.\n",
                                edge_index);
                text_log->PushIndent();
                text_log->Print("edge.m_vi[%d]=%d but edge is not referenced "
                                "in m_V[%d].m_ei[]\n",
        ...
        if (vi0 == vi1) {
            if (text_log) {
                text_log->Print("brep.m_E[%d] edge is not valid.\n",
                                edge_index);
                text_log->PushIndent();
                text_log->Print("edge.m_vi[0]=edge.m_vi[1]=%d but "
                                "edge.IsClosed() is false.\n",
                                vi0);
                text_log->PopIndent();
            }
            return false;
            ...
            if (edge.m_ti[i] == ti) {
                if (text_log) {
                    text_log->Print("brep.m_E[%d] edge is not valid.\n",
                                    edge_index);
                    text_log->PushIndent();
                    text_log->Print("edge.m_ti[%d]=edge.m_ti[%d]=%d (a trim "
                                    "should be referenced once).\n",
                                    i, eti, ti);
                    text_log->PopIndent();
                }
                return false;
...
bool ON_Brep::IsValidVertex(int vertex_index, ON_TextLog *text_log) const {
    if (vertex_index < 0 || vertex_index >= m_V.Count()) {
        if (text_log)
            text_log->Print("brep vertex_index = %d (should be >=0 and "
                            "<%d=brep.m_V.Count() ).\n",
    ...
    for (vei = 0; vei < vertex_edge_count; vei++) {
        ...
        for (i = 0; i < vei; i++) {
            if (vertex.m_ei[i] == ei) {
                ...
                if (edge.m_vi[0] != vertex_index ||
                    edge.m_vi[1] != vertex_index) {
                    if (text_log) {
                        text_log->Print("brep.m_V[%d] vertex is not valid.\n",
                                        vertex_index);
                        text_log->PushIndent();
                        text_log->Print("vertex.m_ei[%d] and vertex.m_ei[%d] = "
                                        "%d but brep.m_E[%d].m_vi[0] = %d",
                                        i, vei, ei, ei, edge.m_vi[0]);
                        text_log->Print("and ON_Brep.m_E[%d].m_vi[1] = %d "
                                        "(both m_vi[] values should be %d).\n",
                ...
                for (j = i + 1; j < vei; j++) {
                    if (vertex.m_ei[j] == ei) {
                        if (text_log) {
                            ...
                            text_log->PushIndent();
                            text_log->Print(
                                "vertex.m_ei[%d,%d,%d] = %d. An open edge "
                                "index should appear once\n",
                                i, vei, j, ei);
                            text_log->Print(
                                "in vertex.m_ei[] and a closed edge index "
                                "should appear twice.\n");
                            text_log->PopIndent();
                        }
                        return false;
        ...
        if (edge.m_vi[0] != vertex_index && edge.m_vi[1] != vertex_index) {
            if (text_log) {
                ...
                text_log->PushIndent();
                text_log->Print(
                    "vertex.m_ei[%d] = %d but ON_Brep.m_E[%d].m_vi[] = "
                    "[%d,%d]. "
...
static bool CheckLoopOnSrfHelper(const ON_Brep &brep,
    ...
    for (int lti = 0; lti < loop.m_ti.Count(); lti++) {
        ...
        if (ti < 0 || ti >= brep.m_T.Count())
            continue;
        if (!CheckTrimOnSrfHelper(srf_domain0, srf_domain1, brep.m_T[ti],
                                  text_log))
            return false;
    }
    return true;
...
ON_Brep::IsValid(ON_TextLog *text_log) const {
    ...
    for (vi = 0; vi < vertex_count; vi++) {
        if (m_V[vi].m_vertex_index == -1) {
            ...
            if (vertex.m_ei.Count() > 0) {
                if (text_log)
                    text_log->Print(
                        "ON_Brep.m_V[%d] is deleted (m_vertex_index = -1) but "
                        "vertex.m_ei.Count() = %d.\n",
    ...
    for (ei = 0; ei < edge_count; ei++) {
        if (m_E[ei].m_edge_index == -1) {
            ...
            if (edge.m_ti.Count() > 0) {
                if (text_log)
                    text_log->Print("ON_Brep.m_E[%d] is deleted (m_edge_index "
                                    "= -1) but edge.m_ei.Count() = %d.\n",
                                    ei, edge.m_ti.Count());
                return ON_BrepIsNotValid();
            }
            if (edge.m_c3i != -1) {
                if (text_log)
                    text_log->Print("ON_Brep.m_E[%d] is deleted (m_edge_index "
                                    "= -1) but edge.m_c3i=%d (should be -1).\n",
                                    ei, edge.m_c3i);
                return ON_BrepIsNotValid();
            }
            if (edge.ProxyCurve()) {
                if (text_log)
                    text_log->Print("ON_Brep.m_E[%d] is deleted (m_edge_index "
                                    "= -1) but edge.m_curve is not NULL.\n",
                                    ei);
                return ON_BrepIsNotValid();
            }
            if (edge.m_vi[0] != -1) {
                if (text_log)
                    text_log->Print(
                        "ON_Brep.m_E[%d] is deleted (m_edge_index = -1) but "
    ...
    for (ti = 0; ti < trim_count; ti++) {
        if (m_T[ti].m_trim_index == -1) {
            ...
            if (trim.m_ei != -1) {
                if (text_log)
                    text_log->Print("ON_Brep.m_T[%d] is deleted (m_trim_index "
                                    "= -1) but trim.m_ei=%d (should be -1).\n",
                                    ti, trim.m_ei);
                return ON_BrepIsNotValid();
            }
            if (trim.m_li != -1) {
            ...
            if (trim.m_c2i != -1) {
                if (text_log)
                    text_log->Print("ON_Brep.m_T[%d] is deleted (m_trim_index "
                                    "= -1) but trim.m_c2i=%d (should be -1).\n",
                                    ti, trim.m_c2i);
                return ON_BrepIsNotValid();
            }
            if (trim.m_vi[0] != -1) {
                if (text_log)
                    text_log->Print(
                        "ON_Brep.m_T[%d] is deleted (m_trim_index = -1) but "
    ...
    for (li = 0; li < loop_count; li++) {
        if (m_L[li].m_loop_index == -1) {
            ...
            if (loop.m_fi != -1) {
                if (text_log)
                    text_log->Print("ON_Brep.m_L[%d] is deleted (m_loop_index "
                                    "= -1) but loop.m_fi=%d (should be -1).\n",
                                    li, loop.m_fi);
                return ON_BrepIsNotValid();
            }
    ...
    for (fi = 0; fi < face_count; fi++) {
        if (m_F[fi].m_face_index == -1) {
            ...
            if (face.m_si != -1) {
                if (text_log)
                    text_log->Print("ON_Brep.m_F[%d] is deleted (m_face_index "
                                    "= -1) but face.m_si=%d (should be -1).\n",
                                    fi, face.m_si);
                return ON_BrepIsNotValid();
            }
            if (face.ProxySurface()) {
                if (text_log)
                    text_log->Print(
                        "ON_Brep.m_F[%d] is deleted (m_face_index = -1) but "
    ...
    for (ti = 0; ti < trim_count; ti++) {
        ...
        if (trim.m_type == ON_BrepTrim::singular) {
            if (trim.m_ei != -1) {
                if (text_log)
                    text_log->Print("ON_Brep.m_T[%d].m_type = singular, but "
                                    "m_ei = %d (should be -1).\n",
                                    ti, trim.m_ei);
                return ON_BrepIsNotValid();
            }
        ...
        if (trim.m_vi[0] != edge.m_vi[evi0]) {
            if (text_log)
                text_log->Print("ON_Brep.m_T[%d].m_bRev3d = %d, but m_vi[0] != "
                                "m_E[m_ei].m_vi[%d].\n",
        ...
        // check tolerances and closed curve directions
        {
            ...
            ON_3dVector trim_der0, trim_der1, srf_du0, srf_dv0, srf_du1,
                srf_dv1;
            ON_Interval trim_domain = trim.Domain();
            ...
            ON_3dVector trim_tangent0 =
                trim_der0.x * srf_du0 + trim_der0.y * srf_dv0;
            trim_tangent0.Unitize();
            ON_3dVector trim_tangent1 =
                trim_der1.x * srf_du1 + trim_der1.y * srf_dv1;
            trim_tangent1.Unitize();
            ON_3dVector edge_tangent0 =
                edge.TangentAt(edge.Domain()[trim.m_bRev3d ? 1 : 0]);
            ON_3dVector edge_tangent1 =
                edge.TangentAt(edge.Domain()[trim.m_bRev3d ? 0 : 1]);
            ...
            if (trim.m_vi[0] == trim.m_vi[1] && edge.m_vi[0] == edge.m_vi[1] &&
                ...
                if (d0 < 0.0 || d1 < 0.0) {
                    if (text_log) {
                        if (trim.m_bRev3d)
                            text_log->Print(
                                "ON_Brep.m_T[%d].m_bRev3d = true, but closed "
                                "curve directions are the same.\n",
                                ti);
                        else
                            text_log->Print(
                                "ON_Brep.m_T[%d].m_bRev3d = false, but closed "
                                "curve directions are opposite.\n",
                                ti);
                    }
                    return ON_BrepIsNotValid();
                }
            ...
            if (d0 > dtol) {
                if (text_log) {
                    text_log->Print(
                        "Distance from start of ON_Brep.m_T[%d] to 3d edge is "
                        "%g.  (edge tol = %g, trim tol ~ %g).\n",
                        ti, d0, etol, t0_tol);
                }
                return ON_BrepIsNotValid();
            }
            if (d1 > dtol) {
                if (text_log) {
                    text_log->Print(
                        "Distance from end of ON_Brep.m_T[%d] to 3d edge is "
                        "%g.  (edge tol = %g, trim tol ~ %g).\n",
                        ti, d1, etol, t1_tol);
                }
                return ON_BrepIsNotValid();
        ...
        // check trim's m_pbox
        {
            if (trim.m_pbox.m_min.z != 0.0) {
                if (text_log)
                    text_log->Print("ON_Brep.m_T[%d].m_pbox.m_min.z = %g "
                                    "(should be zero).\n",
            ...
            if (trim.m_pbox.m_max.z != 0.0) {
                if (text_log)
                    text_log->Print("ON_Brep.m_T[%d].m_pbox.m_max.z = %g "
                                    "(should be zero).\n",
        ...
        if (ON_BrepTrim::seam == trim.m_type) {
            ...
            default:
                if (text_log)
                    text_log->Print(
                        "ON_Brep.m_T[%d].m_type = ON_BrepTrim::seam but m_iso "
                        "is not N/E/W/S_iso.\n",
                        ti);
                return ON_BrepIsNotValid();
            }
    ...
    for (li = 0; li < loop_count; li++) {
        ...
        for (lti = 0; lti < loop.m_ti.Count(); lti++) {
            ...
            if (!loop.m_pbox.IsPointIn(trim.m_pbox.m_min) ||
                ...
                if (text_log)
                    text_log->Print("ON_Brep.m_L[%d].m_pbox does not contain "
                                    "m_T[loop.m_ti[%d]].m_pbox.\n",
                                    li, lti);
                return ON_BrepIsNotValid();
            }
            if (0 == lti) {
                ...
                if (text_log)
                    text_log->Print(
                        ...
                        li, lti - 1, prev_trim_ti, prev_trim_vi1, lti,
                        loop.m_ti[lti], trim.m_vi[0]);
                return ON_BrepIsNotValid();
            }
        ...
        if (first_trim_ti >= 0 && first_trim_vi0 != prev_trim_vi1) {
            ...
            //     in a loop shared vertices.
            if (text_log)
                text_log->Print("ON_Brep.m_L[%d] loop has trim vertex "
                                "mismatch:\n  m_T[loop.m_ti[%d]=%d].m_vi[1] = "
                                "%d != m_T[loop.m_ti[%d]=%d].m_vi[0]=%d.\n",
                                li, lti - 1, prev_trim_ti, prev_trim_vi1, 0,
                                first_trim_ti, first_trim_vi0);
            return ON_BrepIsNotValid();
        }
    }
    ...
    // It looks for boundary trims on seams that should be joined as a seam
    // trim.
    ON_Interval srf_domain[2];
    for (fi = 0; fi < face_count; fi++) {
        const ON_BrepFace &face = m_F[fi];
        ...
        for (int fli = 0; fli < face.m_li.Count(); fli++) {
            ...
            if (li < 0 || li >= m_L.Count())
                continue;
            if (!CheckLoopOnSrfHelper(*this, srf_domain[0], srf_domain[1],
                                      m_L[li], text_log))
                return ON_BrepIsNotValid();
        }
        ...
        for (lti = 0; lti < outer_trim_count; lti++) {
            ...
            if (ON_Surface::N_iso == iso_type ||
                ON_Surface::W_iso == iso_type) {
            ...
            for (lti1 = 0; lti1 < outer_trim_count; lti1++) {
                ...
                s1 = side_interval.NormalizedParameterAt(
                    trim1.PointAtStart()[endpt_index]);
                if (fabs(s1 - 1.0) > side_tol)
                    continue;
                s0 = side_interval.NormalizedParameterAt(
                    trim1.PointAtEnd()[endpt_index]);
                if (fabs(s0) > side_tol)
                    continue;

                if (text_log) {
                    text_log->Print(
                        "ON_Brep.m_F[%d] is on a closed surface. Outer loop "
                        "m_L[%d] contains boundary trims %d and %d.  They "
                        "should be seam trims connected to the same edge.\n",
    ...
    for (ti = 0; seam_trim_count > 0 && ti < trim_count; ti++) {
        ...
        for (int eti = 0; eti < edge.m_ti.Count(); eti++) {
            ...
            text_log->Print("ON_Brep.m_T[%d,%d,%d] are three seam trims with "
                            "the same edge in the same loop.\n",
                            ti, trim1_index, ti1);
            return ON_BrepIsNotValid();
        }

        if (trim1_index < 0 || trim1_index >= trim_count) {
            text_log->Print("ON_Brep.m_T[%d] is a seam trim with no matching "
                            "seam trim in the same loop.\n",
                            ti);
            return ON_BrepIsNotValid();
        }
        ...
        case ON_Surface::S_iso:
            if (ON_Surface::N_iso != m_T[trim1_index].m_iso) {
                if (text_log)
                    text_log->Print(
                        "Seam trim ON_Brep.m_T[%d].m_iso = S_iso but matching "
                        "seam ON_Brep.m_T[%d].m_iso != N_iso.\n",
                        ti, trim1_index);
                return ON_BrepIsNotValid();
            }
            break;
        ...
        case ON_Surface::E_iso:
            if (ON_Surface::W_iso != m_T[trim1_index].m_iso) {
                if (text_log)
                    text_log->Print(
                        "Seam trim ON_Brep.m_T[%d].m_iso = E_iso but matching "
                        "seam ON_Brep.m_T[%d].m_iso != W_iso.\n",
                        ti, trim1_index);
                return ON_BrepIsNotValid();
            }
            break;
        ...
        case ON_Surface::N_iso:
            if (ON_Surface::S_iso != m_T[trim1_index].m_iso) {
                if (text_log)
                    text_log->Print(
                        "Seam trim ON_Brep.m_T[%d].m_iso = N_iso but matching "
                        "seam ON_Brep.m_T[%d].m_iso != S_iso.\n",
                        ti, trim1_index);
                return ON_BrepIsNotValid();
            }
            break;
        ...
        case ON_Surface::W_iso:
            if (ON_Surface::E_iso != m_T[trim1_index].m_iso) {
                if (text_log)
                    text_log->Print(
                        "Seam trim ON_Brep.m_T[%d].m_iso = W_iso but matching "
                        "seam ON_Brep.m_T[%d].m_iso != E_iso.\n",
                        ti, trim1_index);
                return ON_BrepIsNotValid();
            }
            break;
...
bool ON_Brep::SetTrimStartVertex(const int ti0, const int vi) {
    ...
    for (ti = ti0, tvi = 0, counter = 0; ti >= 0 && counter < 512; counter++) {
        ...
        if (trim.m_type == ON_BrepTrim::singular) {
            ...
            if (m_T[ti].m_type != ON_BrepTrim::singular)
                HopAcrossEdge(ti,
                              tvi); // OK if hop fails because ti is a boundary
            continue;
        }
    ...
    for (ti = ti1, tvi = tvi1, counter = 0; ti >= 0 && counter < 512;
         counter++) {
        if (counter > 0) {
            if (ti == ti1 && tvi == tvi1)
                return false; // vertex is not interior - so this should not
                              // happen
        }
        ON_BrepTrim &trim = m_T[ti];
        if (trim.m_type == ON_BrepTrim::singular) {
    ...
    return false; // should have exited by hitting "expected" boundary ~10 lines
                  // above
}
...
int ON_Brep::NextEdge(int ei, int endi, int *next_endi) const {
    ...
    if (edge.m_vi[0] == edge.m_vi[1] && endi) {
        ...
        // appears TWICE in vertex.m_ei[].  The first occurance of
        // edge.edge_index in vertex.m_ei[] corresponds to edge.m_vi[0].  The
        // second occurance of edge.edge_index in vertex.m_ei[] corresponds to
        // edge.m_vi[1].
        vei++;
...
void ON_Brep::SetVertices(void) {
    ...
    for (ei = 0; ei < edge_count; ei++) {
        ...
        if (edge.m_vi[0] >= 0 && edge.m_vi[1] >= 0 &&
            edge.m_vi[0] != edge.m_vi[1] && 0 != edge.EdgeCurveOf() &&
            edge.IsClosed()) {
    ...
    if (edge_ends.Count() > 0) {
        ...
        for (i0 = 0, i1 = 1; i0 < edge_ends.Count(); i0 = i1) {
            vi0 = edge_ends[i0].vi0;
            for (i1 = i0 + 1;
                 i1 < edge_ends.Count() && vi0 == edge_ends[i1].vi0; i1++) {
                // empty body
            }
            vi1 = vi0;
            for (i = i0; i < i1; i++) {
...
void ON_Brep::ClearBoundingBox() { m_bbox.Destroy(); }

void ON_BrepFace::ClearBoundingBox() { m_bbox.Destroy(); }
...
ON_BrepFace::GetBBox(double *box_min,   // [3],
    ...
    if (rc) {
        ...
        if (bGrowBox && box_min && box_max && box_min[0] <= box_max[0]) {
            bbox.Union(
                ON_BoundingBox(ON_3dPoint(box_min), ON_3dPoint(box_max)));
        }
        if (box_min) {
...
bool ON_Brep::SwapTrimParameters(int trim_index) {
    ...
    rc = pC->Reverse();
    if (rc) {
        // take care of proxy house keeping, m_vi[] swapping, and toggle
        // m_bRev3d.
...
bool ON_Brep::IsManifold(ON_BOOL32 *pbIsOriented,
                         ON_BOOL32 *pbHasBoundary) const {
    const int fcnt = m_F.Count();
                    ...
                    if (trim.m_ei >= 0 && trim.m_ei < brep_edge_count) {
                        ...
                        if (edge.m_ti.Count() != 2) {
                            ...
                            if (other_ti == ti) {
                                ...
                                if (0 <= other_trim.m_li &&
                                    brep_loop_count > other_trim.m_li) {
                                    if (m_F[m_L[other_trim.m_li].m_fi].m_bRev)
                                        bFlipOther = !bFlipOther;
                                } else {
                                    ON_ERROR(
                                        "Bogus loop index in other_trim.m_li");
                                    continue;
                                }
...
bool ON_Brep::LoopIsSurfaceBoundary(int loop_index) const {
    // returns true if the loop's trims run along the underlying surface
    // boundary
...
bool ON_Brep::SetTrimDomain(int trim_index, // index of trim in m_T[] array
...
bool ON_Brep::SetEdgeDomain(int edge_index, // index of edge in m_E[] array
                            const ON_Interval &domain) {
    bool rc = false;
...
bool ON_BrepFace::SetDomain(ON_Interval u_dom, ON_Interval v_dom) {
    if (0 == m_brep)
        return false;
        ...
        for (int dir = 0; dir < 2; dir++) {
            ...
            if (mdom.IsIncreasing() && dom0 != dom1) {
                if (mdom == dom0)
                    ...
                    double t0 =
                        dom1.ParameterAt(dom0.NormalizedParameterAt(mdom[0]));
                    double t1 =
                        dom1.ParameterAt(dom0.NormalizedParameterAt(mdom[1]));
                    mdom.Set(t0, t1);
                }
            }
        }
...
ON_BOOL32 ON_BrepFace::SetDomain(int dir, // 0 sets first parameter's domain, 1
                                          // gets second parameter's domain
                                 double t0, double t1) {
    if (dir < 0 || dir > 1 || t0 == ON_UNSET_VALUE || t1 == ON_UNSET_VALUE ||
        t0 >= t1 || 0 == m_brep)
        return false;
...
static int curve_area(ON_3dPoint &start_point, const ON_Curve *curve,
                      const ON_Interval &curve_domain, const ON_Xform *xform,
                      double *area) {
    ...
    *area = 0;
    ...
    if (polycurve) {
        ...
        for (span_i = 0; span_i < span_count; span_i++) {
            ...
            if (curve_domain[0] > span_domain[0] ||
                curve_domain[1] < span_domain[1]) {
                s0 = (curve_domain[0] > span_domain[0]) ? curve_domain[0]
                                                        : span_domain[0];
                s1 = (curve_domain[1] < span_domain[1]) ? curve_domain[1]
                                                        : span_domain[1];
            ...
            if (!curve_area(start_point, segment_curve, segment_curve_domain,
                            xform, &twice_area)) {
                *area = 0.0;
                return false;
            }
...
bool ON_Brep::SortFaceLoops(ON_BrepFace &face) const {
    ...
    for (fli = 0; fli < face_loop_count; fli++) {
        ...
        if (loop_type <= ON_BrepLoop::unknown ||
            loop_type >= ON_BrepLoop::type_count)
            rc = false;
    ...
    for (lti = 0; lti < loop_trim_count; lti++) {
        ...
        if (lti == 0) {
            ...
            if (m_C2[c2i])
                start_point =
                    m_T[ti]
                        .PointAtStart(); // m_C2[c2i]->PointAt(m_T[ti].m_t[0]);
        }
...
bool ON_Brep::SetVertexTolerance(
    ...
    if (vertex.m_tolerance < 0.0 || !bLazySet) {
        ...
        if (vertex_edge_count < 1) {
            ...
            for (vei = 0; vei < vertex_edge_count; vei++) {
                ...
                for (endi = 0; endi < 2; endi++) {
                    if (edge.m_vi[endi] == vertex_index) {
                        d = vertex.point.DistanceTo(
                            edge.PointAt(edge_domain[endi]));
                        if (tolerance < d)
                            tolerance = d;
                    }
                }
                ...
                for (eti = 0; eti < edge_trim_count; eti++) {
                    ...
                    for (endi = 0; endi < 2; endi++) {
                        if (edge.m_vi[endi] == vertex_index) {
                            uv = trim->PointAt(
                                trim->Domain()[trim->m_bRev3d ? 1 - endi
                                                              : endi]);
            ...
            vertex.m_tolerance =
                (tolerance <= ON_ZERO_TOLERANCE) ? 0.0 : 1.001 * tolerance;
        }
    }
    return (vertex.m_tolerance >= 0.0) ? true : false;
}

bool ON_Brep::SetTrimTolerance(ON_BrepTrim &trim, ON_BOOL32 bLazy) const {
        ...
        if (trim.m_li >= 0 && trim.m_li < m_L.Count()) {
            ...
            for (lti = 0; lti < loop_trim_count; lti++) {
                if (loop.m_ti[lti] == trim.m_trim_index) {
                    prev_ti = loop.m_ti[(lti - 1 + loop_trim_count) %
                                        loop_trim_count];
                    ...
                    if (prev_ti >= 0 && next_ti >= 0 && prev_ti < m_T.Count() &&
                        ...
                        if (prev_c2 && c2 && next_c2) {
                            ON_3dPoint prev_end =
                                prev_trim.PointAtEnd(); // prev_c2->PointAt(
                                                        // prev_trim.m_t[1] );
                            ON_3dPoint this_start =
                                trim.PointAtStart(); // c2->PointAt( trim.m_t[0]
                                                     // );
                            ON_3dPoint this_end =
                                trim.PointAtEnd(); // c2->PointAt( trim.m_t[1]
                                                   // );
                            ON_3dPoint next_start =
                                next_trim.PointAtStart(); // prev_c2->PointAt(
                                                          // next_trim.m_t[0] );
                            for (dir = 0; dir < 2; dir++) {
                                if (trim.m_tolerance[dir] < 0.0 || !bLazy) {
                                    ...
                                    d = (ds >= de) ? ds : de;
                                    trim.m_tolerance[dir] =
                                        (d > ON_ZERO_TOLERANCE) ? 1.001 * d
                                                                : 0.0;
                                }
                            }
                        }
    ...
    return (trim.m_tolerance[0] >= 0.0 && trim.m_tolerance[1] >= 0.0) ? true
                                                                      : false;
}

bool ON_Brep::SetEdgeTolerance(ON_BrepEdge &edge, ON_BOOL32 bLazySet) const {
...
void ON_Brep::Dump(ON_TextLog &dump) const {
    ...
    for (c2i = 0; c2i < m_C2.Count(); c2i++) {
        ...
        if (c2) {
            ...
            dump.Print(
                ...
                s, cdom[0], cdom[1], c_start.x, c_start.y, c_end.x, c_end.y);
        } else {
    ...
    for (c3i = 0; c3i < m_C3.Count(); c3i++) {
        ...
        if (c3) {
            ...
            if (!s)
                s = "";
            dump.Print("curve3d[%2d]: %s domain(%g,%g) start(%g,%g,%g) "
                       "end(%g,%g,%g)\n",
                       c3i, s, cdom[0], cdom[1], c_start.x, c_start.y,
                       c_start.z, c_end.x, c_end.y, c_end.z);
        } else {
            dump.Print("curve2d[%2d]: NULL\n", c3i);
        }
    }
    ...
    for (ei = 0; ei < m_E.Count(); ei++) {
        ...
        dump.Print("edge[%2d]: v0(%2d) v1(%2d) 3d_curve(%d) tolerance(%g)\n",
                   ei, edge.m_vi[0], edge.m_vi[1], edge.m_c3i,
                   edge.m_tolerance);
        ...
        if (c3) {
            ...
            dump.Print("domain(%g,%g) start(%g,%g,%g) end(%g,%g,%g)\n",
                       edge.Domain()[0], edge.Domain()[1], edge_start.x,
                       edge_start.y, edge_start.z, edge_end.x, edge_end.y,
                       edge_end.z);
        } else {
    ...
    for (fi = 0; fi < m_F.Count(); fi++) {
        ...
        dump.Print("face[%2d]: surface(%d) reverse(%d) loops(", fi, face.m_si,
                   face.m_bRev);
        int fli;
        ...
        if (face.m_render_mesh) {
            ...
            if (mp) {
                if (0 == mp->CompareGeometrySettings(
                             ON_MeshParameters::FastRenderMesh))
                    mp_style = "Fast";
                else if (0 == mp->CompareGeometrySettings(
        ...
        for (fli = 0; fli < face.m_li.Count(); fli++) {
            ...
            dump.Print("loop[%2d]: type(%s) %d trims(", li, sLoopType,
                       loop.m_ti.Count());
            int lti;
            for (lti = 0; lti < loop.m_ti.Count(); lti++) {
            ...
            for (lti = 0; lti < loop.m_ti.Count(); lti++) {
                ...
                dump.Print(
                    ...
                    ti, trim.m_ei, trim.m_vi[0], trim.m_vi[1],
                    trim.m_tolerance[0], trim.m_tolerance[1]);
                dump.PushIndent();
                dump.Print("type(%s%s) rev3d(%d) 2d_curve(%d)\n", sTrimType,
                           sTrimIso, trim.m_bRev3d, trim.m_c2i);
                if (c2) {
                    trim_start = trim.PointAtStart();
                    trim_end = trim.PointAtEnd();
                    dump.Print("domain(%g,%g) start(%g,%g) end(%g,%g)\n",
                               trim.Domain()[0], trim.Domain()[1], trim_start.x,
                               trim_start.y, trim_end.x, trim_end.y);
                    if (0 != face_srf) {
                        ON_3dPoint trim_srfstart =
                            face_srf->PointAt(trim_start.x, trim_start.y);
                        ON_3dPoint trim_srfend =
                            face_srf->PointAt(trim_end.x, trim_end.y);
                        dump.Print(
...
void ON_Brep::DeleteTrim(ON_BrepTrim &trim, ON_BOOL32 bDeleteTrimEdges) {
    ...
    if (ti >= 0 && ti < m_T.Count()) {
        ...
        if (ei >= 0 && ei < m_E.Count()) {
            ON_BrepEdge &edge = m_E[ei];
            if (bDeleteTrimEdges && edge.m_ti.Count() == 1 &&
                edge.m_ti[0] == ti) {
                edge.m_ti.Empty();
                ...
                for (eti = edge.m_ti.Count() - 1; eti >= 0; eti--) {
                    ...
                    if ((mate_ti >= -1 || seam_ti >= -1) && other_ti >= 0 &&
                        ...
                        if (other_trim.m_trim_index != other_ti)
                            continue;
                        if (mate_ti >= -1 &&
                            other_trim.m_type == ON_BrepTrim::mated) {
                            ...
                            if (mate_ti == -1)
                                ...
                                mate_ti = -2;
                                   ...
                                   other_trim.m_type == ON_BrepTrim::seam &&
                                   other_trim.m_li == trim.m_li) {
                            // trim and other_trim are both seam trims in the
                            // same loop connected to the same edge.
                            if (seam_ti == -1)
                                seam_ti = other_ti;
                            else
                                seam_ti = -2;
                ...
                if (seam_ti >= 0) {
                    ...
                    // Now it is either a boundary trim or is mated to
                    // m_T[mate_ti]
                    m_T[seam_ti].m_type =
                        (mate_ti >= 0)
                            ? ON_BrepTrim::mated     // m_T[mate_ti] is mated to
                                                     // m_T[seam_ti]
                            : ON_BrepTrim::boundary; // m_T[seam_ti] is all
                                                     // alone
                } else if (mate_ti >= 0) {
                    // m_T[mate_ti] just lost its only mate and is now a
                    // boundary
...
int ON_Brep::GetConnectedComponents(ON_SimpleArray<ON_Brep *> &components,
    ...
    if (count > 1) {
        ...
        for (cci = 1; cci <= count; cci++) {
            ...
            if (fi.Count() > 0) {
                ON_Brep *cc = brep.DuplicateFaces(fi.Count(), fi.Array(),
                                                  bDuplicateMeshes);
                if (cc)
...
ON_Brep *ON_Brep::DuplicateFaces(int face_count, const int *face_index,
                                 ON_BOOL32 bDuplicateMeshes) const {
    ...
    for (i = 0; i < m_V.Count() && rc; i++) {
        if (v_remap[i]) {
            ...
            memset(&vertex_copy.m_vertex_user, 0,
                   sizeof(vertex_copy.m_vertex_user));
    ...
    for (i = 0; i < m_E.Count() && rc; i++) {
        if (e_remap[i]) {
            ...
            if (v_remap[edge.m_vi[0]] < 0 || v_remap[edge.m_vi[1]] < 0 ||
                ...
                break;
            ON_BrepEdge &edge_copy = brep_copy->NewEdge(
                brep_copy->m_V[v_remap[edge.m_vi[0]]],
                brep_copy->m_V[v_remap[edge.m_vi[1]]], c3_remap[edge.m_c3i]);
    ...
    for (fi = 0; rc && fi < m_F.Count() && rc && !bFoundBadIdx; fi++) {
        ...
        for (fli = 0; fli < face.m_li.Count() && !bFoundBadIdx; fli++) {
            ...
            for (lti = 0; lti < loop.m_ti.Count() && !bFoundBadIdx; lti++) {
                ...
                if (trim.m_ei >= 0) {
                    i = brep_copy
                            ->NewTrim(brep_copy->m_E[e_remap[trim.m_ei]],
                                      trim.m_bRev3d, loop_copy, i)
                            .m_trim_index;
                } else {
                    i = brep_copy->NewTrim(trim.m_bRev3d, loop_copy, i)
                            .m_trim_index;
                ...
                memset(&trim_copy.m_trim_user, 0,
                       sizeof(trim_copy.m_trim_user));
        ...
        if (tcount > 0) {
            ...
            for (ti = 0; ti < tcount; ti++) {
                ...
                if (vi >= 0 && vi < vcount && -1 == m_V[vi].m_vertex_index) {
                    ...
                    // This error happens when the ON_Brep is invalid to begin
                    // with. However, in order to prevent crashes, we have to
                    // refuse to delete the vertex.  See bugs 55879 and 56191.
                    ON_ERROR("ON_Brep::CullUnusedVertices() - deleted vertex "
                             "referenced by trim.m_vi[0]");
                    m_V[vi].m_vertex_index = vi;
                }
                vi = trim.m_vi[1];
                if (vi >= 0 && vi < vcount && -1 == m_V[vi].m_vertex_index) {
...
bool ON_Brep::CombineCoincidentVertices(ON_BrepVertex &vertex0,
    ...
    if (vertex0.m_vertex_index >= 0 &&
        vertex1.m_vertex_index != vertex0.m_vertex_index) {
        rc = true;
...
ON_BrepEdge *ON_Brep::CombineContiguousEdges(int ei0, int ei1,
...
bool ON_Brep::CombineCoincidentEdges(ON_BrepEdge &edge0, ON_BrepEdge &edge1) {
    ...
    if (edge0.m_edge_index >= 0 && edge1.m_edge_index >= 0 &&
        ...
        edge0.m_vi[0] == edge1.m_vi[0] && edge0.m_vi[1] == edge1.m_vi[1]) {
        ...
        if (edge0.m_tolerance == edge1.m_tolerance &&
            edge0.m_tolerance == 0.0) {
            if (bIsGoodIso1) {
                if (!bIsGoodIso0)
                    bKeep0 = false;
        ...
        for ( eti = 0; eti < etcnt; eti++ ) {
          ...
          if ( ti >= 0 && ti < tcount ) {
            ...
            //set edge tolerance
            if (edge0.m_tolerance != ON_UNSET_VALUE && edge1.m_tolerance !=
        ON_UNSET_VALUE) SetEdgeTolerance(edge0, false); else edge0.m_tolerance =
        ON_UNSET_VALUE;
          }
        }
        edge1.m_ti.Destroy();
        ...
        */
        ...
        for (eti = 0; eti < etcnt; eti++) {
            ...
            if (ti >= 0 && ti < tcount) {
                ...
                // set edge tolerance
                if (EKeep.m_tolerance != ON_UNSET_VALUE &&
...
ON_BOOL32 ON_Brep::HasBrepForm() const { return true; }
...
ON_BrepVertex &ON_Brep::NewPointOnFace(ON_BrepFace &face, double s, double t) {
...
ON_BrepTrim &ON_Brep::NewCurveOnFace(ON_BrepFace &face, ON_BrepEdge &edge,
                                     ON_BOOL32 bRev3d, int c2i) {
...
// and the new ti and di remain the same.  Don't do anything if the chord is
// short relative to the cummulative dist between consecutive points on input.
...
static void AdjustNurbsCurve(ON_NurbsCurve &crv, const ON_3dPoint &P0,
                             const ON_3dPoint &P1)
...
static void AdjustPolylineCurve(ON_PolylineCurve &crv, const ON_3dPoint &P0,
                                const ON_3dPoint &P1)
...
static void AdjustCurve(ON_Curve &crv, const ON_3dPoint &P0,
                        const ON_3dPoint &P1);

static void AdjustPolyCurve(ON_PolyCurve &crv, const ON_3dPoint &P0,
                            const ON_3dPoint &P1)
...
// Afterwards it is up to caller to check to see if the endpoints are where they
// should be.
static void AdjustCurve(ON_Curve &crv, const ON_3dPoint &P0,
                        const ON_3dPoint &P1)

{
    ...
    if (A0.DistanceTo(P0) < ON_SQRT_EPSILON &&
        A1.DistanceTo(P1) < ON_SQRT_EPSILON) {
        crv.SetStartPoint(P0);
        crv.SetEndPoint(P1);
        return;
    }
        ...
        if (c3) {
            ...
            if (edge->ProxyCurveIsReversed())
                ...
                bNewCurve = true; // curve proxy is trimmed
                     ...
                     (EdgeCurveUse < 1 &&
                      EdgeCurveUseCount(edge->m_c3i, 2) > 1))
...
void ON_Brep::StandardizeEdgeCurves(bool bAdjustEnds) {
    ...
    for (ei = 0; ei < edge_count; ei++) {
        int ecc = (ei == edge_count - 1 ||
                   m_E[index[ei + 1]].m_c3i == m_E[index[ei]].m_c3i)
                      ? 2
                      : 1;
...
bool ON_Brep::ShrinkSurface(ON_BrepFace &face, int DisableMask) {
    ...
    if (!bAllTrimsAreIsoTrims) {
        ...
        if ((!bSomeTrimsAreIsoTrims ||
             outer_udom[0] < trim_iso_endbox.m_min.x) &&
            !bIsSrfEdge[0])
            outer_udom[0] -= d;
        if ((!bSomeTrimsAreIsoTrims ||
             outer_udom[1] > trim_iso_endbox.m_max.x) &&
            !bIsSrfEdge[2])
            outer_udom[1] += d;

        d = outer_vdom.Length() * 0.01;
        if ((!bSomeTrimsAreIsoTrims ||
             outer_vdom[0] < trim_iso_endbox.m_min.y) &&
            !bIsSrfEdge[1])
            outer_vdom[0] -= d;
        if ((!bSomeTrimsAreIsoTrims ||
             outer_vdom[1] > trim_iso_endbox.m_max.y) &&
            !bIsSrfEdge[3])
            outer_vdom[1] += d;
    }
    ...
    if (bShrinkIt) {
        ...
        if (small_srf->Trim(0, outer_udom)) {
            ...
            if (si >= 0) {
                ...
                // 5 Dec 2002 Chuck - dont delete original surface if used by
                // more than one face
                if (srf_use == 1)
                    DeleteSurface(srf_index);

                // 1 Nov 2002 Dale Lear - reset face bbox and destroy brep too
                // big bounding box
                ...
                for (int li = 0; li < face.LoopCount(); li++) {
                    ...
                    for (int ti = 0; ti < loop.TrimCount(); ti++) {
                        ...
                        // Since the slop used in calculating m_iso depends on
                        // the srf domain all isos should be rechecked after
                        // shrinking
...
ON_BrepEdge *ON_Brep::Edge(ON_COMPONENT_INDEX edge_index) const {
    ...
    if (ON_COMPONENT_INDEX::brep_edge == edge_index.m_type &&
        edge_index.m_index >= 0 && edge_index.m_index < m_E.Count()) {
...
ON_BrepTrim *ON_Brep::Trim(ON_COMPONENT_INDEX trim_index) const {
    ...
    if (ON_COMPONENT_INDEX::brep_trim == trim_index.m_type &&
        trim_index.m_index >= 0 && trim_index.m_index < m_T.Count()) {
...
ON_BrepLoop *ON_Brep::Loop(ON_COMPONENT_INDEX loop_index) const {
    ...
    if (ON_COMPONENT_INDEX::brep_loop == loop_index.m_type &&
        loop_index.m_index >= 0 && loop_index.m_index < m_L.Count()) {
...
ON_BrepFace *ON_Brep::Face(ON_COMPONENT_INDEX face_index) const {
    ...
    if (ON_COMPONENT_INDEX::brep_face == face_index.m_type &&
        face_index.m_index >= 0 && face_index.m_index < m_F.Count()) {
...
bool ON_BrepTrim::AttachToEdge(int edge_index, bool bRev3d) {
    bool rc = false;
    if (0 != m_brep) {
...
int ON_BrepTrim::TrimCurveIndexOf() const {
    return ((m_brep && m_c2i >= 0 && m_c2i < m_brep->m_C2.Count()) ? m_c2i
                                                                   : -1);
}
...
int ON_BrepLoop::Dimension() const { return 2; }

ON_BOOL32 ON_BrepLoop::GetBBox(double *boxmin, double *boxmax,
...
ON_BOOL32 ON_BrepLoop::Transform(const ON_Xform &) { return false; }
