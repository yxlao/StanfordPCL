...
Adding the concept of Parent and Child dimstyles so that individual dimension
objects can have their own copy of a dimension style to override some settings

  Adding several fields to ON_Dimstyle - This is done with ON_DimStyleExtra
userdata class for now so the SDK doesn't break completely. When the SDK
changes, the data in ON_DimstyleExtra should be moved into ON_Dimstyle.

  Adding the concept of Parent and Child dimstyles to support per object
overrides of dimstyle based properties.  Individual dimensions will be able to
have one or more properties that differ from the dimension style for that
dimension, but the rest of their properties will be controlled by the parent
dimstyle. In this implementation (Rhino 5), dimstyles will only inherit one
level deep.

  The first time an individual dimension has a dimstyle value overridden, a new
child dimstyle is made that is a copy of the dimension's dimstyle. If there is
already a child dimstyle for that dimension, it is used and no new dimstyle is
made. The value being overridden is changed in the child dimstyle and a flag is
set that the field is being overridden.

  When a value is changed in a parent dimstyle, it should look through the other
  dimstyles in the dimstyle table (or your application's equivalent) and change
any of its children appropriately.  Name and Index fields aren't propogated this
way. If the parent's field being changed is not set in the child's
m_valid_fields array, the child's copy of that field should be changed to match
the parent's new value. Changing values in child dimstyles doesn't change values
in their parents.

  When a value that has previously been overridden by an individual dimension
  is set to ByStyle, the corresponding field flag is unset in the valid field
array. If all of the flags in a child dimstyle become unset, the dimension is
set to reference the parent dimstyle directly.
...
class ON_DimStyleExtra : public ON_UserData {
  ...
  public:
    ...
    static ON_DimStyleExtra *DimStyleExtensionGet(ON_DimStyle *pDimStyle,
    ...
    */
    ...
    // 0: Use background color of the viewport.  Initially, gradient backgrounds
    // will not be supported 1: Use the ON_Color returned by MaskColor()
    ...
    double
        m_tolerance_upper_value; // or both upper and lower in symmetrical style
...
// 26 Oct 2010 - Lowell - Changed to always create ON_DimStyleExtra if there's
// not one
ON_DimStyleExtra *
ON_DimStyleExtra::DimStyleExtensionGet(ON_DimStyle *pDimStyle,
    ...
    if (pDimStyle) {
        pExtra = ON_DimStyleExtra::Cast(pDimStyle->GetUserData(
            ON_DimStyleExtra::m_ON_DimStyleExtra_class_id.Uuid()));
        // 2 November 2011 Dale Lear
...
ON_DimStyleExtra::DimStyleExtensionGet(const ON_DimStyle *pDimStyle) {
    ...
    return ON_DimStyleExtra::DimStyleExtensionGet((ON_DimStyle *)pDimStyle,
                                                  false);
}
...
bool ON_DimStyleExtra::IsDefault() const {
    ...
    if (!(m_parent_dimstyle == ON_nil_uuid)) {
        for (int i = 0;
             i < m_valid_fields.Count() && i < ON_DimStyleExtra::eFieldCount;
             i++) {
            if (!m_valid_fields[i])
                return false;
        }
...
ON_BOOL32 ON_DimStyleExtra::Write(ON_BinaryArchive &archive) const {
    //  bool rc = archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK,1,0); Changed
    //  to 1,1 for mask settings 12/12/09 bool rc =
    //  archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK,1,1); Changed to 1,2
    //  for dimscale 12/17/09 bool rc =
    //  archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK,1,2); Changed to 1,3
    //  for source_dimstyle 10/19/10
    ...
    // So now for writing older version files, its multiplied into all distance
    // values
...
int ON_DimStyleExtra::ToleranceStyle() const { return m_tolerance_style; }
...
double ON_DimStyleExtra::BaselineSpacing() const { return m_baseline_spacing; }

bool ON_DimStyleExtra::DrawTextMask() const { return m_bDrawMask; }

void ON_DimStyleExtra::SetDrawTextMask(bool bDraw) {
    m_bDrawMask = bDraw ? true : false;
}

int ON_DimStyleExtra::MaskColorSource() const { return m_mask_color_source; }
...
ON_Color ON_DimStyleExtra::MaskColor() const { return m_mask_color; }

void ON_DimStyleExtra::SetMaskColor(ON_Color color) { m_mask_color = color; }

void ON_DimStyleExtra::SetDimScale(double scale) { m_dimscale = scale; }

double ON_DimStyleExtra::DimScale() const { return m_dimscale; }

void ON_DimStyleExtra::SetDimScaleSource(int source) {
    m_dimscale_source = source;
}

int ON_DimStyleExtra::DimScaleSource() const { return m_dimscale_source; }

void ON_DimStyleExtra::SetSourceDimstyle(ON_UUID source_uuid) {
    m_source_dimstyle = source_uuid;
}

ON_UUID ON_DimStyleExtra::SourceDimstyle() const { return m_source_dimstyle; }
...
ON_OBJECT_IMPLEMENT(ON_DimStyle, ON_Object,
                    "81BD83D5-7120-41c4-9A57-C449336FF12C");

ON_DimStyle::ON_DimStyle() {
    // 26 Oct 2010
    // Can't create this here because reading files won't attach what's read
    // from the file if there's already one there.
...
void ON_DimStyle::GetName(ON_wString &s) const { s = m_dimstyle_name; }
...
void ON_DimStyle::SetIndex(int i) { m_dimstyle_index = i; }

int ON_DimStyle::Index() const { return m_dimstyle_index; }

double ON_DimStyle::ExtExtension() const { return m_extextension; }
...
double ON_DimStyle::ExtOffset() const { return m_extoffset; }

void ON_DimStyle::SetExtOffset(const double e) { m_extoffset = e; }

double ON_DimStyle::ArrowSize() const { return m_arrowsize; }

void ON_DimStyle::SetArrowSize(const double e) { m_arrowsize = e; }

double ON_DimStyle::LeaderArrowSize() const { return m_leaderarrowsize; }

void ON_DimStyle::SetLeaderArrowSize(const double e) { m_leaderarrowsize = e; }

double ON_DimStyle::CenterMark() const { return m_centermark; }

void ON_DimStyle::SetCenterMark(const double e) { m_centermark = e; }

int ON_DimStyle::TextAlignment() const { return m_textalign; }

void ON_DimStyle::SetTextAlignment(ON::eTextDisplayMode a) { m_textalign = a; }

int ON_DimStyle::ArrowType() const { return m_arrowtype; }

void ON_DimStyle::SetArrowType(eArrowType a) { m_arrowtype = a; }

int ON_DimStyle::LeaderArrowType() const { return m_leaderarrowtype; }

void ON_DimStyle::SetLeaderArrowType(eArrowType a) { m_leaderarrowtype = a; }

int ON_DimStyle::AngularUnits() const { return m_angularunits; }

void ON_DimStyle::SetAngularUnits(int u) { m_angularunits = u; }

int ON_DimStyle::LengthFormat() const { return m_lengthformat; }

void ON_DimStyle::SetLengthFormat(int f) { m_lengthformat = f; }

int ON_DimStyle::AngleFormat() const { return m_angleformat; }

void ON_DimStyle::SetAngleFormat(int f) { m_angleformat = f; }

int ON_DimStyle::LengthResolution() const { return m_lengthresolution; }
...
int ON_DimStyle::AngleResolution() const { return m_angleresolution; }
...
int ON_DimStyle::FontIndex() const { return m_fontindex; }

void ON_DimStyle::SetFontIndex(int index) { m_fontindex = index; }

double ON_DimStyle::TextGap() const { return m_textgap; }
...
double ON_DimStyle::TextHeight() const { return m_textheight; }
...
double ON_DimStyle::LengthFactor() const { return m_lengthfactor; }
void ON_DimStyle::SetLengthactor(double factor) { SetLengthFactor(factor); }
...
bool ON_DimStyle::Alternate() const { return m_bAlternate; }
void ON_DimStyle::SetAlternate(bool bAlternate) { m_bAlternate = bAlternate; }
...
void ON_DimStyle::GetPrefix(ON_wString &prefix) const { prefix = m_prefix; }
const wchar_t *ON_DimStyle::Prefix() const { return m_prefix; }
void ON_DimStyle::SetPrefix(wchar_t *prefix) { m_prefix = prefix; }
void ON_DimStyle::SetPrefix(const wchar_t *prefix) { m_prefix = prefix; }

void ON_DimStyle::GetSuffix(ON_wString &suffix) const { suffix = m_suffix; }
const wchar_t *ON_DimStyle::Suffix() const { return m_suffix; }
void ON_DimStyle::SetSuffix(wchar_t *suffix) { m_suffix = suffix; }
void ON_DimStyle::SetSuffix(const wchar_t *suffix) { m_suffix = suffix; }
...
bool ON_DimStyle::SuppressExtension1() const { return m_bSuppressExtension1; }
void ON_DimStyle::SetSuppressExtension1(bool suppress) {
    m_bSuppressExtension1 = suppress;
}
bool ON_DimStyle::SuppressExtension2() const { return m_bSuppressExtension2; }
...
void ON_DimStyle::Composite(const ON_DimStyle &OverRide) {
    /*
      ...
      if( OverRide.IsFieldValid( fn_name))                          {
      ValidateField( fn_name); m_dimstyle_name = OverRide.m_dimstyle_name; } if(
      OverRide.IsFieldValid( fn_index))                         { ValidateField(
      fn_index); m_dimstyle_index = OverRide.m_dimstyle_index; } if(
      OverRide.IsFieldValid( fn_extextension))                  { ValidateField(
      fn_extextension); m_extextension = OverRide.m_extextension; } if(
      OverRide.IsFieldValid( fn_extoffset))                     { ValidateField(
      fn_extoffset); m_extoffset = OverRide.m_extoffset; } if(
      OverRide.IsFieldValid( fn_arrowsize))                     { ValidateField(
      fn_arrowsize); m_arrowsize = OverRide.m_arrowsize; } if(
      OverRide.IsFieldValid( fn_leaderarrowsize))               { ValidateField(
      fn_leaderarrowsize); m_leaderarrowsize = OverRide.m_leaderarrowsize; } if(
      OverRide.IsFieldValid( fn_centermark))                    { ValidateField(
      fn_centermark); m_centermark = OverRide.m_centermark; } if(
      OverRide.IsFieldValid( fn_textgap))                       { ValidateField(
      fn_textgap); m_textgap = OverRide.m_textgap; } if( OverRide.IsFieldValid(
      fn_textheight))                    { ValidateField( fn_textheight);
      m_textheight = OverRide.m_textheight; } if( OverRide.IsFieldValid(
      fn_textalign))                     { ValidateField( fn_textalign);
      m_textalign = OverRide.m_textalign; } if( OverRide.IsFieldValid(
      fn_arrowtype))                     { ValidateField( fn_arrowtype);
      m_arrowtype = OverRide.m_arrowtype; } if( OverRide.IsFieldValid(
      fn_leaderarrowtype))               { ValidateField( fn_leaderarrowtype);
      m_leaderarrowtype = OverRide.m_leaderarrowtype; } if(
      OverRide.IsFieldValid( fn_angularunits))                  { ValidateField(
      fn_angularunits); m_angularunits = OverRide.m_angularunits; } if(
      OverRide.IsFieldValid( fn_lengthformat))                  { ValidateField(
      fn_lengthformat); m_lengthformat = OverRide.m_lengthformat; } if(
      ...
      fn_lengthresolution); m_lengthresolution = OverRide.m_lengthresolution; }
      if( OverRide.IsFieldValid( fn_fontindex))                     {
      ValidateField( fn_fontindex); m_fontindex = OverRide.m_fontindex; } if(
      OverRide.IsFieldValid( fn_lengthfactor))                  { ValidateField(
      fn_lengthfactor); m_lengthfactor = OverRide.m_lengthfactor; } if(
      OverRide.IsFieldValid( fn_bAlternate))                    { ValidateField(
      fn_bAlternate); m_bAlternate = OverRide.m_bAlternate; } if(
      ...
      fn_alternate_lengthformat))        { ValidateField(
      fn_alternate_lengthformat); m_alternate_lengthformat =
      OverRide.m_alternate_lengthformat; } if( OverRide.IsFieldValid(
      fn_alternate_lengthresolution))    { ValidateField(
      fn_alternate_lengthresolution); m_alternate_lengthresolution =
      OverRide.m_alternate_lengthresolution; } if( OverRide.IsFieldValid(
      fn_prefix))                        { ValidateField( fn_prefix); m_prefix =
      OverRide.m_prefix; } if( OverRide.IsFieldValid( fn_suffix)) {
      ValidateField( fn_suffix); m_suffix = OverRide.m_suffix; } if(
      OverRide.IsFieldValid( fn_alternate_prefix))              { ValidateField(
      fn_alternate_prefix); m_alternate_prefix = OverRide.m_alternate_prefix; }
      if( OverRide.IsFieldValid( fn_alternate_suffix))              {
      ValidateField( fn_alternate_suffix); m_alternate_suffix =
      OverRide.m_alternate_suffix; } if( OverRide.IsFieldValid(
      fn_dimextension))                  { ValidateField( fn_dimextension);
      m_dimextension = OverRide.m_dimextension; } if( OverRide.IsFieldValid(
      fn_suppressextension1))            { ValidateField(
      fn_suppressextension1); m_bSuppressExtension1 =
      OverRide.m_bSuppressExtension1; } if( OverRide.IsFieldValid(
      fn_suppressextension2))            { ValidateField(
      fn_suppressextension2); m_bSuppressExtension2 =
      OverRide.m_bSuppressExtension2; }
    */
}

// This function deprecated 5/01/07 LW
void ON_DimStyle::InvalidateField(eField field) { m_valid &= ~(1 << field); }
// This function deprecated 5/01/07 LW
void ON_DimStyle::InvalidateAllFields() { m_valid = 0; }
// This function deprecated 5/01/07 LW
void ON_DimStyle::ValidateField(eField field) { m_valid |= (1 << field); }
// This function deprecated 5/01/07 LW
...
// ver 1.4, Dec 28, 05
double ON_DimStyle::DimExtension() const { return m_dimextension; }
...
void ON_DimStyle::SetFieldOverride(ON_DimStyle::eField field_id,
    ...
    ON_DimStyleExtra *pDE =
        ON_DimStyleExtra::DimStyleExtensionGet(this, bOverride);
    if (pDE) {
...
bool ON_DimStyle::OverrideFields(const ON_DimStyle &src,
    ...
    const ON_DimStyleExtra *pDEsrc =
        ON_DimStyleExtra::DimStyleExtensionGet(&src);
    ...
    const int src_valid_fields_count =
        pDEsrc ? pDEsrc->m_valid_fields.Count() : 0;
        ...
        case fn_alternate_lengthresolution:
            if (!bValidField) {
                SetAlternateLengthResolution(
                    parent.AlternateLengthResolution());
                rc = true;
            }
            break;
        case fn_alternate_angleformat:
...
bool ON_DimStyle::IsChildOf(ON_UUID &parent_uuid) const {
    ...
    const ON_DimStyleExtra *pDE = DimStyleExtension();
    if (pDE && parent_uuid != ON_nil_uuid &&
...
// static
int ON_DimStyle::DefaultToleranceStyle() { return 0; }

// static
int ON_DimStyle::DefaultToleranceResolution() { return 4; }

// static
double ON_DimStyle::DefaultToleranceUpperValue() { return 0.0; }

// static
double ON_DimStyle::DefaultToleranceLowerValue() { return 0.0; }

// static
double ON_DimStyle::DefaultToleranceHeightScale() { return 1.0; }

// static
double ON_DimStyle::DefaultBaselineSpacing() { return 1.0; }

// static
bool ON_DimStyle::DefaultDrawTextMask() { return false; }

// static
int ON_DimStyle::DefaultMaskColorSource() { return 0; }
...
// static
double ON_DimStyle::DefaultDimScale() { return 1.0; }

// static
int ON_DimStyle::DefaultDimScaleSource() { return 0; }
...
void ON_DimStyle::Scale(double scale) {
    if (ON_IsValid(scale) && scale > ON_SQRT_EPSILON && 1.0 != scale) {
        ...
        ON_DimStyleExtra *pDE =
            ON_DimStyleExtra::DimStyleExtensionGet(this, true);
        if (pDE) {
            pDE->Scale(scale);
        }
    }
...
void ON_DimStyle::SetToleranceStyle(int style) {
    bool bCreateIfDoesNotExist =
        (ON_DimStyle::DefaultToleranceStyle() != style);
    ON_DimStyleExtra *pDE =
...
void ON_DimStyle::SetToleranceHeightScale(double scale) {
    bool bCreateIfDoesNotExist =
        (ON_DimStyle::DefaultToleranceHeightScale() != scale);
    ON_DimStyleExtra *pDE =
        ON_DimStyleExtra::DimStyleExtensionGet(this, bCreateIfDoesNotExist);
    if (pDE) {
...
void ON_DimStyle::SetBaselineSpacing(double spacing) {
    bool bCreateIfDoesNotExist =
        (ON_DimStyle::DefaultBaselineSpacing() != spacing);
    ON_DimStyleExtra *pDE =
        ON_DimStyleExtra::DimStyleExtensionGet(this, bCreateIfDoesNotExist);
    if (pDE) {
...
void ON_DimStyle::SetMaskColorSource(int source) {
    bool bCreateIfDoesNotExist =
        (ON_DimStyle::DefaultMaskColorSource() != source);
    ON_DimStyleExtra *pDE =
        ON_DimStyleExtra::DimStyleExtensionGet(this, bCreateIfDoesNotExist);
    if (pDE)
...
void ON_DimStyle::SetDimScaleSource(int source) {
    bool bCreateIfDoesNotExist =
        (ON_DimStyle::DefaultDimScaleSource() != source);
    ON_DimStyleExtra *pDE =
        ON_DimStyleExtra::DimStyleExtensionGet(this, bCreateIfDoesNotExist);
    if (pDE)
...
// This function is temporary and will be removed next time the SDK can be
// modified.
...
bool ON_DimStyle::CompareFields(const ON_DimStyle &other) const {
    ...
    const ON_DimStyleExtra *pDEo =
        ON_DimStyleExtra::DimStyleExtensionGet(&other);
    const ON_DimStyleExtra *pDE = ON_DimStyleExtra::DimStyleExtensionGet(this);
