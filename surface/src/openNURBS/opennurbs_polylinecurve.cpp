...
ON_PolylineCurve::ON_PolylineCurve(const ON_PolylineCurve &L) { *this = L; }
...
int ON_PolylineCurve::Dimension() const { return m_dim; }
    ...
    if (count >= 2 && count == m_t.Count()) {
        ...
        for (i = 1; rc && i < count; i++) {
            if (m_t[i] <= m_t[i - 1]) {
                rc = false;
                if (0 != text_log) {
                    text_log->Print("PolylineCurve m_t[%d]=%g should be less "
                                    "than m_t[%d]=(%g).\n",
                                    i - 1, m_t[i - 1], i, m_t[i]);
                }
            }
        ...
        if (count < 2)
            ...
            text_log->Print("PolylineCurve m_t.Count() = %d and PointCount() = "
                            "%d (should be equal)\n",
                            m_t.Count(), count);
    }
    ...
    if (count >= 1) {
        if (t0 == m_t[0] && t1 == m_t[count])
            ...
            for (int i = 1; i < count; i++) {
                m_t[i] = new_domain.ParameterAt(
                    old_domain.NormalizedParameterAt(m_t[i]));
            }
            rc = true;
        }
    }
...
int ON_PolylineCurve::SpanCount() const { return m_pline.SegmentCount(); }
...
int ON_PolylineCurve::Degree() const { return 1; }
...
ON_PolylineCurve::IsLinear( // true if curve locus is a line segment
    ...
    if (nurbs_curve.m_cv_count >= 2) {
        ...
        nurbs_curve.m_cv_stride =
            (int)(&m_pline[1].x -
                  nurbs_curve.m_cv); // the int converts 64 bit size_t
...
int ON_PolylineCurve::IsPolyline(ON_SimpleArray<ON_3dPoint> *pline_points,
...
bool ON_PolylineCurve::GetNextDiscontinuity(ON::continuity c, double t0,
    ...
    if (segment_count > 0 && t0 != t1) {
        ...
        if (t0 != t1) {
            ...
            if (segment_count >= 2 && parametric_c != ON::C0_continuous) {
                ...
                double s0 = t0;
                double s1 = t1;
                i = ON_NurbsSpanIndex(2, PointCount(), m_t, t0, 0,
                                      (hint) ? *hint : 0);
                double segtol = (fabs(m_t[i]) + fabs(m_t[i + 1]) +
                                 fabs(m_t[i + 1] - m_t[i])) *
                                ON_SQRT_EPSILON;
                if (t0 < t1) {
                    if (t0 < m_t[i + 1] && t1 > m_t[i + 1] &&
                        (m_t[i + 1] - t0) <= segtol && i + 1 < PointCount()) {
                        t0 = m_t[i + 1];
                        i = ON_NurbsSpanIndex(2, PointCount(), m_t, t0, 0,
                                              (hint) ? *hint : 0);
                    }
                    ...
                    if (t0 > m_t[i] && t1 < m_t[i] && (t0 - m_t[i]) <= segtol &&
                        i > 0) {
                        t0 = m_t[i];
                        i = ON_NurbsSpanIndex(2, PointCount(), m_t, t0, 0,
                                              (hint) ? *hint : 0);
                ...
                for (/*empty*/; !rc && 0 < i && i < segment_count &&
                                s0 < m_t[i] && m_t[i] < s1;
                     i += delta_i) {
                    Ev1Der(m_t[i], Pm, D1m, -1, hint);
                    Ev1Der(m_t[i], Pp, D1p, +1, hint);
                    if (parametric_c == ON::C1_continuous ||
                        parametric_c == ON::C2_continuous) {
    ...
    if (segment_count >= 1) {
        ...
        if (t <= m_t[0] || t >= m_t[segment_count]) {
            ...
            if (segment_count >= 2 && desired_continuity != ON::C0_continuous) {
                int i = ON_NurbsSpanIndex(2, PointCount(), m_t, t, 0,
                                          (hint) ? *hint : 0);

                {
                    ...
                    //     domain to real curve domain conversions on the
                    //     interior of a curve domain.
                    double segtol = (fabs(m_t[i]) + fabs(m_t[i + 1]) +
                                     fabs(m_t[i + 1] - m_t[i])) *
                                    ON_SQRT_EPSILON;
                    if (m_t[i] + segtol < m_t[i + 1] - segtol) {
                        if (fabs(t - m_t[i]) <= segtol && i > 0) {
                            t = m_t[i];
                        } else if (fabs(t - m_t[i + 1]) <= segtol &&
                                   i + 1 < PointCount()) {
                            t = m_t[i + 1];
                            i = ON_NurbsSpanIndex(2, PointCount(), m_t, t, 0,
                                                  (hint) ? *hint : 0);
                        }
                    }
                }
                ...
                if (i > 0 && i < segment_count && t == m_t[i]) {
                    // "locus" and "parametric" tests are the same at this
                    // point.
                    desired_continuity =
                        ON::ParametricContinuity(desired_continuity);
                    bPerformTest = true;
                }
            }
        }
...
ON_BOOL32 ON_PolylineCurve::SetStartPoint(ON_3dPoint start_point) {
...
ON_BOOL32 ON_PolylineCurve::SetEndPoint(ON_3dPoint end_point) {
    // 10 March 2009 Dale Lear
    //    I'm using exact compare instead of the fuzzy IsClosed()
    //    check to permit setting the start point.  This fixes
    //    a bug Mikko reported that prevented making polylines
...
ON_PolylineCurve::Evaluate( // returns false if unable to evaluate
    ...
    if (count >= 2) {
        int segment_index =
            ON_NurbsSpanIndex(2, count, m_t, t, side, (hint) ? *hint : 0);
        ...
        if (-2 == side || 2 == side) {
            ...
            if (ON_TuneupEvaluationParameter(side, m_t[segment_index],
                ...
                t = a;
                segment_index =
                    ON_NurbsSpanIndex(2, count, m_t, t, side, segment_index);
            }
        }
        ...
        if (der_count >= 1) {
            ...
            ON_3dVector d =
                1.0 / (t1 - t0) *
                (m_pline[segment_index + 1] - m_pline[segment_index]);
            v[0] = d.x;
            v[1] = d.y;
            if (m_dim == 3)
                v[2] = d.z;
...
bool ON_PolylineCurve::ParameterSearch(double t, int &index,
...
ON_BOOL32 ON_PolylineCurve::Trim(const ON_Interval &domain) {
    ...
    if (segment_count < 1 || m_t.Count() != m_pline.Count() ||
        !domain.IsIncreasing())
        return false;
    ...
    bool bTrimFirstSegment = (m_t[0] < actual_trim_domain[0] ||
                              (0 == s1 && actual_trim_domain[1] < m_t[1]));
    ...
    if (bTrimFirstSegment) {
        ...
        if (0 == s1 && m_t[0] < actual_trim_domain[1] &&
            actual_trim_domain[1] < m_t[1]) {
        ...
        if (bSet0) {
            if (np0 >= 1.0 - ON_SQRT_EPSILON &&
                Q0.DistanceTo(Q1) <= ON_ZERO_TOLERANCE && s1 > 0 &&
                m_t[1] < actual_trim_domain[1]) {
                // trim will leave a micro segment at the start - just remove
                // the first segment
                m_t.Remove(0);
                m_pline.Remove(0);
                s1--;
    ...
    if (bTrimLastSegment) {
        ...
        if (np <= ON_SQRT_EPSILON && Q1.DistanceTo(Q0) <= ON_ZERO_TOLERANCE &&
            s1 > 0) {
            // trim will leave a micro segment at the end - just remove the last
            // segment
...
ON_BOOL32 ON_PolylineCurve::Split(double t, ON_Curve *&left_side,
    ...
    if (count >= 1 && m_t[0] < t && t < m_t[count]) {
        ...
        //   to m_t[0].
        if ((segment_index >= 1 ||
             false == split_at_break && 0 == segment_index) &&
            segment_index < count && m_t[0] < t && t < m_t[count]) {
            int left_point_count =
                (split_at_break) ? segment_index + 1 : segment_index + 2;
...
ON_BOOL32 ON_PolylineCurve::GetCurveParameterFromNurbFormParameter(
    double nurbs_t, double *curve_t) const {
    *curve_t = nurbs_t;
    return true;
}

ON_BOOL32 ON_PolylineCurve::GetNurbFormParameterFromCurveParameter(
    double curve_t, double *nurbs_t) const {
    *nurbs_t = curve_t;
    return true;
}
