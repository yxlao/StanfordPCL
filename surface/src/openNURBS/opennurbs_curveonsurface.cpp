...
ON_CurveOnSurface::IsValid(ON_TextLog *text_log) const {
    ...
    if (m_c3) {
        ...
        if (m_c3->Dimension() != m_s->Dimension()) {
            ON_ERROR("ON_CurveOnSurface::IsValid() m_c3 and m_s have different "
                     "dimensions.");
            return false;
        }
    }
...
void ON_CurveOnSurface::Dump(ON_TextLog &dump) const {
...
int ON_CurveOnSurface::Dimension() const {
...
int ON_CurveOnSurface::Degree() const { return m_c2 ? m_c2->Degree() : 0; }
...
ON_CurveOnSurface::IsArc(  // true if curve locus in an arc or circle
    ...
    return (m_c2 && ON_PlaneSurface::Cast(m_s))
               ? m_c2->IsArc(plane, arc, tolerance)
               : false;
}
...
ON_CurveOnSurface::Evaluate( // returns false if unable to evaluate
    ...
    if (rc) {
        ...
        rc = (m_c2 && m_s) ? m_c2->Evaluate(t, der_count, 3, c[0], side, &chint)
                           : false;
        if (rc) {
            side = 0;
            if (der_count > 0) {
            ...
            if (rc) {
                if (hint) {
                    *hint = (chint & 0xFFFF) | ((shint[0] & 0xFF) << 16) |
                            ((shint[1] & 0xFF) << 24);
                }
                ...
                if (der_count >= 1) {
                    ...
                    if (der_count >= 2) {
                        ...
                        d = ddu * s[1] + ddv * s[2] + du * du * s[3] +
                            2.0 * du * dv * s[4] + dv * dv * s[5];
                        v[0] = d.x;
                        if (dim > 1)
                            v[1] = d.y;
                        if (dim > 2)
                        ...
                        if (der_count >= 3) {
                            ...
                            d = dddu * s[1] + dddv * s[2] +
                                3.0 * du * ddu * s[3] +
                                3.0 * (ddu * dv + du * ddv) * s[4] +
                                3.0 * dv * ddv * s[5] + du * du * du * s[6] +
                                3.0 * du * du * dv * s[7] +
