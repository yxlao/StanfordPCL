...
bool ON_SerialNumberMap::SN_BLOCK::IsValidBlock(
    ON_TextLog *textlog, struct SN_ELEMENT *const *hash_table,
    ...
    for (i = 0; i < m_count; i++) {
        ...
        if (0 == m_sn[i].m_sn_active) {
            ...
            if (0 != m_sn[i].m_id_active) {
                if (textlog)
                    textlog->Print("SN_BLOCK m_sn[%d].m_sn_active = 0 but "
                                   "m_id_active != 0.\n",
                                   i);
                return ON_SerialNumberMap_IsNotValidBlock();
            }
            ...
            if (IdIsNotZero(&m_sn[i].m_id)) {
                const SN_ELEMENT *e;
                for (e = hash_table[IdCRC(&m_sn[i].m_id) % ID_HASH_TABLE_COUNT];
                     0 != e; e = e->m_next) {
                    if (e == &m_sn[i]) {
                ...
                if (0 == e) {
                    // m_sn[i] is not in the hash table but m_id_active
                    // indicates it should be.
                    if (textlog)
                        textlog->Print(
                            "SN_BLOCK m_sn[%d].m_id_active != 0 but the "
                            "element is not in m_hash_table[].\n",
                            i);
                    return ON_SerialNumberMap_IsNotValidBlock();
                }
            } else {
                if (textlog)
                    textlog->Print(
                        "SN_BLOCK m_sn[%d].m_id_active != 0 but m_id = 0.\n",
                        i);
                return ON_SerialNumberMap_IsNotValidBlock();
            }
        }
...
void ON_SerialNumberMap::UpdateMaxSNHelper() {
    m_maxsn = (m_snblk_list_count > 0)
                  ? m_snblk_list[m_snblk_list_count - 1]->m_sn1
                  : 0;
    ...
    if (sn <= m_sn_block0.m_sn1 && m_sn_block0.m_sn0 <= sn) {
        ...
        if (!m_sn_block0.m_sorted) {
            ...
            e = (sn <= m_sn_block0.m_sn1 && m_sn_block0.m_sn0 <= sn)
                    ...
                    : 0;
        } else if (m_sn_block0.m_purged >
                   m_sn_block0.m_count / SN_PURGE_RATIO) {
            InvalidateHashTableHelper();
            m_sn_count -= m_sn_block0.m_purged;
            m_sn_purged -= m_sn_block0.m_purged;
            m_sn_block0.CullBlockHelper();
    ...
    while (i > 0) {
        ...
        if (eblk->m_purged > eblk->m_count / SN_PURGE_RATIO) {
            ...
            if (0 == eblk->m_count) {
                ...
                for (j += ((eblk_array - m_snblk_list) + 1);
                     j < m_snblk_list_count; j++) {
...
size_t ON_SerialNumberMap::ActiveIdCount() const { return m_active_id_count; }
...
ON_SerialNumberMap::FirstElement() const {
    ...
    if (m_sn_block0.m_count > m_sn_block0.m_purged &&
        ...
        if (m_sn_block0.m_purged > 0) {
            ...
            const_cast<ON_SerialNumberMap *>(this)->m_sn_count -=
                m_sn_block0.m_purged;
            const_cast<ON_SerialNumberMap *>(this)->m_sn_purged -=
                m_sn_block0.m_purged;
            const_cast<ON_SerialNumberMap *>(this)
                ->m_sn_block0.CullBlockHelper();
        }
        if (!m_sn_block0.m_sorted) {
...
struct ON_SerialNumberMap::SN_ELEMENT *ON_SerialNumberMap::LastElement() const {
    ...
    if (m_sn_block0.m_count > m_sn_block0.m_purged) {
        ...
        e = const_cast<struct SN_ELEMENT *>(
            &m_sn_block0.m_sn[m_sn_block0.m_count - 1]);
    }
...
ON_SerialNumberMap::FindSerialNumber(unsigned int sn) const {
    struct SN_ELEMENT *e =
        const_cast<ON_SerialNumberMap *>(this)->FindElementHelper(sn);
...
size_t
ON_SerialNumberMap::GetElements(unsigned int sn0, unsigned int sn1,
                                size_t max_count,
    ...
    if (sn0 + 3 <= sn1) {
        ...
        while (sn0 <= sn1) {
            ei = const_cast<ON_SerialNumberMap *>(this)->FindElementHelper(
                sn0++);
    ...
    for (j = 0; j < m_snblk_list_count; j++) {
        ...
        while (ek->m_sn < sn0 || !ek->m_sn_active) {
            ...
            } else if (++j < m_snblk_list_count &&
                       m_snblk_list[j]->m_sn0 <= sn1) {
                k = m_snblk_list[j]->m_count; // always > 0
                ek = &m_snblk_list[j]->m_sn[0];
            } else {
                ek = 0;
    ...
    if (ek) {
        ...
        for (i = j + 1; i < m_snblk_list_count && m_snblk_list[i]->m_sn0 <= sn1;
             i++) {
            c += m_snblk_list[j]->ActiveElementEstimate(ek->m_sn, sn1);
        }
    } else
    ...
    ei = 0;
    if (m_sn_block0.m_count > m_sn_block0.m_purged &&
        sn1 >= m_sn_block0.m_sn0 && sn0 <= m_sn_block0.m_sn1 &&
    ...
    while (ei || ek) {
        if (ei && (!ek || ei->m_sn < ek->m_sn)) {
            ...
            if (ek->m_sn_active)
                elements.Append(
                    *ek); // elements.Append(*ei); // changed by thomas.moerwald
            if (--k) {
                if ((++ek)->m_sn > sn1) {
                    ek = 0;
                }
            } else if (++j < m_snblk_list_count &&
                       sn1 <= m_snblk_list[j]->m_sn0) {
                k = m_snblk_list[j]->m_count; // always > 0
                ek = &m_snblk_list[j]->m_sn[0];
            } else {
                ek = 0;
    ...
    if (m_active_id_count > 0) {
        ...
        if (i != 0 || IdIsNotZero(&id)) {
            if (!m_bHashTableIsValid && sn > 0) {
                ...
                if (e) {
                    if (e->m_id_active &&
                        0 == memcmp(&e->m_id, &id, sizeof(e->m_id))) {
                        e->m_next = 0;
                        e->m_id_active = 0;
                        m_active_id_count--;
                        m_inactive_id = e->m_id;
...
size_t ON_SerialNumberMap::GarbageCollectMoveHelper(
    ON_SerialNumberMap::SN_BLOCK *dst, ON_SerialNumberMap::SN_BLOCK *src) {
    ...
    if (src && dst) {
        ...
        if (n > 0) {
            ...
            memcpy(&dst->m_sn[dst->m_count], &src->m_sn[0],
                   n * sizeof(src->m_sn[0]));
            dst->m_count += n;
            if (dst->m_sorted) {
                dst->m_sn0 =
                    dst->m_sn[0]
                        .m_sn; // set m_sn0 because input dst could have count 0
...
void ON_SerialNumberMap::GarbageCollectHelper() {
        ...
        if (0 == m_snblk_list[i]->m_count) {
            ...
            for (j = i + 1; j < m_snblk_list_count; j++) {
                if (m_snblk_list[j]->m_count > 0) {
                    ...
                    ON_qsort(m_snblk_list + i, m_snblk_list_count - i,
                             sizeof(*m_snblk_list), SN_BLOCK::CompareMaxSN);
                    break;
                }
            }
    ...
    if (m_snblk_list_count > 0 &&
        ...
        struct SN_ELEMENT *snarray = (struct SN_ELEMENT *)onmalloc(
            2 * SN_BLOCK_CAPACITY * sizeof(snarray[0]));
        for (i = 0; i < m_snblk_list_count && m_sn_block0.m_count > 0; i++) {
            ...
            while (j < blk->m_count && k < m_sn_block0.m_count) {
                if (blk->m_sn[j].m_sn < m_sn_block0.m_sn[k].m_sn) {
                    ...
                    // If m_sn_block0.m_sn[m_sn_block0.m_count-1].m_sn is the
                    // largest value, then we should get j == blk->m_count exit.
            ...
            n = blk->m_count - j;
            if (n > 0) {
                memcpy(&snarray[snarray_count], &blk->m_sn[j],
                       n * sizeof(snarray[0]));
                snarray_count += n;
            } else {
                while (k < m_sn_block0.m_count &&
                       m_sn_block0.m_sn[k].m_sn < sn1)
                    snarray[snarray_count++] = m_sn_block0.m_sn[k++];
            }
            n = (snarray_count > SN_BLOCK_CAPACITY) ? SN_BLOCK_CAPACITY
                                                    : snarray_count;
            ...
            if (snarray_count > n) {
                ...
                m_sn_block0.m_sn1 =
                    m_sn_block0.m_sn[m_sn_block0.m_count - 1].m_sn;
            } else {
                m_sn_block0.EmptyBlock();
            }
        }
    ...
    if (m_sn_block0.m_count > SN_BLOCK_CAPACITY / 4) {
        if (m_snblk_list_count > 0) {
            GarbageCollectMoveHelper(m_snblk_list[m_snblk_list_count - 1],
                                     &m_sn_block0);
        }
        if (m_sn_block0.m_count > SN_BLOCK_CAPACITY / 2) {
            ...
            if (0 == m_snblk_list[m_snblk_list_count]) {
                ...
                m_snblk_list[m_snblk_list_count] = (SN_BLOCK *)onmalloc(
                    sizeof(*(m_snblk_list[m_snblk_list_count])));
            }
    ...
    if (0 != e && 0 == e->m_id_active) {
        if (IdIsNotZero(&id)) {
            if (m_active_id_count > 0 &&
                ...
                BuildHashTableHelper();
                i = HashIndex(&id);
                for (const struct SN_ELEMENT *h = m_hash_table[i]; h;
                     h = h->m_next) {
...
void ON_SerialNumberMap::AddBlockToHashTableHelper(
    struct ON_SerialNumberMap::SN_BLOCK *blk) {
    if (m_bHashTableIsValid) {
        SN_ELEMENT *e;
        size_t i, hash_i;
        for (e = blk->m_sn, i = blk->m_count; i--; e++) {
