...
double *ON_Matrix::operator[](int i) { return m[i]; }

const double *ON_Matrix::operator[](int i) const { return m[i]; }

ON_Matrix::ON_Matrix()
    : m(0), m_row_count(0), m_col_count(0), m_Mmem(0), m_row_offset(0),
      m_col_offset(0), m_cmem(0) {}
...
ON_Matrix::ON_Matrix(int row_count, int col_count, double **M,
...
int ON_Matrix::RowCount() const { return m_row_count; }

int ON_Matrix::ColCount() const { return m_col_count; }
    ...
    if (row_count > 0 && col_count > 0) {
        ...
        if (0 != m_rowmem.Array()) {
            ...
            if (rows_per_block > row_count)
                ...
                rows_per_block = 1;
            else if (rows_per_block < row_count &&
                     11 * rows_per_block >= 10 * row_count)
                rows_per_block = row_count;
            ...
            for (i = row_count; i > 0; i -= rows_per_block) {
                ...
                struct DBLBLK *p = (struct DBLBLK *)onmalloc(
                    sizeof(*p) + dblblk_count * sizeof(p->a[0]));
                p->a = (double *)(p + 1);
...
bool ON_Matrix::Create( // E.g., Create(1,5,1,7) creates a 5x7 sized matrix that
    ...
    if (ri1 > ri0 && ci1 > ci0) {
        ...
        if (b) {
            m_row_offset =
                ri0; // this is the only line of code where m_row_offset should
                     // be set to a non-zero value
            m_col_offset =
                ci0; // this is the only line of code where m_col_offset should
                     // be set to a non-zero value
            if (ci0 != 0) {
                int i;
...
bool ON_Matrix::Create(int row_count, int col_count, double **M,
                       bool bDestructorFreeM) {
    Destroy();
...
ON_Matrix &ON_Matrix::operator=(const ON_Matrix &src) {
    if (this != &src) {
        if (src.m_row_count != m_row_count || src.m_col_count != m_col_count ||
            0 == m) {
            Destroy();
...
bool ON_Matrix::SwapRows(int row0, int row1) {
    ...
    row0 -= m_row_offset;
    row1 -= m_row_offset;
    if (this_m && 0 <= row0 && row0 < m_row_count && 0 <= row1 &&
        row1 < m_row_count) {
        if (row0 != row1) {
...
bool ON_Matrix::SwapCols(int col0, int col1) {
    ...
    col0 -= m_col_offset;
    col1 -= m_col_offset;
    if (this_m && 0 <= col0 && col0 < m_col_count && 0 <= col1 &&
        col1 < m_col_count) {
        if (col0 != col1) {
            for (i = 0; i < m_row_count; i++) {
                t = this_m[i][col0];
...
int ON_Matrix::RowReduce(double zero_tolerance, double &determinant,
                         double &pivot) {
    double x, piv, det;
    int i, k, ix, rank;
    ...
    for (k = 0; k < n; k++) {
        ...
        x = 1.0 / this_m[k][k];
        this_m[k][k] = 1.0;
        ON_ArrayScale(m_col_count - 1 - k, x, &this_m[k][k + 1],
                      &this_m[k][k + 1]);
...
int ON_Matrix::RowReduce(double zero_tolerance, double *B, double *pivot) {
    double t;
    double x, piv;
    int i, k, ix, rank;
...
int ON_Matrix::RowReduce(double zero_tolerance, ON_3dPoint *B, double *pivot) {
    ON_3dPoint t;
    double x, piv;
    int i, k, ix, rank;
...
int ON_Matrix::RowReduce(double zero_tolerance, int pt_dim, int pt_stride,
                         double *pt, double *pivot) {
    ...
    const int n = m_row_count <= m_col_count ? m_row_count : m_col_count;
    for (k = 0; k < n; k++) {
        //    onfree( onmalloc( 1)); //  8-06-03 lw for cancel thread
        //    responsiveness
        onmalloc(0); // 9-4-03 lw changed to 0
        ix = k;
        x = fabs(this_m[ix][k]);
...
bool ON_Matrix::BackSolve(double zero_tolerance, int Bsize, const double *B,
                          double *X) const {
    int i;
...
bool ON_Matrix::BackSolve(double zero_tolerance, int Bsize, const ON_3dPoint *B,
                          ON_3dPoint *X) const {
    int i, j;
...
bool ON_Matrix::BackSolve(double zero_tolerance, int pt_dim, int Bsize,
                          int Bpt_stride, const double *Bpt, int Xpt_stride,
                          double *Xpt) const {
...
bool ON_Matrix::IsRowOrthoganal() const {
    ...
    for (i0 = 0; i0 < m_row_count && rc; i0++)
        for (i1 = i0 + 1; i1 < m_row_count && rc; i1++) {
            ...
            if (d0 <= ON_EPSILON || d1 <= ON_EPSILON ||
                fabs(d) >= d0 * d1 * ON_SQRT_EPSILON)
                rc = false;
        }
    return rc;
}
...
bool ON_Matrix::Invert(double zero_tolerance) {
    ...
    for (k = 0; k < n; k++) {
        ...
        x = 1.0 / this_m[k][k];
        this_m[k][k] = 1.0;
        ON_ArrayScale(m_col_count - k - 1, x, &this_m[k][k + 1],
                      &this_m[k][k + 1]);
        I.RowScale(k, x);
        ...
        for (i = 0; i < n; i++) {
            if (i != k) {
                ...
                this_m[i][k] = 0.0;
                if (fabs(x) > zero_tolerance) {
                    ON_Array_aA_plus_B(m_col_count - k - 1, x,
                                       &this_m[k][k + 1], &this_m[i][k + 1],
                                       &this_m[i][k + 1]);
                    I.RowOp(i, x, k);
                }
            }
...
int ON_RowReduce(int row_count, int col_count, double zero_pivot, double **A,
                 double **B, double pivots[2]) {
...
bool ON_SolveSVD(int row_count, int col_count, double const *const *U,
                 const double *invW, double const *const *V, const double *B,
                 double *&X) {
    int i, j;
    double *Y;
    const double *p0;
    ...
    if (row_count < 1 || col_count < 1 || 0 == U || 0 == invW || 0 == V ||
        0 == B)
        return false;

    if (0 == X)
        X = (double *)onmalloc(col_count * sizeof(X[0]));
    Y = (col_count > 128) ? ((double *)onmalloc(col_count * sizeof(*Y)))
                          : workY;
