...
#define LOGI(...)                                                              \
    ((void)__android_log_print(ANDROID_LOG_INFO, "PointCloudStreaming",        \
                               __VA_ARGS__))
#define LOGW(...)                                                              \
    ((void)__android_log_print(ANDROID_LOG_WARN, "PointCloudStreaming",        \
                               __VA_ARGS__))
#define LOGE(...)                                                              \
    ((void)__android_log_print(ANDROID_LOG_WARN, "PointCloudStreaming",        \
                               __VA_ARGS__))
...
class vesKiwiPointCloudApp : public vesKiwiViewerApp {
  public:
    ...
    ~vesKiwiPointCloudApp() { this->unloadData(); }
    ...
    void initShader(const std::string &vertexSource,
    ...
    bool connect(const std::string &host, int port) {
        ...
        if (!mDataRep) {
            mDataRep = vesKiwiStreamingDataRepresentation::Ptr(
                new vesKiwiStreamingDataRepresentation);
        }
...
std::string copyAssetToExternalStorage(std::string filename) {
    ...
    AAsset *asset =
        AAssetManager_open(assetManager, filename.c_str(), AASSET_MODE_UNKNOWN);
    if (asset == NULL) {
...
static int engine_init_display(struct engine *engine) {
    ...
    vesKiwiPointCloudApp::Ptr kiwiApp =
        vesKiwiPointCloudApp::Ptr(new vesKiwiPointCloudApp);
    kiwiApp->initGL();
    ...
    // initialize shaders
    AAsset *vertex_asset = AAssetManager_open(
        assetManager, "vesShader_vert.glsl", AASSET_MODE_UNKNOWN);
    AAsset *fragment_asset = AAssetManager_open(
        assetManager, "vesShader_frag.glsl", AASSET_MODE_UNKNOWN);

    std::string vertex_source =
        std::string(static_cast<const char *>(AAsset_getBuffer(vertex_asset)),
                    AAsset_getLength(vertex_asset));
    std::string fragment_source =
        std::string(static_cast<const char *>(AAsset_getBuffer(fragment_asset)),
                    AAsset_getLength(fragment_asset));
...
static int32_t engine_handle_input(struct android_app *app,
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        // When transitioning from one touch to two touches, we record the
        // position of of the new touch and return.  When transitioning from two
        // touches to one touch, we record the position of the single remaining
        // touch and return.
        ...
        if (AMotionEvent_getPointerCount(event) == 1) {
            ...
            double previousDist =
                sqrt((px0 - px1) * (px0 - px1) + (py0 - py1) * (py0 - py1));
            double currentDist =
            ...
            double newAngle = atan2(y0 - y1, x0 - x1);

            double oldAngle = atan2(py0 - py1, px0 - px1);
