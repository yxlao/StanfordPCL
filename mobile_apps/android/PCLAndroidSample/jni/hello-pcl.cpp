...
#define LOGI(...)                                                              \
    ((void)__android_log_print(ANDROID_LOG_INFO, "hello-ndk", __VA_ARGS__))
...
Java_org_pointclouds_PCLAndroidSample_HelloPCL_boostMkDir(JNIEnv *env,
                                                          jobject) {
    ...
    return env->NewStringUTF(
        "caca masii de pe noul PCL Android care merge bine");
}

JNIEXPORT jstring JNICALL
Java_org_pointclouds_PCLAndroidSample_HelloPCL_smoothPointCloud(JNIEnv *env,
                                                                jobject) {}