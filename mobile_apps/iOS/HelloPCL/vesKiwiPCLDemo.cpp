...
//----------------------------------------------------------------------------
vesKiwiPCLDemo::vesKiwiPCLDemo() { this->Internal = new vesInternal(); }

//----------------------------------------------------------------------------
vesKiwiPCLDemo::~vesKiwiPCLDemo() { delete this->Internal; }
...
void vesKiwiPCLDemo::setLeafSize(double value) {
    if (this->Internal->LeafSize != value) {
        ...
        this->Internal->PolyDataRep->mapper()->setGeometryData(
...
//----------------------------------------------------------------------------
void vesKiwiPCLDemo::initialize(const std::string &filename,
    ...
    this->Internal->PolyDataRep =
        vesKiwiPolyDataRepresentation::Ptr(new vesKiwiPolyDataRepresentation);
    this->Internal->PolyDataRep->initializeWithShader(
        this->Internal->GeometryShader);
    this->Internal->PolyDataRep->mapper()->setGeometryData(
        this->updateGeometryData());
}
...
void ConvertVertexArrays(vtkDataSet *dataSet,
                         vesSharedPtr<vesGeometryData> geometryData,
                         vtkScalarsToColors *scalarsToColors = NULL) {
    vtkUnsignedCharArray *colors =
        vesKiwiDataConversionTools::FindRGBColorsArray(dataSet);
    vtkDataArray *scalars =
        vesKiwiDataConversionTools::FindScalarsArray(dataSet);
    vtkDataArray *tcoords =
        vesKiwiDataConversionTools::FindTextureCoordinatesArray(dataSet);
    if (colors) {
        ...
        if (!colorMap) {
            colorMap = vesKiwiDataConversionTools::GetRedToBlueLookupTable(
                scalars->GetRange());
        }
        vesKiwiDataConversionTools::SetVertexColors(scalars, colorMap,
                                                    geometryData);
    } else if (tcoords) {
        vesKiwiDataConversionTools::SetTextureCoordinates(tcoords,
                                                          geometryData);
    }
}
...
vesGeometryData::Ptr vesKiwiPCLDemo::updateGeometryData() {
    ...
    if (this->Internal->LeafSize != 0.0) {
        vtkSmartPointer<vtkPCLVoxelGrid> voxelGrid =
            vtkSmartPointer<vtkPCLVoxelGrid>::New();
        voxelGrid->SetInputData(this->Internal->PolyData);
        voxelGrid->SetLeafSize(this->Internal->LeafSize,
                               this->Internal->LeafSize,
                               this->Internal->LeafSize);
        voxelGrid->Update();
    ...
    if (this->Internal->PlaneDistanceThreshold != 0.0) {
        vtkSmartPointer<vtkPCLSACSegmentationPlane> fitPlane =
            vtkSmartPointer<vtkPCLSACSegmentationPlane>::New();
    ...
    vesGeometryData::Ptr geometryData =
        vesKiwiDataConversionTools::ConvertPoints(polyData);
...
//----------------------------------------------------------------------------
bool vesKiwiPCLDemo::handleSingleTouchUp() { return false; }
...
//----------------------------------------------------------------------------
void vesKiwiPCLDemo::removeSelfFromRenderer(
