 ...
 *
 * This demo loads a stereo image pair and computes the disparity map and
 * related organized point cloud using the PCL AdaptiveCost-2SO Stereo Matching
 * algorithm. Input pcds should be "stereo_left.pcd" and "stereo_right.pcd"
 * which can be found in the test subfolder within trunk. A rescaled version of
 * the disparity map is displayed, as well as the point cloud.
 *
...
int main(int argc, char **argv) {
    ...
    if (!left_cloud->isOrganized() || !right_cloud->isOrganized() ||
        ...
        std::cout << "Wrong stereo pair; please check input pcds .."
                  << std::endl;
        return 0;
    }
    ...
    stereo.getPointCloud(318.112200, 224.334900, 368.534700, 0.8387445,
                         out_cloud, left_cloud);
