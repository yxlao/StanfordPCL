...
void write_score_image(const float *score_buffer) {
    ...
    for (int y = 0; y < 480; ++y) {
        for (int x = 0; x < 640; ++x) {
            ...
            float d =
                (score_buffer[i_in] - min_score) / (max_score - min_score);
            score_img[3 * i + 0] = 0;
...
void capture(Eigen::Isometry3d pose_in, string point_cloud_fname) {
    ...
    // Benchmark Values for sim_terminal_demo ( march 27 2012): >>> used
    // PolygonMeshModel<< 27840 triangle faces 13670 vertices
...
int main(int argc, char **argv) {
    ...
    if (mode == 0) {
        ...
        m = AngleAxisd(-9.14989, Vector3d::UnitZ()) *
            AngleAxisd(0.20944, Vector3d::UnitY()) *
        ...
        for (double i = 0; i <= 1; i += 1 / ((double)n_poses - 1)) {
            ...
            Eigen::Isometry3d pose;
            pose.setIdentity();
            Eigen::Vector3d trans3 =
                (1 - i) * pose1.translation() + i * pose2.translation();
    ...
    cout << poses.size() << " poses simulated in " << (getTime() - tic_main)
         << "seconds\n";
