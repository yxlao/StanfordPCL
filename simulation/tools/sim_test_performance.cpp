...
void display() {
    ...
    for (int i = 0, n = 0; i < range_likelihood_->getRowHeight(); ++i) {
        for (int j = 0; j < range_likelihood_->getColWidth(); ++j) {
            reference[n++] =
                depth_buffer[(i + range_likelihood_->getRowHeight() *
                                      range_likelihood_->getRows() / 2) *
                                 range_likelihood_->getWidth() +
                             j +
                             range_likelihood_->getColWidth() *
                                 range_likelihood_->getCols() / 2];
        }
    }

    float *reference_vis =
        new float[range_likelihood_visualization_->getRowHeight() *
                  range_likelihood_visualization_->getColWidth()];
    const float *depth_buffer_vis =
        range_likelihood_visualization_->getDepthBuffer();
    // Copy one image from our last as a reference.
    for (int i = 0, n = 0; i < range_likelihood_visualization_->getRowHeight();
         ++i) {
        for (int j = 0; j < range_likelihood_visualization_->getColWidth();
             ++j) {
            reference_vis[n++] = depth_buffer_vis
                [i * range_likelihood_visualization_->getWidth() + j];
        }
    }
    ...
    range_likelihood_visualization_->computeLikelihoods(reference_vis, poses,
                                                        scores);
    ...
    glDrawPixels(range_likelihood_visualization_->getWidth(),
                 range_likelihood_visualization_->getHeight(), GL_RGB,
                 GL_UNSIGNED_BYTE,
    ...
    if (gllib::getGLError() != GL_NO_ERROR) {
        std::cerr << "GL Error: RangeLikelihood::computeLikelihoods: finished"
                  << std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    print_info("Range likelihood performance tests using pcl::simulation. For "
    ...
    textured_quad_ = TexturedQuad::Ptr(new TexturedQuad(
        range_likelihood_->getWidth(), range_likelihood_->getHeight()));
