...
void printHelp(int argc, char **argv) {
    ...
    print_info("acceptable filenames include vtk, obj and ply. ply can support "
               "colour\n");
}
...
void display_score_image(const float *score_buffer) {
    ...
    glDrawPixels(range_likelihood_->getWidth(), range_likelihood_->getHeight(),
                 GL_RGB, GL_UNSIGNED_BYTE, score_img);
...
void display() {
    ...
    glViewport(0, 0, range_likelihood_->getWidth(),
               range_likelihood_->getHeight());
    ...
    if (write_file_) {
        ...
        // vtk seems to misbehave after a little while and wont keep the window
        // on the screen
        ...
        // method2: eventually starts ignoring cin and pops up on screen and
        // closes almost immediately
        //  cout << "enter 1 to cont\n";
        //  cin >> pause;
        //  viewer.wasStopped ();

        // method 3: if you interact with the window with keys, the window is
        // not closed properly
        // TODO: use pcl methods as this time stuff is probably not cross
        // playform
        //     struct timespec t;
        //     t.tv_sec = 100;
...
void on_passive_motion(int x, int y) {
    ...
    double pitch = -(0.5 - (double)y / window_height_) * M_PI *
                   4; // in window coordinates positive y-axis is down
...
int main(int argc, char **argv) {
    ...
    // range_likelihood_ = RangeLikelihoodGLSL::Ptr(new RangeLikelihoodGLSL(1,
    // 1, height, width, scene_, 0));

    range_likelihood_ = RangeLikelihood::Ptr(
        new RangeLikelihood(2, 2, height / 2, width / 2, scene_));
