...
class PCL_EXPORTS RangeLikelihood {
  ...
  public:
     ...
     * @param scores is an output argument. The resulting log likelihoods will
     * be written in score.
     *
     */
    void
    ...
    void setCameraIntrinsicsParameters(int camera_width_in,
                                       int camera_height_in, float camera_fx_in,
                                       float camera_fy_in, float camera_cx_in,
                                       float camera_cy_in) {
    ...
    // Two modes:
    // global=false - PointCloud is as would be captured by an RGB-D camera
    // [default] global=true  - PointCloud is transformed into the model/world
    // frame using the camera pose
    ...
    void setComputeOnCPU(bool compute_on_cpu) {
        compute_likelihood_on_cpu_ = compute_on_cpu;
    }

    void setSumOnCPU(bool sum_on_cpu) { aggregate_on_cpu_ = sum_on_cpu; }
  ...
  private:
     ...
     */
    void computeScores(float *reference, std::vector<float> &scores);

    void computeScoresShader(float *reference);

    void render(
        const std::vector<Eigen::Isometry3d,
                          Eigen::aligned_allocator<Eigen::Isometry3d>> &poses);

    void drawParticles(std::vector<Eigen::Isometry3d,
                                   Eigen::aligned_allocator<Eigen::Isometry3d>>
                           poses);

    void applyCameraTransform(const Eigen::Isometry3d &pose);
