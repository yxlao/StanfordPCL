...
class PCL_EXPORTS Camera {
  public:
    ...
    void setParameters(int width, int height, float fx, float fy, float cx,
                       float cy, float z_near, float z_far);

    Eigen::Matrix4f getProjectionMatrix() { return projection_matrix_; }
    ...
    void set(double x, double y, double z, double roll, double pitch,
             double yaw) {
        x_ = x;
        y_ = y;
        z_ = z;
    ...
    Eigen::Vector3d getYPR() { return Eigen::Vector3d(yaw_, pitch_, roll_); }
