...
#define cutilBankChecker(array, idx)                                           \
    (__cutilBankChecker(threadIdx.x, threadIdx.y, threadIdx.z, blockDim.x,     \
                        blockDim.y, blockDim.z, #array, idx, __FILE__,         \
                        __LINE__),                                             \
     array[idx])
...
inline void __cutilBankChecker(unsigned int tidx, unsigned int tidy,
                               unsigned int tidz, unsigned int bdimx,
                               unsigned int bdimy, unsigned int bdimz,
                               char *aname, int index, char *file, int line) {
    cutCheckBankAccess(tidx, tidy, tidz, bdimx, bdimy, bdimz, file, line, aname,
                       index);
}
