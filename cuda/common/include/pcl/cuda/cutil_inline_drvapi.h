...
inline int cutilDrvGetMaxGflopsGraphicsDeviceId() {
    ...
    while (current_device < device_count) {
        ...
        cutilDrvSafeCallNoSync(cuDeviceGetAttribute(
            &bTCC, CU_DEVICE_ATTRIBUTE_TCC_DRIVER, current_device));
#else
    ...
    while (current_device < device_count) {
        ...
        // If this is a Tesla based GPU and SM 2.0, and TCC is disabled, this is
        // a contendor
...
inline void __cuCheckMsg(const char *msg, const char *file, const int line) {
    ...
    if (CUDA_SUCCESS != err) {
        ...
        fprintf(stderr,
                "cutilDrvCheckMsg -> cuCtxSynchronize API error = %04d in file "
                "<%s>, line %i.\n",
                err, file, line);
        exit(-1);
    }
...
inline bool cutilDrvCudaDevCapabilities(int major_version, int minor_version,
                                        int deviceNum, int argc, char **argv) {
    int major, minor, dev;
...
inline bool cutilDrvCudaCapabilities(int major_version, int minor_version,
                                     int argc, char **argv) {
    return cutilDrvCudaDevCapabilities(major_version, minor_version, 0, argc,
                                       argv);
}
