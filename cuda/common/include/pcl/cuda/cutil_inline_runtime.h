...
#define cutilCheckMsgAndSync(msg)                                              \
    __cutilGetLastErrorAndSync(msg, __FILE__, __LINE__)
...
// Give a little more for Windows : the console window often disapears before we
...
inline void VSPrintf(FILE *file, LPCSTR fmt, ...) {
    ...
    va_list vlist;
    va_start(vlist, fmt);
    while ((_vsnprintf(fmt2, fmt2_sz, fmt, vlist)) <
           0) // means there wasn't anough room
    {
        fmt2_sz *= 2;
        if (fmt2)
            free(fmt2);
...
// NOTE: "%s(%i) : " allows Visual Studio to directly jump to the file at the
// right line when the user double clicks on the error line in the Output pane.
// Like any compile error.
...
inline void __cufftSafeCall(cufftResult err, const char *file, const int line) {
    if (CUFFT_SUCCESS != err) {
        FPRINTF(
            (stderr, "%s(%i) : cufftSafeCall() CUFFT error.\n", file, line));
        exit(-1);
    }
}
...
inline void __cutilGetLastError(const char *errorMessage, const char *file,
                                const int line) {
...
inline void __cutilGetLastErrorAndSync(const char *errorMessage,
...
inline void cutilCudaCheckCtxLost(const char *errorMessage, const char *file,
                                  const int line) {
    cudaError_t err = cudaGetLastError();
    if (cudaSuccess != err) {
...
inline void __cutilQAFinish(int argc, char **argv, bool bStatus) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!STRCASECMP(argv[i], "-qatest") ||
            !STRCASECMP(argv[i], "-noprompt")) {
            bFlag |= true;
        }
    }
